...
Phase 1 진행 시 주의사항
① pp.365–367 보완 시도 금지
Phase 1 실행 모델이 §4.5(Minimization)·§4.6(Weighting) 내용을 일반 지식으로 채워 넣으려 할 수 있다. 해당 구간이 언급되어야 할 때는 반드시 [확인 필요 — pp.365–367 업로드 부재]로 플래그 처리하고 진행.
② §5.3.3을 MUST 카드로 격상 금지
독성동태 설계 내용(pp.412–414)은 핵심 서사와 결합도가 낮다. Curation Map 작성 시 CONTEXT 처리 확인 후, §2 카드 신설 없이 관련 MUST 카드 본문 1–2문장으로만 통합.
③ r 값 오용 함정을 §5 혼동쌍으로 반드시 배치
p.382의 "r=0.96 OLS가 r=0.94 IRLS보다 나쁜 모델" 사례는 교과서가 직접 강조하는 핵심 함정이다. §5 혼동쌍 중 하나로 배치하고 Critical Blow 행을 F 검정 오용 사례에 지정.
