...
**usv의 물리적 의미**: mg(투여 단위)와 µg(농도 단위)의 1000배 불일치를 보정하는 무차원 스케일 인자. $Sn = V \times usv$에서 usv=1/1000. `[Owen Eq.3.6, p.41]`
...
```
---
✦ STEP 1 완료 (V3.0 — 최종 교정판).

다음이 준비되었습니다:
  • §1 세션 헤더 & 로드맵
  • §2 개념 해부 카드 (5개, Apex: 특수 ADVAN+TRANS 조합)
    - 카드 1: 레코드 구조 + IGNORE AND/OR 꼼수 + PIPET vs Owen 불일치 경고
    - 카드 2 Apex: S-parameter 전환 함정 교정 (50,000배→V2/1000배=20배)
               + PIPET Table 4.5 ADVAN3 CMT 오기 경고
    - 카드 3: $ERROR + Log-error 3가지 방어 + Assay/Phase-specific
    - 카드 4: $ESTIMATION + $COVARIANCE + $TABLE + $SIMULATION 통합 (신규)
               + $MSFI/$MSFO 패턴 (신규)
    - 카드 5: $PRED (QTc 코드 3.8 포함)
  • §5 혼동 쌍 해부 (4개, Critical Blow: ADVAN3 vs ADVAN4)
  • §7 자기 테스트 (10개 질문, 보스 딜레마 포함)
  • §8 메타프레임 (실패 모드 4개로 확장)
  • 감지된 소스 유형: Vol II — Owen (Ch.3, pp.28-64) + PIPET (Ch.3-4-7, pp.27-71) 혼합
  • 교정 항목:
    ① Plausible Fallacy 수치 오류 수정: 50,000배 → V2/1000배 (V2=50L 기준 20배)
    ② PIPET Table 4.5 ADVAN3 CMT 오기 플래그 및 Owen 기준 교정표 제공
    ③ PIPET vs Owen IGNORE AND/OR 불일치 명시
    ④ $TABLE + $SIMULATION 레코드 카드 4에 통합
    ⑤ $MSFI/$MSFO 패턴 추가
---
```

"계속"을 입력하면 Step 2 HTML 컴파일러가 실행됩니다. Step 2는 위 내용 전체(§1·§2·§5·§7·§8) + §3(구조적 동형성 지도) + §4(인터랙티브 시뮬레이터) + §6(실용 시나리오 & 진단 병리)를 단일 완성 HTML 파일로 통합합니다.
