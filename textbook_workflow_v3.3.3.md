# 교과서 체화 HTML 워크플로우 · Unified v3.3.3
...
└──────────────────────┬──────────────────────────────┘
                       ↓
┌─────────────────────────────────────────────────────┐
│  PHASE 2 · Source Fidelity Audit (+ T6 Figure Inv. + T7 Page Tag) │
...
╚══════════════════════╤══════════════════════════════╝
                       ↓
┌─────────────────────────────────────────────────────┐
│  PHASE 5 · HTML Compile (+ Figure + Page Tag Render) │
│  도구: 교과서 html 작업자                             │
│  산출: Draft HTML v1                                 │
└──────────────────────┬──────────────────────────────┘
                       ↓  (A급만)
┌─────────────────────────────────────────────────────┐
│  PHASE 6 · HTML QA (+ Figure QA + Splice + Page Tag) │
│  도구: Claude / GPT-5.5                              │
│  산출: Final HTML                                    │
└─────────────────────────────────────────────────────┘
```
...
> 🔖 **Page-level Traceability도 모든 모드에서 유지한다. (v3.3.3 신설)**
> A/B급은 PROMPT 2 T7 (전체 page tag 검증), C급은 PROMPT 2-lite L6 (spot-check).
> 모든 §2 MUST card·독립 수식 블록·핵심 예시 블록은 source page tag를 의무적으로 갖는다.
> Phase 3을 생략하는 C-Fast-Safe 모드에서는 page tag가 사실상 *유일한* traceability 백스톱이 된다.
...
## Source Page Tag 운영 규약 (v3.3.3 신설 — 전체 워크플로우 공통)

본문 traceability를 위한 page tag는 **figure source 표기와 별개**로 운영된다.

| 형식 | 용도 | 사용 예 |
|------|------|--------|
| `[p.XX]` | 단일 페이지 출처 | `[p.123]` |
| `[pp.XX–YY]` | 연속 페이지 출처 | `[pp.45–47]` |
| `[pp.XX, YY]` | 비연속 다중 페이지 출처 | `[pp.45, 89]` |
| `[p.확인 필요]` | 페이지 확인 불가 — Phase 2 audit 검증 대상 | `[p.확인 필요]` |

**부착 위치 (3곳만):**
- §2 MUST Concept Anatomy Card 제목 끝
- 독립 수식 블록의 caption/label 끝
- 핵심 worked example/dataset 블록 헤더 끝

**부착하지 않는 위치:**
- 모든 문장 (시각적 혼잡 유발)
- CONTEXT-tier 1–2문장 inline 설명 (출처는 인접 MUST card에 귀속)
- §1 도입부, §5 confusion pair, §7 self-test, §8 recap (구조 요소)
- Trench-level tip, Master's Lens callout (저자 메시지 ≠ 페이지 인용)

**핵심 원칙:** "검증된 요약"에서 그치지 않고, 최종 HTML 한 파일만 열어도 임의 개념을 교과서 페이지로 즉시 역추적할 수 있어야 한다.
...
- [p.확인 필요] = page tag flag; retain through Phase 4, resolve in Phase 2 if possible.
...
8. SOURCE TAGGING — page-level traceability (v3.3.3 신설):

   Every §2 MUST Concept Anatomy Card, independent equation block, and key worked
   example/dataset block MUST include a source page tag.

   Format:
   - Single-page source        : [p.XX]
   - Multi-page contiguous     : [pp.XX–YY]
   - Multiple non-contiguous   : [pp.XX, YY]
   - Page uncertain             : [p.확인 필요]   ← Phase 2 T7 audit will verify or correct

   Placement (only at these 3 locations):
   - MUST card title end           : "Concept Anatomy: Hepatic Clearance [p.123]"
   - Equation block caption end    : "Eq. 4-7 — Well-Stirred Model [pp.124–125]"
   - Key example/dataset heading   : "Worked Example: IV bolus dosing [p.130]"

   Do NOT attach page tags to:
   - Every sentence (creates visual clutter; tag is card-scope, not sentence-scope).
   - CONTEXT-tier 1–2 sentence references inside MUST cards
     (the parent card's tag covers them).
   - §1 introduction, §5 confusion pair, §7 self-test, §8 recap blocks.
   - Master's Lens callouts and Trench-level tips
     (these are author-message synthesis, not direct page citations).

   Do NOT fabricate page tags. If exact origin is uncertain, use [p.확인 필요]
   and retain through Phase 2 audit.

   Multi-page rule: use [pp.XX–YY] only when the concept is genuinely developed
   across the full range; do not pad ranges to make tags look authoritative.
...
Source page tags ([p.XX] etc.) appear inline as plain text within the markdown.
They will be styled in Phase 5 — do not wrap them in HTML markers in this phase.
...
Assign final priority to every finding from T1–T3, T5, T7:
...
T7. SOURCE PAGE TAG VERIFICATION (v3.3.3 신설)

Page tags in Draft v0 (formats: [p.XX], [pp.XX–YY], [pp.XX, YY], [p.확인 필요])
must be verified against the PDF.

Sub-task T7a — Tag accuracy check:
For every page tag attached to a MUST card / equation block / key example in Draft v0:
  - Open the cited page(s) and verify the concept, equation, or example actually
    appears there (or develops there in the case of multi-page tags).
  - Classify: TAG_MATCH | TAG_ERROR | TAG_UNVERIFIABLE
  - For TAG_ERROR: state the correct page(s).
  - For [p.확인 필요] flags:
      → If resolvable from PDF, propose the specific page(s).
      → If not resolvable, retain the flag for manual review.

Sub-task T7b — Tag presence check:
For every §2 MUST Concept Anatomy Card, independent equation block, and key worked
example/dataset block in Draft v0:
  - Verify that a source page tag is present (per Phase 1 Rule 8).
  - If absent, classify as MISSING_PAGE_TAG and propose the correct page(s).

PRIORITY MAPPING (T7 items in T4):
  - TAG_ERROR              → MUST_FIX (incorrect citation undermines traceability).
  - MISSING_PAGE_TAG       → MUST_FIX (Rule 8 violation).
  - Resolved [p.확인 필요]  → SHOULD_FIX (replace flag with verified page).
  - Unresolved [p.확인 필요]→ OPTIONAL  (retain flag for human review).
  - TAG_UNVERIFIABLE       → OPTIONAL  (downgrade to [p.확인 필요] for manual review).

Do NOT fabricate page numbers when proposing corrections. If the auditor cannot
locate the concept in the PDF, classify as TAG_UNVERIFIABLE rather than guessing.

T7 must NOT modify Draft v0 itself — output corrections in the audit table only.
...
| # | Item | Source Task (T1/T2/T3/T5/T7) | Priority | Rationale |
...
## T7: Source Page Tag Audit (v3.3.3 신설)
### T7a — Tag Accuracy
| Item (card / equation / example name) | Tag in Draft v0 | PDF Verified Page(s) | Classification | Correction |

### T7b — Tag Presence
| Item (card / equation / example name) | Tag Present? | Proposed Page(s) | Classification |
...
- Do not fabricate page numbers in T7 corrections — use TAG_UNVERIFIABLE if uncertain.
...
- T7 must verify against PDF — do not classify TAG_MATCH without inspection.
...
L6. PAGE TAG SPOT-CHECK (v3.3.3 신설)

C-Fast-Safe omits Phase 3 — page tags become the primary traceability backstop.
Spot-check is therefore mandatory even in lite mode.

Sub-task L6a — Tag accuracy spot-check:
Identify any source page tag in Draft v0 that points to a page where the cited
concept does not appear.
Spot-check only — do not enumerate every tag.
Classify each flagged item: TAG_ERROR | TAG_UNVERIFIABLE.
For TAG_ERROR: state the correct page(s).

Sub-task L6b — Missing tag spot-check:
Scan §2 MUST card titles, independent equation blocks, and key example headings.
If any are missing a page tag (per Phase 1 Rule 8), flag as MISSING_PAGE_TAG and
propose the correct page(s).

Do not fabricate page numbers. If unsure, classify as TAG_UNVERIFIABLE.
...
## L6: Page Tag Spot-Check
| Item | Tag in Draft v0 | PDF | Classification | Fix |  (or "No issues detected")

## Priority Summary
  MUST_FIX (any ERROR, NOT_IN_SOURCE, ABSENT key message, TAG_ERROR, MISSING_PAGE_TAG):
...
- Do not fabricate page numbers in L6.
...
- Do not propose new page tags — page tags are reserved for direct PDF citations,
  and Crucible-derived insights are not direct citations. Trench tips do not get [p.XX].
...
- T7 page tag corrections: do not contradict — these will be applied in Phase 4A.
...
Priority rules:
  Audit MUST_FIX     → ADOPT by default. Rejection requires PDF contradiction.
    (this includes T7 TAG_ERROR and MISSING_PAGE_TAG items.)
  Audit T7 SHOULD_FIX (resolved [p.확인 필요]) → ADOPT — replace flag with verified page.
  Audit T7 OPTIONAL (unresolved [p.확인 필요]) → retain flag as-is.
...
Apply deletion rules to Step 1 Draft v0 before writing Content Lock v1:
  ...
  RULE C6 (v3.3.3 신설): Source page tags ([p.XX], [pp.XX–YY], [pp.XX, YY], [p.확인 필요])
           on MUST cards, equation blocks, and key examples MUST be preserved through compression.
           If a card is consolidated or merged, merge its tags accordingly
           (e.g., merging cards on p.123 and p.124 → [pp.123–124]).
           Compression must NOT silently drop page tags.
...
Apply T7 corrections from Audit Report inline:
  - TAG_ERROR              → replace with PDF-verified page(s).
  - MISSING_PAGE_TAG       → insert verified tag at the correct location (per Rule 8).
  - Resolved [p.확인 필요]  → replace flag with verified page(s).
  - Unresolved [p.확인 필요]→ retain flag verbatim.
...
- [p.확인 필요] tags: retain unless Audit T7 resolved them; never silently drop.
- Source page tags: preserve through Compression Pass per RULE C6.
...
Non-function (applies to both passes):
  ...
  - Do not insert FIGURE markers. Figure work is Phase 4C.
  - Do not modify, relocate, or remove source page tags
    ([p.XX], [pp.XX–YY], [pp.XX, YY], [p.확인 필요]).
...
Permitted edits:
  ...
  - Preserve without alteration: all equations, numerical values, source page tags
    ([p.XX], [pp.XX–YY], [pp.XX, YY], [p.확인 필요]), [확인 필요] flags, [복원] flags,
    technical claims, and NONMEM code.
...
Prohibited edits:
  ...
  - Do not move page tags from their card/equation/example anchor positions.
...
Pass 1 output:
  ...
  All source page tags carry forward at their original anchor positions unchanged.
...
Pass 2 output:
  ...
  Step 3: Generate Content Lock v2.
    ...
    All source page tags carry forward at their original positions unchanged.
...
- Do not add new page tags — page tagging belongs to Phase 1 / Phase 4A only.
- [확인 필요] flags and [p.확인 필요] tags must carry forward unchanged through both passes.
...
If FULL MODE:
  ...
  Do not modify, relocate, or remove existing source page tags
  ([p.XX], [pp.XX–YY], [pp.XX, YY], [p.확인 필요]) on MUST cards / equations / examples.
...
If PATCH MODE:
  ...
  - Anchor strings MAY include source page tags (e.g., "Concept Anatomy: Hepatic Clearance [p.123]")
    when they help establish uniqueness.
...
- Do not modify, relocate, or remove source page tags on MUST cards / equations / examples.
- Do not add new source page tags in this phase — page tagging is closed at Phase 4A.
  (Figure markers carry their own internal "Source:" field, which is separate from
  body content [p.XX] tags.)
...
- Do not modify or remove source page tags ([p.XX] etc.).
```
...
QA-8. PATCH MODE SPLICE FIDELITY
...
QA-9. SOURCE PAGE TAG RENDERING (v3.3.3 신설)
  - Every source page tag ([p.XX], [pp.XX–YY], [pp.XX, YY], [p.확인 필요]) in the
    approved Step 1 deliverable is rendered as <span class="source-page"> in HTML.
  - [p.확인 필요] tags additionally carry .source-uncertain class with amber styling.
  - Page tags appear at the locations specified in Phase 1 Rule 8
    (MUST card title, equation block caption, key example heading).
  - No page tags rendered as plain bracketed text (must be wrapped in <span>).
  - No page tags fabricated during HTML rendering
    (count of <span class="source-page"> = count of [p.*] patterns in reference deliverable).
  - No page tags silently removed
    (every reference-deliverable tag has a corresponding HTML <span>).
  - Page numbers preserved exactly (no normalization of digits or page values).
  - Print mode preserves tag visibility (no display:none via @media print).
  - Page tags inside <pre><code> blocks remain as plain text (NOT wrapped in <span>).

  Severity rule: any TAG_MISSING, TAG_FABRICATED, or TAG_NUMBER_ALTERED is CRITICAL.
                 TAG_NOT_WRAPPED (rendered as plain text) is MAJOR.
                 [p.확인 필요] missing source-uncertain class is MINOR.
...
QA-9 page tag failures (missing, fabricated, or page-number altered) are CRITICAL by default —
they break the page-level traceability guarantee of v3.3.3.
...
Rendering, structure, marker styling, figure rendering, splice fidelity, page tag
rendering, and interactivity fixes only.'
[paste QA Report]"
...
- Do not propose new page tags or modify existing page numbers.
...
Phase 1    → Step 1 Draft v0.md                    (with [p.XX] tags per Rule 8)
Phase 2    → Audit Report v1.md                    (T1–T7 포함; T7은 page tag verification)
Phase 3    → Crucible Report v1.md
Phase 4A   → Content Lock v1.md                    (intermediate — integration + compression
                                                     + T7 corrections applied)
Phase 4B-1 → Content Lock v1.5.md                  (intermediate — readability polish;
                                                     page tags preserved verbatim)
...
Phase 5    → Draft HTML v1.html                    (page tags rendered as <span class="source-page">)
             PATCH MODE 사용 시: + Splice Verification Table (HTML 헤더에 comment 삽입)
Phase 6    → QA Report v1.md (QA-1 ~ QA-9) + Final HTML
```
...
| 1 | MUST card / 독립 수식 블록 / 핵심 예시에 page tag 누락 (v3.3.3) | Rule 8 enforcement 후 재출력 |
| 1 | 모든 문장 끝에 [p.XX]가 부착됨 (v3.3.3) | 카드/수식/예시 단위로 통합 — 문장 단위 태깅 금지 |
| 1 | Master's Lens callout, Trench tip, §5/§7/§8 블록에 [p.XX] 부착됨 (v3.3.3) | 해당 위치 page tag 모두 제거 — Rule 8 부착 위치 위반 |
| 1 | 페이지 확신 없는데 임의 [p.XX] 부착됨 (v3.3.3) | [p.확인 필요]로 교체 |
...
| 2 | T7이 누락됨 (v3.3.3) | T7a + T7b 추가 후 재출력 |
| 2 | T7에서 visual inspection 없이 TAG_MATCH로 분류됨 (v3.3.3) | "verify against PDF" 재지시 |
| 2 | T7에서 TAG_UNVERIFIABLE 대신 임의 페이지 번호 제안 (v3.3.3) | "do not fabricate" 재지시, TAG_UNVERIFIABLE로 재분류 |
...
| 2-lite | L6 누락됨 (v3.3.3) | C 모드는 page tag가 유일한 traceability 백스톱 — L6 spot-check 필수 |
| 3 | Crucible Report에 T4 삭제 목록 없음 | Phase 3 재실행 |
| 3 | 삽입 문장이 3문장 이상 | 각 삽입문을 1–2문장으로 재압축 |
| 3 | Crucible이 figure를 제안함 | 해당 항목 무시 (Phase 4C 전담) |
| 3 | Crucible Trench tip이나 bridge에 [p.XX] 부착됨 (v3.3.3) | 제거 — Crucible 산물은 직접 인용 아님 |
...
| 4A | T7 TAG_ERROR 미반영 (v3.3.3) | Audit corrections 적용 후 재출력 |
| 4A | Compression 도중 page tag 누락 (v3.3.3) | RULE C6 위반 — 원위치 복원, 카드 병합 시 [pp.X–Y]로 통합 |
| 4A | 새 [p.XX] 태그가 Phase 4A에서 fabricate됨 (v3.3.3) | Phase 2 T7에서 검증되지 않은 태그는 [p.확인 필요]로 처리 |
| 4B-1 | 문장의 사실·수식·결론이 바뀜 | 해당 문장만 원문으로 복원 |
| 4B-1 | 새 임상 시나리오나 수치가 추가됨 | 추가 내용 삭제, Pass 1 규칙 재확인 |
| 4B-1 | page tag가 readability polish 도중 누락·이동됨 (v3.3.3) | Permitted edits 위반 — 원위치 복원 |
| 4B-2 | Content Lock v2가 v1 대비 8% 초과 | 낮은 가치 annotation 제거 후 재점검 |
| 4B-2 | 같은 용어에 gloss가 두 번 이상 달림 | 첫 등장 1회만 남기고 나머지 삭제 |
| 4B-2 | Annotation 도중 새 [p.XX] 추가됨 (v3.3.3) | 제거 — Phase 4B는 page tag 생성 단계가 아님 |
...
| 4C | Figure marker 삽입 도중 인접 page tag가 변형됨 (v3.3.3) | 원위치 복원 — Phase 4C는 page tag 변경 권한 없음 |
| 5 | 마커가 HTML에 스타일 없이 텍스트로 남음 | 마커별 컴포넌트 매핑 재적용 |
| 5 | Self-Test 정답이 기본 상태에서 노출됨 | JS 아코디언 로직 수정 |
| 5 | [확인 필요]가 HTML에서 사라짐 | `<mark>` 태그 적용 |
| 5 | [p.XX] 태그가 plain text로 렌더링됨 (v3.3.3) | <span class="source-page"> 래핑 적용 |
| 5 | [p.확인 필요]에 source-uncertain 클래스 누락 (v3.3.3) | source-uncertain 추가 |
| 5 | <pre><code> 안의 [p.XX]가 <span>으로 래핑됨 (v3.3.3) | 코드 블록 내부는 plain text 유지 — 래핑 제거 |
| 5 | [p.XX] 태그가 새로 fabricate됨 또는 페이지 번호가 변경됨 (v3.3.3) | CRITICAL — 원본 복원 |
| 5 | @media print에 source-page display:none 적용 (v3.3.3) | 인쇄용 traceability 보존 — 가시 상태로 복원 |
...
| 6 | QA-9에서 page tag 누락·fabricate·번호 변형 발견 (v3.3.3) | CRITICAL — Phase 5 재컴파일 (page tag 렌더링 로직 점검) |
| 6 | QA-9에서 plain text 렌더링 발견 (v3.3.3) | MAJOR — <span class="source-page"> 래핑 적용 |
...
| 도구 | 고정 역할 | 절대 하지 말 것 |
|------|---------|-------------|
| 교과서 html 작업자 | Draft 생성 (Rule 8 page tag 부착 포함) / HTML 컴파일 / Figure 코드 렌더링 (Phase 5) / Mermaid self-check 적용 / PATCH MODE splice 수행 + Verification Table 출력 / page tag → `<span class="source-page">` 변환 | 검증, 리뷰, Figure 선별 판단, anchor 추측 삽입, page tag 신규 생성 (Phase 5에서) |
| Gemini Pro | PDF 원문 대조 감사 / Figure inventory (T6, visual inspection 우선) / Page tag 검증 (T7 — TAG_MATCH/TAG_ERROR/TAG_UNVERIFIABLE/MISSING_PAGE_TAG 분류) | 설명 개선, 내용 창작, Figure 설계, page tag 임의 fabricate |
| 박사과정 멘토 | 통찰 추출, 규제 리스크 탐지 | 전체 재작성, HTML 작업, Figure 제안, Trench tip에 [p.XX] 부착 |
| GPT-5.5 (Phase 4) | 통합 편집 판단, 압축, T7 corrections 반영, page tag 보존 (RULE C6) | 새 리뷰 수행, 내용 창작, page tag 신규 생성 |
| GPT-5.5 (Phase 4C) | Figure 선별·예산 적용·brief 작성·PATCH MODE 판정·verbatim unique anchor 작성 | Mermaid/SVG 코드 직접 작성, 텍스트 수정, page tag 변경 |
| Claude / GPT-5.5 (Phase 6) | HTML QA / Figure 렌더링 QA / Mermaid syntax 검증 / PATCH MODE splice fidelity 검증 (QA-8) / Page tag 렌더링 검증 (QA-9) | HTML 재작성, 내용 추가, splice 실패 추측 보정, page tag 신규 제안 |
...
*v3.3.3 패치: Page-level Traceability 보강 — 6개 minor patch — (1) Phase 1 PROMPT 1에 Rule 8 (Source Tagging) 신설 — 모든 §2 MUST card / 독립 수식 블록 / 핵심 예시 블록에 [p.XX], [pp.XX–YY], [pp.XX, YY], [p.확인 필요] 형식의 source page tag 의무화 + 부착 위치 3곳 명시 + 부착 금지 위치 (per-sentence, Trench tip, §5/§7/§8 블록) 명시 / (2) Phase 2 PROMPT 2에 T7 (Source Page Tag Verification — T7a 정확성 검증 + T7b 누락 검증) 추가 + PROMPT 2-lite에 L6 (Page Tag Spot-Check) 추가 — C-Fast-Safe는 page tag가 유일한 traceability 백스톱이므로 spot-check 필수 / (3) Phase 4A에 RULE C6 (page tag 보존) 신설 + T7 corrections 적용 절차 명문화 / Phase 4B-1·4B-2에 page tag 보존 규칙 명시 + 신규 태그 생성 금지 / Phase 4C에 page tag 변경 금지 명문화 / (4) Phase 5 marker mapping에 [p.XX] / [p.확인 필요] → `<span class="source-page">` 렌더링 규칙 + pattern detection scope (body text only, NOT in <pre><code> or figure markers) + CSS (.source-page, .source-uncertain, @media print) 추가 / (5) Phase 6에 QA-9 (Source Page Tag Rendering — 누락·fabricate·번호 변형은 CRITICAL, plain text 렌더링은 MAJOR) 추가 / (6) Anti-Pattern Reference에 page tag 위반 신호 17개 추가 + 역할 고정표에 page tag 책임 분담 명시*
*핵심 설계 원칙: 기능 분리 → Master's Lens First → Accuracy Before Insight → Editorial Lock → Visual Pedagogy Triage → Page-level Traceability → HTML化*
*Figure 운영 원칙: Pointer 우선 + 선별적 LLM 도식화 + 권한 조건부 Image — 결정은 Step 1, 렌더링은 Step 2 — 그리고 splice는 검증된다*
*Page Tag 운영 원칙: Phase 1 생성 → Phase 2 검증 → Phase 4 보존 → Phase 5 렌더링 → Phase 6 QA — 카드/수식/예시 단위만, 문장 단위 금지 — 검증 없는 페이지 번호는 fabricate가 아니라 [p.확인 필요]로*
