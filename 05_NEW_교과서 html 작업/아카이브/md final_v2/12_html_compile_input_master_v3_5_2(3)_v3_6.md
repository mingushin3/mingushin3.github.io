...
> **핵심 메시지:** 이 세션은 지연을 하나의 현상으로 외우는 것이 아니라, 지연된 hidden state를 찾아 모델 family를 선택하는 훈련입니다.
...
<!-- SLIDE_START: S02 | title: §1 — 세션 헤더와 로드맵 | source_anchor: §1 — 세션 헤더와 로드맵 -->
> **핵심 메시지:** 농도-반응 시간 지연은 먼저 분포, 시스템 적응, 신호전달 중 어느 시계가 늦는지로 분류해야 합니다.
...
<!-- SLIDE_START: S03 | title: 개념 로드맵 | source_anchor: 개념 로드맵 -->
> **핵심 메시지:** 로드맵의 핵심은 hysteresis 방향을 출발점으로 삼되, 최종 판단은 hidden state 식별로 끝내는 것입니다.
...
<!-- SLIDE_START: S04 | title: F12-01 — PK/PD 시간 지연 뒤의 세 가지 숨은 시계 | source_anchor: F12-01 — PK/PD 시간 지연 뒤의 세 가지 숨은 시계 -->
> **핵심 메시지:** 이 figure slide는 원본 그림을 재현하지 않고, 어떤 hidden state 판단을 확인해야 하는지만 안전하게 지정합니다.
...
<!-- SLIDE_START: S05 | title: §2 — 개념 해부 카드 | source_anchor: §2 — 개념 해부 카드 -->
> **핵심 메시지:** 이 slide의 판단 기준은 제목의 개념을 어떤 hidden state와 어떤 실무 검증으로 연결할지 결정하는 것입니다.

# §2 — 개념 해부 카드

---

<!-- SLIDE_START: S06 | title: Card 1 — 효과구획 모델(Effect Compartment Model)과 $k_{e0}$ [⚡ Apex Concept] | source_anchor: Card 1 — 효과구획 모델(Effect Compartment Model)과 $k_{e0}$ [⚡ Apex Concept] -->
> **핵심 메시지:** 효과구획 모델의 핵심 판단은 혈장 농도가 아니라 생체상 농도 $C_e$가 반응을 지배하는지입니다.
...
<!-- MASTER LENS -->
<!-- SLIDE_START: S07 | title: 핵심 통찰 | source_anchor: 핵심 통찰 -->
> **핵심 메시지:** 이 단락의 핵심은 해당 모델 family가 설명하려는 지연의 정체를 한 문장으로 고정하는 것입니다.
...
<!-- SLIDE_START: S08 | title: A. 형식적 정의 | source_anchor: A. 형식적 정의 -->
> **핵심 메시지:** 정의 slide에서는 이 모델이 어떤 숨은 상태를 추가하고 무엇을 관찰값에서 추론하는지 먼저 확정해야 합니다.
...
<!-- SLIDE_START: S09 | title: G&W Fig.3.53 (p.263) | source_anchor: G&W Fig.3.53 (p.263) -->
> **핵심 메시지:** 이 figure slide는 원본 그림을 재현하지 않고, 어떤 hidden state 판단을 확인해야 하는지만 안전하게 지정합니다.
...
<!-- SLIDE_START: S10 | title: B. 핵심 방정식 | source_anchor: B. 핵심 방정식 -->
> **핵심 메시지:** 핵심 방정식은 지연을 만드는 상태변수와 그 상태를 움직이는 rate constant를 분리해서 읽어야 합니다.
...
>
> $$
> \underbrace{A_p}_{\text{혈장 내 약물량}}=\underbrace{D}_{\text{투여량}}\,\overbrace{e^{-\underbrace{K}_{\text{제거속도}}t}}^{\text{시간 }t\text{ 후 잔존분율}}
> $$
>
> - **항별 의미:** 좌변 $A_p$는 시간 $t$에서 혈장 구획에 남아 있는 약물량을 예측합니다. 우변의 $D$는 초기 투여량이고, $e^{-Kt}$는 1차 제거에 의해 시간이 지나며 남는 분율입니다.
> - **실무 직관:** $D$가 커지면 전체 약물량이 비례해서 커지고, $K$나 $t$가 커지면 지수항이 작아져 $A_p$가 감소합니다. 이 식은 효과구획을 구동하는 상류 혈장 입력을 닫힌형으로 제공합니다.
> - **오해 방지:** $K$는 효과 지연 속도가 아니라 혈장 제거 속도입니다. 이 식만으로는 생체상 농도나 반응 지연을 설명하지 못합니다.
...
>
> $$
> \underbrace{\frac{dA_e}{dt}}_{\text{효과구획 약물량 변화}}=\overbrace{k_{1e}A_p}^{\text{혈장에서 유입}}-\overbrace{k_{e0}A_e}^{\text{효과구획에서 소실/평형화}}
> $$
>
> - **항별 의미:** 좌변은 효과구획 약물량 $A_e$가 시간에 따라 얼마나 변하는지를 정의합니다. 우변 첫 항 $k_{1e}A_p$는 혈장에서 효과구획으로 들어오는 입력이고, 둘째 항 $k_{e0}A_e$는 효과구획에서 빠져나가는 1차 항입니다.
> - **실무 직관:** $A_p$나 $k_{1e}$가 커지면 $A_e$가 더 빨리 증가하고, $A_e$나 $k_{e0}$가 커지면 유출 항이 커져 증가가 둔화됩니다. 이 균형이 plasma와 effect site 사이의 시간 지연을 만듭니다.
> - **오해 방지:** $k_{e0}$를 단순 제거상수로만 읽으면 안 됩니다. 여기서는 생체상 평형화 속도를 나타내는 링크 파라미터입니다.
...
>
> $$
> \underbrace{A_e}_{\text{효과구획 약물량}}=\overbrace{\frac{\underbrace{k_{1e}}_{\text{유입속도}}\underbrace{D}_{\text{투여량}}}{\underbrace{k_{e0}-K}_{\text{효과평형화와 혈장제거의 속도 차}}}}^{\text{스케일 계수}}\left(\overbrace{e^{-Kt}}^{\text{혈장 감소}}-\overbrace{e^{-k_{e0}t}}^{\text{효과구획 평형화}}\right)
> $$
>
> - **항별 의미:** 좌변은 시간 $t$에서 효과구획에 존재하는 약물량을 예측합니다. 우변은 투여량과 유입속도로 정해지는 크기, 그리고 혈장 제거와 효과구획 평형화의 두 지수함수 차이로 구성됩니다.
> - **실무 직관:** $D$와 $k_{1e}$가 커지면 효과구획 약물량의 스케일이 커집니다. $k_{e0}$와 $K$의 상대적 차이가 곡선의 지연 형태를 결정하므로, 두 속도가 비슷하면 식별이 어려워질 수 있습니다.
> - **오해 방지:** 두 지수항의 차이는 “두 제거가 경쟁한다”는 뜻이 아니라, 혈장 입력이 사라지는 속도와 효과구획이 따라오는 속도 사이의 시간차를 표현합니다.
...
>
> $$
> \underbrace{k_{1e}=k_{e0}}_{\text{유입과 평형화 속도를 동일하게 두는 식별성 단순화}},\qquad \underbrace{K_p=1}_{\text{혈장-생체상 분배비를 1로 두는 스케일 단순화}}
> $$
>
> - **항별 의미:** $k_{1e}=k_{e0}$는 효과구획 유입과 평형화 속도를 동일하게 두는 표준 단순화이고, $K_p=1$은 혈장-생체상 분배 스케일을 별도로 추정하지 않겠다는 가정입니다.
> - **실무 직관:** 이 가정이 있으면 $C_e$를 혈장 농도 스케일에서 다룰 수 있어 $EC_{50}$와 $k_{e0}$ 추정이 안정화됩니다. 가정이 깨지면 생체상 농도 스케일과 혈장 농도 스케일이 분리됩니다.
> - **오해 방지:** 이 등식들은 생리학적 보편 법칙이 아니라 식별가능성을 확보하기 위한 모델링 약속입니다. 실제 분배가 항상 1이라는 뜻이 아닙니다.
...
>
> $$
> \underbrace{C_e}_{\text{생체상 농도}}=\overbrace{\frac{\underbrace{k_{e0}}_{\text{효과평형화}}\underbrace{D}_{\text{투여량}}}{\underbrace{V_c}_{\text{중심분포용적}}\left(\underbrace{k_{e0}-K}_{\text{속도 차}\,}\right)}}^{\text{농도 스케일}}\left(\overbrace{e^{-Kt}}^{\text{혈장 농도 감소}}-\overbrace{e^{-k_{e0}t}}^{\text{생체상 추종}}\right)
> $$
>
> - **항별 의미:** 좌변 $C_e$는 직접 관찰되지 않는 생체상 농도를 예측합니다. 우변은 투여량을 분포용적과 속도 차이로 보정한 농도 스케일에 두 지수항의 시간차를 곱한 구조입니다.
> - **실무 직관:** $D$가 커지면 $C_e$가 커지고, $V_c$가 커지면 같은 투여량에서 농도 스케일이 낮아집니다. $k_{e0}$가 작을수록 생체상이 혈장을 더 늦게 따라가므로 반응 지연이 커집니다.
> - **오해 방지:** $C_e$는 측정된 혈장 농도가 아니라 모델이 추론한 effect-site 농도입니다. 추정된 $EC_{50}$도 이 스케일 가정의 영향을 받습니다.
...
>
> $$
> \underbrace{\frac{dC_e}{dt}}_{\text{생체상 농도 변화율}}=\underbrace{k_{e0}}_{\text{평형화 속도}}\left(\overbrace{C}^{\text{혈장 농도}}-\overbrace{C_e}^{\text{생체상 농도}}\right)
> $$
>
> - **항별 의미:** 좌변은 생체상 농도가 시간에 따라 변하는 속도입니다. 우변은 혈장 농도와 생체상 농도의 차이를 $k_{e0}$라는 평형화 속도로 좁히는 구조입니다.
> - **실무 직관:** $C>C_e$이면 $C_e$가 증가하고, $C<C_e$이면 $C_e$가 감소합니다. $k_{e0}$가 클수록 생체상이 혈장을 빠르게 따라가고, 작을수록 hysteresis가 커집니다.
> - **오해 방지:** 이 ODE는 모든 지연의 보편 해답이 아닙니다. 지연된 것이 약물 농도가 아니라 response pool이나 transduction chain이면 다른 모델이 필요합니다.
...
<!-- SLIDE_START: S11 | title: C. 핵심 가정과 경계조건 | source_anchor: C. 핵심 가정과 경계조건 -->
> **핵심 메시지:** 가정과 경계조건은 모델이 작동하는 조건뿐 아니라 언제 다른 모델 family로 이동해야 하는지를 알려줍니다.
...
<!-- SLIDE_START: S12 | title: D. 보정된 수치 앵커 | source_anchor: D. 보정된 수치 앵커 -->
> **핵심 메시지:** 수치 앵커는 서로 다른 교과서 사례를 같은 파라미터로 오라벨링하지 않도록 막는 안전장치입니다.
...
<!-- SLIDE_START: S13 | title: E. 그럴듯한 오해 — 회전 데이터에 링크 모델을 끼워 맞추는 경우 | source_anchor: E. 그럴듯한 오해 — 회전 데이터에 링크 모델을 끼워 맞추는 경우 -->
> **핵심 메시지:** 용량별 PD 파라미터가 기전 없이 흔들리면 Link 모델이 다른 지연 상태를 흡수하고 있는지 먼저 의심해야 합니다.
...
<!-- SLIDE_START: S14 | title: G&W Fig.3.59와 Table 3.10 (p.271) | source_anchor: G&W Fig.3.59와 Table 3.10 (p.271) -->
> **핵심 메시지:** 이 figure slide는 원본 그림을 재현하지 않고, 어떤 hidden state 판단을 확인해야 하는지만 안전하게 지정합니다.
...
<!-- SLIDE_START: S15 | title: E-2. 최상위 수준의 그럴듯한 오해 — "CCW hysteresis ⇒ effect compartment" [EXPERT_INFERENCE, v3] | source_anchor: E-2. 최상위 수준의 그럴듯한 오해 — "CCW hysteresis ⇒ effect compartment" [EXPERT_INFERENCE, v3] -->
> **핵심 메시지:** 반시계방향 hysteresis는 effect compartment의 증거가 아니라 effect compartment부터 검토하라는 진단 단서입니다.
...
<!-- SLIDE_START: S16 | title: F. $k_{e0}$ vs $k_{out}$ vs $k_{off}$ | source_anchor: F. $k_{e0}$ vs $k_{out}$ vs $k_{off}$ -->
> **핵심 메시지:** 동일한 단위의 rate constant라도 무엇의 시계인지가 다르면 모델 해석은 완전히 달라집니다.
...
>
> $$
> \overbrace{k_{e0}=0.025}^{\text{생체상 평형 시계}},\quad \overbrace{k_{out}=0.031}^{\text{시스템 회전 시계}},\quad \overbrace{k_{off}=0.018\ \text{min}^{-1}}^{\text{수용체 해리 시계}}
> $$
>
> - **항별 의미:** 이 세 수식형 값은 모두 시간의 역수 단위를 갖지만, 좌표가 서로 다릅니다. $k_{e0}$는 생체상 평형, $k_{out}$은 반응 시스템 turnover, $k_{off}$는 수용체 결합 해리를 나타냅니다.
> - **실무 직관:** 값이 작으면 해당 과정의 시계가 느리고, 값이 크면 빠릅니다. 그러나 어느 값이 더 큰지를 비교하기 전에 무엇의 속도인지 먼저 구분해야 합니다.
> - **오해 방지:** 단위와 크기가 비슷하다는 이유로 같은 기전이라고 결론내리면 안 됩니다. PD21의 핵심은 유사한 숫자가 서로 다른 hidden state를 가리킬 수 있다는 점입니다. 따라서 단회투여(single-dose) 시간 경과만으로는 여러 메커니즘이 비슷하게 피팅될 수 있습니다. 기전 식별(mechanism discrimination)에는 추가 용량이나 washout 행동 같은 *더 풍부한 실험 설계(richer design)*가 필요합니다 [G&W pp.840–846].

<!-- SLIDE_START: S17 | title: G. 실무 체화 요약(Practice Brief) — [EXPERT_INFERENCE, v3] | source_anchor: G. 실무 체화 요약(Practice Brief) — [EXPERT_INFERENCE, v3] -->
> **핵심 메시지:** 실무 적용에서는 plot을 본 뒤 바로 모델을 고르지 말고 검증 순서와 금지 사항을 먼저 정해야 합니다.
...
<!-- SLIDE_START: S18 | title: Recap 1 | source_anchor: Recap 1 -->
> **핵심 메시지:** Recap slide에서는 방금 배운 모델 family를 하나의 실무 선택 원칙으로 압축해야 합니다.
...
<!-- SLIDE_START: S19 | title: Card 2 — 이력현상(Hysteresis) 방향과 기전 트리아지(Mechanism Triage) | source_anchor: Card 2 — 이력현상(Hysteresis) 방향과 기전 트리아지(Mechanism Triage) -->
> **핵심 메시지:** Hysteresis 방향은 모델 선택의 판결문이 아니라 다음 검증 단계로 보내는 첫 번째 triage 신호입니다.

## Card 2 — 이력현상(Hysteresis) 방향과 기전 트리아지(Mechanism Triage)

<!-- MASTER LENS -->
<!-- SLIDE_START: S20 | title: 핵심 통찰 | source_anchor: 핵심 통찰 -->
> **핵심 메시지:** 이 단락의 핵심은 해당 모델 family가 설명하려는 지연의 정체를 한 문장으로 고정하는 것입니다.
...
<!-- SLIDE_START: S21 | title: A. 정의 | source_anchor: A. 정의 -->
> **핵심 메시지:** Hysteresis는 같은 농도에서 시간 방문 순서에 따라 반응이 달라지는 phase-plane 현상입니다.
...
<!-- SLIDE_START: S22 | title: R&T Fig.8-2 (p.235) | source_anchor: R&T Fig.8-2 (p.235) -->
> **핵심 메시지:** 이 figure slide는 원본 그림을 재현하지 않고, 어떤 hidden state 판단을 확인해야 하는지만 안전하게 지정합니다.
...
<!-- SLIDE_START: S23 | title: B. 진단 매트릭스 | source_anchor: B. 진단 매트릭스 -->
> **핵심 메시지:** 방향별 후보 기전은 반드시 후속 분석과 함께 읽어야 하며, 방향만으로 모델을 확정하면 안 됩니다.
...
<!-- SLIDE_START: S24 | title: B-2. 이력현상 collapse 기법 — 루프 기하학을 수치로 변환하기 | source_anchor: B-2. 이력현상 collapse 기법 — 루프 기하학을 수치로 변환하기 -->
> **핵심 메시지:** Loop collapse는 시각적 hysteresis를 단일 $k_{e0}$ 추정 가능성으로 바꾸는 핵심 진단 절차입니다.
...
<!-- SLIDE_START: S25 | title: C. R&T 임상 앵커 | source_anchor: C. R&T 임상 앵커 -->
> **핵심 메시지:** R&T 임상 앵커는 hysteresis 방향이 실제로 다양한 rate-limiting 과정에서 생긴다는 점을 보여줍니다.
...
<!-- SLIDE_START: S26 | title: D. 구조적 해석 | source_anchor: D. 구조적 해석 -->
> **핵심 메시지:** 구조적 해석의 핵심은 loop 방향보다 무엇이 지연되는지를 묻는 질문입니다.
...
<!-- SLIDE_START: S27 | title: E. 실무 체화 요약 (Practice Brief) — [EXPERT_INFERENCE, v3] | source_anchor: E. 실무 체화 요약 (Practice Brief) — [EXPERT_INFERENCE, v3] -->
> **핵심 메시지:** 이 slide의 판단 기준은 제목의 개념을 어떤 hidden state와 어떤 실무 검증으로 연결할지 결정하는 것입니다.
...
<!-- SLIDE_START: S28 | title: Recap 2 | source_anchor: Recap 2 -->
> **핵심 메시지:** Recap slide에서는 방금 배운 모델 family를 하나의 실무 선택 원칙으로 압축해야 합니다.
...
<!-- SLIDE_START: S29 | title: Card 3 — 조절자 $M$을 통한 내성·반동(Tolerance/Rebound via Moderator $M$) | source_anchor: Card 3 — 조절자 $M$을 통한 내성·반동(Tolerance/Rebound via Moderator $M$) -->
> **핵심 메시지:** 내성/반동은 약물 잔류가 아니라 조절자 $M$이라는 시스템 메모리의 지연 효과로 읽어야 합니다.

## Card 3 — 조절자 $M$을 통한 내성·반동(Tolerance/Rebound via Moderator $M$)

<!-- MASTER LENS -->
<!-- SLIDE_START: S30 | title: 핵심 통찰 | source_anchor: 핵심 통찰 -->
> **핵심 메시지:** 이 단락의 핵심은 해당 모델 family가 설명하려는 지연의 정체를 한 문장으로 고정하는 것입니다.
...
<!-- SLIDE_START: S31 | title: A. 형식적 정의 | source_anchor: A. 형식적 정의 -->
> **핵심 메시지:** 정의 slide에서는 이 모델이 어떤 숨은 상태를 추가하고 무엇을 관찰값에서 추론하는지 먼저 확정해야 합니다.
...
<!-- SLIDE_START: S32 | title: B. 조절자 ODE | source_anchor: B. 조절자 ODE -->
> **핵심 메시지:** 이 slide의 판단 기준은 제목의 개념을 어떤 hidden state와 어떤 실무 검증으로 연결할지 결정하는 것입니다.
...
>
> $$
> \underbrace{\frac{dR}{dt}}_{\text{반응 변화율}}=\overbrace{k_{in}S(C)}^{\text{약물 자극이 반응을 밀어 올림}}-\overbrace{k_{out}M}^{\text{조절자 메모리가 반응을 되밀어 낮춤}}
> $$
>
> - **항별 의미:** 좌변은 반응 $R$이 시간에 따라 변하는 속도입니다. 우변 첫 항은 약물 자극 $S(C)$가 내인성 생성률 $k_{in}$을 통해 반응을 증가시키는 부분이고, 둘째 항은 조절자 $M$이 반응을 억제하는 음의 되먹임 항입니다.
> - **실무 직관:** $S(C)$나 $k_{in}$이 커지면 반응이 증가하는 방향으로 움직입니다. $M$이나 $k_{out}$이 커지면 반작용이 커져 반응이 낮아지고 tolerance/rebound 패턴이 형성됩니다.
> - **오해 방지:** $M$을 약물 농도 구획으로 오해하면 안 됩니다. $M$은 이전 반응이 남긴 시스템 메모리이며, 부호가 음성이기 때문에 항상성 회복을 만듭니다.
...
>
> $$
> \underbrace{\frac{dM}{dt}}_{\text{조절자 변화율}}=\underbrace{k_{tol}}_{\text{내성 발달/회복 속도}}\left(\overbrace{R}^{\text{현재 반응}}-\overbrace{M}^{\text{지연된 조절자 상태}}\right)
> $$
>
> - **항별 의미:** 좌변은 조절자 상태 $M$이 변하는 속도입니다. 우변은 현재 반응 $R$과 아직 따라오지 못한 조절자 $M$의 차이를 $k_{tol}$ 속도로 좁히는 구조입니다.
> - **실무 직관:** $R>M$이면 $M$이 증가하여 tolerance가 발달하고, $R<M$이면 $M$이 감소하며 회복됩니다. $k_{tol}$이 클수록 시스템 메모리가 빠르게 형성되고 사라집니다.
> - **오해 방지:** $k_{tol}$은 약물 제거 속도가 아니라 시스템 적응 속도입니다. 이 항이 있어야 반복투여 후 carry-over와 withdrawal rebound를 설명할 수 있습니다.
...
>
> $$
> \underbrace{S(C)=1}_{\text{기저 자극}},\quad \underbrace{R_0=\frac{k_{in}}{k_{out}}}_{\text{기저 반응}},\quad \underbrace{\frac{dM}{dt}=0}_{\text{정상 상태}}\Rightarrow \underbrace{k_{tol}(R-M)=0}_{\text{차이가 사라짐}}\Rightarrow \underbrace{R=M}_{\text{조절자가 반응을 따라잡음}}
> $$
>
> - **항별 의미:** 이 연쇄는 baseline과 steady state를 정의합니다. 자극이 기저 수준이면 반응은 생성률/소실률 비로 정해지고, 정상 상태에서는 조절자 $M$이 반응 $R$을 따라잡습니다.
> - **실무 직관:** $k_{in}$이 커지면 $R_0$가 커지고, $k_{out}$이 커지면 $R_0$가 작아집니다. $R$과 $M$의 차이가 사라질수록 tolerance의 동적 변화는 줄어듭니다.
> - **오해 방지:** $R=M$은 항상 모든 시간에 성립하는 항등식이 아니라 정상 상태에서의 결론입니다. 과도기에는 바로 이 차이가 tolerance와 rebound를 만듭니다.
...
>
> $$
> \underbrace{t_{1/2,k_{out}}}_{\text{겉보기 반응 반감기}}=\frac{\overbrace{\ln 2}^{\text{반감기 상수}}\,\overbrace{R_0}^{\text{기저 반응 크기}}}{\underbrace{k_{in}}_{\text{생성률}}\,\underbrace{S(C)}_{\text{약물 자극}}}
> $$
>
> - **항별 의미:** 좌변은 반응 시스템이 겉보기로 절반 변화하는 시간 척도를 정의합니다. 우변은 기저 반응 크기를 생성률과 약물 자극의 곱으로 나눈 형태입니다.
> - **실무 직관:** $S(C)$가 커지면 분모가 커져 겉보기 반감기는 짧아집니다. 따라서 고용량에서 tolerance가 더 빠르게 보일 수 있지만, 이는 같은 구조에서 자극이 강해진 결과입니다.
> - **오해 방지:** 초보자는 이 식을 $k_{out}$ 자체가 용량에 따라 바뀐다는 뜻으로 오해하기 쉽습니다. 여기서 바뀌는 것은 driver $S(C)$에 의해 보이는 effective time scale입니다.
...
<!-- SLIDE_START: S33 | title: B-2. 음의 되먹임 부호가 수학적으로 필요한 이유 | source_anchor: B-2. 음의 되먹임 부호가 수학적으로 필요한 이유 -->
> **핵심 메시지:** Moderator 항의 음의 부호는 선택사항이 아니라 항상성 안정성을 만드는 구조적 조건입니다.
...
<!-- SLIDE_START: S34 | title: C. AUC 비대칭성 — 내장된 시그니처 | source_anchor: C. AUC 비대칭성 — 내장된 시그니처 -->
> **핵심 메시지:** $AUC_R < AUC_E$ 비대칭은 moderator memory loop가 올바르게 설정되었는지를 확인하는 실무 시그니처입니다.
...
>
> $$
> \underbrace{AUC_R}_{\text{중단 후 rebound 면적}}<\underbrace{AUC_E}_{\text{투여 중 효과 면적}},\qquad \underbrace{AUC_R\approx AUC_E}_{\text{잘못된 대칭 예측 신호}}
> $$
>
> - **항별 의미:** 좌측 부등식은 rebound 면적이 투여 중 효과 면적보다 작아지는 moderator 모델의 비대칭 시그니처를 정의합니다. 우측 근사 등식은 모델이 이 비대칭을 놓칠 때 나타나는 경고 신호입니다.
> - **실무 직관:** $M$이 오래 남을수록 rebound가 커질 수 있지만, 투여 중에는 drug stimulus와 $M$ 형성이 함께 작동하므로 두 면적이 기계적으로 같을 필요가 없습니다.
> - **오해 방지:** rebound가 보인다고 두 면적이 대칭이어야 한다고 생각하면 안 됩니다. 이 비대칭 자체가 moderator 구조를 식별하는 실무 단서입니다.

<!-- SLIDE_START: S35 | title: D. PD13 앵커 — 반복 infusion에서의 내성/반동 | source_anchor: D. PD13 앵커 — 반복 infusion에서의 내성/반동 -->
> **핵심 메시지:** PD13의 핵심은 Model II가 통계적으로 우수한 것보다 $k_{tol}>k_{out}$라는 기전 관계를 회복한다는 점입니다.
...
>
> $$
> \overbrace{k_{tol}=4.2}^{\text{조절자/내성 발달 속도}}>\underbrace{k_{out}=2.9}_{\text{반응 turnover 속도}},\qquad \underbrace{k_{tol}=0.05}_{\text{너무 느린 pool/precursor 적응}}
> $$
>
> - **항별 의미:** 부등식의 좌변은 조절자 메모리의 형성 속도이고 우변은 반응 풀의 turnover 속도입니다. Model II에서 조절자가 더 빨리 형성되므로 단일 infusion 중에도 tolerance가 관찰될 수 있습니다.
> - **실무 직관:** $k_{tol}$이 $k_{out}$보다 크면 response가 완전히 회전하기 전에 counter-regulation이 따라붙습니다. 반대로 너무 작은 $k_{tol}$은 관찰된 빠른 tolerance/carry-over를 설명하기 어렵습니다.
> - **오해 방지:** AIC/WRSS만 보고 끝내면 안 됩니다. 이 부등식은 통계적 우월성이 기전적으로 왜 타당한지를 설명하는 핵심 파라미터 관계입니다.
...
<!-- SLIDE_START: S36 | title: G&W Fig.13.1과 Table 13.1 (pp.806–808) | source_anchor: G&W Fig.13.1과 Table 13.1 (pp.806–808) -->
> **핵심 메시지:** 이 figure slide는 원본 그림을 재현하지 않고, 어떤 hidden state 판단을 확인해야 하는지만 안전하게 지정합니다.
...
<!-- SLIDE_START: S37 | title: E. 대안적 내성/반동 구조 — 압축 맥락 | source_anchor: E. 대안적 내성/반동 구조 — 압축 맥락 -->
> **핵심 메시지:** 대안 구조들은 tolerance 일부를 설명할 수 있지만 rebound와 partial tolerance를 동시에 설명하는지로 걸러야 합니다.
...
<!-- SLIDE_START: S38 | title: F. 부호가 중요한 이유 | source_anchor: F. 부호가 중요한 이유 -->
> **핵심 메시지:** 약이 반응을 밀고 반응이 조절자를 만들며 조절자가 되미는 부호 구조가 tolerance와 rebound를 함께 만듭니다.
...
<!-- SLIDE_START: S39 | title: G. 실무 체화 요약(Practice Brief) — [EXPERT_INFERENCE, v3] | source_anchor: G. 실무 체화 요약(Practice Brief) — [EXPERT_INFERENCE, v3] -->
> **핵심 메시지:** 실무 적용에서는 plot을 본 뒤 바로 모델을 고르지 말고 검증 순서와 금지 사항을 먼저 정해야 합니다.
...
<!-- SLIDE_START: S40 | title: F12-06 — 조절자 메모리 루프 (Moderator Memory Loop) | source_anchor: F12-06 — 조절자 메모리 루프 (Moderator Memory Loop) -->
> **핵심 메시지:** 이 figure slide는 원본 그림을 재현하지 않고, 어떤 hidden state 판단을 확인해야 하는지만 안전하게 지정합니다.
...
>
> $$
> \underbrace{\frac{dR}{dt}=k_{in}S(C)-k_{out}M}_{\text{반응은 자극과 조절자 억제의 차이}},\qquad \underbrace{\frac{dM}{dt}=k_{tol}(R-M)}_{\text{조절자는 반응을 지연 추종}}
> $$
>
> - **항별 의미:** figure marker 안의 두 압축 ODE는 moderator memory loop의 핵심을 정의합니다. 첫 식은 반응이 약물 자극과 조절자 억제의 균형으로 변하고, 둘째 식은 조절자 $M$이 반응 $R$을 지연해서 따라간다는 뜻입니다.
> - **실무 직관:** $S(C)$가 커지면 dosing phase의 반응이 커지고, $M$이 커지면 tolerance/rebound 방향의 반작용이 커집니다. $k_{tol}$이 클수록 메모리 루프가 빠르게 형성됩니다.
> - **오해 방지:** 이 도식의 ODE는 이미 Card 3에서 정의된 수식의 시각 요약입니다. figure marker field key는 보존하고, 수식 의미만 learner-facing annotation으로 풀었습니다.
...
<!-- SLIDE_START: S41 | title: Recap 3 | source_anchor: Recap 3 -->
> **핵심 메시지:** Recap slide에서는 방금 배운 모델 family를 하나의 실무 선택 원칙으로 압축해야 합니다.
...
<!-- SLIDE_START: S42 | title: Card 4 — 신호전달 모델 / Transit Chain(Transduction Model / Transit Chain) | source_anchor: Card 4 — 신호전달 모델 / Transit Chain(Transduction Model / Transit Chain) -->
> **핵심 메시지:** 신호전달 모델은 약물 도달 지연이 아니라 수용체 활성화 이후 관찰 반응까지의 하류 전달 지연을 설명합니다.

## Card 4 — 신호전달 모델 / Transit Chain(Transduction Model / Transit Chain)

<!-- MASTER LENS -->
<!-- SLIDE_START: S43 | title: 핵심 통찰 | source_anchor: 핵심 통찰 -->
> **핵심 메시지:** 이 단락의 핵심은 해당 모델 family가 설명하려는 지연의 정체를 한 문장으로 고정하는 것입니다.
...
<!-- SLIDE_START: S44 | title: A. 형식적 정의 | source_anchor: A. 형식적 정의 -->
> **핵심 메시지:** 정의 slide에서는 이 모델이 어떤 숨은 상태를 추가하고 무엇을 관찰값에서 추론하는지 먼저 확정해야 합니다.
...
>
> $$
> \underbrace{k_{out}}_{\text{단계별 분획 회전율}}=\frac{1}{\underbrace{\tau}_{\text{transit time}}}
> $$
>
> - **항별 의미:** 좌변 $k_{out}$은 각 transit 단계가 얼마나 빨리 비워지고 다음 단계로 전달되는지를 나타냅니다. 우변은 그 속도가 단계 통과 시간 $\tau$의 역수임을 정의합니다.
> - **실무 직관:** $\tau$가 커지면 $k_{out}$은 작아지고 단계 전달은 느려집니다. $\tau$가 작아지면 더 빠른 전달과 짧은 단계 지연을 의미합니다.
> - **오해 방지:** 여기서 $k_{out}$은 Card 3의 response turnover와 같은 기호를 쓰지만, 문맥상 transit 단계의 fractional turnover입니다. 같은 기호라도 모델 상태가 다르면 의미가 달라집니다.

<!-- SLIDE_START: S45 | title: G&W Fig.3.98과 Fig.3.99 (p.323) | source_anchor: G&W Fig.3.98과 Fig.3.99 (p.323) -->
> **핵심 메시지:** 이 figure slide는 원본 그림을 재현하지 않고, 어떤 hidden state 판단을 확인해야 하는지만 안전하게 지정합니다.
...
<!-- SLIDE_START: S46 | title: B. 핵심 방정식 | source_anchor: B. 핵심 방정식 -->
> **핵심 메시지:** 핵심 방정식은 지연을 만드는 상태변수와 그 상태를 움직이는 rate constant를 분리해서 읽어야 합니다.
...
>
> $$
> \underbrace{\frac{dR_e}{dt}}_{\text{수용체-구동 상태 변화}}=\underbrace{\frac{1}{\tau}}_{\text{단계 통과 속도}}\left(\overbrace{E_0S(C)}^{\text{약물 자극이 요구하는 목표 반응}}-\overbrace{R_e}^{\text{현재 구동 상태}}\right)
> $$
>
> - **항별 의미:** 좌변은 수용체 구동 상태 $R_e$의 변화율입니다. 우변은 약물 자극이 정하는 목표 수준 $E_0S(C)$와 현재 상태 $R_e$ 사이의 차이를 $\tau$라는 시간상수로 좁히는 구조입니다.
> - **실무 직관:** $S(C)$나 $E_0$가 커지면 목표 반응이 커지고, $\tau$가 작을수록 $R_e$가 더 빨리 목표를 따라갑니다. 이 단계는 transduction chain의 입력을 만듭니다.
> - **오해 방지:** $R_e$를 혈장 농도 또는 생체상 농도로 오해하면 안 됩니다. 이것은 receptor activation 이후의 반응 신호 상태입니다.
...
>
> $$
> \underbrace{\frac{dR_i}{dt}}_{\text{제 }i\text{ transit 상태 변화}}=\underbrace{\frac{1}{\tau}}_{\text{단계별 전달 속도}}\left(\overbrace{R_{i-1}}^{\text{상류 상태}}-\overbrace{R_i}^{\text{현재 상태}}\right),\quad \underbrace{i=1,2,\ldots,n}_{\text{순차적 단계}}
> $$
>
> - **항별 의미:** 좌변은 $i$번째 transit 상태가 변하는 속도입니다. 우변은 바로 앞 단계 $R_{i-1}$와 현재 단계 $R_i$의 차이를 단계별 통과 시간 $\tau$로 전달하는 구조입니다.
> - **실무 직관:** $n$이 늘어나면 신호가 거쳐야 할 단계가 늘고, $\tau$가 커지면 각 단계가 느려져 관찰 반응의 onset 지연이 길어집니다. 이 구조는 지연된 sigmoid onset을 만들 수 있습니다.
> - **오해 방지:** $n$을 실제 생물학적 단계 수로 단정하면 안 됩니다. PD35처럼 데이터가 지지하는 실용적 cascade 깊이를 나타내는 모델 차원으로 읽어야 합니다.
...
>
> $$
> \underbrace{R_{obs}}_{\text{관찰 반응}}=\underbrace{R_n}_{\text{마지막 transit 상태}}
> $$
>
> - **항별 의미:** 좌변은 실제로 측정되는 반응이고, 우변은 $n$개 transit 단계를 모두 지난 마지막 하류 상태입니다.
> - **실무 직관:** $n$이 늘거나 각 단계의 $\tau$가 길어지면 $R_n$이 늦게 움직이므로 $R_{obs}$의 onset도 늦어집니다.
> - **오해 방지:** $R_{obs}$가 곧 수용체 점유율이나 혈장 농도라는 뜻이 아닙니다. 측정 반응은 cascade 끝의 하류 상태입니다.
...
<!-- SLIDE_START: S47 | title: C. PD35 앵커 — onset 지연과 $n$ 선택 | source_anchor: C. PD35 앵커 — onset 지연과 $n$ 선택 -->
> **핵심 메시지:** PD35에서 $n$ 선택은 실제 cascade 단계 수 선언이 아니라 데이터가 지지하는 지연 해상도의 선택입니다.
...
>
> $$
> \overbrace{n\times\tau}^{\text{총 transit time}}\approx \text{상수},\qquad \underbrace{EC_{50},\ E_0}_{\text{PD 민감도/기저 스케일}}\approx \text{robust across }n
> $$
>
> - **항별 의미:** 첫 식은 단계 수 $n$과 단계별 시간 $\tau$의 곱이 총 지연 시간을 근사적으로 보존함을 뜻합니다. 둘째 표현은 cascade 깊이를 바꾸어도 핵심 PD 스케일이 크게 흔들리지 않음을 나타냅니다.
> - **실무 직관:** $n$을 늘리면 각 단계의 $\tau$는 줄어들 수 있지만 총 지연은 비슷하게 유지될 수 있습니다. 동시에 $EC_{50}$가 안정적이면 delay 구조가 민감도 파라미터를 오염시키지 않는다는 신호입니다.
> - **오해 방지:** $n$이 증가한다고 실제 생물학적 단계 수를 더 정확히 찾았다고 단정하면 안 됩니다. 데이터는 총 지연을 주로 식별하고, 세부 분할은 trade-off를 가질 수 있습니다.
...
<!-- SLIDE_START: S48 | title: D. 구조적 직관 | source_anchor: D. 구조적 직관 -->
> **핵심 메시지:** Transit chain은 단일 1차 지연으로는 만들기 어려운 평평한 초기 구간과 지연된 sigmoid onset을 생성합니다.
...
<!-- SLIDE_START: S49 | title: E. 경계 조건 | source_anchor: E. 경계 조건 -->
> **핵심 메시지:** $n$과 $\tau$는 trade-off를 가지므로 총 지연과 생물학적 단계 수를 혼동하지 않아야 합니다.
...
<!-- SLIDE_START: S50 | title: F. 실무 체화 요약 (Practice Brief) — [EXPERT_INFERENCE, v3] | source_anchor: F. 실무 체화 요약 (Practice Brief) — [EXPERT_INFERENCE, v3] -->
> **핵심 메시지:** 이 slide의 판단 기준은 제목의 개념을 어떤 hidden state와 어떤 실무 검증으로 연결할지 결정하는 것입니다.
...
<!-- SLIDE_START: S51 | title: Recap 4 | source_anchor: Recap 4 -->
> **핵심 메시지:** Recap slide에서는 방금 배운 모델 family를 하나의 실무 선택 원칙으로 압축해야 합니다.
...
<!-- SLIDE_START: S52 | title: §5 — 혼동 쌍 해부 | source_anchor: §5 — 혼동 쌍 해부 -->
> **핵심 메시지:** 혼동 쌍은 같은 loop나 같은 단위를 가진 개념들을 무엇의 시계인지로 다시 분리하는 방어 장치입니다.
...
<!-- CONFUSION -->
<!-- SLIDE_START: S53 | title: Pair 1 — $k_{e0}$ vs $k_{out}$ vs $k_{off}$ | source_anchor: Pair 1 — $k_{e0}$ vs $k_{out}$ vs $k_{off}$ -->
> **핵심 메시지:** $k_{e0}$, $k_{out}$, $k_{off}$는 모두 time$^{-1}$라도 각각 다리, 수조, 빗장이라는 다른 대상을 움직입니다.
...
<!-- CONFUSION -->
<!-- SLIDE_START: S54 | title: Pair 2 — 효과구획 모델 vs 회전/간접반응 모델 | source_anchor: Pair 2 — 효과구획 모델 vs 회전/간접반응 모델 -->
> **핵심 메시지:** 효과구획과 turnover의 결정적 차이는 지연되는 것이 약물 농도인지 생리학적 반응 풀인지입니다.
...
>
> $$
> \underbrace{\frac{dC_e}{dt}=k_{e0}(C-C_e)}_{\text{약물 분포 지연}}\quad \text{vs}\quad \underbrace{\frac{dR}{dt}=k_{in}S(C)-k_{out}R}_{\text{반응 시스템 turnover 지연}}
> $$
>
> - **항별 의미:** 이 표 안의 두 식은 지연의 원인이 서로 다름을 대비합니다. 왼쪽은 생체상 농도 $C_e$가 혈장 농도 $C$를 따라가는 식이고, 오른쪽은 반응 풀 $R$이 생성과 소실의 균형으로 움직이는 식입니다.
> - **실무 직관:** $k_{e0}$가 작으면 약물 분포 지연이 커지고, $k_{out}$이 작으면 반응 시스템 자체가 느리게 회전합니다. 두 모델은 비슷한 loop를 만들 수 있지만 용량·반복투여·회복 행동이 달라집니다.
> - **오해 방지:** 두 식을 “둘 다 1차 지연”으로만 묶으면 안 됩니다. 왼쪽에서 지연되는 것은 약물 농도이고, 오른쪽에서 지연되는 것은 생리학적 반응입니다.
...
<!-- CONFUSION -->
<!-- SLIDE_START: S55 | title: Pair 3 — 반시계방향 vs 시계방향 이력현상 | source_anchor: Pair 3 — 반시계방향 vs 시계방향 이력현상 -->
> **핵심 메시지:** 같은 농도에서 두 번째 방문 반응이 더 강한지 약한지가 hysteresis 방향의 실무적 읽기법입니다.
...
<!-- CONFUSION -->
<!-- SLIDE_START: S56 | title: Pair 4 — 내성 vs 적응 | source_anchor: Pair 4 — 내성 vs 적응 -->
> **핵심 메시지:** 내성과 적응은 모두 시간의존적 변화처럼 보여도 시스템 피로와 항상성 재보정이라는 축이 다릅니다.
...
<!-- CONFUSION -->
<!-- SLIDE_START: S57 | title: Pair 5 — 단일 효과구획 링크 vs 신호전달 연쇄 | source_anchor: Pair 5 — 단일 효과구획 링크 vs 신호전달 연쇄 -->
> **핵심 메시지:** 단일 Link와 transit chain은 모두 지연을 설명하지만, 지연되는 것은 각각 생체상 농도와 하류 신호입니다.
...
<!-- SLIDE_START: S58 | title: §6 — 거장의 진단 렌즈(GOF 시그니처 사전) | source_anchor: §6 — 거장의 진단 렌즈(GOF 시그니처 사전) -->
> **핵심 메시지:** GOF 시그니처는 수렴한 모델을 다시 기전적으로 의심하기 위한 네 가지 실패 패턴입니다.
...
<!-- SLIDE_START: S59 | title: Signature 1 — $EC_{50}$ 용량 분기 패턴($EC_{50}$ dose-bifurcation pattern) | source_anchor: Signature 1 — $EC_{50}$ 용량 분기 패턴($EC_{50}$ dose-bifurcation pattern) -->
> **핵심 메시지:** $EC_{50}$가 용량별로 이동하면 수용체 민감도 변화보다 모델 family 오설정을 먼저 의심해야 합니다.
...
<!-- SLIDE_START: S60 | title: Signature 2 — Trough drift / carry-over 패턴 | source_anchor: Signature 2 — Trough drift / carry-over 패턴 -->
> **핵심 메시지:** 반복투여 trough drift는 PK 누적보다 먼저 PD system memory 누락 여부를 확인하게 만드는 신호입니다.
...
<!-- SLIDE_START: S61 | title: Signature 3 — 지연된 onset / S자형 onset 잔차 패턴 | source_anchor: Signature 3 — 지연된 onset / S자형 onset 잔차 패턴 -->
> **핵심 메시지:** 지연된 onset의 S자형 잔차는 단일 turnover가 cascade 깊이를 충분히 표현하지 못한다는 신호입니다.
...
<!-- SLIDE_START: S62 | title: Signature 4 — OFV / WRSS plateau 패턴 | source_anchor: Signature 4 — OFV / WRSS plateau 패턴 -->
> **핵심 메시지:** OFV/WRSS plateau는 더 복잡한 구조가 기전이 아니라 잡음을 흡수하기 시작했다는 중단 신호입니다.
...
<!-- SLIDE_START: S63 | title: §7 — 자기점검: 능동 회상 모듈 | source_anchor: §7 — 자기점검: 능동 회상 모듈 -->
> **핵심 메시지:** 자기점검의 목표는 수식을 외우는 것이 아니라 각 수식이 어떤 hidden state 판단을 지지하는지 회상하는 것입니다.

# §7 — 자기점검: 능동 회상 모듈

<!-- SELF-TEST -->
<!-- SLIDE_START: S64 | title: Q1 — 회상 | source_anchor: Q1 — 회상 -->
> **핵심 메시지:** $k_{1e}=k_{e0}$는 물리 법칙이 아니라 효과구획 식별가능성을 위한 표준 단순화입니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S65 | title: Q2 — 적용 | source_anchor: Q2 — 적용 -->
> **핵심 메시지:** 반응 $t_{max}$가 같아도 선형 PK를 먼저 확인하지 않으면 Link 모델 선택은 성급합니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S66 | title: Q3 — 오류 탐지 | source_anchor: Q3 — 오류 탐지 -->
> **핵심 메시지:** PD13과 G&W Table 3.10을 혼동하면 Link 오설정 사례와 반복투여 moderator 사례를 잘못 연결하게 됩니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S67 | title: Q4 — 기전 트리아지 | source_anchor: Q4 — 기전 트리아지 -->
> **핵심 메시지:** 반시계방향 loop는 최소 세 기전을 열어두므로 각 후보마다 별도 discriminator가 필요합니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S68 | title: Q5 — PD13 추론 | source_anchor: Q5 — PD13 추론 -->
> **핵심 메시지:** PD13 Model II의 기전적 핵심은 $k_{tol}$이 $k_{out}$보다 빠르다는 단일 부등식입니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S69 | title: Q6 — PD21 추론 | source_anchor: Q6 — PD21 추론 -->
> **핵심 메시지:** 비슷한 단위와 크기는 기전 동일성의 근거가 아니라 혼동 가능성의 경고입니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S70 | title: Q7 — PD35 추론 | source_anchor: Q7 — PD35 추론 -->
> **핵심 메시지:** WRSS 최저값보다 plateau와 parsimony가 PD35의 $n$ 선택을 더 안전하게 지도합니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S71 | title: Q8 — 보스 딜레마, 압축형 | source_anchor: Q8 — 보스 딜레마, 압축형 -->
> **핵심 메시지:** 계산 편의성보다 생물학적으로 정합적인 단일 민감도 구조가 모델 선택의 우선 기준입니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S72 | title: Q9 — 율속 단계 비교 (R&T 앵커) | source_anchor: Q9 — 율속 단계 비교 (R&T 앵커) -->
> **핵심 메시지:** 회복 시간은 약물 제거와 응고인자 turnover 중 더 느린 과정이 rate-limit합니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S73 | title: Q10 — 환자 간 이력현상 변이 | source_anchor: Q10 — 환자 간 이력현상 변이 -->
> **핵심 메시지:** $k_{1e}=k_{e0}$는 물리 법칙이 아니라 효과구획 식별가능성을 위한 표준 단순화입니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S74 | title: ⚡ Q11 — 보스 딜레마(보스 딜레마(Boss Dilemma)) ★★ [EXPERT_INFERENCE, v3] | source_anchor: ⚡ Q11 — 보스 딜레마(보스 딜레마(Boss Dilemma)) ★★ [EXPERT_INFERENCE, v3] -->
> **핵심 메시지:** Effect compartment와 turnover 사이의 선택은 피팅 난이도가 아니라 반복투여/용량검증에서 살아남는 기전으로 결정해야 합니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S75 | title: Q12 — 제한된 샘플링에서의 메커니즘 판별 ★★ [EXPERT_INFERENCE, v3.4] | source_anchor: Q12 — 제한된 샘플링에서의 메커니즘 판별 ★★ [EXPERT_INFERENCE, v3.4] -->
> **핵심 메시지:** $k_{1e}=k_{e0}$는 물리 법칙이 아니라 효과구획 식별가능성을 위한 표준 단순화입니다.
...
<!-- SLIDE_START: S76 | title: §8 — 메타 프레임과 큰 그림 | source_anchor: §8 — 메타 프레임과 큰 그림 -->
> **핵심 메시지:** 큰 그림의 최종 원칙은 rate constant의 단위가 아니라 지연되는 상태의 정체를 먼저 묻는 것입니다.

# §8 — 메타 프레임과 큰 그림

<!-- MASTER LENS -->
<!-- SLIDE_START: S77 | title: A. PK/PD 지식 그래프에서의 위치 | source_anchor: A. PK/PD 지식 그래프에서의 위치 -->
> **핵심 메시지:** 이 세션은 약물 시계, 시스템 시계, 신호 시계를 분리하는 중급 PK/PD 지식 그래프의 관문입니다.
...
<!-- SLIDE_START: S78 | title: B. 이 세션을 피상적으로 학습했을 때 발생하는 실패 모드 | source_anchor: B. 이 세션을 피상적으로 학습했을 때 발생하는 실패 모드 -->
> **핵심 메시지:** 피상적 학습의 실패는 대부분 모든 지연을 같은 Link 문제로 환원할 때 발생합니다.
...
<!-- SLIDE_START: S79 | title: C. 전문가 해석 지점 | source_anchor: C. 전문가 해석 지점 -->
> **핵심 메시지:** 전문가 해석은 ODE 작성 전에 데이터가 요구하는 hidden state를 먼저 보는 능력입니다.
...
<!-- SLIDE_START: S80 | title: Recap 5 | source_anchor: Recap 5 -->
> **핵심 메시지:** Recap slide에서는 방금 배운 모델 family를 하나의 실무 선택 원칙으로 압축해야 합니다.
