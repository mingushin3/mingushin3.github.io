...
> **핵심 메시지:** 이 장은 M1–M4를 먼저 잡고 혼동쌍과 자기 테스트로 설명 가능성을 확인하는 순서로 읽어야 합니다.
...
**그림 참고 사항**: `FIGURE_POINTER`는 교과서 그림을 직접 삽입하지 않습니다. 저작권상 교과서 원그림 대신 텍스트형 참고 콜아웃(callout)으로만 안내합니다. 학습자는 해당 교과서 페이지의 원그림을 따로 확인하면 됩니다.
...
<!-- SLIDE_START: S02 | title: Pre-Sprint — 모델링 철학·용어 | source_anchor: pre-sprint -->
> **핵심 메시지:** 이 세션의 중심은 용어 암기가 아니라 후속 PopPK/PK-PD 판단을 가능하게 하는 사고 언어를 세우는 것입니다.
...
<!-- SLIDE_START: S03 | title: 최신 큐레이션 맵 | source_anchor: curation-map -->
> **핵심 메시지:** Gabrielsson은 모델링 사고 순서를, Rowland & Tozer는 그 사고를 임상 용량 언어로 번역하는 기준을 제공합니다.
...
<!-- SLIDE_START: S04 | title: MUST 계층 | source_anchor: must-tier -->
> **핵심 메시지:** M1–M4는 후속 모델링 세션에서 반복적으로 호출될 최소 판단 노드입니다.

## MUST 계층(MUST tier)

| # | 개념 | 출처(Source) | 잠금 판정(Lock) |
|---|---|---|---|
| M1 | 모델링 회전목마(Modeling Carousel) & 5대 기능 장애(dysfunction) | Gabrielsson Ch.1 | 모델링은 fitting이 아니라 질문→설계→실험→EDA(← 그래프 중심 탐색적 데이터 분석) <!-- ANNOTATION -->→fitting→진단의 순환이라는 사고 순서를 고정합니다. |
...
| M3 | $E_{\max}$ / $C_{50}$(← 최대효과 절반 농도) <!-- ANNOTATION --> / Hill $\gamma$(← 곡선 가파름 계수) <!-- ANNOTATION --> | Rowland & Tozer Ch.2 | 농도-반응 관계를 $E_{\max}$, 효능(potency), 가파름(steepness)으로 분해하는 직접 효과(direct-effect) PD 모델의 골격입니다. |
| M4 | 전환(Turnover)(← 체내 물질의 생성·소실 순환) <!-- ANNOTATION --> | Rowland & Tozer Ch.2 | 반응 지연(response delay)과 내인성 시스템의 동적 평형(dynamic equilibrium)을 이해하는 간접 반응(indirect-response) 모델의 선행 개념입니다. |

<!-- SLIDE_START: S05 | title: CONTEXT 계층 — 압축 흡수 | source_anchor: context-tier -->
> **핵심 메시지:** 부가 개념은 독립 암기 대상이 아니라 M1–M4의 판단을 보조하는 배경 층으로 흡수해야 합니다.
...
<!-- SLIDE_START: S06 | title: §1 세션 헤더 | source_anchor: section-1 -->
> **핵심 메시지:** 모델링은 fitting이 아니라 임상 질문에서 진단까지 이어지는 순환 사고로 시작해야 합니다.
...
<!-- SLIDE_START: S07 | title: 로드맵 | source_anchor: roadmap -->
> **핵심 메시지:** M1은 workflow, M2는 임상 번역, M3는 농도-반응 형태, M4는 시간 지연을 담당합니다.
...
<!-- SLIDE_START: S08 | title: §2 개념 해부 카드 | source_anchor: section-2 -->
> **핵심 메시지:** 네 개의 카드는 모델링 판단을 workflow, linkage, direct PD, turnover라는 네 축으로 나눕니다.

# §2 — 개념 해부 카드(Concept Anatomy Cards)

---

<!-- SLIDE_START: S09 | title: Card M1 — Modeling Carousel | source_anchor: card-m1 -->
> **핵심 메시지:** fitting은 시작점이 아니라 질문·설계·실험·EDA 뒤에 오는 다섯 번째 단계입니다.
...
**왜 결정적인가**: 비선형 모델 적합(nonlinear model fitting)은 파라미터 초기 추정치(parameter initial estimate)를 요구합니다. EDA 없이 fitting을 시작하면 모델 구조(model structure)와 초기 추정치가 데이터에서 나오지 않습니다. 대신 사용자의 추측이나 소프트웨어 기본값이 출발점이 되고, 이때 국소 최솟값(local minimum)에 수렴할 위험이 커집니다. 핵심은 "fitting이 시작점이 아니다"라는 것입니다.

**체화해야 할 단 하나의 직관**: 모델링 회전목마(Modeling Carousel)에서 fitting은 6단계 중 5번째입니다. 앞의 네 단계 — 잠정 모델(tentative model), 설계(design), 실험(experiment), EDA — 가 먼저 모델 구조와 초기 추정치의 근거를 만듭니다. 따라서 fitting 결과의 해석 가능성은 fitting 전에 상당 부분 결정됩니다.
<!-- /MASTER LENS -->

> **숙달 메모(Mastery Note) — [CRUCIBLE_DERIVED]**
> EDA의 역할은 "plot을 한 번 보는 단계"가 아니라 비선형 fitting이 출발할 파라미터 탐색 공간(parameter-space basin)을 데이터 기반으로 좁히는 일입니다. 따라서 회전목마의 앞 단계는 형식적 절차가 아니라 fitting을 해석 가능하게 만드는 사전 정보 생산입니다.


<!-- SLIDE_START: S10 | title: M1-A 정의 | source_anchor: card-m1-a -->
> **핵심 메시지:** 모델링 회전목마는 잠정 모델에서 진단까지 이어지는 순환 workflow이며, 수식 fitting은 그 일부일 뿐입니다.
...
>
> $$
> \underbrace{K}_{\text{소실 속도상수}}=\frac{\underbrace{Cl}_{\text{청소율}}}{\underbrace{V}_{\text{분포용적}}},\quad
> \underbrace{AUC}_{\text{총 노출}}=\frac{\underbrace{D}_{\text{용량}}}{\underbrace{Cl}_{\text{청소율}}},\quad
> \underbrace{t_{1/2}}_{\text{반감기}}=\frac{\ln(2)}{\underbrace{K}_{\text{소실 속도상수}}}
> $$
>
> - **항별 의미:** 세 식의 좌변은 각각 소실 속도상수, 총 노출, 반감기라는 이차 파라미터입니다. 우변은 일차 파라미터 $Cl$, $V$, 투여량 $D$에서 이 해석값들이 어떻게 도출되는지 보여 줍니다.
> - **실무 직관:** $Cl$이 커지면 $K$는 커지고 $AUC$와 $t_{1/2}$는 작아지는 방향으로 움직입니다. $V$가 커지면 $K$는 작아져 반감기가 길어질 수 있습니다. 이 관계는 추정된 일차 파라미터를 노출과 시간 척도로 번역하게 해 줍니다.
> - **오해 방지:** $AUC$는 분포용적이 아니라 clearance에 의해 직접 결정되는 노출 요약값입니다. 반감기는 clearance만의 성질이 아니라 $Cl/V$로 정리된 시간 척도입니다.
...
>
> $$
> \underbrace{\hat C}_{\text{예측 농도}} = \underbrace{\frac{D}{V}}_{\text{초기 농도 척도}}\;\underbrace{\exp\left(-\underbrace{\frac{Cl}{V}}_{\text{소실 속도}}t\right)}_{\text{시간에 따른 감소}}
> $$
>
> - **항별 의미:** 좌변은 시간 $t$에서 모델이 예측하는 농도입니다. 우변의 $D/V$는 투여 직후 농도 스케일을, $Cl/V$는 농도가 얼마나 빠르게 감소하는지를, 지수항은 시간이 지날수록 남는 비율을 뜻합니다.
> - **실무 직관:** $D$가 커지면 예측 농도는 올라가고, $V$가 커지면 초기 농도는 낮아지며, $Cl$이 커지면 시간에 따른 감소가 빨라집니다. 이 식은 dose–concentration–time 관계를 가장 단순한 1구획 언어로 판단하게 해 줍니다.
> - **오해 방지:** $Cl/V$를 clearance 자체와 동일시하면 안 됩니다. 이 항은 clearance가 분포용적에 의해 시간 척도로 변환된 소실 속도입니다.
...
>
> $$
> \underbrace{C}_{\text{관측 농도}} = \underbrace{\hat C}_{\text{모델 예측}} + \underbrace{\varepsilon}_{\text{잔차 오차}}
> $$
>
> - **항별 의미:** 좌변은 실제 관측된 농도이고, 우변은 모델이 설명한 부분과 설명하지 못한 잔차 오차의 합입니다.
> - **실무 직관:** $\varepsilon$가 작고 무작위적으로 분포하면 모델이 관측값의 중심 경향을 잘 설명한다고 볼 수 있습니다. 반대로 잔차에 시간, 농도, 용량 의존 패턴이 남으면 구조 모델이나 오차 모델을 다시 의심해야 합니다.
> - **오해 방지:** $\varepsilon$는 “실험자가 잘못했다”는 뜻이 아닙니다. 측정 오차, 생물학적 변동, 모델이 설명하지 못한 구조가 함께 남은 항입니다.

<!-- SLIDE_START: S11 | title: M1-B 왜 모델링하는가 | source_anchor: card-m1-b -->
> **핵심 메시지:** 모델은 복잡할수록 좋은 것이 아니라 질문을 답하는 데 필요한 만큼만 복잡해야 합니다.
...
<!-- ANCHOR -->
즉, 모델은 지식 과시가 아니라 질문에 맞는 압축 장치입니다. "가장 복잡한 모델"이 아니라 "질문을 답하는 데 필요한 만큼만 복잡한 모델"이 원칙입니다 [G pp.3–4].
<!-- /ANCHOR -->

<!-- SLIDE_START: S12 | title: M1-C 구조적 필연성 | source_anchor: card-m1-c -->
> **핵심 메시지:** EDA는 예쁜 그림이 아니라 모델 구조와 초기 추정치를 생산해 국소 최솟값 위험을 줄이는 단계입니다.
...
<!-- TRENCH -->
**실무 꿀팁(Trench-level tip) [실무 추론]**: EDA의 산출물은 도표(plot) 자체가 아니라 "초기 추정치 후보 1표 + 비선형성 유무 1줄"입니다. 용량 보정 도표(dose-normalized plot)가 비선형성을 시사하면, 결론 전에 데이터셋의 용량/시간/사건 코딩 오류 가능성부터 배제합니다.
<!-- /TRENCH -->

<!-- SLIDE_START: S13 | title: M1-C2 GOF-only fallacy | source_anchor: card-m1-c2 -->
> **핵심 메시지:** 좋은 GOF는 모델이 옳다는 증거가 아니라 진단과 예측 검증으로 넘어가기 위한 출발점입니다.

### C-2. 그럴싸한 오해(Plausible Fallacy) — [EXPERT_INFERENCE, v3]

**그럴싸한 오해**: "GOF가 좋게 나왔으니 모델이 옳다."

**왜 틀렸는가**: Fitting(5단계)은 **가설 생성** 단계이고, 시뮬레이션과 의사결정이 회전목마의 목적입니다. 6단계 "결과 분석 및 모델 판별"은 fitting 이후에 반드시 따릅니다. VPC 없이 GOF만으로 진단 단계를 건너뛰면, fitting이 만든 가설이 검증 없이 의사결정으로 직행합니다. GOF 도표는 적합값(fitted value)이 관측값(observed value)에 가깝다는 것만 보여 줄 뿐 — 예측 타당성(predictive validity)은 별도로 평가해야 합니다.

**실무에서 어떻게 드러나는가 (GOF-Only Submission Trap)**: VPC 없이 GOF만으로 규제 제출을 강행하면, 심사관이 모델의 예측 성능(predictive performance)을 확인할 수 없다는 이유로 결함 통지서(deficiency letter)를 발행하는 패턴으로 이어집니다. 회전목마의 진단 단계(6단계) 생략이 만드는 가장 흔한 규제 실패 양식입니다.

<!-- SLIDE_START: S14 | title: M1-D 가정과 경계 조건 | source_anchor: card-m1-d -->
> **핵심 메시지:** 데이터가 구분해 주는 것과 모델이 설명한다고 주장하는 것을 분리해야 합니다.
...
<!-- SLIDE_START: S15 | title: M1-E 한계 | source_anchor: card-m1-e -->
> **핵심 메시지:** 이 카드는 운영 기준보다 서로 다른 초기 추정치로 국소 최솟값을 확인해야 한다는 원칙을 잠급니다.
...
<!-- SLIDE_START: S16 | title: M1-F 인지 잠금 | source_anchor: card-m1-f -->
> **핵심 메시지:** 질문·그래프·알고리즘·생물학·실무 문서화가 함께 맞아야 fitting 결과를 해석할 수 있습니다.
...
<!-- SLIDE_START: S17 | title: M1-G 제텔카스텐 원자 | source_anchor: card-m1-g -->
> **핵심 메시지:** M1의 최종 기억 단위는 ‘fitting은 Step 5이며 EDA가 구조와 초기값을 만든다’입니다.
...
<!-- RECAP -->
**M1 요약(Recap)**: 모델링의 첫 원칙은 소프트웨어 실행이 아니라 질문과 데이터 구조를 먼저 보는 것입니다. EDA를 건너뛴 fitting은 수렴할 수는 있어도, 왜 그 모델이어야 하는지 설명하지 못합니다.
<!-- /RECAP -->

> **▶ 실무 체화 요약(Practice Brief) — M1 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가**: 모델링 회전목마 없이는 fitting 결과가 왜 그 형태인지 설명하지 못하고, 모델 선별(model selection)이 근거 없는 주관적 선택이 됩니다.
> - **흐름상 역할**: 이 카드는 후속 GOF, 모델 선별, VPC 해석 세션의 사고 순서를 미리 잠그는 관문입니다.
> - **체화 꿀팁**: "왜 EDA가 없으면 초기 추정치를 만들 수 없는가"를 비선형 fitting 알고리즘의 수학적 요구에서 스스로 도출할 수 있을 때 M1이 내재화된 것입니다. 암기가 아니라 구조적 필연에서 이유를 끌어내야 합니다.
> - **실무 활용**: 새 데이터셋을 받을 때 가장 먼저 용량 보정 농도-시간 도표(dose-normalized concentration-time plot)를 그리는 EDA 루틴이 곧 M1이 실무로 출력되는 형태입니다. 이 도표 없이 제어구문(control stream)부터 여는 것이 M1이 경고한 기능 장애입니다.

<!-- SLIDE_START: S18 | title: Figure pointer — G Fig.1.2 | source_anchor: figure-g-1-2 -->
> **핵심 메시지:** G Fig.1.2는 EDA 기반 초기 추정치가 parameter-space basin과 local minimum 위험을 어떻게 바꾸는지 보여 주는 안전장치입니다.

<!-- FIGURE_POINTER -->
Source: Gabrielsson & Weiner 5e, p.6, Figure 1.2
Why this matters: 이 그림은 국소 최솟값(local minimum) 문제를 눈에 보이게 해 줍니다. EDA에서 얻은 초기 추정치(initial estimates)가 장식적 절차가 아니라, fitting 알고리즘이 파라미터 공간(parameter space)의 어느 영역으로 들어갈지를 결정한다는 점을 보여 줍니다.
When to look: Card M1, 특히 §C 구조적 필연성(Structural Necessity)을 읽은 뒤 확인하면 됩니다.
Learner instruction: 노란색 시작점(yellow starting points)과 빨간색 최종 추정치(red final estimates)에 집중해서 보세요. 어떤 시작점이 프로그램을 잘못된 홈(hollow)으로 보내는지, 그리고 왜 EDA가 안전장치가 되는지 스스로 설명해 보면 됩니다.
<!-- /FIGURE_POINTER -->

---

<!-- SLIDE_START: S19 | title: Card M2 — PK/PD Linkage | source_anchor: card-m2 -->
> **핵심 메시지:** 용량 결정은 dose 숫자가 아니라 dose가 만든 노출과 그 노출이 만든 반응으로 설명해야 합니다.
...
**왜 결정적인가**: 임상 처방 질문은 "얼마나, 얼마나 자주, 얼마나 오래, 어떤 투여 경로(route)와 제형(dosage form)으로 줄 것인가"입니다. 이 질문의 답은 용량(dose) 숫자 자체에 있지 않습니다. 대신, dose가 만든 노출-시간 프로파일(exposure-time profile), 그리고 그 노출이 만든 원하는/원치 않는 반응-시간 프로파일(desired/adverse response-time profile)로 답해야 합니다 [T p.3].

**체화해야 할 단 하나의 직관**: PK와 PD는 두 개의 독립 과목이 아니라 하나의 인과 사슬(causal chain)입니다. **투여 요법(dosage regimen) → 체내 노출(exposure within body) → 원하는/원치 않는 효과(desired/adverse effects)**가 연결되어야 합니다. 그래야 요법을 왜 바꾸는지 설명할 수 있습니다.
<!-- /MASTER LENS -->

> **실무 렌즈(Practice Lens) — [TEXTBOOK_DERIVED]**
> 용량 질문을 받을 때 먼저 용량 숫자를 고정하지 말고 노출-시간 프로파일과 반응-시간 프로파일을 분리해서 생각하면 됩니다. 같은 요법이라도 입력(input), 변이(variability), 순응도(adherence)가 노출을 바꾸면 치료역(therapeutic window) 안에 머무는 시간도 달라집니다.


<!-- SLIDE_START: S20 | title: M2-A 정의 | source_anchor: card-m2-a -->
> **핵심 메시지:** PK는 입력에서 농도-시간을, PD는 농도에서 효과-시간을 설명하며 치료 판단에는 둘의 연결이 필요합니다.
...
<!-- SLIDE_START: S21 | title: M2-B 치료역 | source_anchor: card-m2-b -->
> **핵심 메시지:** 치료역은 용량 범위가 아니라 충분한 효과와 허용 가능한 위해가 공존하는 노출 범위입니다.
...
<!-- ANCHOR -->
[해석/교육용 형식화] 실무적으로는 치료역을 "치료 반응 확률이 충분하고 이상 사건 확률이 지나치지 않은 노출 범위"로 읽으면 됩니다. 단, 이 확률 집합 표기는 원문 수식이 아니라 교육용 형식화(formalization)입니다.
<!-- /ANCHOR -->

<!-- SLIDE_START: S22 | title: M2-C 구조적 필연성 | source_anchor: card-m2-c -->
> **핵심 메시지:** Digoxin과 morphine의 요법 차이는 투여 빈도가 아니라 제거 속도와 치료역 운영의 차이로 설명해야 합니다.
...
<!-- SLIDE_START: S23 | title: M2-D 임상적 중요성 | source_anchor: card-m2-d -->
> **핵심 메시지:** PK/PD 연결은 지식 설명이 아니라 요법 설계의 안전성과 합리성을 확보하는 임상 도구입니다.
...
<!-- SLIDE_START: S24 | title: M2-E 현실 세계의 왜곡 | source_anchor: card-m2-e -->
> **핵심 메시지:** 측정 부위, 순응도, 변이는 같은 처방이 같은 노출과 반응을 보장하지 않는 이유입니다.
...
- **순응도 문제(Adherence problem)**: 처방된 요법이 실제 노출로 그대로 이어지지 않습니다. 1일 1회 항고혈압제 코호트에서 1년 말 처방약을 지속한 비율은 약 50%였습니다 [T p.12].
...
- **Warfarin 유전학 교정**: 이 PDF 범위에서는 CYP2C9/CYP2C19 다형성(polymorphisms)과 vitamin K epoxide reductase 기전이 언급됩니다. VKORC1 표기는 이 범위의 원문 표현이 아니므로 사용하지 않습니다 [T p.10].

<!-- TRENCH -->
**실무 꿀팁(Trench-level tip) [실무 추론]**: 순응도 모니터링이 약하면 겉보기 PK 변이(apparent PK variability)가 실제 누락 투여(missed dose)의 가공물(artifact)일 수 있습니다. 이때 모델은 환자 생리 차이가 아니라 입력 이력 오류를 변이로 흡수할 수 있습니다.
<!-- /TRENCH -->

<!-- SLIDE_START: S25 | title: M2-F 제텔카스텐 원자 | source_anchor: card-m2-f -->
> **핵심 메시지:** M2의 최종 기억 단위는 ‘dose regimen은 exposure-time과 response-time으로 번역될 때만 유용하다’입니다.
...
<!-- RECAP -->
**M2 요약(Recap)**: PK/PD 연결의 핵심은 "용량이 아니라 작용 부위 농도(concentration at the site of action)가 반응을 구동(drive)한다"는 점입니다. 치료역은 이 연결이 임상적으로 작동해야 하는 안전 범위입니다.
<!-- /RECAP -->

> **▶ 실무 체화 요약(Practice Brief) — M2 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가**: PK/PD 연결을 놓치면 PK 파라미터를 추정하고도 용량 결정(dosing decision)을 내릴 논리적 근거가 빠져 있게 됩니다. 숫자만 있고 해석이 없는 상태입니다.
> - **흐름상 역할**: 이 카드는 구획 PK, TDM, 노출-반응(exposure-response) 세션이 공유하는 용량→노출→반응 번역 문법을 선행 잠금합니다.
> - **체화 꿀팁**: 어떤 처방을 볼 때 "이 용량이 만드는 노출-시간 프로파일은 무엇이고, 그 프로파일이 치료역 안에 머무는 시간은 얼마나 되는가"를 자동으로 묻게 되면 M2가 내재화된 신호입니다.
> - **실무 활용**: 규제 보고서에서 PK 섹션과 PD 섹션을 완전히 분리하지 않고 PK/PD 연결 섹션으로 연결하는 것이 M2가 실무 문서에서 나타나는 형태입니다.

<!-- SLIDE_START: S26 | title: Figure pointer — T Fig.1-3 | source_anchor: figure-t-1-3 -->
> **핵심 메시지:** T Fig.1-3은 PK와 PD가 effect-time profile로 연결되어야 용량 판단이 완성된다는 점을 시각적으로 고정합니다.

<!-- FIGURE_POINTER -->
Source: Rowland & Tozer 5e, p.5, Figure 1-3
Why this matters: 이 그림은 PK와 PD를 서로 분리된 과목처럼 잘못 대응시키는 흔한 오해를 막아 줍니다. 치료적으로 유용한 예측을 하려면 두 관계가 효과-시간 프로파일(effect-time profile)로 연결되어야 한다는 점을 보여 줍니다.
When to look: Card M2, 특히 §A 정의(Formal Definition)를 읽은 뒤 확인하면 됩니다.
Learner instruction: 농도 대 시간(concentration vs time)에서 농도 대 효과(concentration vs effect)로, 다시 효과 대 시간(effect vs time)으로 이어지는 경로를 따라가 보세요. 둘 중 어느 한 조각만으로는 왜 용량 결정(dosing)에 충분하지 않은지 설명할 수 있을 때까지 그림을 확인하면 됩니다.
<!-- /FIGURE_POINTER -->

---

<!-- SLIDE_START: S27 | title: Card M3 — Emax / C50 / Hill γ | source_anchor: card-m3 -->
> **핵심 메시지:** $E_{\max}$는 천장, $C_{50}$은 효능, Hill $\gamma$는 가파름을 분리해 용량 적정 판단을 가능하게 합니다.
...
**왜 결정적인가**: 농도-반응 관계를 말할 때 "효과가 증가한다"만으로는 부족합니다. $E_{\max}$는 천장(ceiling)을, $C_{50}$은 효능(potency)을, $\gamma$는 곡선 가파름(steepness)을 따로 보여 줍니다. 이 세 가지를 분리하지 못하면 용량 적정(dose titration), 평가변수 선택(endpoint selection), 안전역 해석(safety margin interpretation)이 흐려집니다.

**체화해야 할 단 하나의 직관**: $C_{50}$이 낮다고 항상 좋은 약이 아닙니다. 같은 효능이라도 $E_{\max}$가 낮으면 충분한 임상 반응(clinical response)에 도달하지 못할 수 있습니다. 또한 $\gamma$가 너무 크면 작은 농도 변화가 반응 역치(response threshold)를 급격히 넘게 만든다 [T pp.40–43].
<!-- /MASTER LENS -->

> **실무 렌즈(Practice Lens) — [CRUCIBLE_DERIVED]**
> $\gamma$는 곡선 적합 파라미터(curve-fitting parameter)에 그치지 않습니다. $\gamma$가 클수록 같은 $C_{50}$ 주변에서도 작은 농도 변화가 반응 판단을 더 크게 바꾸므로, 적정 친화적 반응(titration-friendly response)인지 역치형 반응(threshold-like response)인지 먼저 구분해야 합니다.


<!-- SLIDE_START: S28 | title: M3-A 정의 | source_anchor: card-m3-a -->
> **핵심 메시지:** Hill/$E_{\max}$ 식은 농도-반응을 천장·효능·가파름으로 분해하는 직접 효과 PD의 기본 문법입니다.
...
>
> $$
> \underbrace{E}_{\text{예측 효과}} = \frac{\overbrace{E_{\max}}^{\text{효과 천장}}\;\overbrace{C^{\gamma}}^{\text{농도 구동력}}}{\underbrace{C_{50}^{\gamma}}_{\text{효능 기준점}}+\underbrace{C^{\gamma}}_{\text{현재 농도 항}}}
> $$
>
> - **항별 의미:** 좌변은 농도 $C$에서 예측되는 반응입니다. $E_{\max}$는 도달 가능한 최대 반응, $C_{50}$은 최대 반응의 절반을 만드는 농도, $\gamma$는 $C_{50}$ 주변에서 곡선이 얼마나 급하게 변하는지를 뜻합니다.
> - **실무 직관:** $E_{\max}$가 커지면 효과의 천장이 올라가고, $C_{50}$이 낮아지면 같은 효과에 필요한 농도가 낮아지며, $\gamma$가 커지면 작은 농도 변화가 더 큰 반응 차이로 이어집니다. 이 식은 potency, ceiling, steepness를 분리해 용량 적정 가능성을 판단하게 해 줍니다.
> - **오해 방지:** $C_{50}$이 낮다고 항상 임상적으로 우수한 것은 아닙니다. 충분한 $E_{\max}$와 안전한 $\gamma$ 범위가 함께 확인되어야 합니다.
...
>
> $$
> \underbrace{C_{\max}=96\,\mu g/L}_{\text{관찰된 최고 농도}},\quad
> \underbrace{t_{\max}=3.0\,hr}_{\text{최고 농도 도달 시간}},\quad
> \underbrace{AUC}_{\text{총 전신 노출}}
> $$
>
> - **항별 의미:** 좌변 격의 세 노출 지표는 각각 최고 농도, 최고 농도에 도달한 시간, 전체 농도-시간 노출을 요약합니다. 값이 명시된 $C_{\max}$와 $t_{\max}$는 Fig.2-1 예시의 관찰 앵커입니다.
> - **실무 직관:** $C_{\max}$가 높으면 peak-related effect 또는 toxicity를, $t_{\max}$가 빠르면 onset을, $AUC$가 크면 전체 노출 부담을 먼저 의심하게 됩니다. 어떤 지표가 중요한지는 임상 목표와 노출-반응 관계에 따라 달라집니다.
> - **오해 방지:** $C_{\max}$, $t_{\max}$, $AUC$는 서로 대체 가능한 하나의 숫자가 아닙니다. 같은 $AUC$라도 peak와 onset은 다를 수 있습니다.

<!-- SLIDE_START: S29 | title: M3-B γ의 의미 | source_anchor: card-m3-b -->
> **핵심 메시지:** $\gamma$가 커질수록 20–80% 반응 농도 범위가 압축되어 작은 농도 변화가 큰 임상 차이를 만들 수 있습니다.
...
>
> $$
> \underbrace{\gamma=1}_{\text{완만한 곡선}}:\quad \underbrace{C_{20}=0.25C_{50}}_{\text{20\% 반응 농도}},\; \underbrace{C_{80}=4C_{50}}_{\text{80\% 반응 농도}}
> $$
>
> - **항별 의미:** 좌변의 $\gamma=1$은 표준적인 완만한 Hill 곡선 조건을 정의하고, 우변은 20% 반응과 80% 반응을 만드는 농도 위치를 $C_{50}$ 기준으로 보여 줍니다.
> - **실무 직관:** 20%에서 80%까지 올라가려면 농도가 16배 범위로 넓게 움직여야 합니다. 따라서 반응이 비교적 서서히 변해 증상 기반 적정이 상대적으로 자연스럽습니다.
> - **오해 방지:** 이 식은 모든 약물의 고정 규칙이 아니라 Hill 식에서 $\gamma=1$일 때의 농도-반응 기하를 보여 주는 해석 도구입니다.
...
>
> $$
> \underbrace{\gamma=2}_{\text{가파른 곡선}}:\quad \underbrace{C_{20}=0.5C_{50}}_{\text{20\% 반응 농도}},\; \underbrace{C_{80}=2C_{50}}_{\text{80\% 반응 농도}}
> $$
>
> - **항별 의미:** 좌변의 $\gamma=2$는 더 가파른 Hill 곡선 조건이고, 우변은 20%와 80% 반응 농도가 $C_{50}$ 주변으로 압축된다는 것을 보여 줍니다.
> - **실무 직관:** 20%에서 80%까지 필요한 농도 범위가 4배로 줄어듭니다. 작은 농도 변화가 큰 반응 변화로 이어지므로 역치형 반응, 안전역, 적정 간격을 더 신중하게 봐야 합니다.
> - **오해 방지:** $\gamma$가 커진다는 말은 단순히 그래프가 예쁘게 휘어진다는 뜻이 아니라, 임상적으로 허용 가능한 농도 조절 폭이 좁아질 수 있다는 뜻입니다.
...
<!-- ANCHOR -->
$\gamma$는 단순한 곡선 모양이 아니라 처방 사고를 바꿉니다. $\gamma \approx 1$이면 증상 기반 적정(symptom-based titration)이 상대적으로 자연스럽습니다. 반대로 반응이 가파르거나(steep) 역치형(threshold-like)이면 작은 농도 변화가 임상 사건(clinical event)을 바꿀 수 있습니다.
<!-- /ANCHOR -->

<!-- SLIDE_START: S30 | title: M3-C 실제 데이터 앵커 | source_anchor: card-m3-c -->
> **핵심 메시지:** Propranolol은 등급형 Hill 반응을, alfentanil은 이분형 반응 확률 해석을 각각 보여 줍니다.
...
<!-- SLIDE_START: S31 | title: M3-D 측정과 분석 경계 | source_anchor: card-m3-d -->
> **핵심 메시지:** 총 농도와 활성 물질이 일치하지 않으면 정확한 수식도 잘못된 PK/PD 연결로 이어질 수 있습니다.
...
<!-- TRENCH -->
**실무 꿀팁(Trench-level tip) [실무 추론]**: 용량 범위가 $C_{50}$ 주변에만 몰려 있고 고원(plateau) 정보가 없으면 $\gamma$를 자유롭게 추정하기보다 절약적 가정(parsimonious assumption)을 먼저 검토합니다. "$\gamma$를 추정했다"는 사실보다 "데이터가 $\gamma$를 식별(identify)할 수 있는가"가 먼저입니다.
<!-- /TRENCH -->

<!-- SLIDE_START: S32 | title: M3-E 한계 | source_anchor: card-m3-e -->
> **핵심 메시지:** 임상적 $E_{\max}$는 약리학적 최대치보다 안전성 한계에 의해 먼저 제한될 수 있습니다.
...
<!-- SLIDE_START: S33 | title: M3-F 진단 잠금 | source_anchor: card-m3-f -->
> **핵심 메시지:** 반응 척도가 등급형인지 이분형인지, 그리고 $\gamma$를 식별할 데이터가 있는지 먼저 판정해야 합니다.
...
<!-- SLIDE_START: S34 | title: M3-G 제텔카스텐 원자 | source_anchor: card-m3-g -->
> **핵심 메시지:** M3의 최종 기억 단위는 ‘ceiling, potency, steepness, endpoint type을 분리하라’입니다.
...
<!-- RECAP -->
**M3 요약(Recap)**: Hill 방정식은 농도-반응의 "현재 순간"을 정량화합니다. 하지만 측정 농도, 활성 물질, 평가변수 유형, $\gamma$ 식별 가능성(identifiability)을 함께 보지 않으면 정확한 수식도 잘못된 용량 해석으로 이어집니다.
<!-- /RECAP -->

> **▶ 실무 체화 요약(Practice Brief) — M3 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가**: $E_{\max}$/$C_{50}$/$\gamma$를 한 덩어리로 암기하면 평가변수 유형이 바뀔 때 모델 선택이 흔들리고, $\gamma$ 식별 가능성 문제를 인식하지 못한 채 잡음을 파라미터로 추정합니다.
> - **흐름상 역할**: 이 카드는 직접 효과 PD, 노출-반응 분석(exposure-response analysis), 용량 적정 설계 세션의 "천장-효능-가파름" 3축 언어를 선행 잠금합니다.
> - **체화 꿀팁**: propranolol 예시($\gamma\approx1$, $C_{50}=5.3\,\mu g/L$)를 기준으로 $\gamma$ 값을 2, 3으로 올리면 20%–80% 반응 범위가 어떻게 압축되는지 손으로 계산해 보면 — $\gamma$가 단순 지수가 아니라 적정 논리를 바꾼다는 것이 체감됩니다.
> - **실무 활용**: 노출-반응 보고서에서 $C_{50}$ 하나만 보고하지 않고 $E_{\max}$, $\gamma$, 평가변수 유형(등급형 vs 이분형)을 함께 명시하고 $\gamma$ 식별 가능성을 언급하는 것이 M3가 규제 문서에서 요구하는 수준입니다.

<!-- SLIDE_START: S35 | title: Figure pointer — T Fig.2-16 | source_anchor: figure-t-2-16 -->
> **핵심 메시지:** T Fig.2-16은 Hill $\gamma$가 추상적 지수가 아니라 $C_{50}$ 주변 반응 민감도를 바꾸는 임상 판단 변수임을 보여 줍니다.

<!-- FIGURE_POINTER -->
Source: Rowland & Tozer 5e, p.41, Figure 2-16
Why this matters: 이 그림은 $\gamma$를 추상적인 지수(exponent)가 아니라 반응 민감도(response sensitivity)의 눈에 보이는 변화로 바꾸어 줍니다. 가파름(steepness)이 달라질 때 같은 $C_{50}$라도 매우 다르게 작동할 수 있음을 보여 줍니다.
When to look: Card M3, 특히 §B $\gamma$가 실제로 바꾸는 것(What $\gamma$ Really Changes)을 읽은 뒤 확인하면 됩니다.
Learner instruction: 고원(plateau)만 보지 말고 $C_{50}$ 주변의 곡선을 비교해 보세요. 그다음 20–80% 구간을 비교하면서 $\gamma$가 왜 용량 적정(titration) 논리를 바꾸는지 확인하면 됩니다.
<!-- /FIGURE_POINTER -->

---

<!-- SLIDE_START: S36 | title: Card M4 — Turnover | source_anchor: card-m4 -->
> **핵심 메시지:** 농도 최고점과 효과 최고점이 분리되면 직접 효과만이 아니라 내인성 pool의 turnover를 의심해야 합니다.
...
**왜 결정적인가**: 어떤 반응은 약물 농도가 최고점을 찍은 뒤 훨씬 늦게 나타납니다. 이 지연은 단순 분포 지연(distribution delay)일 수도 있고, 내인성 시스템의 전환(turnover) 때문일 수도 있습니다. 둘을 구분하지 못하면 직접 효과 Hill 모델로 설명할 수 없는 시간을 억지로 fitting하게 됩니다.

**체화해야 할 단 하나의 직관**: 정상 상태(steady state)는 움직이지 않는 상태가 아닙니다. **입력 속도(input rate)와 출력 속도(output rate)가 같아 풀 크기(pool size)가 일정하게 보이는 동적 평형(dynamic equilibrium)**입니다 [T p.45]. 따라서 약물이 입력을 바꾸는지 출력을 바꾸는지에 따라 반응 시간 경과(response time course)가 달라집니다.
<!-- /MASTER LENS -->

> **판단 렌즈(Judgment Lens) — [CRUCIBLE_DERIVED]**
> 관찰 데이터에서 농도 최고점과 효과 최고점이 분리되면, 단순 직접 효과 모델만으로 끝내기 전에 전환/지연 사고를 켜야 합니다. 이 판단은 모델을 복잡하게 만들기 위한 것이 아니라 반응이 어느 시스템 풀(system pool)을 통해 나타나는지 묻는 출발점입니다.


<!-- SLIDE_START: S37 | title: M4-A 정의 | source_anchor: card-m4-a -->
> **핵심 메시지:** 정상 상태는 정지 상태가 아니라 입력과 출력이 같아 pool size가 일정해 보이는 동적 평형입니다.
...
>
> $$
> \underbrace{k_t}_{\text{분획 전환 속도}} = \frac{\underbrace{R_t}_{\text{단위시간 전환량}}}{\underbrace{A_{ss}}_{\text{정상상태 풀 크기}}}
> $$
>
> - **항별 의미:** 좌변은 풀 전체 중 단위 시간에 어느 분획이 교체되는지를 정의합니다. 우변은 절대 전환량 $R_t$를 풀 크기 $A_{ss}$로 나누어 시간 척도의 속도로 바꿉니다.
> - **실무 직관:** $R_t$가 커지면 $k_t$는 증가하지만, 같은 $R_t$라도 $A_{ss}$가 크면 $k_t$는 작아집니다. 따라서 “많이 움직인다”와 “빠르게 바뀐다”를 구분할 수 있습니다.
> - **오해 방지:** $R_t$가 크다고 곧 빠른 turnover라고 단정하면 안 됩니다. 큰 pool에서는 큰 양이 오가도 분획 전환은 느릴 수 있습니다.
...
>
> $$
> \underbrace{t_t}_{\text{전환 시간}} = \frac{\underbrace{A_{ss}}_{\text{정상상태 풀 크기}}}{\underbrace{R_t}_{\text{전환 속도}}}
> $$
>
> - **항별 의미:** 좌변은 전체 pool이 한 번 교체되는 데 해당하는 시간 척도입니다. 우변은 pool의 크기를 단위 시간당 전환량으로 나누어 시간으로 변환합니다.
> - **실무 직관:** $A_{ss}$가 커지면 전환 시간은 길어지고, $R_t$가 커지면 전환 시간은 짧아집니다. 이 식은 반응 회복이나 지연 효과의 시간 척도를 직관적으로 판단하게 해 줍니다.
> - **오해 방지:** $t_t$는 개별 분자가 정확히 그 시간만 머문다는 뜻이 아니라, pool 수준에서 해석하는 평균적인 전환 시간 척도입니다.
...
>
> $$
> \underbrace{t_t}_{\text{전환 시간}} = \frac{1}{\underbrace{k_t}_{\text{분획 전환 속도}}}
> $$
>
> - **항별 의미:** 좌변은 전환 시간이고, 우변은 분획 전환 속도의 역수입니다. 분획 속도가 빠를수록 전환 시간은 짧아집니다.
> - **실무 직관:** $k_t$가 2배가 되면 $t_t$는 절반이 됩니다. 이 관계는 지연 반응이 얼마나 천천히 회복될지 또는 얼마나 빠르게 새 정상 상태로 접근할지를 판단하게 해 줍니다.
> - **오해 방지:** 이 식은 $R_t$만으로 과정 속도를 판단할 수 없음을 다시 보여 줍니다. 속도 판단은 pool 크기까지 반영된 $k_t$와 $t_t$로 해야 합니다.
...
<!-- SLIDE_START: S38 | title: M4-B 핵심 예시 | source_anchor: card-m4-b -->
> **핵심 메시지:** Warfarin, paclitaxel, 체수분 예시는 농도 변화와 반응 변화의 시간 척도가 다를 수 있음을 보여 줍니다.

### B. 핵심 예시(Key Examples)

**핵심 예시 — warfarin [T p.8]**
Warfarin sodium 1.5 mg/kg 단회 경구 투여를 5명의 남성 지원자에게 투여했을 때, 혈장 warfarin 농도는 빠르게 올라가지만 prothrombin complex activity 반응은 느리게 변합니다. 이 분리는 농도-시간 프로파일만으로 반응-시간 프로파일을 설명할 수 없음을 보여 줍니다.

**핵심 예시 — paclitaxel [T pp.8–9]**
Paclitaxel은 체내에서 약 2일 내 제거되지만 백혈구 수(leukocyte count) 변화는 매우 느리며, 최저점(nadir)은 1주 이후에 나타나고 기저선 회복(baseline recovery)은 약 3주가 걸릴 수 있습니다. 이것이 전환 구동 반응(turnover-driven response)의 전형적 시그니처입니다.

**핵심 예시 — 총 체수분(total body water) [T pp.45–46]**
총 체수분은 약 42 L이고, 평균 전환 속도는 2.5 L/day, 분획 전환 속도는 0.06 day⁻¹, 전환 시간은 17 days입니다. 사막 환경(desert-like condition)에서는 풀 크기가 거의 일정해도 전환 속도가 21 L/day까지 증가할 수 있고, 이때 분획 전환 속도는 0.5 day⁻¹, 전환 시간은 2 days가 됩니다.

<!-- SLIDE_START: S39 | title: M4-C 구조적 필연성 | source_anchor: card-m4-c -->
> **핵심 메시지:** 전환 모델링의 핵심 질문은 무엇이 보존되고 무엇이 바뀌는 pool dynamics인가입니다.
...
<!-- ANCHOR -->
전환 모델링에서 더 중요한 질문은 "$R_t$, $k_t$, $t_t$ 중 무엇을 외웠는가"가 아닙니다. 핵심 질문은 "이 시스템에서는 풀 크기가 보존되는가, 분획 전환이 보존되는가, 아니면 전환 속도 자체가 바뀌는가"입니다. 체수분 예시는 풀 크기가 피드백으로 유지될 수 있음을, 콜레스테롤/백혈구 예시는 풀 크기 자체가 변할 수 있음을 각각 보여 줍니다 [T p.46].
<!-- /ANCHOR -->

<!-- SLIDE_START: S40 | title: M4-D 경계와 범위 | source_anchor: card-m4-d -->
> **핵심 메시지:** 이 장의 목적은 ODE 코딩이 아니라 지연 반응을 turnover 관점으로 해석하는 판단 기준을 세우는 것입니다.
...
<!-- TRENCH -->
**실무 꿀팁(Trench-level tip) [실무 추론]**: 희소 반응-시간 데이터(sparse response-time data)에서 입력과 출력을 동시에 자유롭게 추정하면 파라미터가 서로 보상할 수 있습니다. 먼저 기저선(baseline), 최고점 시점(peak timing), 회복 시점(recovery timing)을 보고 어떤 전환 구성 요소가 데이터로 식별 가능한지 판단합니다.
<!-- /TRENCH -->

<!-- SLIDE_START: S41 | title: M4-E 진단 잠금 | source_anchor: card-m4-e -->
> **핵심 메시지:** 과정의 빠르기는 절대 전환량보다 분획 전환 속도와 전환 시간이 더 잘 말합니다.
...
<!-- SLIDE_START: S42 | title: M4-F 제텔카스텐 원자 | source_anchor: card-m4-f -->
> **핵심 메시지:** M4의 최종 기억 단위는 ‘steady state는 dynamic equilibrium이며 delayed response는 turnover를 의심하게 한다’입니다.
...
<!-- RECAP -->
**M4 요약(Recap)**: 전환은 "반응이 늦다"를 수학적으로 다루기 전, 무엇이 천천히 바뀌고 있는지 묻는 생리학적 언어입니다. 농도 최고점과 효과 최고점이 분리되면 M4 사고가 켜져야 합니다.
<!-- /RECAP -->

> **▶ 실무 체화 요약(Practice Brief) — M4 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가**: 전환 개념 없이는 지연 반응을 부적합(poor fit) 또는 데이터 오류로 오해하고, 간접 반응 모델이 필요한 시점을 놓칩니다.
> - **흐름상 역할**: 이 카드는 간접 반응 모델, 질병 진행 모델, TMDD 하류 세션이 공유하는 "효과 최고점 지연의 원인 분류" 판단 기준을 선행 잠금합니다.
> - **체화 꿀팁**: 체수분 전환 예시의 수치($A_{ss}=42\,L$, $R_t=2.5\,L/day$ → $k_t=0.06\,day^{-1}$, $t_t=17\,days$)를 사막 조건($R_t=21\,L/day$ → $t_t=2\,days$)과 비교하며 풀 크기가 고정돼도 $k_t$와 $t_t$가 얼마나 크게 달라지는지 직접 손으로 계산하면 속도/분획 속도/시간의 차이가 내재화됩니다.
> - **실무 활용**: PD 모델링 보고서에서 농도 최고점과 효과 최고점의 시간 차이를 먼저 데이터로 기술하고, 그 차이가 전환 구조 채택의 근거임을 명시하는 것이 M4가 실무 문서에서 나타나는 형태입니다.

---

<!-- SLIDE_START: S43 | title: §2 요약 — 30초 진단 격자 | source_anchor: section-2-grid -->
> **핵심 메시지:** 새 데이터셋 앞에서는 workflow, linkage, response shape, turnover delay 네 렌즈를 동시에 켜야 합니다.
...
<!-- SLIDE_START: S44 | title: §5 혼동쌍 해부 | source_anchor: section-5 -->
> **핵심 메시지:** 혼동쌍은 비슷해 보이는 개념을 임상·모델링 판단에서 분리하는 안전장치입니다.

# §5 — 혼동쌍 해부(Confusion Pair Dissection)

<!-- SLIDE_START: S45 | title: 혼동쌍 1 — PK vs PD | source_anchor: confusion-1 -->
> **핵심 메시지:** PK와 PD는 반대말이 아니라 dose→concentration→effect 사슬의 앞뒤 화살표입니다.
...
| 축 | 입력 → 농도-시간 | 농도 → 효과-시간 |
| 데이터 | 혈장/혈청/전혈 농도, $AUC$, $C_{\max}$, $t_{\max}$ | 원하는/원치 않는 반응, 바이오마커, 임상 평가변수 |
...
<!-- SLIDE_START: S46 | title: 혼동쌍 2 — Graded vs Quantal | source_anchor: confusion-2 -->
> **핵심 메시지:** 반응 크기 모델과 사건 확률 모델을 혼동하면 같은 데이터에서도 서로 다른 용량 결론이 나올 수 있습니다.
...
| 모델 감각 | Hill/$E_{\max}$로 연속 반응을 설명 | 누적 빈도/확률로 사건을 설명 |
| 예시 | ketamine 반응, propranolol 심박수 감소 | alfentanil 만족스러운 마취 반응 |
| $C_{50}$ 의미 | $E_{\max}$의 50%를 주는 농도 | 집단의 50%에서 사전 결정된 반응이 나타나는 농도 |
...
<!-- SLIDE_START: S47 | title: 혼동쌍 3 — Turnover Rate vs Fractional Rate | source_anchor: confusion-3 -->
> **핵심 메시지:** 절대 전환량이 커 보여도 pool 대비 분획 전환이 작으면 과정은 느릴 수 있습니다.
...
<!-- SLIDE_START: S48 | title: §7 자기 테스트 | source_anchor: section-7 -->
> **핵심 메시지:** 자기 테스트의 목적은 정답 확인이 아니라 네 판단 축을 말로 재생할 수 있는지 검증하는 것입니다.

# §7 — 자기 테스트: 능동 회상 모듈(Self-Test: Active Recall Module)

<!-- SELF-TEST -->
<!-- SLIDE_START: S49 | title: Q1 — Modeling Carousel | source_anchor: q1 -->
> **핵심 메시지:** fitting이 5단계인 이유를 EDA와 초기 추정치의 역할로 설명할 수 있어야 합니다.
...
<!-- SLIDE_START: S50 | title: Q2 — NCA vs compartmental model | source_anchor: q2 -->
> **핵심 메시지:** 질문이 단순 노출 요약인지 예측·비선형·PK/PD 정량화인지에 따라 분석 틀이 달라집니다.
...
<!-- SLIDE_START: S51 | title: Q3 — Therapeutic window | source_anchor: q3 -->
> **핵심 메시지:** 치료역은 낮은 노출의 무효와 높은 노출의 위해 사이에서 작동하는 exposure range입니다.
...
<!-- SLIDE_START: S52 | title: Q4 — Digoxin vs morphine | source_anchor: q4 -->
> **핵심 메시지:** 요법 차이는 빈도 자체가 아니라 제거 속도와 치료역 유지 전략의 차이로 설명해야 합니다.
...
<!-- SLIDE_START: S53 | title: Q5 — Hill γ | source_anchor: q5 -->
> **핵심 메시지:** $\gamma$ 차이는 20–80% 반응 범위를 16배에서 4배로 압축시켜 적정 논리를 바꿉니다.
...
<!-- SLIDE_START: S54 | title: Q6 — Propranolol vs alfentanil | source_anchor: q6 -->
> **핵심 메시지:** 두 예시는 등급형 반응 곡선과 이분형 사건 확률 해석을 구분하라고 가르칩니다.
...
<!-- SLIDE_START: S55 | title: Q7 — Delayed response | source_anchor: q7 -->
> **핵심 메시지:** 농도 peak와 효과 peak가 분리되면 직접 효과뿐 아니라 전환·지연 구조를 고려해야 합니다.
...
<!-- SLIDE_START: S56 | title: Q8 — Turnover rate | source_anchor: q8 -->
> **핵심 메시지:** $R_t$의 절대 크기보다 pool size로 나눈 $k_t$와 그 역수인 $t_t$가 과정 속도를 더 잘 설명합니다.
...
<!-- SLIDE_START: S57 | title: Q9 — Exposure metric selection | source_anchor: q9 -->
> **핵심 메시지:** 중요한 노출 지표는 $C_{\max}$, $t_{\max}$, $AUC$ 중 임상 목표와 노출-반응 관계가 결정합니다.
...
<!-- SLIDE_START: S58 | title: §8 메타프레임 | source_anchor: section-8 -->
> **핵심 메시지:** 이 세션은 후속 모델링 판단에서 무엇을 먼저 묻고 어떤 언어로 설명할지 정하는 기반입니다.

# §8 — 메타프레임 & 큰 그림(Meta-Frame & Big Picture)

<!-- SLIDE_START: S59 | title: §8-A 위치 | source_anchor: section-8-a -->
> **핵심 메시지:** M1–M4는 용어 목록이 아니라 후속 모델링 판단의 언어를 잠그는 네 축입니다.
...
<!-- SLIDE_START: S60 | title: §8-B 의존 관계 | source_anchor: section-8-b -->
> **핵심 메시지:** 각 MUST 개념을 건너뛰면 후속 GOF, PK, PD, indirect response 판단이 각각 다른 방식으로 무너집니다.

## B. 의존 관계(Dependencies)

- M1 없이 후속 GOF/진단 세션으로 가면, 소프트웨어 출력을 모델 타당성(model validity)으로 오해합니다.
- M2 없이 구획 PK로 가면, 농도-시간 곡선을 임상 반응과 분리해서 보게 됩니다.
- M3 없이 PD 모델링으로 가면, 효능, 천장, 가파름, 평가변수 유형을 한 덩어리로 오해합니다.
- M4 없이 간접 반응으로 가면, 지연 반응을 단순 분포 지연이나 부적합으로만 오해합니다.

<!-- SLIDE_START: S61 | title: §8-C 전문가 해석 지점 | source_anchor: section-8-c -->
> **핵심 메시지:** 전문가는 새 데이터셋 앞에서 질문 기반 workflow, dose→exposure→response, Emax 3축, turnover 지연을 동시에 진단합니다.
...
- **모델링은 질문 기반 워크플로(question-driven workflow)입니다**: 질문 없이 시작하는 fitting은 답 없는 수식입니다. 데이터를 받는 순간 "무엇을 결정하기 위한 모델인가"를 먼저 쓸 수 있어야 합니다. 이 한 줄이 없으면 EDA, fitting, 진단의 모든 단계가 목적지 없는 출력이 됩니다.
- **PK/PD의 핵심 번역 좌표**: 용량 → 노출 → 반응. 이 세 좌표 사이 어디에서 개체간 변이(IIV)가 생기는지 식별하는 것이 거장의 첫 진단입니다. PK 변이인지 PD 변이인지 구분하지 못하면 공변량 탐색(covariate search)을 엉뚱한 층(layer)에서 하게 됩니다.
- **$E_{\max}$/Hill은 "천장-효능-가파름"의 3축 문법입니다**: 이 셋을 분리하지 못하면 노출-반응 해석이 무너집니다. $C_{50}$만 보고하는 보고서는 천장과 가파름 정보를 버린 것입니다. $\gamma$의 식별 가능성 문제를 모르면 잡음을 파라미터로 오인합니다.
- **전환은 "지연된 상태"의 첫 번째 문법입니다**: 반응이 늦는 이유가 분포 지연(distribution lag)인지 내인성 시스템 역학(endogenous system dynamics)인지를 구분하는 관문입니다. 이 구분이 없으면 간접 반응 모델 채택의 근거를 설명할 수 없고, 지연 반응을 부적합이나 이상치(outlier)로 오해하게 됩니다.
<!-- /RECAP -->
