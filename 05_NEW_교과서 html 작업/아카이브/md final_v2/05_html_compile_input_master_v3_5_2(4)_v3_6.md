...
<!-- SLIDE_START: S01 | title: 세션 개요 및 출처 | source_anchor: session-05 -->
> **핵심 메시지:** 2구획 모델은 곡선 모양을 임상 판단으로 바꾸기 위해 macro, micro, physiological 좌표계를 연결하는 언어입니다.
...
<!-- SLIDE_START: S02 | title: 세션 로드맵 | source_anchor: §1 -->
> **핵심 메시지:** 이 세션의 흐름은 곡선 분해에서 시작해 좌표계 선택, volume 선택, dosing 해석으로 이어집니다.
...
<!-- ANCHOR -->
<!-- SLIDE_START: S03 | title: Workflow anchor | source_anchor: §1-workflow-anchor -->
> **핵심 메시지:** 전진 선택 covariate 모델링에서는 phase 기여, 좌표계, volume 정의, 추정 안정성이 동시에 켜져야 합니다.
...
>
> $$
> \underbrace{AUC}_{\text{전신 노출}}=\frac{\underbrace{Dose}_{\text{투여량}}}{\underbrace{CL}_{\text{청소율}}},\qquad \underbrace{V_d}_{\text{겉보기 분포용적}}=\frac{\underbrace{CL}_{\text{청소율}}}{\underbrace{k}_{\text{소실 속도상수}}}
> $$
>
> - **항별 의미:** 좌변은 1구획 선행 지식의 핵심 결과인 노출과 겉보기 분포용적입니다. 우변은 투여량, 청소율, 소실 속도상수의 관계를 보여 줍니다.
> - **실무 직관:** $Dose$가 커지면 AUC가 커지고, $CL$이 커지면 AUC는 줄어들며 $V_d$는 커질 수 있습니다. 이 선행식은 2구획에서 왜 $CL$과 volume을 분리해 생각해야 하는지의 출발점입니다.
> - **오해 방지:** 1구획의 $V_d=CL/k$ 직관을 2구획의 모든 volume에 그대로 적용하면 안 됩니다. 2구획에서는 $V_1$, $V_{ss}$, $V_\beta$가 서로 다른 질문에 답합니다.
...
<!-- SLIDE_START: S04 | title: M1 이중지수 방정식과 잔차법 | source_anchor: M1 -->
> **핵심 메시지:** 이중지수 곡선은 두 제거 과정이 아니라 분포와 제거가 합성된 두 phase의 관측 표현입니다.
...
>
> $$
> \underbrace{C(t)}_{\text{시간 }t\text{의 혈장 농도}}=\overbrace{\underbrace{A}_{\text{빠른 상 절편}}\underbrace{e^{-\alpha t}}_{\text{빠른 감소}}}^{\alpha\text{ phase 기여}}+\overbrace{\underbrace{B}_{\text{느린 상 절편}}\underbrace{e^{-\beta t}}_{\text{느린 감소}}}^{\beta\text{ phase 기여}},\qquad \underbrace{\alpha>\beta>0}_{\text{빠른 상이 더 가파름}}
> $$
>
> - **항별 의미:** 좌변은 시간에 따른 혈장 농도 예측값입니다. 우변의 첫 항은 초기 빠른 phase, 둘째 항은 terminal 느린 phase를 더합니다. $A,B$는 $t=0$으로 외삽한 각 phase의 절편이고, $\alpha,\beta$는 감소 속도를 정하는 slope입니다.
> - **실무 직관:** $A$ 또는 $B$가 커지면 해당 phase의 농도 기여가 커지고, $\alpha$ 또는 $\beta$가 커지면 해당 phase는 더 빨리 사라집니다. 이 식은 early sampling 누락이 $CL$, half-life, volume 해석을 어느 방향으로 흔드는지 판단하게 해 줍니다.
> - **오해 방지:** $\alpha$ phase와 $\beta$ phase를 각각 독립적인 제거 경로로 해석하면 안 됩니다. 두 항은 2구획 ODE의 eigen-component이며, 분포와 제거가 함께 섞인 관측 phase입니다.
...
>
> $$
> \underbrace{C_{resid}(t)}_{\text{초기상 잔차}}=\underbrace{C_{obs}(t)}_{\text{관측 농도}}-\overbrace{\underbrace{B}_{\beta\text{ 절편}}e^{-\underbrace{\beta}_{\text{말단 slope}}t}}^{\text{외삽한 terminal 기여}}
> $$
>
> - **항별 의미:** 좌변은 terminal phase를 제거한 뒤 남는 초기상 농도입니다. 우변은 관측 농도에서 말단상 외삽값을 뺀 값입니다.
> - **실무 직관:** $B$ 또는 $\beta$ 추정이 틀리면 residual curve가 흔들려 $A,\alpha$ 초기 추정치도 같이 흔들립니다. 이 식은 early sampling이 부족할 때 잔차법이 얼마나 취약해지는지 보여 줍니다.
> - **오해 방지:** residual을 “진짜 독립 농도”로 오해하면 안 됩니다. 이는 terminal line이라는 가정에 의존해 만들어진 계산상 잔차입니다.
...
>
> $$
> \underbrace{\alpha}_{\text{빠른 상 slope}}=\frac{\overbrace{\ln(70/10)}^{\text{두 농도비의 로그}}}{\underbrace{130}_{\text{시간 간격}}},\qquad \underbrace{\beta}_{\text{느린 상 slope}}=\frac{\overbrace{\ln(28/10)}^{\text{두 농도비의 로그}}}{\underbrace{450}_{\text{시간 간격}}}
> $$
>
> - **항별 의미:** 좌변은 각각 빠른 phase와 느린 phase의 log-linear slope 절댓값입니다. 우변은 반로그 직선에서 두 점의 농도비 로그를 시간 간격으로 나눈 값입니다.
> - **실무 직관:** 농도비가 커지거나 시간 간격이 짧아지면 slope가 커져 반감기는 짧아집니다. 이 계산은 그림에서 직선을 눈으로 읽은 값이 어떻게 slope로 바뀌는지 보여 줍니다.
> - **오해 방지:** 단위가 혼재된 원천 그림에서는 숫자만 복사하지 말고 시간 단위와 농도 단위를 반드시 확인해야 합니다.
> **검토 메모:** 이 수식은 문맥상 반로그 직선의 두 점 slope 계산으로 읽는 것이 자연스럽습니다. 단, 원문 보존 원칙에 따라 원 수식은 수정하지 않았습니다.


<!-- SLIDE_START: S05 | title: M1 2차 파라미터 | source_anchor: M1-secondary-parameters -->
> **핵심 메시지:** AUC, AUMC, half-life, phase fraction은 곡선의 모양을 노출량과 terminal 해석 가능성으로 번역합니다.
...
>
> $$
> \begin{aligned}\underbrace{AUC_0^\infty}_{\text{총 전신 노출}}&=\overbrace{\frac{\underbrace{A}_{\alpha\text{ 절편}}}{\underbrace{\alpha}_{\alpha\text{ 감소속도}}}}^{\text{빠른 상 면적}}+\overbrace{\frac{\underbrace{B}_{\beta\text{ 절편}}}{\underbrace{\beta}_{\beta\text{ 감소속도}}}}^{\text{느린 상 면적}},\\[2mm]\underbrace{AUMC_0^\infty}_{\text{시간 가중 노출}}&=\overbrace{\frac{A}{\alpha^2}}^{\text{빠른 상 시간가중}}+\overbrace{\frac{B}{\beta^2}}^{\text{느린 상 시간가중}}\end{aligned}
> $$
>
> - **항별 의미:** 좌변의 $AUC$는 총 노출, $AUMC$는 시간으로 가중된 노출입니다. 우변은 각 exponential phase의 면적 또는 시간가중 면적을 더한 것입니다.
> - **실무 직관:** $A,B$가 커지면 해당 phase의 면적이 커지고, $\alpha,\beta$가 커지면 더 빨리 감소하므로 면적은 줄어듭니다. 특히 작은 $\beta$는 $AUMC$를 크게 키워 MRT와 terminal 해석에 큰 영향을 줍니다.
> - **오해 방지:** 큰 $AUMC$를 곧바로 “약물이 오래 남는다”로만 읽으면 안 됩니다. 느린 재분포가 시간가중 면적을 부풀릴 수 있어, plasma와 tissue 시간 척도를 분리해야 합니다.
...
>
> $$
> \underbrace{t_{1/2,\alpha}}_{\text{빠른 상 반감기}}=\frac{\overbrace{\ln2}^{\text{50\% 감소 상수}}}{\underbrace{\alpha}_{\text{빠른 상 slope}}},\qquad \underbrace{t_{1/2,\beta}}_{\text{느린 상 반감기}}=\frac{\overbrace{\ln2}^{\text{50\% 감소 상수}}}{\underbrace{\beta}_{\text{느린 상 slope}}}
> $$
>
> - **항별 의미:** 좌변은 각 phase의 농도 기여가 절반으로 줄어드는 시간입니다. 우변은 exponential slope의 역수에 $\ln2$를 곱한 형태입니다.
> - **실무 직관:** $\alpha$ 또는 $\beta$가 커질수록 해당 반감기는 짧아집니다. 이 수식은 terminal half-life가 실제 dosing 시간 척도를 대표할 수 있는지 먼저 분류하게 해 줍니다.
> - **오해 방지:** $t_{1/2,\beta}$를 항상 clinical/effective half-life로 쓰면 안 됩니다. $\beta$는 느린 재분포와 제거가 섞인 terminal slope일 수 있습니다.
...
>
> $$
> \underbrace{f_1}_{\alpha\text{ 상 면적분율}}=\frac{\overbrace{A/\alpha}^{\alpha\text{ 상 AUC}}}{\underbrace{AUC}_{\text{총 노출}}},\qquad \underbrace{f_2}_{\beta\text{ 상 면적분율}}=\frac{\overbrace{B/\beta}^{\beta\text{ 상 AUC}}}{\underbrace{AUC}_{\text{총 노출}}},\qquad \underbrace{f_1+f_2=1}_{\text{두 phase가 총 AUC를 구성}}
> $$
>
> - **항별 의미:** 좌변은 각 phase가 총 AUC에서 차지하는 비율입니다. 우변은 phase별 AUC를 총 AUC로 나눈 값입니다.
> - **실무 직관:** $B/\beta$가 커지면 $f_2$가 커져 terminal phase가 총 노출과 plateau 해석을 더 잘 대표합니다. $f_2$가 작으면 terminal half-life가 길어도 plasma time-course 판단에는 약할 수 있습니다.
> - **오해 방지:** $f_2$가 크다는 것은 terminal slope가 모든 임상 판단을 자동으로 대체한다는 뜻이 아닙니다. plasma/tissue, single/multiple dosing, input pattern을 함께 봐야 합니다.
...
<!-- SLIDE_START: S06 | title: M1 구조적 필연 | source_anchor: M1-structural-necessity -->
> **핵심 메시지:** 두 지수항은 경험적 장식이 아니라 2×2 선형 ODE system matrix에서 생기는 수학적 필연입니다.
...
>
> $$
> \underbrace{\frac{dA_1}{dt}}_{\text{중앙구획 양의 변화율}}=-\overbrace{\underbrace{(k_{12}+k_{10})}_{\text{말초 이동+제거}}\underbrace{A_1}_{\text{중앙구획 양}}}^{\text{중앙에서 빠져나감}}+\overbrace{\underbrace{k_{21}}_{\text{말초→중앙 속도}}\underbrace{A_2}_{\text{말초구획 양}}}^{\text{중앙으로 돌아옴}}
> $$
>
> - **항별 의미:** 좌변은 중앙구획 amount의 시간 변화입니다. 우변의 음의 항은 중앙에서 말초로 이동하거나 체외로 제거되는 흐름이고, 양의 항은 말초에서 중앙으로 돌아오는 흐름입니다.
> - **실무 직관:** $k_{12}$ 또는 $k_{10}$이 커지면 중앙구획은 더 빨리 줄고, $k_{21}$ 또는 $A_2$가 커지면 중앙으로 돌아오는 양이 늘어납니다. 이 식은 $CL$, $Q$, $V_1$, $V_2$가 농도 곡선의 어느 부분을 바꾸는지 판단하게 해 줍니다.
> - **오해 방지:** 중앙구획 감소를 모두 제거로 해석하면 안 됩니다. $k_{12}$에 의한 감소는 체외 제거가 아니라 말초구획으로의 분포 이동입니다.
...
>
> $$
> \underbrace{\frac{dA_2}{dt}}_{\text{말초구획 양의 변화율}}=\overbrace{\underbrace{k_{12}}_{\text{중앙→말초 속도}}\underbrace{A_1}_{\text{중앙구획 양}}}^{\text{말초로 유입}}-\overbrace{\underbrace{k_{21}}_{\text{말초→중앙 속도}}\underbrace{A_2}_{\text{말초구획 양}}}^{\text{중앙으로 회귀}}
> $$
>
> - **항별 의미:** 좌변은 말초구획 amount의 시간 변화입니다. 우변은 중앙에서 말초로 들어오는 유입과 말초에서 중앙으로 돌아가는 유출의 차이입니다.
> - **실무 직관:** $k_{12}$가 커지면 말초 축적이 빨라지고, $k_{21}$이 커지면 말초에서 빠져나오는 속도가 빨라집니다. 이 식은 느린 재분포가 terminal slope를 지배할 수 있는 이유를 보여 줍니다.
> - **오해 방지:** 말초구획은 단순히 “제거 전 대기 공간”이 아닙니다. 표준 모델에서는 말초에서 직접 제거되지 않으며, 중앙으로 돌아온 뒤 제거된다고 가정합니다.
...
>
> $$
> \underbrace{\alpha+\beta}_{\text{두 eigenvalue의 합}}=\underbrace{k_{12}}_{\text{중앙→말초}}+\underbrace{k_{21}}_{\text{말초→중앙}}+\underbrace{k_{10}}_{\text{중앙 제거}},\qquad \underbrace{\alpha\beta}_{\text{두 eigenvalue의 곱}}=\underbrace{k_{21}k_{10}}_{\text{회귀×제거 결합}}
> $$
>
> - **항별 의미:** 좌변은 macro slope들의 합과 곱입니다. 우변은 micro-rate constants가 system matrix 안에서 만드는 합·곱 제약입니다.
> - **실무 직관:** $k_{12},k_{21},k_{10}$이 변하면 $\alpha,\beta$도 함께 변하므로 phase slope를 독립 숫자처럼 움직일 수 없습니다. 이 관계는 macro-to-micro 변환의 구조적 검산식입니다.
> - **오해 방지:** $\alpha$와 $\beta$를 자유롭게 독립 조정 가능한 curve-fitting 상수로만 생각하면 안 됩니다. 2구획 central-elimination ODE의 rate balance가 이미 제약을 걸고 있습니다.
...
<!-- PRACTICE_BRIEF -->
<!-- SLIDE_START: S07 | title: M1 연습 및 Figure pointer | source_anchor: M1-practice-figure -->
> **핵심 메시지:** 잔차법은 계산 루틴보다 early sampling adequacy와 초기 추정치 감각을 훈련하는 진단 도구입니다.
...
<!-- SLIDE_START: S08 | title: M2 3중 좌표계 | source_anchor: M2 -->
> **핵심 메시지:** 같은 curve라도 macro는 기술, micro는 ODE, physiological은 보고와 covariate 해석을 담당합니다.
...
<!-- SLIDE_START: S09 | title: M2 핵심 변환식 | source_anchor: M2-core-conversions -->
> **핵심 메시지:** 좌표계 변환식은 같은 혈장 곡선을 clearance와 volume의 실무 언어로 옮기는 사전입니다.
...
>
> $$
> \underbrace{CL}_{\text{전신 청소율}}=\underbrace{k_{10}}_{\text{중앙 제거 분율속도}}\underbrace{V_c}_{\text{중앙구획 용적}}
> $$
>
> - **항별 의미:** 좌변은 단위시간당 완전히 제거되는 plasma-equivalent volume입니다. 우변은 중앙구획에서의 제거 속도상수와 중앙 용적의 곱입니다.
> - **실무 직관:** $k_{10}$ 또는 $V_c$가 커지면 같은 농도에서 제거 가능한 양이 커져 $CL$이 증가합니다. 이 수식은 micro-rate를 임상 보고 언어인 clearance로 바꿉니다.
> - **오해 방지:** $k_{10}$과 $CL$을 같은 것으로 쓰면 안 됩니다. $k_{10}$은 분율 속도이고 $CL$은 volume/time 단위의 청소율입니다.
...
>
> $$
> \underbrace{Cld}_{\text{분포 청소율}}=\underbrace{k_{12}}_{\text{중앙→말초 분율속도}}\underbrace{V_c}_{\text{중앙 용적}}=\underbrace{k_{21}}_{\text{말초→중앙 분율속도}}\underbrace{V_t}_{\text{말초 용적}}
> $$
>
> - **항별 의미:** 좌변은 중앙과 말초 사이를 오가는 distribution clearance입니다. 우변은 어느 방향에서 보더라도 같은 교환 흐름을 volume/time 단위로 표현합니다.
> - **실무 직관:** $k_{12}$ 또는 $V_c$가 커지면 중앙에서 말초로 가는 교환 capacity가 커지고, $k_{21}$ 또는 $V_t$가 커지면 반대 방향 표현도 커집니다. 이 식은 $Q$와 $CL$을 구분하는 실무 기준이 됩니다.
> - **오해 방지:** $Cld$ 또는 $Q$는 체외 제거가 아닙니다. 내부 구획 간 교환량이므로 systemic clearance와 혼동하면 dosing 해석이 틀어집니다.
...
>
> $$
> \underbrace{V_c}_{\text{중앙구획 용적}}=\frac{\underbrace{D_{iv}}_{\text{정맥투여 용량}}}{\underbrace{A+B}_{\text{외삽 초기농도 }C_0}}
> $$
>
> - **항별 의미:** 좌변은 bolus 직후 중앙구획 희석 용적입니다. 우변은 정맥투여 용량을 $t=0$ 외삽 농도 $C_0$로 나눈 값입니다.
> - **실무 직관:** $D_{iv}$가 커지면 용적 추정이 커지고, $A+B$가 커지면 같은 용량이 더 높은 초기농도를 만들었으므로 용적은 작아집니다. 이 식은 초기 plasma peak와 bolus dilution을 판단하게 합니다.
> - **오해 방지:** $V_c$를 $V_{ss}$ 또는 $V_\beta$와 바꾸어 쓰면 안 됩니다. 이 값은 bolus 직후 중심구획 희석을 말합니다.
...
>
> $$
> \underbrace{k_{21}}_{\text{말초→중앙 분율속도}}=\frac{\overbrace{\underbrace{A}_{\alpha\text{ 절편}}\underbrace{\beta}_{\text{느린 slope}}+\underbrace{B}_{\beta\text{ 절편}}\underbrace{\alpha}_{\text{빠른 slope}}}^{\text{절편으로 가중된 slope 조합}}}{\underbrace{A+B}_{C_0}}
> $$
>
> - **항별 의미:** 좌변은 말초에서 중앙으로 돌아오는 micro-rate입니다. 우변은 macro 절편과 slope의 조합을 초기농도 절편 합으로 정규화합니다.
> - **실무 직관:** $A\beta+B\alpha$가 커지면 $k_{21}$이 커져 재분포 회귀가 빨라집니다. 이 식은 macro slope/intercept가 micro 전이속도로 변환되는 핵심 통로입니다.
> - **오해 방지:** $k_{21}$은 terminal slope $\beta$와 같은 값이 아닙니다. $\beta$는 전체 system의 느린 eigenvalue이고, $k_{21}$은 말초→중앙 전이 rate입니다.
...
>
> $$
> \underbrace{k_{10}}_{\text{중앙 제거 분율속도}}=\frac{\overbrace{\alpha\beta}^{\text{두 eigenvalue의 곱}}}{\underbrace{k_{21}}_{\text{말초→중앙 전이속도}}}
> $$
>
> - **항별 의미:** 좌변은 중앙구획 제거 micro-rate입니다. 우변은 두 macro slope의 곱을 말초→중앙 rate로 나누어 얻습니다.
> - **실무 직관:** $\alpha\beta$가 커지면 제거 분율속도 추정이 커지고, $k_{21}$이 커지면 같은 eigenvalue 곱에서 $k_{10}$은 작아집니다. 이 관계는 macro 값들이 독립 숫자가 아니라 micro 구조 제약을 받는다는 점을 보여 줍니다.
> - **오해 방지:** $k_{10}$을 terminal slope로 직접 대체하면 안 됩니다. terminal slope는 제거뿐 아니라 재분포와 구획 간 교환의 결합 결과입니다.
...
>
> $$
> \underbrace{k_{12}}_{\text{중앙→말초 분율속도}}=\overbrace{\alpha+\beta}^{\text{두 slope의 합}}-\underbrace{k_{21}}_{\text{말초→중앙}}-\underbrace{k_{10}}_{\text{중앙 제거}}
> $$
>
> - **항별 의미:** 좌변은 중앙에서 말초로 가는 micro-rate입니다. 우변은 system slope의 합에서 이미 설명된 말초 회귀와 제거 rate를 뺀 나머지 전이속도입니다.
> - **실무 직관:** $\alpha+\beta$가 커지면 전체 system 속도가 커지고, $k_{21}$ 또는 $k_{10}$이 커지면 중앙→말초 몫은 줄어듭니다. 이 식은 2구획 ODE의 rate balance를 닫아 줍니다.
> - **오해 방지:** 음수 또는 비현실적 $k_{12}$가 나오면 단순 계산 오류가 아니라 macro 추정, 단위, 가정, 초기치 문제를 의심해야 합니다.
...
<!-- SLIDE_START: S10 | title: M2 구조적 비식별성 | source_anchor: M2-non-identifiability -->
> **핵심 메시지:** 혈장 이중지수 곡선만으로 제거 부위를 증명할 수 없으므로 central elimination은 데이터 결론이 아니라 명시해야 할 가정입니다.
...
<!-- PRACTICE_BRIEF -->
<!-- SLIDE_START: S11 | title: M2 연습 및 Figure pointer | source_anchor: M2-practice-figure -->
> **핵심 메시지:** 좌표계 변환의 실전 목표는 무엇이 식별되고 무엇이 가정인지 분리해 보고서 언어로 닫는 것입니다.
...
<!-- SLIDE_START: S12 | title: M3 3중 분포용적 체계 | source_anchor: M3 -->
> **핵심 메시지:** 2구획에서 volume 질문은 하나가 아니며 bolus 직후, 정상상태, terminal extrapolation이 서로 다른 답을 요구합니다.
...
>
> $$
> \underbrace{V_1}_{\text{초기 중앙 희석}}=\frac{\underbrace{Dose}_{\text{투여량}}}{\underbrace{A+B}_{C_0}},\qquad \underbrace{V_{ss}}_{\text{정상상태 체내량/농도}}=\underbrace{MRT}_{\text{평균체류시간}}\cdot\underbrace{CL}_{\text{청소율}}=\frac{\underbrace{Dose}_{\text{투여량}}\underbrace{AUMC}_{\text{시간가중 노출}}}{\underbrace{AUC^2}_{\text{노출 제곱}}},\qquad \underbrace{V_\beta}_{\text{말단 겉보기 용적}}=\frac{\underbrace{CL}_{\text{청소율}}}{\underbrace{\beta}_{\text{말단 slope}}}=\frac{\underbrace{Dose}_{\text{투여량}}}{\underbrace{AUC}_{\text{노출}}\underbrace{\beta}_{\text{말단 slope}}}
> $$
>
> - **항별 의미:** 좌변은 각각 bolus 직후 중앙 희석 용적, 정상상태 체내량 기준 용적, terminal slope 기반 겉보기 용적입니다. 우변은 각 volume이 어느 관측량과 시간 척도에서 계산되는지 보여 줍니다.
> - **실무 직관:** $A+B$가 커지면 $V_1$은 작아지고, MRT 또는 CL이 커지면 $V_{ss}$는 커지며, $\beta$가 작아지면 $V_\beta$는 크게 부풀 수 있습니다. 이 세 식은 loading dose, amount, terminal extrapolation에 서로 다른 volume을 선택하게 해 줍니다.
> - **오해 방지:** $V_\beta$가 크다고 실제 평형 분포공간이 그만큼 넓다는 뜻은 아닙니다. 느린 재분포가 terminal slope를 작게 만들어 겉보기 용적을 부풀릴 수 있습니다.
...
<!-- SLIDE_START: S13 | title: M3 volume ordering과 기준점 | source_anchor: M3-ordering-anchors -->
> **핵심 메시지:** $V_1\le V_{ss}\le V_\beta$는 보편 법칙이 아니라 표준 central-elimination 2구획 가정 안의 실무 규칙입니다.
...
>
> $$
> \underbrace{V_1}_{\text{초기 중앙 희석}}\le \underbrace{V_{ss}}_{\text{분포 평형 체내량}}\le \underbrace{V_\beta}_{\text{말단 slope 기반 겉보기 용적}}
> $$
>
> - **항별 의미:** 좌변부터 오른쪽으로 갈수록 volume은 더 늦은 시간 척도와 더 많은 분포 정보를 반영합니다.
> - **실무 직관:** $V_\beta/V_{ss}$가 커질수록 terminal slope로 body amount나 loading dose를 해석할 위험이 커집니다. 이 부등식은 volume misuse를 빠르게 감지하는 스크리닝 규칙입니다.
> - **오해 방지:** 이 순서 관계를 모든 약물과 모든 구조에 적용되는 물리 법칙으로 취급하면 안 됩니다. 표준 central-elimination 2구획 가정 안의 working rule입니다.
...
<!-- CONFUSION -->
<!-- SLIDE_START: S14 | title: M3 Plausible fallacy와 임상 사용 규칙 | source_anchor: M3-fallacy-clinical-rule -->
> **핵심 메시지:** 긴 terminal half-life는 넓은 분포의 증거가 아니라 느린 재분포의 신호일 수 있습니다.
...
>
> $$
> \underbrace{t_{1/2,\beta}}_{\text{terminal half-life}}=\frac{\overbrace{0.693}^{\ln 2}}{\underbrace{\beta}_{\text{말단 slope}}},\qquad \underbrace{V_\beta}_{\text{말단 겉보기 용적}}=\frac{\underbrace{CL}_{\text{청소율}}}{\underbrace{\beta}_{\text{말단 slope}}}
> $$
>
> - **항별 의미:** 좌변은 terminal phase의 시간 척도와 terminal slope 기반 volume입니다. 우변은 둘 다 $\beta$에 강하게 의존합니다.
> - **실무 직관:** $\beta$가 작아지면 terminal half-life와 $V_\beta$가 함께 커질 수 있습니다. 따라서 긴 terminal half-life와 큰 $V_\beta$는 느린 재분포의 신호일 수 있습니다.
> - **오해 방지:** 큰 $V_\beta$를 곧바로 “실제 조직 분포가 매우 큼”으로 해석하면 loading dose와 tissue accumulation 판단이 왜곡됩니다.
...
<!-- PRACTICE_BRIEF -->
<!-- SLIDE_START: S15 | title: M3 연습 및 volume figure | source_anchor: M3-practice-figure -->
> **핵심 메시지:** loading dose, body amount, terminal extrapolation은 각각 다른 volume label을 요구합니다.
...
<!-- SLIDE_START: S16 | title: M4 재파라미터화와 condition number | source_anchor: M4 -->
> **핵심 메시지:** 같은 biexponential fit도 parameterization이 바뀌면 추정 기하와 condition number가 100배 이상 달라질 수 있습니다.
...
<!-- SLIDE_START: S17 | title: M4 적합 품질과 추정 안정성 | source_anchor: M4-core-lesson -->
> **핵심 메시지:** WRSS가 낮다는 사실은 deployable model의 충분조건이 아니며 precision과 covariance geometry를 함께 봐야 합니다.
...
<!-- PRACTICE_BRIEF -->
<!-- SLIDE_START: S18 | title: M4 연습 및 모델 선택 schematic | source_anchor: M4-practice-figure -->
> **핵심 메시지:** 재파라미터화는 같은 모델을 예쁘게 다시 쓰는 일이 아니라 estimation geometry를 조정하는 일입니다.
...
<!-- SLIDE_START: S19 | title: M5 Plateau equation | source_anchor: M5 -->
> **핵심 메시지:** plateau 도달은 half-life 하나가 아니라 phase별 기여도와 입력 패턴의 가중합으로 해석해야 합니다.
...
>
> $$
> \underbrace{\frac{C(t)}{C_{ss}}}_{\text{정상상태 대비 접근률}}=\overbrace{\underbrace{f_1}_{\alpha\text{ 상 기여}}\left(1-e^{-\alpha t}\right)}^{\text{빠른 접근 성분}}+\overbrace{\underbrace{f_2}_{\beta\text{ 상 기여}}\left(1-e^{-\beta t}\right)}^{\text{느린 접근 성분}}
> $$
>
> - **항별 의미:** 좌변은 현재 농도가 정상상태 농도의 몇 %에 도달했는지를 나타냅니다. 우변은 빠른 phase와 느린 phase의 plateau 접근을 $f_1/f_2$로 가중해 더합니다.
> - **실무 직관:** $f_1$ 또는 $\alpha$가 크면 초기 접근이 빨라지고, $f_2$가 크거나 $\beta$가 작으면 terminal half-life가 plateau 판단에 더 큰 영향을 줍니다. 이 식은 loading dose 필요성과 plateau 도달 시간을 half-life 하나가 아니라 phase 기여도로 판단하게 합니다.
> - **오해 방지:** terminal half-life가 길다고 plateau가 반드시 늦는 것은 아닙니다. $f_2$가 작으면 plasma는 빠르게 $C_{ss}$에 접근할 수 있습니다.
...
<!-- SLIDE_START: S20 | title: M5 임상 기준점 | source_anchor: M5-clinical-anchors -->
> **핵심 메시지:** Nicardipine과 gentamicin 사례는 terminal half-life가 plasma dosing 판단을 대표하지 못하는 조건을 보여 줍니다.
...
<!-- PRACTICE_BRIEF -->
<!-- SLIDE_START: S21 | title: M5 연습 및 Figure pointers | source_anchor: M5-practice-figures -->
> **핵심 메시지:** half-life를 들으면 계산보다 먼저 terminal/effective, plasma/tissue, single/multiple dosing을 분류해야 합니다.
...
<!-- SLIDE_START: S22 | title: Confusion Pair 1 | source_anchor: §5-pair1 -->
> **핵심 메시지:** bi-exponential curve를 두 elimination pathway로 직역하면 ODE eigen-component와 생리적 제거 과정을 혼동합니다.
...
<!-- SLIDE_START: S23 | title: Confusion Pair 2 Critical Blow | source_anchor: §5-pair2 -->
> **핵심 메시지:** $V_\beta$를 loading dose용 volume으로 쓰는 순간 terminal geometry를 실제 체내량으로 오역하는 치명적 오류가 됩니다.
...
>
> $$
> \underbrace{\frac{V_\beta}{V_{ss}}}_{\text{terminal volume 오용 위험}}\approx \frac{\underbrace{345}_{\text{gentamicin }V_\beta}}{\underbrace{56}_{\text{gentamicin }V_{ss}}}\approx 6.2
> $$
>
> - **항별 의미:** 좌변은 terminal volume이 steady-state volume보다 얼마나 부풀었는지를 보는 위험 지표입니다. 우변은 gentamicin 기준점에서의 비율입니다.
> - **실무 직관:** 비율이 커질수록 $V_\beta$를 loading dose에 쓰는 오류가 첫 dose 과대 산정으로 이어질 위험이 커집니다. 이 수식은 reviewer 방어와 critical care dosing에서 volume 선택을 강제합니다.
> - **오해 방지:** $V_\beta/V_{ss}$ 비율을 “조직 분포가 6배 넓다”로 단순 번역하면 안 됩니다. terminal slope geometry와 재분포 지연이 섞여 있습니다.
...
<!-- SLIDE_START: S24 | title: Confusion Pair 3 | source_anchor: §5-pair3 -->
> **핵심 메시지:** macro, micro, physiological 좌표계를 섞으면 freedom, assumption, reporting interpretation이 동시에 흐려집니다.
...
<!-- SLIDE_START: S25 | title: Confusion Pair 4 | source_anchor: §5-pair4 -->
> **핵심 메시지:** 논문 속 half-life 숫자는 terminal인지 effective인지 확인되기 전까지 dosing interval 판단에 바로 들어갈 수 없습니다.
...
<!-- SLIDE_START: S26 | title: Confusion Pair 5 | source_anchor: §5-pair5 -->
> **핵심 메시지:** 최저 WRSS 모델이 가장 좋은 모델이라는 판단은 추정 안정성과 생물학적 타당성을 누락한 반쪽 판단입니다.
...
<!-- SLIDE_START: S27 | title: Self-Test Q1–Q3 | source_anchor: §7-q1-q3 -->
> **핵심 메시지:** 잔차법, CL 방향, volume ratio를 빠르게 판정할 수 있어야 2구획 곡선을 실무 언어로 읽기 시작합니다.
...
>
> $$
> \underbrace{\left(\frac{V_\beta}{V_{ss}}\right)_{aspirin}}_{\text{aspirin terminal 부풀림}}\approx\frac{13.7}{10.4}\approx1.3,\qquad \underbrace{\left(\frac{V_\beta}{V_{ss}}\right)_{gentamicin}}_{\text{gentamicin terminal 부풀림}}\approx\frac{345}{56}\approx6.2
> $$
>
> - **항별 의미:** 좌변은 두 약물에서 terminal volume과 steady-state volume의 비율입니다. 우변은 각 기준점 숫자의 나눗셈입니다.
> - **실무 직관:** 비율이 1에 가까우면 volume 혼동 위험이 상대적으로 작지만, 6배 수준이면 loading dose와 amount 계산에서 치명적 오차가 될 수 있습니다.
> - **오해 방지:** 이 비율만으로 독성 여부를 직접 예측할 수는 없습니다. 그러나 어떤 volume을 어떤 임상 질문에 써야 하는지 결정하는 강한 경고 신호입니다.


<!-- SLIDE_START: S28 | title: Self-Test Q4–Q6 | source_anchor: §7-q4-q6 -->
> **핵심 메시지:** 구조적 비식별성, terminal half-life 함정, WRSS/condition number 분리는 모델 선택의 최소 방어선입니다.
...
>
> $$
> \underbrace{\text{plateau 접근}}_{C(t)/C_{ss}}\neq \underbrace{\text{terminal }t_{1/2}\text{ 단독 판단}}_{\beta\text{만 보는 해석}},\qquad \underbrace{C(t)/C_{ss}}_{\text{접근률}}=\underbrace{f_1}_{\alpha\text{ 기여}}(1-e^{-\alpha t})+\underbrace{f_2}_{\beta\text{ 기여}}(1-e^{-\beta t})
> $$
>
> - **항별 의미:** 좌변은 plateau 접근을 terminal half-life 하나로 대체할 수 없다는 판단식입니다. 우변은 실제 접근률이 빠른 phase와 느린 phase의 가중합임을 다시 보여 줍니다.
> - **실무 직관:** $f_1$이 크면 초기 접근이 빠르고, $f_2$가 작으면 긴 terminal half-life가 있어도 plasma는 빠르게 $C_{ss}$에 접근할 수 있습니다. 이 판단은 loading dose 필요성 평가를 바꿉니다.
> - **오해 방지:** terminal half-life만 보고 “정상상태 도달이 늦다”고 결론내면 과도한 loading을 권할 수 있습니다.
...
<!-- SLIDE_START: S29 | title: Self-Test Q7–Q9 | source_anchor: §7-q7-q9 -->
> **핵심 메시지:** 희소 설계와 NDA 모델 선택에서는 fit 수치보다 식별성, shrinkage, reviewer 방어 논리가 우선입니다.
...
<!-- SLIDE_START: S30 | title: §8 Knowledge graph | source_anchor: §8-knowledge-graph -->
> **핵심 메시지:** 이번 세션의 공식들은 NONMEM 구조 선택, sparse design, covariate modeling, 다회투여 simulation으로 이어지는 연결 노드입니다.
...
<!-- SLIDE_START: S31 | title: §8 Failure modes | source_anchor: §8-failure-modes -->
> **핵심 메시지:** 2구획 모델의 주요 실패는 대개 terminal geometry를 clearance, volume, amount의 생리적 진실로 과잉해석할 때 발생합니다.
...
<!-- SLIDE_START: S32 | title: §8 Professional moat | source_anchor: §8-professional-moat -->
> **핵심 메시지:** 숙련된 모델러는 semilog curve 하나에서 sampling adequacy, CL bias, volume misuse, shrinkage risk를 동시에 읽습니다.
...
<!-- SLIDE_START: S33 | title: §8 Minimal practical algorithm | source_anchor: §8-minimal-practical-algorithm -->
> **핵심 메시지:** 최소 실무 알고리즘은 곡선 확인에서 시작해 phase 식별성, volume ratio, physiological parameterization, multiple dosing 점검으로 닫힙니다.
