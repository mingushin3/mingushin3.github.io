...
<!-- SLIDE_START: S01 | title: PART A 학습 핸드아웃 진입 | source_anchor: PART_A -->
> **핵심 메시지:** 이 파일의 학습자-facing 핵심은 NCA 수치를 그대로 믿기 전에 가정과 적용 경계를 먼저 감사하는 것입니다.

## PART A — 학습자용 완성 핸드아웃(Learner-Facing Complete Handout)

<!-- SLIDE_START: S02 | title: 학습 안내와 사용 경로 | source_anchor: learner-navigation-aid -->
> **핵심 메시지:** 이 핸드아웃은 AUC에서 출발해 MRT와 분포용적을 거쳐 노출 지표 선택으로 이어지는 감사 순서대로 읽어야 합니다.
...
>
> $$
> \underbrace{t_{1/2}}_{\text{반감기}}=\frac{\underbrace{\ln(2)}_{\text{50\% 감소 상수}}}{\underbrace{k}_{\text{1차 소실속도상수}}}
> $$
>
> - **항별 의미:** 좌변은 농도가 절반으로 감소하는 시간이고, 우변은 1차 소실속도상수 $k$를 50% 감소 시간으로 변환합니다.
> - **실무 직관:** $k$가 커지면 더 빨리 제거되므로 반감기는 짧아지고, $k$가 작아지면 반감기는 길어집니다.
> - **오해 방지:** 이 식은 1차 단일지수 감소 조건에서의 반감기 직관입니다. 다구획·비선형·flip-flop 상황에서는 말단기울기 또는 MRT와 구분해야 합니다.
...
<!-- SLIDE_START: S03 | title: 세션 로드맵 | source_anchor: sec1-roadmap -->
> **핵심 메시지:** NCA는 model-free가 아니라 compartment-free이므로 선형 소실과 말단기울기 가정을 먼저 확인해야 합니다.
...
<!-- SLIDE_START: S04 | title: NCA와 회귀의 역할 차이 | source_anchor: sec1-fig-2-113 -->
> **핵심 메시지:** NCA는 면적을 요약하고 회귀는 곡선의 파라미터를 추정하므로, 비선형 신호가 보이면 역할이 바뀝니다.
...
<!-- SLIDE_START: S05 | title: NCA 출력 읽는 순서 | source_anchor: sec1-practice-lens -->
> **핵심 메시지:** NCA 표는 행 순서가 아니라 말단기울기와 외삽 비율에서 시작하는 가정 감사 순서로 읽어야 합니다.
...
<!-- SLIDE_START: S06 | title: 개념 해부 카드의 전체 구조 | source_anchor: sec2-concept-cards -->
> **핵심 메시지:** 네 개의 카드는 독립 암기 항목이 아니라 AUC 오류가 CL·분포용적·노출 해석으로 전파되는 직렬 체인입니다.

# §2 — 개념 해부 카드(Concept Anatomy Cards)

<!-- SLIDE_START: S07 | title: C1 AUC 계산의 핵심 | source_anchor: C1 -->
> **핵심 메시지:** AUC의 신뢰도는 면적 공식 자체보다 말단기울기 선택과 외삽 꼬리의 방어 가능성에서 결정됩니다.
...
<!-- SLIDE_START: S08 | title: C1 AUC의 형식적 정의 | source_anchor: C1-A-formal-definition -->
> **핵심 메시지:** AUC는 관측 면적과 말단 외삽 면적의 합이므로 관측값과 가정값을 분리해 읽어야 합니다.
...
<!-- SLIDE_START: S09 | title: C1 수식과 출처 고정 규칙 | source_anchor: C1-B-equations -->
> **핵심 메시지:** 사다리꼴 공식은 단순 계산식이지만 적용 구간과 편향 방향을 함께 보지 않으면 노출 해석이 흔들립니다.
...
>
> $$
> \underbrace{AUC_0^{t_{last}}}_{\text{관측 구간 노출}}=
> \sum_i
> \underbrace{\frac{C_i+C_{i+1}}{2}}_{\text{두 시점 평균 농도}}
> \cdot
> \underbrace{\Delta t}_{\text{시간 간격}}
> $$
>
> - **항별 의미:** 좌변은 0부터 마지막 관측 시점까지의 관측 AUC를 정의합니다. 우변은 각 구간의 평균 농도에 시간 폭을 곱한 사다리꼴 면적을 모두 더합니다.
> - **실무 직관:** $C_i$ 또는 $C_{i+1}$가 커지거나 간격 $\Delta t$가 길어지면 해당 구간 AUC가 커집니다. 따라서 드문 표본추출에서는 곡선 모양을 거칠게 근사하게 됩니다.
> - **오해 방지:** 이 식은 관측 구간 면적만 계산합니다. 마지막 시점 이후 꼬리 면적과 $\lambda_z$의 신뢰성은 별도로 검토해야 합니다.
...
<!-- SLIDE_START: S10 | title: 로그-선형 사다리꼴법 | source_anchor: C1-B-log-linear-trapezoid -->
> **핵심 메시지:** 하강 구간에서는 로그-선형 평균 농도가 말단 면적을 더 자연스럽게 근사하지만 0 또는 동일 농도에서는 실패합니다.
...
>
> $$
> \underbrace{AUC_i^{i+1}}_{\text{한 구간 AUC}}=
> \underbrace{\frac{C_i-C_{i+1}}{\ln(C_i/C_{i+1})}}_{\text{로그-선형 평균 농도}}
> \cdot
> \underbrace{\Delta t}_{\text{시간 간격}}
> $$
>
> - **항별 의미:** 좌변은 두 관측점 사이의 구간 AUC입니다. 우변의 분수항은 지수적으로 감소하는 농도에서 산술 평균 대신 쓰는 로그-선형 평균 농도입니다.
> - **실무 직관:** 농도가 완만히 감소하고 $\Delta t$가 길어질수록 이 구간의 노출 기여가 커집니다. 하강 구간에서 선형 사다리꼴의 과대추정을 줄이는 데 도움이 됩니다.
> - **오해 방지:** 이 식은 하강 구간에 쓰는 식입니다. $C_i=0$이거나 $C_i=C_{i+1}$이면 로그항이 성립하지 않으므로 선형 대체가 필요합니다.
...
<!-- SLIDE_START: S11 | title: 혼합 방식의 적용 경계 | source_anchor: C1-B-mixed-approach -->
> **핵심 메시지:** 혼합 방식은 상승과 하강 구간의 곡선 성격을 다르게 본다는 점에서 합리적이지만 무조건 표준으로 과장하면 안 됩니다.
...
<!-- SLIDE_START: S12 | title: 말단 외삽과 C_last 선택 | source_anchor: C1-B-terminal-extrapolation -->
> **핵심 메시지:** 말단 외삽 AUC는 마지막 농도 하나가 아니라 말단기울기와 회귀선 적합성의 함수입니다.
...
>
> $$
> \underbrace{AUC_{t_{last}\to\infty}}_{\text{말단 꼬리 노출}}=
> \frac{\underbrace{C_{last}}_{\text{마지막 농도 또는 }\hat C_{last}}}{\underbrace{\lambda_z}_{\text{말단 소실 기울기}}}
> $$
>
> - **항별 의미:** 좌변은 마지막 관측 이후 무한대까지의 외삽 AUC입니다. 우변은 말단 농도를 말단 소실속도상수로 나누어 단일지수 감소 꼬리 면적을 계산합니다.
> - **실무 직관:** $C_{last}$가 커지면 꼬리 AUC가 커지고, $\lambda_z$가 작아지면 꼬리가 길어져 AUC가 커집니다.
> - **오해 방지:** 이 식은 말단기가 진짜 terminal phase라는 가정 위에 있습니다. $C_{last}$ 하나를 기계적으로 넣는 계산이 아니라 $\hat C_{last}$ 사용 여부와 회귀선 적합성을 함께 판단해야 합니다.

관측 $C_{last}$가 말단 회귀선(terminal regression line)에서 벗어나면, 관측값 대신 회귀 기반 예측값 $\hat C_{last}$ 사용이 일반적으로 권고됩니다. [G p.147]

<!-- SLIDE_START: S13 | title: 외삽 면적 비율 | source_anchor: C1-B-percent-extrapolated -->
> **핵심 메시지:** 외삽 면적 비율은 AUC가 관측 데이터 요약인지 말단기울기 선택의 산물인지 구분하는 핵심 경고등입니다.
...
>
> $$
> \underbrace{\%Extrapolated}_{\text{외삽 의존도}}=
> \frac{\underbrace{AUC_{t_{last}\to\infty}}_{\text{관측 밖 꼬리 면적}}}{\underbrace{AUC_{total}}_{\text{총 AUC}}}
> \times 100
> $$
>
> - **항별 의미:** 좌변은 총 AUC 중 관측 이후 외삽으로 채운 비율입니다. 분자는 말단 꼬리 면적, 분모는 관측 AUC와 외삽 AUC를 합친 총 AUC입니다.
> - **실무 직관:** 외삽 꼬리가 커지거나 총 AUC에서 관측 면적 비중이 작아지면 이 값이 올라가며, 결과가 데이터보다 $\lambda_z$ 선택에 더 의존하게 됩니다.
> - **오해 방지:** 퍼센트가 계산된다고 AUC가 자동으로 신뢰 가능해지는 것은 아닙니다. 높은 외삽 비율은 추가 표본추출 또는 말단기 재검토가 필요하다는 경고입니다.
...
<!-- SLIDE_START: S14 | title: lambda_z 선정 | source_anchor: C1-C-lambda-z-selection -->
> **핵심 메시지:** 자동 선택된 말단기울기는 반로그 도표에서 진짜 terminal phase인지 확인되기 전까지 신뢰할 수 없습니다.
...
<!-- SLIDE_START: S15 | title: C1 맥락 보충 | source_anchor: C1-D-context-notes -->
> **핵심 메시지:** 정상상태 AUC와 LOQ 처리, AUC와 CL의 보고 위치는 모두 AUC를 해석 가능한 노출 지표로 만들기 위한 조건입니다.
...
<!-- SLIDE_START: S16 | title: NCA는 assumption-free가 아니다 | source_anchor: C1-D2-plausible-fallacy -->
> **핵심 메시지:** NCA는 구획 모델을 피할 뿐 선형 소실·말단기울기·충분한 표본추출 가정에서 자유롭지 않습니다.
...
<!-- SLIDE_START: S17 | title: C1 경계 조건 | source_anchor: C1-E-boundary-conditions -->
> **핵심 메시지:** AUC의 경계 조건은 계산 가능 여부가 아니라 계산된 면적을 해석 가능한 노출로 부를 수 있는지의 조건입니다.
...
<!-- SLIDE_START: S18 | title: C1 원자 노트와 요약 | source_anchor: C1-F-atom-recap -->
> **핵심 메시지:** AUC 감사는 값 자체가 아니라 lambda_z 도표, C_last 처리, 외삽 분율 확인에서 시작합니다.
...
<!-- SLIDE_START: S19 | title: C1 AUC 감사 체인 도식 | source_anchor: C1-figure-schematic-audit-chain -->
> **핵심 메시지:** AUC 계산은 사다리꼴 합에서 끝나지 않고 말단 꼬리와 하류 파라미터까지 이어지는 품질관리 체인입니다.
...
<!-- SLIDE_START: S20 | title: C1 실패 모드와 실무 체화 요약 | source_anchor: C1-failure-practice-brief -->
> **핵심 메시지:** C1의 방어 가능성은 외부 임의 기준이 아니라 lambda_z, 외삽 분율, C_last 처리, 표본추출 충분성으로 세워야 합니다.
...
<!-- SLIDE_START: S21 | title: C2 AUMC와 MRT 진입 | source_anchor: C2 -->
> **핵심 메시지:** AUMC와 MRT는 평균처럼 보이지만 말단 꼬리와 입력시간 보정에 AUC보다 더 민감합니다.
...
<!-- SLIDE_START: S22 | title: C2 형식적 정의 | source_anchor: C2-A-formal-definition -->
> **핵심 메시지:** AUMC는 시간으로 가중된 노출이고 MRT는 그 모멘트를 AUC로 나눈 평균 체류시간입니다.
...
<!-- SLIDE_START: S23 | title: AUMC 선형 사다리꼴 계산 | source_anchor: C2-B-AUMC-linear -->
> **핵심 메시지:** AUMC는 농도에 시간을 한 번 더 곱하므로 late-time 관측치의 영향이 구조적으로 커집니다.
...
>
> $$
> \underbrace{AUMC_0^{t_{last}}}_{\text{관측 일차 모멘트}}=
> \sum_i
> \underbrace{\frac{t_iC_i+t_{i+1}C_{i+1}}{2}}_{\text{시간가중 평균 농도}}
> \cdot
> \underbrace{\Delta t}_{\text{시간 간격}}
> $$
>
> - **항별 의미:** 좌변은 관측 구간의 시간가중 농도 면적입니다. 우변은 각 시점 농도에 시간을 곱한 뒤 사다리꼴 방식으로 누적합니다.
> - **실무 직관:** 늦은 시점의 같은 농도는 이른 시점보다 $t$가 곱해져 AUMC에 더 크게 기여합니다. 그래서 tail sampling 부족은 MRT를 특히 흔듭니다.
> - **오해 방지:** AUMC는 AUC의 단순 확장처럼 보이지만 민감도는 다릅니다. 늦은 농도 하나가 시간 가중 때문에 과도하게 영향력을 가질 수 있습니다.

<!-- SLIDE_START: S24 | title: AUMC 꼬리 외삽 | source_anchor: C2-B-AUMC-tail -->
> **핵심 메시지:** AUMC 꼬리는 1/lambda_z와 1/lambda_z^2를 동시에 포함해 말단기울기 오류를 증폭합니다.
...
>
> $$
> \underbrace{AUMC_{t_{last}\to\infty}}_{\text{말단 일차 모멘트 꼬리}}=
> \underbrace{\frac{t_{last}C_{last}}{\lambda_z}}_{\text{마지막 시점 이후의 시간가중 꼬리}}
> +
> \underbrace{\frac{C_{last}}{\lambda_z^2}}_{\text{말단기울기 민감도 증폭 항}}
> $$
>
> - **항별 의미:** 좌변은 마지막 관측 이후의 AUMC 외삽분입니다. 우변 첫 항은 마지막 시점의 시간가중 농도 꼬리, 둘째 항은 시간이 계속 흐르며 누적되는 추가 모멘트입니다.
> - **실무 직관:** $C_{last}$가 크거나 $\lambda_z$가 작으면 AUMC 꼬리가 커집니다. 특히 $1/\lambda_z^2$ 항 때문에 작은 $\lambda_z$ 오류도 크게 증폭됩니다.
> - **오해 방지:** AUMC 꼬리는 AUC 꼬리보다 말단기울기에 더 민감합니다. MRT가 평균이라고 해서 안정적인 값이라고 착각하면 안 됩니다.

<!-- SLIDE_START: S25 | title: MRT의 분자 정의 | source_anchor: C2-B-MRT-molecular -->
> **핵심 메시지:** MRT는 농도 곡선의 장식적 평균이 아니라 개별 분자 체류시간의 평균이라는 물리적 의미를 갖습니다.
...
> \frac{\underbrace{\sum_j t_j}_{\text{모든 분자 체류시간 합}}}{\underbrace{N}_{\text{분자 수}}}
> $$
>
> - **항별 의미:** 좌변은 평균 체류시간입니다. 우변은 각 분자가 계 안에 머문 시간을 모두 더한 뒤 분자 수로 나눈 평균입니다.
> - **실무 직관:** 개별 분자 체류시간이 길어질수록 MRT가 커집니다. 이 정의는 MRT가 단순 곡선 fitting 숫자가 아니라 실제 residence time 평균이라는 의미를 줍니다.
> - **오해 방지:** 이 식은 반감기 정의가 아닙니다. 반감기는 농도 감소의 시간 척도이고, MRT는 분자 체류시간의 평균입니다.

<!-- SLIDE_START: S26 | title: MRT의 혈장 정의 | source_anchor: C2-B-MRT-plasma -->
> **핵심 메시지:** 혈장 MRT는 시간가중 노출을 총 노출로 나눈 값이므로 AUMC와 AUC의 품질을 동시에 상속합니다.
...
>
> $$
> \underbrace{MRT}_{\text{혈장 평균 체류시간}}=
> \frac{\underbrace{AUMC_0^\infty}_{\text{시간가중 총 노출}}}{\underbrace{AUC_0^\infty}_{\text{총 노출}}}
> $$
>
> - **항별 의미:** 좌변은 혈장 자료에서 계산한 평균 체류시간입니다. 분자는 시간가중 총 노출, 분모는 총 노출입니다.
> - **실무 직관:** AUMC가 AUC에 비해 커지면 MRT가 길어집니다. 즉 늦은 시간의 농도 기여가 커질수록 평균 체류시간이 늘어납니다.
> - **오해 방지:** 이 값은 보정 전 관측 MRT일 수 있습니다. IV infusion이나 경구 입력이 있으면 MIT를 빼야 해석 가능한 체류시간이 됩니다.
...
<!-- SLIDE_START: S27 | title: 투여시간 보정과 MIT | source_anchor: C2-C-input-time-correction -->
> **핵심 메시지:** 관측 MRT에는 입력 지연시간이 섞이므로 체류시간을 해석하려면 투여 방식별 MIT를 빼야 합니다.
...
>
> $$
> \underbrace{MRT_{\text{corrected}}}_{\text{보정 MRT}}=\underbrace{\frac{AUMC}{AUC}}_{\text{관측 MRT}}-\underbrace{MIT}_{\text{평균 입력 시간}},\qquad MIT\in\left\{\underbrace{0}_{\text{IV bolus}},\underbrace{T_{inf}/2}_{\text{일정 주입}},\underbrace{1/K_a}_{\text{1차 혈관외 입력}}\right\}
> $$
>
> - **항별 의미:** 좌변은 입력시간을 제거한 체류시간입니다. 우변은 관측 MRT에서 투여 방식이 만든 평균 입력 지연시간을 뺀 값입니다.
> - **실무 직관:** $T_{inf}$가 길거나 $K_a$가 작아져 $1/K_a$가 커지면 차감해야 할 입력시간이 커져 보정 MRT는 짧아집니다.
> - **오해 방지:** MIT를 빼지 않으면 투여 과정의 지연을 몸 안 체류시간으로 오해해 $V_{ss}$까지 왜곡할 수 있습니다.


경구 흡수(oral absorption)에서 $K_a$는 단순 흡수만이 아니라 분해(degradation) 등 병렬 과정을 포함할 수 있습니다. 따라서 $1/K_a$를 “순수 흡수시간”으로 과해석하지 않습니다. [G p.150]

<!-- SLIDE_START: S28 | title: C2 맥락 확장 | source_anchor: C2-D-context-extension -->
> **핵심 메시지:** MRT는 혈장에만 갇힌 개념이 아니라 대사체와 소변 자료에서도 입력 체류시간을 분리해 해석할 수 있습니다.
...
<!-- SLIDE_START: S29 | title: C2 경계 조건 | source_anchor: C2-E-boundary-conditions -->
> **핵심 메시지:** MRT의 안정성은 평균이라는 이름이 아니라 꼬리 표본추출과 입력 방식 식별의 안정성에서 나옵니다.
...
<!-- SLIDE_START: S30 | title: C2 원자 노트와 요약 | source_anchor: C2-F-atom-recap -->
> **핵심 메시지:** MRT를 읽는다는 것은 AUMC 꼬리와 MIT 보정이 모두 방어 가능한지 확인하는 일입니다.
...
<!-- SLIDE_START: S31 | title: C2 AUMC와 일차 모멘트 그림 | source_anchor: C2-fig-AUMC-first-moment -->
> **핵심 메시지:** 농도 곡선과 t·C(t) 곡선을 나란히 보면 AUMC가 왜 꼬리 표본추출에 더 민감한지 즉시 보입니다.
...
<!-- SLIDE_START: S32 | title: C2 숙련 노트와 실무 체화 요약 | source_anchor: C2-mastery-practice-brief -->
> **핵심 메시지:** MRT 값 하나를 보고 안심하면 안 되며 AUMC 외삽과 MIT 보정 근거를 함께 보고해야 합니다.
...
<!-- SLIDE_START: S33 | title: C3 Apex 진입 | source_anchor: C3 -->
> **핵심 메시지:** C3에서는 AUC와 AUMC의 불확실성이 CL과 분포용적이라는 임상 파라미터 이름을 얻습니다.
...
<!-- SLIDE_START: S34 | title: C3 핵심 수식 진입 | source_anchor: C3-A-core-equations -->
> **핵심 메시지:** CL, Vss, Vz는 같은 표에 있어도 각각 제거 능력, 정상상태 분포, 말단기울기 기반 겉보기 부피를 묻습니다.
...
>
> $$
> \underbrace{CL}_{\text{전신 청소율}}=
> \frac{\underbrace{D_{iv}}_{\text{전신에 들어간 IV 용량}}}{\underbrace{AUC_0^\infty}_{\text{총 전신 노출}}}
> $$
>
> - **항별 의미:** 좌변은 전신 청소율입니다. 우변은 정맥 투여량을 총 AUC로 나누어 단위 시간당 제거되는 가상의 혈장 부피를 정의합니다.
> - **실무 직관:** 같은 용량에서 AUC가 커지면 CL은 작아지고, 같은 AUC에서 용량이 커지면 CL은 커집니다.
> - **오해 방지:** IV 식에서는 생체이용률 보정이 필요 없지만 AUC가 말단 꼬리 오류를 포함하면 CL도 그대로 왜곡됩니다.

<!-- SLIDE_START: S35 | title: 경구 겉보기 청소율 | source_anchor: C3-A-CLo -->
> **핵심 메시지:** 경구 투여에서는 AUC가 제거 능력뿐 아니라 생체이용률까지 함께 반영하므로 CL/F로 읽어야 합니다.
...
>
> $$
> \underbrace{CL_o}_{\text{경구 겉보기 청소율}}=
> \frac{\underbrace{CL}_{\text{진짜 청소율}}}{\underbrace{F}_{\text{생체이용률}}}
> =
> \frac{\underbrace{D_{po}}_{\text{경구 투여량}}}{\underbrace{AUC_0^\infty}_{\text{관측 전신 노출}}}
> $$
>
> - **항별 의미:** 좌변은 경구 자료에서 관측되는 겉보기 청소율입니다. 실제 CL이 생체이용률 $F$로 나뉘어 나타납니다.
> - **실무 직관:** $F$가 낮으면 같은 실제 CL이라도 $CL_o$ 또는 $CL/F$는 커져 보입니다. 경구 AUC가 작아져도 같은 방향으로 겉보기 CL이 증가합니다.
> - **오해 방지:** $CL_o$를 IV CL과 동일하게 해석하면 안 됩니다. 경구에서는 제거 능력과 흡수·초회통과가 섞여 있습니다.

<!-- SLIDE_START: S36 | title: 생체이용률 | source_anchor: C3-A-F -->
> **핵심 메시지:** 생체이용률은 경구 AUC와 IV AUC를 용량 보정해 전신 도달 분율을 비교하는 지표입니다.
...
> \underbrace{\frac{AUC_{po}}{AUC_{iv}}}_{\text{노출 비}}
> \cdot
> \underbrace{\frac{D_{iv}}{D_{po}}}_{\text{용량 보정}}
> $$
>
> - **항별 의미:** 좌변은 경구 투여 후 전신에 도달한 분율입니다. 우변은 경구와 IV AUC 비를 용량 차이로 보정한 값입니다.
> - **실무 직관:** 경구 AUC가 커지면 $F$는 커지고, 경구 용량이 IV 용량보다 크면 용량 보정을 통해 단순 AUC 비의 과대해석을 막습니다.
> - **오해 방지:** AUC 비만 보면 생체이용률을 잘못 판단할 수 있습니다. 반드시 투여량 차이를 함께 보정해야 합니다.

<!-- SLIDE_START: S37 | title: 정상상태 분포용적 | source_anchor: C3-A-Vss -->
> **핵심 메시지:** Vss는 AUC, AUMC, CL, 투여시간 보정이 모두 통과한 최종 요약값입니다.
...
> \underbrace{MRT}_{\text{보정 체류시간}}
> \cdot
> \underbrace{CL}_{\text{청소율}}
> =
> \frac{\underbrace{D_{iv}}_{\text{IV 용량}}\cdot \underbrace{AUMC_0^\infty}_{\text{시간가중 노출}}}{\underbrace{(AUC_0^\infty)^2}_{\text{총 노출의 제곱}}}
> $$
>
> - **항별 의미:** 좌변은 정상상태 분포 크기입니다. 우변은 평균 체류시간과 청소율의 곱이며, IV 자료에서는 용량·AUMC·AUC로 표현됩니다.
> - **실무 직관:** MRT가 길거나 CL이 클수록 $V_{ss}$는 커집니다. AUMC 꼬리가 커지면 $V_{ss}$도 부풀 수 있습니다.
> - **오해 방지:** 여기서 MRT는 입력시간 보정이 끝난 값이어야 합니다. MIT를 빼지 않은 MRT를 쓰면 투여 과정의 지연을 분포용적으로 오해할 수 있습니다.

<!-- SLIDE_START: S38 | title: 말단 분포용적 | source_anchor: C3-A-Vz -->
> **핵심 메시지:** Vz는 생리적 정상상태 분포가 아니라 CL과 말단기울기가 결합된 겉보기 부피입니다.
...
>
> $$
> \underbrace{V_z}_{\text{말단 겉보기 분포용적}}=
> \frac{\underbrace{CL}_{\text{청소율}}}{\underbrace{\lambda_z}_{\text{말단 기울기}}}
> =
> \frac{\underbrace{D_{iv}}_{\text{IV 용량}}}{\underbrace{AUC_0^\infty}_{\text{총 노출}}\cdot\underbrace{\lambda_z}_{\text{말단 기울기}}}
> $$
>
> - **항별 의미:** 좌변은 말단기울기에 기반한 겉보기 분포용적입니다. 우변은 청소율을 말단 소실속도상수로 나누거나, 용량을 AUC와 $\lambda_z$의 곱으로 나눈 형태입니다.
> - **실무 직관:** $\lambda_z$가 작아지면 $V_z$는 커집니다. 따라서 잘못 느린 말단기울기를 고르면 $V_z$가 크게 부풀 수 있습니다.
> - **오해 방지:** $V_z$는 $V_{ss}$와 같은 생리적 정상상태 분포용적이 아닙니다. 말단기울기 선택에 강하게 묶인 겉보기 값입니다.

<!-- SLIDE_START: S39 | title: 유효 반감기 | source_anchor: C3-A-effective-half-life -->
> **핵심 메시지:** 유효 반감기는 말단기울기 하나가 아니라 평균 체류시간을 반감기 언어로 변환한 값입니다.
...
> \underbrace{\ln(2)}_{\text{50\% 감소 변환상수}}
> \cdot
> \underbrace{MRT}_{\text{평균 체류시간}}
> $$
>
> - **항별 의미:** 좌변은 유효 반감기입니다. 우변은 평균 체류시간을 50% 감소 시간 척도로 변환합니다.
> - **실무 직관:** MRT가 길어지면 유효 반감기도 길어집니다. 다구획 상황에서는 말단기울기 하나보다 전체 체류시간 정보를 반영하는 시간 척도로 작동합니다.
> - **오해 방지:** 이 식은 전통적 $\ln(2)/\lambda_z$ 반감기와 같은 개념이 아닙니다. MRT 기반으로 다시 정의한 effective half-life입니다.

<!-- SLIDE_START: S40 | title: C3가 Apex인 이유 | source_anchor: C3-B-apex -->
> **핵심 메시지:** C3의 오류는 독립 오류가 아니라 C1의 AUC 꼬리와 C2의 AUMC·MIT 오류가 임상 파라미터로 응결된 결과입니다.
...
<!-- SLIDE_START: S41 | title: 짧은 반감기와 긴 입력시간의 특수 사례 | source_anchor: C3-C-short-half-life-special-case -->
> **핵심 메시지:** 입력 시간이 체류시간 계산을 지배하면 표준 Vss 공식의 적용 영역 자체를 의심해야 합니다.
...
>
> $$
> \underbrace{V_{ss}}_{\text{대안 정상상태 분포용적}}=
> \frac{\underbrace{K_0}_{\text{주입 속도}}\cdot \underbrace{AUC_{t^*\to\infty}}_{\text{정상상태 후 washout AUC}}}{\underbrace{C_{ss}^2}_{\text{정상상태 농도의 제곱}}}
> $$
>
> - **항별 의미:** 좌변은 특수 상황에서의 대안 $V_{ss}$입니다. 우변은 주입 속도와 정상상태 이후 washout AUC를 정상상태 농도 제곱으로 보정합니다.
> - **실무 직관:** washout AUC가 커지면 $V_{ss}$가 커지고, $C_{ss}$가 높으면 같은 washout 면적이라도 분포용적은 작게 계산됩니다.
> - **오해 방지:** 이 식은 표준식을 임의로 대체하는 편의식이 아닙니다. 반감기가 입력시간에 비해 짧아 표준식 적용 영역을 벗어나는 상황을 다루기 위한 대안입니다.

<!-- SLIDE_START: S42 | title: washout 기반 MRT 대안식 | source_anchor: C3-C-washout-MRT -->
> **핵심 메시지:** washout 기반 MRT는 입력시간 지배 상황에서 체류시간을 더 견고하게 읽기 위한 대안 경로입니다.
...
>
> $$
> \underbrace{MRT}_{\text{washout 기반 체류시간}}=
> \frac{\underbrace{AUC_{t^*\to\infty}}_{\text{정상상태 후 washout AUC}}}{\underbrace{C_{ss}}_{\text{정상상태 농도}}}
> $$
>
> - **항별 의미:** 좌변은 정상상태 이후 소실 곡선에서 계산한 MRT입니다. 우변은 washout AUC를 정상상태 농도로 나누어 시간 단위 체류지표를 만듭니다.
> - **실무 직관:** washout AUC가 크면 MRT가 길어지고, 같은 washout AUC라도 $C_{ss}$가 높으면 MRT는 짧아집니다.
> - **오해 방지:** 이 식은 일반적 $AUMC/AUC$ 식을 부정하지 않습니다. 입력시간이 지배적인 특수 상황에서 더 견고한 계산 경로를 제공하는 식입니다.
...
<!-- SLIDE_START: S43 | title: C3 한계와 flip-flop 위험 | source_anchor: C3-D-limitations -->
> **핵심 메시지:** flip-flop에서는 말단기울기가 제거가 아니라 흡수를 반영해도 결과가 그럴듯한 양수로 출력됩니다.
...
<!-- SLIDE_START: S44 | title: C3 원자 노트와 요약 | source_anchor: C3-E-atom-recap -->
> **핵심 메시지:** Vss를 볼 때는 언제나 어떤 MRT를 썼고 입력시간을 뺐는지 함께 물어야 합니다.
...
<!-- SLIDE_START: S45 | title: C3 washout AUC 그림 | source_anchor: C3-fig-2-126 -->
> **핵심 메시지:** washout AUC를 보면 §2.8.8 대안식이 공식 교체가 아니라 적용 영역 변경이라는 점이 드러납니다.
...
<!-- SLIDE_START: S46 | title: C3 실무 체화 요약 | source_anchor: C3-practice-brief -->
> **핵심 메시지:** 표준 Vss와 대안 Vss의 불일치는 어느 값이 예쁜가가 아니라 현재 데이터가 표준식 적용 영역인지의 진단입니다.
...
<!-- SLIDE_START: S47 | title: C4 노출 지표 진입 | source_anchor: C4 -->
> **핵심 메시지:** C4의 핵심은 같은 투여량이라도 몸과 표적이 실제로 본 농도-시간 부담은 다를 수 있다는 것입니다.
...
<!-- SLIDE_START: S48 | title: 노출 지표 선택 | source_anchor: C4-A-exposure-measures -->
> **핵심 메시지:** 노출 지표는 목록이 아니라 endpoint가 peak, cumulative, threshold, unbound 중 무엇을 묻는지에 맞춘 선택입니다.
...
>
> $$
> \underbrace{C_u}_{\text{비결합 농도}}=\underbrace{C_{total}}_{\text{총 농도}}\cdot\underbrace{f_u}_{\text{비결합 분율}}
> $$
>
> - **항별 의미:** 좌변은 약리학적으로 사용 가능한 비결합 농도입니다. 우변은 총 농도에 비결합 분율을 곱해 계산합니다.
> - **실무 직관:** 총 농도가 같아도 $f_u$가 커지면 비결합 농도는 커지고, 단백 결합이 강해 $f_u$가 작아지면 비결합 농도는 작아집니다.
> - **오해 방지:** 총 농도 기준 비교가 항상 약리학적 potency 비교와 일치하지 않습니다. 단백 결합 차이가 있으면 비결합 농도 기준으로 순위가 바뀔 수 있습니다.


<!-- SLIDE_START: S49 | title: §2.9 사례 압축 | source_anchor: C4-B-cases -->
> **핵심 메시지:** §2.9의 사례들은 모두 투여량보다 전신 또는 비결합 노출이 반응을 더 잘 설명한다는 한 문장으로 수렴합니다.
...
<!-- SLIDE_START: S50 | title: PK41 비선형 회귀 전환 | source_anchor: C4-C-PK41 -->
> **핵심 메시지:** PK41에서 용량 증가와 함께 CL이 감소하는 패턴은 NCA 최종값이 아니라 Michaelis-Menten 회귀 전환 신호입니다.
...
>
> $$
> \underbrace{V\cdot\frac{dC}{dt}}_{\text{체내 농도 변화량}}=
> \underbrace{In}_{\text{입력 속도}}-
> \underbrace{CL\cdot C}_{\text{제거 속도}},\qquad
> \underbrace{CL}_{\text{농도 의존 청소율}}=
> \frac{\underbrace{V_{max}}_{\text{최대 제거능}}}{\underbrace{K_m+C}_{\text{포화 경계 + 농도}}}
> $$
>
> - **항별 의미:** 첫 식의 좌변은 분포용적을 고려한 농도 변화율이고, 우변은 입력에서 제거를 뺀 질량수지입니다. 둘째 식은 청소율이 농도에 따라 달라지는 Michaelis-Menten 형태를 보여줍니다.
> - **실무 직관:** 입력 $In$이 커지면 농도 상승 방향으로 작용하고, $CL\cdot C$가 커지면 농도 감소 방향으로 작용합니다. $C$가 커질수록 $CL=V_{max}/(K_m+C)$는 작아져 용량 제한 소실 패턴을 만듭니다.
> - **오해 방지:** PK41에서 CL은 최종 고정 파라미터가 아닙니다. 비선형 상황에서는 $V_{max}$, $K_m$, $V$가 모델 언어의 중심이 됩니다.
...
<!-- SLIDE_START: S51 | title: C4 원자 노트와 요약 | source_anchor: C4-D-atom-recap -->
> **핵심 메시지:** 투여량은 투입량이고 노출은 몸이나 표적이 실제로 본 농도-시간 부담입니다.
...
<!-- SLIDE_START: S52 | title: C4 노출 지표 그림 | source_anchor: C4-fig-2-135 -->
> **핵심 메시지:** Cmax, AUC, Css, td는 농도-시간 프로파일의 서로 다른 부분을 요약하므로 같은 질문에 답하지 않습니다.
...
<!-- SLIDE_START: S53 | title: PK41 용량 제한 신호 그림 | source_anchor: C4-fig-41-2 -->
> **핵심 메시지:** 용량별 CL 감소는 기술 표의 숫자가 아니라 파라미터 언어를 바꾸라는 신호입니다.
...
<!-- SLIDE_START: S54 | title: C4 판단 렌즈와 실무 체화 요약 | source_anchor: C4-judgment-practice-brief -->
> **핵심 메시지:** 용량 의존적 CL 패턴을 보면 질문은 CL이 얼마인가에서 Vmax, Km, V로 설명되는가로 바뀝니다.
...
<!-- SLIDE_START: S55 | title: §5 혼동 쌍 진입 | source_anchor: sec5-confusion-pairs -->
> **핵심 메시지:** 혼동쌍은 같은 출력 표에 가까이 놓인 값들이 실제로는 서로 다른 질문에 답한다는 점을 드러냅니다.

# §5 — 혼동 쌍 해부(Confusion Pair Dissection)

<!-- CONFUSION -->
<!-- SLIDE_START: S56 | title: AUC vs AUMC | source_anchor: sec5-pair1-AUC-AUMC -->
> **핵심 메시지:** AUMC는 AUC보다 late-time 농도를 더 강하게 가중하므로 같은 lambda_z 오류도 MRT와 Vss에서 더 크게 드러납니다.
...
>
> $$
> \overbrace{AUC_{tail}=\frac{C_{last}}{\lambda_z}}^{\text{1차 꼬리}}\qquad\text{vs}\qquad\overbrace{AUMC_{tail}=\frac{t_{last}C_{last}}{\lambda_z}+\frac{C_{last}}{\lambda_z^2}}^{\text{시간가중 꼬리 + 민감도 증폭}}
> $$
>
> - **항별 의미:** 왼쪽은 AUC의 말단 꼬리이고, 오른쪽은 AUMC의 말단 꼬리입니다. AUMC에는 시간가중 항과 $1/\lambda_z^2$ 항이 함께 들어갑니다.
> - **실무 직관:** $\lambda_z$가 작아지면 두 꼬리가 모두 커지지만 AUMC 꼬리는 제곱항 때문에 더 크게 흔들립니다.
> - **오해 방지:** AUC와 AUMC를 같은 정도로 안정적인 면적이라고 보면 안 됩니다. AUMC의 late-time 민감도가 MRT와 $V_{ss}$ 오류를 키웁니다.


<!-- CONFUSION -->
<!-- SLIDE_START: S57 | title: MRT vs 반감기 | source_anchor: sec5-pair2-MRT-half-life -->
> **핵심 메시지:** MRT는 분자 체류시간의 평균이고 반감기는 농도 50% 감소시간이므로 다구획에서는 같은 말이 아닙니다.
...
>
> $$
> \underbrace{MRT}_{\text{체류시간 평균}}=\frac{\underbrace{AUMC}_{\text{시간가중 노출}}}{\underbrace{AUC}_{\text{총 노출}}},\qquad \underbrace{t_{1/2}}_{\text{50\% 감소시간}}=\frac{\underbrace{\ln(2)}_{\text{변환상수}}}{\underbrace{\lambda_z}_{\text{말단기울기}}}
> $$
>
> - **항별 의미:** MRT는 시간가중 노출을 총 노출로 나눈 평균 체류시간이고, 반감기는 말단기울기를 50% 감소시간으로 바꾼 값입니다.
> - **실무 직관:** AUMC가 커지면 MRT가 길어지고, $\lambda_z$가 커지면 반감기는 짧아집니다.
> - **오해 방지:** 단일지수 1구획에서는 두 값이 단순 비율로 연결될 수 있지만, 다구획에서는 MRT가 분포기 정보를 흡수하고 반감기는 말단기울기만 기술합니다.


<!-- CONFUSION -->
<!-- SLIDE_START: S58 | title: Vss vs Vz | source_anchor: sec5-pair3-Vss-Vz -->
> **핵심 메시지:** Vss와 Vz의 불일치 방향은 AUMC·입력보정 문제인지 lambda_z 문제인지 구분하는 단서가 됩니다.
...
>
> $$
> \underbrace{V_{ss}}_{\text{정상상태 분포}}=\underbrace{MRT}_{\text{체류시간}}\cdot\underbrace{CL}_{\text{청소율}},\qquad \underbrace{V_z}_{\text{말단 겉보기 부피}}=\frac{\underbrace{CL}_{\text{청소율}}}{\underbrace{\lambda_z}_{\text{말단기울기}}}
> $$
>
> - **항별 의미:** $V_{ss}$는 체류시간과 청소율로 정상상태 분포를 요약하고, $V_z$는 청소율을 말단기울기로 나눈 말단 기반 겉보기 부피입니다.
> - **실무 직관:** MRT가 커지면 $V_{ss}$가 커지고, $\lambda_z$가 작아지면 $V_z$가 커집니다. 두 값의 어긋남은 서로 다른 가정 실패를 가리킵니다.
> - **오해 방지:** $V_z$를 $V_{ss}$처럼 부하용량 계산에 그대로 쓰면 말단기울기 오류를 생리적 분포용적으로 착각할 수 있습니다.


<!-- SLIDE_START: S59 | title: §5 요약과 숙련 노트 | source_anchor: sec5-recap -->
> **핵심 메시지:** 혼동쌍의 실무 문제는 용어 암기가 아니라 질문과 파라미터의 불일치입니다.
...
<!-- SLIDE_START: S60 | title: §7 자기 테스트 진입 | source_anchor: sec7-self-test -->
> **핵심 메시지:** 자기 테스트는 계산 암기가 아니라 AUC 신뢰도, 비선형 전환, 노출 해석, 적용 경계를 회상하도록 설계되어 있습니다.

# §7 — 자기 테스트: 능동 회상 모듈(Active Recall Module)

<!-- SELF-TEST -->
<!-- SLIDE_START: S61 | title: Q1 사다리꼴 AUC 공식 | source_anchor: sec7-Q1 -->
> **핵심 메시지:** 선형과 로그-선형 사다리꼴법은 같은 AUC 계산이라도 적용 구간과 편향 방향이 다릅니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S62 | title: Q2 lambda_z 조건 | source_anchor: sec7-Q2 -->
> **핵심 메시지:** lambda_z는 최소 관측점 수와 충분한 terminal phase 길이가 확인되어야 외삽의 근거가 됩니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S63 | title: Q3 높은 외삽 AUC | source_anchor: sec7-Q3 -->
> **핵심 메시지:** 외삽 비율이 권고선을 넘으면 AUC는 확정 파라미터보다 예비 추정값으로 보고해야 합니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S64 | title: Q4 PK41 비선형 전환 | source_anchor: sec7-Q4 -->
> **핵심 메시지:** PK41의 NCA-CL 감소는 최종 CL 보고가 아니라 Michaelis-Menten 회귀의 초기값으로 내려가는 신호입니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S65 | title: Q5 MRT 계산 | source_anchor: sec7-Q5 -->
> **핵심 메시지:** 혈장 MRT는 AUMC를 AUC로 나눈 시간 단위 값이므로 계산 결과와 단위를 함께 확인해야 합니다.
...
>
> $$
> \underbrace{MRT}_{\text{혈장 평균 체류시간}}=\frac{\underbrace{177}_{\text{AUMC}}}{\underbrace{44.2}_{\text{AUC}}}\approx \underbrace{4.0\ \mathrm{hr}}_{\text{시간 단위 결과}}
> $$
>
> - **항별 의미:** 좌변은 혈장 MRT이고, 우변은 AUMC 수치를 AUC 수치로 나눈 계산입니다.
> - **실무 직관:** AUMC가 더 커지면 MRT가 길어지고, AUC가 더 커지면 같은 AUMC에서 MRT는 짧아집니다.
> - **오해 방지:** 계산값만 맞추는 문제가 아니라 단위가 시간이 되는지 확인해야 합니다. AUMC의 hr² 성격이 AUC로 나뉘어 hr이 됩니다.


<!-- SELF-TEST -->
<!-- SLIDE_START: S66 | title: Q6 IV CL과 경구 CL/F | source_anchor: sec7-Q6 -->
> **핵심 메시지:** IV에서는 CL을 직접 계산하지만 경구에서는 생체이용률이 섞인 CL/F로 해석해야 합니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S67 | title: Q7 endpoint별 노출 지표 | source_anchor: sec7-Q7 -->
> **핵심 메시지:** 노출 지표 선택은 endpoint가 peak, cumulative, threshold 중 무엇에 가까운지에 따라 달라집니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S68 | title: Q8 총 농도와 비결합 농도 | source_anchor: sec7-Q8 -->
> **핵심 메시지:** 단백 결합이 다르면 총 농도 기준 순위가 비결합 농도 기준에서는 뒤집힐 수 있습니다.
...
<!-- SELF-TEST -->
<!-- SLIDE_START: S69 | title: Q9 표준식과 대안식의 SOP 딜레마 | source_anchor: sec7-Q9 -->
> **핵심 메시지:** 표준식과 대안식의 차이는 SOP 일탈이 아니라 적용 영역 이탈을 문서화하는 근거로 다뤄야 합니다.
...
<!-- SLIDE_START: S70 | title: §7 요약 | source_anchor: sec7-recap -->
> **핵심 메시지:** Q1–Q9는 NCA 계산 자체보다 어떤 조건에서 계산값을 믿을 수 있는지 점검하게 합니다.
...
<!-- SLIDE_START: S71 | title: §8 메타프레임 진입 | source_anchor: sec8-meta-frame -->
> **핵심 메시지:** NCA는 PopPK의 대체물이 아니라 모델링 전후에 가정과 규모를 확인하는 기준점입니다.

# §8 — 메타프레임과 거시 구조(Meta-Frame & Big Picture)

<!-- SLIDE_START: S72 | title: NCA의 위치 설정 | source_anchor: sec8-A-positioning -->
> **핵심 메시지:** NCA는 모델 선택 전 EDA와 초기값, 모델 추정 후 노출 규모 확인이라는 두 위치에서 작동합니다.
...
<!-- SLIDE_START: S73 | title: 추적해야 할 실패 모드 | source_anchor: sec8-B-failure-modes -->
> **핵심 메시지:** NCA의 주요 실패는 말단기울기 오염, 입력보정 누락, 비선형 CL 오독, NCA-IIV 과사용으로 요약됩니다.
...
<!-- SLIDE_START: S74 | title: 전문가 해석 지점 | source_anchor: sec8-C-professional-moat -->
> **핵심 메시지:** 전문가는 NCA 표의 숫자보다 그 숫자가 생긴 가정 감사 추적 기록을 먼저 읽습니다.
...
<!-- SLIDE_START: S75 | title: 최종 압축 원칙 | source_anchor: sec8-D-final-doctrine -->
> **핵심 메시지:** NCA의 본질은 최소한의 선형 소실 가정으로 면적과 시간 모멘트를 요약하는 것입니다.
...
<!-- SLIDE_START: S76 | title: §8 실무 체화와 숙련 노트 | source_anchor: sec8-practice-mastery -->
> **핵심 메시지:** 완성된 NCA 보고서는 숫자 표가 아니라 말단기울기부터 노출 지표까지 추적 가능한 가정 감사 기록이어야 합니다.
