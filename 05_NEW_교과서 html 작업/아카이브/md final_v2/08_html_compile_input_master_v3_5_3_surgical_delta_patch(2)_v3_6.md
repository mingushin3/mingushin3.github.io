...
> **핵심 메시지:** 이 장은 비선형 PK와 DDI를 암기 목록이 아니라 기전 분류와 실무 판단의 순서로 읽어야 합니다.

## PART A — Learner-Facing Complete Handout

<!-- SLIDE_START: S02 | title: 학습자 네비게이션 안내 | source_anchor: learner-navigation -->
> **핵심 메시지:** 먼저 진단 그림으로 비선형성을 확인하고, 이후 capacity·time·binding·DDI 기전으로 분기해야 합니다.
...
> **핵심 메시지:** 비선형 PK의 첫 판단은 선형성이 깨졌는지가 아니라 어떤 물리적 병목이 그 깨짐을 만들었는지입니다.
...
>
> $$
> \underbrace{V_{max}/K_m}_{\text{저농도 intrinsic slope}}\quad;
> \quad\underbrace{f_u}_{\text{비결합 분율}}\quad;
> \quad\underbrace{f_m}_{\text{경로 의존도}}
> $$
>
> - **항별 의미:** 이 inline formula set의 좌변/기호들은 예측식이라기보다, 이후 비선형 PK triage에서 어떤 병목을 먼저 확인할지 정의하는 좌표입니다. $V_{max}/K_m$는 저농도 처리능, $f_u$는 약리학적으로 의미 있는 자유 약물 분율, $f_m$은 DDI에서 막힌 경로가 전체 제거 중 차지하는 몫입니다.
> - **실무 직관:** $V_{max}/K_m$가 작거나 capacity에 가까우면 용량 변화가 노출을 비대칭적으로 흔들고, $f_u$가 바뀌면 total 농도 해석이 흔들리며, $f_m$이 클수록 같은 inhibitor라도 AUC ratio가 커집니다.
> - **오해 방지:** 세 기호는 독립 암기 항목이 아니라 각각 capacity, binding, DDI 취약성을 가리키는 triage 변수입니다.
...
> **핵심 메시지:** 진단·용량·시간·결합·DDI 층위를 분리하면 같은 관측값도 서로 다른 기전 후보로 읽을 수 있습니다.
...
> **핵심 메시지:** 첫 30초의 시각 패턴은 결론이 아니라 다음 검증 실험을 선택하는 triage 신호입니다.
...
>
> $$
> \underbrace{E_H}_{\text{간 추출률}}
> =
> \frac{\underbrace{CL_H}_{\text{간 청소율}}}{\underbrace{Q_H}_{\text{간 혈류}}}
> \qquad;
> \qquad
> \underbrace{V_{max}/K_m}_{\text{저농도 처리능}}\approx \underbrace{Q_H}_{\text{flow limit}}
> $$
>
> - **항별 의미:** 좌변 $E_H$는 간을 한 번 통과할 때 제거되는 비율을 정의하며, $CL_H$는 간 제거 능력, $Q_H$는 간으로 공급되는 혈류 한계입니다.
> - **실무 직관:** $CL_H$가 커져 $Q_H$에 가까워질수록 효소 capacity가 아니라 flow가 실제 상한이 됩니다. $V_{max}/K_m$가 $Q_H$ 근처이면 low-extraction과 high-extraction 해석의 경계에 있습니다.
> - **오해 방지:** 효소 parameter가 커졌다고 항상 간 청소율이 계속 커지는 것은 아닙니다. 혈류가 상한이 되면 enzyme saturation과 flow limitation을 분리해야 합니다.
...
> **핵심 메시지:** 이 도식은 중첩 원리 실패를 모델링·TDM·DDI 의사결정으로 번역하는 경로를 보여 주어야 합니다.
...
> **핵심 메시지:** 각 카드는 한 기전의 이름보다 그 기전을 실무 판단으로 바꾸는 질문을 중심으로 읽어야 합니다.

# §2. 개념 해부 카드(Concept Anatomy Cards)

<!-- SLIDE_START: S08 | title: C1 비선형성 진단 | source_anchor: c1-nonlinearity-diagnostics -->
> **핵심 메시지:** 비선형성 진단은 모델 선택이 아니라 어떤 파라미터가 상수가 아니게 되었는지 묻는 첫 필터입니다.
...
> **핵심 메시지:** AUC/Dose의 방향성은 CL·F·분포·결합·시간 의존성 중 다음에 검증할 축을 지정합니다.
...
>
> $$
> \underbrace{AUC/Dose}_{\text{용량 보정 노출}}
> $$
>
> - **항별 의미:** 좌변은 투여량 1단위당 전신 노출을 정의합니다. 분자 $AUC$는 전체 노출, 분모 $Dose$는 용량 크기 보정입니다.
> - **실무 직관:** 용량이 증가할 때 $AUC/Dose$가 올라가면 CL 감소 또는 F 증가를, 내려가면 CL 증가 또는 F 감소를 의심합니다.
> - **오해 방지:** $AUC/Dose$ 방향만으로 mechanism을 확정하면 안 됩니다. 같은 방향 변화가 서로 다른 CL/F mechanism에서 나올 수 있습니다.
...
> **핵심 메시지:** 비선형 PK에서는 spaghetti plot보다 dose-normalized plot과 AUC/Dose plot이 먼저입니다.
...
> **핵심 메시지:** C1 figure는 AUC/Dose 방향을 기전 명명 전의 시각 분류 문제로 바꿔 주어야 합니다.
...
> **핵심 메시지:** MM 동역학의 핵심은 포화라는 단어가 아니라 CL이 농도의 함수가 되어 용량 조정이 비대칭화된다는 점입니다.
...
>
> $$
> \underbrace{K_m+C}_{\text{MM capacity 분모}}
> \quad\Longrightarrow\quad
> \underbrace{K_m\left(1+C_{u,I}/K_I\right)+C}_{\text{competitive inhibitor가 추가된 분모}}
> $$
>
> - **항별 의미:** 좌변은 substrate 농도 $C$가 enzyme capacity에 접근할 때 나타나는 MM 분모이고, 우변은 inhibitor exposure $C_{u,I}$가 $K_I$ 대비 얼마나 큰지에 따라 apparent $K_m$가 증가하는 DDI 분모입니다.
> - **실무 직관:** $C_{u,I}/K_I$가 커질수록 substrate가 같은 처리 속도를 내려면 더 높은 농도가 필요하므로, inhibited pathway의 유효 청소율이 감소합니다.
> - **오해 방지:** C2와 C5의 식은 서로 다른 장의 새로운 공식이 아니라 같은 mass action 분모가 capacity와 DDI 맥락에서 반복된 것입니다.

<!-- SLIDE_START: S13 | title: C2 핵심 수식 | source_anchor: c2-core-equations -->
> **핵심 메시지:** MM 수식은 같은 분모가 CL 감소, rate plateau, 정상상태 민감도를 동시에 만든다는 사실을 보여 줍니다.
...
>
> $$
> \underbrace{CL(C)}_{\text{농도 의존 청소율}}
> =
> \frac{\overbrace{V_{max}}^{\text{최대 처리능}}}{\underbrace{K_m}_{\text{반포화 농도}}+\underbrace{C}_{\text{현재 농도}}}
> $$
>
> - **항별 의미:** 좌변은 특정 농도 $C$에서의 apparent clearance를 예측합니다. $V_{max}$는 효소/수송체가 낼 수 있는 최대 처리능, $K_m$은 capacity가 절반 수준으로 읽히는 농도 scale, $C$는 현재 substrate 농도입니다.
> - **실무 직관:** $C$가 증가하면 분모가 커져 $CL(C)$는 감소합니다. $V_{max}$가 크면 전체 처리능이 커지고, $K_m$이 크면 같은 농도에서 포화가 덜 진행된 것처럼 보입니다.
> - **오해 방지:** 이 식은 clearance가 상수라는 선형 PK 직관을 깨는 식입니다. $CL(C)$ 감소를 곧바로 F 증가나 adherence 변화로 해석하면 안 됩니다.
...
>
> $$
> \underbrace{Rate}_{\text{제거/처리 속도}}
> =
> \frac{\overbrace{V_{max}}^{\text{속도 상한}}\cdot\underbrace{C}_{\text{기질 농도}}}{\underbrace{K_m}_{\text{농도 scale}}+\underbrace{C}_{\text{포화 구동 농도}}}
> $$
>
> - **항별 의미:** 좌변은 단위시간당 처리되는 양 또는 농도 변화 속도를 예측합니다. 분자는 농도가 올라갈수록 처리 속도를 증가시키지만, 분모의 $K_m+C$가 그 증가를 포화시킵니다.
> - **실무 직관:** 낮은 $C$에서는 rate가 거의 $C$에 비례하고, 높은 $C$에서는 rate가 $V_{max}$에 접근합니다.
> - **오해 방지:** high concentration에서 rate가 plateau에 접근한다고 해서 모든 시간 구간이 완전한 zero-order라는 뜻은 아닙니다. 농도가 내려오면 다시 1차처럼 보이는 영역으로 들어갑니다.
...
>
> $$
> \underbrace{\frac{dC}{dt}}_{\text{농도 변화율}}
> =
> -\frac{\overbrace{V_{max}}^{\text{최대 제거속도}}\cdot\underbrace{C}_{\text{현재 농도}}}{\underbrace{K_m+C}_{\text{포화 분모}}}
> $$
>
> - **항별 의미:** 좌변은 시간에 따른 농도 감소율을 예측합니다. 음수 부호는 제거로 농도가 내려감을 뜻하고, 우변은 MM 제거 속도가 농도에 의해 포화되는 구조를 보여 줍니다.
> - **실무 직관:** $C$가 높을수록 제거 속도는 증가하지만 $V_{max}$를 넘지 못합니다. $K_m$이 큰 경우 같은 $C$에서 덜 포화되어 더 1차에 가깝게 보입니다.
> - **오해 방지:** $V_{max}$의 단위가 농도/시간인지 양/시간인지에 따라 volume scaling이 필요합니다. 원문처럼 약식 표기일 때 단위 해석을 생략하면 안 됩니다.
...
>
> $$
> \underbrace{AUC}_{\text{전신 노출}}
> =
> \frac{\underbrace{C^0}_{\text{초기 농도}}}{2\overbrace{V_{max}}^{\text{최대 제거능}}}
> \left(
> \underbrace{K_m}_{\text{capacity 농도 scale}}+\frac{\underbrace{C^0}_{\text{초기 농도}}}{2}
> \right)
> \quad\text{(IV bolus, MM)}
> $$
>
> - **항별 의미:** 좌변은 IV bolus 후 총 노출을 예측합니다. $C^0$는 시작 농도, $V_{max}$는 제거 capacity, $K_m$은 포화가 노출에 얼마나 남는지를 결정하는 농도 scale입니다.
> - **실무 직관:** $C^0$가 커질수록 AUC는 단순 비례보다 더 빠르게 커질 수 있고, $V_{max}$가 커질수록 AUC는 낮아집니다.
> - **오해 방지:** AUC 식 안에 $K_m$이 들어 있다는 것은 AUC가 CL 상수 하나로만 정해지지 않는다는 뜻입니다. AUC 증가를 곧바로 F 증가로 해석하면 안 됩니다.
...
> **핵심 메시지:** 저농도에서는 1차처럼 보이고 고농도에서는 capacity가 지배하므로 같은 약물도 농도 영역에 따라 다르게 읽힙니다.
...
>
> $$
> \underbrace{C\ll K_m}_{\text{저농도 한계}}\Rightarrow \underbrace{CL}_{\text{거의 상수}}\approx \frac{V_{max}}{K_m}
> \qquad;
> \qquad
> \underbrace{C\gg K_m}_{\text{고농도 한계}}\Rightarrow \underbrace{Rate}_{\text{처리속도}}\approx V_{max}
> $$
>
> - **항별 의미:** 좌측 극한은 농도가 $K_m$보다 충분히 낮아 1차처럼 보이는 영역을, 우측 극한은 농도가 $K_m$보다 충분히 높아 처리 속도가 capacity 상한에 접근하는 영역을 정의합니다.
> - **실무 직관:** 저농도에서는 용량 변화가 비교적 예측 가능하지만, 고농도에서는 작은 용량 증가도 AUC/Css를 크게 밀어 올릴 수 있습니다.
> - **오해 방지:** MM kinetics를 하나의 zero-order label로 외우면 안 됩니다. 실제 해석은 농도 영역이 $K_m$에 비해 어디에 있는지로 결정됩니다.

<!-- SLIDE_START: S15 | title: C2 임상 정량 앵커 | source_anchor: c2-clinical-anchors -->
> **핵심 메시지:** phenytoin·alcohol·ascorbic acid는 비선형성이 항상 같은 방향의 위험을 뜻하지 않음을 보여 주는 세 앵커입니다.
...
>
> $$
> \underbrace{C_{u,ss}}_{\text{정상상태 비결합 농도}}
> =
> \frac{\underbrace{K_m}_{\text{unbound 농도 scale}}\cdot\underbrace{R_0}_{\text{투여 속도}}}{\underbrace{V_m-R_0}_{\text{남은 capacity 여유}}}
> $$
>
> - **항별 의미:** 좌변은 정상상태 unbound 농도를 예측합니다. $R_0$는 투여 속도, $V_m$은 최대 제거 capacity, $V_m-R_0$는 capacity까지 남은 여유입니다.
> - **실무 직관:** $R_0$가 $V_m$에 가까워질수록 분모가 작아져 $C_{u,ss}$가 급격히 증가합니다. $K_m$이 크면 같은 $R_0$에서 필요한 농도 scale이 커집니다.
> - **오해 방지:** 이 식은 phenytoin 용량 증가가 선형적 농도 증가가 아님을 보여 줍니다. $R_0$가 $V_m$에 근접하는 영역에서 작은 증량은 큰 독성 위험이 됩니다.
...
> **핵심 메시지:** Vmax와 Km은 곡률 영역을 통과하는 데이터가 있어야 분리되며, 좁은 농도 범위에서는 ridge에 잠깁니다.
...
> **핵심 메시지:** Vmax–Km Ridge Lock은 수렴 성공이 예측 성공을 보장하지 않는다는 모델링 경고입니다.
...
> **핵심 메시지:** 단일 용량 적합이 좋아도 곡률 영역을 보지 못하면 다중 투여 예측은 구조적으로 실패할 수 있습니다.
...
> **핵심 메시지:** CL이 농도 의존적이면 AUC 증가는 F 증가와 CL 감소를 구분하지 못하므로 생체이용률 판단이 위험해집니다.
...
> **핵심 메시지:** C2 figure는 같은 분모 직관이 CL 감소, rate plateau, 정상상태 민감도로 동시에 나타남을 확인하게 해야 합니다.
...
> **핵심 메시지:** 시간 의존성은 농도 변화가 아니라 효소·수송체·생리학적 상태 자체의 시계가 움직이는 현상입니다.
...
> **핵심 메시지:** turnover ODE는 효소 풀의 생성과 소실을 분리하여 CL(t)가 왜 늦게 변하는지 설명합니다.
...
>
> $$
> \underbrace{\frac{dE}{dt}}_{\text{효소량 변화율}}
> =
> \underbrace{R_{in}}_{\text{효소 생성}}-
> \underbrace{k_{out}E}_{\text{효소 소실}}
> \qquad;
> \qquad
> \underbrace{E_{ss}}_{\text{정상상태 효소량}}
> =
> \frac{\underbrace{R_{in}}_{\text{생성 속도}}}{\underbrace{k_{out}}_{\text{소실 속도상수}}}
> $$
>
> - **항별 의미:** 첫 식의 좌변은 효소 pool이 시간에 따라 얼마나 변하는지 예측하고, 두 번째 식은 생성과 소실이 같아진 정상상태 효소량을 정의합니다.
> - **실무 직관:** $R_{in}$이 증가하면 효소량과 capacity가 증가하고, $k_{out}$이 작으면 회복·소실이 느려져 induction/MBI 효과가 오래 지속됩니다.
> - **오해 방지:** 약물 농도가 빨리 사라져도 enzyme pool은 $k_{out}$ 시계로 회복됩니다. MBI washout을 perpetrator half-life만으로 잡으면 부족할 수 있습니다.
...
>
> $$
> \underbrace{CL_H}_{\text{간 청소율}}
> =
> \frac{\underbrace{Q_H}_{\text{간 혈류}}\cdot\underbrace{f_u}_{\text{비결합 분율}}\cdot\underbrace{CL_{int}}_{\text{내인성 처리능}}}{\underbrace{Q_H}_{\text{flow limit}}+\underbrace{f_u\cdot CL_{int}}_{\text{unbound intrinsic capacity}}}
> $$
>
> - **항별 의미:** 좌변은 간 청소율을 예측하고, 우변은 혈류 공급 한계와 비결합 intrinsic capacity가 함께 결정하는 well-stirred 구조입니다.
> - **실무 직관:** $f_u\cdot CL_{int}$가 작으면 capacity가 지배하고, 매우 커지면 $CL_H$는 $Q_H$에 접근해 flow-limited가 됩니다.
> - **오해 방지:** induction이 $CL_{int}$를 올려도 $CL_H$가 무한히 증가하지 않습니다. high-extraction 영역에서는 혈류가 상한이 됩니다.

<!-- SLIDE_START: S23 | title: C3 자가유도 및 타가유도 앵커 | source_anchor: c3-induction-anchors -->
> **핵심 메시지:** 반복투여 trough envelope는 enzyme clock을 가장 직접적으로 드러내는 설계 신호입니다.
...
> **핵심 메시지:** MBI는 억제제가 사라지는 문제가 아니라 효소 활성이 다시 만들어질 때까지 기다리는 문제입니다.
...
> **핵심 메시지:** 같은 inhibitor exposure라도 reversible inhibition과 MBI는 enzyme turnover 때문에 자릿수 단위로 다른 결과를 낼 수 있습니다.
...
> **핵심 메시지:** Trough Envelope Drift와 Washout Trap은 농도 시계와 효소 시계를 혼동할 때 발생하는 대표 실패 신호입니다.
...
> **핵심 메시지:** C3 schematic은 reversible inhibition과 induction/MBI의 회복 시계가 다름을 한눈에 구분하게 해야 합니다.
...
> **핵심 메시지:** 결합 비선형성에서는 total concentration보다 unbound concentration과 계의 경계가 먼저입니다.
...
>
> $$
> \underbrace{C_u}_{\text{정상상태 비결합 농도}}
> =
> \frac{\underbrace{K^0}_{\text{비결합 input rate}}}{\underbrace{CL_u}_{\text{비결합 청소율}}}
> $$
>
> - **항별 의미:** 좌변은 open in vivo system의 정상상태 unbound concentration을 정의합니다. $K^0$는 들어오는 unbound input, $CL_u$는 unbound drug를 제거하는 능력입니다.
> - **실무 직관:** $K^0$가 증가하면 $C_u$가 올라가고, $CL_u$가 증가하면 $C_u$가 내려갑니다. 단순 displacement는 total 농도를 흔들 수 있지만 이 비율 자체를 지속적으로 바꾸지 못할 수 있습니다.
> - **오해 방지:** closed in vitro binding에서 unbound fraction이 증가했다고 open in vivo steady-state $C_u$도 지속적으로 증가한다고 단정하면 안 됩니다.

<!-- SLIDE_START: S29 | title: C4 핵심 결합 방정식 | source_anchor: c4-binding-equation -->
> **핵심 메시지:** 포화성 결합식은 총 농도와 비결합 농도가 더 이상 선형적으로 대응하지 않는 지점을 보여 줍니다.
...
>
> $$
> \underbrace{C_b}_{\text{결합 농도}}
> =
> \frac{\underbrace{n}_{\text{결합 자리 수}}\cdot\underbrace{P_t}_{\text{총 단백/표적량}}\cdot\underbrace{C_u}_{\text{비결합 농도}}}{\underbrace{K_d}_{\text{해리상수}}+\underbrace{C_u}_{\text{포화 구동 농도}}}
> $$
>
> - **항별 의미:** 좌변은 결합된 약물 농도를 예측합니다. $nP_t$는 결합 가능한 총 capacity, $K_d$는 affinity scale, $C_u$는 결합을 구동하는 비결합 농도입니다.
> - **실무 직관:** $C_u$가 증가하면 $C_b$는 증가하지만 $nP_t$ capacity에 접근하며 포화됩니다. $K_d$가 낮을수록 더 낮은 $C_u$에서 결합이 커집니다.
> - **오해 방지:** total concentration만 보면 결합 포화와 clearance 변화를 구분하기 어렵습니다. $C_u$와 $f_u$를 별도로 봐야 합니다.
...
> **핵심 메시지:** 시험관의 displacement 직관을 열린 생체계에 그대로 적용하면 unbound exposure를 과대해석하게 됩니다.
...
> **핵심 메시지:** 포화성 단백·조직·표적 결합은 total 농도 신호를 왜곡하므로 용량 수준별 fu 확인이 필요합니다.
...
> **핵심 메시지:** 수송 포화는 같은 분모 구조를 공유하지만 secretion과 reabsorption의 방향 차이 때문에 임상 결과가 정반대가 됩니다.
...
>
> $$
> \underbrace{Rate}_{\text{수송 속도}}
> =
> \frac{\overbrace{T_m}^{\text{최대 수송능}}\cdot\underbrace{C_u}_{\text{비결합 농도}}}{\underbrace{K_T}_{\text{수송 포화 scale}}+\underbrace{C_u}_{\text{수송체 점유 구동}}}
> \quad\text{(Eq.16-3)}
> $$
>
> - **항별 의미:** 좌변은 수송체가 처리하는 속도를 예측합니다. $T_m$은 최대 수송 capacity, $K_T$는 수송체 포화 농도 scale, $C_u$는 실제 수송체가 보는 비결합 농도입니다.
> - **실무 직관:** $C_u$가 낮으면 rate는 거의 $C_u$에 비례하고, 높으면 $T_m$에 접근합니다. secretion이면 high concentration에서 CLR이 감소할 수 있고, reabsorption이면 escape valve처럼 CLR이 증가할 수 있습니다.
> - **오해 방지:** 분모 형태가 같아도 transport direction이 반대이면 임상 결과도 반대입니다.
...
> **핵심 메시지:** TMDD는 큰 분자가 아니라 고친화성·저용량 표적 결합이 disposition을 바꿀 때 생깁니다.
...
> **핵심 메시지:** parent–metabolite 동시 관측만으로 metabolite disposition을 단정하면 formation limitation과 disposition을 혼동할 수 있습니다.
...
> **핵심 메시지:** 치환 단독 DDI는 total concentration을 흔들 수 있지만 unbound steady state와 임상 판단을 지속적으로 바꾸지 않을 수 있습니다.
...
> **핵심 메시지:** C4 figure는 닫힌 시험관 결합 직관을 열린 생체계 해석으로 옮기는 오류를 막아야 합니다.
...
> **핵심 메시지:** DDI 예측의 핵심은 inhibitor 이름이 아니라 victim drug이 억제된 pathway에 얼마나 의존하는지입니다.
...
>
> $$
> \overbrace{CL=\frac{V_{max}}{K_m+C}}^{\text{농도-CL capacity}}
> \quad\Longleftrightarrow\quad
> \overbrace{C_{u,ss}=\frac{K_m R_0}{V_m-R_0}}^{\text{정상상태 분모 민감도}}
> \quad\Longleftrightarrow\quad
> \overbrace{\text{AUC ratio}\approx\frac{1}{\frac{f_m}{1+C_{u,I}/K_I}+(1-f_m)}}^{\text{DDI pathway 의존도}}
> $$
>
> - **항별 의미:** 세 좌변은 각각 clearance, steady-state unbound concentration, exposure ratio를 예측·정의합니다. 우변은 모두 capacity 또는 경쟁에 의해 분모가 작아지거나 커질 때 결과가 비선형적으로 바뀌는 구조를 공유합니다.
> - **실무 직관:** $C$와 $R_0$가 capacity에 접근하거나 $C_{u,I}/K_I$가 커져 inhibited pathway가 막히면, 작은 변화가 큰 exposure 변화로 번역됩니다.
> - **오해 방지:** 이 삼각형은 새 주장이나 새 guideline 식이 아니라 원문 세 식을 같은 mass action denominator 관점으로 읽는 학습용 연결입니다.
...
> **핵심 메시지:** AUC ratio 식은 inhibitor strength와 fm이 전체 노출 증가로 어떻게 번역되는지 계산하는 최소 골격입니다.
...
>
> $$
> \underbrace{\text{AUC ratio}}_{\text{병용/비병용 노출비}}
> \approx
> \frac{1}{
> \underbrace{\frac{f_m}{1+C_{u,I}/K_I}}_{\text{억제된 pathway의 남은 기여}}
> +
> \underbrace{(1-f_m)}_{\text{억제되지 않은 우회 경로}}
> }
> $$
>
> - **항별 의미:** 좌변은 inhibitor 병용 후 노출이 몇 배가 되는지 예측합니다. $f_m$은 억제되는 경로 의존도, $C_{u,I}/K_I$는 inhibitor 강도, $(1-f_m)$은 남은 우회 제거 경로입니다.
> - **실무 직관:** $C_{u,I}/K_I$가 커질수록 첫 항이 작아지고, $f_m$이 클수록 우회 경로가 작아져 AUC ratio가 커집니다.
> - **오해 방지:** 강한 inhibitor라도 $f_m$이 작으면 whole-body AUC ratio는 제한됩니다. inhibitor 이름만으로 dose reduction을 결정하면 안 됩니다.
...
> **핵심 메시지:** 임상 DDI 사례는 같은 억제라도 fm, half-life, active metabolite, route effect에 따라 크기가 달라짐을 보여 줍니다.
...
> **핵심 메시지:** 강함·중등도·약함 분류는 exposure 변화의 언어일 뿐이며 MBI 여부와 source boundary를 함께 표시해야 합니다.
...
> **핵심 메시지:** PM에서 남은 우회 경로까지 막히면 residual clearance가 작아져 exposure ratio가 급격히 커집니다.
...
>
> $$
> \underbrace{\text{Maximum exposure ratio}}_{\text{최대 노출 증가비}}
> =
> \frac{1}{
> \underbrace{1-f_m^{POLY}-f_m^{NP}}_{\text{남은 residual clearance fraction}}
> }
> $$
>
> - **항별 의미:** 좌변은 polymorphic pathway 결손과 nonpolymorphic pathway 억제가 겹칠 때 가능한 최대 노출비를 정의합니다. $f_m^{POLY}$와 $f_m^{NP}$는 각각 다형성 경로와 비다형성 경로의 제거 기여율입니다.
> - **실무 직관:** 두 $f_m$의 합이 1에 가까워질수록 분모가 작아져 exposure ratio가 급격히 증가합니다.
> - **오해 방지:** PM 위험은 단순히 genotype 하나의 문제가 아니라 남은 pathway가 inhibitor에 의해 얼마나 더 막히는지의 문제입니다.
...
> **핵심 메시지:** DDI 위험은 시작과 중단이 대칭이 아니며 induction과 MBI에서는 회복 지연이 판단의 핵심입니다.
...
> **핵심 메시지:** C5 figure는 inhibitor strength보다 fm이 exposure ratio의 증폭 가능성을 결정한다는 점을 체감하게 해야 합니다.
...
> **핵심 메시지:** 복합 DDI는 AUC 하나가 아니라 Cmax, t½, V/F, route, timing의 조합으로 기전을 읽어야 합니다.
...
> **핵심 메시지:** digoxin·statin·rifampin 사례는 한 perpetrator가 CL, Vd, F, transporter를 동시에 바꿀 수 있음을 보여 줍니다.
...
> **핵심 메시지:** 같은 약물도 acute/chronic과 oral/IV 조건에 따라 interaction 방향이 바뀔 수 있습니다.
...
> **핵심 메시지:** PD interaction은 농도 변화 없이도 EC50 이동, Emax 감소, receptor ceiling 공유로 반응을 바꿉니다.
...
> =
> \frac{\overbrace{E_{max}}^{\text{최대효과 보존}}\cdot\underbrace{C}_{\text{agonist 농도}}}{
> \underbrace{C}_{\text{agonist 농도}}+
> \underbrace{EC_{50}}_{\text{기본 potency}}
> \cdot
> \overbrace{(1+A/EA_{50})}^{\text{competitive antagonist shift}}
> }
> $$
>
> - **항별 의미:** 좌변은 효과를 예측합니다. $E_{max}$는 보존되는 최대효과, $C$는 agonist 농도, $EC_{50}$는 potency, $(1+A/EA_{50})$는 antagonist가 EC50을 오른쪽으로 이동시키는 factor입니다.
> - **실무 직관:** $A$가 증가하면 denominator가 커져 같은 $C$에서 효과가 감소하지만, 충분히 큰 $C$를 쓰면 $E_{max}$는 회복될 수 있습니다.
> - **오해 방지:** 경쟁적 길항은 Emax를 낮추는 것이 아니라 potency를 낮추는 shift입니다.
...
> \underbrace{E}_{\text{효과}}
> =
> \frac{\overbrace{E_{max}}^{\text{원래 최대효과}}}{\underbrace{(1+A/EA_{50})}_{\text{Emax 감소 factor}}}
> \cdot
> \frac{\underbrace{C}_{\text{agonist 농도}}}{\underbrace{C+EC_{50}}_{\text{agonist potency 구조}}}
> $$
>
> - **항별 의미:** 좌변은 효과를 예측합니다. 첫 factor는 antagonist로 감소한 effective $E_{max}$를, 두 번째 factor는 agonist 농도-효과 포화 구조를 나타냅니다.
> - **실무 직관:** $A$가 증가하면 최대 도달 가능한 효과 자체가 낮아집니다. agonist $C$를 높여도 원래 $E_{max}$를 완전히 회복하지 못합니다.
> - **오해 방지:** 비경쟁적 길항을 competitive right shift처럼 해석하면 high-dose rescue 가능성을 과대평가합니다.
...
>
> $$
> \underbrace{E}_{\text{공유 receptor 효과}}
> =
> \frac{
> \overbrace{E_{max}}^{\text{공유 최대효과}}\cdot
> \left(\underbrace{C_A/EC_{50,A}}_{\text{A 약물 normalized drive}}+
> \underbrace{C_B/EC_{50,B}}_{\text{B 약물 normalized drive}}\right)
> }{
...
> $$
>
> - **항별 의미:** 좌변은 두 full agonist가 같은 receptor를 공유할 때의 총 효과를 예측합니다. 각 $C/EC_{50}$ 항은 potency로 보정한 receptor drive이고, $E_{max}$는 공유 ceiling입니다.
> - **실무 직관:** 두 약물 drive가 증가하면 효과는 증가하지만 하나의 $E_{max}$ ceiling에 접근합니다. 더하기가 효과를 두 배로 만들지는 않습니다.
> - **오해 방지:** 같은 receptor의 full agonist 조합을 독립 효과의 단순 합으로 계산하면 ceiling을 놓칩니다.
...
> **핵심 메시지:** C6 figure는 AUC와 Cmax가 증가하면서 t½가 감소하는 transporter signature를 단순 대사억제와 분리하게 해야 합니다.
...
> **핵심 메시지:** 혼동쌍은 관측값 하나로 mechanism 하나를 단정하는 습관을 깨기 위한 안전장치입니다.
...
> **핵심 메시지:** apparent CL 변화가 농도 때문인지 실제 시간 의존적 생리 변화 때문인지 같은 농도 조건에서 분리해야 합니다.
...
> **핵심 메시지:** 일반 축적은 수위 상승이고 autoinduction은 배수구 자체가 넓어지는 구조 변화입니다.
...
> **핵심 메시지:** AUC/Dose 증가는 enzyme saturation만이 아니라 F 증가나 flow/first-pass 변화에서도 생길 수 있습니다.
...
> **핵심 메시지:** in vitro displacement 직관은 input과 clearance가 계속 작동하는 in vivo 정상상태 판단으로 바로 옮길 수 없습니다.
...
> **핵심 메시지:** 자리 뺏기는 주로 total 농도를 흔들지만 공장 멈추기는 CLint를 줄여 AUC를 지속적으로 키웁니다.
...
> **핵심 메시지:** reversible inhibition은 inhibitor가 사라지면 풀리지만 MBI는 새 효소가 만들어질 때까지 회복되지 않습니다.
...
> **핵심 메시지:** rifampin은 timing과 route에 따라 억제자와 유도자로 다르게 보일 수 있으므로 3차원 매트릭스가 필요합니다.
...
> **핵심 메시지:** 농도-시간이 그대로인데 반응만 변하면 PK DDI가 아니라 PD interaction을 우선 의심해야 합니다.
...
> **핵심 메시지:** 자기 테스트는 식 암기가 아니라 기전 triage를 실제 판단 순서로 회상하게 해야 합니다.

# §7. 자기 테스트: 능동 회상 모듈(Self-Test: Active Recall Module)

<!-- SLIDE_START: S59 | title: Q1 회상 | source_anchor: q1-recall -->
> **핵심 메시지:** 비선형성 진단의 첫 그림은 어떤 파라미터가 상수가 아니게 되었는지 묻기 위한 필터입니다.
...
> **핵심 메시지:** AUC/Dose 증가를 capacity로 단정하지 말고 CL 감소와 F 증가를 모두 후보로 두어야 합니다.
...
> **핵심 메시지:** MM ODE와 linear superposition 식을 혼동하면 출처와 기전이 동시에 어긋납니다.
...
> **핵심 메시지:** phenytoin에서는 R0가 Vm에 가까워질수록 denominator가 작아져 농도가 폭발적으로 증가합니다.
...
> **핵심 메시지:** ascorbic acid는 saturable reabsorption의 escape valve 때문에 high-dose에서 노출 효율이 오히려 떨어집니다.
...
> **핵심 메시지:** Vmax와 Km은 curvature range가 없으면 따로 식별되지 않고 ridge로 함께 움직입니다.
...
> **핵심 메시지:** trough가 체계적으로 이동하면 단순 축적보다 enzyme turnover-linked CL 변화를 먼저 의심해야 합니다.
...
> **핵심 메시지:** MBI는 같은 inhibitor exposure에서도 enzyme recovery clock 때문에 reversible inhibition보다 훨씬 큰 효과를 냅니다.
...
> **핵심 메시지:** total phenytoin 변화만으로 dose를 줄이면 displacement-only 상황에서 unbound 판단을 놓칠 수 있습니다.
...
> **핵심 메시지:** fm이 클수록 우회 경로가 작아져 같은 inhibitor라도 AUC ratio가 크게 증폭됩니다.
...
> **핵심 메시지:** oral interaction이 IV보다 크면 gut wall과 first-pass component를 반드시 분리해야 합니다.
...
> **핵심 메시지:** PM에서 남은 nonpolymorphic pathway까지 막히면 maximum exposure ratio의 denominator가 발산 지점에 접근합니다.
...
>
> $$
> \underbrace{\text{Maximum exposure ratio}}_{\text{PM+inhibitor 최대 위험}}
> =
> \frac{1}{\underbrace{1-f_m^{POLY}-f_m^{NP}}_{\text{남은 제거 여유}}}
> $$
>
> - **항별 의미:** 좌변은 PM 상태와 inhibitor가 겹칠 때의 최대 exposure ratio입니다. 분모는 전체 제거 중 아직 남아 있는 residual fraction입니다.
> - **실무 직관:** $f_m^{POLY}+f_m^{NP}$가 커질수록 남은 제거 여유가 줄어 ratio가 커집니다.
> - **오해 방지:** PM 자체보다 더 위험한 경우는 PM에서 남은 경로까지 병용약으로 막히는 상황입니다.

<!-- SLIDE_START: S71 | title: Q13 적용 | source_anchor: q13-application -->
> **핵심 메시지:** acute transporter inhibition과 chronic enzyme induction은 같은 rifampin에서도 반대 방향 신호를 만들 수 있습니다.
...
> **핵심 메시지:** unbound concentration이 그대로인데 response가 변하면 PD interaction의 EC50 shift와 Emax reduction을 구분해야 합니다.
...
>
> $$
> \overbrace{E=\frac{E_{max}\cdot C}{C+EC_{50}\cdot(1+A/EA_{50})}}^{\text{competitive: EC50 shift, Emax 보존}}
> \qquad\text{vs}\qquad
> \overbrace{E=\frac{E_{max}}{1+A/EA_{50}}\cdot\frac{C}{C+EC_{50}}}^{\text{noncompetitive: Emax 감소}}
> $$
>
> - **항별 의미:** 두 식의 좌변은 모두 response를 예측합니다. 첫 식은 antagonist가 potency 항을 이동시키고, 두 번째 식은 최대효과 자체를 낮춥니다.
> - **실무 직관:** competitive이면 agonist 농도를 높여 효과를 회복할 수 있지만, noncompetitive이면 회복 가능한 ceiling이 낮아집니다.
> - **오해 방지:** concentration-time profile이 변하지 않아도 response가 변할 수 있으므로 PK DDI와 PD DDI를 분리해야 합니다.

<!-- SLIDE_START: S73 | title: Q15 회상 | source_anchor: q15-recall -->
> **핵심 메시지:** parent–metabolite 동시 관측만으로 metabolite disposition을 외삽하면 formation limitation을 disposition으로 오해할 수 있습니다.
...
> **핵심 메시지:** inhibitor strength 분류는 substrate AUC 변화 기준이며 MBI는 reversible 식만으로 과소평가될 수 있습니다.
...
> **핵심 메시지:** 일괄 dose reduction 전에 fm, inhibitor mechanism, route, PM, timing, transporter signature를 순서대로 점검해야 합니다.
...
> **핵심 메시지:** 마지막 조망은 모든 비선형 PK와 DDI 판단을 mass action denominator와 mechanism triage로 회수합니다.

# §8. 메타프레임 및 전체 조망(Meta-Frame & Big Picture)

<!-- SLIDE_START: S77 | title: 한 페이지 기억 모델 | source_anchor: section-8-1-one-page-memory -->
> **핵심 메시지:** 일곱 문장 기억 모델은 세부식을 잊어도 진단 순서를 유지하게 해 주는 압축 지도입니다.
...
> \qquad;
> \qquad
> \underbrace{R_0\to V_m}_{\text{투여속도가 capacity에 접근}}
> \Rightarrow
> \underbrace{C_{ss}\uparrow\uparrow}_{\text{정상상태 발산}}
> $$
>
> - **항별 의미:** $CL(C)$는 clearance가 농도 함수가 되었음을, $R_0\to V_m$는 투여속도가 최대 제거 capacity에 접근함을 나타냅니다.
> - **실무 직관:** capacity 여유가 줄면 정상상태 농도가 비선형적으로 증가하므로 dose increment는 작아야 하고 모니터링 간격은 길어져야 합니다.
> - **오해 방지:** steady state가 존재하더라도 capacity 근처에서는 도달 시간과 농도 민감도가 모두 커집니다.
...
>
> $$
> \underbrace{C_u}_{\text{비결합 노출}}
> =
> \frac{\underbrace{K^0}_{\text{input}}}{\underbrace{CL_u}_{\text{unbound clearance}}}
> $$
>
> - **항별 의미:** 좌변은 open system의 unbound concentration이고, 우변은 input과 unbound clearance의 비율입니다.
> - **실무 직관:** total concentration이 바뀌어도 $K^0$와 $CL_u$가 바뀌지 않으면 steady-state $C_u$는 크게 바뀌지 않을 수 있습니다.
> - **오해 방지:** total-only monitoring은 displacement/binding 상황에서 약효·독성 판단을 잘못 이끌 수 있습니다.
5. **DDI 예측은 $f_m$, 억제제 강도(inhibitor strength, $C_{u,I}/K_I$, $k_{inact}/k_E$), 투여 경로, PM/잔여 경로, 유도/MBI 회복을 함께 봅니다.** [T17 pp.546–561]

> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{f_m}_{\text{pathway 의존도}}
> \quad;
> \quad
> \underbrace{C_{u,I}/K_I}_{\text{가역 억제 강도}}
> \quad;
> \quad
> \underbrace{k_{inact}/k_E}_{\text{MBI 강도 대 회복}}
> $$
>
> - **항별 의미:** 이 세 항은 각각 victim drug의 취약성, reversible inhibitor의 상대 강도, MBI의 불활성화 강도와 enzyme recovery의 균형을 나타냅니다.
> - **실무 직관:** $f_m$이 크고 $C_{u,I}/K_I$ 또는 $k_{inact}/k_E$가 크면 exposure risk가 커집니다. MBI에서는 회복 시간이 별도 판단축입니다.
> - **오해 방지:** DDI 예측은 inhibitor strength 단독이 아니라 victim 의존도와 회복 시계까지 포함해야 합니다.
...
> **핵심 메시지:** 비선형 기전 구분은 PopPK, TDM, DDI, metabolite, PD 모델링의 후속 판단 품질을 좌우합니다.
