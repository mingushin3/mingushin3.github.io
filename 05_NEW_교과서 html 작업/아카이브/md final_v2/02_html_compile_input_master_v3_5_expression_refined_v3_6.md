...
<!-- SLIDE_START: S01 | title: 학습자 네비게이션 안내 | source_anchor: PART A / 학습자 네비게이션 안내 -->
> **핵심 메시지:** 이 자료의 핵심 출발점은 농도 종(species), 결합 구획, 그리고 그림 포인터의 역할을 먼저 고정하는 것입니다.
...
3. FIGURE_POINTER 블록은 교과서 그림을 직접 찾아보라는 안내 블록입니다. 저작권이 있는 교과서 그림을 직접 재현하지 않습니다.
4. §5와 §7로 마무리하시면 됩니다. 이 두 섹션은 자주 발생하는 실수 패턴을 미리 노출하도록 설계되어 있습니다.
...
<!-- SLIDE_START: S02 | title: 학습 경로 | source_anchor: 학습자 네비게이션 안내 / 학습 경로 -->
> **핵심 메시지:** M1에서 M6로 이어지는 순서는 막투과에서 노출 지표 선택까지 분포 판단을 단계적으로 좁히는 경로입니다.

### 학습 경로

M1 막투과 결정인자 → M2 분포 율속 단계 → M3 겉보기 분포용적 구조 → M4 혈장단백결합과 농도의존성 $f_u$ → M5 조직결합/$K_p$/수송체 → M6 총(total) vs 유리(unbound) 노출 지표 → §5 치명적 타격(Critical Blow) → §7 자기 테스트 → §8 A–C 전문가 해석 지점 → §8 D 시스템 통합 시나리오 → §8 E 진단 시그니처 → §8 F 체화 시퀀스.

<!-- SLIDE_START: S03 | title: 학습 전후 체크리스트 | source_anchor: 학습자 네비게이션 안내 / 체크리스트 -->
> **핵심 메시지:** 학습 전후 체크리스트는 총 농도와 유리 농도, 겉보기 용적, figure pointer의 의미를 오해 없이 고정하기 위한 안전장치입니다.
...
- FIGURE_POINTER 블록은 삽입 이미지가 아니라 교과서를 직접 찾아보라는 안내임을 인지하고 있습니다.
...
<!-- SLIDE_START: S04 | title: §1 세션 헤더와 로드맵 | source_anchor: §1 -->
> **핵심 메시지:** 분포용적은 실제 장기 부피가 아니라 체내 총량을 관찰 농도로 환산하는 겉보기 비례상수입니다.
...
<!-- MASTER LENS -->
분포용적은 실제 장기 부피가 아닙니다. **체내 총량(Amount in body)을 농도로 나누기 위해 도입한 겉보기 비례상수(apparent proportionality constant)<!-- ANNOTATION --> (← 실제 부피가 아닌 계산상 비례상수)**입니다. 따라서 이 상수는 대체로 혈장 결합(plasma binding, $f_u$)이 약물을 혈장 쪽에 붙잡는 힘과 조직 결합(tissue binding, $f_{uT}/K_p$)이 조직 쪽으로 끌어당기는 힘의 균형으로 결정됩니다. [R&T pp.102–111]
...
<!-- SLIDE_START: S05 | title: 개념 항법도 | source_anchor: §1 / 개념 항법도 -->
> **핵심 메시지:** 이번 세션은 막투과, 율속 단계, 결합, 수송체, 노출 지표를 하나의 판단 사슬로 연결합니다.
...
<!-- SLIDE_START: S06 | title: 지식 그래프 위치 | source_anchor: §1 / 지식 그래프 위치 -->
> **핵심 메시지:** 이 장의 $V$ 해석은 이전 농도-시간식과 다음 clearance·PopPK 공변량 해석을 연결하는 허브입니다.
...
>
> $$
> \underbrace{C(t)}_{\text{시간 }t\text{의 농도}}=\left(\frac{\underbrace{Dose}_{\text{투여량}}}{\underbrace{V}_{\text{겉보기 분포용적}}}\right)\cdot\underbrace{e^{-kt}}_{\text{일차 소실 감소항}}
> $$
>
> - **항별 의미:** 좌변은 시간에 따른 농도입니다. 우변은 초기 농도에 해당하는 $Dose/V$와 일차 소실을 나타내는 $e^{-kt}$의 곱입니다.
> - **실무 직관:** $Dose$가 커지면 초기 농도는 커지고, $V$가 커지면 같은 용량에서 초기 농도는 낮아지며, $k$가 커지면 더 빨리 감소합니다. 이 식은 이전 세션의 농도-시간식에서 왜 이번 세션의 $V$ 해부가 필요한지 연결합니다.
> - **오해 방지:** $V$를 실제 부피로 읽으면 안 됩니다. 여기서 $V$는 용량을 농도로 바꾸는 겉보기 스케일 파라미터입니다.
- 다음 소실(elimination) 세션에서는 $f_u$가 well-stirred 간 청소율 식의 분자로 들어갑니다.
- PopPK에서는 albumin, α1-AGP, 신·간 질환 공변량(covariate)이 $V$, $CL$, 또는 $f_u$에 어떤 방향으로 연결되는지 판단하는 기반이 됩니다.
...
<!-- SLIDE_START: S07 | title: M1 막투과의 결정인자 | source_anchor: §2 / M1 -->
> **핵심 메시지:** 막투과 판단은 지용성 하나가 아니라 크기, 지용성, 전하, 수송체가 함께 만드는 유효 투과도 판단입니다.

### M1 · 막투과의 결정인자

<!-- MASTER LENS -->
**핵심 직관**: 막투과는 "지용성(lipophilic)이면 통과한다"로 끝나지 않습니다. 실제 투과도(permeability)는 **크기(size) × 지용성(lipophilicity) × 전하(charge) × 수송체(transporter)**가 함께 만듭니다. Eq.4-1의 $P$는 이 네 축과 막 구조가 하나의 숫자로 압축된 거시 투과도(macroscale permeability)입니다. [R&T pp.80–84, 85–90]

#### A. 형식적 정의(Formal Definition)

막 양쪽 수성구획의 유리 농도(unbound concentration)<!-- ANNOTATION --> (← 단백질에 결합하지 않은 농도)가 각각 $C_{u,1}$, $C_{u,2}$일 때 순수송속도(net transport rate)는 다음과 같습니다.

$$\text{Net rate of transport}=P\cdot SA\cdot(C_{u,1}-C_{u,2})$$


> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{\text{Net rate of transport}}_{\text{막 순수송속도}}=\underbrace{P}_{\text{투과도}}\cdot\underbrace{SA}_{\text{표면적}}\cdot\underbrace{(C_{u,1}-C_{u,2})}_{\text{유리농도 기울기}}
> $$
>
> - **항별 의미:** 좌변은 막을 가로질러 순방향으로 이동하는 약물의 순수송속도를 정의합니다. 우변에서 $P$는 막 자체의 투과성, $SA$는 교환 가능한 표면적, $(C_{u,1}-C_{u,2})$는 결합되지 않은 약물이 만드는 실제 확산 구동력입니다.
> - **실무 직관:** $P$, $SA$, 또는 유리 농도차가 커질수록 순수송속도는 증가합니다. 이 식은 흡수·BBB 투과·조직분포 문제에서 총 농도가 아니라 유리 농도 기울기를 먼저 보아야 한다는 판단을 가능하게 합니다.
> - **오해 방지:** 가장 흔한 오해는 총 농도 차이 $C_1-C_2$가 막투과를 직접 민다는 생각입니다. 결합 약물은 일반적으로 막을 통과하지 못하므로, 이 식의 핵심은 $C_u$입니다.

[출처: R&T Eq.4-1, p.80]

결합 약물(bound drug)은 일반적으로 막을 통과하지 못합니다. 따라서 막투과의 구동력(driving force)은 총 농도(total concentration) $C$가 아니라 유리 농도 $C_u$입니다. [R&T p.80]

#### B. 출처 기반 상세(Source-Anchored Details)

- **크기(Size)**: 같은 지용성에서 MW 400 → 800 g/mol로 증가하면 피부 투과도(skin permeability)는 약 300배 낮아집니다. [R&T Fig.4-3, p.81–82] Atenolol 246 g/mol은 위장관 흡수가 가능하지만, oxytocin 1007 g/mol은 GI membrane을 거의 통과하지 못하고, calcitonin-salmon 3432 g/mol은 비강 흡수(nasal absorption)도 약 3% 수준입니다. [R&T p.81]
- **지용성(Lipophilicity)**: BBB 투과도는 대체로 n-octanol/water 분배계수가 증가할수록 증가합니다. 단, vinblastine·vincristine은 지용성만으로 기대되는 값보다 낮은 투과도를 보이는데, 원문은 P-glycoprotein efflux 기질임을 주요 이유로 듭니다. [R&T Fig.4-4, p.82] "100배" 같은 배수는 시각 추정 [확인 필요]로만 취급합니다.
- **전하 / pH 분배(Charge / pH partition)**: 약산·약염기에서는 비이온화형(un-ionized form)이 지질막(lipoidal membrane)을 주로 통과합니다. 평형에서 비이온화 농도는 같아질 수 있지만, pH와 pKa에 따라 총 농도는 달라집니다. [R&T p.83; App.B pp.763–765]
- **수송체(Transporter)**: 수동 촉진 수송체(passive facilitated transporter)는 평형 도달 속도를 바꾸고, 능동 수송체(active transporter)는 정상상태 유리 농도비(steady-state unbound ratio) 자체를 바꿀 수 있습니다. <!-- SLIDE_START: S08 | title: M1 수송체와 유리농도비 | source_anchor: §2 / M1 / transporter -->
> **핵심 메시지:** 능동 수송체가 관여하면 조직 유리 농도비는 단순 수동 평형이 아니라 유입과 유출 투과도의 비로 결정됩니다.
...
>
> $$
> \underbrace{\frac{C_{u,T}}{C_u}}_{\text{조직/혈장 유리농도비}}=\underbrace{\frac{P_{uptake}}{P_{efflux}}}_{\text{유입/유출 수송 균형}}
> $$
>
> - **항별 의미:** 좌변은 조직 유리 농도가 혈장 유리 농도에 비해 어느 정도로 유지되는지를 정의합니다. 우변의 $P_{uptake}$는 조직 안으로 밀어 넣는 수송 능력, $P_{efflux}$는 조직 밖으로 내보내는 수송 능력입니다.
> - **실무 직관:** $P_{uptake}$가 커지면 조직 유리 농도비가 증가하고, $P_{efflux}$가 커지면 감소합니다. 이 식은 수동 평형만으로 설명되지 않는 조직 축적 또는 BBB 배제 현상을 판별하게 합니다.
> - **오해 방지:** 이 비율을 단순한 결합 차이로만 해석하면 안 됩니다. 수송체가 개입하면 조직 유리 농도와 혈장 유리 농도는 1:1 평형을 이루지 않을 수 있습니다.

[출처: R&T Eq.4-26, p.110]

<!-- SLIDE_START: S09 | title: M1 실무 함정과 figure pointer | source_anchor: §2 / M1 / practice -->
> **핵심 메시지:** $logP$가 좋아 보여도 efflux와 이온화 예외를 점검하지 않으면 BBB·장벽 투과 판단이 쉽게 빗나갑니다.

<!-- TRENCH -->
**현장 실무 팁(Trench-Level Tip)**: $logP$와 MW가 좋아도 P-gp/BCRP efflux 기질이면 BBB 또는 장벽에서 유효 투과도(effective permeability)가 크게 낮아질 수 있습니다. 단, Caco-2/PAMPA, Lipinski rule은 이 source 범위 밖이므로 본 핸드아웃에서는 삭제합니다.

> **실패 모드(Failure Mode) — AUDIT_DERIVED**  
> $logP$ 하나로 BBB 또는 조직 투과도를 예측하려고 하면, 크기·이온화·유출 수송체가 만드는 예외를 놓칩니다. 특히 이 세션에서는 "지용성(lipophilicity) = 투과도(permeability)"가 아니라 "유효 투과도 = 물리화학 + 막 구조 + 수송체"로 읽어야 합니다.

<!-- ANCHOR -->
막을 통과하는 성질이 정해지면, 다음 질문은 "조직으로 가는 속도가 혈류(blood flow)에 의해 제한되는가, 막 자체에 의해 제한되는가?"입니다.

<!-- FIGURE_POINTER -->
Source: R&T 5e, p.82, Fig.4-4
Why this matters: BBB 투과도(permeability)가 $logP$만으로 결정되지 않고 크기(size)/유출(efflux) 예외를 갖는다는 점을 실제 좌표로 보여줍니다.
When to look: 이 카드를 읽은 뒤 확인하면 됩니다.
Learner instruction: caffeine, lomustine, vinblastine/vincristine의 위치를 비교해 보세요.
<!-- /FIGURE_POINTER -->

> **▶ 실무 체화 요약(Practice Brief) — M1 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가:** 약물이 막을 통과할 수 있는지가 흡수, 분포, 표적 도달 여부를 결정하기 때문에, 이후 모든 PK 해석의 기초 입력값이 됩니다.
> - **흐름상 역할:** M2의 관류 vs 투과 율속 판단으로 가는 진입점이며, M5의 수송체 매개 분포 해석에 필요한 4축(크기·$logP$·이온화·수송체)을 정의합니다.
> - **체화 꿀팁:** 새 약물을 마주할 때마다 "크기(size) · $logP$ · 이온화(ionization) · 수송체(transporter)" 네 축을 한 줄로 적어보고, 어느 축이 가장 약한 결정인자인지 짚는 습관을 들이면 됩니다.
> - **실무 활용:** BBB 침투 가능성 1차 추정, P-gp/BCRP 기질 여부 평가, 경구 생체이용률(oral bioavailability) 추정의 출발점, NONMEM dataset에서 흡수상 모델 선택의 근거.

---

<!-- SLIDE_START: S10 | title: M2 분포의 율속 단계 | source_anchor: §2 / M2 -->
> **핵심 메시지:** 분포 해석의 첫 분기점은 얼마나 많이 가는가와 얼마나 빨리 가는가를 분리하는 것입니다.

### M2 · 분포의 율속 단계

<!-- MASTER LENS -->
**핵심 직관**: 조직 분포에서는 "얼마나 많이 가는가"와 "얼마나 빨리 가는가"를 분리해야 합니다. 조직 친화도(tissue affinity)가 커지면 평형 범위(equilibrium extent)는 커집니다. 그러나 채워야 할 조직 내 약물량도 커지므로 평형 도달 시간(equilibrium time)은 오히려 길어질 수 있습니다. [R&T pp.94–100]

#### A. 형식적 정의(Formal Definition)

관류 율속 분포(perfusion-rate limited distribution)<!-- ANNOTATION --> (← 혈류 공급이 속도 제한)에서는 막 통과가 충분히 빠릅니다. 이때 조직으로 들어가는 공급 속도는 혈류가 결정하며, 조직 농도가 평형값에 접근하는 경로는 단일지수(mono-exponential) 형태를 따릅니다.
...
>
> $$
> \underbrace{k_T}_{\text{조직 접근 속도상수}}=\frac{\overbrace{Q/V_T}^{\text{조직 단위부피당 혈류}}}{\underbrace{K_{pb}}_{\text{조직/혈액 분포비}}}
> $$
>
> - **항별 의미:** 좌변은 조직 농도가 평형에 접근하는 속도상수입니다. 우변에서 $Q/V_T$는 조직 크기 대비 혈류 공급 속도이고, $K_{pb}$는 혈액 대비 조직에 얼마나 많이 머무는지를 나타냅니다.
> - **실무 직관:** $Q/V_T$가 커지면 평형 접근이 빨라지고, $K_{pb}$가 커지면 채워야 할 조직 저장고가 커져 접근이 느려집니다. 이 식은 onset과 채혈 시점이 평형 분포량만으로 결정되지 않음을 보여줍니다.
> - **오해 방지:** 초보자는 큰 $K_{pb}$를 항상 빠른 조직 도달로 오해합니다. 실제로는 많이 가는 약물일수록 같은 혈류에서 평형 도달이 늦어질 수 있습니다.

<!-- SLIDE_START: S11 | title: M2 분포 반감기 | source_anchor: §2 / M2 / distribution half-life -->
> **핵심 메시지:** 큰 조직 친화도는 더 많이 분포한다는 뜻인 동시에 평형 도달을 늦출 수 있는 시간 지연 신호입니다.
...
>
> $$
> \underbrace{t_{1/2,distribution}}_{\text{분포 반감기}}=\frac{\underbrace{0.693}_{\ln 2}\cdot\underbrace{K_{pb}}_{\text{조직 저장성}}}{\underbrace{Q/V_T}_{\text{조직 관류속도}}}
> $$
>
> - **항별 의미:** 좌변은 조직 분포가 평형까지 절반 접근하는 데 걸리는 시간입니다. 우변은 단일지수 접근에서 $\ln2$와 조직 저장성 $K_{pb}$를 곱하고, 단위 조직 관류속도 $Q/V_T$로 나눈 구조입니다.
> - **실무 직관:** $K_{pb}$가 커지면 분포 반감기는 길어지고, $Q/V_T$가 커지면 짧아집니다. 이 식은 지방·근육처럼 낮은 관류와 높은 친화도가 만나는 조직에서 왜 늦은 분포가 발생하는지 판단하게 합니다.
> - **오해 방지:** 분포 반감기를 제거 반감기와 혼동하지 마세요. 여기서 시간 지연은 소실 능력이 아니라 조직을 채우는 속도 문제입니다.

<!-- SLIDE_START: S12 | title: M2 조직 농도 접근 곡선 | source_anchor: §2 / M2 / tissue concentration -->
> **핵심 메시지:** 조직 농도 시간 경과는 최종 평형값과 그 평형에 접근하는 속도를 분리해서 읽어야 합니다.
...
>
> $$
> \underbrace{C_T(t)}_{\text{시간 }t\text{의 조직농도}}=\underbrace{K_{pb}\cdot C_A}_{\text{평형 조직농도}}\cdot\underbrace{(1-e^{-k_Tt})}_{\text{평형 접근분율}}
> $$
>
> - **항별 의미:** 좌변은 시간 $t$에서의 조직 농도를 예측합니다. 우변의 $K_{pb}\cdot C_A$는 최종 평형 조직 농도이고, $(1-e^{-k_Tt})$는 그 평형값에 어느 정도 접근했는지를 나타냅니다.
> - **실무 직관:** $K_{pb}$ 또는 $C_A$가 커지면 최종 조직 농도는 커지고, $k_T$가 커지면 같은 시간에 더 빨리 평형에 접근합니다. 이 식은 조직 농도 해석에서 '최종 범위'와 '도달 속도'를 분리하게 합니다.
> - **오해 방지:** 초기 조직 농도가 낮다고 조직 친화도가 낮다고 결론 내리면 안 됩니다. $k_T$가 작으면 높은 평형 농도를 향해 매우 천천히 접근할 수 있습니다.

[출처: R&T Eq.4-5~4-7, p.97]

<!-- SLIDE_START: S13 | title: M2 출처 기반 상세와 경계 조건 | source_anchor: §2 / M2 / details -->
> **핵심 메시지:** 관류와 친화도 사례를 읽을 때는 onset, 잔류, 채혈 설계가 서로 다른 질문임을 유지해야 합니다.
...
- Eq.4-5~4-7은 **관류 율속(perfusion-rate limited)** 가정에서만 안전합니다.
- 투과 율속(permeability-rate limited) 조직에서는 $Q$를 올려도 분포 속도가 비례해서 증가하지 않습니다.
- 높은 $K_{pb}$는 "많이 간다"와 동시에 "평형 도달이 늦다"를 의미할 수 있습니다.

> **실무 렌즈(Practice Lens) — CRUCIBLE_DERIVED**  
> 분포 문제를 볼 때는 먼저 범위(extent)와 속도(rate)를 분리합니다. $V$ 또는 $K_p$는 "얼마나 많이"를 말하고, $Q/V_T$와 투과도는 "얼마나 빨리"를 말합니다. 따라서 작용 발현(onset) 또는 채혈 시점(sampling-time) 문제를 평형 파라미터만으로 설명하지 않습니다.

<!-- RECAP -->
**M2 요약(recap)**: 분포 반감기(distribution half-life)는 혈류, 조직 크기, 조직 친화도의 함수입니다. 큰 $K_{pb}$와 낮은 $Q/V_T$가 만나면 분포는 느려집니다.

<!-- FIGURE_POINTER -->
Source: R&T 5e, p.94, Fig.4-13
Why this matters: thiopental에서 뇌(brain)와 지방(fat)의 시간 경과(time-course) 차이가 관류(perfusion)와 친화도(affinity)의 결합 효과임을 보여줍니다.
When to look: 이 카드를 읽은 뒤 확인하면 됩니다.
Learner instruction: 뇌(brain)의 최고점(peak)과 지방(fat)의 잔류를 비교해 보세요.
<!-- /FIGURE_POINTER -->

> **▶ 실무 체화 요약(Practice Brief) — M2 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가:** 동일한 $V_d$라도 평형 도달 속도가 다르면 작용 발현(onset), 최고 농도 시점(peak time), 채혈 설계(sampling design)가 완전히 달라집니다. 그래서 이 개념은 모델 구조 결정의 분기점이 됩니다.
> - **흐름상 역할:** M1의 막투과 입력을 받아 M3의 $V$ 해석 시점(분포 평형 후)을 정의하며, 다구획 모델 vs 단일구획 모델 결정의 근거가 됩니다.
> - **체화 꿀팁:** 새 약물의 첫 농도-시간 곡선에서 분포상(distribution phase)의 굴곡 시점을 기록한 뒤 $Q/V_T\cdot K_{pb}$로 역산해 보고, 모델 가정과 일치하는지 점검하면 됩니다.
> - **실무 활용:** PBPK 조직 평형 시간 추정, Phase 1 PK 채혈 일정 설계, thiopental·penicillin G 같이 혼합 제한을 보이는 약물의 조직 농도 해석.

---

<!-- SLIDE_START: S14 | title: M3 분포용적의 수학 구조 | source_anchor: §2 / M3 -->
> **핵심 메시지:** $V$의 본질은 생리학적 부피가 아니라 체내 총량과 관찰 농도를 잇는 Amount/C 비례상수입니다.

### M3 · 분포용적의 수학 구조 **[⚡ Apex Concept]**

<!-- MASTER LENS -->
**핵심 직관**: $V$는 실제 부피가 아니라 $\text{Amount}/C$입니다. 작은 $V_d$의 알부민 결합 산성 약물에서는 알부민이 페리(ferry)처럼 약물을 혈장과 혈관 외 알부민 공간으로 운반합니다. $f_u\to0$이면 약물은 그 페리에서 거의 내리지 않습니다. 이 때문에 겉보기 $V$는 알부민 분포 용적인 약 7.5 L 근처에서 하한(floor)을 갖습니다. [R&T pp.111–113; App.C pp.767–773]
...
>
> $$
> \underbrace{V}_{\text{겉보기 분포용적}}=\frac{\overbrace{\text{Amount in body}}^{\text{체내 총량}}}{\underbrace{C}_{\text{측정 농도}}}=\frac{\underbrace{A}_{\text{체내 약물량}}}{\underbrace{C}_{\text{농도}}}
> $$
>
> - **항별 의미:** 좌변은 체내 총량을 관찰 농도로 환산하는 겉보기 비례상수입니다. 우변은 실제 장기 부피가 아니라 체내 약물량 $A$를 농도 $C$로 나눈 계산 구조입니다.
> - **실무 직관:** $A$가 커지면 $V$는 커지고, 같은 $A$에서 $C$가 낮아지면 $V$는 커집니다. 이 식은 부하 용량과 농도 해석에서 $V$를 실제 공간이 아니라 Amount/C 좌표로 읽게 합니다.
> - **오해 방지:** 가장 큰 오해는 $V$를 실제 생리학적 부피로 해석하는 것입니다. $V$는 관찰 농도에 대한 겉보기 상수이므로 실제 체수분보다 훨씬 클 수 있습니다.

[출처: R&T Eq.4-8, p.102]

<!-- SLIDE_START: S15 | title: M3 체내 총량 분해 | source_anchor: §2 / M3 / amount decomposition -->
> **핵심 메시지:** 체내 총량은 혈장 저장고와 조직 저장고의 합으로 분해될 때 비로소 $V$의 원인이 보입니다.
...
>
> $$
> \underbrace{A}_{\text{체내 총량}}=\underbrace{V_p\cdot C}_{\text{혈장 내 양}}+\underbrace{V_T\cdot K_p\cdot C}_{\text{조직 내 양}}
> $$
>
> - **항별 의미:** 좌변은 체내 전체 약물량입니다. 우변은 혈장 부피 $V_p$에 혈장 농도 $C$를 곱한 혈장 내 양과, 조직 부피 $V_T$에 조직/혈장 분포비 $K_p$와 $C$를 곱한 조직 내 양의 합입니다.
> - **실무 직관:** $V_p$, $V_T$, $K_p$, 또는 $C$가 커지면 총량 $A$가 증가합니다. 이 식은 관찰 농도 하나가 혈장 저장고와 조직 저장고를 동시에 반영한다는 모델링 판단을 가능하게 합니다.
> - **오해 방지:** $K_p$를 단순한 속도상수로 오해하면 안 됩니다. 여기서는 평형에서 조직이 혈장 대비 얼마나 약물을 담는지 나타내는 분포비입니다.

<!-- SLIDE_START: S16 | title: M3 단일 조직 분포용적 | source_anchor: §2 / M3 / single tissue V -->
> **핵심 메시지:** $V$ 증가는 혈장 부피 증가가 아니라 조직 항 $V_TK_p$ 증가로 읽어야 하는 경우가 많습니다.
...
>
> $$
> \underbrace{V}_{\text{겉보기 }V}=\underbrace{V_p}_{\text{혈장 항}}+\underbrace{V_T\cdot K_p}_{\text{조직 분포 항}}
> $$
>
> - **항별 의미:** 좌변은 혈장 농도 기준 겉보기 분포용적입니다. 우변은 혈장 자체의 기여와 조직 부피에 조직/혈장 분포비를 곱한 조직 기여로 나뉩니다.
> - **실무 직관:** $K_p$가 커지거나 조직 부피 $V_T$가 커지면 $V$는 커집니다. 이 식은 큰 $V_d$를 볼 때 혈장 밖 조직 저장고가 얼마나 관여하는지 판단하게 합니다.
> - **오해 방지:** $V$가 커졌다고 혈장 부피가 커진 것은 아닙니다. 대개는 조직 항이 커진 것으로 해석해야 합니다.

<!-- SLIDE_START: S17 | title: M3 다조직 분포용적 | source_anchor: §2 / M3 / multi tissue V -->
> **핵심 메시지:** 전체 $V$는 여러 조직 기여의 합이므로 하나의 큰 숫자만으로 특정 조직을 단정하면 안 됩니다.
...
>
> $$
> \underbrace{V}_{\text{전체 겉보기 }V}=\underbrace{V_p}_{\text{혈장 항}}+\sum_i\underbrace{V_{T,i}\cdot K_{p,i}}_{\text{조직 }i\text{의 기여}}
> $$
>
> - **항별 의미:** 좌변은 여러 조직을 모두 포함한 겉보기 분포용적입니다. 우변은 혈장 항과 각 조직 $i$의 부피 $V_{T,i}$ 및 분포비 $K_{p,i}$가 만드는 기여의 합입니다.
> - **실무 직관:** 어떤 조직의 $V_{T,i}$ 또는 $K_{p,i}$가 커지면 전체 $V$가 커집니다. 이 식은 PBPK와 PopPK에서 어느 조직 또는 결합 특성이 큰 $V$를 만드는지 분해하게 합니다.
> - **오해 방지:** 전체 $V$ 하나만 보고 특정 조직을 단정하지 마세요. 같은 $V$도 서로 다른 조직 기여 조합으로 만들어질 수 있습니다.

[출처: R&T Eq.4-11~4-13, pp.103–104]

<!-- SLIDE_START: S18 | title: M3 측정 매질별 겉보기 용적 | source_anchor: §2 / M3 / matrix-specific V -->
> **핵심 메시지:** 분모가 총 농도인지 유리 농도인지 혈액 농도인지에 따라 같은 체내 총량도 서로 다른 $V$로 보입니다.
...
>
> $$
> \underbrace{V_u}_{\text{유리농도 기준 }V}=\frac{\underbrace{A}_{\text{체내 총량}}}{\underbrace{C_u}_{\text{유리 농도}}},\qquad \underbrace{V_b}_{\text{혈액농도 기준 }V}=\frac{\underbrace{A}_{\text{체내 총량}}}{\underbrace{C_b}_{\text{혈액 농도}}}
> $$
>
> - **항별 의미:** 좌변들은 측정 매질 또는 농도 종이 바뀌었을 때의 겉보기 용적입니다. 우변은 같은 체내 총량 $A$를 유리 농도 $C_u$ 또는 혈액 농도 $C_b$로 나누어 정의합니다.
> - **실무 직관:** 같은 $A$에서 $C_u$가 낮으면 $V_u$는 커지고, $C_b$가 낮으면 $V_b$가 커집니다. 이 식은 plasma, blood, unbound scale을 바꾸면 $V$의 숫자와 의미도 바뀐다는 판단을 가능하게 합니다.
> - **오해 방지:** 서로 다른 농도 스케일의 $V$, $V_u$, $V_b$를 같은 생리적 부피처럼 비교하면 안 됩니다. 분모가 무엇인지가 해석을 결정합니다.

[출처: R&T Eq.4-14~4-15, p.104]

<!-- SLIDE_START: S19 | title: M3 결합 기반 구조 | source_anchor: §2 / M3 / binding structure -->
> **핵심 메시지:** $V$는 혈장 유리분율과 조직 유리분율의 비를 통해 단백결합의 방향성을 수학적으로 드러냅니다.
...
>
> $$
> \underbrace{V}_{\text{겉보기 }V}=\underbrace{V_p}_{\text{혈장 항}}+\underbrace{V_{TW}\cdot\frac{\overbrace{f_u}^{\text{혈장 유리분율}}}{\underbrace{f_{uT}}_{\text{조직 유리분율}}}}_{\text{결합 기반 조직 항}}
> $$
>
> - **항별 의미:** 좌변은 혈장 총 농도 기준 겉보기 분포용적입니다. 우변은 혈장 항과, 조직 수분/조직 공간 $V_{TW}$에 혈장 유리분율과 조직 유리분율의 비를 곱한 조직 항입니다.
> - **실무 직관:** $f_u$가 증가하면 조직으로 이동 가능한 비결합 비율이 커져 $V$가 증가할 수 있고, $f_{uT}$가 감소하면 조직 결합이 강해져 $V$가 커집니다. 이 식은 단백결합 변화가 $V$ 방향성을 어떻게 바꾸는지 판단하게 합니다.
> - **오해 방지:** $f_u$ 증가를 항상 유리 농도 증가로 바로 연결하지 마세요. 이 식은 $f_u$ 변화가 총 농도 기준 $V$ 자체를 바꿀 수 있음을 보여줍니다.

[출처: R&T Eq.4-25, p.109]

<!-- ANNOTATION --> 여기서 $K_p$를 $f_u/f_{uT}$로 바꾸어 쓰는 이유는, 같은 총량이라도 혈장에 남는 비율과 조직에 붙는 비율이 서로 다르기 때문입니다. 이 전환이 "$V$가 왜 결합의 함수인가"를 보여줍니다.

<!-- SLIDE_START: S20 | title: M3 수송체가 포함된 분포용적 | source_anchor: §2 / M3 / transporter V -->
> **핵심 메시지:** 수송체 항은 결합 항과 별개로 조직 분포를 바꿀 수 있으므로 같은 $V$ 변화라도 원인을 분리해야 합니다.
...
>
> $$
> \underbrace{V}_{\text{겉보기 }V}=\underbrace{V_p}_{\text{혈장 항}}+\underbrace{V_{TW}\cdot\frac{f_u}{f_{uT}}}_{\text{결합 기반 조직 항}}\cdot\underbrace{\frac{P_{uptake}}{P_{efflux}}}_{\text{수송체 불균형 항}}
> $$
>
> - **항별 의미:** 좌변은 수송체가 관여할 때의 겉보기 분포용적입니다. 우변은 혈장 항, 결합 기반 조직 항, 그리고 유입/유출 수송체의 균형을 나타내는 항의 곱으로 조직 기여를 확장합니다.
> - **실무 직관:** $f_u$, $P_{uptake}$가 커지거나 $f_{uT}$, $P_{efflux}$가 작아지면 $V$는 커질 수 있습니다. 이 식은 큰 조직분포가 결합 때문인지 능동 수송 때문인지 분리해 검토하게 합니다.
> - **오해 방지:** 결합 항과 수송체 항을 같은 원인으로 합치지 마세요. 같은 $V$ 증가라도 확인해야 할 실험과 모델링 의미가 다릅니다.

[출처: R&T Eq.4-29, p.111]

<!-- SLIDE_START: S21 | title: M3 소 Vd 알부민 결합 모델 | source_anchor: §2 / M3 / albumin ferry -->
> **핵심 메시지:** 소 $V_d$ 알부민 결합 약물에서는 $f_u\to0$이어도 알부민 분포 용적이 $V$의 하한을 만듭니다.
...
>
> $$
> \underbrace{V}_{\text{소 }V_d\text{ 약물의 }V}=\underbrace{7.5}_{\text{알부민 분포 하한}}+\underbrace{\left(7.5+\frac{V_R}{f_{uR}}\right)}_{\text{혈관외/잔여 분포 계수}}\cdot\underbrace{f_u}_{\text{혈장 유리분율}}
> $$
>
> - **항별 의미:** 좌변은 소 $V_d$ 알부민 결합 약물의 겉보기 분포용적입니다. 우변의 첫 7.5는 알부민이 만드는 하한이고, 괄호 항은 유리분율이 증가할 때 추가로 접근 가능한 분포 공간의 기울기 역할을 합니다.
> - **실무 직관:** $f_u$가 커지면 두 번째 항이 커져 $V$가 증가하지만, $f_u\to0$이어도 $V$는 0이 아니라 7.5 L 근처 하한을 갖습니다. 이 식은 저알부민혈증에서 총 농도와 $V$가 어떻게 함께 변할 수 있는지 판단하게 합니다.
> - **오해 방지:** 초보자는 $f_u\to0$이면 분포용적도 0이라고 생각하기 쉽습니다. 알부민 결합 약물은 알부민이라는 페리를 타고 혈관 외 알부민 공간까지 분포할 수 있습니다.

[출처: R&T Eq.4-30, p.112; App.C Eq.C-13, pp.767–773]

<!-- SLIDE_START: S22 | title: M3 데이터 앵커 | source_anchor: §2 / M3 / data anchors -->
> **핵심 메시지:** 혈장 3 L, 세포외액 16 L, 총 체수분 42 L, 알부민 7.5 L는 $V$ 숫자를 해석하는 좌표축입니다.

#### C. 출처 기반 데이터 앵커(Source-Anchored Data Anchors)

- 혈장 3 L, 세포외액(extracellular water) 16 L, 총 체수분(total body water) 42 L는 $V$ 해석의 생리학적 기준점입니다. [R&T p.111]
- 알부민 겉보기 분포용적은 약 7.5 L이며, cephalosporin Fig.4-25에서 $f_u\to0$ 외삽 절편이 약 7 L로 보입니다. [R&T Table4-5 p.101; Fig.4-25 pp.112–113]
- 같은 cephalosporin 계열에서 $V$는 $f_u$와 함께 증가하지만, $V_u$는 $f_u$가 증가할수록 감소합니다. 이 쌍을 이루는 메시지(paired message)가 "총 농도 기준 $V$"와 "유리 농도 기준 $V$"가 같은 생리적 의미가 아님을 보여줍니다. [R&T Fig.4-25~4-26, p.113]
- Warfarin 신증후군(nephrotic syndrome) 예시: albumin 43→12.5 g/L, $V$ 9.4→13.7 L, $CL$ 0.20→0.58 L/hr, $t_{1/2}$ 36→18 hr. [R&T Table4-12, p.118]
- Adalimumab 같은 고분자량 항체는 $V$가 약 5–6 L로 작고, caffeine/alcohol처럼 작고 자유롭게 투과하는 분자는 $V$가 총 체수분에 접근합니다. [R&T p.111]

<!-- SLIDE_START: S23 | title: M3 정점 오류 | source_anchor: §2 / M3 / apex fallacy -->
> **핵심 메시지:** $f_u$ 증가를 곧바로 부하 용량 증가로 연결하면 총 농도, 유리 농도, $V$의 방향을 혼동할 수 있습니다.

#### D. 정점 오류(Apex Fallacy)

"$f_u$가 증가하면 유리 농도가 증가하므로 부하 용량(loading dose)을 올려야 한다"는 직관은 소 $V_d$ 알부민 결합 약물에서 틀릴 수 있습니다. 같은 총량에서 $f_u$가 증가하면 총 $C$는 낮아지고 $V$는 증가합니다. 그러나 $C_u$는 반드시 같은 방향으로 증가하지 않습니다. R&T의 cephalosporin/warfarin 논리는 이 함정을 정면으로 다룹니다. [R&T pp.112–118]

#### D-2. 그럴듯한 오해(Plausible Fallacy) — Vd를 실제 부피로 오해

> **⚡ 그럴듯한 오해(Plausible Fallacy) — [EXPERT_INFERENCE, v3]**
> - **그럴듯한 오해**: "$V_d$가 크다는 것은 체내 분포 공간이 실제로 크다는 뜻입니다."
> - **왜 틀렸는가**: $V_d$는 체적이 아니라 혈장 농도 관측값으로 역산한 비례상수입니다. 조직 결합이 강하거나 혈장 결합이 약하면 실제 체적의 수십~수백 배도 가능할 수 있습니다. $f_u$와 $f_{uT}$의 비율이 이 겉보기 크기를 결정합니다.
> - **실무에서 어떻게 드러나는가**: $V_d$가 500 L인 약물의 부하 용량을 "체중×4 L/kg"로 설계하다가 실제 분포 공간 개념으로 혼동하면, 부하 용량 계산이 생리학적 의미를 잃습니다. Bayesian TDM에서 총 농도(total $C$)가 정상 범위인데 유리 농도(unbound $C_u$)가 달라진 경우를 생각해 보시면 됩니다. 실제 원인은 $f_u$ 변화인데도 $V_d$ 변화로 오해하는 전형적 실수가 여기에서 비롯됩니다.

<!-- TRENCH -->
**[교과서 외 실무 해석] 알부민 공변량 비대칭(Albumin covariate asymmetry)**: 대 $V_d$ 약물에서 알부민을 $V$ 공변량으로만 넣고 $CL$ 쪽을 보지 않으면 총 농도 기준 $CL$ 변화가 누락될 수 있습니다. ETA-CL vs albumin 관계가 ETA-V vs albumin보다 강하면 $CL$ 공변량도 검토합니다.

**[교과서 외 실무 경고] Vss vs Vβ**: 다구획 모델(multi-compartment model)에서 부하 용량은 $V_{ss}$ 기반, 종말상 반감기(terminal half-life) 해석은 $V_\beta$ 기반이라는 구분을 유지합니다.

> **체화 노트(Mastery Note) — CRUCIBLE_DERIVED**  
> 알부민 변화가 보이면 바로 "용량을 바꿀까?"로 가지 말고, 총 $C$, $C_u$, $V$, $CL$ 중 어느 관찰값이 움직였는지 먼저 분해하시면 됩니다. 이 순서를 지키면 저알부민혈증(hypoalbuminemia) 상황에서 총 농도 변화와 활성 노출(active exposure) 변화를 혼동하지 않습니다.

<!-- SLIDE_START: S24 | title: M3 요약과 figure pointer | source_anchor: §2 / M3 / recap -->
> **핵심 메시지:** M3의 핵심 결론은 $V$를 조직 항, 결합 항, 측정 스케일로 분해해야 한다는 것입니다.

<!-- RECAP -->
**M3 요약(recap)**: $V$는 $V_p+\text{tissue term}$이며, tissue term은 $K_p$ 또는 $f_u/f_{uT}$로 읽을 수 있습니다. 소 $V_d$ 알부민 결합 약물에서는 알부민 분포 용적이 하한(floor)을 만듭니다.

<!-- FIGURE_POINTER -->
Source: R&T 5e, p.113, Fig.4-25 and Fig.4-26
Why this matters: 같은 cephalosporin 계열(series)에서 $V$와 $V_u$가 $f_u$에 대해 반대 방향으로 읽힐 수 있음을 보여줍니다.
When to look: 이 카드를 읽은 뒤 확인하면 됩니다.
Learner instruction: $V$-$f_u$와 $V_u$-$f_u$ 축을 나란히 비교해 보세요.
<!-- /FIGURE_POINTER -->

> **▶ 실무 체화 요약(Practice Brief) — M3 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가:** $V_d$는 부하 용량 계산과 공변량 해석의 중심 파라미터입니다. 그 본질이 겉보기 비례상수(apparent proportionality constant)임을 놓치면 후속 모든 추론이 흔들립니다.
> - **흐름상 역할:** M4·M5의 결합 정보($f_u$, $f_{uT}$)와 M6의 노출 지표를 $V$라는 단일 수학적 연결 고리(mathematical bridge) 위에 통합하는 세션의 중심 카드입니다.
> - **체화 꿀팁:** 새 약물의 $V$ 값을 보면 즉시 혈장 3 L · 세포외액 16 L · 총 체수분 42 L · 알부민 7.5 L 중 어느 기준점(anchor)에 가까운지 분류하면 됩니다. 이 분류만으로 결합 양상(binding regime)이 절반 이상 보입니다.
> - **실무 활용:** 부하 용량 설계, 저알부민혈증/투석 환자에서 V 변화 해석, NONMEM에서 V 위에 공변량 spec할 때의 방향(positive/negative) 판단, Bayesian TDM에서 V 이상값의 진단.

---

<!-- SLIDE_START: S25 | title: M4 혈장단백결합과 fu | source_anchor: §2 / M4 -->
> **핵심 메시지:** $f_u$는 고정 상수가 아니라 단백 농도, 친화도, 결합 부위, 유리 농도에 의해 달라지는 변환 조건입니다.

### M4 · 혈장단백결합과 fu의 농도의존성

<!-- MASTER LENS -->
**핵심 직관**: $f_u$는 고정 상수가 아닙니다. 혈장 단백 농도(protein concentration), 결합 친화도(binding affinity), 결합 부위 수, 유리 농도(unbound concentration) 등 여러 인자의 함수이기 때문입니다. 따라서 "총 농도 ↔ 유리 농도" 변환은 $f_u$가 일정하다는 검증 위에서만 안전합니다. [R&T pp.105–108; G&W pp.690–693]
...
>
> $$
> \underbrace{f_u}_{\text{유리분율}}=\frac{\underbrace{C_u}_{\text{유리 농도}}}{\underbrace{C}_{\text{총 농도}}}
> $$
>
> - **항별 의미:** 좌변은 전체 농도 중 결합되지 않은 비율을 정의합니다. 우변은 유리 농도 $C_u$를 총 농도 $C$로 나눈 비입니다.
> - **실무 직관:** $C_u$가 같은 총 농도에서 커지면 $f_u$는 증가하고, 결합 농도가 커져 총 농도 $C$가 커지면 $f_u$는 낮아집니다. 이 식은 총 농도를 유리 농도로 변환할 수 있는지의 출발점을 제공합니다.
> - **오해 방지:** $f_u$를 고정 상수로 자동 가정하지 마세요. 결합 포화, 단백 농도 변화, 질환 상태에 따라 달라질 수 있습니다.

[출처: R&T Eq.4-17, p.105]

<!-- SLIDE_START: S26 | title: M4 결합 친화도 | source_anchor: §2 / M4 / association constant -->
> **핵심 메시지:** $K_a$는 결합량 자체가 아니라 유리 약물과 이용 가능한 단백 부위를 기준으로 한 친화도 파라미터입니다.

단일 결합 부위에서 결합 상수(association constant)는 다음과 같습니다.
...
>
> $$
> \underbrace{K_a}_{\text{결합 친화도}}=\frac{\underbrace{C_{b,t}}_{\text{결합 약물 농도}}}{\underbrace{C_u}_{\text{유리 약물 농도}}\cdot\underbrace{P}_{\text{비어 있는 단백/결합부위}}}
> $$
>
> - **항별 의미:** 좌변은 약물과 단백 결합의 친화도를 정의합니다. 우변은 결합된 약물 농도를 유리 약물 농도와 이용 가능한 단백 또는 결합 부위 농도의 곱으로 나눈 값입니다.
> - **실무 직관:** $C_{b,t}$가 커지면 $K_a$ 추정은 커지고, 같은 결합량에서 $C_u$ 또는 $P$가 커지면 작아집니다. 이 식은 단백결합 강도가 $f_u$ 변화의 원인인지 판단하게 합니다.
> - **오해 방지:** $K_a$를 약물 농도 자체로 오해하지 마세요. 이는 결합 평형의 친화도 파라미터이며, 단백 농도와 함께 해석해야 합니다.

[출처: R&T Eq.4-18, p.105]

<!-- SLIDE_START: S27 | title: M4 질량작용과 fu | source_anchor: §2 / M4 / mass law -->
> **핵심 메시지:** 결합 친화도와 단백 가용성이 커질수록 $f_u$는 낮아지므로 총-유리 변환의 전제도 달라집니다.
...
>
> $$
> \underbrace{f_u}_{\text{유리분율}}=\frac{1}{1+\underbrace{K_a}_{\text{친화도}}\cdot\underbrace{f_{up}}_{\text{유리 단백분율}}\cdot\underbrace{P_t}_{\text{총 단백농도}}}
> $$
>
> - **항별 의미:** 좌변은 혈장 유리분율입니다. 우변의 분모는 결합 친화도, 이용 가능한 단백분율, 총 단백농도가 커질수록 커지는 결합 가능성을 나타냅니다.
> - **실무 직관:** $K_a$, $f_{up}$, $P_t$가 증가하면 분모가 커져 $f_u$는 감소합니다. 이 식은 단백 농도 변화나 결합 친화도 차이가 총-유리 변환에 미치는 방향을 판단하게 합니다.
> - **오해 방지:** 단백 농도만 보거나 친화도만 보면 불완전합니다. $f_u$는 결합 친화도와 단백 가용성의 곱 구조로 결정됩니다.

[출처: R&T Eq.4-19, p.105]

<!-- SLIDE_START: S28 | title: M4 PK47 단백결합 모델 | source_anchor: §2 / M4 / PK47 -->
> **핵심 메시지:** PK47 모델은 $C_u$, 단백 농도, 부위 수, 친화도를 함께 적합해야 $f_u$의 농도의존성을 볼 수 있음을 보여줍니다.
...
>
> $$
> \underbrace{f_u}_{\text{유리분율}}=1-\frac{1}{1+\underbrace{\frac{C_u}{n[P_T]}}_{\text{농도/결합용량 비}}+\underbrace{\frac{1}{K_a\cdot n[P_T]}}_{\text{친화도-용량 역항}}}
> $$
>
> - **항별 의미:** 좌변은 단일 결합 부위 클래스 모델에서의 유리분율입니다. 우변은 유리 농도 $C_u$, 결합 부위 수 $n$, 총 단백 농도 $[P_T]$, 친화도 $K_a$가 함께 $f_u$를 결정한다는 구조입니다.
> - **실무 직관:** $C_u$가 커지면 결합 부위가 상대적으로 포화되어 $f_u$가 증가할 수 있고, $n[P_T]$ 또는 $K_a$가 커지면 결합 능력이 커져 낮은 농도에서 $f_u$가 낮아집니다. 이 식은 PPB 포화성과 단백 농도 효과를 분리해 적합하게 합니다.
> - **오해 방지:** 이 식을 모든 결합 데이터에 자동 적용하지 마세요. 단일 결합 부위 클래스 가정이 맞지 않으면 잔차 패턴이 체계적으로 남을 수 있습니다.

[출처: G&W Eq.47:1, p.691]

<!-- SLIDE_START: S29 | title: M4 저 Cu 근사식 | source_anchor: §2 / M4 / low-Cu approximation -->
> **핵심 메시지:** 낮은 유리 농도 영역에서는 친화도와 총 결합용량이 $f_u$를 지배하는 빠른 진단식이 됩니다.
...
>
> $$
> \underbrace{f_u}_{\text{저 }C_u\text{ 근사 유리분율}}\approx\frac{1}{1+\underbrace{K_a}_{\text{친화도}}\cdot\underbrace{n[P_T]}_{\text{총 결합용량}}}
> $$
>
> - **항별 의미:** 좌변은 낮은 유리 농도 영역에서의 근사 유리분율입니다. 우변은 결합 친화도 $K_a$와 총 결합용량 $n[P_T]$의 곱이 유리분율을 지배한다는 단순화입니다.
> - **실무 직관:** $K_a$ 또는 $n[P_T]$가 커지면 $f_u$는 감소합니다. 이 근사식은 낮은 농도에서 단백 농도 또는 결합 친화도 변화가 유리분율에 미치는 방향을 빠르게 판단하게 합니다.
> - **오해 방지:** 근사식의 적용 범위를 잊지 마세요. $C_u$가 결합용량에 비해 충분히 낮지 않으면 농도 의존적 포화를 놓칠 수 있습니다.
...
<!-- SLIDE_START: S30 | title: M4 경계 조건과 실무 렌즈 | source_anchor: §2 / M4 / boundary -->
> **핵심 메시지:** 단일 $f_u$를 곱하는 변환은 $f_u$가 시간·농도·용량 범위에서 안정적일 때만 방어 가능합니다.

#### C. 경계 조건(Boundary Conditions)

- Eq.47:1은 단일 결합 부위 클래스(single binding-site class) 모델입니다. 다중 부위 클래스가 있으면 잔차 패턴(residual pattern)이 체계적으로 남을 수 있습니다.
- $AUC_u=AUC\times f_u$는 $f_u$가 시간·농도에 대해 충분히 일정할 때만 안전합니다. [G&W p.167]
- G&W는 $f_{ub}>1$이 가능하다고 명시합니다. 이는 bound/free ratio이므로, 결합 분율(fraction bound)과 혼동하지 않도록 주의해야 합니다. [G&W p.168]

<!-- TRENCH -->
**[교과서 외 실무 해석] 단백 공변량 선택(Protein covariate choice)**: 산성 약물은 알부민, 염기성 약물은 α1-AGP 변화에 먼저 주목합니다. R&T Table4-9는 알부민과 α1-AGP가 질환 상태에 따라 달라질 수 있음을 보여줍니다. [R&T Table4-9, p.114]

**[교과서 외 실무 경고] 단백결합 측정법 편향(PPB method bias)**: 평형투석(equilibrium dialysis)과 한외여과(ultrafiltration)는 방법 편향이 다를 수 있습니다. 이 문장은 PDF 원문이 아니라 실험 데이터 수령 단계의 실무 주의로만 사용합니다.

> **실무 렌즈(Practice Lens) — TEXTBOOK_DERIVED**  
> 총 $AUC$에 단일 $f_u$를 곱하기 전에는, 그 $f_u$가 의사결정에 쓰는 농도 범위에서 유지되는 값인지 확인해야 합니다. PK47의 핵심은 결합 파라미터 적합(binding parameter fit) 자체보다 "변환 단축식의 안전 조건"을 드러내는 데 있습니다.

<!-- RECAP -->
**M4 요약(recap)**: $f_u$는 단백 농도와 결합 부위 포화에 따라 변할 수 있습니다. 총-유리 변환(total-to-unbound conversion)은 $f_u$ 일정 가정이 무너지면 함께 무너집니다.

<!-- FIGURE_POINTER -->
Source: G&W 5e, p.691, Fig.47.1
Why this matters: $f_u$가 $C_u$와 단백 농도(protein concentration)에 따라 변하는 비선형 결합 곡선(binding curve)을 직접 보여줍니다.
When to look: 이 카드를 읽은 뒤 확인하면 됩니다.
Learner instruction: 낮은/높은 단백 농도 곡선(low/high protein curves)이 $n$과 $K_a$ 추정에 주는 정보를 확인해 보세요.
<!-- /FIGURE_POINTER -->

> **▶ 실무 체화 요약(Practice Brief) — M4 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가:** $f_u$는 총 농도와 유리 농도를 잇는 변환계수입니다. 시간·농도·dose에 따라 변하면 모든 단축식($AUC_u=AUC\times f_u$, $C_u=C\times f_u$)이 무너지기 때문입니다.
> - **흐름상 역할:** M3의 $V$ 구조와 M6의 노출 지표를 연결하는 핵심 매개 파라미터로, "변환 단축식의 안전 조건"을 정의합니다.
> - **체화 꿀팁:** 용량 정규화 $AUC$(dose-normalized AUC)가 용량에 따라 변할 때, 포화성 청소율(saturable $CL$)을 의심하기 전에 용량별 ex vivo $f_u$를 먼저 확인하세요. 진단 순서가 뒤집히면 원인 추적이 어긋납니다.
...
<!-- SLIDE_START: S31 | title: M5 조직결합과 Kp | source_anchor: §2 / M5 -->
> **핵심 메시지:** 큰 $V_d$는 혈장에 많이 풀려 있다는 뜻이 아니라 조직에 강하게 붙잡힌다는 신호일 수 있습니다.

### M5 · 조직결합과 Kp의 결정인자

<!-- MASTER LENS -->
**핵심 직관**: 큰 $V_d$는 "혈장에서 많이 풀려 있다"는 뜻이 아닐 수 있습니다. 오히려 "조직에서 더 강하게 붙잡힌다"는 신호인 경우가 많습니다. 염기성 약물(basic drug)의 큰 $V_d$는 혈장 결합보다 조직 결합, 특히 산성 인지질(acidic phospholipid) 친화도로 설명되는 경우가 많습니다. [R&T pp.108–111]

#### A. 형식적 정의(Formal Definition)

조직-혈장 평형 분포비 $K_p$<!-- ANNOTATION --> (← 조직/혈장 평형 농도비)는 조직 농도와 혈장 농도의 평형 비율입니다. R&T는 Eq.4-25와 Eq.4-12를 비교해 $K_p\sim f_u/f_{uT}$로 해석할 수 있음을 설명합니다. [R&T pp.104, 109–110]


> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{K_p}_{\text{조직/혈장 분포비}}\sim\frac{\underbrace{f_u}_{\text{혈장 유리분율}}}{\underbrace{f_{uT}}_{\text{조직 유리분율}}}
> $$
>
> - **항별 의미:** 좌변은 조직 농도와 혈장 농도의 평형 비율입니다. 우변은 혈장에서 풀려 이동 가능한 비율과 조직에서 풀려 있는 비율의 상대적 균형으로 $K_p$를 읽는 해석입니다.
> - **실무 직관:** $f_u$가 커지면 조직으로 갈 수 있는 비율이 커져 $K_p$가 커질 수 있고, $f_{uT}$가 작아지면 조직 결합이 강해져 $K_p$가 커질 수 있습니다. 이 관계는 큰 $V_d$의 원인을 혈장 결합과 조직 결합으로 나누어 판단하게 합니다.
> - **오해 방지:** 이 관계를 모든 상황의 정확한 항등식처럼 쓰면 안 됩니다. 수송체나 비평형 조건이 지배하면 단순 결합비 해석이 깨질 수 있습니다.

<!-- SLIDE_START: S32 | title: M5 출처 기반 상세 | source_anchor: §2 / M5 / details -->
> **핵심 메시지:** 조직 결합, 산성 인지질, BBB 수송체 사례는 같은 큰 $V$도 서로 다른 원인으로 만들어질 수 있음을 보여줍니다.
...
<!-- SLIDE_START: S33 | title: M5 혈장-혈액 변환 | source_anchor: §2 / M5 / plasma-to-blood -->
> **핵심 메시지:** 혈액/혈장 농도비는 hematocrit, $f_u$, 적혈구 결합이 함께 결정하므로 고정 상수처럼 쓰면 위험합니다.
...
>
> $$
> \underbrace{\frac{C_b}{C}}_{\text{혈액/혈장 농도비}}=1+\underbrace{H}_{\text{적혈구용적률}}\left(\underbrace{f_u\cdot K_{PBC}}_{\text{적혈구 분배 기여}}-1\right)
> $$
>
> - **항별 의미:** 좌변은 혈액 농도와 혈장 농도의 비입니다. 우변은 기준값 1에 적혈구용적률 $H$와 유리분율 및 적혈구 분배계수 $K_{PBC}$가 만드는 보정항을 더한 구조입니다.
> - **실무 직관:** $H$가 커질수록 적혈구 분배 효과가 더 크게 반영되고, $f_u\cdot K_{PBC}$가 1보다 크면 혈액/혈장 비가 증가합니다. 이 식은 plasma 기반 농도와 blood 기반 농도를 변환할 때 필요한 조건을 판단하게 합니다.
> - **오해 방지:** $C_b/C$를 고정 상수처럼 다루면 안 됩니다. hematocrit, $f_u$, 적혈구 결합이 바뀌면 같은 약물도 혈액-혈장 변환이 달라질 수 있습니다.

[출처: R&T App.D, pp.775–776]

<!-- SLIDE_START: S34 | title: M5 경계 조건과 실무 실패 모드 | source_anchor: §2 / M5 / boundary -->
> **핵심 메시지:** 수송체가 지배적이면 단순한 조직-혈장 유리농도 평형 가정은 깨질 수 있습니다.
...
> Eq.4-29에서 결합 항(binding term)과 수송체 항(transporter term)을 같은 종류의 원인으로 합쳐 해석하면 안 됩니다. $f_{uT}$는 조직 결합을 나타내고, $P_{uptake}/P_{efflux}$는 능동 분포 불균형을 나타냅니다. 따라서 둘은 같은 $V$ 변화를 만들 수 있어도 실험적 확인 방법과 모델링 의미가 다릅니다.

<!-- RECAP -->
**M5 요약(recap)**: $K_p$는 조직 친화도의 숫자 표현이고, $f_{uT}$는 조직 결합의 역신호(inverse signal)입니다. 수송체가 있으면 $K_p$는 결합뿐 아니라 유입/유출 불균형(uptake/efflux imbalance)까지 반영합니다.

> **▶ 실무 체화 요약(Practice Brief) — M5 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가:** 큰 $V_d$의 원인이 혈장 $f_u$ 증가인지 조직 $f_{uT}$ 감소인지 구분하지 못하면, 단백결합 변화의 임상 결과 방향이 정반대로 도출됩니다.
> - **흐름상 역할:** M3의 조직 항($V_{TW}\cdot f_u/f_{uT}$)을 결합($f_{uT}$)과 수송체($P_{uptake}/P_{efflux}$)로 분해해 M2의 율속 단계와 다시 연결하는 단계입니다.
> - **체화 꿀팁:** 염기성 약물에서 큰 $V_d$를 보면 산성 인지질 결합을 1순위 가설로, 수송체 기질 가능성을 2순위로 점검하세요. Eq.4-29의 두 항을 같은 종류의 원인으로 합치지 않는 것이 핵심입니다.
> - **실무 활용:** PBPK Kp 예측 모델 선택, BBB 약물의 조직 농도 해석, propranolol·amiodarone 같은 대 Vd 염기성 약물의 공변량 모델링, indinavir의 CSF/혈장 격차 해석.

---

<!-- SLIDE_START: S35 | title: M6 노출 지표 선택 | source_anchor: §2 / M6 -->
> **핵심 메시지:** 노출 지표 선택은 숫자 선택이 아니라 어떤 농도 종과 시간 패턴이 효과를 만든다는 인과적 선언입니다.

### M6 · 노출 지표 선택: 총(total) vs 유리(unbound)

<!-- MASTER LENS -->
**핵심 직관**: 노출 지표(exposure metric)는 "하나의 $AUC$ 숫자"가 아닙니다. 즉, 노출 지표는 어떤 농도 종(species)이 어떤 시간 패턴으로 약리 효과를 유발하는가에 대한 인과적 선언입니다. 총 농도가 단백결합 차이에 교란(confounded)되면 유리 농도가 활성 비교 기준(active comparator)이 됩니다. [G&W pp.163–169]
...
>
> $$
> \underbrace{C}_{\text{총 농도}}=\frac{\underbrace{C_u}_{\text{유리 농도}}}{\underbrace{f_u}_{\text{유리분율}}}
> $$
>
> - **항별 의미:** 좌변은 총 농도입니다. 우변은 유리 농도를 유리분율로 나누어, 결합된 부분까지 포함한 총 농도로 환산하는 구조입니다.
> - **실무 직관:** $C_u$가 커지면 총 농도도 커지고, 같은 $C_u$에서 $f_u$가 작아지면 총 농도는 커집니다. 이 식은 총 농도-반응과 유리 농도-반응이 왜 서로 다른 결론을 낼 수 있는지 보여줍니다.
> - **오해 방지:** 총 농도가 높다고 활성 유리 농도가 반드시 높은 것은 아닙니다. $f_u$가 낮으면 총 농도는 높아도 $C_u$는 제한될 수 있습니다.

<!-- SLIDE_START: S36 | title: M6 유리 AUC 단축식 | source_anchor: §2 / M6 / AUCu -->
> **핵심 메시지:** $AUC_u=AUC\times f_u$는 유리분율이 일정하다는 조건이 지켜질 때만 안전한 단축식입니다.
...
>
> $$
> \underbrace{AUC_u}_{\text{유리 노출}}=\underbrace{AUC}_{\text{총 노출}}\times\underbrace{f_u}_{\text{유리분율}}
> $$
>
> - **항별 의미:** 좌변은 유리 농도 기준의 시간-노출입니다. 우변은 총 농도 기준 AUC에 유리분율을 곱해 유리 노출로 환산하는 단축식입니다.
> - **실무 직관:** 총 $AUC$ 또는 $f_u$가 커지면 $AUC_u$는 커집니다. 이 식은 총 노출이 단백결합 차이에 교란될 때 활성 종에 가까운 노출 지표를 계산하는 판단을 가능하게 합니다.
> - **오해 방지:** 이 식은 $f_u$가 일정할 때만 안전합니다. 시간·농도·용량에 따라 $f_u$가 변하면 단일 $f_u$를 곱한 $AUC_u$는 왜곡될 수 있습니다.

[출처: G&W Eq.2:373, p.167]

<!-- SLIDE_START: S37 | title: M6 출처 기반 상세 | source_anchor: §2 / M6 / details -->
> **핵심 메시지:** 같은 AUC라도 peak, duration, species scale이 다르면 효능·독성 판단은 달라질 수 있습니다.

#### B. 출처 기반 상세(Source-Anchored Details)

<!-- ANNOTATION --> 다시 말해, M6의 질문은 "$AUC$를 쓸 것인가?"가 아니라 "어느 종(species)의 $AUC$ 또는 어떤 시간 패턴을 쓸 것인가?"입니다.
...
<!-- SLIDE_START: S38 | title: M6 경계 조건과 figure pointer | source_anchor: §2 / M6 / boundary -->
> **핵심 메시지:** 총 농도와 유리 농도 중 무엇을 측정할지는 의사결정이 어느 종에 민감한지에 따라 결정됩니다.

#### C. 경계 조건(Boundary Conditions)

- $f_u$가 용량·시간·농도 의존적이면 $AUC_u=AUC\times f_u$ 단축식은 무효가 될 수 있습니다.
- 총 $AUC$가 같아도 최고 농도 매개 독성(peak-driven toxicity) 또는 역치 초과 시간 매개 효과(duration-above-threshold effect)는 다를 수 있습니다. [G&W Fig.2.136, p.164]
- "유리 농도를 항상 측정하라"가 아니라 "의사결정이 유리 종(species)에 민감한 상황에서 측정하라"가 G&W의 결론입니다. [G&W p.169]

> **실무 렌즈(Practice Lens) — TEXTBOOK_DERIVED**  
> 노출 지표는 보고서의 숫자 선택이 아니라 인과적 주장(causal claim)의 선택입니다. $AUC$, $C_{max}$, $t_d$ 중 무엇을 전면에 둘지는 "효과를 만든 시간 패턴이 무엇인가"와 "총(total)과 유리(unbound) 중 어떤 종(species)이 비교 가능한가"를 함께 답해야 정당화됩니다.

<!-- RECAP -->
**M6 요약(recap)**: 총 농도는 분석 친화적(assay-friendly) 지표이고, 유리 농도는 활성 종(species)에 가까운 지표입니다. 어느 쪽을 쓸지는 결합 차이, 종간 외삽(species extrapolation), 노출 형태(exposure shape), 의사결정 위험도(decision risk)가 결정합니다.

<!-- FIGURE_POINTER -->
Source: G&W 5e, p.163, Fig.2.134
Why this matters: 용량(dose), 총 농도(total concentration), 유리 농도(unbound concentration)가 서로 다른 역가(potency) 결론을 만들 수 있음을 보여줍니다.
When to look: 이 카드를 읽은 뒤 확인하면 됩니다.
Learner instruction: 총 농도-반응(total-response)과 유리 농도-반응(unbound-response)에서 역가 순위(potency order)가 어떻게 바뀌는지 확인해 보세요.
<!-- /FIGURE_POINTER -->

> **▶ 실무 체화 요약(Practice Brief) — M6 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가:** 노출 지표 선택은 단순한 보고 형식이 아니라 효과의 인과 메커니즘에 대한 주장입니다. 잘못 고르면 종간 비교와 용량 결정이 동시에 왜곡됩니다.
> - **흐름상 역할:** M1~M5의 모든 분포 정보를 "어느 종(species)의 어떤 시간 패턴이 효과를 만드는가"라는 단일 의사결정으로 수렴시키는 세션의 종착점입니다.
> - **체화 꿀팁:** $AUC$, $C_{max}$, $t_d$ 중 무엇을 전면에 둘지는 기전(mechanism)이 결정합니다. 효과가 최고 농도 매개(peak-driven)이면 $C_{max}$, 누적 매개(cumulative-driven)이면 $AUC$, 역치 매개(threshold-driven)이면 $t_d$입니다. 이 매핑을 먼저 한 줄로 적은 후 종(species)/스케일(scale)을 결정하면 됩니다.
...
<!-- SLIDE_START: S39 | title: §5 혼동쌍 1: C vs Cu | source_anchor: §5 / confusion pair 1 -->
> **핵심 메시지:** $C$와 $C_u$의 선택은 참/거짓 문제가 아니라 현재 의사결정의 인과적 종을 고르는 문제입니다.
...
<!-- CONFUSION -->
| 구분 | $C$, 총 농도(total concentration) | $C_u$, 유리 농도(unbound concentration) |
|---|---|---|
| 정의 | 결합 + 비결합 | 비결합만 |
...
> $C$ vs $C_u$ 판단은 네 단계로 고정합니다. 인과적 종(species) → 분석 스케일(assay scale) → 변환 유효성(conversion validity) → 의사결정 위험도(decision risk)입니다. 이 순서가 없으면 "유리(unbound)가 더 맞다"와 "총(total)도 충분하다"가 둘 다 근거 없는 주장으로 변합니다.
...
<!-- SLIDE_START: S40 | title: §5 혼동쌍 2: 관류 vs 투과 | source_anchor: §5 / confusion pair 2 -->
> **핵심 메시지:** 분포 범위와 분포 속도는 독립 차원이므로 $V_d$ 하나만으로 시간 경과를 설명할 수 없습니다.
...
분포 범위(distribution extent, $V_d$의 크기)는 약이 최종적으로 얼마나 넓게 퍼지는지를 결정합니다. 분포 속도(distribution rate, $Q$와 조직 혈류·막 투과도)는 그 최종 상태에 얼마나 빨리 도달하는지를 결정합니다. 빠른 분포 속도 + 작은 $V_d$는 짧은 시간 안에 좁은 구획에 가두는 패턴이고, 느린 분포 속도 + 큰 $V_d$는 오래 걸려 넓게 퍼지는 패턴입니다. 이 두 차원은 독립적으로 작동하므로, $V_d$ 하나만으로는 분포 과정의 시간 경과를 논할 수 없습니다.

<!-- SLIDE_START: S41 | title: §5 혼동쌍 3: fu vs fuT | source_anchor: §5 / confusion pair 3 -->
> **핵심 메시지:** $f_u$와 $f_{uT}$는 모두 유리분율이지만 $V$에서는 분자와 분모로 작동해 반대 방향의 효과를 만들 수 있습니다.

### 혼동쌍 3 — $f_u$ vs $f_{uT}$

<!-- CONFUSION -->
$f_u$는 혈장에서 유리 분율이고, $f_{uT}$는 조직에서 유리 분율입니다. $V=V_p+V_{TW}(f_u/f_{uT})$이므로 $f_u$가 커지면 $V$는 커질 수 있고, $f_{uT}$가 작아지면 $V$는 크게 커집니다. [R&T Eq.4-25, p.109]


> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{V}_{\text{겉보기 }V}=\underbrace{V_p}_{\text{혈장 항}}+\underbrace{V_{TW}\left(\frac{\overbrace{f_u}^{\text{혈장 유리분율}}}{\underbrace{f_{uT}}_{\text{조직 유리분율}}}\right)}_{\text{조직 결합 항}}
> $$
>
> - **항별 의미:** 좌변은 겉보기 분포용적입니다. 우변은 혈장 항과 혈장/조직 유리분율 비율이 반영된 조직 항으로 구성됩니다.
> - **실무 직관:** $f_u$가 증가하거나 $f_{uT}$가 감소하면 조직 항이 커져 $V$가 증가할 수 있습니다. 이 식은 $f_u$와 $f_{uT}$가 서로 반대 방향의 임상적 의미를 가질 수 있음을 판단하게 합니다.
> - **오해 방지:** $f_u$와 $f_{uT}$를 모두 '유리분율'이라는 이유로 같은 방향 효과로 취급하면 안 됩니다. 혈장 유리분율과 조직 유리분율은 $V$에서 비율로 작동합니다.

**⚡ 기억 고리(Memory Hook) — *정적 균형점 vs 동적 제거 그림자 (Vss vs Vβ)* — [EXPERT_INFERENCE, v3]**
$V_{ss}$는 분포와 제거가 완전히 독립인 **정적 균형 부피**입니다. 제거 속도는 $V_{ss}$에 영향을 주지 않습니다. $V_\beta$는 종말상 소실 기울기(terminal elimination slope)에서 역산한 **겉보기 부피**입니다. 조직에서 계속 약이 돌아오는 재분포(redistribution) 흐름을 반영하기 때문에 제거가 느릴수록 $V_{ss}$보다 커집니다. 따라서 $V_\beta$를 $V_{ss}$ 대신 쓰면 정상상태 분포 부피를 과대추정하고 부하 용량이 잘못 계산됩니다. 이 기억 고리는 $f_u/f_{uT}$ 분기를 다룰 때 함께 점검할 사항을 가리킵니다. 다구획 모델에서 $V$를 $V_{ss}$로 쓰는가, $V_\beta$로 쓰는가를 명시적으로 확인하면 됩니다.

<!-- SLIDE_START: S42 | title: §5 혼동쌍 4: 소 Vd vs 대 Vd | source_anchor: §5 / confusion pair 4 -->
> **핵심 메시지:** 단백결합 변화의 임상 방향은 소 $V_d$ 알부민 결합 양상인지 대 $V_d$ 조직 결합 양상인지에 따라 달라집니다.
...
$AUC$는 총 화물량(결합 + 유리)이고, $AUC_u=f_u\times AUC$는 실제로 수용체에 도달 가능한 활성 화물량입니다. 알부민 결합이 강한 약물에서 저알부민혈증이 생기면, $AUC$는 줄어도 $AUC_u$는 오히려 늘 수 있습니다. 바로 이 때문에 노출-반응(exposure-response) 분석을 $AUC$로 수행하면 효과 방향이 역전되는 결과가 나옵니다. 소 $V_d$ vs 대 $V_d$ 분기를 판단할 때, 실제로 의사결정에 쓰이는 지표가 $AUC$인지 $AUC_u$인지를 함께 묻지 않으면 PPB 변화의 임상 결과 방향을 오독합니다.


> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{AUC_u}_{\text{활성 유리 노출}}=\underbrace{f_u}_{\text{유리분율}}\times\underbrace{AUC}_{\text{총 노출}}
> $$
>
> - **항별 의미:** 좌변은 유리 농도 기준 노출이고, 우변은 총 노출 중 유리분율에 해당하는 부분을 추출한 값입니다.
> - **실무 직관:** $AUC$ 또는 $f_u$가 커지면 $AUC_u$는 증가합니다. 이 식은 단백결합 변화가 있는 상황에서 총 노출과 활성 노출이 같은 방향으로 움직이는지 판단하게 합니다.
> - **오해 방지:** 총 $AUC$가 낮아졌다고 항상 활성 노출이 낮아졌다고 결론 내리면 안 됩니다. $f_u$ 변화가 더 크면 $AUC_u$의 방향은 달라질 수 있습니다.

---

<!-- SLIDE_START: S43 | title: §7 자기 테스트 Q1 | source_anchor: §7 / Q1 -->
> **핵심 메시지:** Eq.4-1을 이해했다면 막투과의 농도차가 총 농도가 아니라 유리 농도차임을 설명할 수 있어야 합니다.
...
<!-- SLIDE_START: S44 | title: §7 자기 테스트 Q2 | source_anchor: §7 / Q2 -->
> **핵심 메시지:** 투과도 판단에서는 크기 증가와 지용성 증가가 반대 방향으로 작동하며 수송체 예외가 항상 남습니다.
...
<!-- SLIDE_START: S45 | title: §7 자기 테스트 Q3 | source_anchor: §7 / Q3 -->
> **핵심 메시지:** $K_{pb}$ 증가는 더 많이 분포한다는 신호이면서 분포 반감기를 늘리는 시간 지연 신호입니다.
...
>
> $$
> \underbrace{t_{1/2,distribution}}_{\text{분포 반감기}}=\frac{\underbrace{0.693}_{\ln2}\cdot\underbrace{K_{pb}}_{\text{조직 친화도/저장성}}}{\underbrace{Q/V_T}_{\text{조직 관류속도}}}
> $$
>
> - **항별 의미:** 좌변은 분포 과정의 반감기입니다. 우변은 조직 저장성 $K_{pb}$가 시간 지연을 늘리고, 단위 조직 관류속도 $Q/V_T$가 시간 지연을 줄이는 구조입니다.
> - **실무 직관:** $K_{pb}$가 커지면 분포 반감기가 길어지고, $Q/V_T$가 커지면 짧아집니다. 이 문항은 '많이 분포한다'와 '빨리 분포한다'를 구분하는 판단을 훈련합니다.
> - **오해 방지:** 이 반감기를 종말상 소실 반감기로 읽지 마세요. 여기서는 조직 평형 접근 속도를 묻고 있습니다.

<!-- SLIDE_START: S46 | title: §7 자기 테스트 Q4 | source_anchor: §7 / Q4 -->
> **핵심 메시지:** 소 $V_d$ 알부민 결합 약물에서는 $f_u\to0$이어도 알부민 분포 용적이 $V$의 하한을 만듭니다.
...
<!-- SLIDE_START: S47 | title: §7 자기 테스트 Q5 | source_anchor: §7 / Q5 -->
> **핵심 메시지:** Warfarin 신증후군 사례는 albumin 감소가 총 농도 기준 $V$, $CL$, 반감기를 함께 바꿀 수 있음을 묻습니다.
...
<!-- SLIDE_START: S48 | title: §7 자기 테스트 Q6 | source_anchor: §7 / Q6 -->
> **핵심 메시지:** $AUC_u=AUC\times f_u$는 단축식이므로 $f_u$가 안정적이라는 조건을 먼저 확인해야 합니다.
...
>
> $$
> \underbrace{AUC_u}_{\text{유리 노출}}=\underbrace{AUC}_{\text{총 노출}}\times\underbrace{f_u}_{\text{일정하다고 가정한 유리분율}}
> $$
>
> - **항별 의미:** 좌변은 유리 노출이고, 우변은 총 노출에 유리분율을 곱한 단축 계산입니다.
> - **실무 직관:** $AUC$ 또는 $f_u$가 증가하면 계산상 $AUC_u$가 증가합니다. 이 문항은 단축식 사용 전 $f_u$ 안정성을 확인해야 한다는 실무 판단을 묻습니다.
> - **오해 방지:** $f_u$가 시간·농도·용량에 따라 변하면 하나의 $f_u$를 곱하는 방식은 실제 유리 AUC를 보존하지 못할 수 있습니다.

<!-- SLIDE_START: S49 | title: §7 자기 테스트 Q7 | source_anchor: §7 / Q7 -->
> **핵심 메시지:** 같은 AUC라도 peak와 threshold-above time이 다르면 독성 또는 효과는 달라질 수 있습니다.
...
<!-- SLIDE_START: S50 | title: §7 자기 테스트 Q8 | source_anchor: §7 / Q8 -->
> **핵심 메시지:** PK47에서는 저/고 단백 농도 동시 적합이 $n$과 $K_a$의 식별성을 높이는 핵심 장치입니다.
...
<!-- SLIDE_START: S51 | title: §7 자기 테스트 Q9 | source_anchor: §7 / Q9 -->
> **핵심 메시지:** 총 AUC가 종간 차이를 보이고 유리 $EC_{u50}$가 정렬되면 안전 마진의 중심은 유리 노출이어야 합니다.
...
<!-- SLIDE_START: S52 | title: §7 자기 테스트 Q10 | source_anchor: §7 / Q10 -->
> **핵심 메시지:** 용량 정규화 AUC 감소를 보면 포화성 clearance와 포화성 PPB를 $f_u$ 자료로 먼저 분리해야 합니다.
...
<!-- SLIDE_START: S53 | title: §8 메타프레임 A: 위치 설정 | source_anchor: §8 / A -->
> **핵심 메시지:** 이 세션의 분포 개념은 이후 clearance, TDM, PBPK, PopPK 공변량 모델링의 공통 기반입니다.
...
<!-- SLIDE_START: S54 | title: §8 메타프레임 B: 의존성 | source_anchor: §8 / B -->
> **핵심 메시지:** 분포 개념이 약하면 부하 용량, 공변량, 노출 지표, 종간 외삽, 단백결합 오류가 연쇄적으로 발생합니다.

### B. 의존성(Dependencies)

이 섹션을 약하게 이해하면 다음 오류가 발생합니다.

1. **부하 용량 오류**: $V$가 겉보기 비례상수임을 잊고 생리학적 부피처럼 해석합니다.
2. **공변량 오류**: 알부민 변화가 총 $CL$, 총 $V$, 유리 노출에 미치는 방향을 구분하지 못합니다.
3. **노출 지표 오류**: 동일 $AUC$를 동일 효과로 가정하고 최고 농도/지속 시간 정보를 잃습니다.
4. **종간 외삽 오류**: 총 농도-반응의 역가 순위를 활성 유리 종(species)의 역가 순위로 착각합니다.
5. **단백결합 오류**: $f_u$를 상수로 가정하고 포화성 결합 또는 단백 농도 변화를 놓칩니다.

<!-- SLIDE_START: S55 | title: §8 메타프레임 C: 전문가 해석 지점 | source_anchor: §8 / C -->
> **핵심 메시지:** 전문가 수준의 핵심은 분포를 위치 문제가 아니라 결합, 투과도, 혈류가 만드는 평형과 시간 경과 문제로 읽는 것입니다.
...
<!-- SLIDE_START: S56 | title: §8 메타프레임 D: 시스템 통합 지점 | source_anchor: §8 / D -->
> **핵심 메시지:** 임상약리 요약, TDM, NONMEM 공변량 해석은 M1–M6 카드를 동시에 호출할 때 실무 문단이 됩니다.

### D. 시스템 통합 지점(System Integration Points)

- **임상약리 요약**: 용량-노출 선형성(dose-exposure linearity)을 논할 때, M1(흡수/투과), M3(V/용량 의존성), M4(PPB 포화 여부), M6(노출 지표 선택) 네 카드가 하나의 문단 안에서 논리적으로 연결됩니다.
- **TDM**: 저알부민혈증, 신부전, α1-AGP 변화가 동시 발생하면 총 농도만으로는 활성 노출을 놓칠 수 있습니다.
- **NONMEM**: 알부민/α1-AGP 공변량은 $V$에만 기계적으로 붙이는 것이 아니라, 총(total) vs 유리(unbound) 스케일과 $CL$ 해석까지 함께 결정해야 합니다.

> **체화 노트(Mastery Note) — CRUCIBLE_DERIVED**  
> 위 세 통합 지점은 두 가지 **구체적 실무 단락**으로 작동합니다. 카드 단위 학습이 끝났는지를 자가 점검하려면 다음 두 단락을 한 번씩 자기 언어로 재구성해 보세요.
>
> **시나리오 1 — 임상약리 요약(용량-노출 선형성 단락)**  
> 한 단락이 6개 카드를 동시에 호출합니다: 약물이 막을 통과해 혈장에 들어오는가(M1) → 같은 용량에서 $V$가 용량에 따라 변하는가(M3) → $f_u$가 용량에 따라 변하는가(M4) → 조직 분포가 포화성 수송체에 묶여 있는가(M5) → 어떤 종(species)의 어떤 지표로 선형성을 판정하는가(M6). 이 단락이 자연스럽게 흘러야 6개 카드가 단일 시스템으로 작동합니다.
>
> **시나리오 2 — ICU 환자 TDM 의사결정**  
> 저알부민혈증(albumin↓) + 급성 신부전(uremia, fu 결합 경쟁) + α1-AGP 상승(외상 반응)이 동시 발생하는 한 환자의 한 용량 결정을 상상해 보시면 됩니다. M4($f_u$가 알부민·변위체·α1-AGP의 다중 함수) → M3(소 $V_d$ 약물에서 $V$가 $f_u$에 비례 증가) → M6(총 농도 "정상 범위"가 독성 $C_u$를 가릴 수 있음)을 한 결정에 묶어야 합니다. 이 시퀀스가 카드 단위로 분리되어 있으면 실무 통합이 끊깁니다.

<!-- SLIDE_START: S57 | title: §8 메타프레임 E: 진단 시그니처 1 | source_anchor: §8 / E / signature 1 -->
> **핵심 메시지:** 알부민 공변량 비대칭은 $V$만 보지 말고 $CL$ 공변량까지 함께 점검하라는 PopPK 진단 신호입니다.

### E. 실무 진단 시그니처(Practical Diagnostic Signatures)

> **체화 노트(Mastery Note) — CRUCIBLE_DERIVED**  
> 본 세션 개념의 오용은 PopPK 진단 플롯에 **반복 가능한 시그니처 패턴**을 남깁니다. 이름을 부여하면 진단 속도가 카드 단위 → 시그니처 단위로 압축됩니다. 아래 시그니처는 모두 [교과서 외 실무 해석]이며, 출처 PDF의 직접 인용이 아닙니다.

**시그니처 1 — 알부민 공변량 비대칭 패턴(Albumin Covariate Asymmetry Pattern)**  
*기전(Mechanism)*: 대 $V_d$ 약물에서 $V$ ~ albumin만 공변량 spec하고 $CL$ ~ albumin을 누락.  
...
<!-- SLIDE_START: S58 | title: §8 메타프레임 E: 진단 시그니처 2 | source_anchor: §8 / E / signature 2 -->
> **핵심 메시지:** PK47 단일 부위 모델의 V자형 잔차는 결합 부위 클래스가 하나가 아닐 수 있음을 알려주는 신호입니다.
...
<!-- SLIDE_START: S59 | title: §8 메타프레임 E: 진단 시그니처 3 | source_anchor: §8 / E / signature 3 -->
> **핵심 메시지:** $V_{ss}$와 $V_eta$를 혼동하면 부하 용량과 종말상 반감기 해석이 서로 다른 방향으로 틀어집니다.
...
<!-- SLIDE_START: S60 | title: §8 메타프레임 F: 체화 시퀀스 | source_anchor: §8 / F -->
> **핵심 메시지:** 새 PK 문제는 species 결정, $V_d$ 양상 결정, $f_u$ 안정성 점검, 지표 정당화 순서로 풀어야 합니다.

### F. 체화 시퀀스(Mastery Sequence)

> **체화 노트(Mastery Note) — EXPERT_INFERENCE**  
> 새 PK 문제를 만났을 때 본 세션의 6개 카드를 가장 빠르게 작동시키는 4단계 시퀀스입니다. 본 시퀀스는 출처 PDF의 직접 권고가 아니라, 본 핸드아웃의 내적 통합 지침입니다. 카드 학습이 끝난 후, 새 사례에 이 시퀀스를 반복 적용하면 분리된 카드가 단일 진단 시퀀스로 통합됩니다.

1. **Species 결정 — 지금 보고 있는 농도는 무엇인가?**  
   총 $C$인가, 유리 $C_u$인가, 혈액 $C_b$인가? 단위와 측정 매질을 먼저 고정합니다. (M1·M4·M6의 진입점)


> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{C}_{\text{총 농도}},\quad \underbrace{C_u}_{\text{유리 농도}},\quad \underbrace{C_b}_{\text{혈액 농도}}
> $$
>
> - **항별 의미:** 이 표기는 하나의 예측식이 아니라 측정 종(species)과 매질(matrix)을 구분하는 정의식에 가깝습니다. 각 기호는 총 농도, 유리 농도, 혈액 농도를 서로 다른 좌표계로 분리합니다.
> - **실무 직관:** 같은 약물량이라도 어떤 농도 종을 쓰느냐에 따라 $V$, $CL$, 노출-반응 해석이 달라집니다. 이 구분은 새 PK 문제를 시작할 때 가장 먼저 해야 할 판단입니다.
> - **오해 방지:** $C$, $C_u$, $C_b$를 같은 농도의 다른 이름처럼 취급하면 안 됩니다. 분모와 측정 매질이 달라지면 파라미터 해석도 바뀝니다.

2. **$V_d$ 양상 결정(regime) — 어느 모델이 작동하는가?**  
   소 $V_d$ 알부민 결합 양상(App.C / Eq.C-13, $V\lesssim15$ L)인가, 대 $V_d$ 조직 결합 양상(Eq.4-25 또는 4-29)인가? 이 분기에 따라 $f_u$ 변화의 임상적 결과 방향이 정반대가 됩니다. (M3·M5의 분기점)


> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{V\lesssim15\ \mathrm{L}}_{\text{소 }V_d\text{ 알부민 결합 양상}}
> $$
>
> - **항별 의미:** 좌변의 부등식은 겉보기 분포용적이 작은 영역에 있는지를 판정하는 실무적 분기 표시입니다. 이는 알부민 결합 양상과 조직 결합 양상을 구분하기 위한 기준점 역할을 합니다.
> - **실무 직관:** $V$가 이 범위에 가까우면 알부민 분포 하한과 혈장 단백결합 변화가 중요해지고, 훨씬 큰 $V$에서는 조직 결합과 수송체 해석이 중요해집니다. 이 분기는 $f_u$ 변화의 임상적 방향을 판단하게 합니다.
> - **오해 방지:** 이 부등식을 절대적 생리학 경계로 오해하지 마세요. 핵심은 숫자 자체가 아니라 소 $V_d$ 알부민 결합 양상과 대 $V_d$ 조직 결합 양상의 구분입니다.

3. **$f_u$ 안정성 점검 — 단축식이 유효한가?**  
   지금 비교하려는 용량·시간·농도 범위에서 $f_u$가 일정한가? 일정하지 않다면 $AUC_u=AUC\times f_u$ 단축식과 $C_u=C\times f_u$ 변환은 무효가 될 수 있습니다. (M4 경계 점검)


> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{AUC_u}_{\text{유리 노출}}=\underbrace{AUC}_{\text{총 노출}}\times\underbrace{f_u}_{\text{유리분율}},\qquad \underbrace{C_u}_{\text{유리 농도}}=\underbrace{C}_{\text{총 농도}}\times\underbrace{f_u}_{\text{유리분율}}
> $$
>
> - **항별 의미:** 좌변들은 유리 노출과 유리 농도입니다. 우변은 총 노출 또는 총 농도에 유리분율을 곱해 유리 스케일로 변환하는 단축 구조입니다.
> - **실무 직관:** $AUC$, $C$, 또는 $f_u$가 커지면 계산된 유리 지표가 커집니다. 이 식들은 총 농도 자료만 있을 때 유리 노출을 근사할 수 있는지 판단하게 합니다.
> - **오해 방지:** 핵심 오해는 $f_u$가 언제나 일정하다고 가정하는 것입니다. $f_u$가 변하면 이 변환식은 단순 계산이 아니라 잘못된 가정이 됩니다.

4. **지표 정당화(Metric justification) — 어느 종(species)의 어느 시간 패턴인가?**  
   의사결정의 인과 기전이 $C_{max}$ 매개인가, $AUC$ 매개인가, $t_d$ 매개인가? 그리고 그 기전이 총 농도와 유리 농도 중 어느 종(species)에 작동하는가? (M6 결정 단계)
...
## v3.5 Final Checklist

| 항목 | 판정 | 근거 |
|---|---|---|
| PART A만 수정 | PASS | PART A learner-facing prose, FIGURE_POINTER field values, and math notation only were refined; PART B source text was appended unchanged. |
| PART B guardrail 보존 | PASS | Original PART B text is preserved verbatim from `## PART B — Compiler-Only Appendix` to its original end; this checklist is a separate sibling appendix. |
| 과학적 내용 무변경 | PASS | Edits are limited to expression, Korean style, and notation normalization. |
| 수치·약물·예시 무추가 | PASS | Existing numerical anchors, drug examples, and case examples were retained without additions. |
| page tag 보존 | PASS | Existing `[R&T ...]` and `[G&W ...]` page tags were retained. |
| source label 보존 | PASS | Existing TEXTBOOK_DERIVED / AUDIT_DERIVED / CRUCIBLE_DERIVED / EXPERT_INFERENCE labels were retained. |
| figure marker 경계 태그 보존 | PASS | All `<!-- FIGURE_POINTER -->` and `<!-- /FIGURE_POINTER -->` markers remain paired. |
| FIGURE_POINTER 필드 키 보존 | PASS | Source / Why this matters / When to look / Learner instruction keys were preserved. |
| FIGURE_POINTER source bibliographic identity 보존 | PASS | Author/source identity, edition, page number, and figure number strings were retained. |
| FIGURE_POINTER 필드값 설명문 한국어화 | PASS | Field values were converted to Korean-friendly learner-facing prose while preserving source identity. |
| HTML compiler marker 보존 | PASS | Existing compiler markers such as MASTER LENS, ANNOTATION, TRENCH, CONFUSION, SELF-TEST, RECAP remain. |
| code block 내부 무변경 | PASS | Fenced code block content was excluded from transformations. |
| plain-text 수식의 MathJax/LaTeX화 | PASS | Backticked and plain-text mathematical expressions in PART A were normalized where applicable. |
| 일반 영어 산문 한국어화 | PASS | Remaining general learner-facing English prose was Koreanized where not a protected technical term. |
| career-critical term 첫 등장 병기 | PASS | Core technical terms retain Korean-English paired expression where needed. |
| 약어 첫 등장 규칙 준수 | PASS | Major PK/PD and modeling abbreviations remain paired or conventional. |
| 박사과정 학습자용 친절한 전문 강의체 적용 | PASS | Imperative/report-style prose was adjusted to polite expert lecture style. |
| 존댓말 기반 친절한 전문 해설체 적용 | PASS | Sentence endings were softened into `~입니다/~합니다/~하세요` style where learner-facing. |
| Phase 5 HTML 컴파일 준비 상태 유지 | PASS | Section/card structure, markers, page tags, source labels, and fenced code blocks are preserved. |
