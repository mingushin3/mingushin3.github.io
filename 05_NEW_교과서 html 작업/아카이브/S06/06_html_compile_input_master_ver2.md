# 06_HTML Compile Input Master — ver2

## Version & Patch Log

**Revision**: ver2 (surgical patches applied to ver1 master)
**Patch scope**: §5 protocol-mandated Memory Hooks 복원 + Audit Required Patch #7 (Holford message) 통합 + Crucible Grade A4·A5 강화. 본문 텍스트, 수식, page tag, figure marker, splice anchor는 변경하지 않았다.
**Patches applied**:
...
| Certificate | Status | Basis (ver2) |
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A는 Learner Navigation Aid와 §1–§8 학습 본문만 포함하며, compiler/audit/process log는 PART B로 분리했다. ver2에서 §5 기억 고리 3건이 학습 본문 내부에 통합되어 standalone 학습용 완결성이 강화되었다. |
| Zero-Omission Certificate | PASS | Scope Lock의 C1–C4, PK41, R&T A/H anchors, Audit MUST/SHOULD corrections, 4C KEEP markers를 모두 반영했다. ver2에서 Audit Required Patch #7 (Holford 메시지)와 Step 1 Protocol §5 기억 고리 의무 항목을 보강했다. |
| Mastery-Uplift Certificate | PASS | 8개 이하의 bounded augmentation을 관련 concept 옆에 source-status label과 함께 삽입했다. ver2에서 Crucible Grade A4 (이중 V_ss sanity check)와 A5 (NCA-IIV inflation cascade)를 §8에 강화 통합했다. |
| Source-Boundary Certificate | PASS | 모든 augmentation은 TEXTBOOK_DERIVED/AUDIT_DERIVED/CRUCIBLE_DERIVED/EXPERT_INFERENCE로 표기했고, 새 수치·새 page tag·새 named example을 추가하지 않았다. ver2 patches는 모두 기존 page tags 범위 안에서 작동한다. |
| HTML-Readiness Certificate | PASS | PART B에 marker mapping, figure/page-tag/navigation/MathJax/self-test/rendering/guardrail 규칙을 포함했다. ver2 patches는 새 marker를 추가하지 않으며 기존 splice anchor를 변경하지 않는다. |
...
| **⚡ 기억 고리** | AUC tail은 $\lambda_z$의 1차 항 하나뿐이지만, AUMC tail은 $1/\lambda_z$와 $1/\lambda_z^2$ 두 항이 동시에 들어간다. **시간이 한 번 더 가중되는 순간 후방 꼬리가 지배적이 된다** — 그래서 같은 terminal slope 오류라도 MRT와 $V_{ss}$로 흘러갈 때 더 강하게 증폭된다. [G pp.144–145] | |
...
| **⚡ 기억 고리** | MRT는 *분자가 머문 시간의 평균*, $t_{1/2}$는 *농도가 절반이 되는 시간*이다. 1구획 monoexponential에서는 $MRT=1/\lambda_z$, $t_{1/2}=\ln(2)/\lambda_z$로 단순한 비율 관계($1/\ln 2\approx 1.443$)이지만, 다구획에서는 **MRT가 분포기 정보까지 흡수하는 반면 $t_{1/2}$는 terminal slope만 기술한다**. 이 때문에 effective half-life는 $\ln(2)\cdot MRT$로 다시 정의된다. [G p.151; T pp.789–793] | |
...
| **⚡ 기억 고리** | $V_{ss}$의 분모에는 $AUMC$와 input correction이 모두 들어오고, $V_z$의 분모에는 $AUC$와 $\lambda_z$만 들어온다. 따라서 **두 부피가 어긋나는 방향 자체가 어디서 가정이 흔들렸는지의 단서가 된다** — 표준 $V_{ss}$만 비정상적으로 부풀거나 음수에 가까워지면 input correction 영역의 문제, $V_z$만 부풀면 terminal slope 영역의 문제다. [G pp.151, 157] | |
...
### B9. Zero-Omission Coverage Matrix (ver2)
...
| **C3-v2 Audit Required Patch #7** | **Holford message — CL/CL/F as primary reporting parameter, AUC as exposure metric** | **C1 D Context notes (third bullet); §8 C Professional moat extended bullet; §8 D Mastery Note CRUCIBLE_DERIVED** | **PRESENT (ver2)** | **None** |
...
| **C7-v2 Crucible Grade A4** | **Dual V_ss formula sanity check elevated to non-crisis use** | **§8 C Professional moat — fifth bullet (ver2)** | **PRESENT (ver2)** | **None** |
| **C7-v2 Crucible Grade A5** | **NCA-IIV inflation cascade signature naming** | **§8 B failure mode #4 extended (ver2)** | **PRESENT (ver2)** | **None** |
| **C7-v2 Step 1 Protocol §5 mandate** | **Memory Hooks (⚡ 기억 고리) for each confusion pair encoding the structural reason for the difference** | **§5 Pair 1 / Pair 2 / Pair 3 — added as last row in each comparison table (ver2); all source-anchored, no "always" claim, no new page tags** | **PRESENT (ver2)** | **None** |
...
| **C9-v2 ver2 patch boundary** | **ver2 patches do not introduce new named examples, new numerical thresholds, new page tags, new figures, new Crucible/Step 1 raw prose; only source-anchored reformulations and previously-approved Crucible Grade A items** | **Patch Log at top + each ver2 patch labeled with [G p.XX] or [실무 추론] / [CRUCIBLE_DERIVED]** | **PRESENT (ver2)** | **None** |

### B10. Micro-Patch Log

No structural micro-patches needed in ver1 PART A composition. ver2 introduces six surgical patches (V2-P1 through V2-P6) listed in the Version & Patch Log at the top of this document. All ver2 additions are inline within existing §-blocks and existing tables, do not modify any equation, do not modify any figure marker, do not modify any splice anchor in the PATCH MODE Splice Verification Table, and do not introduce new source page tags beyond [G p.148] which already appears in PART A's C1 §2.8.5 region.

### B11. Mastery Augmentation Log (ver2 update)
...
**Memory Hooks (added in ver2 — *not* counted as Mastery Augmentation; these are Step 1 Protocol §5 mandated structural insights, source-anchored, integrated as table rows rather than label augmentations.)**

| MH-ID | Location | Type | Source basis | Risk |
|---|---|---|---|---|
| MH-1 | §5 Pair 1 last row | ⚡ 기억 고리 | [G pp.144–145, Eq.2:313/2:320] — first-moment tail's $1/\lambda_z + 1/\lambda_z^2$ structure | Low |
| MH-2 | §5 Pair 2 last row | ⚡ 기억 고리 | [G p.151; T pp.789–793] — effective half-life $\ln(2)\cdot MRT$, multi-compartment behavior | Low |
| MH-3 | §5 Pair 3 last row | ⚡ 기억 고리 | [G pp.151, 157] — formula divergence as diagnostic, no "always" claim | Low |
