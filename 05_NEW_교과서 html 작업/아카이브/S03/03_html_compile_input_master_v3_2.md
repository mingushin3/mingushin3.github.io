...
**이 자료 활용법:** §1을 한 번 통독하여 전체 골격을 잡은 뒤, §2의 Card 1–5를 순서대로 학습한다. §5로 자주 혼동하는 개념 쌍을 정리하고, §7로 능동 회상(active recall)을 훈련하며, §8에서 이 챕터가 이후 PopPK 흡수 모델링과 어떻게 연결되는지 확인한다.

**학습 경로:** `C(t) Master Equation → tmax/Cmax → F → V/F + flip-flop → ka,app vs ka,true → confusion pairs → self-test → meta-frame`.

**시작 전 확인 사항:** IV bolus 1구획 PK, CL, V, 반감기(half-life), 지수함수 감쇠(exponential decay), AUC에 대한 기본 이해가 선행되어야 한다.

**학습 후 달라지는 것:** 경구 단독 데이터에서 식별 가능한 것(`V/F`, `CL/F`, apparent `ka`, terminal slope ambiguity)과 추가 근거 없이는 식별할 수 없는 것을 스스로 구분할 수 있어야 한다.

**그림 저작권 안내:** 교과서 원그림은 재현하지 않는다. `FIGURE_POINTER` 블록은 HTML 컴파일러에게 텍스트 전용 교과서 참조 콜아웃을 생성하도록 지시하며, `FIGURE_SCHEMATIC`은 재작도(redraw) 개요이지 저작권 이미지 재현이 아니다.
...
## 0. 갱신된 큐레이션 맵(Updated Curation Map)
...
<!-- ANNOTATION -->
IV bolus는 투여 직후 전신 순환에 약물이 들어와 **초기 농도 C0**를 만든다. 반면 경구/혈관외 투여는 **초기 농도 0에서 흡수와 소실이 경쟁하며 만들어지는 차이지수 곡선(← 두 지수함수의 차이로 생긴 곡선)**이다. 따라서 이 곡선에서는 F와 V가 `V/F`로 묶이고, ka와 kel은 terminal slope 해석에서 서로 뒤바뀔 수 있다. 이 때문에 경구 단독 데이터는 “잘 맞는 모델”과 “진짜 생리학”을 자동으로 구분해주지 않는다. **그리고 같은 dose를 기준으로 비교하면, 흡수 단계가 끼어드는 순간 oral 곡선의 peak는 IV bolus 대비 낮아지고 시점도 뒤로 밀린다. 즉 흡수는 단지 “늦게 들어오는 것”이 아니라, peak의 높이와 시점을 동시에 재정의하는 단계다** [T Fig.6-3, p.172].
...
<!-- MASTER LENS -->
**거장의 시각:** 경구 1구획 모델의 핵심은 농도 곡선을 “두 지수 함수의 차이”로 보는 것이다. 상승부는 흡수 과정이 아직 남아 있다는 신호이고, 하강부는 반드시 소실(elimination)만의 신호가 아니다.

**체화 직관:** `ka − kel`은 단순한 분모가 아니라 두 지수 과정이 시간축에서 얼마나 분리되어 보이는지를 결정한다. `ka ≈ kel`이면 두 과정은 그래프상 거의 분리되지 않아 한계식으로 수렴한다. 반면 `ka < kel`이면 혈장에 들어온 약물은 빠르게 제거되지만, 관찰 terminal decline은 남아 있는 흡수 과정에 의해 느리게 보일 수 있다.

**A. 형식적 정의(Formal Definition)**

<!-- ANNOTATION -->
1구획 모델, 1차 흡수, 1차 소실, linear PK(← dose 비례성이 유지되는 PK)를 가정한 혈관외 투여 농도-시간 방정식(extravascular concentration-time equation)이다. 흡수 부위의 약물은 시간에 따라 1차로 줄어들고, 그중 F fraction이 전신 순환에 도달한다 [G p.30; T pp.171–172].
...
0차 흡수는 central elimination 구조는 그대로 두고 입력 함수(input function)만 바꾼 경우다. 흡수가 지속되는 동안에는 입력이 거의 일정하고, 입력이 끝난 뒤에는 washout만 남는다 [G Eq.2:66–2:67, p.38; T Fig.6-2, p.171]. PK3에서 zero-order model의 AIC가 first-order보다 좋았지만, 교재는 이것이 물리적으로 “진짜 zero-order absorption”을 증명하는 것은 아니라고 경고한다. 여러 dose, 반복투여, IV 비교가 있어야 기전(mechanism) 해석이 가능하다 [G pp.483–486].
...
> **Practice Brief — Card 1 [EXPERT_INFERENCE, v3]**  
> Master Equation 식을 보지 않고 종이에 적은 후, `ka < kel`인 경우와 `ka > kel`인 경우 차이지수 곡선의 모양이 어떻게 달라지는지 손으로 스케치하라. 두 경우의 terminal slope label을 각각 적고, 어느 경우에 flip-flop 위험이 발생하는지 표시하라. 마지막으로, 같은 dose·F·V 조건에서 곡선의 진폭(Cmax)이 두 경우 사이에 어떻게 다르게 나타나는지 한 줄로 설명하라.
...
F, dose, V는 linear system에서 곡선의 높이를 위아래로 scale한다. 그러나 `dC/dt = 0`은 “높이”가 아니라 “기울기가 0이 되는 시간 위치”를 찾는 조건이다. 따라서 곡선 전체가 위아래로 커지거나 작아져도 tmax의 시간 위치는 바뀌지 않는다. 그래서 linear 1구획 oral PK에서는 dose나 F가 바뀌면 Cmax와 AUC는 바뀌지만 tmax는 바뀌지 않는다 [T Fig.6-5, p.174]. **단, 이 “tmax 불변” 성질은 absorption 단계만 건드렸을 때의 이야기다. CL이 변하면 AUC뿐 아니라 tmax와 Cmax까지 함께 움직이고, V가 변하면 Cmax와 tmax가 서로 다른 방향으로 이동한다. 즉 “F·dose만 변했을 때”와 “disposition(CL/V)이 변했을 때”는 곡선 변화 패턴이 서로 다른 신호로 나타난다. 이것이 absorption 변화와 disposition 변화를 구분하는 첫 번째 단서다** [T Figs.6-10–6-11, pp.179–180].

**C. 잔차법(Method of Residuals)**

말기(terminal) 직선 구간에서 `kel`을 먼저 추정한다. 그 다음 역외삽(back-extrapolation) 직선에서 관찰 농도를 빼 잔차 직선(residual line)을 만들면, 이 잔차 기울기가 `−ka`가 된다. 두 직선의 교점은 lag-time 추정에 사용될 수 있다 [G pp.31–33; T Appendix F pp.781–783]. 단, 이 방법은 terminal slope가 정말 elimination을 반영한다는 전제를 둔다. 따라서 flip-flop 상황에서는 위험하다.
...
> **Practice Brief — Card 2 [EXPERT_INFERENCE, v3]**  
> 어떤 oral 약물에서 dose를 2배로 늘렸더니 Cmax가 약 1.7배, AUC가 약 1.7배, Tmax는 거의 동일하게 관찰되었다. 이 결과가 linear 1구획 oral PK 가정과 정합적인가? 만약 정합적이지 않다면, sub-proportional Cmax/AUC를 만들 수 있는 absorption-side 기전 한 가지와 disposition-side 기전 한 가지를 각각 제시하라. (힌트: dose-dependent first-pass, saturable absorption transporter, dose-dependent CL 등 §B의 "tmax 불변" 성질이 깨지는 조건을 떠올려라.)
...
<!-- ANNOTATION -->
동일 약물의 IV 기준 투여(reference)가 있을 때, 혈장 AUC 기반 생체이용률(bioavailability) F(← 전신 순환 도달 분율)는 다음과 같이 계산한다.
...
두 혈관외 제형(extravascular formulation) A/B를 비교할 때:
...
상대 생체이용률(Relative F)은 IV 기준 투여 없이도 두 제품의 전신 전달(systemic delivery) 차이를 비교할 수 있다 [T pp.183–185].

**C. 소변 데이터 해석 시 주의점(Urine Data Caveat)**

혈장 AUC만이 유일한 경로는 아니다. R&T는 요중 배설 데이터(urinary excretion data)로도 생체이용률, 상대 생체이용률, 신장 청소율(renal clearance)을 추정할 수 있음을 제시한다. 단, 이 해석은 미변화체 배설 분율(fraction excreted unchanged)과 신장 처리(renal handling)가 비교 조건에서 일정하다는 전제를 요구한다 [T Eq.6-18–6-21, pp.187–188].

**D. 기전적 F(Mechanistic F)**

<!-- ANNOTATION -->
생리학적으로 F는 장관 내강(gut lumen)에서의 흡수 분율(fraction absorbed), 장관 초회 통과(intestinal first-pass, ← 장 통과 중 대사/손실), 간 초회 통과(hepatic first-pass, ← 간 통과 중 대사/손실)가 곱해진 결과로 이해할 수 있다.
...
R&T는 생동성(bioequivalence)에서 시험/기준(test/reference) ratio의 90% CI와 0.80–1.25 기준을 설명한다 [T Fig.6-13, p.183]. 본 세션에서는 “Frel만 같아도 충분한가?”가 핵심 질문이다. Frel이 같아 AUC가 유사하더라도 ka가 다르면 Cmax와 tmax가 달라질 수 있다. 따라서 흡수의 정도(extent, 얼마나 많이)와 흡수의 속도(rate, 얼마나 빨리)는 항상 함께 읽어야 한다.
...
> **Practice Brief — Card 3 [EXPERT_INFERENCE, v3]**  
> 같은 약물의 i.m.과 oral solution을 동일 dose로 비교한 결과, AUC ratio (oral/i.m.)가 약 0.47이었다. 다음 두 질문에 답하라: **(1)** 이 47%를 oral의 absolute F로 단정할 수 있는가? 단정할 수 없다면 어떤 가정이 추가로 필요한가? **(2)** plasma 채혈이 어렵고 urine data만 사용해 F를 추정하려면 fraction excreted unchanged·renal handling에 대해 어떤 가정이 두 투여 경로 사이에서 유지되어야 하는가? (힌트: T pp.187–188의 caveat을 떠올려라.)
...
<!-- MASTER LENS -->
<!-- ANNOTATION -->
**거장의 시각 — Apex Big Idea (convolution lens) [EXPERT_INFERENCE, v3]:** 경구 concentration-time curve는 **input function (ka·lag·gastric emptying·multiple sites) ⊗ bioavailability F ⊗ disposition (CL·V)**의 합성곱(convolution)으로 형성된다. 다시 말해, 어느 한 축만으로는 곡선 형태를 완전히 설명할 수 없으며, 같은 곡선 모양이 서로 다른 (input, F, disposition) 조합으로 만들어질 수 있다. 이것이 V와 F가 분리되지 않고 `V/F`로 묶이는 이유, terminal slope label이 ka와 kel 사이에서 뒤집힐 수 있는 이유, 그리고 fitted ka가 실제 흡수가 아닐 수 있는 이유를 모두 한 번에 설명하는 상위 framing이다.

**거장의 시각:** 경구 단독 모델에서 좋은 OFV와 작은 RSE는 “모델이 데이터를 잘 설명한다”는 뜻이지, V와 F가 원리적으로 분리되었다는 뜻이 아니다. 수치적 식별가능성(numerical identifiability)과 구조적 식별가능성(structural identifiability, ← 원리적으로 분리 추정 가능한지)은 다르다.
...
**저자의 4-part 결론 (one-line mnemonic) [T p.185]:** 단일 oral dose만 가진 상태에서, **(i) AUC는 사다리꼴법으로 *계산*은 가능하지만, (ii) CL은 F를 모르므로 *계산 불가*이고, (iii) terminal half-life는 무엇이 rate-limiting인지(absorption인지 disposition인지) 모르면 elimination/absorption 어느 쪽에도 *confidently 할당 불가*이며, (iv) V는 *추정 불가*다.** 이 네 줄이 Apex의 핵심 메시지이며, “경구 단독에서 무엇이 식별 가능하고 무엇이 식별 불가능한가”를 한 번에 떠올리는 체크리스트다.

**B. Flip-flop 실용적 식별가능성(Practical Identifiability)**

일반적으로 `ka > kel`이면 경구 말기 기울기(oral terminal slope)는 소실(elimination)을 반영한다. 그러나 `ka < kel`이면 남아 있는 흡수 과정(absorption process)이 더 느린 과정이 되므로, 관찰되는 말기 기울기는 흡수를 반영할 수 있다. 이때 말기 반감기(terminal half-life)를 소실 반감기(elimination half-life)로 읽으면 소실을 잘못 해석하게 된다 [G Fig.2.17, p.30; T Case C, pp.175–176].
...
- **Fluticasone propionate:** median tmax는 90분이고 Cmax는 30–90분에 나타나지만, 흡수 반감기(absorption half-life)는 3.8–4시간으로 해석된다. early tmax가 빠른 흡수를 증명하지 않는다 [T Fig.6-7, p.177].
- **Theophylline:** 음식/물(food/water)은 흡수 동역학(absorption kinetics)을 바꾸지만 말기 반감기(terminal half-life)는 약 6.3 h로 유지되어 흡수 변화와 체내 분포·소실(disposition) 변화를 구분하는 예시가 된다 [T Fig.6-8, p.178].
- **Penicillin G:** 난용성(sparingly soluble) 근육주사 제형은 흡수가 속도 제한 단계(rate-limiting)가 될 수 있음을 보여준다 [T Fig.6-9, p.178].
- **PK2:** lag-time model은 `Ka = 0.043 min⁻¹`, `K = 0.0088 min⁻¹`, `V/F = 32 L`, `tlag = 16 min`을 제시한다 [G p.480]. 같은 사례에서 `F = 82%`와 `true V = 20 L` 문구는 `32×0.82≈26 L`과 산술상 충돌하므로, 본 handout에서는 “원문 수치 충돌 [확인 필요]”로 유지하고 계산값을 source-confirmed value로 확정하지 않는다 [G p.480].
- **PK2 Solution III:** 역전된 초기 추정치(reversed initial estimate)는 다른 `ka/kel/V/F` 해로 수렴할 수 있음을 보여준다. 단, OCR/unit 표기에는 `[확인 필요]` 플래그를 유지한다 [G pp.481–482].
- **PK3:** 1차 모델은 `Ka≈Ke`와 큰 CV/RSE 문제를 보이고, 0차 모델은 AIC가 낮다(85.2 vs 76.2). 그러나 더 좋은 적합(better fit)이 진짜 0차 흡수의 증명이 되는 것은 아니다 [G Table 3.1, p.486].

**C-2. Plausible Fallacy (그럴싸한 오류) [EXPERT_INFERENCE, v3]**

> ⚡ **Apex Concept 전용 — 그럴듯한 오해**  
>  
> **그럴듯한 오해:** "Cmax가 낮고 Tmax가 길어졌으면 흡수가 느려진 것이다."  
>  
> **왜 틀렸는가:** 경구 곡선의 형태는 F(생체이용률), ka(흡수 속도), lag time, 위배출 속도, CL(분포·제거)이 모두 동시에 기여한다 — 이것이 §A의 convolution lens 핵심이다. 흡수만 보면 위험한 이유는, **first-pass 증가**, **용해도 저하**, **flip-flop 동역학**이 모두 동일한 방향의 Cmax 저하·Tmax 지연을 만들기 때문이다. 즉 같은 곡선 변화가 적어도 세 가지 서로 다른 기전으로 설명될 수 있고, 그 중 어느 것이 실제 원인인지는 곡선 모양만으로는 분리되지 않는다.  
>  
> **실무에서 어떻게 드러나는가:** 제형 변경 생동성 시험(BE trial)에서 Cmax 저하를 "흡수 불량"으로 단정해 재조제할 때, 실제 원인이 first-pass 변화라면 IV 크로스오버 데이터 없이는 F와 ka를 분리할 수 없다. 따라서 "ka 감소 → 제형 재조제"라는 즉각 처방은 원인 진단을 건너뛴 결정이다. AUC 동시 변화·terminal slope·dose proportionality 신호를 함께 읽지 않으면, 잘못된 방향으로 개발 자원이 투입될 위험이 있다.

**D. NONMEM 보고 시 함의(Reporting Implication)**

경구 단독 PopPK에서는 F를 1로 고정(fix)하고 `V/F`, `CL/F`로 보고하는 것이 구조적으로 정직하다. `F1`을 추정(estimate)하거나 임의 값으로 고정한 뒤 V와 CL을 절대값(absolute value)처럼 보고하면, 사전 가정(prior assumption)이 추정된 파라미터처럼 보일 위험이 있다. `[교과서 외 해석 — NONMEM reporting practice]`
...
> **Practice Brief — Card 4 [EXPERT_INFERENCE, v3]**  
> oral-only 데이터로 적합된 모델에서 `V/F = 50 L`, `CL/F = 5 L/hr`, 두 파라미터의 RSE가 모두 < 10%로 보고되었다. 이 결과를 그대로 "V = 50 L, CL = 5 L/hr"로 임상보고서에 옮겨 적기 전에, 반드시 함께 명시해야 할 caveat **세 가지**를 작성하라. (힌트: §A의 4-part 결론 [T p.185], §B flip-flop 가능성, 그리고 numerical vs structural identifiability 구분에서 각각 한 줄씩 도출 가능하다.)
...
적합된 `ka`가 커졌다는 것은 곧바로 “흡수가 빨라졌다”를 뜻하지 않는다. “gut compartment에서 더 빨리 사라졌다”는 뜻일 수도 있다. AUC가 함께 변하지 않으면 흡수 개시·속도(onset/rate) 변화일 가능성이 크고, AUC도 함께 변한다면 흡수 정도(extent) 변화가 동반된 것이다 [G pp.40–41].

**C. 다중 흡수 부위(Multiple Absorption Sites)**

흡수 부위가 여러 개이면 농도-시간 곡선(concentration-time curve)은 다중 peak 또는 비정형 상승부를 보일 수 있다. G는 buccal + GI 같은 multiple absorption site 구조를 Eq.2:80–2:82로 제시한다 [G pp.46–47]. 이 경우 단일 ka는 여러 입력 과정(input process)을 하나로 눌러 담은 요약 파라미터(summary parameter)로 읽어야 한다.
...
> **Practice Brief — Card 5 [EXPERT_INFERENCE, v3]**  
> Food effect 시험에서 high-fat meal 후 oral 약물의 적합 ka가 절반으로 떨어졌다. 이를 즉시 "흡수 속도가 느려졌다"로 단정하기 전에 확인해야 할 보조 신호 **두 가지**를 제시하라. 첫째 신호는 §A의 `ka,app = ka,true + kd` 분해에서 도출되어야 하고, 둘째 신호는 Trench tip의 AUC ratio 비교에서 도출되어야 한다. 두 신호 중 어느 하나라도 흔들리면 ka 변화가 단순한 "흡수 속도" 변화가 아닐 가능성을 고려해야 한다.
...
Flip-flop 동역학에서는 `ka < kel`이므로 **흡수가 제거의 병목**이 된다 — terminal slope가 ka를 반영한다. Disposition-limited에서는 `kel < ka`이므로 **제거/분포가 병목**이다 — terminal slope가 kel을 반영한다. **IV 크로스오버 데이터 없이 경구 데이터만으로는 두 메커니즘을 구별할 수 없다는 것이 핵심 함정이다.** 즉 같은 terminal slope 숫자가 어느 병목에서 나온 것인지가 임상 결정(dosing interval, accumulation 예측)을 가른다.

**⚡ 기억 고리 (Memory Hook) — *정점 시각 vs 흡수 종료 시각*: [EXPERT_INFERENCE, v3]**  
Tmax는 흡수 속도와 제거 속도가 같아지는 **교차점**이지, 흡수가 완료되는 시점이 아니다. Tmax 이후에도 흡수는 계속되며 단지 제거가 더 빠를 뿐이다. 따라서 **Tmax 지연이 "흡수 완료 지연"이 아닐 수 있다** — 제거가 느려진 것만으로도 Tmax는 뒤로 밀린다. 이는 Card 2 §B에서 다룬 "F·dose만 변하면 tmax 불변, disposition이 변하면 tmax도 이동한다"는 진단 신호와 정확히 같은 맥락이다.
...
| 혼동 | 정리 |
|---|---|
| F가 증가하면 tmax도 빨라진다? | linear 조건에서 F는 곡선의 높이를 조정(scale)하지만 tmax는 ka와 kel의 상대 크기로 결정된다 [G p.35; T p.174]. |
| ka가 증가하면 AUC도 증가한다? | F와 CL이 일정하면 ka 변화는 곡선 형태/tmax/Cmax를 바꾸지만 총 AUC는 바꾸지 않는다 [T p.176]. |
| AUC가 같으면 제형 차이가 없다? | AUC가 같아도 Cmax/tmax가 다를 수 있으므로 속도(rate) 차이는 남는다 [T pp.181–183]. |

**⚡ 기억 고리 (Memory Hook) — *얼마나 들어오나 vs 얼마나 빨리 들어오나*: [EXPERT_INFERENCE, v3]**  
F는 투여량의 몇 분율이 systemic circulation에 도달하는지를 결정하는 **비율**(게이트의 열림 크기)이고, ka는 흡수 부위에서 얼마나 빠르게 이동하는지의 **속도**다. AUC는 F에 비례하고, Tmax와 Cmax의 형태는 ka/K(= ka/CL·V) 비율에 의해 결정된다. **F가 낮아도 ka가 빠르면 날카로운 peak를 만들고, F가 높아도 ka가 느리면 넓고 낮은 peak를 만든다.** 즉 peak의 모양만 보고 F를 추정하는 것은 두 축을 섞어 읽는 가장 흔한 오류다.

**⚡ 기억 고리 (Memory Hook) — *속도계 vs 적산계*: [EXPERT_INFERENCE, v3]**  
흡수 속도(ka)는 **속도계(순간 속도)**이고, 흡수 정도(F)는 **적산계(누적량)**다. 속도계는 피크의 높이와 위치를 결정하고, 적산계는 전체 면적(AUC)을 결정한다. **속도계를 올려도 적산계가 늘지 않는다 — ka를 증가시켜도 F가 그대로면 AUC는 변하지 않는다.** 따라서 food/formulation effect 보고에서 "AUC ratio"와 "Cmax ratio"를 분리해 보고하면 적산계와 속도계가 자동으로 구분된다.
...
| PK3 교훈 | 1차 모델은 ka·kel 비분해와 RSE 문제 | 0차 모델이 AIC상 우월했지만 기전 증명은 아님 [G pp.483–486] |

<!-- RECAP -->
**§5 Recap:** 경구 PK에서 가장 위험한 혼동은 기울기 명칭(slope label), 겉보기 파라미터(apparent parameter), 속도/정도(rate/extent)를 서로 바꿔 부르는 것이다. 이름표가 바뀌면 같은 숫자도 다른 임상 결론을 만든다.

---

## §7. 자기 테스트: 능동 회상 모듈(SELF-TEST)
...
`ka`는 흡수 부위 소실 속도(absorption-site loss rate), `F`는 전신 순환 도달 분율(systemic fraction), `Dpo`는 투여량, `V`는 분포용적(volume), `kel`은 소실 속도(elimination rate)이며, 괄호 안의 항은 소실 지수와 흡수 지수의 차이를 나타낸다 [G p.30; T p.782].
...
**Answer:** 아니다. Frel = 1.0은 AUC 기반으로 흡수 정도(extent)가 같다는 뜻이다. ka가 다르면 Cmax와 tmax가 달라질 수 있고, 발현(onset)이나 최고 농도 관련 안전성(peak-related safety)이 중요한 약물에서는 임상적 차이가 남을 수 있다 [T pp.181–183].
...
**Answer:** 안 된다. `ka < kel`이면 oral curve의 관찰 terminal slope는 더 작은 rate constant인 `ka`를 따를 수 있다. 이 경우 terminal half-life는 약 `ln2/0.05 = 13.9 h`로 보인다. `kel = 0.5 hr⁻¹`는 혈장에 들어온 뒤의 true elimination일 수 있지만, oral-only terminal slope만으로 확정할 수 없다.

<!-- SELF-TEST -->
### Q6. PK3에서 zero-order model의 AIC가 first-order보다 낮았다. “흡수가 진짜 0차”라고 결론 내릴 수 있는가?

**Answer:** 아니다. PK3는 zero-order input model이 해당 dataset을 더 잘 설명한다는 것을 보여준다. 그러나 진짜 기전(true mechanism)을 확정하려면 용량 범위(dose range), 반복 투여(repeated dosing), IV 데이터 등 추가 근거가 필요하다 [G pp.483–486].
...
## ⚡ 보스 딜레마 ★★ — Final Self-Test Capstone [EXPERT_INFERENCE, v3]

<!-- SELF-TEST -->
### Boss Dilemma. 제형 변경 후 Cmax↓·Tmax↑: 두 가설을 어떻게 방어하는가?

**시나리오:** 경구 데이터만 있는 Phase 1 데이터셋에서 Cmax가 낮고 Tmax가 지연된 결과가 관찰되었다. 제형 팀은 "흡수가 느려졌다(ka 감소)"라고 결론지었다. 모델러로서 두 가지 방어 가능한 해석을 제시하라:

- **(A) 흡수 속도 감소(ka 감소)로 해석**하고 absorption model을 first-order에서 transit compartment로 변경한다.
- **(B) first-pass 효율 감소(F 감소, apparent CL 증가 → AUC 감소)로 해석**하고 F의 공변량 탐색을 우선한다.

각 선택을 규제 문서에서 어떻게 방어하는가?

**거장의 Trade-off 논리 (Answer):**

(A)는 모델 복잡도를 높이고 Tmax 재현에 집중하지만, IV 데이터 없이는 F와 ka의 교락(confounding, ← 두 요인이 얽혀 분리되지 않는 상태)을 해결하지 못한다. 이는 Card 4 §A의 convolution lens가 경고하는 바로 그 한계다. 보고 시 "transit compartment가 Tmax 지연을 더 잘 설명하지만, F의 동시 변화 가능성은 본 데이터로는 배제할 수 없다"고 명시해야 한다.

(B)는 파라미터 수를 단순하게 유지하지만, 만약 ka 변화가 진짜 원인이었다면 AUC와 곡선 형태가 모두 어긋날 수 있다. 보고 시 "AUC ratio가 통계적으로 유의하게 감소했고 이는 F-driven 해석을 지지하지만, 동시에 ka 변화 가능성을 sensitivity analysis로 평가했다"고 기술해야 한다.

**규제 방어 핵심 — 분리 진단 알고리즘:**

> **Step 1.** "AUC가 같은가 다른가"를 먼저 확인하여 두 가설을 분리한다.  
> &nbsp;&nbsp;&nbsp;&nbsp;— AUC 변화 없음 + Cmax↓·Tmax↑ → 가설 (A) ka 감소가 우세  
> &nbsp;&nbsp;&nbsp;&nbsp;— AUC 감소 + Cmax↓·Tmax↑ → 가설 (B) F 감소 또는 (A+B) 복합 가능성  
> **Step 2.** IV 크로스오버 데이터를 요청하거나, 가능하지 않으면 **두 모델 모두를 적합시켜 sensitivity analysis로 보고서에 명시**한다.  
> **Step 3.** 단일 가설만 채택하지 않고 "두 해석 모두 데이터와 정합적이며, 임상 dosing 결정은 두 해 모두에서 안전한 영역에 있는가"를 평가한다.

→ **가장 위험한 자세는 "ka 감소 = 흡수 불량 = 제형 재조제"의 단순 추론이다.** 이는 Card 4 Plausible Fallacy(C-2)가 경고한 정확히 그 함정이며, BE trial의 잘못된 의사결정 사슬을 만들어낸다.
...
본 세션은 IV bolus 이후 첫 번째 임상 현실, 즉 **흡수 단계가 있는 계**를 다룬다. 이후 다회투여(multiple dosing), 방출 조절형(modified-release), 음식 효과(food effect), 생동성(BE), PopPK 흡수 모델로 나아가더라도, 판단 기준은 결국 다음 네 가지 질문으로 돌아온다.
...
| 실패 모드 | 실제 결과 |
|---|---|
| V/F를 V로 보고 | 공변량 효과(covariate effect)를 분포 변화로 오해. |
| CL/F를 CL로 보고 | F 변화가 clearance 변화처럼 보임. |
| 말기 기울기(terminal slope)를 무조건 kel로 해석 | flip-flop에서 반감기와 축적(accumulation) 판단 오류. |
| ka 변화를 흡수 정도(absorption extent) 변화로 단정 | 음식/제형 효과(food/formulation effect) 메시지 오류. |
| tmax 용량 의존성(dose-dependence)을 무시 | 포화성 흡수, 초회 통과 포화, 비선형 소실 신호를 놓침. |
| 0차 모델의 더 나은 적합을 기전 증명으로 단정 | 서방형/용해도/용출 근사를 생리학적 확정으로 과장. |

### C. 전문가 해석 지점(Professional Moat)

30년 차 pharmacometrics 연구자는 경구 모델 결과를 볼 때 먼저 “모델이 맞았는가?”가 아니라 “무엇이 식별되었는가?”를 묻는다. Phase 1 SAD(단회 상승 용량 시험) 회의에서 senior가 던지는 실무 질문은 단순하다: **“FIH(최초 임상 투여) 단계에서 IV arm 또는 absolute F 확인 가능성을 평가했는가?”** 이 질문 하나가 F, V/F, CL/F, flip-flop, ka,app의 모든 위험을 앞으로 끌어낸다. `[교과서 외 해석]`

### D. Ch.7 맥락 — 한 문장 요약

ka와 F의 변동성은 BCS, 위배출(gastric emptying), 용출(dissolution), 수송체(transporter), 장관/간 초회 통과(intestinal/hepatic first-pass) 같은 생리·물리화학적 요인에서 비롯된다. 본 세션의 수식은 그 복잡성을 1차 입력과 F라는 두 개의 겉보기(apparent) 축으로 압축해 읽는 첫 단계다 [T pp.197–220].
