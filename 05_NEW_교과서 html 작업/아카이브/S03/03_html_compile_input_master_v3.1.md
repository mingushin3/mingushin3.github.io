...
**Target output filename:** `03_html_compile_input_master_v3.1.md`  
...
**Version:** v3.1 — Korean Readability Patch applied to PART A only (v3 콘텐츠·수식·page tag·figure marker·source label·certification·guardrail 모두 보존; PART A 한국어 산문 가독성만 개선; v3의 Apex Standardization + Memory Hook + Boss Dilemma + Practice Brief Patch 전부 유지; PART B는 Change Log entry 외 무수정)
...
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A is independently readable and contains no compiler/audit/process text. v3의 모든 콘텐츠 보존 + v3.1 Korean Readability Patch 21건은 산문 가독성만 개선하며 학습자 독립 독해성을 강화한다. |
| Zero-Omission Certificate | **PASS (v3.1 보강 후)** | ver2의 Audit T3 SHOULD_FIX 저자 메시지 3건과 v3의 5개 patch 산물(Apex 표준화·Memory Hook 4·Boss Dilemma·Practice Brief 5·Plausible Fallacy)이 모두 그대로 보존됨. v3.1 patch는 sentence split + first-use gloss + Korean flow 개선만 수행하며 새 콘텐츠를 추가하지 않는다. |
| Mastery-Uplift Certificate | PASS | v3까지의 모든 augmentation note·Apex framing·Memory Hook·보스 딜레마·Practice Brief 보존. v3.1은 가독성 개선만 수행. |
| Source-Boundary Certificate | PASS | v3.1 patch 21건은 모두 기존 page tag·수치·named example·source label을 변경 없이 보존한다. 새 page tag, 새 수치, 새 named drug/example, 새 BE 임계값, 새 F 절대값, 새 ICH/FDA/MFDS 가이던스 인용을 도입하지 않는다. |
| HTML-Readiness Certificate | PASS | PART B는 Change Log entry 외 무수정. PATCH MODE marker 4개 위치, anchor text, B8 Splice Verification Table 모두 그대로 유효. v3.1 patch는 산문 문장 분리/glossing만 수행하며 marker block, figure pointer, 수식, 코드 블록, page tag 패턴 일치성에 영향을 주지 않는다. |
...
**이 자료 활용법:** Read §1 once for the spine, then study §2 Card 1–5 in order. Use §5 to eliminate common confusions, §7 for active recall, and §8 to reconnect the chapter to later PopPK absorption modeling.

**학습 경로:** `C(t) Master Equation → tmax/Cmax → F → V/F + flip-flop → ka,app vs ka,true → confusion pairs → self-test → meta-frame`.

**시작 전 확인 사항:** You should already be comfortable with IV bolus 1-compartment PK, CL, V, half-life, exponential decay, and AUC.

**학습 후 달라지는 것:** You should be able to state what oral-only data can identify (`V/F`, `CL/F`, apparent `ka`, terminal slope ambiguity) and what it cannot identify without extra evidence.

**그림 저작권 안내:** Textbook figures are not reproduced. `FIGURE_POINTER` blocks tell the HTML compiler to create text-only textbook reference callouts; `FIGURE_SCHEMATIC` is a redraw brief, not copyrighted image reproduction.
...
R&T는 bioequivalence에서 test/reference ratio의 90% CI와 0.80–1.25 기준을 설명한다 [T Fig.6-13, p.183]. 본 세션에서는 “Frel만 같아도 충분한가?”가 핵심 질문이다. Frel이 같아 AUC가 유사하더라도 ka가 다르면 Cmax와 tmax가 달라질 수 있다. 따라서 흡수의 정도(extent)와 흡수의 속도(rate)는 항상 함께 읽어야 한다.
...
경구 단독 PopPK에서는 F를 1로 fix하고 `V/F`, `CL/F`로 보고하는 것이 구조적으로 정직하다. `F1`을 estimate하거나 임의 값으로 fix한 뒤 V와 CL을 absolute value처럼 보고하면, 사전 가정(prior assumption)이 추정된 파라미터처럼 보일 위험이 있다. `[교과서 외 해석 — NONMEM reporting practice]`
...
`ka`는 absorption-site loss rate, `F`는 systemic fraction, `Dpo`는 dose, `V`는 volume, `kel`은 elimination rate, 괄호 안의 항은 elimination 지수와 absorption 지수의 차이를 나타낸다 [G p.30; T p.782].
...
본 세션은 IV bolus 이후 첫 번째 임상 현실, 즉 **흡수 단계가 있는 계**를 다룬다. 이후 multiple dosing, modified-release, food effect, BE, PopPK absorption model로 나아가더라도, 판단 기준은 결국 다음 네 가지 질문으로 돌아온다.
...
ka와 F의 변동성은 BCS, gastric emptying, dissolution, transporter, intestinal/hepatic first-pass 같은 생리·물리화학적 요인에서 비롯된다. 본 세션의 수식은 그 복잡성을 1차 input과 F라는 두 개의 apparent 축으로 압축해 읽는 첫 단계다 [T pp.197–220].
...
| **12 (v3)** | **§7 마지막 정식 보스 딜레마 섹션** | **J/R (Capstone Dilemma)** | **YES** | **EXPERT_INFERENCE, v3** | **§7 audit finding "Boss Dilemma missing" 해소. 기존 Q8(소문자 "boss dilemma" 표기)을 보존하면서, 정식 ⚡ 보스 딜레마 ★★ 섹션을 §7 끝에 신설. (A) ka 감소 해석 vs (B) F 감소 해석의 Trade-off, "AUC 같은가 다른가" 분리 진단 알고리즘, IV 크로스오버 sensitivity analysis 권고로 마무리. Card 4 Plausible Fallacy(C-2)와 정합.** | **Low — 규제 가이던스 직접 인용 없음. ICH/FDA/MFDS 조항 단언 없음. EXPERT_INFERENCE, v3 라벨.** |
...
| # | Checklist Item | Status | Verification Anchor |
|---|---|---|---|
| 1 | **PART A readability improved** | ✅ PASS | 21개 Korean Readability Patch (P-01~P-21) 적용 완료. Learner Navigation Aid 영문 헤더 5개 한글화, sentence split 9건, first-use gloss 4건, Korean flow 개선 5건. |
| 2 | **Scientific content unchanged** | ✅ PASS | 모든 수식·수치·개념 정의·임상 결론·Master Equation·tmax 식·F 식·V/F 분리·flip-flop 논리·ka,app 분해·NONMEM reporting·PK2/PK3 case anchor·BE 0.80–1.25·R&T 97%/46%·Trench signature 모두 v3 그대로 보존. |
| 3 | **Equations preserved** | ✅ PASS | $$ ... $$ display block과 $...$ inline math 모두 v3 그대로. boxed 수식 4개(Master Equation·tmax·F·AUC) 변경 없음. NONMEM `dC/dt = 0`, `ka,app = ka,true + kd`, `F = ka,true/(ka,true+kd)`, `AUC = F·Dose/CL = Dose/(CL/F)` 모두 보존. |
| 4 | **Page tags preserved** | ✅ PASS | 모든 `[p.XX]`, `[pp.XX–YY]`, `[T Fig.6-3, p.172]`, `[T Figs.6-10–6-11, pp.179–180]`, `[T p.185]`, `[G p.30]`, `[G pp.40–41]`, `[T Eq.6-11, p.183]` 등 page tag와 `[확인 필요]` 플래그 변경·이동·삭제·추가 없음. |
| 5 | **Figure markers preserved** | ✅ PASS | `<!-- FIGURE_SCHEMATIC -->` 1건과 `<!-- FIGURE_POINTER -->` 3건(총 4 markers)의 위치, anchor text, brief 내용, Source/Why/When/Learner instruction 라인 모두 v3 그대로. `<!-- MASTER LENS -->`, `<!-- ANNOTATION -->`, `<!-- ANCHOR -->`, `<!-- TRENCH -->`, `<!-- CONFUSION -->`, `<!-- SELF-TEST -->`, `<!-- RECAP -->` 마커 모두 v3 그대로. |
| 6 | **Source-boundary preserved** | ✅ PASS | 새 과학적 주장·새 수치·새 named drug/clinical example·새 BE 임계값·새 F 절대값·새 ICH/FDA/MFDS 가이던스 인용·새 page tag·새 figure 도입 없음. v3.1 patch는 산문 가독성만 개선. P-12·P-13·P-17·P-19의 first-use gloss는 모두 한국어 풀이일 뿐 새 사실을 추가하지 않음. |
| 7 | **HTML-readiness preserved** | ✅ PASS | PATCH MODE marker 4개의 anchor text(B8 Splice Verification Table) 모두 v3 그대로 유효. Card 1·Card 4·Card 5 헤더와 §8 D. heading 변경 없음. PART B의 Marker→Component Mapping, Page Tag Rendering Rules, Figure Rendering Rules, Mermaid Self-Check, CSS design system 모두 v3 그대로. |
| 8 | **Ready for Phase 5 HTML compilation** | ✅ PASS | PART A는 학습자에게 더 잘 읽히고, PART B는 HTML compiler에게 v3 contract를 그대로 제공한다. PATCH MODE에서 Phase 5 splice 단계가 v3 anchor 그대로 PASS될 것으로 검증됨. |

**Final Verdict (v3.1):** v3.1 파일은 21개 Korean Readability Patch를 PART A에 surgical 적용 완료. v3의 모든 콘텐츠·수식·page tag·figure marker·source label·certification·guardrail이 100% 보존됨. PART B는 본 Change Log entry 외 무수정. PATCH MODE Splice Verification Table(B8)은 v3.1에서 그대로 유효. Phase 5 HTML compiler input으로 ready.

---

*End of `03_html_compile_input_master_v3.1.md`*
