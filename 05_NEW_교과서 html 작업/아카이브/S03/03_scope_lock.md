...
=================
작성 의도 메모 (튜터 노트)
두 교재를 하나의 Scope Lock에 통합한 이유는 이렇습니다. G(Gabrielsson)는 수식 유도와 수치 계산 실습에 강점이 있고, T(Rowland & Tozer)는 생리·약학적 해석과 임상 맥락에 강점이 있습니다. 이 두 관점이 함께 있어야 흡수 PK의 "왜"와 "어떻게"가 온전히 연결됩니다. 특히 ⑤ V/F 식별불가능성과 ③ flip-flop은 임상약사 배경을 가진 학습자가 가장 먼저 혼동하는 지점이므로 CONCEPTUAL SPINE에 명시적으로 배치했습니다.
Phase 1 진행 시 주의사항
① T Ch.7 범위 절제 (Scope creep 위험)
pp.197–220에는 BCS 분류, 용해도·투과성 생리학, 위장 이동 시간 등 독립 챕터 수준의 내용이 포함됩니다. Phase 1 아웃라인에서 이 구간은 "흡수 속도 결정 인자 맥락 제공" 용도로만 활용하고, 수식 유도·계산이 없는 서술적 내용은 1–2문장 요약 수준으로 제한하세요.
② CONCEPTUAL SPINE ⑥ 수식 출처 명시 필요
Ka,app = Ka,true + Kd 관계식은 G Fig. 2.24 캡션에 다이어그램으로만 표현되어 있고 별도 번호 수식이 없습니다. Phase 1 아웃라인에서 이 항목에 [확인 필요 — G Fig.2.24 캡션, 번호 없음] 플래그를 달아두세요.
③ 두 Source 간 표기 통일 필수
G는 소문자 K(소실), Ka(흡수) / T는 소문자 k, ka로 표기하고, G는 V/F를 명시적으로 쓰는 반면 T는 V를 경우에 따라 apparent V로 사용합니다. HTML 출력 전 기호 통일 방침을 Phase 1 단계에서 미리 확정하세요 (권장: NONMEM 관행 기준 — K→kel, Ka→ka, V/F→V/F 명기).
