...
# 02_HTML Compile Input Master — v3.1
...
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A contains navigation aid plus §1-§8 learner body only; audit/compiler/process material is isolated in PART B. ver2의 §8 E·F augmentation 및 v3의 M3 Plausible Fallacy / §5 Memory Hooks 4종 / M1~M6 Practice Brief 6종은 모두 learner-facing이며 verbatim 가독성 유지. v3.1 Korean Readability Patch는 동일 learner body의 산문 표현만 다듬으며 구조·라벨·marker를 변경하지 않음. |
...
2. §2를 순서대로 학습하라. M1은 약물이 막을 통과할 수 있는지, M2는 분포 속도를 결정하는 요인, M3은 V의 수학적 의미, M4·M5는 plasma 및 tissue 결합, M6은 어떤 exposure metric이 정당화될 수 있는지를 다룬다.
...
5. §8로 닫아라. §8 D는 6개 카드가 하나의 시스템으로 작동하는 두 가지 통합 시나리오를, §8 E는 세 가지 진단 시그니처(Albumin Covariate Asymmetry / PK47 Single-Site Misfit / Vss vs Vβ Confusion)를, §8 F는 새 PK 문제에 이 자료를 적용하는 4단계 Mastery Sequence를 제시한다.

### 학습 경로

M1 membrane permeability → M2 distribution rate limitation → M3 apparent V structure → M4 plasma protein binding and concentration-dependent fu → M5 tissue binding/Kp/transporter → M6 total vs unbound exposure → §5 Critical Blow → §7 self-test → §8 A–C professional synthesis → §8 D system integration scenarios → §8 E diagnostic signatures → §8 F mastery sequence.

### 학습 시작 전 체크리스트

- Total concentration `C`와 unbound concentration `Cu`를 구분할 수 있다.
- Apparent volume은 실제 장기 부피가 아님을 안다.
- FIGURE_POINTER 블록은 삽입 이미지가 아니라 교과서를 직접 찾아보라는 안내임을 인지한다.

### 학습 완료 후 체크리스트

- 수동 막 확산의 driving force가 total `C`가 아닌 `Cu`인 이유를 설명할 수 있다.
- 분포 extent와 distribution rate를 독립적인 차원으로 분리하여 논할 수 있다.
- `Vp + tissue term`, `Kp`, `fu/fuT`가 동등한 렌즈임을 가정이 만족될 때만 사용할 수 있다.
- `AUCu = AUC × fu` 단축식이 안전한 경우와 안전하지 않은 경우를 진술할 수 있다.
- Unsupported regulatory claim을 도입하지 않고 total vs unbound exposure metric 선택을 방어할 수 있다.
- §8 E의 세 가지 진단 시그니처를 명명하고 각각의 GOF 패턴을 설명할 수 있다.
- §8 F의 4단계 Mastery Sequence를 처음 보는 PK 사례에 적용할 수 있다.
...
<!-- MASTER LENS -->
**핵심 직관**: 막투과는 “lipophilic이면 통과한다”로 끝나지 않는다. 실제 permeability는 **size × lipophilicity × charge × transporter**가 함께 만든다. Eq.4-1의 $P$는 이 네 축과 막 구조가 하나의 숫자로 압축된 거시 투과도(macroscale permeability)이다. [R&T pp.80–84, 85–90]
...
> - **흐름상 역할:** M2의 perfusion vs permeability 율속 판단으로 가는 진입점이며, M5의 transporter-driven distribution 해석에 필요한 4축(size·logP·ionization·transporter)을 정의한다.
> - **체화 꿀팁:** 새 약물을 마주할 때마다 "size · logP · ionization · transporter" 네 축을 한 줄로 적어보고, 어느 축이 가장 약한 결정인자인지 짚는 습관을 들여라.
> - **실무 활용:** BBB 침투 가능성 1차 추정, P-gp/BCRP 기질 여부 평가, oral bioavailability 추정의 출발점, NONMEM dataset에서 absorption phase 모델 선택의 근거.
...
Perfusion-rate limited distribution<!-- ANNOTATION --> (← 혈류 공급이 속도 제한)에서는 membrane crossing이 충분히 빠르다. 이때 조직으로 들어가는 공급 속도는 blood flow가 결정하며, 조직 농도가 평형값에 접근하는 경로는 단일지수(mono-exponential) 형태를 따른다.
...
> - **왜 알아야 하는가:** 동일한 Vd라도 평형 도달 속도가 다르면 onset, peak time, sampling 설계가 완전히 달라지기 때문에, 모델 구조 결정의 분기점이 된다.
> - **흐름상 역할:** M1의 막투과 입력을 받아 M3의 V 해석 시점(분포 평형 후)을 정의하며, multi-compartment 모델 vs 단일구획 모델 결정의 근거가 된다.
> - **체화 꿀팁:** 새 약물의 첫 concentration-time plot에서 distribution phase의 굴곡 시점을 기록한 뒤 $Q/V_T·K_{pb}$로 역산해 보고, 모델 가정과 일치하는지 점검하라.
> - **실무 활용:** PBPK tissue 평형 시간 추정, Phase 1 PK sampling schedule 설계, thiopental·penicillin G 같이 mixed limitation을 보이는 약물의 tissue concentration 해석.
...
#### D-2. Plausible Fallacy — Vd as Physical Volume

> **⚡ Plausible Fallacy — [EXPERT_INFERENCE, v3]**
> - **그럴듯한 오해**: "Vd가 크다는 것은 체내 분포 공간이 실제로 크다는 뜻이다."
> - **왜 틀렸는가**: Vd는 체적이 아니라 plasma concentration 관측값으로 역산한 비례상수다. 조직 결합이 강하거나 plasma 결합이 약하면 실제 체적의 수십~수백 배도 가능하다. fu와 fuT의 비율이 이 겉보기 크기를 결정한다.
> - **실무에서 어떻게 드러나는가**: Vd가 500 L인 약물의 loading dose를 "체중×4 L/kg"로 설계하다가 실제 분포 공간 개념으로 혼동하면, 로딩 용량 계산이 생리학적 의미를 잃는다. Bayesian TDM에서 total C가 정상 범위인데 unbound C가 달라진 경우를 생각해 보라. 실제 원인은 fu 변화인데도 Vd 변화로 오해하는 전형적 실수가 여기에서 비롯된다.
...
> - **왜 알아야 하는가:** Vd는 loading dose 계산과 covariate 해석의 중심 파라미터인데, 그 본질이 apparent proportionality constant임을 놓치면 후속 모든 추론이 흔들린다.
> - **흐름상 역할:** M4·M5의 binding 정보(fu, fuT)와 M6의 exposure metric을 V라는 단일 수학적 연결 고리(mathematical bridge) 위에 통합하는 세션의 중심 카드다.
> - **체화 꿀팁:** 새 약물의 V 값을 보면 즉시 plasma 3 L · ECW 16 L · TBW 42 L · albumin 7.5 L 중 어느 anchor에 가까운지 분류하라 — 이 분류만으로 binding regime이 절반 이상 보인다.
> - **실무 활용:** Loading dose 설계, hypoalbuminemia/투석 환자에서 V 변화 해석, NONMEM에서 V 위에 covariate spec할 때의 방향(positive/negative) 판단, Bayesian TDM에서 V 이상값의 진단.
...
<!-- MASTER LENS -->
**핵심 직관**: $f_u$는 고정 상수가 아니다. 혈장 단백 농도(protein concentration), 결합 친화도(binding affinity), 결합 부위 수, 유리 농도(unbound concentration) 등 여러 인자의 함수이기 때문이다. 따라서 “total concentration ↔ unbound concentration” 변환은 $f_u$가 일정하다는 검증 위에서만 안전하다. [R&T pp.105–108; G&W pp.690–693]
...
- G&W는 `fub > 1`이 가능하다고 명시한다. 이는 bound/free ratio이므로, fraction bound와 혼동하지 않도록 주의한다. [G&W p.168]
...
> - **왜 알아야 하는가:** fu는 total과 unbound 농도를 잇는 변환계수이며, 시간·농도·dose에 따라 변하면 모든 단축식($AUCu = AUC \times f_u$, $C_u = C \times f_u$)이 무너지기 때문이다.
> - **흐름상 역할:** M3의 V 구조와 M6의 exposure metric을 연결하는 핵심 매개 파라미터로, "conversion shortcut의 안전 조건"을 정의한다.
> - **체화 꿀팁:** dose-normalized AUC가 dose에 따라 변할 때, saturable CL을 의심하기 전에 dose별 ex vivo fu를 먼저 확인하라 — 진단 순서가 뒤집히면 원인 추적이 어긋난다.
> - **실무 활용:** 저알부민혈증·요독증·α1-AGP 변화 환자에서 TDM 해석, drug-drug displacement interaction 평가, NDA 단계에서 unbound concentration 측정 여부 결정, PopPK에서 protein covariate(albumin/α1-AGP) spec의 근거 제시.
...
> **▶ Practice Brief — M5 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가:** 큰 Vd의 원인이 plasma fu 증가인지 tissue fuT 감소인지 구분하지 못하면, protein binding 변화의 임상 결과 방향이 정반대로 도출된다.
> - **흐름상 역할:** M3의 tissue term($V_{TW}\cdot f_u/f_{uT}$)을 binding(fuT)과 transporter(P_uptake/P_efflux)로 분해해 M2의 율속 단계와 다시 연결하는 단계다.
> - **체화 꿀팁:** Basic drug에서 큰 Vd를 보면 acidic phospholipid binding을 1순위 가설로, transporter substrate 가능성을 2순위로 점검하라 — Eq.4-29의 두 항을 같은 종류의 원인으로 합치지 않는 것이 핵심이다.
> - **실무 활용:** PBPK Kp 예측 모델 선택, BBB 약물의 tissue concentration 해석, propranolol·amiodarone 같은 large-Vd basic drug의 covariate modeling, indinavir의 CSF/plasma 격차 해석.
...
<!-- MASTER LENS -->
**핵심 직관**: exposure metric은 “하나의 AUC 숫자”가 아니다. 즉, exposure metric은 어떤 concentration species가 어떤 시간 패턴으로 약리 효과를 유발하는가에 대한 인과적 선언이다. Total concentration이 protein binding 차이에 confounded되면 unbound concentration이 active comparator가 된다. [G&W pp.163–169]
...
> **▶ Practice Brief — M6 체화 4축 — [EXPERT_INFERENCE, v3]**
> - **왜 알아야 하는가:** Exposure metric 선택은 단순한 보고 형식이 아니라 효과의 인과 메커니즘에 대한 주장이며, 잘못 고르면 species 비교와 dose 결정이 동시에 왜곡된다.
> - **흐름상 역할:** M1~M5의 모든 분포 정보를 "어느 species의 어떤 시간 패턴이 효과를 만드는가"라는 단일 의사결정으로 수렴시키는 세션의 종착점이다.
> - **체화 꿀팁:** AUC, Cmax, td 중 무엇을 전면에 둘지는 mechanism이 결정한다 — 효과가 peak-driven이면 Cmax, 누적-driven이면 AUC, threshold-driven이면 td. 이 매핑을 먼저 한 줄로 적은 후 species/scale을 결정하라.
> - **실무 활용:** IND/NDA 단계 PK/PD 보고서의 exposure metric 정당화, species extrapolation을 위한 unbound concentration 사용 결정, 치료역이 좁은(narrow therapeutic index) 약물의 안전 마진 분석, regulatory 문서에서 "왜 AUC가 아니라 AUCu인가"를 방어하는 한 단락 작성.
...
Total concentration C는 사진(관찰 가능한 대리 지표)이고, unbound concentration Cu는 수용체에 실제로 작용하는 행위자다. 같은 C에서 fu가 다르면 Cu가 다르고, 같은 Cu에서 fu가 다르면 C가 다르다. 좌표계를 잘못 선택하면 "농도가 정상인데 효과가 없다" 또는 "농도가 낮은데 독성이 있다"는 역설이 나온다.
...
Distribution extent(Vd의 크기)는 약이 최종적으로 얼마나 넓게 퍼지는지를, distribution rate(Q, 조직 혈류와 막 투과성)는 그 최종 상태에 얼마나 빨리 도달하는지를 결정한다. 빠른 distribution rate + 작은 Vd는 짧은 시간 안에 좁은 구획에 가두는 패턴이고, 느린 distribution rate + 큰 Vd는 오래 걸려 넓게 퍼지는 패턴이다. 이 두 차원은 독립적으로 작동하므로, Vd 하나만으로는 분포 과정의 시간 경과를 논할 수 없다.
...
Vss는 분포와 제거가 완전히 독립인 **정적 균형 부피** — 제거 속도가 Vss에 영향을 주지 않는다. Vβ는 terminal elimination slope에서 역산한 **겉보기 부피**로, 조직에서 계속 약이 돌아오는 재분포 흐름을 반영하기 때문에 제거가 느릴수록 Vss보다 커진다. 따라서 Vβ를 Vss 대신 쓰면 steady-state 분포 부피를 과대추정하고 loading dose가 잘못 계산된다. (이 memory hook은 fu/fuT 분기를 다룰 때 함께 점검할 사항을 가리킨다: multi-compartment 모델에서 V를 Vss로 쓰는가, Vβ로 쓰는가를 명시적으로 확인하라.)
...
AUC는 총 화물량(결합 + 유리), AUCu = fu × AUC는 실제로 수용체에 도달 가능한 활성 화물량이다. 알부민 결합이 강한 약물에서 저알부민혈증이 생기면, AUC는 줄어도 AUCu는 오히려 늘 수 있다. 바로 이 때문에 exposure-response 분석을 AUC로 수행하면 효과 방향이 역전되는 결과가 나온다. (Small-Vd vs Large-Vd 분기를 판단할 때, 실제로 의사결정에 쓰이는 metric이 AUC인지 AUCu인지를 함께 묻지 않으면 PPB 변화의 임상 결과 방향을 오독한다.)
...
**Answer**: dose별 ex vivo(체외 측정) $f_u$를 확인한다. $f_u$가 dose/Cu에 따라 증가하면 saturable PPB가 원인일 수 있고, $f_u$가 일정하면 clearance nonlinearity를 우선 의심한다. 이 문항은 교과서 수식의 실무 적용이며, 특정 PDF case는 아니다.
...
<!-- RECAP -->
**분포는 “약물이 어디에 있는가”가 아니다. Plasma와 tissue 사이에서 binding, permeability, flow가 함께 형성하는 equilibrium 문제이자 시간 경과(time-course) 문제다.** Total concentration은 관찰값이고, unbound concentration은 많은 경우 causal species에 더 가깝다. $V$는 그 둘을 amount와 연결하는 apparent constant이다.

### D. System Integration Points

- **임상약리 요약**: dose-exposure linearity를 논할 때, M1(흡수/투과), M3(V/용량 의존성), M4(PPB 포화 여부), M6(exposure metric 선택) 네 카드가 하나의 문단 안에서 논리적으로 연결된다.
...
>
> **Scenario 1 — 임상약리 요약(dose-exposure linearity 단락)**  
> 한 단락이 6개 카드를 동시에 호출한다: 약물이 막을 통과해 plasma에 들어오는가(M1) → 같은 dose에서 V가 dose에 따라 변하는가(M3) → fu가 dose에 따라 변하는가(M4) → tissue distribution이 saturable transporter에 묶여 있는가(M5) → 어떤 species의 어떤 metric으로 linearity를 판정하는가(M6). 이 단락이 자연스럽게 흘러야 6개 카드가 단일 시스템으로 작동한다.
>
> **Scenario 2 — ICU 환자 TDM 의사결정**  
> 저알부민혈증(albumin↓) + acute renal failure(uremia, fu binding 경쟁) + α1-AGP 상승(트라우마 반응)이 동시 발생하는 한 환자의 한 dose 결정을 상상하라. M4(fu가 albumin·displacer·α1-AGP의 다중 함수) → M3(small-Vd 약물에서 V가 fu에 비례 증가) → M6(total concentration "정상 범위"가 toxic Cu를 가릴 수 있음)을 한 결정에 묶어야 한다. 이 시퀀스가 카드 단위로 분리되어 있으면 실무 통합이 끊긴다.

### E. Practical Diagnostic Signatures

> **Mastery Note — CRUCIBLE_DERIVED**  
> 본 세션 개념의 misuse는 PopPK 진단 플롯에 **반복 가능한 시그니처 패턴**을 남긴다. 이름을 부여하면 진단 속도가 카드 단위 → 시그니처 단위로 압축된다. 아래 시그니처는 모두 [교과서 외 실무 해석]이며, 출처 PDF의 직접 인용이 아니다.

**Signature 1 — Albumin Covariate Asymmetry Pattern**  
*Mechanism*: large-Vd 약물에서 V ~ albumin만 covariate spec하고 CL ~ albumin을 누락.  
*GOF 시그니처*: ETA-V vs albumin scatter는 weak positive correlation에 머물지만, ETA-CL vs albumin scatter는 강한 negative correlation을 보인다. simulation에서 albumin 극단군의 prediction interval이 systematic하게 어긋난다.  
*한 줄 진단*: ETA-CL vs albumin이 ETA-V vs albumin보다 강한 상관을 보이면 CL covariate를 함께 spec하라.

**Signature 2 — PK47 Single-Site Misfit Pattern**  
*Mechanism*: 실제로 multiple binding-site class를 가진 약물에 single-site PK47 모델(G&W Eq.47:1)을 fit.  
*GOF 시그니처*: low-Cu 영역에서 fu가 systematically underestimated, high-Cu 영역에서 overestimated. residual plot에서 V-shape 또는 inverted-V 패턴. low/high protein concentration 동시 fit 시 두 데이터셋의 residual 방향이 반대.  
*한 줄 진단*: fu vs Cu residual plot이 V-shape이면 두 번째 binding-site class를 추가하라.

**Signature 3 — Vss vs Vβ Confusion Pattern**  
*Mechanism*: 2-compartment 이상 모델에서 NONMEM output의 V를 Vβ(elimination phase apparent V)로 무비판 사용해 loading dose 계산.  
*GOF 시그니처*: distribution phase에서는 IPRED가 정확하나 elimination phase tail에서 systematic underprediction. CWRES가 시간에 따라 monotonic drift.  
*한 줄 진단*: elimination phase tail의 CWRES drift가 보이면 Vss와 Vβ를 분리하라 — loading dose는 Vss, terminal half-life 해석은 Vβ.

### F. Mastery Sequence

> **Mastery Note — EXPERT_INFERENCE**  
> 새 PK 문제를 만났을 때 본 세션의 6개 카드를 가장 빠르게 작동시키는 4-step 시퀀스다. 본 시퀀스는 source PDF의 직접 권고가 아니라, 본 handout의 내적 통합 지침이다. 카드 학습이 끝난 후, 새 사례에 이 시퀀스를 반복 적용하면 분리된 카드가 단일 진단 시퀀스로 통합된다.

1. **Species 결정 — 지금 보고 있는 농도는 무엇인가?**  
   Total `C`인가, unbound `Cu`인가, blood `Cb`인가? 단위와 측정 matrix를 먼저 고정한다. (M1·M4·M6의 진입점)

2. **Vd regime 결정 — 어느 모델이 작동하는가?**  
   Small-Vd albumin-bound regime(App.C / Eq.C-13, V≲15 L)인가, large-Vd tissue-bound regime(Eq.4-25 또는 4-29)인가? 이 분기에 따라 fu 변화의 임상적 결과 방향이 정반대가 된다. (M3·M5의 분기점)

3. **fu 안정성 점검 — 단축식이 유효한가?**  
   지금 비교하려는 dose·time·concentration 범위에서 fu가 일정한가? 일정하지 않다면 `AUCu = AUC × fu` 단축식과 `Cu = C × fu` 변환은 무효가 될 수 있다. (M4 boundary check)

4. **Metric 정당화 — 어느 species의 어느 시간 패턴인가?**  
   Decision의 causal mechanism이 Cmax-driven인가 AUC-driven인가 td-driven인가? 그리고 그 mechanism이 total과 unbound 중 어느 species에 작동하는가? (M6 결정 단계)

> 이 4-step은 §5 Critical Blow Practice Lens(causal species → assay scale → conversion validity → decision risk)와 isomorphic이며, 카드의 "왜"와 "어떻게"를 한 진단 시퀀스로 융합한다.
