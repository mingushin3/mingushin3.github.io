# 02_Content Lock v2.1 — 분포의 생물학: 막투과·분포용적·단백결합

> **Phase**: 4C Content Lock v2.1  
...
> **Output rule**: Markdown text only. Figure markers inserted; no figure code generated.  
> **Figure mode**: FULL MODE — body text preserved, Phase 4C markers inserted in reading order.

---

## Pass 1 — Edit Log Summary

| Location | Edit type | Before (excerpt) | After (excerpt) |
|---|---|---|---|
| §1/M1 | R1/R2 | dense definitions | split into definition → consequence. |
| M2/M3 | R1/R5 | rate/extent and V/fu packed together | separated amount, rate, and binding logic. |
| M4–M6 | R1/R3 | binding/exposure claims compressed | clarified when conversion or metric choice is safe. |
| §5–§8 | R2/R3 | decision logic implicit | made causal-species and downstream-error logic explicit. |

**Step B**: Polished v1.5 was generated as the intermediate layer; the final non-duplicated deliverable below is Content Lock v2.

## Pass 2 — Annotation Decision Record

| Type | Location | Proposed annotation | Risk | Verdict |
|---|---|---|---|---|
| A | §1 Big Idea | apparent proportionality constant gloss | Low | Annotate |
| A | M1/M2/M5 first-use terms | unbound, perfusion-rate limited, $K_p$ glosses | Low | Annotate |
| B | M3 binding equations | bridge from $K_p$ to $f_u/f_{uT}$ | Low | Annotate |
| B | M6 exposure choice | bridge from species to AUC choice | Medium | Annotate |
| C | M3 albumin ferry | extra analogy | High | Do not annotate |

**Final recommendation**: Low-risk annotations and one medium-risk M6 bridge were inserted with `<!-- ANNOTATION -->`.

---

## Updated Curation Map

본 세션의 중심 질문은 하나다. **약물은 혈장 밖으로 얼마나 나가며, 그 양은 무엇이 결정하는가?** 답은 한 카드에서 끝나지 않는다. 막투과 속도(M1)가 분포 율속(M2)을 정하고, 그 결과가 apparent volume의 수학 구조(M3), plasma binding(M4), tissue binding/Kp(M5), total vs unbound exposure 선택(M6)으로 이어질 때 전체 그림이 보인다.
...
- Audit T6 Figure Inventory를 Phase 4C에서 재판정했고, retained figure markers를 reading order로 삽입했다.
