...
# 05_HTML Compile Input Master — v3.1

## Phase 4D Certification (v3.1)

| Certificate | Status | Basis |
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A starts at learner-facing §1, excludes audit/compiler/process tables, and contains the complete spliced study handout. v3: §2 카드별 Practice Brief 추가로 능동 회상 closure 강화. |
...
<!-- TRENCH -->
**Trench tip:** 잔차법은 production NONMEM 알고리즘이 아니다. 손으로 curve-stripping을 하는 이유는 simultaneous nonlinear regression에 입력할 initial estimate의 감각을 기르기 위해서다. [실무 확장]
...
**Aspirin warning:** R&T의 aspirin 650 mg IV bolus 예시는 early sampling을 놓치면 terminal-only one-compartment 해석으로 $CL=0.98$ L/min을 얻게 되지만, biexponential 해석에서는 $CL=683$ mL/min이다. 즉 terminal-only one-compartment 해석은 CL을 약 44% 과대추정하며, 이를 바탕으로 maintenance dose를 계산하면 과대추정 위험이 생긴다. [R&T pp.615–620]

**Boundary conditions:** linear kinetics, 중앙구획(central compartment) 내 빠른 혼합(mixing), distribution phase 관측, mammillary 배열(mammillary structure; 중앙구획이 허브인 방사형 다구획 구조), central elimination 가정이 필요하다. 이 조건이 깨지면 bi-exponential fit은 가능해도 physiological interpretation은 흔들린다. [G&W pp.57–65; R&T pp.617–619]
...
**Reporting caveat:** 보고서에서는 “중앙구획에서만 제거(elimination from central compartment only)”와 “말초구획 단순 통합(peripheral compartment lumping)” 가정을 명시해야 한다. tissue PK, large protein drugs, target-mediated elimination이 의심되는 경우에는 이 가정에 대한 기전적 근거(mechanistic justification)를 함께 제시해야 한다. [R&T p.618–619; 실무 추론]
...
**RSE caveat:** condition number는 parameter 간 정보 직교성을, RSE<!-- ANNOTATION -->(← relative standard error; 각 추정치의 상대표준오차, %로 표현)는 각 parameter 자체의 추정 신뢰성을 본다. PK8 Table 8.1에는 RSE가 없으므로 그 표만으로 production 결정을 끝내지 않는다. [실무 확장]

**PK7 model discrimination bridge:** G&W PK7은 mono/bi/tri-exponential discrimination에서 AIC/F-test, precision, correlations, residuals, simplest adequate model rule을 함께 보게 한다. 특히 **weighting scheme이 다른 두 모델을 단순 F-test로 비교해서는 안 된다.** 두 모델의 likelihood가 서로 다른 measurement-error assumption 위에 서 있기 때문이다. 이를 무시하면 model discrimination 결론 자체가 무효가 된다. NDA submission의 모델 정당화 논리는 이 단순 사실 위에 서 있다. [G&W pp.505–512]
...
**Answer key:** terminal half-life인지 effective/plasma half-life인지, $f_2$가 얼마인지, 그리고 축적이 plasma 구획인지 서서히 평형에 도달하는 조직(slowly equilibrating tissue)인지 물어야 한다. Gentamicin-like case에서는 terminal term accumulation과 plasma therapeutic fluctuation을 구분해야 한다. [R&T pp.635–641]
...
당신은 어느 모델을 deployable로 채택하는가? 그리고 그 결정을 reviewer에게 어떻게 방어하는가?

**Answer key (수석 모델러의 Trade-off 논리):**

**채택: Model B (ODE physiological).** 그러나 정답은 모델 선택 자체보다 **방어 논리의 구조**에 있다.

1. **WRSS는 fit residual의 크기, condition number는 추정 geometry의 안정성**이다. 두 축은 서로 대체할 수 없다. 단일 축으로 결정할 수 없는 상황은 NDA에서 흔하다.
2. **Production model의 첫째 조건은 reproducibility** — 다른 시드, 다른 초기치, 다른 covariate 추가에서도 동일한 결론이 재현되는가. condition number 3,186은 covariance step instability와 covariate inclusion 시 결과 비예측성을 시사한다.
3. **Reviewer 방어 논리** — "Takada는 lowest WRSS이지만 (a) covariance geometry가 100배 이상 불안정하고 (b) time-dependent volume 표현은 covariate-volume 관계 해석에 추가 해석 부담을 만든다. ODE physiological 모델은 fit이 거의 동등하면서 (a) precision/correlation이 안정적이고 (b) clearance/volume 좌표계로 일반적 PopPK 보고 관행에 부합한다." [실무 추론: regulatory defense]
4. **만약 fit 차이가 임상적으로 유의미한 수준 (예: AUC 차이 >10%)이면** — 두 모델 모두를 sensitivity analysis로 보고하고 final dosing recommendation은 두 모델 간 robust한 영역에서 도출한다. PK8 자체는 그 정도 fit 차이를 보여주지 않는다.
5. **함정:** "WRSS가 더 낮으니 Takada가 진짜 모델에 가깝다"는 추론은 **부적절한 weighting scheme 비교**일 수도 있다 (Q6 + PK7 bridge 참조). 무가중치 RSS와 weighted RSS는 직접 비교 대상이 아니다. [G&W pp.508–517; R&T 일반 PopPK practice; 실무 추론]

**다음 깊이 질문:** Model B를 채택했는데 reviewer가 "그렇다면 Takada가 보여준 lower WRSS는 어떤 정보를 담고 있나"라고 묻는다면? — "lower WRSS는 time-dependent $V$ 가정이 fit residual을 약간 감소시킨다는 신호이지, 그 가정이 mechanistically validated되었다는 의미는 아니다. mechanistic validation 없이는 production model로 채택할 수 없다"가 표준 답변이다. [실무 추론]
...
5. CWRES<!-- ANNOTATION -->(← conditional weighted residuals; FOCE 기반 가중잔차) vs TIME이 early positive 또는 S-shaped pattern이면 structural misspecification 또는 missing distribution phase를 의심한다.  
...
*v3.1 = v3 + Korean Readability Patch (21 Low-risk surgical edits to PART A prose only).*
*No PART B changes other than appended v3 → v3.1 Changelog and v3.1 Final Checklist.*
