...
# 05_HTML Compile Input Master — v3.2

## Phase 4D Certification (v3.2)

| Certificate | Status | Basis |
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A starts at learner-facing §1, excludes audit/compiler/process tables, and contains the complete spliced study handout. v3: §2 카드별 Practice Brief 추가로 능동 회상 closure 강화. v3.2: 영어 서브 헤딩·테이블 헤더·일반 영어 표현을 한국어 또는 한글(English) 병기로 전환하여 한국어 학습자 가독성 대폭 개선. |
| Zero-Omission Certificate | PASS | Scope, Audit, Crucible, Content Lock v2, PDF anchors, and Phase 4C marker coverage were checked; no unresolved high-impact omission remains. v3: §5 Memory Hook 4쌍 보충으로 confusion-pair audit 항목 완전 흡수. PK7/PK8 condition number anchors (29.69, 125.2, 2243, 2306, 3186) 모두 그대로 보존. |
| Mastery-Uplift Certificate | PASS (강화) | ver1: Five bounded expert-synthesis augmentations adjacent to M1–M5. ver2: M3 [⚡ Apex Concept] 명시 배지, §7 Q9 Socratic Dilemma, §8C 7번째 Professional moat 항목, §8B Pseudo-Stable V 시그니처. v3: M3 Plausible Fallacy 정식 V5.0 구조화, §5 Memory Hook 4쌍, §2 카드별 Practice Brief 5개, §5 Critical Blow 중복 해소(V5.0 1개 제한 준수). |
| Source-Boundary Certificate | PASS | All augmentations are explicitly labeled EXPERT_INFERENCE/source-bound synthesis and add no new page tags, numbers, drugs, or external claims. v3 신규 추가도 동일 — Memory Hook 4쌍은 본문 기존 수치/관계만 사용, Plausible Fallacy v3는 M3 본문 V_β=CL/β 관계만 활용, Practice Brief는 본문 carry-forward 형식. |
| HTML-Readiness Certificate | PASS | PART B includes compilation contract, marker mapping, page-tag rules, figure rules, navigation-anchor integrity rules, and guardrails for Phase 5. v3: FIGURE marker 7개 그대로 유지. Memory Hook → CONFUSION 마커 내 별도 행으로 렌더, Practice Brief → 신규 PRACTICE_BRIEF 마커 추가(B4 매핑 갱신). |

## v3 → v3.1 Korean Readability Patch Changelog

| # | Location | Type | Change | Source-Boundary | Rationale |
|---|---|---|---|---|---|
| V31-01 | §1 Big Idea (MASTER LENS) | Sentence split + Korean flow improvement | 긴 명사 나열 문장을 두 문장으로 분리. 의미·수치·page tag 변경 없음. | 과학적 의미 동일. 새 사실 없음. [Korean Readability Patch v3.1]. | 한국어 학습자 독해 부담 경감. PART B contract·marker·page tag 영향 없음. |
| V31-02 | §1 Workflow anchor (ANCHOR) | Korean flow improvement | "좌표계는 ... 결정한다" → "좌표계 선택에 따라 ... 결정된다"로 주어-서술어 관계 자연화. | 동일. [v3.1]. | 한국어 자연스러움 개선. |
| V31-03 | §2 Card M1 Big Idea | Korean flow improvement | 산문 내 `+`/`/` 연산자 제거, "재분포(redistribution)" 표준 용어 사용. | 동일 의미. [v3.1]. | 한국어 산문에서 이질적인 기호 제거. |
| V31-04 | §2 Card M1 Trench tip | Korean flow improvement | "에 넣을 ... 얻기 위해서다" → "에 입력할 ... 기르기 위해서다"로 자연화. | 동일. [v3.1]. | 동사 자연화. |
| V31-05 | §2 Card M1 Structural necessity `[수학적 해석]` | Concept clarification + Korean flow | "임의로 움직이는 4개 숫자가 아니라 ... 제약을 반영한다" → "독립적으로 움직이는 임의의 숫자가 아니라 ... 제약을 받는 값들이다"로 명료화. | 동일. [v3.1]. | "제약을 반영하다"의 추상성 해소. |
| V31-06 | §2 Card M1 Aspirin warning | Korean flow improvement | "overestimate하고 ... 과대평가 위험이 있다" → "과대추정하며 ... 과대추정 위험이 생긴다"로 한국어화 + 용어 일관화. | 동일. [v3.1]. | 영-한 혼용 표현 정리, 용어 통일. |
| V31-07 | §2 Card M1 Boundary conditions | Korean flow + First-use gloss | "central에서의 빠른 mixing" → "중앙구획(central compartment) 내 빠른 혼합(mixing)"; mammillary structure에 첫 등장 주석 추가. | 동일. [v3.1]. | 학습자 친화적 첫-등장 설명. |
| V31-08 | §2 Card M1 MASTER AUGMENTATION | Tone smoothing | "elimination label로 부르는 순간" → "'elimination phase'라 명명하는 순간"으로 어색한 합성 표현 정리. | 동일. [v3.1]. | 한국어 어휘 자연화. |
| V31-09 | §2 Card M2 Big Idea | Korean flow improvement | "clearance/volume language" → "clearance와 volume의 언어"로 슬래시 제거. | 동일. [v3.1]. | 한국어 산문 슬래시 제거. |
| V31-10 | §2 Card M2 Reporting caveat (첫 문장) | First-use gloss | 'elimination from central compartment only' / 'peripheral compartment lumping' 가정에 한국어 주석 병기. | 동일. [v3.1]. | 학습자 첫-등장 이해 보조. |
| V31-11 | §2 Card M2 Reporting caveat (둘째 문장) | First-use gloss + Korean flow | "mechanistic justification이 필요하다" → "기전적 근거(mechanistic justification)를 함께 제시해야 한다". | 동일. [v3.1]. | 한국어 자연화 + 첫-등장 설명. |
| V31-12 | §2 Card M3 Plausible Fallacy `왜 틀렸는가` | Sentence split | em-dash 대신 마침표로 분리하여 두 문장으로 만듦. | 동일. [v3.1]. | 긴 복합문 가독성 개선. |
| V31-13 | §2 Card M4 PK7 model discrimination bridge | Sentence split | em-dash 분리, 단문 3개로 재구성. | 동일. [v3.1]. | 긴 인과 사슬 문장 가독성 개선. |
| V31-14 | §2 Card M5 Big Idea | First-use gloss | "slow tissue" 첫 등장에 "(서서히 평형에 도달하는 조직)" 주석 추가. | 동일. [v3.1]. | 학습자 친화적 첫-등장 설명. |
| V31-15 | §2 Card M5 Trench tip Lab rule | Concept clarification | 모호한 지시어 "둘이 다르면" → "이 구분이 달라지면"으로 명확화. | 동일. [v3.1]. | 지시어 모호성 제거. |
| V31-16 | §5 Pair 1 Memory Hook MH-01 첫 문장 | Korean flow improvement | 서술어 없는 첫 절 보완 ("단계, ... 단계다" → "단계이고, ... 단계다"). | 동일. [v3.1]. | 한국어 통사 자연화. |
| V31-17 | §5 Pair 2 Critical Blow 행 | Sentence split + Korean flow | 긴 한 문장을 두 문장으로 분리. "그 결과" 연결어 추가. | 동일. [v3.1]. | NDA Deficiency 시나리오 가독성 개선. |
| V31-18 | §5 Pair 3 Memory Hook MH-03 첫 문장 | Korean flow improvement | 서술어 없는 첫 절 보완 ("순환량(고속도로 통행량), ... 순 배출량(출구)이다" → "순환량(고속도로 통행량)이고, ... 순 배출량(출구)이다"). | 동일. [v3.1]. | 한국어 통사 자연화. |
| V31-19 | §7 Q8 Answer key | First-use gloss + Korean flow | "축적이 plasma인지 slowly equilibrating tissue인지" → "축적이 plasma 구획인지 서서히 평형에 도달하는 조직(slowly equilibrating tissue)인지". | 동일. [v3.1]. | 한국어 보조 + 첫-등장 설명. |
| V31-20 | §8 B Failure modes — Pseudo-Stable V Signature 셀 | Korean flow improvement | 단편(fragment) → 완결 문장으로 보완. | 동일. [v3.1]. | 단편 문장 완결성 회복. |
| V31-21 | §8 C Professional Moat 항목 7 | Korean flow improvement | "못 보면 ... 결정 못 한다" → "인식하지 못하면 ... 결정할 수 없다"로 정확한 동사화. | 동일. [v3.1]. | 한국어 어휘 정밀도 개선. |

### v3 → v3.1 영향 분석

**텍스트 길이:** v3 대비 약 +0.5% 증가 (gloss 첨부 + 문장 분리에 따른 미세 증가). 누적(ver1→v3.1) 증가율 약 +7.0%로 8% 한계 내.

**Marker 수:** FIGURE markers 7개·ANNOTATION 11개·SELF-TEST 9개·PRACTICE_BRIEF 5개·§5 Memory Hook 4개 — **변동 없음**.

**Source-Boundary 검증:** 새 page tag 0개·새 약물/수치 0개·새 reference 0개·새 figure marker 0개. PK7/PK8 condition number anchors (29.69 / 125.2 / 2243 / 2306 / 3186) 5개 모두 보존. Aspirin direction (terminal-only overestimates CL) 보존. Gentamicin volumes (V₁=14, Vss=56, Vβ=345) 보존. Nicardipine 12 hr / 1 hr to 50% Css 보존.

**HTML-readiness:** PART B compilation contract / marker mapping / page tag rules / figure rules / navigation rules 모두 변경 없음. v3.1 patch는 모두 PART A 산문 한정.
...
| # | Location | Type | Change | Source-Boundary | Rationale |
|---|---|---|---|---|---|
| V3-01 | §2 Card M3 `<!-- CONFUSION -->` Plausible Fallacy | Plausible Fallacy 정식 V5.0 구조화 | 기존 v2의 간단한 "Plausible Fallacy + Correction" 2줄 형식을 V5.0 표준 3-블록 구조로 확장: (a) 그럴듯한 오해, (b) 왜 틀렸는가 (β=f(CL,Q,V₁,V₂) 복합함수 + V_β=CL/β 재분포 흐름), (c) 실무에서 어떻게 드러나는가 (loading dose 과대추정 메커니즘). | M3 본문의 V_β=CL/β 관계와 V_β/V_ss/V_1 ordering만 사용. 새 약물·새 수치 없음. [EXPERT_INFERENCE, v3]. | V5.0 Step 1 Spec "Apex Concept 1개에만 적용" 요구사항 정식 충족. β slope의 합성 메커니즘과 loading dose 임상 결과 연결을 명시화. |
| V3-02 | §5 Pair 1 (Bi-exponential vs two elimination processes) | Memory Hook 추가 | "이사 vs 청소" 구조 메모리 훅 신설 — α phase = distribution(이사), β phase = elimination(청소) 근사 레이블이지 기계적 분리 아님. | M1/M5 본문의 phase 해석 메시지 동일. 새 사실 없음. [EXPERT_INFERENCE, v3]. | V5.0 §5 Memory Hook 의무 항목 충족. 두 phase 동시 진행이라는 미묘한 메커니즘을 비유로 인코딩. |
| V3-03 | §5 Pair 2 (V₁/Vss/V_β 3중 체계) | Memory Hook 추가 + 명칭 확장 | 기존 "Vss vs V_β" → "V₁ / Vss / V_β" 3중 비교로 명칭 확장. "현재 vs 균형 vs 제거 그림자" 구조 메모리 훅 추가 — V₁ < Vss ≤ V_β는 2구획 시스템의 수학적 필연. | M3 본문의 3중 volume table 동일. gentamicin 수치 (V₁=14, Vss=56, V_β=345 L) 그대로. [EXPERT_INFERENCE, v3]. | M3 Apex Concept과 직접 대응되는 confusion pair에 V5.0 Memory Hook 정식 인코딩. Loading dose / AUC 정규화 / terminal half-life 의존 부피 분리 명시. |
| V3-04 | §5 Pair 3 (Macro/Micro/Physiological) | Memory Hook 추가 | "내부 고속도로 vs 나가는 출구" 구조 메모리 훅 — Q (intercompartmental CL, 내부 순환) vs CL (systemic, 외부 배출) 구분. Q >> CL이면 1구획 근사 가능성. | M2 본문의 좌표계 변환 공식과 동일 (CL=k₁₀·V_c, Q=k₁₂·V_c=k₂₁·V_t). 새 수치 없음. [EXPERT_INFERENCE, v3]. | Macro/Micro/Physiological 3중 좌표계 구분에서 가장 자주 혼동되는 핵심 파라미터 쌍(Q vs CL)을 메모리 훅으로 정착. |
| V3-05 | §5 Pair 4 (Terminal vs Effective half-life) | Memory Hook 추가 + Critical Blow 강등 | "마지막 페이지 vs 전체 이야기" 구조 메모리 훅 추가. 동시에 "Critical Blow" 레이블을 "치명적 구분 (Critical distinction)"으로 강등 (V5.0은 1개 Critical Blow만 허용; Pair 2가 우선). | M5 nicardipine/gentamicin 본문 동일. terminal half-life 12 hr / 87 hr 수치 그대로. [EXPERT_INFERENCE, v3]. | V5.0 "각 쌍마다 ⚡ 기억 고리 한 줄" 의무 충족. Critical Blow 중복 해소(V5.0 §5 위반 항목 1건 차단). |
| V3-06 | §5 전체 | Critical Blow 중복 해소 | Pair 2 (V₁/Vss/V_β)에만 `**치명적 타격**` 행 유지. Pair 4 (terminal vs effective t½)의 Critical Blow → "치명적 구분 (Critical distinction)"으로 레이블 변경. | scientific content 변경 없음. nicardipine 12 hr / gentamicin 87 hr / loading dose 과대 위험 메시지 그대로. | V5.0 Step 1 Spec "최대 1개 Critical Blow" 절대 제약 준수. 임상·규제 파급력이 더 큰 V_β/Vss 혼동을 우선 보전. |
| V3-07 | §2 Card M1–M5 | Practice Brief 5개 추가 | 각 카드 말미(Recap 직후, MASTER AUGMENTATION 직전)에 `<!-- PRACTICE_BRIEF -->` 블록 신설. 카드별 핵심 능동 회상 과제 1–2개를 30–60초 분량으로 압축. | 본문 기존 수치/예시/공식만 사용. M1=잔차법 외삽 절차, M2=좌표계 변환 점검, M3=volume 선택 결정, M4=condition number 해석, M5=half-life 분류 질문. [EXPERT_INFERENCE, v3]. | V5.0 active recall 강화 항목. PhD 학생이 카드 1개를 다 읽은 직후 30초 안에 핵심을 능동 재현하도록 유도. §7 Self-Test와 별개로 카드 단위 micro-practice 제공. |
| V3-08 | PART B B4 (Marker mapping) | 신규 마커 1종 등록 | `PRACTICE_BRIEF` 마커 → "Active recall mini-card" 컴포넌트 매핑 추가. 스타일: green left border + 미니 prompt-answer 토글. | PART B 기존 매핑 변경 없음. 추가만. | Phase 5 HTML compiler가 v3 신규 Practice Brief 블록을 일관된 스타일로 렌더하도록 contract 갱신. |
| V3-09 | PART B B11 (Mastery Augmentation Log) | AUG-10~13 신규 기록 | AUG-10: M3 Plausible Fallacy v3, AUG-11: §5 Memory Hook 4쌍, AUG-12: §5 Critical Blow 중복 해소, AUG-13: §2 Practice Brief 5개. | 모두 [EXPERT_INFERENCE, v3] 태그로 visibly separated. | Source-boundary 추적성 유지. |

### v3 영향 분석

**텍스트 길이:** ver2 대비 약 +4% 증가 (Memory Hook 4쌍 + Plausible Fallacy 확장 + Practice Brief 5개). 누적(ver1→v3) 증가율 약 +6.5%로 8% 한계 내. 학습자 부하: §5 Memory Hook은 최대 1문장 비유, Practice Brief는 30–60초 단위 micro-task로 부하 미미.

**Marker 수:**
- FIGURE markers: 7개 (변동 없음 — PK7/PK8 anchor 보전 절대 제약)
- ANNOTATION markers: 11개 (변동 없음)
- SELF-TEST 블록: 9개 (변동 없음)
- PRACTICE_BRIEF 블록: 5개 (v3 신규)
- §5 CONFUSION 블록 내 `⚡ 기억 고리` 행: 4개 (v3 신규)

**Source-Boundary 검증:**
- 새 page tag 부착: 0개
- 새 약물/수치/임상 데이터 인용: 0개
- 새 EXPERT_INFERENCE 블록: Memory Hook 4쌍 + Plausible Fallacy v3 1개 + Practice Brief 5개 = 10개, 모두 [EXPERT_INFERENCE, v3] 태그로 분리
- ABSOLUTE constraint 위반: 0건
- **PK7/PK8 condition number anchor 보존 검증:** 29.69 / 125.2 / 2243 / 2306 / 3186 — 5개 수치 모두 §2 Card M4 본문에 그대로 유지. weighting scheme 경고 메시지(F-test 비교 금지) 그대로 유지.

**Splice Verification (v3):** PATCH MODE marker insertion은 ver2와 동일 — 7개 figure marker 모두 같은 anchor 위치 유지. v3 추가 항목(Memory Hook, Plausible Fallacy v3, Practice Brief)은 모두 PART A의 인라인 보강이며 splice anchor와 무관.

**V5.0 Spec 준수 검증:**
- §5 Critical Blow 개수: 1개 (Pair 2만) — V5.0 "최대 1개" 통과
- §2 Apex Concept 개수: 1개 (M3) — V5.0 "1개만" 통과
- §2 Plausible Fallacy 개수: 1개 (M3 Apex만) — V5.0 "Apex 1개에만 적용" 통과
- §5 Memory Hook 적용: 4쌍 (Pair 1, 2, 3, 4) — V5.0 "각 쌍마다 ⚡ 기억 고리" 4쌍 충족; Pair 5(WRSS vs better model)는 방법론적 확인 항목으로 메모리 훅 적용 대상 외
...
<!-- MASTER LENS -->
**Big Idea:** 2구획 모델은 “곡선이 두 번 꺾인다”는 관찰을 $A,\alpha,B,\beta$라는 macro-constant<!-- ANNOTATION -->(← 곡선의 절편·기울기 상수)로 분해한다. 그다음 이를 $CL,Q,V_1,V_2$ 또는 $V_c,CL,Cld,V_t$처럼 해석 가능한 좌표계로 다시 옮긴다. 이 좌표계 선택은 단순한 표기 문제가 아니다. clearance 추정 방식, volume의 임상적 해석, covariance 안정성, 그리고 multiple-dosing simulation 결과까지 모두 이 선택에 따라 달라진다. [G&W pp.59–71; R&T pp.615–641]
...
<!-- ANCHOR -->
**실무 흐름 기준점(Workflow Anchor):** PopPK 공변량 모델 구축의 전진 선택(forward inclusion) 단계에서는 다섯 카드가 동시에 켜진다. 먼저 $f_2$는 신기능 변화가 어느 phase를 바꿀지 예측한다(M1/M5). 다음으로 좌표계 선택에 따라 covariate를 $CL$에 붙일지 $\beta$에 붙일지가 결정된다(M2). $V_{ss}$와 $V_\beta$ 구분은 체중·tissue amount 해석을 좌우하고(M3), condition number/RSE는 그 covariate model을 실제로 유지할 수 있는지 판단한다(M4). [실무 통합]
...
<!-- MASTER LENS -->
**개념 Big Idea:** IV bolus 뒤 plasma curve가 semilog plot<!-- ANNOTATION -->(← 농도축만 로그인 그래프)에서 하나의 직선이 아니라 두 직선의 합으로 보인다면, 이것은 단순한 시각적 꺾임이 아니다. early phase는 주로 distribution과 일부 elimination이 혼합된 결과로, terminal phase는 느린 조직 재분포(redistribution)와 elimination의 합성 결과로 해석해야 한다. 2구획 분석은 이 curve를 “두 phase의 합”으로 분해하는 데서 시작한다. [G&W pp.59–60; R&T pp.615–616]

**정식 정의(Formal Definition):**
...
**G&W Fig.2.43 기준점(Anchor):** $A\approx70$, $B=28$, $\alpha=\ln(70/10)/130=0.0150\ \text{min}^{-1}$, $\beta=\ln(28/10)/450=0.00229\ \text{min}^{-1}$로 제시된다. 원본 그림/캡션의 단위 표기가 혼재되어 보이므로 최종 시각화 단계에서는 `[단위 확인 필요]`를 유지한다. [G&W p.60]
...
<!-- TRENCH -->
**Trench tip:** 잔차법은 실용(production) NONMEM 알고리즘이 아니다. 손으로 곡선 박리(curve-stripping)를 하는 이유는 동시 비선형 회귀(simultaneous nonlinear regression)에 입력할 초기 추정치(initial estimate)의 감각을 기르기 위해서다. [실무 확장]

**구조적 필연(Structural Necessity):** 2구획 central/peripheral amount ODE는 다음과 같다.

$$\frac{dA_1}{dt}=-(k_{12}+k_{10})A_1+k_{21}A_2$$

$$\frac{dA_2}{dt}=k_{12}A_1-k_{21}A_2$$

[수학적 해석] 선형 상수계수 2×2 ODE의 해가 두 지수항의 합으로 나타나기 때문에, $\alpha$와 $\beta$는 이 system matrix에서 유래한다. 따라서 $\alpha+\beta=k_{12}+k_{21}+k_{10}$, $\alpha\beta=k_{21}k_{10}$ 관계가 생기며, macro 4개는 서로 독립적으로 움직이는 임의의 숫자가 아니라, micro-parameter 공간의 수학적 제약을 받는 값들이다. [G&W pp.61–62; R&T pp.618–619]

**그래프를 볼 때 3초 점검표(3-second Checklist):** 꺾임점(knee)이 어디 있는가($\alpha/\beta$), 두 phase의 면적 비율이 어느 쪽으로 기우는가($f_1/f_2$), terminal 직선이 정말 직선인가(3구획 가능성). [G&W p.63; R&T p.632]

**Aspirin 경고(Aspirin Warning):** R&T의 aspirin 650 mg IV bolus 예시는 early sampling을 놓치면 terminal-only one-compartment 해석으로 $CL=0.98$ L/min을 얻게 되지만, biexponential 해석에서는 $CL=683$ mL/min이다. 즉 terminal-only one-compartment 해석은 CL을 약 44% 과대추정하며, 이를 바탕으로 유지 용량(maintenance dose)을 계산하면 과대추정 위험이 생긴다. [R&T pp.615–620]

**경계 조건(Boundary Conditions):** 선형 약동학(linear kinetics), 중앙구획(central compartment) 내 빠른 혼합(mixing), 분포상(distribution phase) 관측, mammillary 배열(mammillary structure; 중앙구획이 허브인 방사형 다구획 구조), 중앙구획 제거(central elimination) 가정이 필요하다. 이 조건이 깨지면 bi-exponential fit은 가능해도 생리학적 해석(physiological interpretation)은 흔들린다. [G&W pp.57–65; R&T pp.617–619]
...
<!-- PRACTICE_BRIEF -->
**🎯 Practice Brief — M1 (30–60초 능동 회상) [EXPERT_INFERENCE, v3]:**

1. **잔차법 절차 재현:** "G&W Fig.2.43에서 $B=28$, $\beta=0.00229$ min⁻¹을 얻었다. $t=30$ min 관측치 $C_{obs}=58$이라면 residual은? 그 residual을 어디에 놓으면 $\alpha$와 $A$가 나오는가?" — 답: $C_{resid}=58-28e^{-0.00229\times30}\approx58-26.1=31.9$, semilog plot에 놓고 slope/intercept를 읽는다.
2. **Aspirin direction 회상:** "terminal-only 1구획 해석은 biexponential 해석에 비해 CL을 과대 추정인가, 과소 추정인가? 따라서 maintenance dose는 어느 방향으로 틀리는가?" — 답: 과대 추정(0.98 vs 0.683 L/min, 약 44% 높음), maintenance dose도 과대 추정 위험.


> **[MASTER AUGMENTATION — EXPERT_INFERENCE | source-bound synthesis]** Semilog curve-stripping을 ‘계산법’으로만 보지 말고, sampling adequacy를 진단하는 lens로 읽는다. early point가 없으면 \(A,\alpha\)가 약해지고, terminal line이 휘면 2구획 자체가 부족할 수 있으며, \(f_2\)가 작으면 terminal slope를 'elimination phase'라 명명하는 순간 clinical time-scale 판단이 흔들린다.
...
<!-- MASTER LENS -->
**개념 Big Idea:** 같은 2구획 curve는 세 좌표계로 표현된다. Macro는 curve description, micro는 ODE transition, physiological은 clearance와 volume의 언어다. 실무에서는 대개 physiological 좌표계를 쓰지만, 그 좌표계가 왜 같은 curve를 설명하는지는 macro/micro 변환을 이해할 때 보인다. [G&W pp.60–71; R&T pp.618–619]
...
<!-- ANNOTATION -->
좌표계라는 말은 같은 곡선을 서로 다른 질문에 맞춰 다시 쓰는 방식이라는 뜻이다. 곡선을 잘 기술하는 좌표와 임상적으로 해석하기 좋은 좌표가 반드시 같지는 않다.
...
**비식별성(Non-identifiability)의 핵심:** plasma 이중지수 곡선만으로는 중앙구획 제거(central elimination only), 말초구획 제거(peripheral elimination only), 양 구획 제거(both-compartment elimination)를 구별할 수 없다. 통상 중앙구획 단독 제거 모델을 쓰는 이유는 생리적으로 그럴듯하기 때문이지, plasma data가 그것을 증명했기 때문이 아니다. [G&W p.65; R&T pp.618–619]

**보고 시 주의사항(Reporting Caveat):** 보고서에서는 “중앙구획에서만 제거(elimination from central compartment only)”와 “말초구획 단순 통합(peripheral compartment lumping)” 가정을 명시해야 한다. 조직 약동학(tissue PK), 거대 단백 약물(large protein drugs), 표적 매개 제거(target-mediated elimination)가 의심되는 경우에는 이 가정에 대한 기전적 근거(mechanistic justification)를 함께 제시해야 한다. [R&T p.618–619; 실무 추론]

**NONMEM 연결(NONMEM Bridge):** `[실무 확장]` ADVAN3/TRANS4의 표준 표현은 $CL,V_1,Q,V_2$이며, $K=CL/V_1$, $K_{12}=Q/V_1$, $K_{21}=Q/V_2$로 ODE 전이(transition)를 만든다. 이 코드는 첨부 PDF의 직접 내용이 아니므로 구현 참고(implementation note)로만 둔다.

**분배 주의사항(Partition Caveat):** 생리학적 ODE에서 중앙/말초 농도가 정상상태(steady state)에서 같아진다는 표현은 분배(partitioning)가 없다는 가정 위에 있다. 실제 조직 농도(tissue concentration)가 plasma와 다르면 서로 다른 분포 청소율(distribution clearance) 또는 분배 계수(partition coefficient)가 필요하지만, 두 구획 모두의 data 없이는 식별이 어렵다. [G&W p.70]

<!-- RECAP -->
**Recap:** M2의 실무 의미는 “macro를 micro로 바꾸는 공식 암기”가 아니라, **plasma curve만으로 식별되는 것과 가정으로 보충하는 것을 분리하는 능력**이다.

<!-- PRACTICE_BRIEF -->
**🎯 Practice Brief — M2 (30–60초 능동 회상) [EXPERT_INFERENCE, v3]:**

1. **좌표계 식별:** "$A,\alpha,B,\beta$를 보고하면 어느 좌표계인가? $V_c, k_{10}, k_{12}, k_{21}$은? $CL, V_1, Q, V_2$는? 셋 중 covariate 보고에 가장 적합한 것은?" — 답: macro / micro / physiological. Covariate는 보통 physiological에 붙인다.
2. **비식별성 자가 점검:** "plasma biexponential curve만으로 'central elimination only', 'peripheral elimination only', 'both compartment elimination'을 구별할 수 있는가? 통상 어느 가정을 쓰는가? 왜?" — 답: 구별 불가. central elimination only를 쓰지만 그 이유는 데이터가 증명해서가 아니라 생리적 그럴듯함 때문. 보고서에 가정을 명시해야 한다.
...
### Card M3 — V₁ / Vss / Vβ 3중 분포용적 체계 [⚡ Apex Concept]

<!-- MASTER LENS -->
**[⚡ Apex Concept]** 본 세션에서 가장 난해하고 임상·규제 파급력이 큰 단 하나의 카드. terminal slope geometry를 body amount로 직역하는 순간 loading dose, tissue accumulation, multiple dosing 해석이 한꺼번에 무너진다. R&T가 *"terminal half-life ceases to have simple application"*이라고 경고한 영역의 직접 출구. [Curation Map Apex 지정; R&T pp.628–641]
...
**순서 관계(Ordering):** 표준 중앙구획 제거(central-elimination) 2구획 가정에서 $V_1\le V_{ss}\le V_\beta$로 생각할 수 있다. 하지만 이 부등식은 보편 법칙(universal law)이 아니라 모델 가정 안에서의 실무 규칙(working rule)이다. [R&T pp.629–630]

**Aspirin 기준점(Anchor):** aspirin은 $V_1=6.5$ L, $V_{ss}=10.4$ L, $V_\beta=13.7$ L로 차이는 있지만 치명적으로 벌어지지는 않는다. [R&T pp.617, 629–630]

**Gentamicin 기준점(Anchor):** gentamicin에서는 $V_1=14$ L, $V_{ss}=56$ L, $V_\beta=345$ L로 $V_\beta$가 $V_{ss}$보다 약 6배 크다. terminal half-life 87 hr는 plasma elimination half-life처럼 행동하지 않으며, 초기 phase half-life 약 4 hr가 therapeutic plasma concentration 조절에 더 중요해진다. [R&T pp.630, 635–641]

**Thiopental 직관(Intuition):** R&T thiopental dog example에서 fat partitioning이 크고 adipose tissue가 total apparent Vd의 큰 몫을 차지하므로, 친유성(lipophilicity)·관류(perfusion)·분배(partitioning) 정보는 새 약물의 $V_{ss}/V_1$과 $V_\beta$ 부풀림을 사전에 의심하게 해주는 단서다. [R&T pp.614–615]

<!-- CONFUSION -->
**Plausible Fallacy [EXPERT_INFERENCE, v3]:**

- **그럴듯한 오해:** "terminal half-life가 길면 2구획 모델에서 분포가 넓다는 뜻이다."
- **왜 틀렸는가:** terminal half-life는 $\beta$ slope의 역수 ($0.693/\beta$)로 결정되는데, $\beta$는 $CL,Q,V_1,V_2$의 복합함수다. $V_\beta=CL/\beta$로 계산되어 $V_{ss}$보다 항상 크거나 같으며, 두 구획 간 재분포 흐름이 반영된 겉보기 부피다. terminal half-life가 길더라도, $V_1$이 작고 $Q$가 느린 경우에는 초기 분포가 아니라 **재분포 속도**가 rate-limiting 요인이 된다. 즉 "넓은 분포"가 아니라 "느린 재분포"가 긴 terminal half-life의 진짜 원인일 수 있다. (이전 버전의 "$V_\beta$가 terminal curve geometry의 산물"이라는 메시지는 이 메커니즘 안에 흡수되어 있다.)
- **실무에서 어떻게 드러나는가:** loading dose를 $V_\beta$ 기반으로 계산하면 $V_{ss}$보다 과대추정되어 과도한 초기 농도를 유발한다. 반대로 $V_{ss}$로 loading dose를 계산하면 실제 분포 용량에 가깝다. 즉 같은 terminal half-life 수치라도, "$V_\beta$로 loading"한 환자와 "$V_{ss}$로 loading"한 환자는 첫 30–60분 plasma peak이 수 배 차이날 수 있다 — gentamicin-like 약물에서 특히 위험하다 ($V_\beta=345$ L vs $V_{ss}=56$ L, 약 6배). [R&T pp.628–630, 638–639]
...
<!-- PRACTICE_BRIEF -->
**🎯 Practice Brief — M3 (30–60초 능동 회상) [EXPERT_INFERENCE, v3]:**

1. **Volume 선택 의사결정:** "Gentamicin에서 $V_1=14$ L, $V_{ss}=56$ L, $V_\beta=345$ L다. (a) Loading dose 계산, (b) AUC 정규화, (c) terminal half-life 해석, (d) bolus 직후 plasma peak 예측 — 각각 어느 부피를 쓰는가?" — 답: (a) $V_{ss}$, (b) $CL$로 정규화(부피는 직접 안 씀), (c) $V_\beta/CL$, (d) $V_1$.
2. **Plausible Fallacy 역질문:** "어떤 약물의 terminal half-life가 매우 길다는 보고를 받았다. '이 약은 분포가 넓다'고 결론내기 전에 무엇을 확인해야 하는가?" — 답: $V_1$이 작고 $Q$가 작으면 "넓은 분포"가 아니라 "느린 재분포"가 긴 terminal half-life의 원인일 수 있다. $V_1, V_{ss}, V_\beta$ 비율부터 본다.
...
<!-- MASTER LENS -->
**개념 Big Idea:** 같은 biexponential data를 5개 parameterization으로 fit하면 WRSS<!-- ANNOTATION -->(← weighted residual sum of squares; fit 잔차 크기 지표)는 비슷해도 parameter precision과 condition number<!-- ANNOTATION -->(← 추정 불안정성의 수치 지표)는 크게 달라진다. 즉 model form이 같아도 좌표계가 estimation geometry를 바꾼다. [G&W pp.513–517]

**PK8 기준점(Anchor):** Compound X 100 µg IV bolus data에서 G&W는 5가지 parameterization을 비교한다. Table 8.1의 condition number는 ODE model 29.69, Macro model 125.2, Colburn 2243, Reparameterized model 2306, Takada 3186이다. 따라서 PK8의 범위는 **29.69–3,186**이고, 최대/최소 비율은 약 **107배**다. [G&W pp.513–517]
...
**핵심 교훈(Core Lesson):** 가장 낮은 WRSS가 실용 모델(production model)의 충분조건은 아니다. Takada model은 WRSS가 낮아도 condition number가 높다. 반대로 ODE model은 condition number가 가장 낮다. [G&W p.516]

<!-- TRENCH -->
**Trench tip:** condition number가 폭증하면 좌표계부터 바꾸기 전에 THETA 초기치를 먼저 확인한다. 초기치가 자릿수부터 틀리면 좌표계와 무관하게 정보 행렬(information matrix)이 나빠질 수 있다. 디버깅 순서: 초기 추정치 → 로그 영역 파라미터화 → 재파라미터화. [실무 디버깅 순서]

**RSE 주의사항(RSE Caveat):** condition number는 파라미터 간 정보 직교성을, RSE<!-- ANNOTATION -->(← relative standard error; 각 추정치의 상대표준오차, %로 표현)는 각 파라미터 자체의 추정 신뢰성을 본다. PK8 Table 8.1에는 RSE가 없으므로 그 표만으로 실용 모델 결정을 끝내지 않는다. [실무 확장]

**PK7 모델 판별 연결(Model Discrimination Bridge):** G&W PK7은 mono/bi/tri-exponential discrimination에서 AIC/F-test, 정밀도(precision), 상관성(correlations), 잔차(residuals), 최소 적절 모델 규칙(simplest adequate model rule)을 함께 보게 한다. 특히 **weighting scheme이 다른 두 모델을 단순 F-test로 비교해서는 안 된다.** 두 모델의 우도(likelihood)가 서로 다른 측정오차 가정(measurement-error assumption) 위에 서 있기 때문이다. 이를 무시하면 모델 판별(model discrimination) 결론 자체가 무효가 된다. NDA 제출의 모델 정당화 논리는 이 단순 사실 위에 서 있다. [G&W pp.505–512]
...
<!-- PRACTICE_BRIEF -->
**🎯 Practice Brief — M4 (30–60초 능동 회상) [EXPERT_INFERENCE, v3]:**

1. **Condition number 매칭:** "PK8 Table 8.1에서 ODE=29.69, Macro=125.2, Colburn=2243, Reparameterized=2306, Takada=3186이다. lowest와 highest의 비율은? '비슷한 fit'에서 가장 unstable한 모델은?" — 답: 약 107배. Takada (3,186)가 가장 unstable. 단, Takada는 WRSS는 가장 낮다 — 두 축 분리 사고의 핵심.
2. **F-test 함정:** "weighting scheme이 다른 두 모델을 단순 F-test로 비교했다. 이 비교가 무효인 이유 한 문장으로?" — 답: 두 모델의 likelihood가 서로 다른 measurement-error assumption 위에 있어 F-test의 분포 가정이 깨진다 — NDA model justification 논리도 함께 무너진다.
...
<!-- MASTER LENS -->
**개념 Big Idea:** terminal half-life는 자주 유용하다. 그러나 distribution kinetics가 강할 때는 “약물이 사라지는 속도”보다 “slow tissue(서서히 평형에 도달하는 조직)에서 plasma로 돌아오는 속도”를 더 많이 반영할 수 있다. 그래서 plateau 도달, tissue accumulation, renal impairment dosing은 $f_1/f_2$와 투여 방식까지 함께 봐야 한다. [R&T pp.631–641]
...
**Nicardipine 기준점(Anchor):** 말단 반감기(terminal half-life)가 약 12 hr로 길지만, plasma 농도는 1 hr 안에 약 50% Css에 도달하고 약 15 hr에 90% Css에 도달한다. 말단 반감기만 보고 부하 용량(loading dose) 필요성을 판단하면 초기 과량 투여 위험이 생길 수 있다. [R&T p.631]

**Gentamicin 기준점(Anchor):** 말단 반감기가 87 hr이어도 plasma 치료적 거동(therapeutic behavior)은 약 4 hr 초기상에 의해 좌우된다. 말단항 기반 축적 지수(accumulation index)는 16배가 될 수 있지만, 이것은 plasma 최고/최저 농도 축적과 동일하지 않고 서서히 평형에 도달하는 조직(slowly equilibrating tissue)의 축적을 말한다. [R&T pp.635–641]

**조직 vs plasma 정상상태(Tissue vs Plasma Plateau):** plasma가 정상상태에 빨리 접근해도 서서히 평형에 도달하는 조직은 훨씬 늦게 접근할 수 있다. 따라서 반응 부위(response site)가 말초/조직 쪽이면 plasma 반감기만으로 작용 개시(onset)와 지속(duration)을 해석하면 안 된다. [R&T pp.633–634]
...
<!-- TRENCH -->
**Lab rule:** 누군가 “이 약물의 half-life는 X시간”이라고 말하면 바로 묻는다: “terminal? effective? plasma? tissue?” 이 구분이 달라지면 multiple-dosing simulation 결과도 달라진다. [실무 확장]

<!-- RECAP -->
**Recap:** M5의 실전 규칙은 **half-life 하나로 dosing을 말하지 말고, $f_1/f_2$, volume term, input pattern, sampling time을 같이 본다**는 것이다.

<!-- PRACTICE_BRIEF -->
**🎯 Practice Brief — M5 (30–60초 능동 회상) [EXPERT_INFERENCE, v3]:**

1. **Half-life 분류 4질문:** "회의에서 누군가 '이 약물의 half-life는 X시간'이라고 말한다. 즉시 되물어야 할 4가지 분류 질문은?" — 답: (a) terminal? effective? (b) plasma? tissue? (c) single dose? multiple dosing? (d) $f_2$는 얼마인가?
2. **Plateau 시그니처 매칭:** "$f_2 \gg 0.8$인 약물과 nicardipine처럼 $f_2$가 작은 약물 — 어느 쪽이 terminal half-life로 plateau 시간을 잘 설명하는가? 다른 쪽은 어느 시간 척도가 맞는가?" — 답: $f_2 \gg 0.8$이면 terminal half-life가 plateau를 잘 설명. 작으면 plasma는 빠르게 plateau에 접근하므로 terminal half-life는 dosing 판단에 부적합 (nicardipine: 12 hr terminal vs 1 hr to 50% Css).
...
$\alpha$ phase는 약이 혈장에서 조직으로 **이사 가는** 분포 단계이고, $\beta$ phase는 주로 **청소**(제거)가 지배하는 단계다. 그러나 두 단계가 완전히 분리된 것이 아니라 항상 **동시에** 일어난다 — $\alpha$ 단계에도 제거가 일어나고, $\beta$ 단계에도 분포 평형 조정이 일어난다. "$\alpha$ = 분포, $\beta$ = 제거"는 **근사 레이블이지 기계적 분리가 아니다.** 이 레이블을 두 개의 elimination pathway로 곡해하는 순간 central elimination 가정이 무너진다.
...
|---|---|---|
| terminal slope로 계산한 $V_\beta$를 body amount/loading dose용 volume으로 쓴다. | steady-state amount 또는 loading dose 사고에는 $V_{ss}$가 더 가까운 volume이다. $V_\beta$는 terminal slope geometry의 산물이다. $V_1$은 bolus 직후 central dilution이고, 셋은 서로 다른 시점·서로 다른 임상 질문에 대응한다. | Gentamicin: $V_\beta=345$ L, $V_{ss}=56$ L, $V_1=14$ L. $V_\beta$를 “진짜 body volume”처럼 쓰면 tissue redistribution과 plasma dosing을 혼동한다. [R&T pp.630, 635–641] |
| **치명적 타격** | NDA/IND 제출 또는 critical care 상황의 loading dose 결정에서 $V_\beta$를 $V_{ss}$ 대신 사용하면 gentamicin-like 약물의 첫 dose가 약 6배 과대 산정될 수 있다. 그 결과 (1) 심사관이 dosing rationale의 volume 정의 일관성 결여를 Deficiency Letter 사유로 지적하거나, (2) 환자에게 nephrotoxicity/ototoxicity 임계 농도를 첫 60분 내 초과하는 plasma peak이 발생할 위험이 있다. | |

**⚡ 기억 고리 (Memory Hook) — *현재 vs 균형 vs 제거 그림자* [EXPERT_INFERENCE, v3]:**  
$V_1$은 투여 직후 약이 도달한 **즉각적 분포 공간**(혈장 + 빠른 조직), $V_{ss}$는 분포와 제거가 독립인 **평형 상태의 겉보기 부피**, $V_\beta$는 terminal slope에서 역산한 **제거 편향이 혼합된 겉보기 부피**이다. $V_1 < V_{ss} \le V_\beta$는 2구획 시스템의 수학적 필연이다. **Loading dose는 $V_{ss}$에, AUC 정규화는 $CL$에, terminal half-life는 $V_\beta/CL$에 의존한다 — 세 부피를 혼용하면 세 계산이 모두 틀린다.**
...
좌표계 전환에서 가장 자주 혼동되는 핵심 쌍은 **$Q$ vs $CL$**이다. $Q$ (intercompartmental clearance)는 중앙구획과 말초구획 사이를 오가는 **내부 순환량**(고속도로 통행량)이고, $CL$ (systemic clearance)은 체내에서 완전히 빠져나가는 **순 배출량**(출구)이다. $Q$가 크면 두 구획이 빠르게 균형을 잡고(짧은 $\alpha$ phase), $CL$이 크면 전체 약물이 빠르게 제거된다. **$Q \gg CL$이면 2구획 성격이 약해져 1구획처럼 보일 수 있다** — 이것이 macro($\alpha,\beta$) ↔ physiological($Q,CL$) 좌표계 변환에서 비로소 보이는 시스템 진단 단서다.

### Pair 4 — Terminal half-life vs Effective/clinical half-life [치명적 구분 (Critical distinction)]

<!-- CONFUSION -->
| Trap | Correction | 치명적 구분 (Critical distinction) |
|---|---|---|
...
> **Critical Blow 강등 사유 [v3]:** V5.0은 §5에 최대 1개의 Critical Blow만 허용한다. 본 세션의 Critical Blow는 임상·규제 파급력이 더 큰 Pair 2 ($V_1$/$V_{ss}$/$V_\beta$ 혼동, NDA Deficiency Letter 가능성)에 우선 배정한다. 본 Pair 4의 위험은 여전히 임상적으로 중요하지만 "치명적 구분(Critical distinction)" 수준으로 라벨링한다.
...
<!-- SELF-TEST -->
G&W Fig.2.43에서 말단상(terminal phase)을 외삽하여 $B=28$, $\beta=\ln(28/10)/450$를 얻었다. 초기 관측치에서 무엇을 빼야 $\alpha$ 상(phase)을 얻는가?

**정답:** 각 초기 관측치 $C_{obs}(t_i)$에서 $B e^{-\beta t_i}$를 뺀다. 잔차(residual)를 반로그 도표(semilog plot)에 다시 놓고 기울기/절편을 구하면 $\alpha$와 $A$가 나온다. [G&W p.60]

### Q2. Aspirin CL 방향 판단

<!-- SELF-TEST -->
Aspirin 650 mg IV bolus에서 말단상 전용 1구획 해석은 $CL=0.98$ L/min, 이중지수 해석은 $CL=683$ mL/min이다. 말단상 전용 해석은 실제 CL을 과소평가하는가, 과대평가하는가?

**정답:** 과대평가한다. $0.98$ L/min은 683 mL/min보다 약 44% 높다. 따라서 유지 용량(maintenance dose) 계산은 과대평가 위험이 있다. [R&T pp.615–620]

### Q3. 용적 비율 적용(Volume Ratio Application)

<!-- SELF-TEST -->
Aspirin과 gentamicin 중 $V_\beta/V_{ss}$ 비율이 더 큰 약물은 무엇이며, 그것이 왜 중요한가?

**정답:** Gentamicin이다. Aspirin은 $13.7/10.4\approx1.3$이지만 gentamicin은 $345/56\approx6.2$다. 큰 비율은 말단 기울기 유래 용적을 정상상태 체내량 용적처럼 쓰면 안 된다는 경고다. [R&T pp.629–641]

### Q4. 구조적 비식별성(Structural Non-identifiability)

<!-- SELF-TEST -->
동일한 이중지수 plasma 곡선을 중앙구획 제거, 말초구획 제거, 양 구획 제거 모델이 모두 설명할 수 있다. plasma data만으로 무엇을 결론낼 수 없는가?

**정답:** 제거 부위(elimination site)를 식별할 수 없다. 중앙구획 단독 제거는 보통 생리학적 타당성(physiologic plausibility) 때문에 선택되는 실무 가정(working assumption)이며, 조직 data나 기전적 근거(mechanistic justification) 없이 plasma 곡선만으로 증명된 것이 아니다. [G&W p.65; R&T pp.618–619]

### Q5. 말단 반감기의 함정(Terminal Half-life Trap)

<!-- SELF-TEST -->
Nicardipine처럼 말단 반감기가 12 hr인데 1 hr에 50% Css에 도달하는 사례에서 말단 반감기만으로 부하 용량(loading dose)을 권하면 왜 위험한가?

**정답:** plateau approach가 terminal term만으로 결정되지 않고 $f_1/f_2$ 가중합으로 결정되기 때문이다. plasma는 이미 빠르게 올라가는데 terminal half-life만 보면 “너무 늦게 도달한다”고 오판하여 과도한 loading을 권할 수 있다. [R&T pp.631–633]
...
**정답:** WRSS는 fit residual 크기, condition number는 parameter estimation geometry를 본다. 둘 중 하나만 좋다고 deployable model은 아니다. PK8의 핵심은 “동일 데이터/비슷한 fit에서도 좌표계가 condition number를 크게 바꾼다”는 것이다. [G&W pp.513–517]
...
**정답:** “약물의 $V_2$가 진짜 작다”가 아니라 distribution phase가 data에 없어 $V_2$가 식별되지 않는 Phantom V2 상황을 의심한다. [실무 확장]
...
**정답:** terminal half-life인지 effective/plasma half-life인지, $f_2$가 얼마인지, 그리고 축적이 plasma 구획인지 서서히 평형에 도달하는 조직(slowly equilibrating tissue)인지 물어야 한다. Gentamicin-like case에서는 terminal term accumulation과 plasma therapeutic fluctuation을 구분해야 한다. [R&T pp.635–641]

### Q9. 보스 딜레마 — PK8 모델 채택 결정 (★★ Socratic Dilemma)

<!-- SELF-TEST -->
당신은 NDA submission 마감 1주일 전에 있다. PK8 데이터로 PopPK 모델 후보가 둘로 좁혀졌다.
...
당신은 어느 모델을 실용 모델(deployable model)로 채택하는가? 그리고 그 결정을 심사관(reviewer)에게 어떻게 방어하는가?

**정답 (수석 모델러의 Trade-off 논리):**

**채택: Model B (ODE 생리학적 모델).** 그러나 정답은 모델 선택 자체보다 **방어 논리의 구조**에 있다.

1. **WRSS는 fit residual의 크기, condition number는 추정 geometry의 안정성**이다. 두 축은 서로 대체할 수 없다. 단일 축으로 결정할 수 없는 상황은 NDA에서 흔하다.
2. **실용 모델의 첫째 조건은 재현성(reproducibility)** — 다른 시드, 다른 초기치, 다른 공변량 추가에서도 동일한 결론이 재현되는가. condition number 3,186은 covariance step instability와 covariate inclusion 시 결과 비예측성을 시사한다.
3. **심사관 방어 논리(Reviewer Defense)** — "Takada는 lowest WRSS이지만 (a) 공분산 기하학(covariance geometry)이 100배 이상 불안정하고 (b) 시간 의존적 용적 표현은 공변량-용적 관계 해석에 추가 해석 부담을 만든다. ODE 생리학적 모델은 적합이 거의 동등하면서 (a) 정밀도/상관성이 안정적이고 (b) 청소율/용적 좌표계로 일반적 PopPK 보고 관행에 부합한다." [실무 추론: regulatory defense]
4. **만약 적합 차이가 임상적으로 유의미한 수준 (예: AUC 차이 >10%)이면** — 두 모델 모두를 민감도 분석(sensitivity analysis)으로 보고하고 최종 투여 권고(final dosing recommendation)는 두 모델 간 견고한(robust) 영역에서 도출한다. PK8 자체는 그 정도 fit 차이를 보여주지 않는다.
5. **함정:** "WRSS가 더 낮으니 Takada가 진짜 모델에 가깝다"는 추론은 **부적절한 가중법(weighting scheme) 비교**일 수도 있다 (Q6 + PK7 bridge 참조). 무가중 RSS와 가중 RSS는 직접 비교 대상이 아니다. [G&W pp.508–517; R&T 일반 PopPK practice; 실무 추론]

**다음 깊이 질문:** Model B를 채택했는데 심사관이 "그렇다면 Takada가 보여준 lower WRSS는 어떤 정보를 담고 있나"라고 묻는다면? — "lower WRSS는 시간 의존적 $V$ 가정이 적합 잔차를 약간 감소시킨다는 신호이지, 그 가정이 기전적으로 검증(mechanistically validated)되었다는 의미는 아니다. 기전적 검증(mechanistic validation) 없이는 실용 모델로 채택할 수 없다"가 표준 답변이다. [실무 추론]

<!-- RECAP -->
**Self-test recap:** 9개 질문(8 회상·적용 + 1 Socratic Dilemma)을 모두 통과하면, 2구획 모델을 “공식”이 아니라 “모델 선택·volume 해석·dosing simulation·규제 방어 논리의 사고 도구”로 사용할 준비가 된 것이다.
...
| **Pseudo-Stable V across renal function** | gentamicin-like 약물에서 $V_\beta$ vs CrCl plot이 거의 평탄하지만 multiple-dose plasma trough VPC<!-- ANNOTATION -->(← visual predictive check; 모델-관측치 시각 진단)는 신부전군에서 systematic underprediction. $V_\beta=CL/\beta$ 관계 때문에 신부전 시 CL과 $\beta$가 함께 감소하면, 두 값의 비율이 안정해 보이는 착시가 생긴다. | covariate를 $V_\beta$가 아닌 $V_{ss}/V_c$에 attach하고, plasma trough VPC와 tissue accumulation 예측을 분리 검증. [실무 추론] |
...
1. Semilog 곡선의 꺾임점(knee)과 말단 직선성(terminal straightness)을 보고 2구획/3구획 가능성을 가른다.  
2. $f_2$를 보고 말단 반감기가 임상 시간 척도(clinical time scale)를 대표할지 판단한다.  
3. $V_\beta/V_{ss}$ 비율로 terminal volume 오용 위험을 본다.  
4. WRSS가 낮아도 condition number/RSE/상관성이 나쁘면 실용 모델(production model)로 보류한다.  
5. CWRES<!-- ANNOTATION -->(← conditional weighted residuals; FOCE 기반 가중잔차) vs TIME이 초기 양의 값 또는 S자 패턴이면 구조적 오설정(structural misspecification) 또는 분포상 누락을 의심한다.  
6. $\eta$-shrinkage가 높으면 covariate effect의 해석 가능성을 낮게 본다. [실무 기준]
7. **Covariate model의 forward inclusion 단계에서 5개 카드(M1–M5)가 동시에 켜지는 것을 본다** — $f_2$ 직관(M1/M5) → 좌표계 선택(M2) → $V_{ss}$ vs $V_\beta$ attach 결정(M3) → condition number/RSE 안정성 점검(M4)이 한 번의 covariate 추가에서 모두 작동한다. 이 동시 활성화를 인식하지 못하면 covariate를 *어디에 붙어야 하는지*조차 결정할 수 없다. [실무 통합 — §1 Workflow anchor의 §8 재호출]
