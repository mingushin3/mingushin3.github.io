# PASS 1 — Readability & Flow Polish Log

| Location (§ + card) | Edit type (R1–R5) | Before (excerpt) | After (excerpt) |
|---|---|---|---|
| §1 Big Idea | R1, R2 | “본질은 관측값을 … 분해하고, 그 분산인 … 추정” | mixed-effects model 설명을 두 문장으로 분리하고 “그다음”의 추정 순서를 명시. |
| §1 Big Idea / G&W bridge | R2 | “공변량 모델은 control stream에서 시작하지 않고…” | “따라서”를 추가해 EDA-first와 control stream 사이의 인과를 명시. |
| §2 C1 Big Idea | R1 | “θ는…, η는…, ε는…” | θ/η/ε 정의를 두 문장으로 분리해 개념 load를 낮춤. |
| §2 C1 Data Anchor | R1, R3 | warfarin과 nortriptyline 예시가 한 문장에 결합 | 두 예시를 분리하고 “평균만 보면 문제가 가려진다”를 바로 뒤에 배치. |
| §2 C1 NAD/NPD bridge | R1 | NAD와 NPD 정의가 한 문장에 병렬 | 두 접근을 별도 문장으로 분리. |
| §2 C1 Structural Necessity | R2 | “C2와 C3는 모두 잘못된다” | “C2 residual model / C3 covariate model”로 causal target을 명시. |
| §2 C2 Big Idea | R4 | “실제로는 측정/잔차 구조의 문제인 것이…” | 중첩 명사구를 줄이고 “문제가 …처럼 보인다”로 압축. |
| §2 C3 Big Idea | R1, R5 | “ω² 안에 섞인 생리학을 분리해 이름을 붙이는 작업” | 첫 문장을 분리해 “줄이는 기술이 아님”과 “분리 작업”을 단계화. |
| §2 C3 PK50 interpretation | R1, R2 | “이는 … 뜻이 아니라, 같은 12명 자료를…” | 부정문과 정정문을 분리해 “아님 → 뜻함” 구조를 명확화. |
| §2 C4 Big Idea | R1 | “η가 실제로는 … 합일 수 있음” | smooth distribution이 아닐 수 있다는 중간 문장을 삽입. |
| §5 Pair 1/3 recap | R1 | distinction 문장이 길게 연결 | 정의·귀결을 별도 문장으로 분리. |
| §8 Final Recap | R1 | C2/C3/C4의 역할이 한 문장에 누적 | C2–C4 역할을 두 문장으로 분리. |

**PASS 1 Step B — Polished Content Lock v1.5 status**: v1.5는 위 edit log의 sentence-level polish를 반영한 중간본으로 생성되었고, 아래 PASS 2 annotation의 기반 텍스트로 사용하였다. 섹션 재배열, source page tag 이동, equation/code 변경은 수행하지 않았다.
...
|---|---|---|---|---|
| A | §1 Big Idea | NONMEM mixed-effects model | “fixed/random effects를 함께 추정” first-use gloss | Low |
| A | §2 C1 Formal Definition | Ω/ω², Σ/σ² | 두 variance level의 차이를 1문장으로 설명 | Low |
| A | §2 C3 Big Idea | Covariate modeling | “변이의 설명변수를 모델에 넣는 작업” first-use gloss | Low |
| B | §2 C3 PK50 Anchor | fu는 “ω² reduction”이 아니라 좌표계 변환 | covariate model과 reparameterization의 구분 bridge | Low |
| A | §2 C4 Formal Definition | polymodal distribution | “봉우리가 여러 개인 분포” first-use gloss | Low |
| C | §2 C1 Structural Necessity | ω²/σ² competition | “세 그릇에 변이를 나눠 담는” analogy | Medium–High; 흐름 방해 가능 |
| B | §5 Pair 2 | total vs unbound coordinate | coordinate system bridge 추가 | Medium; 이미 본문이 충분히 명확 |

## Step 2. Final Recommendation List

**Must annotate (Low risk)**
- §1 Big Idea: NONMEM mixed-effects model first-use gloss.
- §2 C1 Formal Definition: Ω/ω² vs Σ/σ² level distinction.
- §2 C3 Big Idea: Covariate modeling first-use gloss.
- §2 C3 PK50 Anchor: covariate model vs reparameterization bridge.
- §2 C4 Formal Definition: polymodal distribution first-use gloss.

**Optional (Medium risk, high learning value)**
- §5 Pair 2 coordinate-system bridge: 보류. Pair 2 본문이 이미 corrected critical blow를 포함하므로 과잉 설명 위험이 있음.

**Do not annotate**
- C1 ω²/σ² structural analogy: 전문 독자에게 다소 장황하고, “분산 보존” 직관을 오히려 단순화할 위험이 있어 제외.

## Step 3. Content Lock v2

# 13_Content Lock v2 — 개체 간 변이(IIV/BSV)와 공변량 모델링

**Source lock**: R&T 5e Ch.12–13 (p.361–410); G&W 5e Ch.4 (p.333–352) + PK50 (p.704–710)  
**Output scope**: §1 → §2 → §5 → §7 → §8  
**Length control**: Content Lock v1 대비 +8% 이내에서 readability polish와 minimal annotation만 반영.  
**Figure policy**: Audit T6 Figure Inventory는 본 문서에서 판정하지 않고 Phase 4C로 이월.

---

## Updated Curation Map — Content Lock v2
