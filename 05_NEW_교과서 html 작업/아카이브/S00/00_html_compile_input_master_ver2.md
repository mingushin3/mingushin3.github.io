# 00_HTML Compile Input Master — ver2

**Output file**: `00_html_compile_input_master_ver2.md`
**Assembly date**: 2026-05-06
**Assembly mode**: PATCH MODE
**Phase**: 4D — Learner-Complete + Mastery-Enriched + HTML-Ready Master Assembly
**Revision basis**: ver1 reviewer found one Critical and three Minor issues; this ver2 applies bounded fixes without altering scientific content, source page tags, equations, figure briefs, or augmentation budgets.
...
|---|---|---|
| Learner-Standalone Certificate | **PASS (ver1: conditional)** | PART A now contains only learner-facing content. The Compression Compliance Note that previously appeared as `§8.D` in PART A has been moved to `B0` in PART B, restoring B1 contract compliance. |
| Zero-Omission Certificate | PASS | Scope-required MUST concepts, data anchors, Audit MUST/SHOULD corrections, Crucible-adopted insights, and all 3 approved Figure Pointers are present or intentionally protected in guardrails. Coverage Matrix re-verified. |
| Mastery-Uplift Certificate | PASS | Five bounded augmentations remain at M1, M2, M3, M4, and §5 Pair 2; all are ≤3 sentences and source-status labeled. Plus one structural memory-hook addition at §5 Pair 1 (paraphrased from T p.5; no new claim). |
| Source-Boundary Certificate | PASS | Augmentations are labeled TEXTBOOK_DERIVED or CRUCIBLE_DERIVED. ver2 changes are formatting and relocation; no new numbers, page tags, examples, equations, or external claims were introduced. |
| HTML-Readiness Certificate | PASS | PART B contains rendering contracts, marker mappings, page-tag rules, navigation anchor integrity rules, figure restrictions, and regression guardrails. FIGURE_POINTER blocks reformatted to one-field-per-line for parser robustness. |
...
**⚡ 기억 고리**: PK는 *input → concentration* 의 화살표이고 PD는 *concentration → effect* 의 화살표다. 두 화살표는 **같은 선상의 앞뒤 화살표**이지 좌우의 반대 화살표가 아니다. 따라서 한쪽 화살표만 그리면 dose-to-effect 사슬이 끊겨 임상 의사결정이 멈춘다 [T p.5].
<!-- /CONFUSION -->
...
### B9. Zero-Omission Coverage Matrix (ver2 re-verified)
...
| C6 Page tag integrity | Existing [p.XX]/[pp.XX–YY] tags preserved; no new page tags created by mastery notes or by ver2 changes | PART A body | PRESENT | None |
...
| C9 Canonical integrity | Content Lock v2 learner body preserved except approved figure pointer splices, the bounded Mastery Augmentation Layer, and ver2 assembly edits (relocation of §8.D to B0; FIGURE_POINTER reformatting; Pair 1 memory hook addition; §1 Roadmap Apex visual cue) | PART A; logs below | PRESENT | None |
| C10 Learner-Standalone (ver2) | PART A contains no audit/process/compiler appendix text; §8.D was relocated to B0 | PART A inspection | PRESENT | None |

### B10. Micro-Patch Log

ver1 had no scientific micro-patches. ver2 also introduces no scientific micro-patches; the changes below are assembly-only:
...
## Final Checklist (ver2)

| Check | Status |
|---|---|
| PART A alone is learner-facing and complete | PASS — §8.D process content removed from PART A and relocated to B0 |
| PART A contains no audit/process/compiler appendix text | PASS |
| Approved figure markers are present exactly once | PASS — 3 markers, reformatted to one-field-per-line |
...
| Mastery augmentations are labeled and within B-Standard budget | PASS — 5 original augmentations + 1 ver2 memory-hook addition (paraphrased from PDF, labeled TEXTBOOK_DERIVED) |
| No unsupported numerical values, new examples, equations, or page tags were introduced by augmentation or by ver2 edits | PASS |
...
| ver1 → ver2 changes are assembly/relocation/formatting only and do not alter scientific content | PASS |
| §5 Pair 1 has a structural memory hook (ver2) | PASS |
| §1 Roadmap shows Apex visual cue consistent with M1 card heading (ver2) | PASS |
| FIGURE_POINTER blocks parseable line-by-line by Phase 5 (ver2) | PASS |
