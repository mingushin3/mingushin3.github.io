...
<!-- SECTION_CORE: SC-01 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 농도 종과 그림 포인터의 규칙을 먼저 고정하라 — $C$, $C_u$, $V$를 구분하지 않으면 뒤의 모든 분포 판단이 흔들린다.
...
<!-- SECTION_CORE: SC-02 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $V$는 장기 부피가 아니라 Amount/$C$ 변환자다 — 막투과·결합·수송체를 이 좌표로 묶어야 분포의 전체 지도가 보인다.
...
>
> $$
> \underbrace{C(t)}_{\text{농도-시간}}=\left(\frac{\underbrace{Dose}_{\text{투여량}}}{\underbrace{V}_{\text{겉보기 }V}}\right)\cdot\underbrace{e^{-kt}}_{\text{소실항}}
> $$
...
<!-- SECTION_CORE: SC-03 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 막투과는 $logP$ 하나가 아니라 크기·전하·수송체·$C_u$ 기울기가 함께 결정한다 — 총 농도 차이를 먼저 보는 습관을 버려라.
...
>
> $$
> \underbrace{\text{Net rate of transport}}_{\text{순수송}}=\underbrace{P}_{\text{투과도}}\cdot\underbrace{SA}_{\text{표면적}}\cdot\underbrace{(C_{u,1}-C_{u,2})}_{\text{유리농도차}}
> $$
...
>
> $$
> \underbrace{\frac{C_{u,T}}{C_u}}_{\text{조직/혈장 }C_u}=\underbrace{\frac{P_{uptake}}{P_{efflux}}}_{\text{유입/유출}}
> $$
...
<!-- SECTION_CORE: SC-04 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 분포는 “얼마나 많이”와 “얼마나 빨리”를 분리해서 읽어라 — $K_{pb}$는 범위를 키우면서 시간도 늦출 수 있다.
...
>
> $$
> \underbrace{k_T}_{\text{접근속도}}=\frac{\overbrace{Q/V_T}^{\text{조직관류}}}{\underbrace{K_{pb}}_{\text{조직저장}}}
> $$
...
>
> $$
> \underbrace{t_{1/2,distribution}}_{\text{분포 반감기}}=\frac{\underbrace{0.693}_{\ln 2}\cdot\underbrace{K_{pb}}_{\text{조직 저장성}}}{\underbrace{Q/V_T}_{\text{조직관류}}}
> $$
...
>
> $$
> \underbrace{C_T(t)}_{\text{조직농도}}=\underbrace{K_{pb}\cdot C_A}_{\text{평형농도}}\cdot\underbrace{(1-e^{-k_Tt})}_{\text{접근분율}}
> $$
...
<!-- SECTION_CORE: SC-05 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $V=A/C$를 혈장항·조직항·측정 스케일로 분해하라 — 큰 $V$ 하나만 보고 조직 원인을 단정하지 마라.
...
>
> $$
> \underbrace{V}_{\text{겉보기 }V}=\frac{\overbrace{\text{Amount in body}}^{\text{총량}}}{\underbrace{C}_{\text{측정 }C}}=\frac{\underbrace{A}_{\text{약물량}}}{\underbrace{C}_{\text{농도}}}
> $$
...
>
> $$
> \underbrace{A}_{\text{총량}}=\underbrace{V_p\cdot C}_{\text{혈장량}}+\underbrace{V_T\cdot K_p\cdot C}_{\text{조직량}}
> $$
...
>
> $$
> \underbrace{V}_{\text{겉보기 }V}=\underbrace{V_p}_{\text{혈장항}}+\underbrace{V_T\cdot K_p}_{\text{조직항}}
> $$
...
>
> $$
> \underbrace{V}_{\text{전체 }V}=\underbrace{V_p}_{\text{혈장항}}+\sum_i\underbrace{V_{T,i}\cdot K_{p,i}}_{\text{조직 }i}
> $$
...
>
> $$
> \underbrace{V_u}_{\text{유리 }V}=\frac{\underbrace{A}_{\text{총량}}}{\underbrace{C_u}_{\text{유리 }C}},\qquad \underbrace{V_b}_{\text{혈액 }V}=\frac{\underbrace{A}_{\text{총량}}}{\underbrace{C_b}_{\text{혈액 }C}}
> $$

[출처: R&T Eq.4-14~4-15, p.104]

<!-- SECTION_CORE: SC-06 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $f_u/f_{uT}$, 수송체, 알부민 하한을 함께 봐라 — 같은 $V$ 변화라도 임상적 원인은 완전히 다를 수 있다.
...
>
> $$
> \underbrace{V}_{\text{겉보기 }V}=\underbrace{V_p}_{\text{혈장항}}+\underbrace{V_{TW}\cdot\frac{\overbrace{f_u}^{\text{혈장 }f_u}}{\underbrace{f_{uT}}_{\text{조직 }f_u}}}_{\text{결합조직항}}
> $$
...
>
> $$
> \underbrace{V}_{\text{겉보기 }V}=\underbrace{V_p}_{\text{혈장항}}+\underbrace{V_{TW}\cdot\frac{f_u}{f_{uT}}}_{\text{결합조직항}}\cdot\underbrace{\frac{P_{uptake}}{P_{efflux}}}_{\text{수송체항}}
> $$
...
>
> $$
> \underbrace{V}_{\text{소 }V_d\text{ 약물의 }V}=\underbrace{7.5}_{\text{알부민 분포 하한}}+\underbrace{\left(7.5+\frac{V_R}{f_{uR}}\right)}_{\text{혈관외/잔여 분포 계수}}\cdot\underbrace{f_u}_{\text{혈장 }f_u}
> $$
...
<!-- SECTION_CORE: SC-07 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $f_u$는 고정 상수가 아니라 $K_a$, 단백 농도, 결합 부위, $C_u$가 만드는 조건부 변환자다.
...
>
> $$
> \underbrace{f_u}_{\text{유리분율}}=\frac{\underbrace{C_u}_{\text{유리 }C}}{\underbrace{C}_{\text{총 }C}}
> $$
...
>
> $$
> \underbrace{f_u}_{\text{저 }C_u\text{ 근사 }f_u}\approx\frac{1}{1+\underbrace{K_a}_{\text{친화도}}\cdot\underbrace{n[P_T]}_{\text{결합용량}}}
> $$
...
<!-- SECTION_CORE: SC-08 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $K_p$와 혈액/혈장 비는 조직 결합·적혈구 분배·수송체가 만든다 — 단순 평형 가정을 먼저 의심하라.
...
>
> $$
> \underbrace{K_p}_{\text{조직/혈장비}}\sim\frac{\underbrace{f_u}_{\text{혈장 }f_u}}{\underbrace{f_{uT}}_{\text{조직 }f_u}}
> $$
...
>
> $$
> \underbrace{\frac{C_b}{C}}_{\text{혈액/혈장비}}=1+\underbrace{H}_{\text{Hct}}\left(\underbrace{f_u\cdot K_{PBC}}_{\text{RBC 분배}}-1\right)
> $$
...
<!-- SECTION_CORE: SC-09 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 노출 지표는 숫자가 아니라 인과 종의 선언이다 — $AUC$, $AUC_u$, $C_{max}$ 중 무엇이 반응을 미는지 먼저 정하라.
...
>
> $$
> \underbrace{C}_{\text{총 }C}=\frac{\underbrace{C_u}_{\text{유리 }C}}{\underbrace{f_u}_{\text{유리분율}}}
> $$
...
>
> $$
> \underbrace{AUC_u}_{\text{유리 AUC}}=\underbrace{AUC}_{\text{총 AUC}}\times\underbrace{f_u}_{\text{유리분율}}
> $$
...
<!-- SECTION_CORE: SC-10 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $C$ vs $C_u$, 관류 vs 투과, $f_u$ vs $f_{uT}$를 섞는 순간 분포 해석은 겉보기 숫자놀음으로 무너진다.
...
>
> $$
> \underbrace{V}_{\text{겉보기 }V}=\underbrace{V_p}_{\text{혈장항}}+\underbrace{V_{TW}\left(\frac{\overbrace{f_u}^{\text{혈장 }f_u}}{\underbrace{f_{uT}}_{\text{조직 }f_u}}\right)}_{\text{조직 결합 항}}
> $$
...
>
> $$
> \underbrace{AUC_u}_{\text{활성 }AUC_u}=\underbrace{f_u}_{\text{유리분율}}\times\underbrace{AUC}_{\text{총 AUC}}
> $$

---

<!-- SECTION_CORE: SC-11 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Q1–Q5는 막투과·분포속도·알부민 하한을 되묻는다 — 식을 외우지 말고 어떤 종과 속도를 묻는지 잡아라.
...
>
> $$
> \underbrace{t_{1/2,distribution}}_{\text{분포 반감기}}=\frac{\underbrace{0.693}_{\ln2}\cdot\underbrace{K_{pb}}_{\text{조직저장}}}{\underbrace{Q/V_T}_{\text{조직관류}}}
> $$
...
<!-- SECTION_CORE: SC-12 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Q6–Q10은 $AUC_u$, peak, PK47, 안전마진을 압박한다 — 단축식과 총노출 해석의 조건을 끝까지 검증하라.
...
>
> $$
> \underbrace{AUC_u}_{\text{유리 AUC}}=\underbrace{AUC}_{\text{총 AUC}}\times\underbrace{f_u}_{\text{고정 }f_u}
> $$
...
<!-- SECTION_CORE: SC-13 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** M1–M6는 clearance, TDM, PBPK, PopPK 공변량 해석으로 이어지는 하나의 분포 판단 엔진이다.
...
<!-- SECTION_CORE: SC-14 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 진단 시그니처와 체화 시퀀스로 마무리하라 — $V$, $f_u$, $CL$, 노출 지표의 연결을 새 PK 문제에 즉시 적용하라.
...
>
> $$
> \underbrace{C}_{\text{총 }C},\quad \underbrace{C_u}_{\text{유리 }C},\quad \underbrace{C_b}_{\text{혈액 }C}
> $$
...
>
> $$
> \underbrace{V\lesssim15\ \mathrm{L}}_{\text{소 }V_d\text{/알부민}}
> $$
...
>
> $$
> \underbrace{AUC_u}_{\text{유리 AUC}}=\underbrace{AUC}_{\text{총 AUC}}\times\underbrace{f_u}_{\text{유리분율}},\qquad \underbrace{C_u}_{\text{유리 }C}=\underbrace{C}_{\text{총 }C}\times\underbrace{f_u}_{\text{유리분율}}
> $$
