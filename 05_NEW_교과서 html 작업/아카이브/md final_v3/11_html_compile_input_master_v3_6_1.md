...
<!-- SECTION_CORE: SC-01 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Delayed response는 ODE 선택 문제가 아니라 느린 clock 배정 문제다 — hysteresis·turnover·target consumption·PK/PD rate-limiting을 먼저 분해하라.
...
<!-- SECTION_CORE: SC-02 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $R_0=k_{in}/k_{out}$와 $1/k_{out}$을 먼저 고정하라 — turnover baseline과 hysteresis는 response pool의 생산·소실 균형에서 출발한다.
...
>
> $$
> \underbrace{\frac{dR}{dt}}_{\text{변화율}}=\underbrace{k_{in}}_{\text{생산}}-\underbrace{k_{out}R}_{\text{pool 소실}}
> $$
...
>
> $$
> \underbrace{R_0}_{\text{기저값}}=\frac{\underbrace{k_{in}}_{\text{생산}}}{\underbrace{k_{out}}_{\text{소실 clock}}}
> $$
...
>
> $$
> \underbrace{k_{in}}_{\text{생산}}=\underbrace{R_0}_{\text{기저값}}\cdot\underbrace{k_{out}}_{\text{소실 clock}}
> $$
...
>
> $$
> \underbrace{\frac{dR}{dt}}_{\text{변화율}}=\underbrace{k_{out}}_{\text{회복 clock}}\left(\underbrace{R_0-R}_{\text{기저-현재 거리}}\right)
> $$
...
<!-- SECTION_CORE: SC-03 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 네 가지 turnover model은 반응 방향이 아니라 $I(C)$·$S(C)$가 $k_{in}$에 붙는지 $k_{out}R$에 붙는지가 결정한다.
...
>
> $$
> \underbrace{I(C)}_{\text{남은 activity}}=1-\underbrace{\frac{\overbrace{I_{max}}^{\text{최대억제}}\,\overbrace{C^n}^{\text{농도·Hill}}}{\underbrace{IC_{50}^n}_{\text{절반농도}}+\underbrace{C^n}_{\text{농도항}}}}_{\text{억제분율}},\quad 0\le I_{max}\le 1
> $$
...
>
> $$
> \underbrace{S(C)}_{\text{촉진배율}}=1+\underbrace{\frac{\overbrace{E_{max}}^{\text{최대촉진}}\,\overbrace{C^n}^{\text{농도·Hill}}}{\underbrace{EC_{50}^n}_{\text{절반농도}}+\underbrace{C^n}_{\text{농도항}}}}_{\text{촉진분율}}
> $$
...
>
> $$
> \begin{aligned}\text{Model 1: }&\underbrace{R_{ss}}_{\text{새 정상상태}}=\underbrace{R_0}_{\text{기저값}}\underbrace{I(C_{ss})}_{\text{억제함수}}\\\text{Model 2: }&\underbrace{R_{ss}}_{\text{새 정상상태}}=\frac{\underbrace{R_0}_{\text{기저값}}}{\underbrace{I(C_{ss})}_{\text{loss 억제}}}\\\text{Model 3: }&\underbrace{R_{ss}}_{\text{새 정상상태}}=\underbrace{R_0}_{\text{기저값}}\underbrace{S(C_{ss})}_{\text{촉진함수}}\\\text{Model 4: }&\underbrace{R_{ss}}_{\text{새 정상상태}}=\frac{\underbrace{R_0}_{\text{기저값}}}{\underbrace{S(C_{ss})}_{\text{loss 촉진}}}\end{aligned}
> $$

> **Annotated equation — 수식 해부**
>
> $$
> \begin{aligned}\underbrace{\Delta R_{M1}}_{\text{M1 변화}}&=\underbrace{R_0}_{\text{기저값}}\underbrace{I_{max}}_{\text{최대억제}}\\\underbrace{\Delta R_{M2}}_{\text{M2 변화}}&=\frac{\underbrace{R_0}_{\text{기저값}}\underbrace{I_{max}}_{\text{최대억제}}}{\underbrace{1-I_{max}}_{\text{잔여 loss}}}\\\underbrace{\Delta R_{M3}}_{\text{M3 변화}}&=\underbrace{R_0}_{\text{기저값}}\underbrace{E_{max}}_{\text{최대촉진}}\\\underbrace{\Delta R_{M4}}_{\text{M4 변화}}&=\frac{\underbrace{R_0}_{\text{기저값}}\underbrace{E_{max}}_{\text{최대촉진}}}{\underbrace{1+E_{max}}_{\text{촉진 보정}}}\end{aligned}
> $$
...
<!-- SECTION_CORE: SC-04 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $t_{ss}$와 peak shift는 강한 triage signal일 뿐이다 — PK clock·dose range·noise·unmodeled nonlinearity를 지우기 전까지 기전 확정으로 쓰지 마라.
...
<!-- SECTION_CORE: SC-05 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $I_{max}$와 $E_{max}$는 이름이 아니라 붙는 scale이 의미를 정한다 — direct $E$와 turnover $R_{ss}$를 같은 거리로 읽지 마라.
...
>
> $$
> \underbrace{E}_{\text{관찰효과}}=\underbrace{E_0}_{\text{기저효과}}+\underbrace{\frac{E_{max}C^n}{EC_{50}^n+C^n}}_{\text{절대효과}}
> $$

> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{E}_{\text{관찰효과}}=\underbrace{E_0}_{\text{기저효과}}\left(1+\underbrace{\frac{E_{max}C^n}{EC_{50}^n+C^n}}_{\text{배율효과}}\right)
> $$

> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{R_{ss}}_{\text{약물 정상상태}}=\underbrace{R_0}_{\text{system 기저값}}\left(1+\underbrace{\frac{E_{max}C^n}{EC_{50}^n+C^n}}_{\text{input 배율}}\right)
> $$
...
<!-- SECTION_CORE: SC-06 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 초기값은 guess가 아니라 식별성 감사다 — $R_0$, $k_{out}$, potency, maximum effect의 자릿수를 plot에서 먼저 잠가라.
...
>
> $$
> \underbrace{\frac{dA}{dt}}_{\text{활성 변화}}=-\underbrace{k_t}_{\text{소실 clock}}\,\underbrace{A}_{\text{잔여 활성}}
> $$
...
>
> $$
> \underbrace{R_{syn}}_{\text{합성속도}}=\underbrace{\frac{A_2-A_1}{\Delta t}}_{\text{관찰변화}}+\underbrace{k_t\frac{A_1+A_2}{2}}_{\text{소실보정}}
> $$
...
<!-- SECTION_CORE: SC-07 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 비가역 효과는 plasma half-life가 아니라 $K_{kill}\cdot C\cdot R$와 target/cell pool 회복 clock이 duration을 지배한다.
...
>
> $$
> \underbrace{\frac{dR}{dt}}_{\text{생존 변화}}=-\underbrace{K_{kill}}_{\text{kill potency}}\,\underbrace{C}_{\text{노출}}\,\underbrace{R}_{\text{잔여 pool}}
> $$
...
>
> $$
> \underbrace{SF}_{\text{생존분율}}=\exp\left(-\underbrace{K_{kill}}_{\text{kill potency}}\cdot\underbrace{AUC}_{\text{누적노출}}\right)
> $$
...
<!-- SECTION_CORE: SC-08 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Turnover와 effect compartment가 같은 curve를 만들 때는 $k_{out}$와 $k_{e0}$의 fit보다 지연의 소속을 증명해야 한다.
...
>
> $$
> \underbrace{\frac{dR}{dt}}_{\text{변화율}}=\underbrace{k_{in}S(C)}_{\text{생산 촉진}}-\underbrace{k_{out}R}_{\text{소실}}
> $$
...
>
> $$
> \underbrace{\frac{dC_e}{dt}}_{\text{effect-site 변화}}=\underbrace{k_{e0}}_{\text{평형 clock}}\left(\underbrace{C-C_e}_{\text{농도차}}\right),\quad \underbrace{R}_{\text{반응}}=\underbrace{f(C_e)}_{\text{effect-site 함수}}
> $$
...
<!-- SECTION_CORE: SC-09 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $t_D$ 공식은 PK-rate-limited일 때만 살아 있다 — slow clock이 PD recovery이면 dose log 공식은 임상적으로 무효다.
...
>
> $$
> \underbrace{Response}_{\text{관찰반응}}=\underbrace{E(0)}_{\text{시작효과}}-\underbrace{m}_{\text{E-logC 기울기}}\,\underbrace{k}_{\text{PK clock}}\,\underbrace{t}_{\text{시간}}
> $$
...
>
> $$
> \underbrace{t_D}_{\text{duration}}=\underbrace{\frac{1}{k}}_{\text{PK clock 역수}}\ln\left(\frac{\underbrace{Dose}_{\text{투여량}}}{\underbrace{C_{min}V}_{\text{최소 유효량}}}\right)=\underbrace{\frac{1}{k}}_{\text{PK clock 역수}}\ln\left(\frac{\underbrace{Dose}_{\text{투여량}}}{\underbrace{A_{min}}_{\text{효과 최소량}}}\right)
> $$
...
>
> $$
> \underbrace{\Delta t_D}_{\text{duration 추가분}}=\frac{\underbrace{\ln 2}_{\text{2배 로그}}}{\underbrace{k}_{\text{PK clock}}}=\underbrace{t_{1/2}}_{\text{PK 반감기}}
> $$
...
<!-- SECTION_CORE: SC-10 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 혼동쌍은 용어 암기가 아니라 clock-location 오류 차단 장치다 — 같은 모양의 식이라도 slow state가 다르면 의사결정이 바뀐다.
...
>
> $$
> \begin{aligned}\text{Reversible: }&\frac{dR}{dt}=\underbrace{k_{in}}_{\text{재생산}}-\underbrace{k_{out}R}_{\text{소실}}\\\text{Irreversible: }&\frac{dR}{dt}=-\underbrace{K_{kill}}_{\text{kill potency}}\underbrace{C}_{\text{노출}}\underbrace{R}_{\text{잔여 target/cell}}\end{aligned}
> $$
...
>
> $$
> \underbrace{t_D}_{\text{duration}}\xrightarrow[\text{PK-rate-limited}]{}\underbrace{\frac{1}{k}\ln\left(\frac{Dose}{A_{min}}\right)}_{\text{PK clock 공식}},\qquad \underbrace{t_D}_{\text{duration}}\xrightarrow[\text{PD-rate-limited}]{}\underbrace{\text{system recovery / target replacement}}_{\text{PD clock}}
> $$
...
<!-- SECTION_CORE: SC-11 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Memory hook의 목적은 $R_0$, $R_{ss}$, rebound, duration, AUEC, identifiability를 각각 다른 질문으로 분리시키는 것이다.
...
>
> $$
> \underbrace{k_{in}}_{\text{수원 속도}}=\underbrace{k_{out}}_{\text{배수 clock}}\cdot\underbrace{baseline}_{\text{pool 크기}},\quad \underbrace{k_{out}}_{\text{반응 clock}}=\frac{1}{\underbrace{MRT_{response}}_{\text{평균 체류시간}}},\quad \underbrace{t_{1/2,response}}_{\text{반응 반감기}}=\frac{0.693}{\underbrace{k_{out}}_{\text{소실 clock}}}
> $$
...
>
> $$
> \underbrace{\Delta WRSS}_{\text{fit 차이}}\approx 2,\qquad \underbrace{k_{out}}_{\text{turnover clock}}\approx\underbrace{k_{e0}}_{\text{effect-site clock}}\approx 5.6\;h^{-1}
> $$
...
<!-- SECTION_CORE: SC-12 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Self-test 전반부는 공식 암기가 아니라 Model 1·$t_{ss}$·peak shift·사례 anchor를 동시에 검증하는 장치다.
...
>
> $$
> \underbrace{\frac{dR}{dt}}_{\text{변화율}}=\underbrace{k_{in}I(C)}_{\text{production 억제}}-\underbrace{k_{out}R}_{\text{소실}}
> $$
...
>
> $$
> \underbrace{R_{ss}}_{\text{약물 정상상태}}=\underbrace{R_0}_{\text{기저값}}\underbrace{I(C_{ss})}_{\text{남은 production}}
> $$
...
<!-- SELF-TEST -->
<!-- SECTION_CORE: SC-13 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Self-test 후반부는 wrong clock과 비식별성 앞에서 $t_D$, model family, sensitivity argument를 함께 말하게 만든다.
...
<!-- SECTION_CORE: SC-14 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 최종 mastery는 hysteresis 방향 → action site → $t_{ss}$ → identifiability → PK-rate-limited $t_D$ 순서로 clock을 잠그는 능력이다.
