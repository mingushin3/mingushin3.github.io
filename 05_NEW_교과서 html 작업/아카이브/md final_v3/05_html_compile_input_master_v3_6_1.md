...
<!-- SECTION_CORE: SC-01 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 2구획 모델은 $A,\alpha,B,\beta$ 곡선을 $CL,Q,V_1,V_2$ 임상 언어로 번역하는 좌표계 훈련이다 — 이 연결을 놓치면 curve가 dosing 판단으로 넘어가지 못한다.
...
>
> $$
> \underbrace{AUC}_{\text{총노출}}=\frac{\underbrace{Dose}_{\text{용량}}}{\underbrace{CL}_{\text{CL}}},\qquad \underbrace{V_d}_{\text{Vd}}=\frac{\underbrace{CL}_{\text{CL}}}{\underbrace{k}_{\text{소실률}}}
> $$
...
<!-- SECTION_CORE: SC-02 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $A,\alpha,B,\beta$는 두 제거경로가 아니라 2×2 ODE가 만든 phase 분해다 — AUC·AUMC·$f_2$로 terminal 해석 가능성을 먼저 판정하라.
...
>
> $$
> \underbrace{C(t)}_{\text{혈장농도}}=\overbrace{\underbrace{A}_{\text{α절편}}\underbrace{e^{-\alpha t}}_{\text{α감소}}}^{\alpha\text{상 기여}}+\overbrace{\underbrace{B}_{\text{β절편}}\underbrace{e^{-\beta t}}_{\text{β감소}}}^{\beta\text{상 기여}},\qquad \underbrace{\alpha>\beta>0}_{\text{α>β}}
> $$
...
>
> $$
> \underbrace{C_{resid}(t)}_{\text{초기잔차}}=\underbrace{C_{obs}(t)}_{\text{관측값}}-\overbrace{\underbrace{B}_{\beta\text{ 절편}}e^{-\underbrace{\beta}_{\text{β slope}}t}}^{\text{말단 외삽}}
> $$
...
>
> $$
> \underbrace{\alpha}_{\text{빠른 상 slope}}=\frac{\overbrace{\ln(70/10)}^{\text{농도비 로그}}}{\underbrace{130}_{\text{시간간격}}},\qquad \underbrace{\beta}_{\text{느린 상 slope}}=\frac{\overbrace{\ln(28/10)}^{\text{농도비 로그}}}{\underbrace{450}_{\text{시간간격}}}
> $$
...
>
> $$
> \begin{aligned}\underbrace{AUC_0^\infty}_{\text{총노출}}&=\overbrace{\frac{\underbrace{A}_{\alpha\text{ 절편}}}{\underbrace{\alpha}_{\alpha\text{ 감소속도}}}}^{\text{빠른 상 면적}}+\overbrace{\frac{\underbrace{B}_{\beta\text{ 절편}}}{\underbrace{\beta}_{\beta\text{ 감소속도}}}}^{\text{느린 상 면적}},\\[2mm]\underbrace{AUMC_0^\infty}_{\text{시간가중노출}}&=\overbrace{\frac{A}{\alpha^2}}^{\text{빠른 상 시간가중}}+\overbrace{\frac{B}{\beta^2}}^{\text{느린 상 시간가중}}\end{aligned}
> $$
...
>
> $$
> \underbrace{t_{1/2,\alpha}}_{\text{빠른 상 반감기}}=\frac{\overbrace{\ln2}^{\text{반감상수}}}{\underbrace{\alpha}_{\text{빠른 상 slope}}},\qquad \underbrace{t_{1/2,\beta}}_{\text{느린 상 반감기}}=\frac{\overbrace{\ln2}^{\text{반감상수}}}{\underbrace{\beta}_{\text{느린 상 slope}}}
> $$
...
>
> $$
> \underbrace{f_1}_{\alpha\text{상 AUC분율}}=\frac{\overbrace{A/\alpha}^{\alpha\text{ 상 AUC}}}{\underbrace{AUC}_{\text{총노출}}},\qquad \underbrace{f_2}_{\beta\text{상 AUC분율}}=\frac{\overbrace{B/\beta}^{\beta\text{ 상 AUC}}}{\underbrace{AUC}_{\text{총노출}}},\qquad \underbrace{f_1+f_2=1}_{\text{AUC 합=1}}
> $$
...
>
> $$
> \underbrace{\frac{dA_1}{dt}}_{\text{중앙 변화율}}=-\overbrace{\underbrace{(k_{12}+k_{10})}_{\text{말초+제거}}\underbrace{A_1}_{\text{중앙량}}}^{\text{중앙 유출}}+\overbrace{\underbrace{k_{21}}_{\text{말초→중앙}}\underbrace{A_2}_{\text{말초량}}}^{\text{중앙 회귀}}
> $$
...
>
> $$
> \underbrace{\frac{dA_2}{dt}}_{\text{말초 변화율}}=\overbrace{\underbrace{k_{12}}_{\text{중앙→말초}}\underbrace{A_1}_{\text{중앙량}}}^{\text{말초 유입}}-\overbrace{\underbrace{k_{21}}_{\text{말초→중앙}}\underbrace{A_2}_{\text{말초량}}}^{\text{중앙 회귀}}
> $$
...
>
> $$
> \underbrace{\alpha+\beta}_{\text{고유값 합}}=\underbrace{k_{12}}_{\text{중앙→말초}}+\underbrace{k_{21}}_{\text{말초→중앙}}+\underbrace{k_{10}}_{\text{중앙 제거}},\qquad \underbrace{\alpha\beta}_{\text{고유값 곱}}=\underbrace{k_{21}k_{10}}_{\text{회귀×제거}}
> $$
...
<!-- SECTION_CORE: SC-03 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Macro는 curve를 설명하고 micro는 ODE를 닫으며 physiological parameter는 보고와 covariate 해석을 책임진다 — central elimination은 데이터가 아니라 가정임을 명시하라.
...
>
> $$
> \underbrace{CL}_{\text{전신 CL}}=\underbrace{k_{10}}_{\text{중앙 제거율}}\underbrace{V_c}_{\text{중앙 V}}
> $$
...
>
> $$
> \underbrace{Cld}_{\text{분포 CL}}=\underbrace{k_{12}}_{\text{중앙→말초}}\underbrace{V_c}_{\text{중앙 V}}=\underbrace{k_{21}}_{\text{말초→중앙}}\underbrace{V_t}_{\text{말초 V}}
> $$
...
>
> $$
> \underbrace{V_c}_{\text{중앙 V}}=\frac{\underbrace{D_{iv}}_{\text{IV 용량}}}{\underbrace{A+B}_{\text{초기 }C_0}}
> $$
...
>
> $$
> \underbrace{k_{21}}_{\text{말초→중앙}}=\frac{\overbrace{\underbrace{A}_{\alpha\text{ 절편}}\underbrace{\beta}_{\text{느린 slope}}+\underbrace{B}_{\beta\text{ 절편}}\underbrace{\alpha}_{\text{빠른 slope}}}^{\text{절편가중 slope}}}{\underbrace{A+B}_{C_0}}
> $$
...
>
> $$
> \underbrace{k_{10}}_{\text{중앙 제거율}}=\frac{\overbrace{\alpha\beta}^{\text{고유값 곱}}}{\underbrace{k_{21}}_{\text{말초→중앙 전이속도}}}
> $$
...
>
> $$
> \underbrace{k_{12}}_{\text{중앙→말초}}=\overbrace{\alpha+\beta}^{\text{slope 합}}-\underbrace{k_{21}}_{\text{말초→중앙}}-\underbrace{k_{10}}_{\text{중앙 제거}}
> $$
...
<!-- SECTION_CORE: SC-04 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $V_1$, $V_{ss}$, $V_\beta$는 서로 다른 시간 질문의 답이다 — loading dose에 $V_\beta$를 넣는 순간 terminal geometry가 체내량으로 둔갑한다.
...
>
> $$
> \underbrace{V_1}_{\text{초기희석}}=\frac{\underbrace{Dose}_{\text{용량}}}{\underbrace{A+B}_{C_0}},\qquad \underbrace{V_{ss}}_{\text{SS 체내량/C}}=\underbrace{MRT}_{\text{MRT}}\cdot\underbrace{CL}_{\text{CL}}=\frac{\underbrace{Dose}_{\text{용량}}\underbrace{AUMC}_{\text{시간가중노출}}}{\underbrace{AUC^2}_{\text{AUC 제곱}}},\qquad \underbrace{V_\beta}_{\text{말단 V}}=\frac{\underbrace{CL}_{\text{CL}}}{\underbrace{\beta}_{\text{β slope}}}=\frac{\underbrace{Dose}_{\text{용량}}}{\underbrace{AUC}_{\text{노출}}\underbrace{\beta}_{\text{β slope}}}
> $$
...
>
> $$
> \underbrace{V_1}_{\text{초기희석}}\le \underbrace{V_{ss}}_{\text{분포평형}}\le \underbrace{V_\beta}_{\text{말단 V}}
> $$
...
>
> $$
> \underbrace{t_{1/2,\beta}}_{\text{말단 반감기}}=\frac{\overbrace{0.693}^{\ln 2}}{\underbrace{\beta}_{\text{β slope}}},\qquad \underbrace{V_\beta}_{\text{말단 V}}=\frac{\underbrace{CL}_{\text{CL}}}{\underbrace{\beta}_{\text{β slope}}}
> $$
...
<!-- SECTION_CORE: SC-05 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Reparameterization은 표기 치장이 아니라 estimation geometry 조정이다 — WRSS보다 condition number, RSE, covariance 안정성을 함께 보라.
...
<!-- SECTION_CORE: SC-06 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Plateau와 dosing interval은 terminal $t_{1/2}$ 하나로 닫히지 않는다 — $f_1$, $f_2$, $\alpha$, $\beta$가 만든 접근률을 먼저 계산하라.
...
>
> $$
> \underbrace{\frac{C(t)}{C_{ss}}}_{\text{SS 접근률}}=\overbrace{\underbrace{f_1}_{\alpha\text{ 상 기여}}\left(1-e^{-\alpha t}\right)}^{\text{빠른 접근}}+\overbrace{\underbrace{f_2}_{\beta\text{ 상 기여}}\left(1-e^{-\beta t}\right)}^{\text{느린 접근}}
> $$
...
<!-- SECTION_CORE: SC-07 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 2구획의 치명적 오류는 phase, volume, 좌표계, half-life, fit 품질을 서로 바꾸어 읽는 데서 시작된다 — 혼동쌍을 여기서 끊어라.
...
>
> $$
> \underbrace{\frac{V_\beta}{V_{ss}}}_{\text{말단V 오용}}\approx \frac{\underbrace{345}_{\text{GM }V_\beta}}{\underbrace{56}_{\text{GM }V_{ss}}}\approx 6.2
> $$
...
<!-- SECTION_CORE: SC-08 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 자가검증의 목표는 계산 정답이 아니라 reviewer 질문 방어다 — CL bias, volume ratio, 식별성, shrinkage, condition number를 즉시 판정하라.
...
>
> $$
> \underbrace{\left(\frac{V_\beta}{V_{ss}}\right)_{aspirin}}_{\text{ASA 말단부풀림}}\approx\frac{13.7}{10.4}\approx1.3,\qquad \underbrace{\left(\frac{V_\beta}{V_{ss}}\right)_{gentamicin}}_{\text{GM 말단부풀림}}\approx\frac{345}{56}\approx6.2
> $$
...
>
> $$
> \underbrace{\text{plateau 접근}}_{C(t)/C_{ss}}\neq \underbrace{\text{말단반감기 단독}}_{\beta\text{만 보는 해석}},\qquad \underbrace{C(t)/C_{ss}}_{\text{접근률}}=\underbrace{f_1}_{\alpha\text{ 기여}}(1-e^{-\alpha t})+\underbrace{f_2}_{\beta\text{ 기여}}(1-e^{-\beta t})
> $$
...
<!-- SECTION_CORE: SC-09 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 세션의 최종 실무 알고리즘은 semilog curve에서 시작해 volume misuse와 sparse design risk를 거쳐 deployable PopPK 판단으로 닫힌다.
