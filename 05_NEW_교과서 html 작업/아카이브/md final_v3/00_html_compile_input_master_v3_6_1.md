...
<!-- SECTION_CORE: SC-01 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $M1$–$M4$와 혼동쌍·자기 테스트는 용어 암기를 PopPK/PK-PD 판단 언어로 바꾸는 진입로다.
...
<!-- SECTION_CORE: SC-02 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $M1$ workflow, $M2$ linkage, $M3$ direct PD, $M4$ turnover를 먼저 잠가야 후속 모델링 판단이 흔들리지 않는다.
...
<!-- SECTION_CORE: SC-03 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $Cl$, $V$, $K$, AUC를 만지기 전에 fitting이 Step 5라는 사실을 몸에 새겨라 — EDA가 구조와 초기값을 만든다.
...
>
> $$
> \underbrace{K}_{\text{소실 속도}}=\frac{\underbrace{Cl}_{\text{정화 부피/시간}}}{\underbrace{V}_{\text{분포 공간}}},\quad
> \underbrace{AUC}_{\text{총 노출}}=\frac{\underbrace{D}_{\text{투여량}}}{\underbrace{Cl}_{\text{정화 부피/시간}}},\quad
> \underbrace{t_{1/2}}_{\text{반감기}}=\frac{\ln(2)}{\underbrace{K}_{\text{소실 속도}}}
> $$
...
>
> $$
> \underbrace{\hat C}_{\text{예측 농도}} = \underbrace{\frac{D}{V}}_{\text{초기 스케일}}\;\underbrace{\exp\left(-\underbrace{\frac{Cl}{V}}_{\text{소실 속도}}t\right)}_{\text{시간 감소}}
> $$
...
>
> $$
> \underbrace{C}_{\text{관측 농도}} = \underbrace{\hat C}_{\text{모델 예측}} + \underbrace{\varepsilon}_{\text{잔차}}
> $$
...
<!-- SECTION_CORE: SC-04 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** GOF·수렴·초기값·figure pointer는 모두 같은 질문으로 모인다 — 이 모델이 예측 가능한가, 아니면 보기 좋게 맞았을 뿐인가.
...
<!-- SECTION_CORE: SC-05 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Dose regimen은 dose 숫자가 아니라 exposure-time과 response-time으로 번역될 때만 치료역 안에서 의미를 갖는다.
...
<!-- SECTION_CORE: SC-06 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $E_{\max}$, $C_{50}$, $\gamma$는 효과의 천장·효능·가파름을 분리하는 직접 효과 PD의 세 축이다.
...
>
> $$
> \underbrace{E}_{\text{예측 효과}} = \frac{\overbrace{E_{\max}}^{\text{효과 천장}}\;\overbrace{C^{\gamma}}^{\text{농도 구동}}}{\underbrace{C_{50}^{\gamma}}_{\text{효능 기준}}+\underbrace{C^{\gamma}}_{\text{현재 농도}}}
> $$
...
>
> $$
> \underbrace{C_{\max}=96\,\mu g/L}_{\text{최고 농도}},\quad
> \underbrace{t_{\max}=3.0\,hr}_{\text{도달 시간}},\quad
> \underbrace{AUC}_{\text{총 노출}}
> $$
...
>
> $$
> \underbrace{\gamma=1}_{\text{완만}}:\quad \underbrace{C_{20}=0.25C_{50}}_{\text{20\% 농도}},\; \underbrace{C_{80}=4C_{50}}_{\text{80\% 농도}}
> $$
...
>
> $$
> \underbrace{\gamma=2}_{\text{가파름}}:\quad \underbrace{C_{20}=0.5C_{50}}_{\text{20\% 농도}},\; \underbrace{C_{80}=2C_{50}}_{\text{80\% 농도}}
> $$
...
<!-- SECTION_CORE: SC-07 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Propranolol·alfentanil·측정 매트릭스는 $E_{\max}$ 식보다 먼저 endpoint type과 활성 농도를 확인하라고 가르친다.
...
<!-- SECTION_CORE: SC-08 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $R_t$, $A_{ss}$, $k_t$, $t_t$를 분리해야 농도 peak와 효과 peak가 어긋나는 지연 반응을 turnover 언어로 읽는다.
...
>
> $$
> \underbrace{k_t}_{\text{분획 속도}} = \frac{\underbrace{R_t}_{\text{전환량/시간}}}{\underbrace{A_{ss}}_{\text{풀 크기}}}
> $$
...
>
> $$
> \underbrace{t_t}_{\text{전환 시간}} = \frac{\underbrace{A_{ss}}_{\text{풀 크기}}}{\underbrace{R_t}_{\text{전환량/시간}}}
> $$
...
>
> $$
> \underbrace{t_t}_{\text{전환 시간}} = \frac{1}{\underbrace{k_t}_{\text{분획 속도}}}
> $$
...
<!-- SECTION_CORE: SC-09 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $M1$–$M4$를 혼동쌍에 대입해 보라 — 비슷한 단어를 분리하지 못하면 모델링 판단도 함께 무너진다.
...
<!-- SECTION_CORE: SC-10 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Q1–Q4는 workflow·NCA·therapeutic window·digoxin/morphine을 말로 재생해 용량 사고의 뼈대를 검증한다.
...
<!-- SECTION_CORE: SC-11 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Q5–Q9는 $\gamma$, endpoint type, delayed response, $k_t/t_t$, exposure metric을 골라내는 실전 판단력을 검증한다.
...
<!-- SECTION_CORE: SC-12 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 마지막 메타프레임은 새 데이터셋 앞에서 $M1$–$M4$ 중 무엇을 먼저 켜야 하는지 결정하는 전문가용 체크포인트다.
