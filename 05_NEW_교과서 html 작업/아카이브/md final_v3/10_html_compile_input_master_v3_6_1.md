...
<!-- SECTION_CORE: SC-01 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $K_d$, $EC_{50}$, Hill $n$, hysteresis, AUEC를 한 흐름으로 묶어라 — PD 판단은 potency와 시간 지연을 동시에 읽을 때 시작된다.
...
<!-- SECTION_CORE: SC-02 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $K_d$는 결합층, $EC_{50}$는 기능층이다 — 이 둘을 섞으면 $E_{max}$ 해석과 공변량 판단이 처음부터 무너진다.
...
<!-- SECTION_CORE: SC-03 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $E_{max}$, $EC_{50}$, Hill $n$을 분리해라 — 천장·중간점·가파름이 함께 용량 조절 안전 마진을 결정한다.
...
<!-- SECTION_CORE: SC-04 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Hill $n$은 농도 범위와 고원부 증거 없이는 방어되지 않는다 — 곡률을 보려면 설계가 먼저 곡률을 건드려야 한다.
...
<!-- SECTION_CORE: SC-05 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** hysteresis loop는 잡음이 아니라 직접 혈장-효과 연결의 실패 신호다 — 고리의 방향과 원천을 먼저 읽어라.
...
<!-- SECTION_CORE: SC-06 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $C_e(t)$ 지연과 turnover 지연을 구분해라 — effect compartment와 indirect response는 서로 다른 외삽 인과를 만든다.
...
<!-- SECTION_CORE: SC-07 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** AUEC와 duration은 PK AUC의 그림자가 아니다 — 효과 면적과 지속 시간은 반응 시스템의 시간척도가 지배한다.
...
>
> $$
> \underbrace{\frac{\tau_{drug}}{\tau_{system}}}_{\text{약물/시스템 시간비}}
> $$
...
<!-- SECTION_CORE: SC-08 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $K_d/EC_{50}$, ordinary/sigmoid, PK/PD 율속, direct/indirect를 갈라라 — 같은 모양의 용어가 다른 결정을 요구한다.
...
<!-- SECTION_CORE: SC-09 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 자기 테스트는 계산 훈련이 아니다 — 어떤 가정을 버리고 어떤 delay 구조를 선택할지 말로 증명해라.
...
<!-- SECTION_CORE: SC-10 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 마지막 기준은 source discipline이다 — 출처 안의 개념과 실무 확장을 분리해야 v3.6.1 전체가 방어된다.
