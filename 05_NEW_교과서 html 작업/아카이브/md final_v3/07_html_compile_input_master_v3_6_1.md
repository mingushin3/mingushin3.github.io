...
<!-- SECTION_CORE: SC-01 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $C_{ss}$·$R_{ac}$·$C_{av,ss}$를 하나의 정상상태 공식으로 뭉개지 마라 — 이 파일은 시간·수준·진폭·임상제약을 갈라 읽는 항법 지도다.
...
<!-- SECTION_CORE: SC-02 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 항정상태를 “도달했는가”로만 묻지 마라 — $t_{1/2}$는 시간, $CL$은 수준, $\tau/t_{1/2}$는 진폭을 각각 지배한다.
...
<!-- SECTION_CORE: SC-03 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** M1–M10은 PK clock, dosing-design bridge, PD/safety clock으로 나눠 읽어라 — 같은 steady state라도 시계가 다르면 판단이 달라진다.
...
<!-- SECTION_CORE: SC-04 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** §2의 각 카드는 하나의 지배 질문만 잠근다 — 지금부터 $CL$, $t_{1/2}$, $R_{ac}$, $C_{av,ss}$를 서로 바꿔 쓰는 습관을 끊어라.
...
<!-- SECTION_CORE: SC-05 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Plateau 도달 시간은 목표 농도 높이가 아니라 $K$와 $t_{1/2}$가 정한다 — 3–4 half-lives 규칙은 수준이 아니라 비율의 시간표다.
...
<!-- SECTION_CORE: SC-06 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $R_{ac}$는 dose가 아니라 $\tau/t_{1/2}$의 함수다 — 축적 배율과 절대 노출 수준을 섞는 순간 regimen 조정 방향이 뒤집힌다.
...
<!-- SECTION_CORE: SC-07 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $C_{av,ss}$는 농도이고 $A_{av,ss}$는 amount다 — $V$ bridge 없이 두 평균 plateau를 등치하면 차원 오류가 생긴다.
...
<!-- SECTION_CORE: SC-08 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Terminal slope를 곧바로 elimination으로 번역하지 마라 — $K_a<K$이면 tail은 흡수/input 시계가 쥔다.
...
<!-- SECTION_CORE: SC-09 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Therapeutic window는 PK 식의 장식이 아니라 $C_{upper}$·$C_{lower}$·benefit–harm을 regimen 위에 거는 임상 제약이다.
...
<!-- SECTION_CORE: SC-10 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Loading dose는 시작 위치, maintenance dose는 장기 입력률이다 — $V/F$와 $CL$의 역할을 한 dose 규칙으로 합치지 마라.
...
<!-- SECTION_CORE: SC-11 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** TW 알고리즘은 $C_{upper}/C_{lower}$를 $\tau_{max}$와 $D_{M,max}$로 번역하는 feasibility screen이다 — 최종 처방 자동기가 아니다.
...
<!-- SECTION_CORE: SC-12 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** PK plateau와 PD·toxicity plateau는 같은 시계가 아니다 — turnover, recovery time, fractionation을 보지 않으면 효과와 위해를 잘못 읽는다.
...
<!-- SECTION_CORE: SC-13 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** §5 혼동 쌍은 기호 암기가 아니라 결정자 분리 훈련이다 — $C_{ss}$, SS flag, flip-flop, MRT를 같은 언어로 처리하지 마라.
...
<!-- SECTION_CORE: SC-14 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 자기 테스트는 계산 정답보다 조건 선언이 먼저다 — 어떤 식이 어느 가정에서만 유효한지 말해야 regimen 판단이 방어된다.
...
<!-- SECTION_CORE: SC-15 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 전문가 해자는 복잡한 식의 양이 아니다 — 시간·수준·진폭·임상제약을 즉시 분해하는 사고 습관이 해자다.
