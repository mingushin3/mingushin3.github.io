# 11_HTML Compile Input Master — v3 (Memory-Hooks-and-Boss-Dilemma-Strengthened)
...
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A는 §1–§8 학습자 본문만 포함; audit/process/compiler 자료는 PART B에 격리. v3 추가 사항(hidden-state 진단 줄, §5 Memory Hooks, Boss Dilemma, Practice Briefs)도 모두 PART A 학습자 본문 안에 위치 — PART B에 새 가드레일 항목만 추가. |
| Zero-Omission Certificate | PASS | v2 Crucible Grade A 5종 모두 PRESENT 유지. **v3 추가**: Independent Reviewer audit 4 gap (Memory Hooks, Boss Dilemma, Pool 1/2 본문 이동, hidden-state thread) 모두 PRESENT. HOLD_UNRESOLVED 없음. |
| Mastery-Uplift Certificate | PASS | v2의 8개 Mastery Anchor 위에 **v3에서 4종 학습 보강 layer**를 추가(8개 hidden-state 진단 줄, 7쌍 §5 Memory Hooks, 보스 딜레마, Card 7 Pool 1/2 명시 문단, 8개 Practice Brief). 모두 bounded·adjacent·source-labeled. v2 본문 재작성은 수행되지 않음. |
| Source-Boundary Certificate | PASS | 모든 v3 추가는 [EXPERT_INFERENCE, v3] 라벨로 명시. 새 drug example·page tag·외부 fact 도입 없음 — 모든 v3 prose는 v2 본문에 이미 존재하는 사실의 *재구조화* 또는 *비유적 인코딩*에 한정. |
| HTML-Readiness Certificate | PASS | PART B B1-B12 + 신규 B13(v2→v3 변경 로그) 완비. Marker·page tag·navigation anchor·responsive/print·forbidden restoration 규칙 보존. Card 7 헤더의 [⚡ Apex Concept] 배지는 B8 Splice Verification Table 및 Insertion Map에 동기화됨. |

## Assembly Mode

PATCH MODE (v3 유지)

Rationale: `11_Content Lock v2.1.md`는 figure marker insertion plan이며, learner-facing canonical body는 `11_Content Lock v2.md`에서 가져옴. 6개의 marker block이 지정 위치에 splice됨. v3는 v2의 splice 결과를 변경하지 않는다(B8 Splice Verification Table은 Card 7 anchor 텍스트 동기화만 갱신; 6개 marker block 위치·내용 동일).

## Input Manifest (v3 추가 행만 표시; 나머지는 v2와 동일)

| File | Role | Authority level | Used for | Notes |
|---|---|---|---|---|
| `11_html_compile_input_master_ver2.md` (v2) | v3 base substrate | A8 | v3의 모든 §1-§8 본문은 v2에서 그대로 carry-over | v3는 4 gap fix layer만 확장 |
| Independent Master Reviewer audit (v3 audit) | v3 gap-fix authority | A6 | 4 gap (Memory Hooks·Boss Dilemma·Pool 1/2 본문·hidden state thread) 식별 | v3 patches 1-7의 근거 |
| `11_html_compile_input_master.md` (v1) | v2의 base substrate | A8 (carry-over) | v2의 모든 §1-§8 본문은 v1에서 carry-over | v2는 v1 위에 누적 작업, v3는 v2 위에 누적 작업 |
| (v1·v2의 Input Manifest 모든 항목 carry-over) | — | — | — | v3는 v1·v2 위에 누적적으로 작업 |
...
- **v3 추가**: 모든 v3 layer(hidden-state 진단 줄 8개, §5-6 Memory Hooks 7쌍, ⚡ 보스 딜레마, Card 7 Pool 1/2 명시 문단, Practice Brief 8개)는 [EXPERT_INFERENCE, v3] 라벨로 명시한다. 새 drug example·page tag·외부 fact·NONMEM 코드 도입 금지. 비유적 인코딩(예: "수원 vs 배수구", "전등 vs 전기장판", "지도가 둘 다 정확 vs 지도가 틀림")은 직관 강화 목적의 *인코딩*이지 새 사실 도입이 아니다.
- **v3 추가 — Card 7 헤더 동기화 규칙**: Card 7의 H2 헤더 텍스트는 `## Card 7 — [⚡ Apex Concept] Turnover vs Effect Compartment Discrimination Crisis`로 표준화되었다. B8 Splice Verification Table 및 Insertion Map의 Card 7 anchor 텍스트도 동일 텍스트로 동기화되어 있어야 splice가 깨지지 않는다.
