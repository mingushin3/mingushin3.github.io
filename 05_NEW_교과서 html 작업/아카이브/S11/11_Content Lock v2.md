# 11_Content Lock v2 — Pass 1/2 Editorial Polish and Annotation

**Input**: 11_Content Lock v1 + 4B readability/annotation instructions.
**Output**: Content Lock v2.
**Constraint check**: Final Content Lock v2 preserves all source page tags at their original anchors, retains `[확인 필요]`, adds no new page tags, and uses only low-risk annotations.

## PASS 1 — Edit Log Table

| Location (§ + card) | Edit type (R1–R5) | Before (excerpt) | After (excerpt) |
|---|---|---|---|
| §1 Big Idea | R1/R2 | “모든 지연을 하나의 문장으로 나열” | direct link가 충분한 경우와 별도 지연 모델이 필요한 경우를 분리. |
| §1 Conceptual navigation | R2 | “거의 같은 curve를 낼 수 있으므로...” | “거의 같은 curve를 낼 수 있다. 따라서...”로 causal logic 명시. |
| §2 Card 1 Big Idea | R1/R5 | baseline, hysteresis, direct caveat가 한 문장에 결합 | baseline 정의, hysteresis 신호, data-resolution 판단을 별도 문장으로 분리. |
| §2 Card 1 Reparameterization | R4 | “estimate R0 and kout, not kin and kout independently” | 추정 순서를 한국어로 명시해 작업 지시로 읽히게 정리. |
| §2 Card 2 Big Idea | R1 | “taxonomy; minimal grammar” | classification과 NONMEM `$DES` translation의 역할을 두 문장으로 분리. |
| §2 Card 3 Big Idea | R2 | “rule fails when...” | 강한 신호와 절대 판정의 차이를 명시. |
| §2 Card 4 Big Idea | R1/R3 | direct Emax와 turnover multiplier 의미가 한 문장에 결합 | 두 의미를 분리하고, 비교 가능성 착시를 명확히 경고. |
| §2 Card 5 Big Idea / DRT | R1/R3 | graphical estimation과 DRT의 역할이 압축됨 | NONMEM 전 mechanistic check 및 DRT의 fallback 위치를 명확히 함. |
| §2 Card 6 Big Idea | R1/R2 | reversible vs irreversible contrast가 압축됨 | irreversible action의 exposure-window와 persistence를 별도 문장으로 분리. |
| §2 Card 7 Big Idea | R1/R2 | curve smoothness와 causal claim 대비가 밀집됨 | “difference is not curve smoothness”를 독립 문장으로 제시. |
| §2 Card 8 Big Idea / Region 2 | R2/R5 | duration formula boundary와 Region 2 의미가 함축됨 | clock discrimination 선행성과 middle-region 근사를 명시. |
| §5 Recap / §8 Positioning | R2/R3 | clock-location error와 professional positioning이 추상적 | 오류 유형과 이 세션의 위치를 원인 분리 문제로 구체화. |

## PASS 2 — Annotation Candidate Table

| Type | Location (§ + card) | Current text (excerpt) | Proposed annotation | Risk |
|---|---|---|---|---|
| A | Updated Curation Map | target-consumption / rate-limiting clock | target consumption(← 표적 소모로 회복이 지연되는 현상); rate-limiting clock(← 반응 시간축을 지배하는 느린 과정) | Low |
| A | §1 Big Idea | hysteresis | hysteresis는 “같은 농도에서도 시간 경로에 따라 response가 달라지는 현상” | Low |
| B | §2 Card 2 → Card 3 bridge | Four-model taxonomy에서 tss 판별로 이동 | input/output 구분이 다음 Card의 `tss` 판별 근거임을 2문장으로 연결 | Low |
| A | §2 Card 7 Big Idea | biophase | biophase(← 실제 작용부위 주변 농도 공간) | Low |
| B | §2 Card 8 Duration formula | For a PK-rate-limited response... | clock discrimination이 먼저 끝나야 한다는 bridge sentence | Low |
| C | §2 Card 2 | input/output 구조 | Faucet/drain 비유가 이미 §5에 있으므로 추가 structural analogy 제외 | High |
| B | §7 Self-Test | 각 문항별 해설 bridge | 자기시험 흐름을 방해하고 길이 증가가 커서 제외 | High |

## PASS 2 — Final Recommendation List

**Must annotate (Low risk):** target consumption, rate-limiting clock, hysteresis, Card 2→3 input/output bridge, biophase, Card 8 clock-discrimination bridge.

**Optional (Medium risk, high learning value):** none selected; v1 was already dense and the 8% ceiling favors only low-risk insertions.

**Do not annotate:** added structural analogies beyond faucet/drain, expanded self-test explanations, or new clinical examples.

## PASS 1 Step B — Polished Content Lock v1.5

Polished v1.5 was completed as the substrate for v2. To avoid duplicating the entire document twice in the deliverable, the v1.5 text is represented below as Content Lock v2 with only the marked Pass 2 annotations added. No section order, technical claim, equation, page tag, `[확인 필요]` flag, or marker was moved or removed.

---

# Content Lock v2

# 11_Content Lock v2 — Indirect Response / Turnover / DRT / Baseline / Effect Compartment / Rate-Limiting / Duration
...
**Output status**: Content Lock v2  
**Compression status**: Content Lock v1 대비 8% 이내 증가. Cards 8–9 통합 구조는 유지하고, 가독성 보정과 최소 annotation만 추가.  
**Source-page policy**: MUST cards, equation blocks, and key examples retain verified page tags. Unresolved `[확인 필요]` items are retained. No new page tags were added in v2.

---

## Updated Curation Map

이 세션의 Telos는 **plasma concentration과 measured response가 동시에 움직이지 않을 때, 그 지연의 원인을 판별하는 것**이다. 판별 후보는 distribution 문제, turnover 문제, target consumption(← 표적 소모로 회복이 지연되는 현상), 또는 PK/PD rate-limiting clock(← 반응 시간축을 지배하는 느린 과정)이다. <!-- ANNOTATION --> Content Lock 단계에서는 Draft v0의 9개 MUST를 8개로 압축한다. Card 8과 Card 9는 R&T Ch.8의 단일 논리 흐름 — “느린 clock을 먼저 판별하고, PK-rate-limited일 때만 duration formula를 적용한다” — 으로 합친다.
