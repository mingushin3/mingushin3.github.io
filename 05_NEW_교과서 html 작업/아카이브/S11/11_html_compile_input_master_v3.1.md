...
# 11_HTML Compile Input Master — v3.1 (Korean-Readability-Patched)

## Version Note

**v3 → v3.1 변경 사유**: v3는 Independent Master Reviewer audit의 4 gap을 외과적으로 메우고 All-Pass 판정을 받았으나, 35건의 PART A 학습자-facing 영문 산문이 한국어 독해 흐름을 끊는다는 점이 v3.1 readability audit에서 식별되었다 — Big Idea 6개 카드 오프너, §7 Q1–Q9 질문 본문, §8 산문, Card 내 Recap·Trench·Structural 영문 블록, Learner Navigation 영문 안내. v3.1은 v3 본문(fact·equation·page tag·marker·Apex·Critical Blow·Mastery Anchor·Boss Dilemma·Memory Hooks·Practice Brief·hidden-state thread)을 100% 무손상 유지한 채로, 영문 산문을 같은 의미의 한국어 산문으로 교체하는 35건 surgical patch만 적용한다. 모든 v3.1 patch는 Low risk이며 Korean flow improvement 단일 change-type에 한정된다. 새 fact·수치·예시·page tag·외부 reference·figure marker는 도입되지 않는다.
...
| Certificate | Status | Basis (v3.1) |
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A는 §1–§8 학습자 본문만 포함; audit/process/compiler 자료는 PART B에 격리. v3.1 readability patch는 모두 PART A 학습자 본문 안에서 영문→한국어 산문 교체로만 작동 — 새 가드레일 도입 없음. |
| Zero-Omission Certificate | PASS | v3 Crucible Grade A 5종 + v3 audit 4 gap fix 모두 PRESENT 유지. v3.1 patch는 정보 추가·삭제·이동 없이 surface form만 한국어 친화적으로 변환. HOLD_UNRESOLVED 없음. |
| Mastery-Uplift Certificate | PASS | v3의 8개 Mastery Anchor + 7쌍 Memory Hook + Boss Dilemma + 8개 hidden-state diagnostic + 8개 Practice Brief 모두 byte-equivalent 의미로 보존. v3.1은 본문 재작성·재구조화를 수행하지 않음. |
| Source-Boundary Certificate | PASS | v3.1 patch는 모두 영문 산문→한국어 산문 surface 교체에 한정. 새 drug example·page tag·외부 fact·수치·예시·regulatory claim 도입 0건. R&T 직접 인용문(*"Dose linearity in pharmacokinetics..."*)은 attributed quotation으로 영문 보존. |
| HTML-Readiness Certificate | PASS | PART B B1-B13 그대로 보존; B14(v3→v3.1 변경 로그) 신규 추가. Marker·page tag·navigation anchor·responsive/print·forbidden restoration·Insertion Map·Splice Verification Table 모두 byte-level 보존. FIGURE_POINTER·FIGURE_SCHEMATIC 블록 내 영문 brief 미변경. |

## Assembly Mode

PATCH MODE (v3.1 유지)

Rationale: `11_Content Lock v2.1.md`는 figure marker insertion plan이며, learner-facing canonical body는 `11_Content Lock v2.md`에서 가져옴. 6개의 marker block이 지정 위치에 splice됨. v3.1은 v3의 splice 결과를 변경하지 않는다(B8 Splice Verification Table은 byte-level 동일; 6개 marker block 위치·내용·anchor 텍스트 동일). v3.1 patch는 marker block 외부의 학습자 산문에만 적용된다.

## Input Manifest (v3.1 추가 행만 표시; 나머지는 v3와 동일)

| File | Role | Authority level | Used for | Notes |
|---|---|---|---|---|
| `11_html_compile_input_master_v3.md` (v3) | v3.1 base substrate | A8 | v3.1의 모든 §1-§8 본문 의미는 v3에서 carry-over | v3.1은 영문 산문→한국어 산문 patch layer만 적용 |
| v3.1 Korean Readability Patch (v3.1 audit) | v3.1 readability authority | A6 | 35건 영문 산문 식별 (Big Idea 오프너·§7 질문·§8 산문·Recap·Trench·Structural·Learner Nav) | v3.1 patches P-01–P-35의 근거 |
| `11_html_compile_input_master_ver2.md` (v2) | v3의 base substrate | A8 (carry-over) | v2의 모든 §1-§8 본문은 v1에서 carry-over | v3는 v2 위에 누적 작업, v3.1은 v3 위에 누적 작업 |
| (v1·v2·v3의 Input Manifest 모든 항목 carry-over) | — | — | — | v3.1은 v1·v2·v3 위에 누적적으로 작업 |
...
가역적 turnover 모델은 system이 반응을 지속적으로 생산하고 제거하기 때문에 baseline으로 돌아간다. 비가역적 작용은 다르다. 약물 노출 기간 동안 반응 단위, 표적, 또는 세포가 영구적으로 제거된다. 따라서 drug may disappear while the biological consequence persists. [G pp.256–260; T pp.251–252]
...
<!-- CONFUSION -->
## §5-6. ⚡ Memory Hooks — 7 Critical Conceptual Pairs [EXPERT_INFERENCE, v3]
...
### Pair 1 — direct effect vs indirect response
...
### Pair 2 — Model I (production inhibition) vs Model II (loss inhibition)
...
### Pair 3 — kin vs kout (turnover rate parameters)
...
### Pair 4 — baseline R0 vs steady-state Rss (drug effect)
...
### Pair 5 — rebound vs natural recovery
...
### Pair 7 — Pool 1/2 model non-identifiability vs model misspecification
...
이 세션은 direct exposure-response modeling과 고급 disease/response systems 사이에 위치한다. 여기서부터 modeler는 curve fitting을 넘어 원인 분리를 해야 한다. 이 단계가 처음으로 다음을 분리해야 하는 지점이다:
...
|---|---|
| Treating every delay as effect compartment | `ke0`이 biology를 흡수한다. 새 투여 프로토콜에서 dosing simulation이 실패한다. |
| Treating tss as deterministic model proof | 기전적 주장이 설계 근거를 과대평가한다. |
| Confusing `Emax` units | 교차 연구 또는 in vitro/임상 potency 비교가 오해를 유발한다. |
| Applying `tD` to PD-rate-limited drugs | Duration 예측이 생물학적으로 무의미해진다. |
| Ignoring baseline drift | 질병 진행 또는 일주기 변동이 약물 효과로 오독된다. |
...
Indirect response modeling은 "지연을 추가하는 것"이 아니다. 올바른 인과 clock에 지연을 할당하는 것이다. 따라서 잠긴 workflow는 다음과 같다:
