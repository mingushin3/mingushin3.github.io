# 09_HTML Compile Input Master — v2

## Version Note

**v2 변경 요지**: v1의 5대 인증서 PASS 판정은 그대로 유지된다. v2는 Step 1 V5.0 Apex Edition protocol의 두 가지 표현 장치를 source-boundary 안에서 추가 반영한다.

1. **§5 Pair 2 (Full TMDD vs Michaelis-Menten Approximation)에 Critical Blow 행 추가** — Step 1 protocol의 "파급력 최대 혼동쌍 1개에만 적용" 규칙에 따라, regulatory cost·timing 주장 없이 mechanism-only 1문장으로 임상 의사결정 영향 경로를 명시한다. Crucible A7(C-2 시나리오의 mechanistic statement 압축)과 정합한다.
2. **§7 Self-Test에 SR 태그(★★/★/○) 및 "다음 깊이 질문" 추가** — Step 1 §7 protocol의 spaced-repetition 가중치 표시와 Socratic 후속 질문 형식. 본문 사실 변경 없이 active recall 효과만 강화한다.

두 추가 항목 모두 새로운 사실 주장·수치·교과서 외 시나리오를 도입하지 않는다. EXPERT_INFERENCE / source-bounded 원칙을 유지한다.

## Phase 4D Certification

| Certificate | Status | Basis |
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A contains learner navigation plus the complete cleaned learner body; audit/process/compiler tables are kept out of PART A. v2 SR tags and next-depth questions reinforce active recall scaffolding. |
...
| Mastery-Uplift Certificate | PASS | PART A includes 8 bounded, adjacent, source-status-labeled mastery augmentations + 2 v2 presentational augmentations (AUG-9 Critical Blow, AUG-10 SR tags/next-depth questions) without broad rewrite. |
| Source-Boundary Certificate | PASS | Augmentations are labeled as AUDIT_DERIVED, CRUCIBLE_DERIVED, or EXPERT_INFERENCE; micro-patches use verified PDF/Audit basis and page tags. v2 additions introduce no new factual claims, costs, timings, or out-of-source scenarios. |
| HTML-Readiness Certificate | PASS | PART B preserves Phase 5 rendering rules, figure/page-tag rules, audit/crucible guardrails, and splice verification. v2 SR tags and Critical Blow are simple presentational elements that the existing Marker Mapping handles via standard CSS. |
...
| C9 Canonical integrity | Content Lock v2 learner body preserved except 7 figure insertions, 3 micro-patches, 8 labeled augmentations + 2 v2 presentational augmentations (AUG-9 Critical Blow, AUG-10 SR tags/next-depth questions) | Assembly logs | PRESENT | None |
