...
=================
거장의 시각에서 이 Scope Lock이 포착한 핵심:
이 세션은 표면적으로 "항체 PK"를 다루지만, 실제 체화 목표는 더 심층적입니다. Gabrielsson의 Turnover 파트(§2.6)는 kin/kout 패러다임이라는 단 하나의 수학적 뼈대가 호르몬·단백질·체온·피드백 전체를 통일하는 방식을 보여주고, PK27은 그 뼈대 위에 TMDD 완전 모델이 어떻게 얹히는지를 데이터와 함께 증명합니다. R&T Ch.21은 이 모든 것을 임상·규제 언어로 번역합니다.
두 소스의 역할 분담이 명확해야 Step 1에서 수식 도출(G)과 임상 해석(R&T)이 섞여서 혼탁해지는 것을 방지할 수 있습니다. Scope Lock에 이를 명시한 이유입니다.
Phase 1 진행 시 주의사항
① MUST 카드 수 상한 사전 설정
