...
**이 자료 활용법:** §1을 한 번 읽어 전체 뼈대를 잡은 뒤, §2 C1–C5를 순서대로 학습하고, §7에 도전하기 전에 §5로 세 가지 혼동을 먼저 차단하세요. Figure pointer는 교과서 그림을 그대로 복제하지 않는다. Pointer가 등장하면 출처 PDF·교과서에서 명시된 figure를 직접 펼쳐 확인한다.

**각 C-카드는 ▶ 실무 체화 요약(Practice Brief)으로 마무리되며, 네 가지 체화 축(왜 알아야 하는가 / 흐름상 역할 / 체화 꿀팁 / 실무 활용)을 압축한다.** §2는 알로메트릭 의사결정 워크스루(Allometric Decision Walkthrough)로 마무리되어 C1→C5를 하나의 워크플로우로 연결한다.

**학습 경로:**

1. C1: `Y=a·BW^b`가 단순 회귀선이 아닌 **스케일 법칙**임을 이해한다.
2. C2: CL exponent와 `b=1` fallacy를 확실히 잡는다.
3. C3: flow 지수(`b`)와 extent 지수(`d`)를 분리하고 시간·속도 결과를 도출한다.
4. C4: Dedrick plot을 지수 구조의 진단 도구로 활용한다.
5. C5: allometric dose 외삽을 최종 임상 용량이 아닌 equal-AUCu backbone으로 다룬다.
6. §2 폐막 Walkthrough: 동물 PK 데이터셋 하나를 C1→C5 흐름으로 통과시켜 전체 흐름을 잠근다.
7. §5–§7: source 불일치, 근거 없는 규제 언어, 지수 혼동 오류를 스스로 차단할 수 있는지 점검한다.

**시작 전 점검:** CL, V, AUC, half-life, unbound fraction(← 비결합 분율), first-order elimination을 정의할 수 있는지 확인한다.  
**완료 후 체크:** `0.75`와 `1.0`이 왜 호환되지 않는지, 시간이 왜 `d-b`로 스케일링되는지, 소아·노인 용량에서 왜 functional age와 장기 기능 보정이 체중 외에 추가로 필요한지 설명할 수 있어야 한다.
...
> **▶ Target Mental Model — [TEXTBOOK_DERIVED, ver2 V1]**  
> 이 세션을 닫는 순간, 다음 4단계가 자동 회상되어야 한다.
> 1. **변수 종류부터 묻는다** — flow/rate인가(CL, GFR, blood flow → $b\approx 0.75$), space/extent인가(V, blood volume, tissue mass → $d\approx 1$), time인가($V/CL → BW^{d-b}$), rate constant인가($CL/V → BW^{b-d}$).
> 2. **종간 dose translation은 unbound exposure 관점**으로 읽는다. AUCu equality이면 $Dose \propto BW^b$, mg/kg 비는 $BW^{b-1}$이다.
> 3. **예외 먼저 찾는다** — fu, CYP isoform, nonlinear PK, transporter, enterohepatic recirculation, route/formulation 차이.
> 4. **Source language 지킨다** — PDF-supported(G&W/R&T 수식, PK28/PK29 anchor, R&T age/renal/BSA caution)와 not directly supported(ICH/FDA/MABEL/NONMEM/QSP 단정)를 분리한다. 이 4단계 자체가 §8 D Final Mental Model의 압축이며, §1–§8 전체가 이 4단계의 정당화 과정이다.
...
<!-- ANNOTATION --> `a`는 화합물·변수의 절편(intercept), `b`는 체중이 바뀔 때 해당 변수의 변화율을 결정하는 지수(exponent)다.
...
- **$a$:** 약물 의존적(drug-dependent) 또는 화합물 의존적(compound-dependent) 절편(intercept). 단위는 $b$에 의존한다.
- **$b$:** 대체로 변수 유형에 의존하는(variable type-dependent) 지수. 다만 실제 CL exponent는 자료 품질, 종 수, 비선형성, 종간 단백 결합(protein binding) 차이 등에 따라 변동할 수 있다 [G&W p.177–178].

#### B. 도출(Derivation) — 왜 거듭제곱 함수인가

G&W는 대사율(metabolic rate)이 $BW^{0.75}$로 스케일링하고, 에너지 함량(energy content)이 대략 $BW^1$에 비례하므로 turnover time(← 에너지 순환 주기)이 $BW^{0.25}$로 나온다고 제시한다 [G&W Eq.2:375–377, p.173]. 즉 알로메트리는 “체중이 커질수록 절대 처리량은 증가하지만, 단위 체중당 처리 효율은 감소한다”는 생리학적 압축이다.

표면적·부피 직관도 같은 방향을 준다. 기하학적으로 표면적(surface)은 길이²(length²), 부피(volume)는 길이³(length³)이므로 표면적은 부피^(2/3)에 비례한다. 그러나 실제 생리학 변수는 단순 외부 표면적보다 더 복잡해, Brody의 경험적 지수는 0.5–0.8 범위로 나타난다 [G&W p.173–174].

#### C. 구조적 의미(Structural Meaning)

<!-- ANCHOR -->
Log-log 직선은 단순히 “회귀가 잘 맞는다”는 뜻이 아니다. `[교과서 외 수학적 해석]`으로 말하면, 이는 체중 배율이 달라져도 같은 지수 법칙이 유지되는 scale-invariant(← 척도 불변) 구조를 가정한다는 뜻이다. 이 가정이 깨지는 순간이 알로메트리 실패(allometry failure)의 시작이다.

#### D. 경계 조건(Boundary Conditions)

G&W는 알로메트리(allometry)가 물리적 수송 과정(physical transport processes)에 지배되는 변수에서, 그리고 약물 분포가 혈장 단백 결합(plasma protein binding)에 크게 좌우되지 않는 물질에서 특히 유용하다고 설명한다. 반대로 종간 대사·배설(metabolism/excretion)의 질적 차이, CYP450 isozyme 구성(makeup) 차이, 혈장 단백 결합 차이가 크면 알로메트리는 실패할 수 있다 [G&W p.173].
...
> - **왜 알아야 하는가:** $Y=a\cdot BW^b$는 종간 PK 차이를 “회귀선”이 아닌 **scale-invariance 주장**으로 옮기는 언어다. 이 한 식이 깨지는 약물이 어떤 약물인지 식별하지 못하면 C2–C5의 모든 추론이 무너진다.  
> - **흐름상 역할:** 항법도의 **루트 노드**. $a$는 후속 카드들에서 구체값으로 채워지고, $b$는 C2(CL)와 C3(V)에서 각각 다른 prior를 받는다. 즉 C1은 “수식 골격”, C2/C3는 “지수 채움”, C4/C5는 “결과 활용”이다.  
> - **체화 꿀팁:** 회귀 결과를 보면 **먼저 잔차 패턴과 종 수**를 본다. log-log slope의 점추정값은 거짓 정밀도를 줄 수 있다. `b`는 자연상수가 아니라 **생리학적 prior**라고 입으로 말하며 풀어본다.  
> - **실무 활용:** Animal-to-human translation 보고서 첫 문단은 “이 화합물은 scale-invariance가 유지될 metabolic/transport profile인가?”에 답해야 한다. 이 답이 “아니다”이면 C2의 $b\approx 0.75$ prior를 그대로 적용할 근거가 약해진다.

---

### ━━ C2. [⚡ Apex Concept] Clearance Exponent $b\approx0.75$ ━━

<!-- MASTER LENS -->
**개념 Big Idea:** 청소율(Clearance)은 “공간의 크기”가 아니라 **단위 시간당 처리 흐름**이다. 그래서 V처럼 BW¹로 증가하지 않고, 대사율·혈류·GFR과 같은 functional variable(← 기능적 처리량 변수)처럼 대략 $BW^{0.75}$로 증가한다 [G&W Table 2.22, p.180; R&T Fig.22-1, p.732].

#### A. 정형 정의(Formal Definition)

$$CL_i=a\cdot BW_i^b \quad [G\&W\ Eq.2:380,\ p.176;\ PK28\ Eq.28:1,\ p.611]$$

G&W는 CL/대사율(metabolic rate)의 대표 exponent를 0.75로 제시하지만, 실제 CL exponent는 약 0.2에서 >1까지 변할 수 있다고 경고한다 [G&W p.178]. Fig.2.159는 91개 화합물의 CL exponent 분포가 주로 0.5–1.0 범위에 놓임을 보여준다 [G&W p.191].

#### B. 그럴듯한 오해(Plausible Fallacy) — `b=1` 선형 per-kg 스케일링

**오류:** “mg/kg 처방에 익숙하니 청소율(clearance)도 kg에 선형 비례한다.” 이것이 선형 per-kg 스케일링(linear per-kg scaling, ← kg당 값이 일정하다는 가정) 오류다.  
<!-- ANNOTATION -->
**교정:** CL은 흐름·속도 변수(rate/flow variable)이므로 `b≈0.75`가 생리학적 prior(physiological prior)다.
...
> <!-- TRENCH -->
> **▶ Plausible Fallacy 보강 — `b=0.75` over-trust — [EXPERT_INFERENCE, v3]**
>
> - **그럴듯한 오해:** “CL exponent $b=0.75$를 적용하면 종간 스케일링 문제가 해결된다.”
> - **왜 틀렸는가:** $0.75$는 metabolic rate scaling의 empirical prior이지 모든 약물에 적용되는 생물학적 법칙이 아니다. G&W는 실제 CL exponent가 $0.2$에서 $>1$까지 분포한다고 경고한다 [G&W p.178]. 단백질 결합, transporter 표현, 종간 효소 활성 차이, maturation 요인이 모두 예외를 만들 수 있다. 즉 $0.75$는 **출발점(prior)**이지 **종점(law)**이 아니다.
> - **실무에서 어떻게 드러나는가:** FIH 용량을 $b=0.75$로만 계산했다가 실제 human CL이 다른 exponent를 따를 경우, Phase 1 starting dose가 과대 또는 과소추정될 수 있다. Safety review에서 “exponent justification”을 요구하는 IND meeting 피드백이 나올 수 있다. 이 fallacy는 위의 `b=1` 오류와 **거울상 관계**에 있다 — 한쪽은 mg/kg에 묶여 $b$를 1로 고정하고, 다른 쪽은 $b=0.75$를 무비판적으로 신뢰한다. 두 fallacy를 동시에 차단해야 C2가 닫힌다.

#### C. 실패 조건(Failure Conditions)

알로메트리 실패(Allometry failure)는 단순한 무작위 오차(random error)가 아니라 **메커니즘 신호(mechanism signal)**일 수 있다. G&W Fig.2.145는 scale 가능한 화합물(scalable compound)에서는 70-kg human CL prediction interval이 약 10-fold 범위이지만, scale 어려운 화합물(less scalable compound)에서는 약 1000-fold 범위까지 넓어질 수 있음을 보여준다 [G&W p.174].

주요 실패 트리거(failure trigger):
...
`[교과서 외 실무 해석]` PopPK 또는 종간 회귀에서 추정된 $b>1$이 나오면 “추정값을 그대로 믿을 것인가?”가 아니라 “종간 단백 결합(binding), 수송체(transporter), 포화(saturation), 자료 품질(data quality) 중 무엇이 기울기(slope)를 밀어 올렸는가?”를 먼저 묻는다. 반대로 두 종만으로 얻은 $b<0.5$는 생리학적 신호보다 통계적 지렛대 부족(statistical leverage failure)일 가능성이 크다.
...
> <!-- TRENCH -->
> **▶ Practice Brief — C2 체화 4축 — [EXPERT_INFERENCE, ver2 V2]**  
> - **왜 알아야 하는가:** Apex 카드. CL exponent 하나의 선택이 mouse→human 7.4배, rat→human 4.1배, 20-g mouse 케이스 7.7배의 **order-of-magnitude 격차**를 만든다. `b=0.75` vs `b=1.0`은 “소수점 차이”가 아니라 **첫 human exposure estimate의 자릿수**를 바꾼다.  
> - **흐름상 역할:** $b\approx 0.75$는 C5의 equal-AUCu backbone($Dose\propto BW^b$)에 직접 들어가는 **유일한 지수**. C3의 V exponent와 조합되어 C4 Dedrick의 시간축도 결정한다. C2가 흔들리면 C5 dose translation 전체가 흔들린다.  
> - **체화 꿀팁:** **“흐름 변수 → 0.75”**라는 입버릇을 먼저 만든다. 그 다음 `b=1`을 보면 “이건 mg/kg 사고가 들어왔다”라고 진단한다. $b>1$은 mechanism 신호, $b<0.5$는 statistical leverage failure로 분류하는 진단 레퍼토리를 굳힌다.  
> - **실무 활용:** FIH exposure translation 표에서 CL column을 채울 때, $b=0.75$를 primary로 두되 $b=0.5, 0.62, 0.75, 1.0$ sensitivity를 같은 표에 병기하는 것이 방어적이다 (Q8 보스 딜레마(Boss Dilemma) 응답 패턴). $f_u$ 차이가 크면 total CL이 아니라 $CL_u$를 스케일링한다는 Trench 규칙을 동시에 적용한다.
...
<!-- MASTER LENS -->
**개념 Big Idea:** V는 flow(← 처리 흐름)가 아니라 extent(← 분포 공간의 크기)다. 체액, 혈액량, 조직 부피가 대체로 체중에 비례하므로 분포용적은 대략 BW¹로 증가한다. 이 $d\approx1$과 CL의 $b\approx0.75$의 차이 0.25가 종간 시간 척도를 만든다.

#### A. 정형 정의(Formal Definition)

$$V_i=c\cdot BW_i^d \quad [G\&W\ Eq.2:382,\ p.179;\ PK28\ Eq.28:2,\ p.611]$$

G&W Table 2.22는 혈액량 지수(blood volume exponent) 0.99, 골격근량(skeletal mass) 1.09, Vd 약 1.0을 제시한다 [G&W p.180].
...
<!-- ANCHOR -->
학습자는 “$t_{1/2}\propto BW^{0.25}$”를 암기하는 데 그치기 쉽다. 그러나 더 중요한 것은 **큰 동물일수록 모든 일차 속도상수(first-order rate constant)가 작아진다**는 사실이다. 같은 분수 $V/CL$을 시간으로 읽으면 반감기(half-life)이고, 뒤집어 $CL/V$로 읽으면 속도상수(rate constant)다. 다시 말해, 큰 동물은 절대 CL이 크지만 단위 시간당 제거되는 분율(fraction)은 작다.
<!-- ANNOTATION -->
...
V exponent는 대체로 1에 가깝지만, $V_{ss}$는 조직 친화성(tissue affinity), 지방 분포(fat distribution), 수송체(transporter), 단백 결합(protein binding)에 따라 0.8–1.2 범위로 흔들릴 수 있다. R&T Ch.14의 비만(obesity) 예시도 “체중(body weight)이 항상 생물학적으로 관련된 분포용적(biologically relevant volume)은 아니다”라는 경고로 읽어야 한다 [R&T p.439].
...
> <!-- TRENCH -->
> **▶ Practice Brief — C3 체화 4축 — [EXPERT_INFERENCE, ver2 V2]**  
> - **왜 알아야 하는가:** $d\approx 1$이라는 prior가 깨지지 않는다고 가정하면 PK29 compound X(d≈1.18)처럼 multicompartment 약물에서 Dedrick superposition이 무너진다. V의 지수를 따로 추적할 능력이 없으면 시간 축이 잘못 정규화된다.  
> - **흐름상 역할:** $b$와 $d$의 **차이($d-b$)** 자체가 핵심 산출물. C3는 C2의 산출($b$)과 결합해 **시간 축 두 개**($t_{1/2} \propto BW^{0.25}$, $k_{el} \propto BW^{-0.25}$)를 만들어 C4 Dedrick의 x축에 직접 입력된다.  
> - **체화 꿀팁:** “큰 동물 = 절대 CL 큼, but fraction 제거 느림”을 한 호흡으로 외운다. **$V/CL$과 $CL/V$를 동시에** 적어가며 sign을 점검한다 — half-life는 양수 지수, rate constant는 음수 지수.  
> - **실무 활용:** PopPK에서 V exponent를 1로 fix하기 전에 log-log V vs BW slope가 0.9–1.1 안인지 확인한다 (C4 Trench와 동일 진단). 비만·소아·노인 데이터에서 $V_{ss}$가 0.8–1.2로 흔들리면 BW 기반 V 추정에 ideal/lean body mass 대안 covariate을 검토한다.
...
<!-- MASTER LENS -->
**개념 Big Idea:** Dedrick plot(← 종별 PK 곡선을 공통 척도 위에 겹치는 변환)은 종간 PK curve를 “같은 과정의 서로 다른 시간 척도”로 표현한 뒤 하나의 공통 축 위에 포개어 보이는 변환이다.
<!-- ANNOTATION --> 시간축만 조정하는 것이 아니라, 용량 정규화 농도(dose-normalized concentration) 축도 BW exponent에 맞게 조정한다.

#### A. 정형 정의(Formal Definitions)

**기본 Dedrick plot(Elementary Dedrick plot)** — `d=1` 가정:

$$y=\frac{C}{Dose/BW},\qquad x=\frac{t}{BW^{1-b}}$$

**복합 Dedrick plot(Complex Dedrick plot)** — `d\neq1` 일반형:

$$y=\frac{C}{Dose/BW^d}=\frac{C\cdot BW^d}{Dose},\qquad x=\frac{t}{BW^{d-b}}$$

Kallynochron은 $d=1$ 가정 하의 kg당 시간 척도(per-kg time scale)이고, Apolysichron은 $d$를 명시적으로 반영한 일반화 생리학적 시간 척도(generalized physiological time scale)이다 [G&W §2.10.6–2.10.7, p.184–189].
...
따라서 농도(concentration)는 $Dose/BW^d$로, 시간(time)은 $BW^{d-b}$로 정규화되어야 한다. `[교과서 외 수학적 해석]`으로 말하면 이는 Buckingham π 정리(Buckingham π theorem)류의 차원 해석(dimensional analysis)과 같은 직관이다. 변수들이 가진 질량·시간·부피(mass/time/volume) 차원을 제거하면 독립적인 무차원 그룹(dimensionless group)이 남고, Dedrick 변환은 그 그룹의 PK 버전이다.

#### C. AUC 관계식(AUC relation)

기본 Dedrick(Elementary Dedrick) 맥락에서 G&W는 AUC가 $1/a$로 정리됨을 보인다 [G&W Eq.2:386, p.179]. 이것은 C5의 'equal unbound AUC' 조건과 무관한 별개의 사실이 아니라, 같은 논리가 수식으로 먼저 나타나는 전단계다.

#### D. 데이터 앵커(Data Anchor) — PK28 and PK29

**PK28 Methadone**: mouse/rat/man IV bolus 자료를 기본 Dedrick plot(elementary Dedrick plot)으로 중첩한다 [G&W p.184–186; PK28 p.611–614]. 단, case는 단일 dose level이며 G&W도 ≥2 dose levels, 다회 투여·항정상태(multiple dosing/steady state), 모델 잘못 지정(model misspecification) 배제가 필요하다고 경고한다 [PK28 p.614].
...
- 체중 범위(Weight range): 산술적으로 70/0.020 = 3500-fold; PK29 case 본문은 약 3000-fold로 표기할 수 있다. 본문에서는 `약 3000–3500-fold`로 표기한다.

> <!-- TRENCH -->
> **[TRENCH — Elementary vs Complex Dedrick switch]** log-log $V$ vs $BW$ slope가 0.9–1.1 밖이면 기본 Dedrick(Elementary, $d=1$ 가정)을 고집하지 않는다. PK29처럼 $d\approx1.18$이면 $C/(Dose/BW^d)$와 $t/BW^{d-b}$를 쓰는 복합 Dedrick(Complex Dedrick)으로 전환한다.

<!-- RECAP -->
**C4 핵심:** Dedrick superposition은 곡선 적합(curve-fitting) 장식이 아니라 CL exponent와 V exponent를 동시에 검증하는 진단(diagnostic) 도구다. Elementary가 실패하면 “plot이 안 예쁜 것”이 아니라 “d=1 가정이 깨졌을 가능성”을 먼저 본다.
...
> <!-- TRENCH -->
> **▶ Practice Brief — C4 체화 4축 — [EXPERT_INFERENCE, ver2 V2]**  
> - **왜 알아야 하는가:** Dedrick은 “결과 그림” 이 아니라 **C2/C3 지수 쌍의 진단 도구**다. Elementary가 깨지면 d≠1 가능성·compartment 비율·binding mismatch 중 어느 것이 신호인지 빠르게 분류할 수 있어야 한다.  
> - **흐름상 역할:** C4는 C1–C3가 만든 **수식적 예측**을 **데이터에 한 번 부딪쳐 보는 검증대**다. 통과하면 C5 dose backbone이 신뢰성 있게 적용되고, 실패하면 C5 적용 전에 d 추정을 다시 검토한다.  
> - **체화 꿀팁:** Elementary($t/BW^{1-b}$)와 Complex($t/BW^{d-b}$)의 **시간축 차이만 비교**하면서 외운다. log-log V vs BW slope **0.9–1.1**이라는 switch trigger를 외부 cheat sheet에 적어 두고 매번 확인한다. AUC=1/a라는 elementary 결론은 C5 equal-AUCu의 수학적 전단계임을 의식한다.  
> - **실무 활용:** PK28(d=1 anchor)과 PK29(d=1.18 anchor)를 **벤치마크 쌍**으로 기억한다. 새 화합물에서 종간 V exponent를 추정했을 때, PK29처럼 1을 넘어가면 즉시 Complex Dedrick으로 전환하고 이를 보고서에 명시한다 (단일 dose level, model misspecification 배제 검토 caution을 함께 적시).
...
<!-- ANNOTATION --> 실제 FIH starting dose는 이 backbone 위에 안전 여유(safety margin), 독성학(toxicology), 약리학(pharmacology), 노출-반응(exposure-response), 투여 경로(route), 제형(formulation), PD 지표(metric)를 더해 결정한다.
...
1. 비결합 AUC(unbound AUC)가 약리 효과(pharmacologic effect)와 연결된다;
2. 표적 친화도(target affinity)와 관련 약리학(relevant pharmacology)이 종간 크게 다르지 않다;
3. PK가 선형 구간(linear range)에 있다;
4. fu(비결합 분율, unbound fraction) 차이가 있으면 $CL_u$로 보정한다;
5. AUC가 적절한 노출 지표(exposure metric)이다. 최고농도 기반 독성(Cmax-driven toxicity) 또는 역치 초과 시간 지표(time-above-threshold metric)이면 별도 판단이 필요하다.

G&W는 동일 AUC(equal AUC)라도 농도-시간 곡선(concentration-time curve)의 모양(shape)은 상당히 다를 수 있음을 명시한다 [G&W p.190]. 따라서 C5는 AUC 백본일 뿐이며, 전체 곡선 등가성(full curve equivalence)을 보장하지 않는다.

#### C. 소아·노인 맥락(Pediatric and elderly context) — R&T 기반

R&T Ch.14는 연령(age), 체중(weight), 성별(gender)이 약물 반응 변동성(drug response variability)의 중요한 원천이며, 생활연령(chronologic age)이 기능적 연령(functional age)을 정의하지 않는다고 경고한다 [R&T p.411–412]. 또한 연령을 고려하지 않는 일률 용량(flat dosing)은 개별 노인 환자(elderly patient)의 필요를 충족하지 못할 수 있다고 설명한다 [R&T p.412].

R&T Key Relationships는 BSA를 다음처럼 체중 기반으로 근사한다:

$$BSA=1.73\cdot\left(\frac{Weight}{70}\right)^{0.75} \quad [R\&T\ p.436]$$

그리고 60세 일반 성인(typical 60-year-old adult) 대비 소아 유지 용량(child maintenance dose)을 다음처럼 제시한다:
...
다만 R&T는 mg/kg 또는 mg/1.73 m² 규칙의 폭넓은 사용(broader use)은 적용 한계(연령, 체조성, 신기능)를 명시하지 않으면 의심스럽다고 경고한다 [R&T p.435]. 즉 **BW 알로메트리는 시작점이지, 소아·노인 용량 결정 공식 전체를 대체하지 않는다.**

#### D. 노인 대사 진술 교정(Elderly metabolism statement correction)

“CYP3A4 metabolism이 60세 이상 매년 1% 감소”라는 문장은 과단정이다. R&T는 크레아티닌 청소율(creatinine clearance)이 성인기(adulthood)에서 대략 1%/yr 감소한다는 경험 법칙(rule of thumb)을 제시하고 [R&T p.422], CYP3A 기질(substrate)에서는 노인군(elderly group)이 청년군(young adult)보다 낮은 청소율을 보이며 그 연령 차이를 해석할 때 1%/yr 크레아티닌 청소율 경험치(heuristic)가 관련된다고 설명한다 [R&T p.424]. 따라서 최종 문장은 다음처럼 제한한다: **노인에서는 신·간 기능(renal/hepatic function)과 생물학적 연령(biological age)을 별도로 보아야 하며, CYP3A 청소율도 청년 대비 감소할 수 있지만 단순 선형 시간 함수로 쓰지 않는다.**
...
> Equal-AUCu dose backbone은 “최종 FIH dose”가 아니라 노출 번역(exposure translation)의 중심축이다. R&T의 age/weight 논리는 이 축 위에 functional age와 organ function을 더해야 개인 환자 용량이 된다는 점을 보강한다.

> <!-- TRENCH -->
> **▶ Practice Brief — C5 체화 4축 — [EXPERT_INFERENCE, ver2 V2]**  
> - **왜 알아야 하는가:** C5를 standalone FIH starting-dose 공식처럼 적으면 NDA·IND 정당화에서 곧장 **방어 불가**다. 이 식은 backbone이지 final answer가 아니다. 또한 R&T Ch.14의 핵심 메시지(chronologic age ≠ functional age)를 누락하면 소아·노인 dosing 자체가 단순화 오류로 무너진다.  
> - **흐름상 역할:** C1–C4가 산출한 지수 쌍과 시간축이 **여기서 dose라는 단일 임상 액션으로 수렴**한다. C5는 “종간 수식”과 “인간 환자 수식” 사이의 마지막 다리이며, 이 다리 위에 safety margin, PD metric, route/formulation이 얹힌다.  
> - **체화 꿀팁:** “Equal AUCu가 곧 같은 PD를 보장하지 않는다”와 “mg/kg 비는 $BW^{b-1}$이다”를 동시에 외운다. 큰 동물 → 작은 mg/kg, 작은 동물 → 큰 mg/kg이라는 방향성을 손가락 방향으로 굳힌다. CYP3A 1%/yr 같은 단정문은 입에서 자동 차단한다.  
> - **실무 활용:** FIH dose justification 보고서에서 C5 backbone은 한 줄($Dose_{man}=Dose_{animal}\cdot (BW_{man}/BW_{animal})^b$)로 적되, 그 위에 (i) 안전성 margin, (ii) AUC vs Cmax target 선택 근거, (iii) fu 보정, (iv) route/formulation 고려를 별도 섹션으로 분리한다. 소아/노인 적용 시 BSA 근사·R&T pediatric maintenance 식·creatinine clearance heuristic을 backbone 위에 한 층씩 얹는다.

---

> <!-- MASTER LENS -->
> ## ▶ §2 폐막 — Allometric Decision Walkthrough — [EXPERT_INFERENCE, ver2 V3]
>
> 이 walkthrough는 **새로운 수치도, 새로운 named example도 도입하지 않는다.** C1–C5 본문에서 이미 확정된 수식·anchor·진단만 사용하여, 학습자가 **하나의 동물 PK 데이터셋을 받아 인간 노출 추정에 도달하는 5단계 의사결정**을 한 호흡에 통과시키는 시연이다. 이 walkthrough를 닫는 순간 §1 Target Mental Model이 닫힌다.
>
> **Step 1 — Variable Type Diagnosis (C1 ↔ C2/C3 진입점)**  
> 받은 변수가 무엇인지 먼저 묻는다. CL/GFR/blood flow면 **flow variable**이므로 $b\approx 0.75$ prior로 진입한다 (C2). V/blood volume/tissue mass면 **extent variable**이므로 $d\approx 1$ prior로 진입한다 (C3). 이 분류 자체가 틀리면 C4–C5 전체가 무너진다. 한 줄 cheat: “**flow면 0.75, extent면 1**.”
>
> **Step 2 — Prior Validity Check (C1 scale-invariance + C2 failure conditions)**  
> $b\approx 0.75$ prior를 그대로 적용해도 되는지 점검한다. 다음 6개 trigger 중 하나라도 강하게 의심되면 prior를 sensitivity analysis로 격하한다: 종간 fu 차이, CYP isoform 차이, nonlinear MM kinetics, enterohepatic recirculation, route/formulation 차이, central/peripheral V ratio drift. fu 차이가 크면 total CL이 아니라 $CL_u$로 스케일링한다 (Eq.2:421 라인).
>
> **Step 3 — Time-axis Derivation (C3)**  
> $b$와 $d$로부터 두 시간 축을 동시에 적는다. $t_{1/2} \propto BW^{d-b} \approx BW^{0.25}$ (큰 동물 = 긴 반감기), $k_{el} \propto BW^{b-d} \approx BW^{-0.25}$ (큰 동물 = 작은 fraction 제거 속도). 이 두 식의 부호를 동시에 보지 못하면 C4 정규화가 어긋난다.
>
> **Step 4 — Dedrick Diagnostic Pass (C4)**  
> 종간 자료가 있으면 Elementary Dedrick($d=1$ 가정, $t/BW^{1-b}$)을 먼저 시도한다. log-log V vs BW slope가 **0.9–1.1 안**이면 Elementary로 충분하다. 0.9–1.1을 벗어나면 Complex Dedrick($t/BW^{d-b}$, $C\cdot BW^d/Dose$)으로 전환한다. PK28(elementary 통과) ↔ PK29($d\approx 1.18$로 Complex 필요)가 그 벤치마크 쌍이다. Curve fan-out이 남으면 그건 “예쁜 overlay 실패”가 아니라 **mechanism signal**이다.
>
> **Step 5 — Equal-AUCu Dose Backbone Application (C5 + R&T 보강층)**  
> $Dose_{man}=Dose_{animal}\cdot (BW_{man}/BW_{animal})^b$를 backbone 한 줄로 적되, 그 위에 (i) AUC vs Cmax-driven toxicity 선택 근거, (ii) fu 보정, (iii) PK linearity 가정 점검, (iv) safety margin/toxicology, (v) route/formulation, (vi) PD metric — 6개 layer를 명시적으로 얹는다. 소아/노인 환자에 적용할 때는 R&T Ch.14의 BSA 근사식, pediatric maintenance 식, chronologic vs functional age 경고, creatinine clearance ~1%/yr heuristic을 backbone 위에 다시 한 층 얹는다.
>
> **닫는 한 줄:** 이 5단계가 자동 회상되어야 §1 Target Mental Model이 닫힌다. 닫힌 학습자는 “0.75를 외우는 사람”이 아니라 “0.75를 prior로 두고 자료가 prior를 언제 밀어내는지 판단하는 사람”이다.
...
**치명적 타격(Critical Blow):** rat 250 g→human 70 kg에서 $b=0.75$ 대신 $d=1$처럼 선형 scaling하면 차이는 $(70/0.25)^{0.25}\approx4.1$배다. mouse 23 g→human에서는 약 7.4배, R&T 20-g mouse 예시는 7.7배다. 이 오류는 “소수점 차이”가 아니라 첫 human exposure estimate의 order를 바꾼다.

**⚡ 기억 고리 (Memory Hook) — *기능 vs 공간*: [EXPERT_INFERENCE, v3]**  
CL은 **기능적 처리 능력** — 체중이 커질수록 대사율이 체중보다 느리게 증가하므로 $b<1$. V는 **물리적 분포 공간** — 체중에 비례하여 증가하므로 $d\approx 1$. 결론: $t_{1/2}=0.693\cdot V/CL\propto BW^{d-b}\approx BW^{0.25}$ — 체중이 클수록 half-life가 길어지는 현상은 CL과 V의 서로 다른 scaling에서 **수학적으로 따라오는 결과**이지, 별도의 가정이 아니다. 이 한 줄을 입에 두면 “큰 동물은 절대 CL은 크지만 단위 시간당 fraction은 작다”는 C3 Failure Mode가 자동 차단된다.
...
**기억 고리:** Kallynochron은 'kg 단위로 정규화한 생리학적 시간'이고, Apolysichron은 '$BW^d$ 단위로 정규화한 생리학적 시간'이다. $d=1$이면 둘은 같아 보이지만, PK29 compound X처럼 $d>1$이면 차이가 드러난다.
...
Simple allometry는 $y=a\cdot BW^b$만으로 종간 스케일링을 완성하는 **직선 도로** — 종간 유사성이 높고 조정 요인이 없을 때 유효. Correction factors(MLP, brain weight, PPB correction)는 특정 약물에서 metabolic 또는 binding 차이가 클 때 필요한 **우회로** — 우회로를 쓸 이유가 없는 도로에서 강제로 우회하면 오히려 정확도가 떨어진다. BSA 변환($1.73(W/70)^{0.75}$)은 임상적으로 굳어진 우회로의 한 형태이며, 그 자체로 maturation/binding/disease state 문제를 해결하지 않는다는 점을 잊지 않는다.
...
*왜 이 질문 — [EXPERT_INFERENCE, ver2 V4]:* C1의 수식 골격을 손에서 입으로 옮길 수 있는지 확인. 이 회상이 안 되면 C2 이후 모든 추론이 흔들린다.
...
*왜 이 질문 — [EXPERT_INFERENCE, ver2 V4]:* 도출 한 번 손으로 해보면 $V/CL$과 $CL/V$의 부호가 반대라는 사실이 영구 각인된다. C3 Failure Mode("$k_{el}$ 부호 놓침")의 직접 예방책.
...
*왜 이 질문 — [EXPERT_INFERENCE, ver2 V4]:* 학술 보고서·심사 문서에서 source-internal 수치 불일치를 임의로 하나로 "고정"하는 것은 무결성 위반이다. citation scope를 함께 적는 습관이 들어야 한다.
...
*왜 이 질문 — [EXPERT_INFERENCE, ver2 V4]:* 보스 딜레마(Boss Dilemma). 점추정값과 physiological prior가 충돌할 때 무엇을 primary로 두고 무엇을 sensitivity로 보일지를 입으로 정당화할 수 있어야 한다. 동시에 ICH/FDA 단정문을 자발적으로 차단하는 source language discipline이 동작하는지 확인한다.
...
1. **동물-인간 노출 번역 표(Animal-to-human exposure translation table):** C1이 표 골격(table equation), C2가 CL column, C3가 V·시간 column, C4가 곡선 중첩 검증(curve-superposition check), C5가 equal-AUCu dose backbone이다.
2. **모집단 PK 체중 공변량 결정(PopPK weight covariate decision):** `[교과서 외 구현 해석]` WT exponent를 fix할지 estimate할지 결정할 때 C2의 physiological prior와 C3의 V exponent가 동시에 필요하다.
...
### C. 전문가 해석 지점(Professional Moat) — diagnostic, not motivational
