# 세션 15 — 모델 구축의 기술: 초기값 · GOF · 모델 판별 · 실험 설계 — v4.0
...
AIC와 SC는 nested를 요구하지 않습니다. 하지만 **equal weighting은 반드시 요구**해요. [G&W p.389]
...
AIC/SC의 **가장 낮은 값이 자동으로 좋은 모델을 뜻하지는 않아요.** GOF(모델 적절성)는 잔차, parameter 정밀도, 정확도, 상관행렬, 조건수, F-test, AIC/SC를 함께 보는 **복합 진단 도구군**입니다. [G&W pp.387–391]
