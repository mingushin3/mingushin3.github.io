# 03 — 경구(혈관외) PK: F · ka · tmax · 흡수 — v4.0 보강본_final
...
| Transit/Erlang/Weibull 입력 함수 | §8 §E | 1차 입력 가정이 안 맞을 때의 확장 좌표계 (출처 범위 밖 — Savic et al. 2007, Langenbucher 1972). |
...
- **장기지속형 주사 제형 (LAI/Depot — IM/SC 저장소 입력) (보강):** Sandostatin LAR Depot(octreotide acetate, 매 4주), Zolodex(goserelin acetate implant, 매 3개월), Mirena(levonorgestrel-releasing IUD, 5년) 같은 modified-release 비경구 제형은 흡수 자체가 수 주~수 년 단위로 율속됩니다 [R&T p.170]. 이런 약물의 PK 곡선에서 관찰되는 terminal slope는 본질적으로 **흡수 속도상수**예요 ($k_a < k_{el}$, flip-flop의 임상 정의 [G&W Fig.2.17, p.30; R&T Case C, pp.175–176]). 정량 표기로 보면:
...
NONMEM 구현은 ADVAN2에서 매우 작은 KA를 두는 식으로, 또는 ADVAN1+TRANS2(IV-like elimination)에 시간 의존 입력으로 다룹니다. → terminal half-life를 "elimination half-life"로 보고하면 dose adjustment 판단이 완전히 어긋나는 가장 명확한 사례군이에요. R&T 본문은 이런 비경구 modified-release 제형을 chapter 도입부에 명시하는데, 흡수가 elimination 시계와 완전히 분리될 수 있다는 점이 oral PK와 동일한 flip-flop 좌표 안에서 작동합니다.
...
> 🔎 **충돌의 가능한 출처 가설 (보강):** 원문을 다시 정독하면 PK2의 V/F 추정은 **solution II에서 나온 회귀값**과 **deconvolution으로 구한 F=82%**의 곱으로 V≈20 L가 보고됩니다 [G&W p.480]. 동일 사례에서 NCA 기반 F는 ~64%로도 제시되어 있어요 [G&W p.480]. 이 충돌의 가능한 출처는 네 가지로 추정됩니다 — **(i)** solution II의 V/F가 정확히 32 L가 아니라 25 L 영역일 가능성(원문 본문에는 V/F = 30 L가 *initial estimate*, solution III에서는 V/F = 6.54 L도 등장 — 즉 동일 PK2가 여러 solution path별로 다른 V/F를 갖는다 [G&W pp.481–482]), **(ii)** F가 deconvolved 82%인지 NCA 60–64%인지에 따라 V 환산값이 24~26 L vs 19~20 L로 갈림, **(iii)** 원문이 V≈20 L로 반올림 보고, **(iv) — 추가 가설:** IV reference의 **dose range가 oral dose와 다른 영역**에 있어 linearity assumption이 일부 위배되었을 가능성. 특히 NCA 기반 F 산정은 IV $AUC_{iv}/D_{iv}$를 reference로 쓰는데, IV의 dose 영역이 oral의 dose 영역과 겹치지 않으면 $F$ 산정에 systematic bias가 들어가고, 이 bias가 그대로 $V = (V/F) \times F$ 환산값에 흘러갑니다. 정량적으로:

$$
\underbrace{V_{est}}_{\text{V 환산값}} = \underbrace{(V/F)_{NLR}}_{\text{NLR solution별 추정}} \times \underbrace{F_{?}}_{\text{NCA or deconv F}}
\quad\text{where}\quad
\underbrace{F_{?}}_{\text{출처 따라 다른 F}} \in \{\overbrace{F_{NCA}}^{\sim 0.60-0.64},\;\overbrace{F_{deconv}}^{\sim 0.82}\}
$$

학습자가 모델링 보고서를 검토할 때 이런 **"같은 사례 내 다른 추정 경로"가 만드는 V·F·CL 환산 충돌**을 진단하는 훈련용 사례로 보시면 됩니다. 특히 NCA → NLR 전환 단계에서 가장 흔히 마주치는 systematic bias 출처가 바로 (iv) 영역이에요.
...
> 🔎 **충돌 가설 (보강):** 원문 PK2는 동일 사례 안에서 V/F가 *initial estimate*(30 L), *solution II*, *solution III*(6.54 L)별로 다르게 등장하고, F도 *deconvolved*(82%)와 *NCA*(60~64%) 두 값이 함께 제시됩니다 [G&W pp.480–482]. 그러므로 V≈20 L는 **solution II의 V/F × deconvolved F**의 곱으로 보고된 결과이고, 32 L는 다른 solution path의 V/F일 가능성이 높습니다. **추가 가설(보강 보강):** IV reference dose range가 oral dose 영역과 다르면 F 산정에 linearity violation에 의한 systematic bias가 들어가고, 그 bias가 V 환산값으로 흘러갑니다:

$$
\underbrace{V_{est}}_{\text{환산 V}}=\underbrace{(V/F)_{NLR}}_{\text{solution별 다름}}\times\underbrace{F_{ref}}_{\text{NCA vs deconv}}
$$

학습자가 모델링 보고서를 검토할 때 같은 데이터셋에서도 **추정 경로(NCA, deconvolution, NLR solution I/II/III)에 따라 V·F·CL 환산값이 다르게 떨어진다**는 점, 그리고 **IV reference의 dose linearity가 F 산정에 sneaky하게 들어간다**는 점을 진단하는 훈련 사례예요.
...
### E. 1차 입력 가정이 안 맞을 때 — Transit/Erlang/Weibull 입력 함수 (보강)

지금까지 다룬 입력 함수는 **1차(first-order)와 0차(zero-order) 두 가지**예요. G&W §2.2.9 [G&W p.43]는 두 입력의 조합도 언급하지만, 실제 데이터에서 더 자주 만나는 패턴은 **"1차도 0차도 아닌 sigmoidal absorption"**입니다. 이런 곡선을 1차 모델로 강제로 맞추면 lag-time이 비현실적으로 커지거나 $k_a$가 작은 값에 고착돼요. 이 자리에서 등장하는 게 transit compartment / Erlang / Weibull 같은 확장 입력 함수입니다.

> **출처 범위 노트:** 본 핸드아웃의 주 출처(G&W 5e Ch.2 §§2.2.4–2.2.12 [G&W pp.28–47], R&T 5e Ch.6 [R&T pp.169–195], Ch.7 [R&T pp.197–220])는 transit compartment / Erlang / Weibull을 명시적으로 다루지 않습니다. 아래 한 단락은 PopPK 실무에서 1차 흡수가 안 맞을 때 가장 흔히 검토되는 **확장 모델 좌표계**의 개념적 anchor이며, 본 핸드아웃 내 정량 anchor는 외부 reference에 기댑니다. G&W 본문은 multiple absorption sites만 Eq.2:80–2:82로 명시적으로 다루므로 [G&W pp.46–47], 그 너머는 별도 출처 표기합니다.

#### E-1. Transit compartment / Erlang absorption

흡수 부위에서 중심구획까지를 단일 step이 아니라 **n개의 직렬 통과 단계(transit chain)**로 두는 모델이에요. 임의의 i번째 transit 컴파트먼트에 대해:

$$
\underbrace{\frac{dA_i}{dt}}_{\text{i단계 변화}} = \underbrace{k_{tr}\cdot A_{i-1}}_{\text{직전 단계 유입}} - \underbrace{k_{tr}\cdot A_i}_{\text{다음 단계 유출}}
\quad(i=1,2,\ldots,n)
$$

마지막 transit($A_n$)에서 중심구획으로 들어가는 입력률:

$$
\underbrace{\text{Input}_{\text{central}}(t)}_{\text{중심구획 유입}} = \underbrace{k_{tr}\cdot A_n(t)}_{\text{마지막 transit}} = \underbrace{F\cdot D\cdot \frac{(k_{tr}\,t)^n\, e^{-k_{tr}\,t}}{n!}\cdot k_{tr}}_{\text{Erlang(n+1, k}_\text{tr}\text{) 분포 곡선}}
$$

이건 **Erlang 분포(특수 형태의 Gamma 분포)**의 모양을 갖습니다. 주요 특성:

- **NTR(number of transit compartments) = 1** → 1차 흡수와 동일 ($k_a = k_{tr}$)
- **NTR ≫ 1** → 입력 함수가 sigmoidal해지고 자연스러운 lag가 생김 (별도 $t_{lag}$ 파라미터 불필요)
- **Mean Transit Time:**

$$
\underbrace{MTT}_{\text{평균 통과시간}} = \underbrace{\frac{n+1}{k_{tr}}}_{\text{n개 chain의 누적 통과시간}}
$$

`[참고 — 출처 범위 밖: Savic RM, Jonker DM, Kerbusch T, Karlsson MO. Implementation of a transit compartment model for describing drug absorption in pharmacokinetic studies. J Pharmacokinet Pharmacodyn 2007;34(5):711–726.]`

→ Lag-time 모델이 "step function"처럼 t < t_lag에서 입력이 정확히 0이다가 t = t_lag에서 갑자기 시작하는 데 비해, transit chain은 부드럽게 흡수가 시작되는 형태를 자연스럽게 잡습니다. NONMEM에서는 `$DES` 블록에 n개의 컴파트먼트를 명시하거나, explicit MTT 파라미터화로 구현해요.

#### E-2. Weibull absorption — 서방형/용해도 제한 사례

Weibull 입력 함수는 누적 분율을 다음과 같이 정의합니다.
...
$$
\underbrace{\frac{dF}{dt}}_{\text{순간 흡수율}} = \underbrace{\frac{\beta}{T_d}\left(\frac{t}{T_d}\right)^{\beta-1}}_{\text{shape × scale 항}}\cdot\underbrace{\exp\!\left[-\left(\frac{t}{T_d}\right)^\beta\right]}_{\text{지수 감쇠}}
$$

주요 특성:

- **β < 1**: 초반 빠른 release, 점차 둔화 (용해도 제한 또는 burst release 제형)
- **β = 1**: 1차 흡수와 등가 ($k_a = 1/T_d$)
- **β > 1**: sigmoidal release (controlled-release, osmotic pump 같은 zero-order 근사 제형)

`[참고 — 출처 범위 밖: Langenbucher F. Linearization of dissolution rate curves by the Weibull distribution. J Pharm Pharmacol 1972;24(12):979–981.]`

→ Weibull은 **dissolution kinetics가 1차에서 벗어날 때** input function 표현에 자주 쓰입니다. modified-release 제형의 BE/식사 효과 시험에서 NCA만으로는 못 보는 release shape 차이를 잡아내는 도구예요.

#### E-3. 어느 모델을 언제 쓰는가 — 진단 표

| 곡선 특징 | 1차 적합 시 증상 | 권장 입력 함수 |
|---|---|---|
| 곡선이 "step function"처럼 lag 후 시작 | lag-time이 비현실적으로 커짐 | Transit compartment (NTR 작은 정수) |
| 흡수 시작이 점차 가속됨 (sigmoidal rise) | $k_a$가 작은 값에 고착, residual error 증가 | Transit compartment (NTR 큰 값) 또는 Weibull (β > 1) |
| Modified-release 곡선의 release shape가 1차에서 명백히 벗어남 | poor fit at multiple time points | Weibull (β fit) |
| 다중 peak 또는 비정형 상승부 | 단일 $k_a$로 OFV 안정되지만 잔차 패턴 이상 | Multiple absorption sites [G&W Eq.2:80–2:82, pp.46–47] |
| Dose escalation에서 곡선 형태 변화 + dose-normalized AUC ↓ | 1차 모델이 점점 안 맞음 | Card 5 §D의 saturable input 좌표 |

> 💼 **실무 인사이트 — Transit과 lag-time의 trade-off:** transit compartment는 NTR 파라미터(또는 고정 정수)를 추가하면서 일반적으로 더 부드러운 sigmoidal absorption profile을 제공합니다. 하지만 transit chain은 lag-time과 trade-off 관계예요 — 같은 sigmoidal 시작을 lag로 잡을 수도, transit으로 잡을 수도 있어서 두 파라미터가 동시에 추정되면 식별가능성이 떨어질 수 있어요. 일반적으로 **lag-time을 0으로 고정하고 transit으로만 흡수 시작을 잡는 게** identifiability 측면에서 더 안전한 경우가 많습니다. 정량적으로 표현하면:

$$
\underbrace{\text{Lag model}}_{\text{step function 시작}}: t < t_{lag} \Rightarrow C=0 \quad\text{(이산 trade-off)}
$$
...
→ Card 4의 식별가능성 논리가 그대로 이어집니다 — 1차 입력에서 transit/Weibull로 옮기는 순간 새로운 apparent parameter($k_{tr}$, NTR, $T_d$, β)가 등장하고, **이들이 분리 추정 가능한가**는 다시 IV 데이터·dose range·sampling design의 문제로 돌아갑니다.
...
**최종 요약:** 경구 PK의 핵심은 $C(t)$를 잘 맞추는 데서 끝나지 않습니다. 그것보다 더 중요한 게 있어요 — **$C(t)$에서 실제로 분리 가능한 것과 분리 불가능한 것을 정직하게 가르는 일.** 경구 단독 데이터는 $V/F$, $CL/F$, 겉보기 $k_a$, terminal slope ambiguity를 남깁니다. 이 네 가지를 명시하면 handout은 안전하고, 생략하면 모델은 그럴듯하지만 위험해집니다. 그리고 마지막 두 축 — **BCS/BDDCS는 수식 이전에 약물의 물리화학적 좌표가 답의 절반을 미리 결정한다는 사실**을 보여주는 framework이고, **Transit/Erlang/Weibull은 1차 입력 가정이 깨질 때 다음 좌표계가 어디인지**를 알려주는 확장 framework입니다.

---

<!--
## v3.8 자가검증 체크리스트 (보강본_final 업데이트)
...
| Overbrace/underbrace annotation | 신규 수식 전수 | ✓ |
| G&W/R&T 출처 정확성 | 페이지/Eq./Fig./Table 번호 | ✓ |
| §8 §E 신설 — 출처 범위 노트 | Transit/Erlang/Weibull | ✓ (Savic 2007, Langenbucher 1972 명시) |

보강 항목 누적 요약:
- Card 4 사례 앵커: PK2 V=20 vs 26 충돌의 가능한 출처 가설 박스 추가 (§7 Q4와 연동) — **보강본_final에서 (iv) IV reference linearity violation 가설 추가**
- Card 4 사례 앵커: **(보강본_final 신규)** Depot/LAI 제형 (Sandostatin LAR Depot, Zolodex, Mirena) flip-flop 임상 앵커 추가 [R&T p.170]
...
- §7: Q9 (dose-normalized AUC 진단) 추가, Q4 충돌 가설 박스에 IV reference linearity 가설 추가
- §8: §D BCS·BDDCS framework 추가 [R&T Tables 7-8/7-9, pp.219–220]
- §8: **(보강본_final 신규) §E Transit/Erlang/Weibull 입력 함수 framework 추가 — Savic et al. 2007 (출처 범위 밖), Langenbucher 1972 (출처 범위 밖) 명시 인용**
- §8: 변화 상황 표 1행 + 실패 모드 표 1행 추가 (superposition 위배)
- §8: §E → §F (Ch.7 맥락), §F → §G (최종 확정 문장)으로 renumber
- §0 큐레이션 지도: Transit/Erlang/Weibull 행 CONTEXT 표에 추가
- 표기 일관성: 수식 annotation에 overbrace/underbrace 보강 — 보강본_final에서 모든 신규 수식 적용
-->
...
- v4.0 보강본 → v4.0 보강본_final 변환 (현재): 검토 의견 추가 반영. (1) Card 4 §C 사례 앵커에 **장기지속형 주사 제형(LAI/Depot)** flip-flop 임상 앵커 추가 [R&T p.170] — Sandostatin LAR Depot, Zolodex, Mirena, (2) Card 4 §C PK2 충돌 가설 박스에 **(iv) IV reference dose range linearity violation** 가설 추가 + overbrace/underbrace annotation, (3) §7 Q4 충돌 박스 동기화 + 환산 수식 overbrace/underbrace 보강, (4) **§8 §E 신설 — Transit compartment / Erlang / Weibull 입력 함수 framework** (1차 입력 가정이 안 맞을 때의 확장 좌표계). 출처 범위 노트 명시(G&W 5e Ch.2 §§2.2.4–2.2.12 [G&W pp.28–47], R&T 5e Ch.6 [R&T pp.169–195], Ch.7 [R&T pp.197–220]는 transit/Erlang/Weibull을 명시적으로 다루지 않음); 외부 reference 정확 인용 (Savic RM et al. J Pharmacokinet Pharmacodyn 2007;34(5):711–726; Langenbucher F. J Pharm Pharmacol 1972;24(12):979–981), (5) §0 큐레이션 지도의 CONTEXT 표에 Transit/Erlang/Weibull 행 추가, (6) §8 §E → §F (Ch.7 맥락), §F → §G (최종 확정 문장)으로 renumber. 보강본 → 보강본_final 모든 신규 수식에 overbrace/underbrace annotation 적용, G&W·R&T 페이지/Eq./Fig./Table 번호 정확 인용. 원본 콘텐츠·약물·수치·출처·수식·SLIDE_START/SECTION_CORE 마커 전수 보존.
-->
