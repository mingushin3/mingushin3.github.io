# 세션 04 — 청소율(Clearance): 신·간·IVIVE·대사체 — $CL_R$, $CL_H$, well-stirred, parent-metabolite — v4.0 보강본_final

> **본 보강본_final의 변경 요약 (v4.0 → v4.0 final)**
> - **v4.0에서 완전 보존**: §1, §1.5, M1, M3, M5–M7, M9, M11–M14, §5 혼동 쌍 1–3, 5–10, §7 Q1–Q8, §8A–§8G의 v4.0 본문 내용
> - **v4.0에서 추가 부분 보강 (final)**:
>   - **M8**: 측정 매트릭스 구분 (plasma vs serum vs whole blood) + $f_u \leftrightarrow f_{ub}$ 명시 변환식 추가
>   - **M10**: Sirianni-Pang Extended Clearance Model 정량식(B절) + Transporter induction vs inhibition (C절) 신규 추가
>   - **§5 Pair 4 ($f_u$ vs $f_{ub}$)**: 측정 매트릭스 anchor (tacrolimus/cyclosporine 전혈 vs warfarin plasma vs digoxin serum) 한 줄 추가
> - **v4.0에서 이미 반영된 항목 (재확인)**: M2의 1차 재흡수 가정 한계 + 신장 transporter 임상 anchor; M2.5 (eGFR/담즙/장간순환); M4의 CYP 분포·다형성·염증; M10.5 (Phase II UGT/SULT); §8G (Allometric scaling); Q9의 산출 단계 명시화
> - **수식 보강 원칙**: 새로 추가하는 수식은 모두 `\overbrace` / `\underbrace` annotation 적용. 가브리엘슨(G&W) / 토저(R&T) 출처 page·equation 번호 정확 기재.
...
$$
\overbrace{\underbrace{CL_R}_{\text{순 신장 CL}}=\underbrace{(1-F_R)}_{\text{재흡수 잔여율}}\cdot(\underbrace{CL_f}_{\text{여과}}+\underbrace{CL_S}_{\text{분비}})}^{\text{1차 재흡수 가정 하의 단순 모델 [R\&T p.139, Eq 5-17–5-18]}}, \qquad \underbrace{CL_f}_{\text{여과 CL}}=\underbrace{f_u}_{\text{유리분율}}\cdot\underbrace{GFR}_{\text{여과율}}
$$

$CL_R > f_u \cdot GFR$이면 분비(secretion)가 작동하고 있다는 뜻이고, $CL_R < f_u \cdot GFR$이면 재흡수(reabsorption)가 일어나거나 측정 방식이 잘못됐는지 의심해야 합니다.

**🆕 1차 재흡수 가정의 한계** [R&T pp.142–148, Figs 5-16 & 5-18]

위 $(1-F_R)$ 인자는 **재흡수가 농도와 무관한 일정 분율로 진행된다는 1차(first-order) 재흡수 가정**에 기댄 단순 모델이에요. 실제 신세뇨관 재흡수는 **세 가지 생리·물리화학 변수에 강하게 의존**합니다 — ① **소변 유속(urine flow)**, ② **소변 pH**, ③ **약물의 친유성/이온화도**.
...
| 약물·조건 | 산성 소변 (pH 4.9–5.3) | 알칼리성 소변 (pH 7.8–8.2) | 출처 |
|---|---|---|---|
| **Methamphetamine** (pKa 10, 약염기) | 회수율 **70–80%** | 회수율 **1–2%** | R&T p.145, Fig 5-16 |
| **Phenobarbital** (pKa 7.2, 약산) | 낮은 $CL_R$ | 알칼리화 시 $CL_R$ ↑, 유속 의존성 ↑ | R&T p.147, Fig 5-18 |

→ 단일 약물에서 **소변 pH 하나로 $CL_R$가 자릿수 단위로 흔들립니다.** 이게 단순 $(1-F_R)$ 분율로 봉합될 수 없는 비선형성의 직접 증거예요. M3의 methamphetamine 사례가 바로 이 가정의 한계를 보여주는 임상 anchor고, 강제 이뇨(forced diuresis)와 pH 조절이 약물 중독 치료에 사용되는 이유도 여기 있습니다 [R&T p.147].

**🆕 신장 분비 단계 transporter DDI — 임상 정량 anchor** [R&T pp.140–142; R&T 분포 챕터 Fig 4-8 p.86]

신장 근위세뇨관에는 분명한 transporter 지도가 있어요 — basolateral 쪽에 **OAT1/OAT3, OCT2**가, apical 쪽에 **MATE1/MATE2-K, MRP2/MRP4, P-gp**가 배치돼 있어서 약물의 유입과 유출이 분리되어 일어납니다. 이 단계에서 일어나는 transporter DDI는 well-stirred처럼 단일 식으로 압축되지 않고, 분비 청소율 자체를 직접 흔듭니다.

| DDI 짝 | 기전 | 결과 | 출처 |
|---|---|---|---|
| **Metformin + cimetidine** | OCT2/MATE inhibition | Metformin AUC 약 1.5배 증가 (정량 anchor) | 임상 DDI 가이던스 표준 사례 |
| **Probenecid + penicillin** | OAT inhibition | β-lactam 분비 차단, 농도 유지 (고전적 anchor) | R&T Chapter 17 DDI 참조 |
| **Dolutegravir + metformin** | MATE1 inhibition | Metformin 노출 상승 | 임상 라벨링 anchor |
| **Tenofovir nephrotoxicity** | OAT1/3 매개 uptake | 세뇨관 세포내 농도 상승 → 신독성 | 임상 PV 사례 |

→ 신장 transporter DDI는 **간 OATP1B1 DDI**(M10에서 다룸)와 대칭적이지만 임상 anchor가 상대적으로 적게 다뤄지는 영역이에요. PopPK에서 신장 covariate를 GFR 단독으로만 잡으면 OCT2/MATE 다형성이나 동반 약물 효과가 잔차 변이로 흡수돼버립니다.
...
담즙 배설과 장간순환은 소실 곡선을 바꿀 수 있지만, 본 문서에서는 M2의 경로 분해 안에서 한꺼번에 다룹니다 [R&T pp.137–138] (정량 anchor는 신규 카드 M2.5 참조). 폐 청소율은 장기 가산성 해석에서 특수 예외로만 기억합니다 [R&T p.128].
...
<!-- SLIDE_START: M2.5 | title: M2.5. eGFR 추정식·담즙 청소율·장간순환(NEW) -->
<!-- SECTION_CORE: SC-04B -->

### 🆕 M2.5. eGFR 추정식·담즙 청소율·장간순환(eGFR estimation + biliary clearance + enterohepatic cycling)

**앞에서 → 여기서**. M2가 신장 경로의 *생리학적 분해*를 세웠다면, 이 카드는 **그 식의 covariate를 임상에서 무엇으로 채우느냐**(eGFR 추정식)와 **신장 가산성 식에서 빠져 있던 두 가지 경로**(담즙 + 장간순환)를 채워 넣습니다.

**거장의 시각.** 신부전 라벨링 보고서를 받으면 거의 항상 "CrCl 30 mL/min에서 50% 감량" 식 권고가 등장합니다. 그런데 그 CrCl이 **어떤 추정식으로 산출됐는지**(Cockcroft-Gault인지 CKD-EPI인지)는 거의 보고되지 않아요. 같은 환자에서 두 식의 차이가 20–30%인 경우가 흔합니다. 이 차이가 라벨링 cutoff 결정을 흔드는 자리에 들어가면, 같은 환자가 식에 따라 "감량 대상"이 되거나 안 되거나 갈라져요.

#### A. eGFR 추정식 — PopPK covariate 선택 framework

**신기능 covariate 운영의 3대 추정식**

| 추정식 | 입력 변수 | 단위 | 임상 위치 |
|---|---|---|---|
| **Cockcroft-Gault (CG)** | 혈청 Cr, 연령, 체중, 성별 | mL/min | 고전 표준. 약물 라벨 cutoff (예: 신부전 감량 권고)에서 여전히 가장 자주 쓰임 |
| **MDRD** | 혈청 Cr, 연령, 인종, 성별 | mL/min/1.73 m² | 만성 신질환 진단·분류 |
| **CKD-EPI** | 혈청 Cr (또는 Cystatin C), 연령, 인종, 성별 | mL/min/1.73 m² | 정상 신기능 영역에서 MDRD보다 정확. 현재 임상 권장 |
...
**PopPK covariate 선택 시 본능적 점검 사항**:
- **단위 차이 점검**: CG는 mL/min (절대 청소율), MDRD/CKD-EPI는 mL/min/1.73 m² (체표면적 정규화). 약물 라벨링의 cutoff가 어떤 단위인지 확인하지 않으면 같은 환자가 다르게 분류돼요.
- **소아·노인·비만 환자**: CG는 체중 항으로 비만 환자에서 과대평가(adjusted body weight 적용 필요). CKD-EPI는 노인에서 더 정확.
- **약물 라벨 cutoff와의 정합성**: 대부분 신부전 용량 권고는 CG 기반이라, PopPK에서 CKD-EPI로 covariate를 잡아도 라벨링 검증 단계에서는 CG로 재계산하는 게 안전합니다.

#### B. 담즙 청소율(Biliary clearance) [R&T pp.137–138, Eq 5-16]

신장과 간 외 **제3의 배설 경로**입니다. M2의 가산성 식에서 $CL_{other}$로 묶이지만, 실제로는 정량 anchor를 가질 수 있어요.
...
| 파라미터 | 단위 | 생물학적 의미 | 변화 원인 |
|---|---:|---|---|
| Bile flow | mL/min | 담즙 생성 속도 (간 정상값) | 약 0.5–0.8 mL/min [R&T p.137] |
| $C_{bile}/C_{plasma}$ | ratio | 담즙 농축비 | apical efflux transporter (P-gp, MRP2, BCRP) 활성 |
| $CL_{\text{biliary}}$ | mL/min | 담즙 경로 CL | 최대 500 mL/min 이상 가능 (간 혈류로 제한) |

> 🔑 **고담즙 CL의 4가지 조건** [R&T p.138]: ① apical efflux transporter 활성 기질, ② 극성, ③ 분자량 > 350 g/mol, ④ 이온화 가능. 글루쿠로니드(MW > 350, 극성, pKa ~3)가 전형적인 고담즙 CL 기질이에요 — 이게 M10.5의 Phase II 대사체 처리와 직접 연결됩니다.

#### C. 장간순환(Enterohepatic cycling) [R&T p.138, Fig 5-12]

**중요한 정정**: 장간순환은 **소실(elimination)이 아니라 분포(distribution)의 구성요소**입니다 [R&T p.138]. 즉, 약물이 담즙으로 분비됐다가 장에서 재흡수되면 *체외 배설이 일어나지 않은* 것이고, 따라서 청소율 가산성 식 $CL = CL_R + CL_H + CL_{\text{other}}$에는 장간순환 자체가 들어가지 않습니다 — 들어가는 건 그중 **재흡수되지 않고 분변으로 빠진 부분**만이에요.
...
**임상 신호** — 장간순환의 흔적은 **PK 곡선의 secondary peak**(2차 피크), **반감기 연장**, 그리고 **β-glucuronidase 영향을 받는 항생제 동반투여 시 변화**로 나타납니다. 특히 글루쿠로니드 대사체가 장내 세균 β-glucuronidase에 의해 가수분해되어 모약물로 되돌아오는 메커니즘이 흔해요 — **mycophenolate mofetil(MMF)의 MPA-MPAG 순환**이 대표적입니다.

#### D. NONMEM 구현 신호 (PopPK 실무 anchor)

- 장간순환 존재 시 **2차 피크가 잡히는 시점에 dose 입력(MTIME 또는 transit chain)을 넣어 fitting**합니다. 단순 1구획 또는 2구획으로 fitting하면 terminal slope가 시간에 따라 변화하는 인상을 줘서 비선형 PK로 오인할 수 있어요.
- 담즙 CL은 **drug-specific bile collection study가 없으면 직접 추정 불가**입니다. NONMEM에서 $CL_{\text{biliary}}$를 자유 파라미터로 두면 hepatic CL과 confounding이 발생합니다.

**이 카드의 한 줄**: 신부전 covariate를 정의할 때 본능은 **어떤 eGFR 식이 라벨 cutoff와 정합하는지** 먼저 확인하는 거예요. 그리고 신장·간 외 제3 배설 경로(담즙·장간순환)를 만났을 때, **장간순환은 분포의 일부이지 소실이 아니라는** 정정을 본능적으로 떠올립니다. 이 두 가지를 놓치면 신부전 라벨이 잘못된 cutoff에서 결정되고, 2차 피크가 비선형 PK로 오진단됩니다.
...
<!-- SLIDE_START: M4 | title: M4. Well-stirred 간 청소율과 4-모델 압축 -->
<!-- SECTION_CORE: SC-06 -->
...
#### B. 🆕 $CL_{int}$ 분해 — 효소·유전형·염증의 세 입력

위 식에서 $CL_{int}$는 단일 숫자가 아니에요. 실제로는 **효소 분포 + 다형성 + 시간 의존 변동**의 합성입니다.

##### B1. 주요 CYP 효소 분포 [R&T pp.121–123, Figs 5-2 & 5-3]

| CYP 효소 | 간 단백 abundance | 처방약 CL 기여 | 특기 사항 |
|---|---|---|---|
| **CYP3A4/5** | 최대 (간·장벽 양쪽) | ~40–50% | 가장 광범위한 기질 |
| **CYP2D6** | ~2% | ~25% | 다형성 임상 영향 큼 (염기성 약물) |
| **CYP2C9** | 다수 | 산성 약물 우세 | warfarin, NSAIDs |
| **CYP2C19** | 다수 | omeprazole, clopidogrel | 다형성 |
| **CYP1A2** | 다수 | caffeine, theophylline | 흡연 유도 |
| **CYP2E1** | 다수 | 알코올, halothane | acetaminophen 활성화 |

CYP3A4와 CYP2D6를 *간 단백 abundance*와 *임상 CL 기여*가 비대칭적인 좌표로 기억해두면 임상 직관이 잡힙니다 — CYP2D6는 abundance가 2%인데 처방약 CL의 25%를 담당해요 [R&T p.122].

##### B2. 🆕 CYP 다형성 — 정량 phenotype [R&T pp.549–559, Fig 17-15, Table 17-6]

대표적인 **CYP2D6** 다형성 분류와 그에 따른 약물 노출 변화:

| Phenotype | 유전형 | 활성 분율 | 임상 결과 |
|---|---|---|---|
| **PM** (Poor metabolizer) | *4/*4 등 무효 대립유전자 | ~0 | substrate AUC ↑↑ (4배 이상) [R&T p.549] |
| **IM** (Intermediate) | *10/*10 등 활성 감소 | 0.25–0.5 | 중간 노출 |
| **EM** (Extensive) | 야생형 | 1.0 (참조) | 정상 노출 |
| **UM** (Ultra-rapid) | *1×N gene duplication | 1.5–2+ | substrate AUC ↓ → 약효 부족 (codeine→morphine의 경우 독성) |

$$
\overbrace{\text{Maximum exposure ratio (PM, inhibitor)}}^{\text{R\&T Eq 17-18, p.557}}=\frac{1}{\underbrace{1-f_{m,\text{POLY}}-f_{m,\text{NP}}}_{\text{차단되지 않은 경로 분율}}}
$$

여기서 $f_{m,\text{POLY}}$가 0.75에 도달하면 PM에서 nonpolymorphic pathway 강력 억제 시 노출비가 10배 이상까지 증가할 수 있습니다 [R&T p.557, Fig 17-15]. **Desipramine + quinidine** (CYP2D6 강력 억제) 조합에서 EM이 사실상 PM phenotype으로 전환되며 CL이 4배 감소, $t_{1/2}$가 4배 연장됩니다 [R&T p.549].

→ 그래서 **CYP polymorphism 위에 다른 경로 inhibitor가 들어가는 시나리오**가 임상 DDI 위험이 가장 크게 증폭되는 자리예요.

##### B3. 🆕 염증성 CYP 다운레귤레이션 (Cytokine-mediated)

패혈증·외상·종양·자가면역 등 **염증성 상태에서 IL-6, TNF-α, IL-1β가 hepatic CYP 발현을 억제**합니다. CYP3A4가 가장 민감한 것으로 알려져 있고, CYP1A2·2C9·2D6·2C19도 영향을 받아요. PopPK 실무에서는:

- **임상 PK가 정상 자원자 데이터에서 외삽되지 않는 cohort**(중증 감염, ICU 환자, 진행성 종양)에서 자주 관찰
- $CL_{int}$가 환자 상태에 따라 시간에 의존적으로 변하므로, 베이지안 MAP에서 individual estimate가 지속적으로 한 방향으로 치우치면 의심
- COVID-19 같은 cytokine storm 상태에서 narrow therapeutic window 약물(immunosuppressant, anticoagulant)의 노출 변동이 임상적으로 보고됨

→ 이게 단순 IIV(개체간 변이)가 아니라 **체계적 IOV(occasion 변이)** 신호로 분리되어야 PopPK fitting이 안정화됩니다.

**이 카드의 한 줄**: 간 CL을 해석할 때 반드시 짚는 비교는 단 하나 — **$Q_H$ vs $f_{ub} \cdot CL_{int}$**. 이걸 안 보고 가면 high/low extraction 분류가 통째로 흔들리고, 단백결합·DDI 해석이 뒤집힙니다. 그리고 분류를 plasma 기준으로 하는 순간 단위 자체가 어긋나기 때문에(→ M8), $CL_b$로 변환한 뒤에 비교한다는 순서가 본능이 돼야 해요. **그리고 $CL_{int}$ 자체가 효소 분포·다형성·염증이라는 3-축 합성임을 항상 기억합니다.**


<!-- SLIDE_START: M5 | title: M5. 고/저 추출률과 투여 경로 매트릭스 -->
<!-- SECTION_CORE: SC-07 -->
...
<!-- SLIDE_START: M6 | title: M6. IVIVE 함정: 단일점, MMP, 데이터 압축 -->
<!-- SECTION_CORE: SC-08 -->
...
② **MMP 보정 함정**: 간 g당 microsomal protein 보정은 평균값 하나로 끝나지 않아요. 변이와 비율 처리에 민감합니다 [G&W p.87]. G&W Table 2.8은 MMP를 20/50/77로 바꿨을 때 같은 약물이 low extraction → intermediate → high까지 *분류 자체*가 바뀌는 것을 보여줍니다.
...
<!-- SLIDE_START: M7 | title: M7. PK5 혈장+소변 동시 적합 -->
<!-- SECTION_CORE: SC-09 -->
...
<!-- SLIDE_START: M8 | title: M8. 혈장-혈액 비와 혈액 청소율 -->
<!-- SECTION_CORE: SC-10 -->
...
#### B. 🆕 $f_u \leftrightarrow f_{ub}$ 명시 변환식 — IVIVE·PBPK 단위 정합성의 1차 점검

PopPK·PBPK 검토 회의에서 가장 자주 발견되는 *조용한* 단위 오류는 **plasma 측정에서 얻은 $f_u$를 well-stirred 식의 $f_{ub}$ 자리에 그대로 대입하는 것**입니다. 두 분율은 서로 다른 농도 기준 위에 정의되어 있어요:
...
비결합 농도 $C_u$는 plasma·blood 어느 쪽에서 측정하든 같습니다(자유 분자는 RBC와 plasma water 사이에서 동일하다는 평형 전제). 따라서 [R&T App.D pp.775–776 Eq D-4–D-5]:

$$
\overbrace{\underbrace{f_{ub}}_{\text{blood 유리분율}}=\underbrace{f_u}_{\text{plasma 유리분율}}\cdot\underbrace{\frac{C}{C_b}}_{\text{변환비 }R}}^{\text{App.D 변환식 — well-stirred에 넣기 직전 필수 step}}
$$

| 시나리오 | $R = C/C_b$ | 결과 |
|---|---:|---|
| $R = 1$ (RBC와 plasma 분배 균등) | 1.0 | $f_u = f_{ub}$ — 변환 불필요 |
| $R < 1$ (약물이 RBC에 농축됨, 예: tacrolimus, cyclosporine) | 약 0.05–0.5 | $f_{ub} \ll f_u$ — plasma 기준 $f_u$를 그대로 쓰면 well-stirred 분자가 통째로 과대평가 |
| $R > 1$ (약물이 plasma에 우세) | > 1 | $f_{ub} > f_u$ — 드물지만 발생 |

→ **이 변환을 생략하면** well-stirred 식의 $f_{ub} \cdot CL_{int}$ 분자가 잘못된 단위에서 평가되어, 같은 약물이 high extraction으로 분류되거나 그 반대로 분류되는 자릿수 오류가 생깁니다 [R&T pp.124–128].

#### C. 🆕 측정 매트릭스 구분 — Plasma vs Serum vs Whole Blood

임상 PK/TDM 결과를 받아 PopPK에 입력할 때 **어느 매트릭스에서 측정됐는지**를 확인하지 않으면, 위 §B의 $f_u \leftrightarrow f_{ub}$ 변환을 적용해야 할 약물에 잘못된 입력값이 들어갑니다.

| 매트릭스 | 정의 | 임상 측정의 대표 약물 |
|---|---|---|
...
| **Whole blood** (전혈) | RBC 포함 전체 혈액 | **tacrolimus, cyclosporine, sirolimus, everolimus** — RBC에 강하게 결합/분배 |

**왜 tacrolimus·cyclosporine은 전혈 측정이 표준인가**: 두 약물 모두 RBC 결합/분배가 매우 강해서 $C/C_b$가 약 **0.05–0.2** 수준이에요. Plasma 농도는 hematocrit 변동에 따라 자릿수 단위로 흔들리지만, whole blood 농도는 상대적으로 안정합니다. → 그래서 **이식 환자 TDM은 whole blood**를 표준 매트릭스로 채택해요.

**PopPK 실무 신호**:
- **매트릭스가 mismatch 상태로 입력되면**: tacrolimus whole blood 농도를 plasma 농도로 잘못 해석하면 $V$가 약 5–20배 과소평가되고, 모든 후속 PK 파라미터가 어긋납니다.
- **NONMEM control stream**에서 `$INPUT`에 `MATRIX` 또는 `CMT` flag로 매트릭스를 명시하고, $C/C_b$ 변환을 covariate 또는 fixed scalar로 처리하는 게 표준입니다.
- **CRO·임상 검사실 보고서 검토 시 본능적 점검**: 단위 옆의 매트릭스 표기(예: "ng/mL whole blood")를 확인하기 전에 PopPK 모델을 fitting하지 마세요.

> 🔑 **3-매트릭스 본능 점검:** PK 결과를 받자마자 본능은 ① 매트릭스(plasma/serum/whole blood) → ② $C/C_b$ → ③ well-stirred 식에서 $f_u$를 $f_{ub}$로 변환 — 이 3단 정합성을 확인하는 것이에요. 한 단계만 빠져도 hepatic extraction 분류와 PBPK 입력이 통째로 흔들립니다.
...
<!-- SLIDE_START: M9 | title: M9. 투과성 속도 제한 간 흡수: App.E ρ -->
<!-- SECTION_CORE: SC-11 -->
...
<!-- SLIDE_START: M10 | title: M10. 확장 청소율: 수송체–효소 구조 -->
<!-- SECTION_CORE: SC-12 -->
...
#### B. 🆕 Sirianni-Pang Extended Clearance Model — 정량식

M9의 $\rho$ 개념을 **3개 막투과 단계로 펼친 정량 모델**입니다. Sirianni & Pang(1997)이 well-stirred의 단순 가정을 풀어내고 uptake-metabolism-efflux를 서로 다른 청소율로 분리했어요 [R&T App.E pp.778–779; R&T pp.136–138 본문 box]:

$$
\overbrace{\underbrace{CL_{H,b}}_{\text{Extended 간 CL}_b}=\frac{\underbrace{Q_H}_{\text{혈류}}\cdot\underbrace{f_{ub}}_{\text{혈액 유리분율}}\cdot\underbrace{PS_{inf}}_{\text{유입 투과}}\cdot(\underbrace{CL_{int,met}}_{\text{대사 CL}}+\underbrace{PS_{eff,bile}}_{\text{담즙 efflux}})}{\underbrace{Q_H}_{\text{혈류}}(\underbrace{PS_{eff,sin}+CL_{int,met}+PS_{eff,bile}}_{\text{세포 내 처리 합}})+\underbrace{f_{ub}\cdot PS_{inf}}_{\text{유입 단계}}(\underbrace{CL_{int,met}+PS_{eff,bile}}_{\text{비유입 처리}})}}^{\text{Sirianni-Pang Extended Clearance [R\&T App.E Eq E-9–E-15]}}
$$

| 기호 | 의미 | 생리학적 자리 |
|---|---|---|
| $PS_{inf}$ | sinusoidal uptake permeability·surface | basolateral OATP/OCT/OAT |
| $PS_{eff,sin}$ | sinusoidal efflux permeability·surface | basolateral MRP3/4 (혈액 방향) |
| $CL_{int,met}$ | 세포 내 대사 청소율 | CYP/UGT |
| $PS_{eff,bile}$ | canalicular efflux permeability·surface | apical P-gp/BCRP/MRP2 (담즙 방향) |

**극한 분기**:

| 조건 | 결과 | 임상 함의 |
|---|---|---|
| $PS_{inf} \gg CL_{int,met} + PS_{eff,bile}$ | $CL_{H,b} \to$ 기본 well-stirred 환원 (M4와 같음) | 수동확산 우세 약물 |
| $PS_{inf} \ll CL_{int,met}$ | **Uptake-limited** ($CL_{H,b} \approx f_{ub} \cdot PS_{inf}$) | OATP 기질 (statin, valsartan 등) — uptake transporter DDI가 청소율을 직접 흔듦 |
| $PS_{eff,bile} \gg CL_{int,met}$ | **Biliary efflux-limited** | P-gp/BCRP 기질, MRP2 다형성 영향 |

**임상 anchor [R&T pp.136–138 box]**:
- **Statin 계열 (rosuvastatin, pitavastatin)**: OATP1B1 uptake-limited. SLCO1B1*5 다형성에서 AUC가 2–3배 상승하고, cyclosporine·gemfibrozil OATP 억제 DDI가 효소 억제보다 큰 영향을 줍니다.
- **Pravastatin**: 모약물의 거의 전체가 OATP1B1 uptake에 의존 — 그래서 hepatic CL 예측이 well-stirred 단순 적용에서 5–10배 underprediction되는 IVIVE 함정의 대표 사례.

→ **이게 M9의 $\rho$가 정성적이라면, Sirianni-Pang은 그것의 정량 분해입니다.** PopPK에서 OATP 기질 약물을 다룰 때 covariate 자리는 well-stirred의 $f_{ub}$가 아니라 Sirianni-Pang의 $PS_{inf}$입니다.

#### C. 🆕 Transporter Induction (vs Inhibition) — 대칭적 축

본 챕터의 모든 DDI 사례가 **inhibition**(억제) 일색이면, 임상 라벨링 검토에서 **induction**(유도) DDI를 놓치게 됩니다. 두 기전은 분자생물학적으로 다릅니다 — inhibition은 *효소·수송체 활성*을 즉시 떨어뜨리지만, induction은 *효소·수송체 단백 발현*을 시간 의존적으로 높입니다 (며칠–수주 timescale).

**핵심 inducer와 영향 받는 효소·수송체** [R&T pp.121–123, Figs 5-2, 5-3; R&T Ch.17 DDI section]:

| Inducer | 작용 기전 | 영향 받는 효소·수송체 | 임상 결과 |
|---|---|---|---|
| **Rifampin** | PXR ligand → 광범위 induction | CYP3A4, CYP2B6, CYP2C9, **P-gp, BCRP, OATP1B1/1B3** | 동반 약물(warfarin, midazolam, statin, digoxin, contraceptive) AUC ↓ 30–80% |
| **Carbamazepine** | PXR/CAR → CYP + 수송체 | CYP3A4, **P-gp** | 자기유도(autoinduction) — 시간에 따라 carbamazepine 자체 CL이 증가 |
| **St. John's Wort** | PXR | CYP3A4, P-gp | herbal-drug DDI의 고전 anchor (cyclosporine 거부반응 사례) |
| **Phenytoin, Phenobarbital** | CAR | CYP3A, CYP2C, P-gp | 전형적 enzyme inducer |
...
여기서 $\alpha_{ind}, \beta_{ind}$는 induction fold (rifampin의 경우 CYP3A4에서 약 5–10, P-gp에서 약 2–3).

**Induction이 inhibition과 다른 점**:
- **시간 의존성**: inhibition은 약물 농도에 따라 즉시; induction은 mRNA 합성 → 단백 합성 → 활성 발현의 1–2주 timescale
- **워시아웃**: rifampin 중단 후 효소 활성이 baseline으로 돌아오는 데 약 2주 — DDI 평가 design에서 washout period가 핵심
- **임상 영향 방향**: inhibition은 substrate 노출 ↑ → 독성 위험; induction은 substrate 노출 ↓ → **치료 실패 위험** (피임실패, 거부반응, 항응고 효과 상실)

**PopPK 실무 신호**:
- Rifampin 같은 inducer가 covariate에 포함된 cohort에서는 **inducer 노출 기간**(days since rifampin start)을 covariate로 모델링하는 게 가장 정확. 단순 binary covariate는 induction의 시간 진행을 놓침.
- **Self-induction (carbamazepine)**: $CL(t)$가 시간에 따라 변하는 비정상상태 — 1구획 fitting이 어긋나면 induction time course를 먼저 점검.

> 🔑 **DDI 검토 본능:** "inhibitor만 있는 DDI 목록"은 불완전합니다. Rifampin·carbamazepine·St. John's Wort·smoking을 inducer 축에 항상 같이 놓고, **효소만이 아니라 수송체 induction**(P-gp, OATP)을 같은 자리에서 점검합니다.

**이 카드의 한 줄**: 수송체 기질의 간 소실을 다룰 때, 본능은 **uptake/metabolism/efflux 중 어느 단계가 병목인지**를 따로 점검하는 것이에요. enzyme-only 사고에 빠지면 transporter DDI와 efflux 병목을 한꺼번에 놓치고, empirical scalar로 봉합하면 구조 원인이 숨겨져서 다음 cohort에 외삽할 때 통째로 빗나갑니다. **그리고 inhibition 일색 사고는 induction DDI 위험(치료 실패)을 놓치므로, Sirianni-Pang 좌표 위에 inducer/inhibitor를 양 축으로 두는 것이 본능이어야 합니다.**

<!-- SLIDE_START: M10.5 | title: M10.5. Phase II 대사 — 글루쿠로니드화 축 (NEW) -->
<!-- SECTION_CORE: SC-12B -->

### 🆕 M10.5. Phase II 대사 — 글루쿠로니드화 축(Phase II metabolism: UGT/SULT axis)

**앞에서 → 여기서**. M4가 $CL_{int}$를 CYP 중심으로 펼쳤다면, 이 카드는 **CYP에 가려져 있던 Phase II 대사축**을 추가합니다. M11의 대사체 율속단계로 넘어가기 전에 필수입니다.

**거장의 시각.** "대사 = CYP"라는 무의식적 등식이 박혀 있으면, UGT 기질 약물에서 다형성·DDI·신생아 대사 미성숙·암 화학요법 독성 시그널을 한꺼번에 놓치게 됩니다. CYP가 abundance와 처방약 비중에서 가장 크긴 하지만, **임상에서 가장 격렬한 phenotype 차이를 만드는 자리**는 오히려 UGT 쪽에 자주 있어요(irinotecan/SN-38이 대표 예).

#### A. Phase I vs Phase II — 좌표 정립 [R&T p.121]

| 분류 | 반응 | 특기사항 |
|---|---|---|
| **Phase I** | 산화·환원·가수분해 (oxidation·reduction·hydrolysis) | CYP가 대표 |
| **Phase II** | 접합 (glucuronidation·sulfation·acetylation·glutathione 등) | UGT·SULT·NAT·GST |

Phase I과 Phase II는 **흔히 순차적**이에요 — CYP가 hydroxyl group을 만들고, UGT가 거기에 glucuronic acid를 붙입니다. 그래서 **CYP 기질 약물 대부분이 Phase II 후속 반응을 거치고**, parent의 운명이 두 단계 합성으로 결정됩니다.

#### B. UGT 슈퍼패밀리 [R&T pp.121–122]

**UGT (UDP-glucuronosyltransferase)** — 약물과 endogenous compound에 glucuronic acid를 붙여 극성을 높이고 담즙·소변 배설을 촉진합니다. UGT의 임상 중요성:

| 측면 | 내용 |
|---|---|
| **주요 기질** | UGT1A1: irinotecan, estradiol, etoposide, bilirubin / UGT2B7: morphine, zidovudine |
| **임상 phenotype** | UGT1A1*28 다형성 → Gilbert syndrome → SN-38 (irinotecan 활성 대사체) 축적 → 중성구감소·설사 위험 ↑ |
| **DDI** | Valproate-lamotrigine (UGT2B7 inhibition) — lamotrigine 노출 증가, 발진·SJS 위험 |
| **발달 약동학** | 신생아 UGT 미성숙 → chloramphenicol gray baby syndrome (글루쿠론산 접합 부족으로 chloramphenicol 축적) |

$$
\underbrace{CL_{int,\text{total}}}_{\text{전체 내재 CL}}=\underbrace{\sum_i CL_{int,\text{CYP}_i}}_{\text{Phase I 합}}+\overbrace{\underbrace{\sum_j CL_{int,\text{UGT}_j}}_{\text{Phase II UGT 합}}+\underbrace{CL_{int,\text{SULT}}+CL_{int,\text{others}}}_{\text{기타 접합}}}^{\text{Phase II 총합 — 흔히 누락되는 좌표}}
$$

#### C. 임상 Anchor 사례

**C1. Irinotecan / SN-38 (UGT1A1*28)** [R&T p.122]: irinotecan은 carboxylesterase에 의해 활성 대사체 SN-38로 전환되고, SN-38은 UGT1A1에 의해 SN-38G로 비활성화됩니다. UGT1A1*28 동형접합체(Gilbert syndrome, ~10% Caucasian)에서는 SN-38 비활성화가 느려져 dose-limiting myelosuppression과 diarrhea 위험이 증가해요. → **약물 라벨에 *28/*28 환자 시작 용량 감량 권고**가 들어가 있는 대표 anchor.

**C2. Lamotrigine + Valproate (UGT2B7 inhibition)**: 두 약물 단독에서 lamotrigine $t_{1/2}$는 약 25 h. Valproate 동반투여 시 UGT2B7 억제로 lamotrigine clearance가 절반 가까이 감소하고 $t_{1/2}$가 약 50 h로 연장. → **lamotrigine 시작 용량을 절반 이하로 낮추는 적정(titration)**이 필요합니다 (Stevens-Johnson syndrome 위험).

**C3. Mycophenolate mofetil (MPA) — UGT 기질 + 장간순환**: MMF → MPA → MPAG(UGT) → 담즙 → 장내 β-glucuronidase → MPA로 재가수분해 → 재흡수. → **2차 피크가 잡히는 PK 곡선**과 항생제 동반투여 시 MPAG-MPA 순환 차단으로 MPA 노출 감소 가능 (M2.5 enterohepatic cycling 좌표와 직접 연결).

**C4. Morphine / M6G (UGT2B7)** [M13에서도 등장]: morphine의 활성 대사체 morphine-6-glucuronide는 UGT2B7에 의해 생성됩니다. 신부전에서 M6G가 축적되어 진정·호흡억제 위험 증가 — M14의 active metabolite 신부전 시나리오와 정확히 같은 구조예요.

#### D. NONMEM 실무 신호

- **UGT 기질 약물**의 PopPK에서 CYP-style covariate(연령·체중·간기능)만 잡으면 UGT 발달 미성숙(소아)이나 다형성(\*28)이 IIV로 흡수되어 model misspecification 위험.
- **Pharmacogenomic covariate**(UGT1A1*28, UGT2B7\*2 등)는 dichotomous 또는 ordinal covariate로 model에 들어갑니다.
- **장간순환 + UGT 글루쿠로니드 결합 시**: 2차 피크 fitting을 위해 transit chain 또는 delayed input compartment 필요 (M2.5와 연결).

**이 카드의 한 줄**: 대사 = CYP라는 등식을 풀어내고 **Phase II 축이 평행하게 존재함**을 기본 사고에 넣습니다. 안 그러면 irinotecan dose 결정, lamotrigine titration, neonate PK, 장간순환 모델링이 한꺼번에 무너집니다. 다음 카드(M11)의 율속단계가 UGT-CYP 순차 chain에서 어떻게 결정되는지를 보면, 이 좌표가 왜 미리 필요했는지가 분명해집니다.


<!-- SLIDE_START: M11 | title: M11. 대사체 처리의 율속단계 -->
<!-- SECTION_CORE: SC-13 -->
...
<!-- SLIDE_START: M12 | title: M12. AUC(m)/AUC 분해 -->
<!-- SECTION_CORE: SC-14 -->
...
<!-- SLIDE_START: M13 | title: M13. 초회통과 대사체 기여와 정상상태 대사체 -->
<!-- SECTION_CORE: SC-15 -->
...
<!-- SLIDE_START: M14 | title: M14. 신장애 활성 대사체 시나리오와 상호전환 -->
<!-- SECTION_CORE: SC-16 -->
...
> **⚡ 기억 고리**: Plasma 식에서 blood 식으로 옮길 때 $R = C/C_b$를 곱해야 합니다. $f_u$와 $f_{ub}$는 *분모가 다른 두 측정량*이지 같은 변수의 별명이 아니에요 — 이 한 곱이 빠지면 well-stirred의 분자($f_{ub} \cdot CL_{int}$)가 통째로 잘못된 단위에서 평가됩니다. **🆕 측정 매트릭스 anchor**: tacrolimus·cyclosporine은 **whole blood**(RBC 강한 분배), warfarin·phenytoin은 **plasma**, digoxin은 **serum** 매트릭스 표준 — TDM 결과를 PopPK 입력으로 받을 때 매트릭스 mismatch가 $V$ 자릿수 오류로 이어집니다 (→ M8 §C).
...
**🆕 답 방향 (산출 단계 명시화) [R&T pp.673–675]**:

**Step 1. 정상 상태에서 청소율 분해**:
...
→ Parent CL은 30 → 15 (절반으로 감소), 즉 parent 노출 약 **2배** 증가.

**Step 3. 🆕 $f_m$ 자동 증가 (무뇨 시)** — 이게 13배 폭증의 진짜 출발점:
...
즉, **무뇨 상태에서 $f_m$은 0.3에서 0.6으로 자동으로 두 배 증가**합니다. 왜냐하면 metabolic formation CL의 *절대값*은 유지되지만, 신장 경로가 닫히면서 *전체 parent CL 중 차지하는 비율*이 올라가기 때문이에요.

**Step 4. 대사체 노출 비율 계산** ($AUC(m) \propto F \cdot f_m / CL(m)$):
...
→ 모약물은 **약 2배**, 대사체는 **약 13배** 증가합니다. 모약물 노출만 보고 절반으로 감량(parent-only 50% reduction)하면 대사체는 여전히 약 6–7배 과노출 상태로 남아요.

**Step 5. 통합 용량비** [R&T p.674]: 정상 총 활성 농도(0.51 mg/L)를 유지하려면 용량을 정상의 **약 0.14배**로 낮춰야 합니다. parent-only 50% 감량으로는 도달 불가.

**핵심 교훈**: '소수 대사 경로(minor pathway)'는 '소수 안전성 우려(minor concern)'가 아니에요. **신장이 닫히는 순간 $f_m$이 자동으로 상승**하고, 대사체 신장 의존도가 높으면 $CL(m)$이 무너지면서 두 요인이 곱해져 대사체 노출이 자릿수 단위로 폭증합니다. 두 효과가 곱해져 13배를 만드는 거예요 — $f_m$ 2배 증가 × $CL(m)$ 약 6.7배 감소 = **약 13.3배**.
...
| **🆕 DS-5** | Induction Time-Course Misfit | rifampin·carbamazepine 동반투여 cohort에서 $CL$이 baseline 대비 점진적으로 증가하는 시간 의존 변화 — 1구획 fitting 시 terminal slope가 시간에 따라 점차 가팔라짐 | Self-induction 또는 외인성 induction (PXR/CAR 매개)의 mRNA → 단백 timescale (~1–2주). Binary covariate로는 induction 진행을 잡지 못함. | M10 §C (Transporter induction) |
| **🆕 DS-6** | Matrix Mismatch (Whole blood vs Plasma) | tacrolimus·cyclosporine cohort에서 추정 $V$가 비정상적으로 작거나 hematocrit 변동이 거대한 잔차 변이로 흡수됨 | TDM 결과(whole blood)를 plasma로 잘못 해석. $f_u \to f_{ub}$ 변환 누락과 같은 류의 단위 mismatch — 매트릭스 표기를 본 적이 없을 가능성. | M8 §B, §C (Plasma/serum/whole blood) |
...
| **🆕 $f_u \to f_{ub}$ 변환** | well-stirred 또는 PBPK 입력 전에 $f_{ub} = f_u \cdot (C/C_b)$를 명시 적용; $R$ 변동이 큰 약물에서는 covariate로 처리. |
| **🆕 측정 매트릭스 표기** | `$INPUT`에 matrix flag (plasma/serum/whole blood); tacrolimus·cyclosporine 등 RBC 분배 큰 약물은 whole blood 기본. |
| **🆕 Sirianni-Pang Extended CL** | OATP 기질 약물 PopPK에서 $f_{ub}$ covariate가 비유의/계수 0일 때, $PS_{inf}$ 또는 SLCO1B1 genotype을 covariate 자리에 검토. |
| **🆕 Induction time course** | rifampin·carbamazepine inducer 동반 cohort에서 *days since inducer start*를 time-varying covariate로 모델링 (binary로는 induction 진행 누락). |
...
9. **🆕 측정 매트릭스 (plasma/serum/whole blood)와 $f_u \to f_{ub}$ 변환은 well-stirred 또는 PBPK 입력 직전 필수 점검 단계입니다** [R&T App.D pp.775–776; R&T pp.124–128].
10. **🆕 Extended Clearance (Sirianni-Pang)는 OATP 기질 약물에서 well-stirred IVIVE underprediction을 분해합니다 — uptake/metabolism/biliary efflux 단계별 PS 항이 covariate 자리가 됩니다** [R&T pp.136–138 box; R&T App.E pp.778–779].
11. **🆕 DDI 검토에서 inhibition 일색 사고는 induction DDI (rifampin, carbamazepine, St. John's Wort)의 치료 실패 위험을 놓칩니다. 효소와 수송체 induction을 양 축으로 두는 것이 본능이어야 합니다** [R&T pp.121–123, Ch.17].


### 🆕 §8G. Allometric scaling — NONMEM 체중 covariate 출발점(NEW)

**왜 이 절이 필요한가**: 본 챕터 전체가 *생리학적 분해*를 다뤘다면, §8G는 그 분해된 파라미터를 **환자 간(혹은 종 간)에 어떻게 정규화하는가**를 정합니다. NONMEM에서 체중·연령·BSA covariate를 PopPK에 도입하는 첫 단계가 여기 있어요. 이게 빠지면 소아·비만 환자 dose adjustment, FIH dose 예측, interspecies scaling이 모두 ad-hoc scalar로 풀리게 됩니다.

#### A. Allometric 표준식 [G&W p.87, Table 2.7; G&W §2.10 interspecies scaling]

**핵심 거듭제곱 법칙(power law)** — 체중에 대한 PK 파라미터의 표준 지수:
...
| 파라미터 | 거듭제곱 지수 | 생리학적 근거 |
|---|---:|---|
| $CL$ (총 청소율) | **0.75** | 대사율의 종간 거듭제곱 법칙 (Kleiber's law), 간 효소 용량의 체구 의존성 |
| $Q$ (구획 간 청소율) | **0.75** | 혈류 비례 (간·신장 모두 BW^0.75) |
| $V$ (분포용적) | **1.0** | 체액 부피와 거의 정비례 |
| $K_a$ (흡수속도) | **−0.25** 또는 0 | 시간 척도 — 큰 동물은 흡수가 느려지는 경향 |

→ **그래서 $t_{1/2} = 0.693 \cdot V/CL \propto BW^{1.0 - 0.75} = BW^{0.25}$** — 큰 동물일수록 반감기가 느리게 증가. 종간 비교에서 자주 등장하는 관계식이에요.

#### B. 종간 PK 파라미터 비교 [G&W p.87, Table 2.7]

| Species | Body weight (kg) | Liver weight (g) | Liver blood flow (mL/min) |
|---|---:|---:|---:|
| Human | 70 | 1500 | 1450 |
| Dog | 10 | 320 | 309 |
| Rabbit | 2.5 | 77 | 177 |
...
→ 이 표를 보면 간 혈류와 간 무게가 체중과 거의 1.0 power로 따라가지 않음을 알 수 있어요 (예: rat에서 mouse로 BW가 12배 줄 때, liver flow는 7.7배만 감소). 이게 **interspecies CL 예측에서 단순 mg/kg 변환이 실패하는 이유**의 정량 anchor입니다.

#### C. 소아·신생아 scaling — Maturation Function 보강

순수 allometric만으로는 **소아 효소 미성숙**과 **신생아 신장 발달**을 잡지 못해요. 그래서 PopPK에서는 일반적으로 다음 두 항을 곱합니다:
...
| 변수 | 의미 |
|---|---|
| $PMA$ | Postmenstrual age (수태후 연령, 주 단위) |
| $TM_{50}$ | 성인 활성의 50%에 도달하는 PMA — UGT2B7은 약 36 주, CYP3A4는 약 45 주 |
| $H$ | Hill coefficient (효소 발달 속도의 sigmoidity) |

→ **신생아 UGT 미성숙**(M10.5 chloramphenicol gray baby anchor)이 이 framework에 정확히 끼워 들어가요.

#### D. 비만 환자 scaling — ABW vs IBW vs LBW

비만 환자에서 어떤 체중을 쓸 것인가? 약물 친수성/지용성에 따라 다릅니다.

| 약물 특성 | 적용 체중 | 예시 |
|---|---|---|
| 친수성 약물 (분포 제한) | **IBW** (Ideal Body Weight) 또는 **LBW** (Lean Body Weight) | aminoglycosides, vancomycin (loading은 ABW, maintenance는 IBW) |
| 지용성 약물 (조직 분포 큼) | **ABW** (Adjusted Body Weight) | 일부 마취제 |
| 중간 | $LBW + 0.4 \times (ABW - LBW)$ 등 dosing weight | 다수 임상 가이드라인 |

#### E. NONMEM 구현 — 표준 covariate 식

NONMEM control stream에서 가장 흔히 등장하는 형태:
...
**중요 결정 사항**:
- **Exponent를 고정할지 추정할지**: 데이터가 풍부하면 추정, 소아처럼 데이터가 sparse하면 0.75/1.0 고정이 안정적
- **Centering body weight**: 보통 70 kg (성인 표준) 또는 데이터셋의 median을 씁니다
- **Maturation function의 식별가능성**: $TM_{50}$과 $H$는 같이 추정하기 어려운 경우가 많아서, prior 문헌값을 reference로 fixing하는 게 일반적

#### F. 흔한 함정 — 비전형 power exponent

- **Aminoglycosides**: lean tissue 분포에 가까워서 LBW 기반 scaling이 더 정확합니다 (ABW로 dose 계산 시 신독성 위험 ↑).
- **신생아 신청소율**: GFR 자체가 maturation 함수로 변하므로, $CL_R$의 BW 지수는 단순 0.75가 아니라 (BW^0.75) × maturation 형태로 들어갑니다.
- **TMDD 약물**: target-mediated component는 receptor abundance를 반영하니까 BW^0.75에 따르지 않는 경우가 많아요.

#### G. 본 챕터 다른 카드와의 연결

| 카드 | 본 §8G와의 연결 |
|---|---|
| **M2 / M2.5** | 신기능 covariate (CrCl, eGFR) 위에 체중 scaling을 곱하는 구조 |
| **M4** | $Q_H$ scaling (BW^0.75) — interspecies hepatic CL 예측의 핵심 |
| **M10.5** | UGT maturation function — 신생아 PK에서 결정적 |
| **M14** | 소아 신부전 + 활성 대사체 시나리오는 BW scaling + maturation + impairment 3중 보정 필요 |

**이 절의 한 줄**: PopPK covariate 모델의 첫 단추는 **$CL \propto BW^{0.75}$ + $V \propto BW^{1.0}$ + maturation function**입니다. 이 framework 없이 mg/kg 단위 ad-hoc scaling으로 가면 소아 dose 결정·FIH 예측·interspecies extrapolation이 통째로 흔들리고, NONMEM에서 체중을 단순 linear covariate로 잡으면 BW^0.75 정합성이 무너집니다.

---

<!-- v4.0 보강본_final 변환 노트
- v4.0 → v4.0 final 변경 요약:
  · M8: B절 ($f_u \leftrightarrow f_{ub}$ 명시 변환식) + C절 (Plasma/Serum/Whole blood 측정 매트릭스) 신규 추가
    - 출처: R&T App.D pp.775–776 Eq D-4–D-5, R&T pp.124–128
  · M10: B절 (Sirianni-Pang Extended Clearance 정량식) + C절 (Transporter induction vs inhibition) 신규 추가
    - 출처: R&T App.E pp.778–779 Eq E-9–E-15, R&T pp.136–138 box, R&T pp.121–123 Figs 5-2/5-3, R&T Ch.17
  · §5 Pair 4: tacrolimus/cyclosporine/warfarin/digoxin 매트릭스 anchor 한 줄 추가
  · §8D: DS-5 (Induction Time-Course Misfit) + DS-6 (Matrix Mismatch) 신규 추가
  · §8E: $f_u \to f_{ub}$ 변환, 측정 매트릭스, Sirianni-Pang, Induction time course 4개 항목 추가
  · §8F: 9–11번 한 페이지 요약 항목 신규 추가
- v4.0의 기존 보강 항목 (보존):
  · §1, §1.5, M1, M3, M5–M7, M9, M11–M14, §5 혼동 쌍 1–3, 5–10, §7 Q1–Q8, §8A–§8G의 v4.0 본문 내용
  · M2 (1차 재흡수 가정 + methamphetamine/phenobarbital + 신장 transporter DDI)
  · M2.5 (eGFR 추정식 + 담즙 청소율 + 장간순환)
  · M4 B1–B3 (CYP 분포 + CYP2D6 다형성 정량 + 염증성 CYP downregulation)
  · M6 G&W Table 2.8 MMP 보정 함정
  · M10.5 (Phase II UGT/SULT)
  · Q9 5단계 산출 (f_m 0.3 → 0.6 자동 증가 명시)
  · §8G (Allometric scaling)
- 수식 보강 원칙 준수: 새 수식은 모두 overbrace/underbrace annotation 적용 ✓
- 출처 표기 일관성: G&W (Gabrielsson & Weiner) / R&T (Rowland & Tozer) 페이지·식 번호 정확 기재 ✓
- 원본 보존: §5 혼동쌍 10개, §7 Q1–Q8, M1, M3, M5–M7, M9, M11–M14 본문 완전 보존 ✓
- final 단계에서 새로 추가된 정량 anchor: 
  · $f_{ub} = f_u \cdot R$ 변환식 (R&T App.D Eq D-4–D-5)
  · tacrolimus/cyclosporine R ≈ 0.05–0.2 (whole blood 표준 anchor)
  · Sirianni-Pang Extended Clearance 4-PS 분해 (R&T App.E)
  · OATP1B1 기질 IVIVE underprediction 5–10배 anchor
  · Rifampin induction fold: CYP3A4 5–10배, P-gp 2–3배
  · Carbamazepine autoinduction time course
-->

---

**문서 ID**: C-260517-082655-M9X
