# 비구획 분석(NCA) — AUC, AUMC, MRT, $V_{ss}$와 노출 평가의 실무 경계 — 보강본_final (v4.2)
...
> 🔗 **PopPK로 가는 다리 — NCA missing ≠ PopPK missing:** NCA의 BLQ 처리는 본질적으로 **결측(missing) 또는 보간(interpolation)** 패러다임이에요 — 데이터 한 점을 단순 제외하거나 양 옆 값으로 채웁니다. NONMEM 등 PopPK 도구에서는 이걸 **likelihood-based 방법**으로 격상시킵니다. Beal 분류의 **M3** 방법은 "BLQ 관측이 LOQ보다 작을 likelihood가 모델 적합도에 기여한다"는 **censoring framework**이고(M5/M6/M7은 그 변형들이에요), NCA의 missing 처리보다 더 많은 정보를 끌어냅니다. 그래서 NCA에서 결측 처리한 BLQ도 PopPK 단계에서는 M3로 다시 들어와 추정 정밀도를 회복할 수 있어요 — **두 영역에서 "missing"이라는 같은 단어가 가리키는 처리는 다릅니다.** 보고서 검토 시에는 NCA의 BLQ 처리 규칙(SOP/SAP)과 PopPK의 BLQ likelihood 옵션이 일관되게 정렬됐는지 확인합니다. [G&W p.153; PopPK 표준 framework]
$$
\underbrace{\text{NCA}}_{\text{point exclusion}}:\ C_{BLQ}\to\text{missing}\quad\Longrightarrow\quad
...
📖 **§2.8.7 표준식 vs §2.8.8 대안식 — 적용 영역 비교표** [G&W pp.151, 157]
G&W §2.8.8 원문은 적용 영역을 직접 명시합니다 — *"When the half-life is short in relation to the length of dosing or, in other words, **the duration of infusion is much greater than $t_{1/2}$**, we propose an alternative approach to Equations 2:328 and 2:337 for estimation of MRT and $V_{ss}$"* [G&W p.157]. 즉 표준식과 대안식의 **분기는 $T_{inf}/t_{1/2}$ 비율 하나**에서 갈립니다.

| 적용 영역(Application Domain) | Eq.2:337 (표준 MRT 식, §2.8.7) | Eq.2:366 (washout 대안식, §2.8.8) |
|---|---|---|
| $T_{inf}/t_{1/2}$ 비율 | 작거나 중간 — 입력이 elimination에 비해 빠른 영역 | 큼 — *"duration of infusion is much greater than $t_{1/2}$"*, infusion이 elimination을 우세하게 만드는 영역 |
| 필요 데이터 | $AUMC_0^\infty$, $AUC_0^\infty$, MIT($T_{inf}/2$) | 정상상태 농도 $C_{ss}$, washout $AUC_{t^*\to\infty}$, 주입속도 $K_0$ |
| 핵심 수식 | $V_{ss}=\overbrace{D_{iv}\cdot AUMC/(AUC)^2}^{\text{모멘트비}}$ | $V_{ss}=\overbrace{K_0\cdot AUC_{t^*\to\infty}/C_{ss}^2}^{\text{washout 기반}}$ |
| 수치 거동 | 정상 양수 — 적용 영역 안에서 안정 | 표준식이 0 또는 음수가 되는 영역에서 **양수 보존** [G&W p.157] |
| 안정성 신호 | AUMC 꼬리·MIT 보정 신뢰 가능 | $C_{ss}$ 안정, washout AUC 잘 정의됨 |
| Sanity check | 두 식 결과가 잘 맞으면 → 표준식 영역 OK | 두 식 결과가 크게 벌어지면 → $T_{inf}/t_{1/2}$ 영역 문제 신호, 대안식 채택 |
...
이 비교표 한 장이 SOP deviation 문서의 1면이에요 — 표준식과 대안식 중 어느 쪽을 선택할지의 **수학적 정당화가 $T_{inf}/t_{1/2}$ 비율 한 변수로 환원**된다는 점이 그대로 보입니다.
...
## Q13. 협역 치료역 다구획 약물의 NCA 보고서에서 $V_{ss}=12$ L, $V_z=20$ L, $V_c=4$ L가 나왔습니다. 표적 농도 $C_{target}=2$ mg/L에 도달하기 위한 부하용량(loading dose)을 산정할 때 어느 부피를 써야 하나요? 잘못 선택하면 어떤 결과가 나오나요?

**정답:** **협역 치료역 다구획 약물의 부하용량 산정 기준은 $V_c$ (중앙구획 부피, $V_1$)입니다 — $V_{ss}$가 아니에요.** 이유는 IV bolus 직후 약물은 일시적으로 **중앙구획에만 분포**하니까요. 분포 평형이 일어나기 전 짧은 시간 동안 $C_0 = LD/V_c$가 됩니다.

$$
\underbrace{LD_{\text{임상 표준}}}_{V_c\ \text{기준}}=\overbrace{V_c\cdot C_{target}}^{\text{중앙구획 dilution}}=4\times 2=\underbrace{8\ \text{mg}}_{\text{표적 도달}}
$$

만약 $V_{ss}$로 잘못 계산하면 $LD=24$ mg이고 초기 농도는 $C_0=LD/V_c=24/4=6$ mg/L로 **표적의 3배**까지 치솟습니다. $V_z$로 계산하면 $LD=40$ mg, $C_0=10$ mg/L로 **표적의 5배**예요. 협역 치료역 약물에서는 이 초기 peak가 곧 일시적 peak 독성을 의미합니다.
...
R&T Chapter 19가 신부전 환자의 gentamicin 용량 조정에서 **terminal phase를 무시하고 $V_1$ 기반으로 LD를 계산**하는 임상 관행을 명시하는 이유가 정확히 이거예요 — 협역 치료역 약물에서 $V_{ss}$나 $V_z$ 기반 LD는 **NDA submission의 결정적 하자**가 됩니다. 정리하면 **부하용량은 $V_c$, 유지용량은 $CL$, $V_{ss}$는 분포 평형 후 농도 해석 기준** — 세 부피의 역할 분리가 임상 안전성의 기초입니다. [G&W pp.66, 151; R&T pp.626, 632–636]

**§7 요약(Recap)**: Q1–Q3은 AUC 신뢰도, Q4는 비선형 전환, Q5–Q6은 NCA 계산, Q7–Q8은 노출 해석, Q9는 C3 Apex의 적용 경계예요. Q10은 BLQ 처리 분기, Q11은 정상상태 PTF 차이, Q12는 BE 통계 판정 — 이 네 항목은 NCA가 PopPK/임상약리 실무로 넘어가는 접점이에요. **Q13은 §5 쌍3의 Critical Blow($V_z$ vs $V_{ss}$ 혼동)를 능동 회상으로 한 번 더 회수하는 자리**입니다 — 부하용량 산정에서 $V_c$ 기준이 임상 표준이라는 점이 안전성 정당화의 마지막 잠금장치예요.
...
| **PopPK BLQ likelihood 처리** | NCA의 BLQ 위치별 규칙(SOP/SAP) | **Beal M3/M5/M6/M7** censoring framework에서 NCA의 missing 규칙과 PopPK의 censored likelihood가 정렬되어야 추정 정밀도 회복 가능 — 정렬 깨지면 BLQ 가까운 농도 구간의 $\omega^2$/$\sigma^2$ 분리 실패 |
...
📖 **NCA BLQ ↔ PopPK Beal M3 정렬 보충:** C1 §C-2에서 본 NCA의 BLQ 규칙(pre-first/intervening/terminal 위치별 분기)은 **PopPK의 censored likelihood 처리와 한 쌍**으로 가야 해요. Beal (2001) **M3** 방법은 "BLQ 관측 $C_{obs}<LOQ$가 모델 적합도에 **censored likelihood** $P(C_{obs}<LOQ\mid\theta)$로 기여한다"는 framework이고, **M5는 LOQ/2 substitution, M6는 첫 BLQ만 LOQ/2 후 나머지 결측, M7은 BLQ를 0 처리** — 모두 M3의 단순화 변종이에요. 핵심은 **NCA에서 결측 처리한 BLQ가 PopPK 단계에서 M3로 들어와야 추정 정밀도가 회복**된다는 점입니다. 두 영역의 BLQ 규칙이 정렬되지 않으면 BLQ 가까운 농도 구간의 IIV($\omega^2$)와 RUV($\sigma^2$) 분리가 실패하고, **terminal phase의 $\omega^2$가 RUV로 흡수**되거나 그 반대 현상이 일어나요. 즉 NCA 보고서의 BLQ 처리 기록은 PopPK 모델 specification 자체의 기초 자료입니다. [G&W p.153 — "treat as missing values rather than setting them to LOQ, zero or something else"]
...
<!--
# 출력 후 자가검증 체크리스트 (보강본_final / v4.2)

| 항목 | 기준 | 결과 |
|---|---|---|
| SLIDE_START 개수 | §섹션수 + 카드수 합산 | 9 개 (v4.1 동일) |
...
| BLQ 처리 카드 (C1 §C-2) | 위치별 분기, G&W p.153 인용 | ✓ (v4.1) |
| §2.8.8 방향 명확화 (C3 §C) | T_inf↑ 방향 명시 | ✓ (v4.1) |
| V_c(V_1) 기준 loading dose (§5 쌍3) | R&T pp.626, 632–636 출처 | ✓ (v4.1) |
| 정상상태 노출 지표 (C4 §B-2) | AUC_τss, PTF%, R_ac 정의 | ✓ (v4.1) |
| Partial AUC (C4 §B-3) | MR vs IR 비교 | ✓ (v4.1) |
| BE 통계 + Model validation (§8 D) | 90% CI 0.80-1.25, VPC/CWRES/NPDE/MAP | ✓ (v4.1) |
| Q10–Q12 (자기 테스트) | BLQ, PTF, BE | ✓ (v4.1) |
| **NCA missing → PopPK Beal M3 가교 (C1 §C-2)** | M3 censoring framework 명시 | ✓ 신규 (v4.2) |
| **§2.8.7 vs §2.8.8 적용 영역 비교표 (C3 §C)** | T_inf/t_{1/2} 비율 한 변수로 환원 | ✓ 신규 (v4.2) |
| **Q13 (자기 테스트) — LD with V_c** | $V_c=4$/$V_{ss}=12$/$V_z=20$ 수치 anchor + 3배/5배 peak | ✓ 신규 (v4.2) |
| **§8 D PopPK BLQ likelihood 행 추가** | Beal M3/M5/M6/M7 정렬 | ✓ 신규 (v4.2) |
...
<!-- v4.1 → 보강본_final (v4.2) 변환 노트 (Q3 v1 vs v2 비교 + 추가 검토 의견 반영)
v1·v2 비교 검토에서 명시된 "v2의 base에 v1의 Q13을 이식"과, 두 번째 정독에서 발견된 정밀화·누락 항목을 알맞은 위치에 추가. v4.1의 모든 내용은 변경 없이 보존.

[V. v1 결정적 강점 이식 — Q3 검토 의견 V.5번]
1) §7 Q13 — 협역 치료역 다구획 약물의 부하용량 산정 (V_c 기준)
   - v1만의 결정적 강점으로 평가된 자기테스트 항목
   - §5 쌍 3 Critical Blow를 능동 회상으로 회수하는 자리
   - 수치 anchor: V_c=4 L, V_ss=12 L, V_z=20 L, C_target=2 mg/L
   - C_0 = LD/V_c 위계 (1배 / 3배 / 5배 peak) overbrace 식으로 표기
   - 출처: G&W pp.66, 151; R&T pp.626, 632–636

[Q1. 추가 사실관계 정밀화 — §2.8.7 vs §2.8.8 적용 영역 표]
2) C3 §C — §2.8.7 표준식(Eq.2:337) vs §2.8.8 대안식(Eq.2:366) 적용 영역 비교표
   - 검토 의견: "G&W §2.8.7의 *full Eq.2:337* (input correction)과 *§2.8.8의 short half-life 특수 케이스* 둘의 적용 영역 그림을 한 번에 보여주는 표가 있으면 더 강력해집니다"
   - 6행 비교표(비율, 데이터, 수식, 거동, 안정성, sanity check) + T_inf/t_{1/2} 분기 cases 식
   - G&W p.157 원문 직접 인용: "the duration of infusion is much greater than t_{1/2}"
   - 출처: G&W pp.151, 157

[Q2. NCA → PopPK 가교 — Beal M3 method]
3) C1 §C-2 — NCA missing ≠ PopPK missing 가교 단락
   - 검토 의견 Q2-4번: "NCA의 BLQ는 M1 또는 0/LOQ/2 substitution이 주류지만, PopPK에서는 Beal M3 (likelihood-based maximum) 또는 M5/M6/M7 방법이 표준"
   - NCA point exclusion vs PopPK censored likelihood 대비식 (overbrace)
4) §8 D — PopPK BLQ likelihood 행 추가
   - "PopPK BLQ likelihood 처리" 새 행: NCA의 BLQ 위치별 규칙과 Beal M3/M5/M6/M7 정렬 필요성
   - 보충 단락: M3/M5/M6/M7 정의, 정렬 깨질 때 ω²/σ² 분리 실패 신호
   - 출처: G&W p.153 (NCA missing 원칙); Beal 2001 / Bergstrand & Karlsson 2009 framework

[수식 표기 규칙 — v4.1과 동일]
- 모든 추가 수식 overbrace/underbrace로 단위·의미 동시 표시
- T_inf/t_{1/2} 분기 case 식, NCA→PopPK 대비식, C_0 위계 식 모두 적용

[보존 감사 — v4.1과 동일]
- v4.1의 모든 §, 카드, 표, 수식, 출처 anchor 그대로 유지
- 보강 영역 외 문자열 변경 없음
- SLIDE_START 마커 9개 그대로 유지 (Q13은 §7 자기 테스트 카드 내부에 추가, 구조 변경 없음)
- Turbojoint PK41 수치, R&T Table A-1/A-2/H-1 수치 보존
- v4.1 추가 영역(§C-2 BLQ 분기표, §B-2 정상상태 지표, §B-3 Partial AUC, §5 V_c LD 식, §8 D BE/Model validation) 모두 변경 없음
-->
