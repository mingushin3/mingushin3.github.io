...
**Target filename**: `04_html_compile_input_master_v3.1.md`  
**Assembly date**: 2026-05-07 (v3.1 Korean Readability Patch revision)  
...
- `R < 1`이면 plasma concentration이 blood concentration보다 낮다. 비율 사용 방식에 따라 plasma CL이 실제보다 다르게 보일 수 있다.
- Hepatic extraction에는 plasma `CL_H/Q_H`가 아닌 `E_H = CL_{H,b}/Q_H`를 사용해야 한다.
...
Passive diffusion / permeability가 충분히 커서 세포 내외의 unbound concentration이 빠르게 평형에 가까워지면 `ρ→1`이 되어 basic well-stirred model로 환원된다. 반대로 uptake permeability가 작으면 `ρ<1`이고 clearance는 permeability-limited가 된다 [TE pp.778–779].
...
대사체가 임상적으로 중요한 이유는 다섯 가지다: **action, toxicity, inhibition, induction, displacement**. 그러나 metabolite가 그런 성질을 가져도 충분한 concentration에 도달하지 않으면 therapeutic concern은 작다 [T20 p.659]. 즉, “active metabolite가 있다”와 “임상적으로 중요하다”는 같은 말이 아니다.
...
Methylprednisolone hemisuccinate → methylprednisolone, tolbutamide → hydroxytolbutamide, propranolol → naphthoxylactic acid는 대사체 곡선이 rate-limiting step과 상대적 clearance를 어떻게 구분하는지 보여주는 예시다. 개별 figure 수치는 이 문서에서 확장하지 않는다.
...
Oral parent 투여 후 관찰되는 대사체는 두 입력의 합이다:
...
Interconversion(← 상호전환)은 parent와 대사체가 서로를 재생성할 수 있음을 뜻한다. 따라서 metabolite 쪽 renal impairment가 parent의 apparent elimination까지 늦출 수 있다. Clofibric acid is retained as a qualitative example; figure-derived numeric anchors are not locked `[T20 p.679; 확인 필요]`.
...
> Q9에는 두 가지 *방어 가능한* 입장이 존재한다 — 어느 쪽을 택하든 그 결정은 라벨/CSR에서 어떻게 *defendable*한지를 함께 답해야 한다.  
>  
> **(a) Combined-exposure dose adjustment.** parent + active metabolite의 통합 활성을 정상 수준으로 맞추는 dose ratio (Tozer worked example의 약 0.14)를 적용한다. *방어 논리*: metabolite가 parent와 equipotent에 가깝거나 potency-weighted sum이 toxicity driver라면 combined Css를 anchor 삼는 것이 가장 보수적이다. *Trade-off 비용*: parent-driven therapeutic effect를 underdose할 위험. 라벨 문구 요구: "based on combined active exposure".  
>  
> **(b) Two-tier monitoring.** parent dose는 parent CL 변화에만 맞추되, active metabolite TDM 또는 PK/PD biomarker로 안전 안내선을 운영한다. *방어 논리*: parent와 metabolite의 potency 차이가 분명하고 metabolite assay와 turnaround time이 가능하면 일률 감량보다 정밀하다. *Trade-off 비용*: assay validation, sampling design, action threshold가 모두 라벨에 명시되어야 한다.  
>  
> **두 입장 모두에서 방어 불가능한 것**: "parent `f_e` 변화에 비례한 일률 감량" 단독. T20 p.675는 위 (1)/(2)/(3) scenario rule로 이를 수식적으로 차단한다. Sponsor가 active metabolite의 ESRD PK를 측정하지 않은 상태에서 parent-only 비례 감량을 제출하면, scenario (3) 가능성을 닫지 못한 채 권고가 발행되는 것이며, 이것이 본 dilemma의 *진짜* 위험이다.
...
**END — 04_html_compile_input_master_v3.1.md**
