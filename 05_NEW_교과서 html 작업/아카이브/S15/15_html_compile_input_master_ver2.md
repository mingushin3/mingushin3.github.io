# 15_HTML Compile Input Master — ver2

**Target file**: `15_html_compile_input_master_ver2.md`  
**Assembly date**: 2026-05-06 (ver2 enrichment date)  
**Assembly mode**: PATCH MODE  
**Phase**: 4E — Learner-Complete + Mastery-Enriched + Embodiment-Augmented + HTML-Ready Master Assembly
...
| **혼동쌍 표 행 `**치명적 타격(Critical Blow)** *— [교과서 외 실무 해석]*`** *(ver2)* | Highlighted row inside CONFUSION table | distinct row background (e.g., subtle red/amber tint) within the existing 혼동쌍 #1 table; not a separate table |
| **블록쿼트 `> **Mastery Note / Failure Mode / Practice Lens / Judgment Lens — [CRUCIBLE_DERIVED|TEXTBOOK_DERIVED]**`** | Phase 4D mastery augmentation callout | preserve as-is from Phase 4D; existing styling (no change in ver2) |
