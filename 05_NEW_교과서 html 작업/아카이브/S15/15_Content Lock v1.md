...
**Mode**: A-Critical  
**Output status**: Content Lock v1 — source-cleaned, compressed, Phase 4C figure work excluded.
...
본 세션은 p.352 Fig.4.16의 **modeling carousel** 중 Step 4 Explore data, Step 5 Fit model(s), Step 6 Analyze output를 다루고, 그 결과를 다음 실험의 Design으로 되돌리는 구간이다. 즉, 초기값 획득에서 시작해 GOF 판독, 경쟁 모델 선택, outlier 판단, 편미분 기반 sampling design, sensitivity analysis까지 이어지는 한 사이클의 실행 척추다. [p.352]

<!-- MASTER LENS -->
### Big Idea

이 범위의 핵심은 “좋은 통계량을 계산하는 법”이 아니라 **통계량을 적용해도 되는 조건을 아는 법**이다. F-test, AIC/SC, correlation coefficient, WRSS, partial derivatives는 각각 강력하지만, nested 조건, weighting 조건, residual randomness, parameter identifiability, sampling sensitivity를 벗어나면 오히려 잘못된 모델 확신을 만든다. [pp.369–391, pp.399–405]
...
<!-- ANCHOR -->
위 흐름은 닫힌 고리다. 초기값이 나쁘면 회귀가 잘못된 minimum으로 간다; 잔차가 무작위가 아니면 모델 또는 weighting이 틀린다; 모델 비교 조건을 어기면 통계 검정이 무의미하다; 파라미터가 상관되면 다음 실험의 sampling design을 바꿔야 한다. [p.353, pp.369–389, pp.399–405]
...
<!-- RECAP -->
**§1 Recap**: 이 세션의 목적은 모델링 결과를 “좋아 보인다”가 아니라 “조건을 만족하므로 비교·해석 가능하다”로 바꾸는 것이다.
...
<!-- MASTER LENS -->
**Big Idea**: 초기값은 회귀 알고리즘의 출발점이 아니라, 모델러가 EDA에서 추출해 알고리즘에 넘겨주는 **domain-informed prior**다. Gabrielsson은 초기값이 나쁘면 wrong final values, unwanted local minima, physiologically impossible parameter values로 수렴할 수 있다고 경고한다. [p.353]

#### A. Formal Definition

Initial estimate acquisition은 nonlinear regression을 시작하기 전에 그래프, linear regression, NCA, prior compound knowledge로 starting parameter vector를 구성하는 절차다. 이는 modeling carousel의 Explore data 단계에 속한다. [pp.352–353]

#### B. Source-locked derivation anchors

**B-1. IV bolus one-compartment example**: 두 피험자에게 동일한 10 mg IV bolus를 투여한 뒤 semi-log concentration–time plot에서 slope를 읽는다. [pp.353–354]
...
이 식은 PDF에 표시된 형태를 보존한다. 산술 부호는 source-internal inconsistency가 있으므로, 수학적으로 재작성하려면 $[\ln(56.77)-\ln(124)]/(24-0)$ 또는 $[\ln(124)-\ln(56.77)]/(0-24)$가 된다. 이는 Draft source mismatch가 아니라 원문 내부 부호 문제다. Baseline 120 sec와 $k_{out}\approx0.03\ h^{-1}$로 $k_{in}=3.6\ \text{sec}\cdot h^{-1}$를 얻는다. [p.357]
...
원문은 일반적으로 $0.1\cdot IE$부터 $10\cdot IE$까지의 relative range를 권하지만, Table 4.4처럼 source-specific boundary가 예외를 만들 수 있다. [p.364]
...
초기값은 “정확한 값”일 필요는 없지만, physiologically plausible하고 objective function surface에서 잘못된 basin으로 들어가지 않을 정도로 근접해야 한다. 또한 compound knowledge base를 일찍 만들고 regression objective를 명확히 해야 한다; 그렇지 않으면 modeling은 끝없는 journey가 된다. [p.353]
...
<!-- MASTER LENS -->
**Big Idea**: Observed vs predicted curve만 보면 systematic error가 숨어 보인다. Residual plot은 모델이 놓친 구조, 시간 스케일, 또는 error model을 노출하는 최소 진단 도구다. [pp.368–370]

#### A. Formal Definition

Residual은 observed value와 calculated/predicted value 사이의 vertical distance다. [p.369]
...
Residual은 random scatter가 되어야 한다. Consecutive positive/negative residuals는 run이며, cluster나 systematic run은 model inadequacy를 의심하게 한다. [pp.369–372]
...
<!-- ANCHOR -->
잔차 패턴은 단순한 그림 모양이 아니라 “모델이 설명하지 못한 시간 스케일 또는 분산 구조”의 노출이다. Banana pattern은 빠른 phase가 빠졌다는 신호일 수 있고, fan pattern은 구조보다 error variance model의 문제일 수 있다. [pp.372–376]
...
Model misspecification은 observed curve overlay보다 residual plot에서 먼저 보인다. Overlay는 curve와 data의 절대 위치를 보여주지만, residual plot은 오차의 부호와 순서를 보존하므로 systematic deviation을 증폭한다. [pp.369–376]
...
<!-- MASTER LENS -->
**Big Idea**: F-test는 “더 복잡한 모델이 WRSS를 줄였는가”를 묻는 도구가 아니다. **Full model과 reduced model이 hierarchical/nested이고, 같은 weighting scheme으로 fit되었을 때만** 추가 parameter의 의미를 검정하는 도구다. [pp.387–388]

#### A. Formal Definition

Nested model이란 full model에서 특정 parameter를 고정하면 reduced model로 collapse되는 관계다. Sigmoid Emax model은 exponent $n=1$로 고정하면 ordinary Emax model이 되므로 nested 관계다. [p.388]
...
<!-- ANCHOR -->
Nested 조건은 parameter 개수 차이가 아니라 **한 모델이 다른 모델의 제한된 형태인가**의 문제다. `[교과서 외 해석]` 기하학적으로 말하면 reduced model은 full model parameter space 안의 제한된 부분공간이어야 하며, 그래야 WRSS 감소량이 “추가 parameter가 설명한 개선”으로 해석된다.
...
AIC/SC의 lowest value가 자동으로 adequate model을 뜻하지 않는다. GOF는 residuals, parameter precision, accuracy, correlation matrix, condition number, F-test, AIC/SC를 함께 보는 battery of tools다. [pp.387–391]
...
<!-- MASTER LENS -->
**Big Idea**: 좋은 fit은 좋은 parameter를 보장하지 않는다. Parameter precision과 correlation은 모델 자체보다 **sampling design이 parameter를 서로 분리해 보여주었는가**에 좌우된다. [pp.380–385]

#### A. Accuracy, precision, and CV%

Accuracy는 bias와 관련되고, precision은 estimate의 uncertainty와 관련된다. 원문은 dartboard를 통해 accurate/inaccurate와 precise/imprecise를 구분한다. [pp.379–380]
...
<!-- ANCHOR -->
Fig.4.55–4.57의 핵심은 **ridge minimum**이다. 두 parameter가 ridge 위에서 같이 움직이면 OFV/WRSS가 거의 변하지 않아 univariate estimate는 그럴듯해도 joint uncertainty는 크다. Design A/B 비교는 같은 모델도 sampling design에 따라 Cl/V 또는 IC50/Imax correlation이 크게 달라짐을 보여준다. [pp.383–385]

#### D. Fixing one parameter

Fig.4.55는 한 parameter를 고정하면 다른 parameter의 confidence interval이 작아질 수 있음을 보여준다. [pp.383–384] 그러나 `[교과서 외 실무 해석]` fix 전략은 정당화 부담을 통계에서 biology/literature/prior study로 옮긴다. Fix value의 출처와 적용 가능성을 설명하지 못하면 precision이 좋아진 것이 아니라 uncertainty를 숨긴 것이다.
...
<!-- MASTER LENS -->
**Big Idea**: Partial derivative plot은 “어디서 sample하면 어떤 parameter가 가장 잘 보이는가”를 말해준다. 예측값이 parameter 변화에 민감한 시간·농도 영역이 그 parameter를 estimate하는 정보의 중심이다. [pp.399–403]
...
True parameter values를 사전에 알 수 없으므로, derivative maxima에만 sample을 몰아넣지 말고 전체 concentration range에도 추가 samples를 둔다. 원문은 pooled data를 무비판적으로 design decision에 쓰는 위험도 경고한다. [p.402]
...
<!-- MASTER LENS -->
**Big Idea**: Sensitivity analysis는 fitting 후 해석이 아니라, 다음 실험을 설계하기 전 “어떤 parameter uncertainty가 prediction을 얼마나 흔드는가”를 보는 pre-fitting stress test다. [pp.403–405]

#### A. Formal Definition

Sensitivity analysis는 특정 parameter를 일정 비율로 변화시켜 concentration–time 또는 response–time prediction이 어떻게 달라지는지 보는 절차다. 원문의 예시는 parameter를 +100% 또는 −50%로 변화시켜 model output 변화를 비교한다. [pp.404–405]
...
Capacity-dependent kinetics와 time-dependent kinetics가 동시에 존재하면 Day 1 AUC와 steady-state AUC 비교가 상쇄되어 interpretation을 흐릴 수 있다. Fig.5.20의 핵심은 exposure가 변하지 않는 것처럼 보여도 capacity와 induction이 동시에 작동할 수 있다는 점이다. [p.414]
...
<!-- TRENCH -->
Weighting 선택은 관행이 아니라 residual pattern으로 정당화한다. Fan shape이면 relative error weighting을 의심하고, 균질 잔차이면 constant absolute error가 더 자연스럽다. [pp.373–374]
...
`[교과서 외 해석]` PK data처럼 y-range가 넓으면 r은 dynamic range의 영향을 크게 받을 수 있다. 그래서 r은 “전체 설명력”의 보조 숫자일 뿐, GOF 판정의 주연이 아니다.
...
<!-- TRENCH -->
두 종류의 오류를 바꿔 처방하면 모델이 더 복잡해지기만 한다. Fan shape에 compartment를 추가하거나 banana pattern에 weighting만 바꾸는 것이 대표적 실패다.
...
원문은 table만 보면 A와 B의 deviating points를 거의 알아보기 어렵고, graphical presentation이 매우 중요하다고 강조한다. [p.390]
...
**정답**: 먼저 같은 weighting으로 재적합해 비교 가능성을 확보한다. 그래도 F-test/AIC가 단독 결론을 주지 못하면 residual pattern, mechanism plausibility, external data, visual prediction을 함께 triangulate한다. PDF는 AIC difference threshold를 정의하지 않으므로, 낮은 AIC 하나만으로 mechanistic plausibility를 이길 수 없다. [p.389, p.391]
...
<!-- MASTER LENS -->
이 세션은 PK/PD 모델링에서 “모델을 세우는 기술”보다 “모델을 버리거나 다시 설계하는 기술”에 가깝다. Initial estimates는 fit의 출발점을 정하고, residuals는 모델 결함을 드러내며, F-test/AIC/SC는 비교 조건을 제한하고, precision/correlation은 설계의 약점을 노출하며, partial derivatives와 sensitivity analysis는 다음 실험의 sampling을 바꾼다. [p.353, pp.369–391, pp.399–405]
...
경험 많은 모델러는 “어느 모델의 AIC가 낮은가”만 보지 않는다. 두 모델이 비슷한 WRSS/AIC를 보이는데 residual pattern과 mechanism이 모두 애매하면, 어느 한쪽을 고르는 대신 **둘 다 wrong mechanism일 가능성**을 먼저 의심한다. 이 역방향 추론이 model selection을 parameter-count game이 아니라 mechanistic hypothesis revision으로 바꾼다.
