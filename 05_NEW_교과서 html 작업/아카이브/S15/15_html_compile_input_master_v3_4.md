# 15_html_compile_input_master_v3.3.md

**v3.3 note:** v3.2 PASS 인증 위에 적용한 외과적(surgical) PATCH MODE 보완. 단일 목적은 §2 MUST 1–6 카드 6개에 카드별 **실무 체화 요약(Practice Brief) 4축 블록**을 균일하게 추가하여 단권화 학습자가 모든 카드에서 동일한 능동 회상 closure를 받도록 하는 것이다. 새 페이지 태그·새 수식·새 약물·새 수치 anchor·새 외부 reference는 **0건** 도입. v3.2의 모든 학습자-facing 본문, 수식, 페이지 태그, source label, figure marker, HTML compiler marker는 byte-level 보존됨. §8D `GOF Checklist locked from Table 4.9` 표 무변경. MUST 3 `[⚡ Apex Concept]` 배지 + F-test validity 본문 무변경. §5 혼동쌍 4개 표 잠금 일체 무변경. §7 Q9 Boss Dilemma 정답 방향 무변경.

**v3.2 note (preserved):** This version applies a Korean-Dominant Readability Patch to PART A learner-facing prose. General English prose was converted to Korean where possible, while career-critical pharmacometrics terms are retained as Korean-English paired expressions on first use. Scientific content, equations, numerical anchors, page tags, source labels, figure markers, compiler markers, section structure, and PART B guardrails are preserved. No new scientific claims, examples, numbers, page tags, figures, or external references were added.
...
# 15_HTML Compile Input Master — v3.3

**Target file**: `15_html_compile_input_master_v3.3.md`  
**Assembly date**: 2026-05-08 (v3.3 Practice-Brief Uniformity Patch date; v3.2 / v3.1 patch base preserved)  
**Assembly mode**: PATCH MODE  
**Phase**: 4E + v3 Surgical Patch + v3.1 Korean Readability Patch + v3.2 Korean-Dominant Readability Patch + v3.3 Practice-Brief Uniformity Patch
...
**v3.3 변경 요약** (Practice-Brief Uniformity Patch — v3.2 → v3.3):
- PART A §2 MUST 1–6 카드 6개에 카드별 **실무 체화 요약(Practice Brief) 4축 블록**을 균일하게 추가하여 단권화 학습자가 모든 카드에서 동일한 능동 회상 closure를 받도록 함.
- 4축 구성: **(1) 왜 알아야 하는가**, **(2) 흐름상 역할**, **(3) 체화 꿀팁**, **(4) 실무 활용**. 모든 카드의 Practice Brief는 동일한 4축 형식·동일한 라벨 `[EXPERT_INFERENCE, v3.3]`을 가진다.
- 적용 위치: 각 MUST 카드의 마지막 Mastery Augmentation 블록(Failure Mode / Mastery Note / Judgment Lens / Practice Lens) 직후, 다음 카드의 `### ▣ MUST [N+1]` 헤더 직전(MUST 6은 §5 시작 직전).
- 라벨링: 모든 Practice Brief는 `[EXPERT_INFERENCE, v3.3]`로 통일. 카드 본문에 이미 존재하는 내용만 사용하여 작성됨 — 새 페이지 태그·새 약물·새 수치 anchor·새 외부 reference 일체 도입 안 함.
- v3.2 불변 영역 모두 유지 + v3.3 추가 불변 영역: §2 MUST 1–6 카드의 모든 기존 본문(Big Idea, A–E subsections, Recap, 체화 꿀팁, 모든 Mastery Augmentation 블록, 모든 figure marker), §5 혼동쌍 4개 표 일체, §7 Q1–Q9 정답 방향 일체, §8 Meta-Frame 본문 일체, 특히 §8D **GOF Checklist locked from Table 4.9** 표·MUST 3 [⚡ Apex Concept] 배지·F-test validity conditions 본문 byte-level 보존.
- 새 과학 주장·새 수치·새 임상 예시·새 페이지 태그·새 reference·새 figure marker·새 source label 추가 없음.
- PART B 내용 무변경(이 Version Note, B1의 v3.3 contract clause, Phase 4E Certification 표의 v3.3 row, B8 v3.3 splice integrity note, B9 C15 row, B10 Micro-Patch Log의 v3.3 entry, B11 Mastery Augmentation Log의 #17–#22 entries, B12-C Change Log 신설, 파일 말미 v3.3 Final Checklist 추가만 예외).
- HTML readiness 영향 없음. Phase 5 PATCH MODE Splice Verification Table 유효성 유지(7개 anchor 모두 verbatim 보존됨).
...
| Practice-Brief Uniformity Certificate (v3.3, 신설) | PASS | §2 MUST 1–6 카드 6개 모두에 균일한 4축 실무 체화 요약(Practice Brief) blockquote 추가 완료(왜 알아야 하는가 / 흐름상 역할 / 체화 꿀팁 / 실무 활용). 모든 Practice Brief는 라벨 `[EXPERT_INFERENCE, v3.3]`을 가지며, 카드 본문에 이미 존재하는 내용만으로 작성되었음. 새 페이지 태그·수식·수치 anchor·약물 예시·외부 reference 일체 도입 없음. v3.2의 source-fidelity byte-level 보존. §8D GOF Checklist locked from Table 4.9 표 무변경. MUST 3 [⚡ Apex Concept] 배지·F-test validity 본문 무변경. §5 혼동쌍 4개 표 잠금 일체 무변경. §7 Q9 Boss Dilemma 정답 방향 무변경. |
...
> - **왜 알아야 하는가**: 비선형 회귀(nonlinear regression)는 초기값 품질이 수렴(convergence)과 국소 최솟값(local minimum) 회피를 결정하므로, 그래프·잔차 기반 초기값 도출 없이 fitting을 시작하면 그 이후의 모든 진단(MUST 2)·모델 판별(MUST 3)·정밀도 평가(MUST 4)가 출발점부터 흔들린다.
> - **흐름상 역할**: 본 세션의 입구이자 후속 5개 카드(MUST 2–6) 모두에 대한 사전 조건; EDA·항법도 사고를 §4 모델 구축 단계로 번역하는 다리.
> - **체화 꿀팁**: 본문의 graphical method (semi-log slope·intercept·peeling)을 손으로 한 번 적용해 보고, "fitting을 열기 전에 종이 위에 초기값 후보 1개를 먼저 적는다" 루틴을 30분 안에 정착시킨다.
> - **실무 활용**: 새 PK 데이터셋을 받았을 때 NONMEM control stream을 열기 전에 EDA 도표 위에서 V·CL·Ka 초기값을 graphical로 도출하고, 이 값을 `$THETA` initial로 명시 입력하여 수렴 안정성을 확보한다.
...
> - **왜 알아야 하는가**: 잔차 패턴(residual pattern)에 trend·systematic deviation·heteroscedasticity가 남아 있으면 그것은 random noise가 아니라 **모델 결함(model misspecification)의 신호**이며, 이를 ε(잔차 분산)로 흡수해 통과시키면 가짜 IIV 또는 가짜 covariate signal이 후속 모델에서 생성된다.
> - **흐름상 역할**: MUST 1의 fitting 결과를 검증하는 첫 관문; MUST 3의 F-test 적용 전 필수 단계; §8D GOF Checklist의 입력 단계.
> - **체화 꿀팁**: 본문 잔차 plot 패턴 분류(random scatter / systematic banana·run / fan-shaped heteroscedasticity / leverage point)를 빈 종이에 네 개 모양으로 그려 보고, 각 모양이 시사하는 모델 결함 원인을 한 줄씩 옆에 적기.
> - **실무 활용**: NONMEM 적합 후 CWRES/IWRES vs time, vs IPRED plot에서 trend가 보이면 weighting 변경 전에 **구조 모델 결함 가능성**을 먼저 보고서에 명시하고, fan shape에서만 weighting 재검토로 넘어간다.
...
> - **왜 알아야 하는가**: F-test는 **nested + equal weighting** 두 조건에서만 valid한데, 이 두 조건을 무시하고 non-nested 모델 비교나 different weighting 비교에 F-test를 적용하면 통계적 결론 자체가 무의미해지며, 모델 판별의 정량적 근거가 무너진다(Table 4.8의 더블 위반이 정확히 이 사례).
> - **흐름상 역할**: 본 세션의 Apex Concept; MUST 2의 잔차 진단을 통과한 모델 후보들 사이의 정량 비교 단계; §5 혼동쌍 #1 (nested+equal vs non-nested 또는 different weighting)의 직접 모태.
> - **체화 꿀팁**: "Nested + Equal Weighting" 두 조건을 mantra로 박고, 본문 F-test critical value·자유도(degrees of freedom)와 함께 "non-nested에는 AIC, weighting 다르면 F-test도 AIC도 불가" 분기 규칙을 카드 끝에서 자문(self-quiz).
> - **실무 활용**: 모델 비교 보고서에서 F-test를 사용하기 전에 두 모델이 nested 관계인지·잔차 weighting이 동일한지를 본문에 명시 검증한 후에만 ΔOFV 또는 F-statistic을 인용한다.
...
> - **왜 알아야 하는가**: 정밀도(precision, CV%/SE)와 파라미터 간 상관(correlation)을 동시에 보지 않으면 점추정값(point estimate)의 신뢰성이 식별 가능성(identifiability) 위에 있는지 알 수 없으며, ridge minimum 위에 놓인 파라미터 쌍을 놓치면 NONMEM 출력의 숫자가 데이터가 식별할 수 없는 quantity를 추정한 가짜 결과일 수 있다.
> - **흐름상 역할**: $COV step 출력 해석의 spine; MUST 3 F-test 통과 모델의 후속 검증 단계; MUST 5 sampling design으로 곧장 연결되는 진단(상관 높음 → 다음 cohort design 변경).
> - **체화 꿀팁**: NONMEM 출력 표를 받으면 파라미터 정밀도(SE/CV%) 칸과 correlation matrix를 *동시에* 한 화면에 띄워, "fit이 좋다 → parameter가 좋다" 자동 추론을 끊고 두 신호를 30초 안에 교차 스캔하는 루틴을 정착시킨다(소스가 보편적 cutoff을 정의하지 않는다는 점에 유의).
> - **실무 활용**: PopPK 보고서에서 모든 파라미터에 점추정값 + SE/CV% + 주요 correlation을 함께 표기하고, 상관이 높은 파라미터 쌍에는 모델 단순화가 아니라 **다음 cohort sampling design 변경**을 권고로 명시한다.
...
> - **왜 알아야 하는가**: 채혈 시점(sampling time) 선택이 어떤 파라미터를 식별 가능하게 만드는지 정량화하지 못하면 임상시험 PK design이 직관에 의존하게 되고, 결과적으로 MUST 4의 정밀도·상관 진단이 design의 한계로 인해 무너지는 sampling identifiability 함정으로 직결된다.
> - **흐름상 역할**: MUST 4의 정밀도 평가를 *사전적*으로 설계하는 자리; 임상시험 PK 채혈 protocol 설계의 정량 근거; sparse-data fitting이 가능한 design 입구.
> - **체화 꿀팁**: 본문 편미분 ∂C/∂θ가 큰 시점이 그 파라미터에 *민감한* 시점이라는 직관을 손으로 1구획 모델에 적용해 ka, V, CL 각각의 sensitivity peak 시점을 그려 보고, "information이 모인 곳에 anchor + 전체 range에 보험"의 두 층 골격으로 design을 잡는다.
> - **실무 활용**: DDI 연구 또는 신약 Phase 1 PK study의 채혈 시점을 결정할 때 편미분 sensitivity 분석을 protocol의 정당화 근거로 첨부하여, "왜 이 시점인가"의 답을 통계적·정량적 언어로 제시한다.
...
> - **왜 알아야 하는가**: 모델 파라미터 중 어느 파라미터가 출력(예측 농도·AUC·response profile)에 민감하고 어느 파라미터가 둔감한지를 모르면 모델의 robustness를 평가할 수 없고, 규제 제출 시 "이 파라미터의 불확실성이 결론을 바꾸는가" 질문에 답할 수 없다.
> - **흐름상 역할**: 본 세션의 종합 출구; MUST 4의 정밀도 평가와 MUST 5의 sampling design을 통합한 risk assessment 자리; modeling carousel의 사이클을 다음 실험 design으로 되돌리는 마지막 매듭.
> - **체화 꿀팁**: 본문 민감도 분석 절차(파라미터를 ±50% 또는 +100%·−50%로 변동 → output 변화 측정)를 빈 종이에 한 파라미터에 대해 손으로 적용해 보고 "민감 vs 둔감"을 한 줄로 분류한 뒤, **"흔들어도 살아남는 결론만 보고하라"** 원칙을 카드의 핵심 행동 규칙으로 박는다(Toxicokinetic 영역에서는 capacity-dependent와 time-dependent 과정이 동시에 작동하면 흔들기에서 두 효과의 역방향 상쇄가 노출됨).
> - **실무 활용**: NDA 또는 IND 제출 dossier에서 핵심 파라미터에 대한 sensitivity analysis 결과를 첨부하여, dose·safety margin·sampling schedule 결론을 흔들 수 있는 핵심 파라미터를 모델 risk 항목으로 명시한다.
