# 07_html_compile_input_master_v3.1.md
...
**How to use this handout**: 먼저 Curation Map에서 M1–M10의 결정자 구조를 훑고, §2의 concept cards를 순서대로 읽은 뒤, §5 confusion pairs와 §7 self-test로 오개념을 제거한다. Figure pointer는 교과서 원그림을 복제하지 않는다. 표시된 source figure를 원문 PDF에서 직접 열어 확인하라는 안내 신호로 이해하면 된다.

**Learning route**: M1–M2에서 `level`과 `time`을 분리하고, M3–M4에서 `accumulation`, `fluctuation`, `average concentration`, `average amount`를 분리한다. M5–M6에서 input/response 해석의 예외를 잠근 뒤, M7–M8에서 loading, maintenance, therapeutic-window design으로 조립한다. M9–M10은 PK plateau로 환원되지 않는 **PD 시계**와 fractionation에 의존하는 **toxicity 누적**을 따로 본다.

**Before you start**: `CL`, `t1/2`, `τ/t1/2`, `Cav,ss`, `Aav,ss`, therapeutic window(← benefit–harm을 모두 고려한 농도 구간)가 서로 다른 질문에 답한다는 점을 기억한다.  
...
| ID | 개념 | 핵심 잠금 문장 | 핵심 수식 / anchor | §2 카드 위치 | Source tag |
|---|---|---|---|---|---|
...
| M6 | Therapeutic Window | TW는 "좋은 농도 범위"가 아니라 benefit과 harm의 balance를 농도 축 위에 투영한 의사결정 구간이다. | therapeutic window(← benefit–harm balance의 농도 구간)와 therapeutic concentration range 구별; TI는 전임상 ratio와 임상 exposure sensitivity가 다름 | §2-M6 (full card) | [R&T pp.267–281] |
| M7 | Loading + maintenance | plateau를 기다리지 않고 목표 근처에서 시작하려면 loading dose가 필요하고, 유지에는 average input rate가 필요하다. | $D_M=D_L(1-e^{-K\tau})$; $D_L=R_{ac}D_M$; bolus+infusion anchor | §2-M7 (full card) | [R&T pp.293–295; R&T pp.326–329; G pp.537–539] |
...
| M9 | PD-driven plateau | effect plateau는 drug PK plateau보다 늦을 수 있다. 무엇의 half-life인지 먼저 물어야 한다. | atorvastatin: PK $t_{1/2}=14$ h, PD plateau 3–4 weeks; erythropoietin: PK $t_{1/2}=9$ h, hematocrit plateau ≈70 days | **§2-M9 (brief card, ver2 신설)** + §2-M6 PK/PD clocks 절 | [R&T pp.341–345] |
| M10 | Dose fractionation toxicity | 같은 daily dose라도 fractionation이 recovery time을 바꾸면 toxicity가 달라진다. | Daptomycin Study A: 75 mg/kg q24h vs 25 mg/kg q8h, CPK 991 vs 3996 U/L | **§2-M10 (brief card, ver2 신설)** + §2-M7 daptomycin anchor | [R&T pp.351–353] |
...
- **M9**: 효과 plateau는 turnover/cell lifespan이라는 **별개의 시계**가 결정할 수 있다 — drug PK plateau가 답이 아닐 수 있다.
- **M10**: 같은 daily dose라도 fractionation이 recovery time을 어그러뜨리면 toxicity가 누적된다 — exposure variable 하나로 위해를 설명하지 않는다.
...
<!-- TRENCH -->
**Trench-Level Tip**: 이 알고리즘은 linear PK, fixed interval, 그리고 target concentration과 임상 response 사이의 충분한 직접 연결을 전제한다. Single-dose therapy나 PD plateau가 turnover로 지배되는 약물에서는 그대로 적용하지 않는다 [R&T p.279; R&T pp.341–345].
...
plasma drug concentration이 plateau에 도달했다고 해서 임상 효과가 plateau에 도달한 것은 아니다. 효과의 plateau는 **drug의 half-life**가 아니라 **biological turnover 또는 cell lifespan이라는 별개의 시계**가 결정한다 [R&T pp.341–345].
...
<!-- ANCHOR -->
**Bridge to M2 / M8**: M2의 “3–4 half-lives ≈ 90%” 규칙을 PD plateau에 그대로 옮기면 onset을 과대평가한다. M8 TW-driven algorithm은 즉각적 concentration-response 가정이 성립할 때만 유효하다. PD plateau가 turnover에 의해 지배되는 약물에서는 algorithm 적용 전에 PD 시간척도 검토를 먼저 수행해야 한다.

<!-- RECAP -->
**Recap**: PK plateau ≠ PD plateau. 효과의 시계는 drug 분자가 아니라 endogenous turnover system이 정할 수 있다.

---

<!-- TRENCH -->
**Trench-Level Tip — PD Plateau Timing Error — [EXPERT_INFERENCE, v3]**: PD plateau 도달 시점 예측에서 가장 흔한 함정은 drug $t_{1/2}$ 숫자를 turnover system에 그대로 대입하는 것이다. atorvastatin처럼 PK $t_{1/2}\approx14$ h인 약물의 maximal cholesterol response를 "3–4 half-lives = 2–3일"로 예측하면, 실제 plateau인 3–4주까지 regimen 결정 시점을 심각하게 앞당기게 된다 [R&T pp.341–345]. 효과 평가 시점을 설계하기 전에 반드시 "무엇의 half-life인가 — drug인가, turnover/cell lifespan인가"를 먼저 확인한다.

> **Mastery Note — [TEXTBOOK_DERIVED]**  
> indirect response model의 핵심 직관은 “약물은 turnover의 input 또는 output rate를 바꿀 뿐, response variable 자체는 turnover system의 시간상수로 움직인다”는 것이다. atorvastatin과 EPO는 단순히 “느린 약물”이 아니라 “turnover에 의해 효과가 매개되는 약물”이다.
...
“같은 daily dose이면 같은 toxicity”라는 직관은 종종 틀린다. fractionation(분할 투여) 패턴이 **target tissue의 recovery time**과 어긋나면, average exposure가 같아도 toxicity가 폭발할 수 있다 [R&T pp.351–353].

### Source-locked anchor
...
toxicity를 단일 변수($C_{max}$ 또는 AUC) 하나로만 설명하지 않는다. fractionation이 짧은 recovery interval을 만들면, target tissue가 회복하지 못한 상태에서 다음 노출이 시작되어 누적 손상이 일어날 수 있다.

<!-- ANCHOR -->
**Bridge to M3 / M7**: M3의 $R_{ac}$ 식은 plasma compartment의 누적만 다룬다. M10은 “drug 자체의 누적”이 아닌 “tissue 손상의 누적”이라는 다른 차원의 누적을 말한다. M7의 loading + maintenance 분리도 dose size만 보지 말고 fractionation pattern과 recovery time을 함께 보아야 함을 시사한다.

<!-- RECAP -->
**Recap**: 같은 daily dose ≠ 같은 toxicity. recovery time이 fractionation interval보다 길면 누적 손상이 발생할 수 있다.

---

<!-- TRENCH -->
**Trench-Level Tip — Comparator Definition Error — [EXPERT_INFERENCE, v3]**: fractionation toxicity 분석에서 비교군을 잘못 정의하면 결론 전체가 뒤집힌다. Daptomycin Study A에서 "같은 daily dose" 비교군이 75 mg/kg q24h vs 25 mg/kg q8h임을 직접 확인해야 한다 [R&T pp.351–353]. 이를 25 mg/kg q24h vs 25 mg/kg q8h로 오독하면 daily dose가 3배 차이 나므로 fractionation의 순수 효과를 분리할 수 없다. 문헌에서 "same dose" 또는 "same daily dose" 표현이 등장할 때마다 분자와 분모를 직접 계산하여 실제 total daily dose를 확인하는 습관이 필수다.
...
**⚡ 기억 고리 (Memory Hook)** — *slope 시간 vs centroid 시간*:  
