# 07_HTML Compile Input Master — **v3**
...
<!-- ANCHOR -->
**Bridge to M2 / M8**: M2의 “3–4 half-lives ≈ 90%” 규칙을 PD plateau에 그대로 옮기면 onset을 과대평가한다. M8 TW-driven algorithm은 즉각적 concentration-response 가정 위에 서 있으므로, PD plateau가 turnover에 의해 지배되는 약물에서는 algorithm 적용을 일시 중단하고 PD 시간척도 검토로 돌아간다.
...
4. **Regimen B를 1차 선택으로 정당화하려면**:
   ...
   - IIV 추정치(또는 사전 사전분포)가 $C_{upper}$ 침범 risk를 받아들일 수 있는 수준임을 simulation으로 보여주어야 함.
