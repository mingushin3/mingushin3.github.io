...
# 06_HTML Compile Input Master — v3.1

## Version & Patch Log

**Revision**: v3.1 (Korean Readability Patch applied to v3 master)
...
## Phase 4D Certification (v3, preserved in v3.1)

| Certificate | Status | Basis (v3, v3.1 carry-forward) |
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A는 Learner Navigation Aid와 §1–§8 학습 본문만 포함하며, compiler/audit/process log는 PART B로 분리했다. v3에서 §2 카드별 4축 Practice Brief가 균일하게 통합되어 standalone 학습용 진단 체크리스트의 일관성이 강화되었다. v3.1에서는 본문 한국어 가독성을 추가로 개선하여 standalone 학습 효율을 더 높였다. |
| Zero-Omission Certificate | PASS | Scope Lock의 C1–C4, PK41, R&T A/H anchors, Audit MUST/SHOULD corrections, 4C KEEP markers, ver2 패치(Holford 메시지·NCA-IIV inflation cascade·Step 1 §5 Memory Hooks)를 모두 반영했다. v3에서 Step 1 Protocol §2-C2(Apex + Plausible Fallacy)와 §5(Critical Blow 행), §2 카드별 4축 Practice Brief 의무 항목을 추가 보강했다. v3.1은 어떤 항목도 삭제·축소하지 않았다. |
| Mastery-Uplift Certificate | PASS | bounded augmentation을 관련 concept 옆에 source-status label과 함께 삽입했다. v3에서 [EXPERT_INFERENCE, v3] 라벨로 Apex Concept Plausible Fallacy, Critical Blow, 4축 Practice Brief를 추가하되, 새 수치·새 page tag·새 named example을 도입하지 않았다. v3.1은 새 mastery augmentation을 도입하지 않았다. |
| Source-Boundary Certificate | PASS | 모든 augmentation은 TEXTBOOK_DERIVED/AUDIT_DERIVED/CRUCIBLE_DERIVED/EXPERT_INFERENCE로 표기했고, 새 수치·새 page tag·새 named example을 추가하지 않았다. v3 patches는 모두 기존 page tags 범위 안에서 작동하며, 새로 도입된 [EXPERT_INFERENCE, v3] 블록은 기존 학습 본문의 source-anchored 결론을 재공식화한 것이다. v3.1 Korean Readability Patch는 산문 표현만 조정하며 어떤 source label·page tag·source-anchored 결론도 변경하지 않았다. |
| HTML-Readiness Certificate | PASS | PART B에 marker mapping, figure/page-tag/navigation/MathJax/self-test/rendering/guardrail 규칙을 포함했다. v3 patches는 새 marker를 추가하지 않으며 기존 splice anchor를 변경하지 않는다. 신규 callout(`Apex Concept`, `Plausible Fallacy`, `Practice Brief`)은 기존 Practice Lens / Mastery Note marker 컨벤션과 동일한 렌더 규칙을 따른다. v3.1은 marker, figure pointer, page tag, MathJax 수식, splice anchor를 한 건도 변경하지 않으므로 HTML-readiness가 그대로 유지된다. |

## Assembly Mode

PATCH MODE — `06_Content Lock v2.1(3).md` is a Figure Marker Patch Plan / Insertion Map, not a full re-emitted learner body. PART A was constructed from `06_Content Lock v2(6).md` §1–§8 by exact insertion of approved 4C figure markers, followed by bounded Mastery Augmentation Layer additions. v3.1 inherits the v3 PATCH MODE assembly without re-splicing.
...
**How to use this handout**: PART A만 읽어도 학습용 handout으로 사용할 수 있다. 먼저 §1의 직렬 workflow를 잡고, §2의 C1→C4를 순서대로 읽은 뒤, §5 혼동쌍과 §7 self-test로 점검한다.
...
### D-2. Plausible Fallacy — *"NCA는 모델 가정이 없으니 어떤 데이터에도 올바른 답을 준다"* [EXPERT_INFERENCE, v3]
...
> **Practice Brief — 4축 [EXPERT_INFERENCE, v3]**  
...
**v3.1 Cross-cut verification (this revision)**: v3.1은 v3의 8항목 All-Pass 체크리스트를 100% 보존한다. v3.1이 추가하는 18개 패치(P-KR-01 ~ P-KR-18)는 모두 학습자-facing PART A의 한국어 산문 가독성 개선에 한정되며 — 어떤 수식도, 어떤 page tag도, 어떤 source label도, 어떤 figure marker도, 어떤 splice anchor도, 어떤 [EXPERT_INFERENCE / CRUCIBLE_DERIVED / TEXTBOOK_DERIVED / AUDIT_DERIVED] 라벨도, 어떤 Memory Hook 내용도, 어떤 Critical Blow 내용도, 어떤 PART B guardrail도 변경하지 않는다. 따라서 v3의 통과 근거(위치 인용)는 v3.1에서도 그대로 유효하며, Phase 5 HTML 컴파일러는 본 v3.1 파일의 PART A 전체를 그대로 렌더하면 된다.

---

## v3.1 Final Checklist

이 체크리스트는 v3.1 Korean Readability Patch가 v3의 비협상적 보존 항목을 한 건도 침해하지 않으면서 18개 학습자-facing 한국어 산문 개선만 정확히 적용했음을 검증한다.

| # | 검증 항목 | 통과 근거 | Status |
|---:|---|---|:---:|
| 1 | **PART A readability improved** — 18개 학습자-facing 한국어 산문 패치(P-KR-01 ~ P-KR-18)가 §1 Practice Lens, Learner Navigation Aid, C1/C2/C3/C4 Big Idea·Practice Brief·Annotation, §8 A·C에 surgical하게 적용되어 한국어 독해성과 active-learning clarity가 개선되었다. | Version & Patch Log "Patches applied (v3.1, this revision)" 표 18행; B10 Micro-Patch Log v3.1 paragraph | ✅ PASS |
| 2 | **Scientific content unchanged** — v3.1 패치는 어떤 과학적 주장, 수치, 임계값(20–25% extrapolated 권고선·3–4 half-lives·PK41 1.6→0.9 L·h⁻¹·kg⁻¹ 등), regulatory claim, exposure metric 정의도 변경하지 않았다. | C1 D Context notes·E Boundary conditions·C3 §2.8.8 수치 anchors·C4 PK41 anchor·§7 Q4 numerical estimates 모두 v3와 동일; B12 v3.1 Cross-cut verification | ✅ PASS |
| 3 | **Equations preserved** — 모든 LaTeX 수식 블록(linear/log-linear trapezoidal AUC, AUMC tail, MRT, Vss=MRT·CL, Vz=CL/λz, t½eff, Eq.2:366/2:367 등)이 v3와 byte-level 동일하다. | C1 B Equations·C2 B Equations·C3 A Core equations·C3 C §2.8.8 alternative equations·C4 C Michaelis-Menten ODE 모두 변경 없음 | ✅ PASS |
| 4 | **Page tags preserved** — `[G p.XX]`, `[G pp.XX–YY]`, `[T p.XX]`, `[T pp.XX–YY]`, `[G pp.XX, YY]` 형식의 모든 source page tag가 v3와 동일한 위치·동일한 페이지 번호·동일한 형식으로 유지된다. 새 page tag 생성 없음. | PART A 전반의 모든 인용; B3 Page Tag Rendering Rules; B9 C6 Page tag integrity row | ✅ PASS |
| 5 | **Figure markers preserved** — 6개의 KEEP figure marker(`<!-- FIGURE_POINTER -->` × 5, `<!-- FIGURE_SCHEMATIC -->` × 1)와 그 내부의 Source/Why/When/Learner instruction/Mode/Title/Visual objective/Core message/Elements/Suggested rendering/Caption/Alt text/Source relation 필드 전체가 v3와 동일하다. | §1 FIG-P01·C1 FIG-R02·C2 FIG-P03·C3 FIG-P04·C4 FIG-P05·C4 FIG-P06; B2 KEEP plan; B9 C5 figure row | ✅ PASS |
| 6 | **Source-boundary preserved** — `[EXPERT_INFERENCE]`, `[EXPERT_INFERENCE, v3]`, `[CRUCIBLE_DERIVED]`, `[TEXTBOOK_DERIVED]`, `[AUDIT_DERIVED]`, `[실무 추론]`, `[실무 확장]`, `[실무 해석]`, `[해석]` 등 모든 source-status label이 v3와 동일한 위치에 유지된다. 새 source label 추가 없음. Memory Hook 내용·Critical Blow 내용·Plausible Fallacy 내용·4축 Practice Brief의 [EXPERT_INFERENCE, v3] 라벨 모두 무변경. | §1·§2·§5·§7·§8 모든 callout 라벨; B6 Crucible Guardrails; B9 C9-v3·C10-v3.1 patch boundary rows | ✅ PASS |
| 7 | **HTML-readiness preserved** — 모든 HTML compiler marker(`<!-- MASTER LENS -->`, `<!-- ANNOTATION -->`, `<!-- ANCHOR -->`, `<!-- TRENCH -->`, `<!-- CONFUSION -->`, `<!-- SELF-TEST -->`, `<!-- RECAP -->`, `<!-- FIGURE_POINTER -->`, `<!-- FIGURE_SCHEMATIC -->`)와 PART B의 marker → component mapping, page tag rendering rule, navigation anchor rule, splice verification anchors가 v3와 동일하다. B8 Splice Verification Table의 5개 anchor(§1 Recap·C1 Recap·C2 Recap·C3 Recap·C4 Recap)는 v3.1 패치 대상에서 제외되어 verbatim 일치한다. | PART A 전반의 모든 marker; B4 Marker → component mapping; B8 PATCH MODE Splice Verification Table; B10 Micro-Patch Log v3.1 paragraph | ✅ PASS |
| 8 | **Ready for Phase 5 HTML compilation** — Compilation Contract(B1), Figure Rendering Instructions(B2), Page Tag Rendering Rules(B3), HTML Compiler Requirements(B4), Audit Guardrails(B5), Crucible Guardrails(B6), Deprecated and Forbidden Restorations(B7), PATCH MODE Splice Verification Table(B8), Zero-Omission Coverage Matrix(B9), Mastery Augmentation Log(B11), v3 All-Pass Checklist(B12)가 모두 v3와 동일한 통과 근거를 유지한다. Phase 5 HTML 컴파일러는 본 v3.1 파일의 PART A 전체를 그대로 렌더하면 되며, marker mapping·anchor matching·source page tag pattern detection이 v3와 동일하게 작동한다. | B1–B12 모든 섹션; B12 v3.1 Cross-cut verification; Phase 4D Certification 5개 모두 PASS 유지 | ✅ PASS |

**v3.1 최종 결론**: 본 v3.1 파일은 (i) v3의 모든 PRESENT 인증을 byte-level 보존하고, (ii) v3.1 Korean Readability Patch 18건을 학습자-facing PART A 한국어 산문에만 surgical하게 적용했으며, (iii) 비협상적 보존 항목(Memory Hook 내용·Critical Blow·Holford message·NCA-IIV cascade·page tags·equations·figure markers·source labels·splice anchors)을 한 곳도 침해하지 않았다. **Phase 5 HTML 컴파일러는 본 v3.1 파일의 PART A 전체를 그대로 렌더하면 된다.**
