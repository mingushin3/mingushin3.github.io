...
=================
작성 근거 메모 (거장의 시각)
이 Scope Lock에서 가장 중요한 판단은 Mode 분기입니다. §2.3 신장 청소율은 임상약사 배경에서 직관적 접근이 가능하므로 B-Standard로 충분하지만, §2.5의 Well-stirred vs. Parallel tube 모델 비교와 IVIVE 섹션은 A-Critical이 필수입니다. 이 부분은 오개념 발생 빈도가 높고, 실제 PopPK 공변량 모델에서 간기능을 어떻게 다룰 것인가와 직결되기 때문입니다. AIMS BioScience 컨설팅 데이터에서도 이 판단이 실무로 연결될 가능성이 큽니다.
항목판단 기준Mode A-Critical (Ch.5·20)fm, CL(m), 추출비 오류가 PopPK 모델 구조 자체를 왜곡하므로 최고 검증 수준 필요Mode B-Standard (App.D·E)수식 유도의 논리 흐름이 목표 — 개념 오류보다 전개 순서 학습이 우선Image rights: None교재 Figure는 전면 저작권 보호. SVG 독자 제작으로 대체 명시Key Equations 별도 명기HARD RULE 위반을 방지하기 위해 보존 대상 수식을 명시적으로 리스트화
Phase 1 진행 시 주의사항
1. 파일명의 "대사체(代謝體)" 처리
