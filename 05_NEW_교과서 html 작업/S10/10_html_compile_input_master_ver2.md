# 10_HTML Compile Input Master — ver2

## Version Note (ver2)

This is **ver2** of the HTML Compile Input Master, produced after a focused review against `10_Audit_Report_v1.md`, `10_Crucible_Report_v1.md`, `10_Content_Lock_v2.md`, and `10_Content_Lock_v2_1.md`. The ver1 master already passed all five Phase 4D certificates at a conditional level. ver2 closes six remaining Audit `T5 SHOULD_FIX` items and Crucible `Grade A/B` insights that ver1 had absorbed only partially. All six additions are insertions only — no body sentence from Content Lock v2 is rewritten, no page tag is altered, no new examples or numbers are introduced beyond what Audit/Crucible/Content Lock v2 had already classified as `ADOPT` or `PARTIAL_ADOPT`.
...
> **Visual fast-diagnostic — 0.5초 식별:** Plot의 x-axis 단위가 한 자리수 이상 차이 나면(예: aspirin plasma plot은 0–2 hours, antiplatelet effect plot은 0–192 hours) PD-rate-limited 즉시 확정. 같은 시간축에서 두 곡선이 비슷한 모양이면 PK-rate-limited. **이 진단은 fit statistic이나 모델 비교 이전에 EDA 단계에서 plot의 시간 축만 보아도 가능하다 — 두 plot이 *다른 시간 척도*에서 그려졌다는 사실 자체가 율속 process가 다르다는 직접 증거이기 때문이다.** [R&T p.251–253; PD-rate-limited 시각 진단의 source anchor]
...
| C4 Audit T5 Coverage — SHOULD_FIX | (a) Ideal PD measure + control + 2–4 dose levels; (b) R&T objectives in §1; (c) §3.6.6 framework bridge; (d) acenocoumarol/phenprocoumon as PK/PD rate-limiting anchor | (a) §1 + ver2 I2 in C2 §B; (b) ver2 I1 in §1; (c) ver2 I4 in C5 §A; (d) ver2 I5 in C5 §C | PRESENT (ver2) | All four SHOULD_FIX items now closed |
...
| C6 Page Tag Integrity | Preserve G&W/R&T page tags and uncertainty flags | PART A inherited v2 source tags; B3 enforces rendering without fabrication; ver2 insertions reuse existing page tags only | PRESENT | No new page tags added in ver2 |
| C7 Crucible Grade A Preservation | Dose-invariant peak time, Hill n as titration lever, ke0 as lumped delay, PD3 design range, PK/PD rate-limiting ratio, Intuition #2 (precision improvement), Intuition #3 (time-axis fast-diagnostic) | PART A C2/C4/C5 plus mastery notes; ver2 I3 (Intuition #2) and I6 (Intuition #3); no unsupported numeric expert claims added | PRESENT (ver2 expanded) | Two Grade A intuitions newly explicit |
...
| C9 Canonical Body Integrity | Content Lock v2 learner body retained with approved 4C markers plus labeled augmentations | PART A §1–§8; B8 splice verification incl. ver2 insertions | PRESENT | No broad rewrite; ver2 added six labeled blockquote insertions only |
...
### B11. Mastery Augmentation Log — ver2
...
| Add new page tags in ver2 | Rejected: Phase 4D / Phase 4 is closed for page-tag generation; ver2 reuses only existing tags |
| Add Crucible Grade B Tip #5 (`k_t` prior 우회로 with platelet 10일/prothrombin 1–2일) | Deferred: requires careful boundary between R&T turnover ranges and external prior values; out of scope for ver2 minimal-delta release |
