...
# 01_html_compile_input_master.md — v3.1

> **Version note (v3.1):** PART A에 31개의 Low-risk Korean Readability Patch를 적용했다. 변경 범위는 (a) §1 Learner Navigation Aid 섹션의 영어 안내문·표 헤더·체크포인트 행을 한국어로 통일(P-001 ~ P-014), (b) C1–C5 카드의 Big Idea / Plausible Fallacy / Practice Brief / Boundary 일부 문장을 한국어 독해 흐름에 맞춰 분리·재조립(P-015 ~ P-028, P-030), (c) §5 Pair 1 "Critical correction" 레이블을 "핵심 정정"으로 변경(P-031), (d) C5 Practice Brief 흐름상 역할의 오탈자(`brige` → `bridge`) 수정(P-029). **과학적 본문, 수치 anchor(Compound A S1–S4 numerics, Creatinine/Inulin, midazolam/gentamicin), 모든 page tag(`[G p.XX]`, `[R&T p.XX]`), `<!-- FIGURE_POINTER -->`/`<!-- FIGURE_SCHEMATIC -->`/`<!-- MASTER LENS -->`/`<!-- TRENCH -->`/`<!-- CONFUSION -->`/`<!-- SELF-TEST -->`/`<!-- RECAP -->`/`<!-- ANCHOR -->`/`<!-- ANNOTATION -->` 마커, MathJax 수식, NONMEM 관련 표기, source label은 일체 변경하지 않았다.** Phase 5 HTML compiler는 v3.1 PART A를 그대로 렌더링 대상으로 삼을 수 있다. 자세한 변경 내역은 PART B §B15 v3.1 Change Log 참조. v3 change log는 §B14에 그대로 유지.
...
- Distribution phase 중 관찰되는 빠른 decline이 반드시 elimination $K$를 의미하지는 않는다. midazolam은 초기 약 1시간 동안 distribution phase를 보인다. 이후 terminal phase에서 elimination half-life가 해석된다. [R&T p.61–62]
- Extravascular flip-flop에서는 terminal slope이 absorption rate를 반영할 수 있으므로 이 세션에서는 배경 맥락으로만 다룬다. [G p.29–30]
...
<!-- ANCHOR -->
결국 *distribution-phase AUC 대 전체 AUC의 비율* 하나가 모델 단순화의 가능성을 결정한다. midazolam처럼 50% 부근이면 1구획 근사가 일부 임상 목적에서 안전하고, gentamicin처럼 95%가 분포 위상 동안 빠져나가면 terminal slope를 elimination 대표값으로 쓰는 것 자체가 위험하다. [R&T p.64–66]
...
- AUMC는 $t\cdot C(t)$의 적분이므로 terminal 영역에서의 extrapolation 오차가 AUC보다 크게 증폭된다. 이것이 NCA 보고서에서 $MRT$ 추정의 안정성이 $t_{1/2}$보다 낮은 이유다. 그래서 terminal phase 데이터의 품질이 결정적으로 중요하다. [G p.19]
...
- PART B contains process logs, coverage matrix, compiler rules, guardrails, ver2 patch log, v3 change log, and v3.1 change log only.
- No HTML was generated.
- No Mermaid/SVG code was generated.
- ver2 patches (P1–P5) applied; v3 patches (V1–V6) applied; v3.1 Korean readability patches (KR-001–KR-031) applied; canonical scientific body unchanged.
- Critical MathJax rendering bug (BEL char + 'pprox' typo) FIXED in ver2 and preserved in v3 / v3.1.
- v3.1 surgical patches preserve all v3 numerics, page tags, figure markers, self-test answers, and equations.
- Final output filename: `01_html_compile_input_master_v3.1.md`.
