# 01_Content Lock v1 — One-Compartment IV Kinetics: CL · V · t½ · K
...
**Content status:** Phase 4A locked text. Figure inventory is carried forward to Phase 4C unchanged; no figure markers are inserted here.

---

## Updated Curation Map

이 세션의 핵심은 하나다. **$CL$과 $V$는 독립적 primary parameter이고, $K$와 $t_{1/2}$는 그 두 값에서 파생되는 secondary parameter다.** 이 위계를 잘못 잡으면 semi-log plot 해석, 유지용량, 항정상태 시간, NONMEM parameterization이 모두 흔들린다. [G p.17; R&T p.56–58]
...
<!-- MASTER LENS -->
**Big Idea:** $CL$과 $V$는 약물 disposition을 결정하는 두 primary coordinate이고, $K$와 $t_{1/2}$는 그 좌표가 만든 시간 그림자다. 그림자($K$, $t_{1/2}$)만 보고 원인($CL$인지 $V$인지)을 단정하면 유지용량, 부하용량, 공변량 위치가 모두 틀릴 수 있다. [G p.17; R&T p.58]
...
<!-- ANCHOR -->
R&T의 opening figures가 이 구조를 가장 빠르게 보여준다. Drugs A/B는 같은 초기 농도지만 slope와 AUC가 다르고, Drugs C/D는 같은 half-life지만 초기 농도와 AUC가 다르다. 즉 “처음 높이”와 “기울기”는 서로 다른 primary determinant의 표면 현상이다. [R&T p.54–55]
...
<!-- MASTER LENS -->
$CL$은 “농도가 얼마나 빨리 내려가는가”가 아니라 **단위 시간당 완전히 비워지는 가상의 혈장/혈액 부피**다. 이 관점에서 $CL=Q\cdot E$는 즉시 직관화된다. 혈류량 $Q$가 약물을 장기에 가져오고, 추출비 $E$가 그중 제거되는 비율을 정한다. 완전 추출자라도 $CL$은 $Q$를 넘을 수 없다. [R&T p.55–56]
...
AUC는 시간 전체에 걸친 exposure integral이다. 1차 IV bolus에서 제거된 총량은 $CL\cdot AUC$이고, 전체 dose가 결국 제거되므로 다음이 된다. [G p.19; R&T p.59–60]
...
<!-- ANCHOR -->
**정정된 핵심:** Subject 1과 4는 $K$와 $t_{1/2}$가 비슷하지만 $CL$은 다르다. S1은 $CL=0.1$ L/min, S4는 $CL=0.2$ L/min이다. S4에서 $V$도 2배 커져 $K=CL/V$가 비슷하게 보존된 것이며, 같은 $R_{in}$에서는 $C_{ss}$가 같지 않다. [G p.473–474; G Eq.2:22, p.23]
...
<!-- MASTER LENS -->
$V$는 실제 해부학적 부피가 아니라 **체내 총량을 혈장 농도로 환산하는 apparent dilution space**다. 같은 100 units를 넣어도 물에만 녹으면 $V=10$ L, charcoal에 결합해 물에서 거의 안 보이면 $V=100$ L이 된다. 총량은 같지만 측정 가능한 농도가 다르기 때문이다. [G Fig.2.3, p.14–15; R&T p.63–64]
...
R&T는 distribution equilibrium이 완성되기 전 time-zero extrapolation만으로 $V$를 확신하기 어렵기 때문에, terminal phase에서 $CL$과 $k$로 $V$를 구하는 우회식을 제시한다. [R&T p.63]
...
Quinacrine의 $V=40{,}000$ L는 $V$가 실제 체적이 아니라 tissue binding과 measurement space의 불일치를 흡수한 환산률임을 보여주는 source-bound 극단값이다. [G p.20]
...
<!-- MASTER LENS -->
$K$는 독립적 제거 능력이 아니라 **체내 amount 중 단위 시간당 제거되는 fraction**이다. 분율 속도는 제거 능력($CL$)을 분포 공간($V$)으로 나눈 값이므로 $K=CL/V$가 된다. [G p.17; R&T p.56]
...
- Distribution phase 중 관찰되는 빠른 decline은 반드시 elimination $K$가 아니다. midazolam은 초기 약 1시간 동안 distribution phase를 보이고 이후 terminal phase에서 elimination half-life가 해석된다. [R&T p.61–62]
...
<!-- MASTER LENS -->
$t_{1/2}$는 “약물이 빠지는 속도” 자체가 아니라 **$CL$과 $V$가 합성해 만든 시간 척도**다. R&T의 핵심 문장은 명확하다: half-life와 elimination rate constant는 대개 $CL$과 $V$에 의존하지, 그 반대가 아니다. [R&T p.53, p.58]
...
Constant infusion에서 항정상태 도달 정도는 half-life 단위로 표현된다. Gabrielsson Fig.2.8은 1 half-life 50%, 2 half-lives 75%, 3 half-lives 87.5%, 3–4 half-lives에 약 90% 도달을 보여준다. [G p.22]
...
R&T Fig.3-5는 2시간까지 AUC가 전체 AUC의 48%라고 제시하며, Fig.3-6은 distribution과 elimination의 속도 경쟁을 두 시나리오로 설명한다. Midazolam처럼 distribution이 elimination보다 빠르면 terminal phase가 elimination을 대표하기 쉽다. [R&T p.64–65]
...
<!-- MASTER LENS -->
$MRT$는 약물 분자가 체내에 평균적으로 머무는 시간이다. $t_{1/2}$가 “절반으로 줄어드는 시간”이라면, $MRT$는 농도-시간 곡선의 시간 방향 평균, 즉 1차 모멘트다. [G p.19; R&T p.60]
...
<!-- ANCHOR -->
4명 모두 $MRT/t_{1/2}$가 약 1.44에 모인다. 이 비율은 1구획 IV bolus의 수학적 signature다. 이 세션에서는 이 사실까지만 lock하고, 다구획 또는 extravascular MRT 전개는 후속 세션으로 보낸다. [G p.19, p.474; R&T p.60–61]
...
<!-- RECAP -->
**§5 Recap:** 세 혼동쌍은 모두 “표면적으로 비슷하지만 위계가 다르다”는 문제다. $CL$ vs $K$는 primary/secondary, $V$ vs $V_{ss}$는 apparent/steady-state, $t_{1/2}$ vs $MRT$는 median-like time/mean residence time의 차이다.
...
**정답:** AUC와 $C^0$를 분리해서 본다. $CL$ 감소는 $AUC=Dose/CL$ 증가로 나타난다. $V$ 증가는 $C^0=D/V$ 감소로 나타난다. 둘이 동시에 변하면 $t_{1/2}$ 하나로는 원인 분리가 불가능하다. [G p.17–20; R&T p.58–60]
...
이 세션은 PK/PD 학습 그래프의 zero point다. 이후의 모든 구조는 여기서 갈라진다.
...
자동화 도구는 IV bolus data에 monoexponential fit을 빠르게 수행하고 $CL$, $V$, $K$, $t_{1/2}$를 출력할 수 있다. 그러나 전문가의 가치는 **숫자를 출력하는 것**이 아니라 **숫자의 위계를 읽는 것**이다.
