# 세션 09 — TMDD: Target-Mediated Drug Disposition, full vs MM approximation, $K_d$/$K_m$/target turnover — v4.0 보강본_final
...
ADA(anti-drug antibody)와 immunogenicity는 PK 자체를 바꿀 수 있으니, 설명되지 않는 청소율 증가가 보이면 target biology뿐 아니라 ADA도 같이 점검해야 합니다 [R&T p.725]. ADA가 TMDD signature 자체를 시점에 따라 어떻게 변형시키는지는 M4 §F의 "ADA와 시간 의존 변형" 단락에서 별도로 다룹니다.
...
#### Anti-drug antibody (ADA)와 시간 의존 변형 — TMDD signature의 또 다른 분기점

soluble vs membrane-bound가 **target 쪽에서 오는 분기**라면, ADA는 **숙주(host) 면역계 쪽에서 오는 분기**예요. PK27이 다루는 single-dose 데이터에서는 ADA 신호가 보이지 않습니다. 그런데 임상 mAb의 repeated-dose 데이터에서는 ADA 형성이 **TMDD signature 자체를 시점에 따라 변형**시켜요. R&T는 이 메커니즘을 "Immunogenicity-Mediated Clearance"라는 별도 단원에서 다룹니다 [R&T p.712, "Immunogenicity-Mediated Clearance"]. 본문 인용을 그대로 풀면 — anti-drug antibody가 형성되면 mAb-ADA 면역복합체(immune complex)가 phagocytic cell의 uptake와 lysosomal degradation을 자극해서 mAb 청소율을 끌어올린다는 거예요 [R&T p.712].

| TMDD signature 변형 | 임상·모델링 신호 | 출처 |
|---|---|---|
| **Phase B(linear) 청소율의 급격한 증가** | nonspecific linear CL이 target turnover 변화 없이 시간에 따라 증가, 곡선이 phase B 구간에서 가파르게 떨어짐 | R&T p.712 |
| **개체 간 변이(BSV) 폭발** | 일부 환자에서 trough가 sub-therapeutic으로 급격히 떨어지고, 나머지는 그대로 — bimodal 분포 | R&T p.712 |
| **시점 의존 PK (time-varying CL)** | 같은 환자의 PK가 dosing 초기와 후반에서 다른 청소율 — IOV 또는 dose-time interaction 형태 | R&T p.712 |

ADA 형성의 **시간 척도**는 R&T Fig.21-19(cynomolgus monkey의 anti-tetanus toxoid ADA 시간경과)가 보여줘요 — **ADA titer가 약 200시간(약 8~9일)에서 검출되기 시작해 약 400~500시간(약 17~21일)에 정점**에 도달합니다 [R&T p.725, Fig.21-19]. 이 정도 시간 척도면 multiple-dose mAb PK 데이터의 두 번째·세 번째 dose 구간부터 covariate-naive linear CL 가정이 깨질 수 있어요.

수식으로 보면 Full TMDD ligand 항에 시간 의존 1차 sink 항이 추가되는 형태예요.

$$
\overbrace{\frac{dL}{dt}}^{\text{ligand 변화율}}
=
\underbrace{(\text{기존 TMDD 항})}_{\text{M4.B의 nonspecific CL + TMDD 경로}}
-\overbrace{\underbrace{k_{ADA}(t)\cdot L}_{\text{면역복합체 매개 sink}}}^{\text{시간 의존 1차 추가 항}}
$$

여기서 $k_{ADA}(t)$는 ADA titer와 함께 시간에 따라 증가하는 청소율 기여분으로, R&T Fig.21-19의 시간 척도에 따라 dose 후 약 200시간부터 켜져요. 이 항이 도입되면 — (i) Phase B의 linear clearance가 시간에 따라 증가하고, (ii) covariate 모델에서 baseline-time interaction 또는 occasion-dependent CL로 잡히며, (iii) 같은 환자의 PK가 시점에 따라 다른 청소율을 갖는 **IOV(inter-occasion variability) 또는 time-varying covariate** 형태로 나타납니다.

ADA 형성 위험은 단일 분자 특성이 아니에요. R&T가 정리한 위험 인자들은 다음과 같습니다 [R&T p.725].

| 위험 인자 | ADA 형성 위험 위계 | 출처 |
|---|---|---|
| 투여 경로 | **s.c. > i.m. > i.v.** | R&T p.725 |
| 분자 응집(aggregation) 정도 | 응집 ↑ → 면역원성 ↑ | R&T p.725 |
| 단백질의 인간 유사도 | murine > chimeric > humanized > human | R&T p.725 |

R&T는 추가로 — **neutralizing antibody**는 치료 mAb의 활성을 떨어뜨리는 게 일반적인 반면, mAb의 **노출(exposure)**은 청소율 변화의 방향에 따라 **증가할 수도 감소할 수도** 있다고 강조해요 [R&T p.712, paraphrased: 면역복합체가 phagocytic uptake를 자극해 CL이 올라가는 게 흔하지만, 일부 경우에는 ADA가 sink로 작용하지 않고 단순 결합 buffer로 작용해 노출이 일시적으로 늘기도 합니다]. 그래서 "ADA = 항상 청소율 증가"로 외우면 안 됩니다.

> 💡 **PK27 single-dose 데이터는 ADA 신호를 가르치지 않습니다.** Repeated-dose 임상 데이터에서 covariate 모델로 설명 안 되는 청소율 증가나 BSV 폭발이 보이면, target biology 변동 가설보다 **ADA를 먼저 의심**하는 게 일반적으로 안전해요. R&T Fig.21-19의 시간 척도(약 200~500 h)는 이 의심 시점을 정량적으로 지원합니다 [R&T p.725, Fig.21-19].

> ⚠️ **모델링 함정** — ADA-매개 청소율 증가를 단순한 "target turnover 변화"로 잡으면 $k_{in}$이나 $R_0$를 잘못 추정하게 돼요. 두 신호는 **시간 위계가 다릅니다** — target turnover 변화는 보통 baseline 시점부터 차이가 있는 반면, ADA-매개 변형은 **dose 후 약 200h 이후부터 켜지는 시간 지연 신호**예요. 그래서 ADA가 의심되는 데이터에서는 **dose-elapsed-time covariate**나 occasion-based IOV 구조로 두 신호를 분리할 수 있는지 먼저 살펴봐야 합니다.

**실무 한 줄**: ADA 형성 이후의 TMDD signature는 single-dose 형태의 4-phase에서 벗어나서 **"시점에 따라 다른 phase 구조"**를 보입니다. 그래서 multiple-dose mAb 데이터를 분석할 때는 **(i) ADA assay 결과 가용성, (ii) dosing occasion별 IOV 구조, (iii) Phase B 영역의 시간 의존 CL drift** — 이 세 가지를 ADA 가능성 점검표로 갖고 가야 해요. PK27이 가르치는 4-phase 구조는 **single-dose 데이터의 정량 지문**이고, ADA가 개입하는 multiple-dose 데이터에서는 **이 지문이 시간에 따라 흐려진다**는 사실이 분석의 출발 전제가 됩니다.
...
| **ADA-매개 면역복합체 형성** | **apparent Phase B CL, IOV, $k_{ADA}(t)$** | → | **시점 의존 청소율 증가, BSV 폭발, sub-therapeutic trough** | **ADA assay + dose-occasion IOV 점검, time-varying covariate 검토** [R&T p.712] |
...
| Biologics 임상약리 리포트 | 모델 경계와 외삽 방어, **soluble vs membrane-bound target 분기**, **ADA-매개 time-varying CL 변형** | MM/QSS/Full TMDD 선택 근거 방어 |
