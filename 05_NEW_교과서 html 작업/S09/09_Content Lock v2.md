# 09_Content Lock v2 — Editorial Process Record

## PASS 1 — Edit Log Summary

| Location (§ + card) | Edit type | Before (excerpt) | After (excerpt) |
|---|---|---|---|
| Curation Map | R1/R2 | “항체 PK 자체가 아니라…” | 핵심 대상을 두 문장으로 분리하고 target-mediated disposition 첫 사용을 명확화. |
| §1 Big Idea | R1/R3 | “큰 약물이 천천히 빠진다…” | 원인과 PK27 결과를 분리해 읽기 흐름을 개선. |
| M1 Big Idea/Bucket | R2/R5 | turnover baseline 설명 | pool 교체 속도라는 mental image를 추가. |
| M2 Big Idea/Bridge | R1/R2 | ADME mechanism 나열 | receptor-mediated uptake/FcRn salvage 첫 사용을 짧게 풀이. |
| M3 Big Idea/Hierarchy | R1/R2 | concentration hierarchy 설명 | threshold crossing → slope 차이를 명시. |
| M4 Apex/Binding | R1/R2 | 8 parameter, ODE, `kon`/`kin` | ODE system 의미와 binding-vs-synthesis 구분을 선명화. |
| M5 Big Idea/Formal | R1/R2 | MM reduction 설명 | `R`/`RL`을 접는 모델이라는 causal bridge 추가. |
| §8 Meta-frame | R4 | 긴 mechanism 나열 | small molecule 대비 mAb의 복합 mechanism을 두 문장으로 분리. |

## PASS 2 — Annotation Candidate Table

| Type | Location (§ + card) | Current text (excerpt) | Proposed annotation | Risk |
|---|---|---|---|---|
| A | Curation Map | target-mediated disposition | 표적 결합이 약물 배치를 바꾸는 현상 | Low |
| A | Curation Map M5 | occupancy | 표적 중 ligand가 결합한 비율 | Low |
| A | M1 | endogenous compound | 체내에서 원래 생성되는 물질 | Low |
| B | M1 | bucket/water turnover | pool 전체가 얼마나 빨리 교체되는가 | Low |
| A | M2 | receptor-mediated uptake | 수용체 결합 후 세포 내 유입 | Low |
| A | M2 | FcRn salvage | IgG를 분해에서 회수하는 기전 | Low |
| A/B | M4 | ODE system | 시간에 따른 상태변화 방정식 묶음 | Low |
| B | M5 | MM reduction | `R`/`RL`을 직접 추적하지 않음 | Low |

## Final Recommendation List

**Must annotate (Low risk)**: 위 candidate table의 모든 Low-risk 항목.  
**Optional (Medium risk, high learning value)**: 없음.  
**Do not annotate**: 새로운 수치, 새로운 clinical scenario, 새로운 source claim, execution-environment-specific NONMEM settings.
...
**Mode**: Content Lock v2 / Readability-polished / Minimal annotation applied  
...
**Hard constraints applied**: No HTML; no figure markers; source page tags preserved/inserted; unsupported regulatory/cost scenarios deleted or explicitly labeled; Content Lock v2 preserves Content Lock v1 meaning, section order, equations, and source page tags.
...
Next: Phase 4C Visual Pedagogy Triage, then HTML compile. 본 Content Lock v2는 텍스트 확정본이며, figure markers는 의도적으로 삽입하지 않았다.
