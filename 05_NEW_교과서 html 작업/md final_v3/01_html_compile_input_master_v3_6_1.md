...
<!-- SECTION_CORE: SC-01 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $CL$과 $V$를 원인 좌표로 고정하라 — $K$·$t_{1/2}$·$MRT$는 그 둘에서 내려오는 결과다
...
<!-- SECTION_CORE: SC-02 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 초기농도·기울기·AUC를 한 지표처럼 묶지 마라 — $C^0$는 $V$, AUC는 $CL$, 시간축은 $CL/V$가 지배한다
...
<!-- SECTION_CORE: SC-03 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $CL$은 AUC·$C_{ss}$·유지용량을 지배하는 정화 능력이다 — $K$가 같아도 $CL$이 다르면 임상 판단은 갈라진다
...
>
> $$
> \underbrace{\text{Rate of elimination}}_{\text{제거속도}}=\underbrace{CL}_{\text{정화부피/시간}}\cdot\underbrace{C}_{\text{현재농도}}
> $$
...
>
> $$
> \underbrace{CL}_{\text{정화부피/시간}}=\underbrace{Q}_{\text{혈류(운반)}}\cdot\underbrace{E}_{\text{추출분율}}
> $$
...
<!-- SECTION_CORE: SC-04 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $V$는 실제 부피가 아니라 총량을 농도로 환산하는 겉보기 공간이다 — 큰 $V$를 체적처럼 읽는 순간 부하용량 해석이 무너진다
...
>
> $$
> \underbrace{V}_{\text{환산공간}}=\frac{\underbrace{A_b}_{\text{체내총량}}}{\underbrace{C}_{\text{측정농도}}}
> $$
...
>
> $$
> \underbrace{V}_{\text{초기희석공간}}=\frac{\underbrace{D_{iv}}_{\text{IV 용량}}}{\underbrace{C^0}_{\text{외삽초기농도}}}
> $$
...
>
> $$
> \underbrace{V}_{\text{말단용적}}=\frac{\underbrace{CL}_{\text{정화능}}}{\underbrace{K}_{\text{말단기울기}}}=\underbrace{1.44}_{1/\ln 2}\cdot\underbrace{CL}_{\text{정화능}}\cdot\underbrace{t_{1/2}}_{\text{반감기}}
> $$
...
>
> $$
> \underbrace{V}_{\text{관찰용적}}=\underbrace{V_u}_{\text{비결합용적}}\cdot\underbrace{f_u}_{\text{유리분율}}
> $$
...
<!-- SECTION_CORE: SC-05 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $K$는 독립 제거 능력이 아니라 $CL/V$의 시간 그림자다 — 분율 제거를 읽어야 반로그 직선이 이해된다
...
>
> $$
> \underbrace{\frac{dC}{dt}}_{\text{농도변화}}=-\underbrace{K}_{\text{분율속도}}\cdot\underbrace{C}_{\text{현재농도}}=-\frac{\underbrace{CL}_{\text{정화능}}}{\underbrace{V}_{\text{분포공간}}}\cdot\underbrace{C}_{\text{현재농도}}
> $$
...
>
> $$
> \underbrace{C}_{\text{시간 }t\text{ 농도}}=\underbrace{C^0}_{\text{초기농도}}\cdot\underbrace{e^{-K\cdot t}}_{\text{지수감소}}=\frac{\underbrace{D}_{\text{용량}}}{\underbrace{V}_{\text{분포공간}}}\cdot e^{-(\underbrace{CL}_{\text{정화능}}/\underbrace{V}_{\text{분포공간}})\cdot t}
> $$
...
>
> $$
> \underbrace{\frac{A}{Dose}}_{\text{남은분율}}=\underbrace{e^{-K t}}_{\text{지수잔존}}=\left(\frac12\right)^{\underbrace{n}_{t/t_{1/2}}},\quad \underbrace{n}_{\text{반감기수}}=\frac{\underbrace{t}_{\text{시간}}}{\underbrace{t_{1/2}}_{\text{반감기}}}
> $$
...
<!-- SECTION_CORE: SC-06 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $t_{1/2}$는 제거 능력이 아니라 $V/CL$이 만든 시간 척도다 — 말단 기울기가 진짜 소실을 대표하는지 먼저 의심하라
...
<!-- SECTION_CORE: SC-07 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $MRT$는 절반 시간이 아니라 AUMC/AUC의 시간가중 평균이다 — $1.443\times t_{1/2}$는 1구획 IV bolus에서만 안전하다
...
>
> $$
> \underbrace{AUC_0^\infty}_{\text{총노출}}=\int_0^\infty \underbrace{C(t)}_{\text{농도함수}}\,dt
> $$
...
>
> $$
> \underbrace{AUMC_0^\infty}_{\text{시간가중노출}}=\int_0^\infty \underbrace{t}_{\text{시간가중치}}\cdot\underbrace{C(t)}_{\text{농도함수}}\,dt
> $$
...
>
> $$
> \underbrace{MRT}_{\text{평균체류}}=\frac{\underbrace{AUMC_0^\infty}_{\text{시간가중노출}}}{\underbrace{AUC_0^\infty}_{\text{총노출}}}
> $$
...
<!-- SECTION_CORE: SC-08 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 혼동쌍은 표면 지표를 원인 좌표로 되돌리는 훈련이다 — $CL$·$V$·$t_{1/2}$·$MRT$를 같은 말로 섞지 마라
...
<!-- SECTION_CORE: SC-09 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 자기 테스트는 계산 문제가 아니라 위계 복원 훈련이다 — $C_{ss}$는 $CL$, 시간상수는 $V/CL$, 구조 방어는 분포 단계에서 판정하라
...
<!-- SECTION_CORE: SC-10 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 전문가의 해석력은 숫자가 아니라 위계 번역에서 나온다 — $CL$·$V$·$K$·$t_{1/2}$를 임상 결정 좌표로 다시 배치하라
