...
<!-- SECTION_CORE: SC-01 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Session 13은 평균 파라미터가 아니라 $\eta$·$\varepsilon$·covariate·subgroup을 어느 변이 그릇에 담을지 정하는 수업이다.
...
<!-- SECTION_CORE: SC-02 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** C1→C2→C3→C4 순서로 $\theta$·$\eta$·$\varepsilon$를 분리하고, IIV·IOV·RUV가 서로 다른 그릇임을 먼저 잠가라.
...
<!-- SECTION_CORE: SC-03 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $\theta$는 중심, $\eta$는 개인 위치, $\varepsilon$는 관측 잔차다 — 이 세 층을 분리하지 않으면 `$OMEGA`와 `$SIGMA`는 숫자일 뿐이다.
...
> +\underbrace{g(\boldsymbol{\theta},\boldsymbol{\eta_i},x_{ij})}_{\text{오차 스케일}}\underbrace{\varepsilon_{ij}}_{\text{잔차}}
> $$
>
...
>
> $$
> \underbrace{CL_i}_{\text{개인 CL}}=\underbrace{CL}_{\text{모집단 CL}}\cdot\underbrace{\exp(\eta_i)}_{\text{IIV 배율(양수)}}
> $$
>
...
<!-- SECTION_CORE: SC-04 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** `$OMEGA` 구조와 shrinkage를 먼저 보라 — EBE는 개인 참값이 아니라 모델·자료·추정법이 만든 조건부 그림자다.
...
<!-- SECTION_CORE: SC-05 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $\varepsilon$의 형태를 먼저 고정하라 — residual error를 틀리면 $\omega^2$와 covariate signal이 모두 가짜로 움직인다.
...
>
> $$
> \underbrace{Y}_{\text{관측값}}=\underbrace{F}_{\text{예측값}}+\underbrace{\varepsilon}_{\text{절대오차}}
> $$
>
...
>
> $$
> \underbrace{Y}_{\text{관측값}}=\underbrace{F}_{\text{예측값}}\left(1+\underbrace{\varepsilon}_{\text{상대오차}}\right)
> $$
>
...
>
> $$
> \underbrace{Y}_{\text{관측값}}=\underbrace{F}_{\text{예측값}}\underbrace{\exp\left(\underbrace{\varepsilon}_{\text{로그 잔차}}\right)}_{\text{오차 배율}}
> $$
>
...
>
> $$
> \underbrace{\ln Y}_{\text{로그 관측값}}=\underbrace{\ln F}_{\text{로그 예측값}}+\underbrace{\varepsilon}_{\text{로그 잔차}}
> $$
>
...
>
> $$
> \underbrace{Y}_{\text{관측값}}=\underbrace{IPRED}_{\text{개인 예측}}+\overbrace{\sqrt{\underbrace{(\theta_{prop}\cdot IPRED)^2}_{\text{비례 성분}}+\underbrace{\theta_{add}^2}_{\text{가법 성분}}}}^{\text{W: 잔차 스케일}}\underbrace{\varepsilon}_{\text{표준 잔차}}
> $$
>
...
<!-- SECTION_CORE: SC-06 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $\omega^2$–$\sigma^2$ leakage를 잡지 못하면 잔차 문제는 공변량 성공처럼 보이고, 공변량 실패는 잔차 문제처럼 숨는다.
...
<!-- SECTION_CORE: SC-07 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** C3의 핵심은 $\eta$를 줄이는 것이 아니라 $CL$, $CRCL$, $f_u$, $CL_u$가 어떤 좌표계에서 변이를 설명하는지 분리하는 것이다.
...
>
> $$
> \underbrace{\Delta OFV}_{\text{OFV 변화}}\ >\ \underbrace{3.84}_{\text{LRT 기준}}
> $$
>
...
>
> $$
> \underbrace{CL_i}_{\text{개인 CL}}=\underbrace{\theta_1}_{\text{기저 CL}}+\underbrace{\theta_2\cdot GFR_i}_{\text{GFR 설명분}}+\underbrace{\eta_i}_{\text{남은 편차}}
> $$
>
...
>
> $$
> \underbrace{CL_i}_{\text{개인 CL}}=\underbrace{\theta_1}_{\text{CRCL=90 기준}}\cdot\underbrace{\left(\frac{CRCL_i}{90}\right)^{\theta_2}}_{\text{CRCL 배율}}\cdot\underbrace{\exp(\eta_i)}_{\text{남은 IIV}}
> $$
>
...
>
> $$
> \underbrace{CL}_{\text{total CL}}=\underbrace{f_u}_{\text{비결합분율}}\cdot\underbrace{CL_u}_{\text{unbound CL}}
> $$
>


<!-- SECTION_CORE: SC-08 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** PK 좌표계를 unbound로 정리해도 PD 변이는 남는다 — Vanishing Covariate Cascade와 VPC 불일치가 그 경계선을 드러낸다.
...
>
> $$
> \underbrace{CL=11.4\ \mathrm{L\cdot h^{-1}}}_{\text{total CL}}\quad (\underbrace{CV=28\%}_{\text{total CV}}),\qquad \underbrace{CL_u=720\ \mathrm{L\cdot h^{-1}}}_{\text{unbound CL}}\quad (\underbrace{CV=9\%}_{\text{unbound CV}})
> $$
>
...
>
> $$
> \underbrace{C_{total}}_{\text{총 농도}}\ >\ \underbrace{50\ \mu\mathrm{g\cdot L^{-1}}}_{\text{total 안전경계}}
> $$
>
...
>
> $$
> \underbrace{E_{max}}_{\text{최대 반응능}}\ <\ \underbrace{1}_{\text{완전반응 기준}}
> $$
>
...
<!-- SECTION_CORE: SC-09 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** C4에서는 매끄러운 $\eta$ 분포 뒤의 genotype·phenotype 하위군을 찾아야 한다 — 범주형 효과는 cell size와 식별가능성이 먼저다.
...
>
> $$
> \underbrace{p^2}_{\text{동형접합 1}}+\underbrace{2pq}_{\text{이형접합}}+\underbrace{q^2}_{\text{동형접합 2}}=\underbrace{1}_{\text{총 빈도}}
> $$
>
...
>
> $$
> \underbrace{CL_i}_{\text{개인 CL}}=\underbrace{\theta_1}_{\text{기준 CL}}\cdot\underbrace{\theta_2^{I(CYP2D6PM_i=1)}}_{\text{PM 효과}}\cdot\underbrace{\exp(\eta_i)}_{\text{남은 IIV}}
> $$
>
...
<!-- SECTION_CORE: SC-10 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 이봉형 분포와 empty phenotype cell은 “효과 없음”이 아니라 “하위구조를 볼 정보가 부족함”일 수 있다.
...
>
> $$
> \underbrace{RSE}_{\text{상대 표준오차}}\ >\ \underbrace{80\%}_{\text{불안정 경고}}
> $$
>
> *↑ 이 부등식은 원문에서 practice decision rule로 제시된 경고 기준으로 읽는 것이 자연스럽습니다. 원문 보존 원칙에 따라 원 표현은 수정하지 않았습니다.*
...
<!-- SECTION_CORE: SC-11 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $\omega^2$ vs $\sigma^2$, total $CL$ vs unbound $CL_u$, PK vs PD, 평균 vs 분포 모양을 혼동하면 같은 변이를 네 번 잘못 읽는다.
...
>
> $$
> \underbrace{\omega^2}_{\text{IIV 그릇}}\quad \text{vs}\quad \underbrace{\sigma^2}_{\text{RUV 그릇}}
> $$
>
...
<!-- SECTION_CORE: SC-12 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Self-test 초반부는 $CL_i=CL\exp(\eta_i)$, residual randomness, PK50 $f_u$ 좌표계의 경계를 즉시 말할 수 있는지 묻는다.
...
>
> $$
> \underbrace{CL_i}_{\text{개인 CL}}=\underbrace{CL}_{\text{모집단 중심}}\cdot\underbrace{\exp(\eta_i)}_{\text{양수 배율}}
> $$
>
...
>
> $$
> \underbrace{CV(CL)^2}_{\text{total CL 변이}}\approx\underbrace{CV(f_u)^2}_{\text{fu 변이}}+\underbrace{CV(CL_u)^2}_{\text{CLu 변이}}
> $$
>
> *↑ 원문은 이 식을 사용하지 않는 이유를 설명합니다. 원문 보존 원칙에 따라 식 자체와 삭제 판단은 변경하지 않았습니다.*


<!-- SELF-TEST -->
<!-- SECTION_CORE: SC-13 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Self-test 후반부는 unbound PK로도 남는 PD 변이, 이봉형 $\eta$, ethnicity 해석, residual-first 판단을 방어하게 만든다.
...
>
> $$
> \underbrace{E_{max}}_{\text{최대효과}}\ <\ \underbrace{1}_{\text{완전반응 기준}}
> $$
>
...
<!-- SECTION_CORE: SC-14 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Session 13의 해자는 $\omega^2$를 생리학·유전학·행동·측정 구조의 혼합물로 읽고 NONMEM 출력으로 방어하는 능력이다.
...
>
> $$
> \underbrace{\eta\text{-shrinkage}}_{\text{EBE 분해능}}<\underbrace{30\%}_{\text{경고선}},\quad \underbrace{RSE}_{\text{불확실성}}>\underbrace{80\%}_{\text{불안정}},\quad \underbrace{condition\ number}_{\text{수치 조건성}}>\underbrace{1000}_{\text{조건성 경고}},\quad \underbrace{|r|}_{\text{상관}} > \underbrace{0.7}_{\text{공선성 경고}}
> $$
>
