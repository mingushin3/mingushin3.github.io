...
<!-- SECTION_CORE: SC-01 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** NCA 학습은 AUC 숫자를 믿는 훈련이 아니라, 말단기울기·외삽·입력보정의 가정을 먼저 감사하는 훈련이다.
...
>
> $$
> \underbrace{t_{1/2}}_{\text{반감기}}=\frac{\underbrace{\ln(2)}_{\text{50\% 변환}}}{\underbrace{k}_{\text{소실속도}}}
> $$
...
<!-- SECTION_CORE: SC-02 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** NCA는 model-free가 아니라 compartment-free다 — 비선형 신호가 보이는 순간 면적 요약은 회귀 초기값으로 내려간다.
...
<!-- SECTION_CORE: SC-03 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** AUC는 관측 사다리꼴과 $\lambda_z$ 꼬리의 합이다 — $\lambda_z$와 \%Extrapolated를 못 지키면 $CL$·$V_z$·노출 해석이 함께 무너진다.
...
> \sum_i
> \underbrace{\frac{C_i+C_{i+1}}{2}}_{\text{평균 농도}}
> \cdot
> \underbrace{\Delta t}_{\text{시간폭}}
> $$
...
> \underbrace{\frac{C_i-C_{i+1}}{\ln(C_i/C_{i+1})}}_{\text{로그평균 농도}}
> \cdot
> \underbrace{\Delta t}_{\text{시간폭}}
> $$
...
> \frac{\underbrace{C_{last}}_{\text{말단 농도 또는 }\hat C_{last}}}{\underbrace{\lambda_z}_{\text{말단기울기}}}
> $$
...
> \frac{\underbrace{AUC_{t_{last}\to\infty}}_{\text{꼬리 AUC}}}{\underbrace{AUC_{total}}_{\text{총 AUC}}}
> \times 100
> $$
...
<!-- SECTION_CORE: SC-04 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** C1의 방어력은 AUC 값이 아니라 $C_{last}$ 처리, 외삽 분율, 말단기 도표까지 추적한 감사 기록에서 나온다.
...
<!-- SECTION_CORE: SC-05 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** AUMC와 MRT는 평균처럼 보이지만 $1/\lambda_z^2$ 꼬리와 MIT 보정 때문에 AUC보다 더 먼저 흔들린다.
...
> \sum_i
> \underbrace{\frac{t_iC_i+t_{i+1}C_{i+1}}{2}}_{\text{시간가중 평균}}
> \cdot
> \underbrace{\Delta t}_{\text{시간폭}}
> $$
...
>
> $$
> \underbrace{MRT}_{\text{평균 체류}}=
> \frac{\underbrace{\sum_j t_j}_{\text{체류시간 합}}}{\underbrace{N}_{\text{분자 수}}}
> $$
...
>
> $$
> \underbrace{MRT_{\text{corrected}}}_{\text{보정 MRT}}=\underbrace{\frac{AUMC}{AUC}}_{\text{관측 MRT}}-\underbrace{MIT}_{\text{입력 평균시간}},\qquad MIT\in\left\{\underbrace{0}_{\text{IV bolus}},\underbrace{T_{inf}/2}_{\text{주입 중간}},\underbrace{1/K_a}_{\text{흡수 평균}}\right\}
> $$
...
<!-- SECTION_CORE: SC-06 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** MRT를 보고하려면 AUMC 꼬리와 투여 입력시간을 함께 보고하라 — 평균 체류시간은 꼬리와 입력 지연을 동시에 상속한다.
...
<!-- SECTION_CORE: SC-07 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $CL$, $CL/F$, $F$, $V_{ss}$, $V_z$, $t_{1/2,eff}$는 같은 표의 숫자가 아니라 AUC·AUMC·입력보정 오류가 이름을 바꾼 결과다.
...
> \underbrace{MRT}_{\text{보정 MRT}}
> \cdot
> \underbrace{CL}_{\text{정화}}
> =
...
> \underbrace{\ln(2)}_{\text{50\% 변환}}
> \cdot
> \underbrace{MRT}_{\text{평균 체류}}
> $$

<!-- SECTION_CORE: SC-08 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $V_{ss}$ 표준식이 흔들리면 더 예쁜 값을 고르지 말고, 데이터가 표준식 적용 영역을 벗어났는지 먼저 진단하라.
...
<!-- SECTION_CORE: SC-09 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 노출 지표는 투여량의 장식이 아니다 — endpoint가 peak·cumulative·threshold·unbound 중 무엇을 묻는지에 맞춰 $C_{max}$, AUC, $C_{ss}$, $t_d$, $C_u$를 골라라.
...
>
> $$
> \underbrace{C_u}_{\text{비결합 농도}}=\underbrace{C_{total}}_{\text{총 농도}}\cdot\underbrace{f_u}_{\text{비결합률}}
> $$
...
>
> $$
> \underbrace{V\cdot\frac{dC}{dt}}_{\text{체내 변화}}=
> \underbrace{In}_{\text{입력}}-
> \underbrace{CL\cdot C}_{\text{제거}},\qquad
...
<!-- SECTION_CORE: SC-10 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** AUC와 AUMC, MRT와 반감기, $V_{ss}$와 $V_z$는 비슷한 이름으로 서로 다른 실패를 숨긴다 — 질문이 다르면 파라미터도 달라야 한다.
...
>
> $$
> \underbrace{MRT}_{\text{체류시간 평균}}=\frac{\underbrace{AUMC}_{\text{시간가중 노출}}}{\underbrace{AUC}_{\text{총 노출}}},\qquad \underbrace{t_{1/2}}_{\text{반감기}}=\frac{\underbrace{\ln(2)}_{\text{50\% 변환}}}{\underbrace{\lambda_z}_{\text{말단기울기}}}
> $$
...
>
> $$
> \underbrace{V_{ss}}_{\text{정상상태 V}}=\underbrace{MRT}_{\text{체류시간}}\cdot\underbrace{CL}_{\text{정화}},\qquad \underbrace{V_z}_{\text{말단 V}}=\frac{\underbrace{CL}_{\text{정화}}}{\underbrace{\lambda_z}_{\text{말단기울기}}}
> $$
...
<!-- SECTION_CORE: SC-11 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Q1–Q4는 계산 공식을 외우는 문제가 아니라 AUC 편향, $\lambda_z$ 조건, 높은 외삽, PK41 비선형 전환을 즉시 식별하는 훈련이다.
...
>
> $$
> \underbrace{MRT}_{\text{혈장 MRT}}=\frac{\underbrace{177}_{\text{AUMC}}}{\underbrace{44.2}_{\text{AUC}}}\approx \underbrace{4.0\ \mathrm{hr}}_{\text{시간 결과}}
> $$
