...
<!-- SECTION_CORE: SC-01 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 초기값·GOF·WRSS·설계 피드백은 fit 버튼 뒤가 아니라 모델 구축 루프 전체를 닫는 판단 축이다.
...
<!-- SECTION_CORE: SC-03 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $K$에서 $t_{1/2}$·AUC·$Cl$·$V$로 이어지는 초기값 사슬을 먼저 잠가라 — 잘못된 anchor는 회귀를 엉뚱한 basin으로 밀어 넣는다.
...
<!-- SECTION_CORE: SC-04 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 잔차 $\epsilon$와 WRSS는 곡선의 미관이 아니라 시간·농도 축에 남은 구조 오류와 가중치 오류를 드러낸다.
...
<!-- SECTION_CORE: SC-05 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** F-test는 nested+equal weighting에서만 쓰고, AIC/SC도 같은 WRSS 좌표가 아니면 모델 비교 언어가 아니다.
...
<!-- SECTION_CORE: SC-06 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** CV%·$r$·상관행렬은 fit의 화려함 뒤에 숨어 있는 parameter 식별가능성과 ridge를 먼저 의심하게 만든다.
...
<!-- SECTION_CORE: SC-07 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 편미분 peak와 정규화 민감도는 $A,\alpha,B,\beta$가 가장 잘 보이는 sampling time을 설계 좌표로 바꾼다.
...
<!-- SECTION_CORE: SC-08 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 민감도 분석은 $K_m$·$V_{max}$·capacity 변화가 dose·safety margin 결론을 흔드는 지점을 미리 찌른다.
...
<!-- SECTION_CORE: SC-09 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Nested·$r$·banana/fan·outlier 혼동을 풀지 못하면 GOF 판단은 숫자 계산이 아니라 오진이 된다.
...
<!-- SECTION_CORE: SC-10 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Self-test는 $K$·WRSS·F-test·derivative peak를 본문 없이 꺼내 실제 모델링 판단으로 쓰게 만든다.
