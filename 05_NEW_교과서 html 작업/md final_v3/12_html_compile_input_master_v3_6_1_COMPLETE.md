...
<!-- SECTION_CORE: SC-01 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $k_{e0}$·$k_{out}$·$k_{tol}$·$n\times\tau$ 중 무엇이 늦는지 먼저 명명하라 — 지연 크기보다 hidden state가 모델 family를 결정한다.
...
<!-- SECTION_CORE: SC-02 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Card 1은 $C_e$와 $k_{e0}$로 분포 지연을 시험하는 첫 관문이다 — CCW loop를 보아도 곧장 답으로 확정하지 마라.
...
>
> $$
> \underbrace{C_e}_{\text{생체상 농도}}=\overbrace{\frac{\underbrace{k_{e0}}_{\text{ke0 평형화}}\underbrace{D}_{\text{투여량}}}{\underbrace{V_c}_{\text{Vc 희석공간}}\left(\underbrace{k_{e0}-K}_{\text{속도 차}\,}\right)}}^{\text{농도 스케일}}\left(\overbrace{e^{-Kt}}^{\text{혈장 감소}}-\overbrace{e^{-k_{e0}t}}^{\text{생체상 추종}}\right)
> $$
...
>
> $$
> \underbrace{\frac{dC_e}{dt}}_{\text{Ce 변화율}}=\underbrace{k_{e0}}_{\text{ke0 추종속도}}\left(\overbrace{C}^{\text{혈장 농도}}-\overbrace{C_e}^{\text{생체상 농도}}\right)
> $$
...
<!-- SECTION_CORE: SC-03 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $k_{e0}$ 가설은 선형 PK, 용량 불변 $t_{max}$, loop collapse, 용량별 PD 파라미터 안정성을 통과해야 살아남는다.
...
>
> $$
> \overbrace{k_{e0}=0.025}^{\text{Ce 평형 시계}},\quad \overbrace{k_{out}=0.031}^{\text{turnover 시계}},\quad \overbrace{k_{off}=0.018\ \text{min}^{-1}}^{\text{koff 해리 시계}}
> $$
...
<!-- SECTION_CORE: SC-04 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Hysteresis 방향은 출발 신호일 뿐이다 — 같은 loop라도 $C_e$, turnover, tolerance, transduction이 서로 다른 hidden state를 만들 수 있다.
...
<!-- SECTION_CORE: SC-05 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** R&T 임상 예시는 방향 판독을 기전 판정으로 승격하지 말라고 가르친다 — collapse와 구조 해석으로 $k_{e0}$를 검증하라.
...
<!-- SECTION_CORE: SC-06 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Card 3의 핵심은 $M$이다 — $R$이 움직인 뒤 $M$이 따라오면서 tolerance, rebound, carry-over가 생긴다.
...
>
> $$
> \underbrace{\frac{dM}{dt}}_{\text{M 변화율}}=\underbrace{k_{tol}}_{\text{ktol 적응속도}}\left(\overbrace{R}^{\text{현재 R}}-\overbrace{M}^{\text{지연 M}}\right)
> $$
...
<!-- SECTION_CORE: SC-07 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** PD13은 $k_{tol}$·$k_{out}$ 경쟁이 반복 infusion의 trough drift와 rebound를 만든다는 실험적 앵커다.
...
>
> $$
> \overbrace{k_{tol}=4.2}^{\text{ktol 내성속도}}>\underbrace{k_{out}=2.9}_{\text{kout turnover}},\qquad \underbrace{k_{tol}=0.05}_{\text{느린 precursor}}
> $$
...
>
> $$
> \underbrace{\frac{dR}{dt}=k_{in}S(C)-k_{out}M}_{\text{자극−M 억제}},\qquad \underbrace{\frac{dM}{dt}=k_{tol}(R-M)}_{\text{M 지연추종}}
> $$
...
<!-- SECTION_CORE: SC-08 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** Card 4는 지연된 약물 도달이 아니라 receptor 이후 $R_1\rightarrow R_n$ 신호전달 연쇄를 모델링한다.
...
<!-- SECTION_CORE: SC-09 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** $n$과 $\tau$는 생물학적 단계 수가 아니라 onset 형상을 맞추는 식별 가능한 지연 구조다.
...
<!-- SECTION_CORE: SC-10 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 혼동 쌍은 같은 단위와 같은 loop에 속지 않게 만드는 방어선이다 — $k_{e0}$, $k_{out}$, $k_{off}$의 시계를 분리하라.
...
<!-- SECTION_CORE: SC-11 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** GOF 시그니처는 잔차와 파라미터 흔들림을 hidden state 언어로 번역하는 진단 사전이다.
...
<!-- SECTION_CORE: SC-12 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 자기점검은 정답 암기가 아니라 $C_e$·$R/M$·$R_n$ 중 어떤 상태가 늦는지 즉시 고르는 훈련이다.
...
<!-- SELF-TEST -->
<!-- SECTION_CORE: SC-13 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** PD21·PD35·Boss Dilemma는 표면 fit보다 설계, washout, 반복투여, onset 형상이 모델 선택을 결정한다는 압박 시험이다.
...
<!-- SECTION_CORE: SC-14 -->
> 🎯 **[지금 이 슬라이드들이 존재하는 이유]** 이 세션의 결론은 단순하다 — delay term을 추가하기 전에 hidden state를 명명하고, ODE는 그 다음에 선택하라.
