# 03_HTML Compile Input Master — ver2

**Target output filename:** `03_html_compile_input_master_ver2.md`  
...
**Version:** ver2 — Author-Key-Message Reinforcement Patch (3건의 Audit T3 SHOULD_FIX 저자 메시지를 본문 명제로 promote; 모든 PATCH MODE marker·page tag·수치·named example 보존)

## Phase 4D Certification — ver2

| Certificate | Status | Basis |
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A is independently readable and contains no compiler/audit/process text. ver2의 보강 3문장도 동일 톤으로 통합. |
| Zero-Omission Certificate | **PASS (ver2 보강 후)** | ver1에서 누락 위험으로 분류된 Audit T3 SHOULD_FIX 저자 핵심 메시지 3건 (T Fig.6-3 absorption-delays / T Figs.6-10–6-11 disposition effects / T p.185 oral-only identifiability summary)을 ver2에서 본문 명제로 promote. High-impact PDF/Audit/Crucible 요소 모두 본문에 살아있음. |
| Mastery-Uplift Certificate | PASS | PART A contains five bounded, source-labeled augmentation notes without broad rewrite. T p.185 author summary 보강은 Apex card의 mnemonic 강도를 정합적으로 강화. |
| Source-Boundary Certificate | PASS | All augmentations are labeled by source status and introduce no new page tags, unsupported numbers, or new named examples. ver2 보강 3문장은 모두 Audit-검증된 기존 page tag만 사용 (T p.172 / T pp.179–180 / T p.185). 새 수치·새 명명 예시·새 수식 도입 없음. |
| HTML-Readiness Certificate | PASS | PART B contains Phase 5 rendering instructions, marker mapping, page-tag rules, guardrails, and splice verification. ver2 보강은 PATCH MODE marker 위치를 변경하지 않으므로 Splice Verification Table 그대로 유효. |
...
F, dose, V는 linear system에서 곡선의 높이를 위아래로 scale한다. 그러나 `dC/dt = 0`은 “높이”가 아니라 “기울기가 0이 되는 시간 위치”를 찾는 조건이다. 따라서 곡선 전체가 위아래로 커지거나 작아져도 tmax의 시간 위치는 바뀌지 않는다. 그래서 linear 1구획 oral PK에서는 dose나 F가 바뀌면 Cmax와 AUC는 바뀌지만 tmax는 바뀌지 않는다 [T Fig.6-5, p.174]. **단, 이 “tmax 불변” 성질은 absorption 단계만 건드렸을 때의 이야기다. CL이 변하면 AUC가 변할 뿐 아니라 tmax와 Cmax까지 함께 움직이고, V가 변하면 Cmax와 tmax가 서로 다른 방향으로 영향을 받는다 — 즉 곡선의 변화 패턴이 “F·dose만 변했을 때”와 “disposition(CL/V)이 변했을 때”에서 서로 다른 신호로 나타나며, 이것이 absorption 변화와 disposition 변화를 구분하는 첫 번째 단서다** [T Figs.6-10–6-11, pp.179–180].
