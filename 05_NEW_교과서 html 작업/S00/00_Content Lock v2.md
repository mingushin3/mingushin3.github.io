# 4B Processing Record — Readability & Annotation

## PASS 1 — Edit Log Summary

| Location (§ + card) | Edit type | Before (excerpt) | After (excerpt) |
|---|---|---|---|
| Updated Curation Map | R1/R4 | “두 권의 교과서가 서로 다른 질문에 답하는 진입점이다...” | Gabrielsson 축과 Rowland & Tozer 축을 두 문장으로 분리 |
| §1 Big Idea | R1 | “순환 사고이며, PK/PD는...” | modeling statement와 PK/PD translation statement를 분리 |
| §2 M1 Big Idea | R1/R2 | “데이터에서 오지 않고 사용자의 추측이나...” | EDA 부재 → default 출발점 → local minimum risk 인과를 분리 |
| §2 M1 Why Model | R2/R3 | NCA와 model 필요 조건이 한 문장에 병렬 | “질문이 summary이면 NCA / 예측·비선형·PK/PD이면 model”로 대비 강화 |
| §2 M1 Structural Necessity | R5 | “예쁜 그림이 아니라 입력을 생산” | EDA가 Step 5 fitting 입력을 만든다는 mental image 강화 |
| §2 M2 Big Idea | R2/R3 | “이 질문은 dose 자체가 아니라...” | dose 숫자가 답이 아니라 exposure/response profile이 답임을 명시 |
| §2 M2 Formal Definition | R1/R2 | PK만 있으면/PD만 있으면 | PK-only와 PD-only의 한계를 각각 독립 문장으로 분리 |
| §2 M2 Structural Necessity | R2 | trial-and-error 대비 문장 | 관찰 기반 개선과 원리 설명의 한계를 분리 |
| §2 M3 Big Idea | R1/R4 | “Emax는 ceiling, C50은 potency...” | 각 parameter가 무엇을 분리하는지 병렬 구조로 정리 |
| §2 M3 γ section | R1/R2 | γ range와 all-or-none 양상 | 일반 범위 → 농도 범위 압축 → all-or-none 인과 순서로 분리 |
| §2 M3 Measurement | R1/R4 | bound/unbound 및 binding 조건 장문 | total 농도, unbound 의존성, 예외 조건을 세 문장으로 분리 |
| §2 M4 Big Idea | R2 | delay 원인 구분 문장 | distribution delay와 turnover delay를 명시적으로 분리 |
| §2 M4 Structural Necessity | R1 | Rt/kt/tt 질문 장문 | 외우는 질문 vs system 보존 질문을 분리 |
| §5 Confusion Pair 2 | R1/R2 | “문제는 Hill equation 자체가 아니라...” | 문제의 위치를 endpoint-model mismatch로 재강조 |
| §8 Professional Lock | R1/R2 | 긴 한 문장 lock | 질문→exposure→response→model complexity 순서로 분리 |

## PASS 2 — Annotation Candidate Table

| Type | Location (§ + card) | Current text (excerpt) | Proposed annotation | Risk |
|---|---|---|---|---|
| A | Updated Curation Map M1 | EDA | EDA(← 그래프 중심 탐색적 데이터 분석) | Low |
| A | Updated Curation Map M3 | C50 | C50(← 최대효과 절반 농도) | Low |
| A | Updated Curation Map M3 | Hill γ | γ(← 곡선 가파름 계수) | Low |
| A | Updated Curation Map M4 | Turnover | Turnover(← 체내 물질의 생성·소실 순환) | Low |
| B | §2 M1 C | EDA가 fitting 입력을 만든다 | Pass 1에서 충분히 명시됨 | Do not annotate |
| B | §2 M3 B | γ와 처방 사고 | Pass 1에서 충분히 연결됨 | Do not annotate |
| C | §2 M4 | dynamic equilibrium | 비유 삽입 시 실제 physiology를 과도 단순화할 위험 | Do not annotate |

## PASS 2 — Final Recommendation List

**Must annotate (Low risk)**: EDA, C50, Hill γ, Turnover first-use glosses only.  
**Optional (Medium risk, high learning value)**: 없음.  
**Do not annotate**: 추가 bridge sentence와 analogy는 독해 흐름을 끊거나 과도 설명 위험이 있어 제외.

## Length Gate

- Content Lock v1 body characters: 31,309
- Content Lock v2 body characters: 31,801
- Increase: 1.57%
- Constraint: ≤ 8% increase — PASS
...
**Output type**: Text-final lock only; no HTML; no figure markers
