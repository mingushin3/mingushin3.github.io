# 08_html_compile_input_master_v3.1.md
...
# 08_HTML Compile Input Master — v3.1 (Korean Readability Patch)

## Phase 4D Certification (v3.1 재인증)
...
| Korean-Readability Certificate (v3.1 신설) | PASS | v3.1에서 PART A 학습자 본문에 11개 surgical 한국어 가독성 patch (P-01 ~ P-11) 적용. 모든 patch는 (a) 의미·수치·수식·page tag·figure marker 변경 없음, (b) 영어식 추상 명사 나열을 한국어 동사 중심 문장으로 정리, (c) 긴 문장의 sentence split, (d) 첫 등장 용어의 단발 gloss 추가, (e) 영어 prose 블록의 한국어화에 한정. PART B rendering contract 100% 보존. |
...
**이 자료 활용법:** PART A는 학습자 본문입니다. §1을 한 번 읽어 mechanism triage 지도를 파악한 뒤, C1–C6를 순서대로 학습하세요. FIGURE_POINTER 블록은 교과서 원본 그림을 직접 확인하라는 안내이고, 마지막으로 §5 혼동쌍과 §7 자기테스트로 마무리하세요.

**학습 순서:**
1. Superposition과 dose-normalized exposure 패턴으로 비선형성을 진단한다.
2. Capacity, time, binding, DDI/transporter, PD interaction 중 유력한 mechanism을 식별한다.
3. 해당 mechanism을 modeling, TDM, DDI 해석으로 번역한다.
4. §5 혼동쌍과 Boss Dilemma(§7)로 자기점검한다.
...
<!-- MASTER LENS -->
이번 session의 한 문장 핵심은 다음이다. **비선형 PK는 "선형 superposition이 깨지는 현상"에 머무르지 않는다. 즉, capacity·time·flow·binding·interaction 중 어느 물리적 병목이 농도, 시간, route, 환자 genotype과 결합하여 비선형성을 만드는지를 식별하는 문제다.**
...
> **Practice Brief — [EXPERT_INFERENCE, v3]**  
...
> **⚠️ 그럴듯한 오해:** "단일 용량 fit이 좋으면 다중 투여 예측도 정확하다."
>
> **왜 틀렸는가 (구조적 분석):** MM 동역학에서 단일 용량 fit은 $V_{max}$–$K_m$ 공간의 ridge를 따라 **여러 $(V_{max}, K_m)$ 조합이 같은 fit을 만들 수 있다.** 단일 용량 데이터는 한 시점의 농도 궤적만 제약하지 분모 $K_m+C$의 곡률 영역을 좁게만 통과하기 때문이다. 다중 투여로 농도가 trough까지 떨어지고 다시 쌓이는 동안, 같은 $V_{max}/K_m$ ratio를 갖지만 절댓값이 다른 두 모델은 trough envelope를 **완전히 다른 위치**로 예측한다. 즉, $V_{max}$ 단독도 $K_m$ 단독도 아닌 **곡률 영역 통과 여부**가 다중 투여 예측 정확도를 결정한다.
>
> **실무에서 어떻게 드러나는가:** Phenytoin 단일 용량 PK fitting으로 다중 투여 $C_{ss}$를 예측한 뒤, 환자에서 독성 농도(>20 mg/L) 또는 무효 농도(<10 mg/L)가 나오는 패턴. NONMEM에서는 OFV가 잘 수렴해도 $V_{max}$–$K_m$ RSE가 모두 50% 이상이고 $\rho(V_{max}, K_m)$가 음수로 강하게 나오면 즉시 **"Single-dose Fit Trap"**을 의심한다. 이 패턴이 보이면 단일 모델 fit으로 임상 의사결정을 내리지 말고, dose ladder를 추가하거나 $K_m$를 외부 정보로 prior fix한 뒤 재추정한다. [v3]
...
7. **Mass action equivalence triangle:** 비선형 PK·TDM·DDI 직관은 같은 분모 구조의 외화다 → **§2 C5 mass action equivalence triangle 참조** [v3 consolidation; full equation triangle 및 도출은 §2 C5 ANNOTATION 단독 보존]. [G p.115], [T16 p.503], [T17 p.550]
...
- **TDM:** phenytoin-like capacity limitation에서는 small dose change가 큰 concentration change를 만들고 (Vm 20%↓ → Css 2× 변화), total-only monitoring은 displacement/binding 상황에서 misleading하다. plateau 도달 시간이 capacity 근접에서 발산한다는 점 역시 모니터링 간격 결정과 직결된다. [T16 pp.491–506], [T17 p.543]
...
| 6 | §2 C1~C6 Practice Brief 6개 추가 (즉각 실행 지시) | ✅ PASS | 각 카드 RECAP 직후 `> **Practice Brief — [EXPERT_INFERENCE, v3]**` blockquote 6개 |
...
*v3.1 file generated by Korean Readability Patch editor. Source: 08_html_compile_input_master_v3.md → 08_html_compile_input_master_v3.1.md. v3.1 patches (P-01 ~ P-11) applied as surgical Korean prose improvements only. No new scientific claims, page tags, numerical anchors, figure decisions, equations, or external references were added. Body integrity: v3의 모든 기존 수치·수식·source label·page tag·figure marker 100% 보존.*
