# 08_HTML Compile Input Master — ver2

## Phase 4D Certification (ver2 재인증)

| Certificate | Status | Basis |
| --- | --- | --- |
| Learner-Standalone Certificate | PASS | PART A는 학습자 네비게이션으로 시작하고, audit table·compiler-only 텍스트 없이 splice된 본문만 포함. ver2 보강은 동일 학습자 흐름을 유지함. |
| Zero-Omission Certificate | **PASS (ver2 강화)** | ver1의 "PRESENT_COMPRESSED" 항목 중 Audit MATCH로 검증된 specific clinical anchor·정량 수식·진단 시그너처를 surgical하게 복원함 (B11 Mastery Augmentation Log 추가 항목 7–14 참고). |
| Mastery-Uplift Certificate | **PASS (ver2 강화)** | 거장 시각의 30초 진단 시그널, mass action equivalence triangle, master clock 위계, 명명 NONMEM 진단 시그너처를 통합. 모두 source-labeled 상태로 추가됨. |
| Source-Boundary Certificate | PASS | 모든 ver2 추가 내용은 (a) Audit T1/T2/T3 MATCH 분류 source-backed 정량값, (b) Audit MATCH 분류 거장 진단 표현, (c) CRUCIBLE_DERIVED label로 epistemic status 명확. NDA·FDA·CPIC·외부 주장 추가 없음. |
| HTML-Readiness Certificate | PASS | PART B의 marker 매핑·figure 렌더링·page-tag 규칙·navigation·MathJax·accordion·regression 가드 전부 보존. ver2 추가 내용은 기존 마커 syntax를 따름. |

## Assembly Mode

**PATCH MODE + ver2 Surgical Augmentation.** ver1의 본문 spine을 보존한 상태에서, Audit MATCH 분류 anchor 누락 13건과 Crucible 명명 진단 시그너처 누락을 본문에 surgical하게 삽입. 모든 추가는 기존 마커 컴포넌트 및 source tag 규약을 따른다.
...
| 5 | $V_{max}/K_m$ 값이 **hepatic blood flow $Q_H$에 근접** | flow-extraction 분기 (high-extraction ↔ low-extraction 전환점); ethanol prototype | $E_H$ 계산 후 well-stirred model 가정 점검 [G p.140, Fig.18.4] |
...
이 6개 시그널은 §2 C1–C6의 진단 어휘로 그대로 회수된다. 학습자는 이 표를 처음 보고는 외우지 않아도 되며, 각 §2 카드의 "Diagnostic signatures" 섹션에서 다시 만나며 의미가 누적되도록 설계되었다.
...
<!-- ANNOTATION -->
**거장의 시각 (ver2 보강) — [CRUCIBLE_DERIVED]:** $V_{max}$와 $K_m$의 분모 구조 $K_m+C$는 substrate–enzyme reversible binding의 **quasi-steady-state assumption (QSSA)**이 만든 mass action의 산물이다. 이 분모가 그대로 Stage 3 DDI에서 inhibitor competitive term $K_m(1+C_{u,I}/K_I)+C$로 확장된다. 같은 mass action 골격의 다른 표현이라는 점을 보면, Eq.2:266 ↔ Eq.16-7 ↔ Eq.17-11이 한 줄로 이어진다. 또한 $V_{max}/K_m$ 비율이 hepatic blood flow $Q_H$ 근처가 되면 high-extraction ↔ low-extraction 분기점에 도달한다. [G pp.115–118], [G p.140 Fig.18.4]
...
이 AUC 식의 의미: high-C 영역에서는 농도 감소가 거의 선형이라 시간평균이 $C^0/2$에 가깝고, $K_m$ 부근으로 들어오면 exponential 가속이 시작되어 평균이 그보다 작아진다. **AUC 안에 $K_m$이 들어와 있다는 것이 capacity limitation의 수학적 흔적**이다. [G p.117]
...
### Clinical anchors (ver2 정량 보강)

**1. Phenytoin — capacity limitation의 정량 prototype** [T16 pp.491, 503–506]:

- Typical 성인 estimates: $V_m\approx 500$ mg/day, $K_m\approx 0.4$ mg/L (unbound 기준), $f_u\approx 0.1$, 따라서 total 기준 $K_m'\approx 4$ mg/L
- 치료역(total): 10–20 mg/L → 이미 $K_m'$의 2.5–5배 영역에 머문다는 의미. 즉 임상 농도 자체가 capacity 근처
- Opening clinical case: 300 mg/day에서 4 mg/L이던 환자가 500 mg/day로 증량 후 36 mg/L → **67% dose increase가 9× concentration increase**
- Steady-state denominator equation:
...
- $V_m$ sensitivity: $V_m$ 20% 감소가 $C_{ss}$를 **2배**로 만들고, $V_m$ 20% 증가가 $C_{ss}$를 약 **33% 감소**시킨다. 즉 capacity 근처에서는 $V_m$의 작은 변화가 $C_{ss}$를 비대칭적으로 흔든다. [T16 p.506]

- **Time-to-plateau divergence (Eq.16-10):** 300/350/400/425 mg/day 각각의 plateau는 6/9.3/16/22.7 mg/L이지만, 그 plateau에 도달하는 시간은 dose가 $V_m$에 가까울수록 **수일에서 수주로 발산**한다. capacity 근처에서는 모니터링 간격을 plateau 도달 추정 후로 옮겨야 한다는 임상 함의로 직접 연결된다. [T16 Fig.16-10, p.505]

- **Mixed pathway (Eq.16-11):** 한 약물의 제거 경로 중 nonlinear 경로의 비중 ($f_m^{NL}$)이 0.5 이상이면 dose-response 비대칭이 임상적으로 의미 있는 수준으로 나타난다. [T16 p.506]

**2. Alcohol** [T16 pp.500–502], [G pp.139–141], [G pp.556–562]:

- Approximate $V_m\approx 10$ g/hr, $K_m\approx 100$ mg/L, $V_d\approx 42$ L (70 kg) → 거의 zero-order plateau intuition을 제공
...
**3. Ascorbic acid — phenytoin의 거울상 (역방향 비선형)** [T16 p.492]:

- 75 mg/day에서 9 mg/L, 10,000 mg/day에서 약 19 mg/L → **133-fold dose increase가 약 2-fold concentration increase만** 만든다.
- 이유: 농도 상승과 함께 renal clearance가 크게 증가 (CLR < 0.5 mL/min @ 9 mg/L → 21 mL/min @ 19 mg/L, **>42-fold 증가**) — saturable reabsorption의 escape valve
...
### Diagnostic signatures (ver2 명명 보강)

> **Master's named diagnostic — [CRUCIBLE_DERIVED]**  
...
<!-- ANNOTATION -->
여기서 "제거"는 enzyme amount가 사라진다는 뜻만이 아니라, enzyme activity가 회복 전까지 유효하지 않다는 뜻이다. 회복은 inhibitor concentration이 사라지는 속도만이 아니라 enzyme resynthesis/degradation clock에 의해 제한된다.

### Quantitative MBI anchors (ver2 신설)
...
### Diagnostic signatures (ver2 명명 정리)

> **Master's named diagnostics — [CRUCIBLE_DERIVED]**  
> 
...
### Saturable binding 사례 (ver2 정량 보강)
...
- **Bosentan (TMDD prototype):** dose 증량 시 $V_{ss}$가 $0.67$ → $0.16$ L/kg로 **감소** — high-affinity, low-capacity endothelin receptor target binding이 포화되면서 organ에 잠긴 약물 비율이 줄어들기 때문. small molecule에서도 TMDD 거동이 나타나는 예. [T16 Fig.16-20, p.515]
...
### Saturable transport equation [T16 p.495]
...
### Drug–metabolite nonlinear model (ver2 복원 — §2.7.6 source-backed)
...
- **Model 구조:** 2-compartment drug + 1-compartment metabolite + MM elimination. ODEs는 Eq.2:304–2:308 영역, parameter estimates는 Tables 2.18–2.20에 제시된다. flip-flop 거동(metabolite 농도 prolonged tail)이 분포 kinetics가 아니라 parent의 MM elimination 때문일 수 있어 오독 위험이 높다.
...
<!-- ANNOTATION -->
**Mass action equivalence triangle (ver2 신설) — [CRUCIBLE_DERIVED]:** 본 세션의 수학적 정점은 다음 세 식이 **같은 분모 구조의 다른 표현**이라는 점이다.
...
### Inhibitor classification (ver2 신설 — Table 17-5) [T17 p.549]

| 분류 | 기준 (substrate AUC fold-increase) | 추가 표시 |
|---|---|---|
| Strong | ≥ 5-fold OR > 80% CL 감소 | MBI는 별도 (b) marking |
| Moderate | 2- to 5-fold | — |
| Weak | 1.25- to 2-fold | — |

**Substrate sensitivity 분류 (Table 17-6):** strong inhibitor와 병용 시 **≥4-fold AUC increase**를 보이는 substrate가 별도 분류. 이 분류는 NDA/IND DDI risk assessment의 표준 어휘이며, **본 문서에서는 textbook reference로만 유지하고 FDA Guidance와 직접 동일시하지 않는다**.

### PM × inhibitor amplification

Eq.17-18은 polymorphic pathway와 nonpolymorphic residual pathway가 동시에 고려될 때 maximum exposure ratio가 커질 수 있음을 보여준다. PM 환자에서 남은 nonpolymorphic pathway까지 strong inhibitor가 막으면 exposure ratio가 크게 뛸 수 있다.
...
### Core multifaceted anchors (ver2 정량 보강)
...
| 조건 | 우세 mechanism | 예 |
|---|---|---|
| Acute IV | Transporter inhibition (예: OATP1B1) | Atorvastatin–rifampin acute |
| Acute oral | Transporter + gut/first-pass | Rosuvastatin–cyclosporine |
| Chronic IV | Enzyme induction onset | Carbamazepine 자가유도 |
| Chronic oral | Enzyme induction × first-pass × transporter 복합 | Long-term rifampin rifabutin |

### PD interaction layer (ver2 수식 spine 복원)

**Receptor-level PD interactions** [G pp.224–227]:

- **다중 결합 부위 모델 (Eq.3:48)** [G p.224]: 한 ligand가 여러 결합 site에 cooperative하게 결합할 때의 response 곡선. Hill 형태 fitting의 mechanistic 출처.
...
- **경험적 두 약물 모델 (Eq.3:51, Table 3.2)** [G p.226]: 두 효과가 작용 site도 mechanism도 다를 때의 일반화된 합산/곱셈 framework.
- **거울이성질체 상호작용 (Eq.3:52–3:53)** [G p.227]: 같은 receptor의 R/S 거울이성질체가 서로 다른 affinity를 가질 때. ketamine R-/S- enantiomer 농도-효과 관계가 직접 example.

**Same-receptor full agonist combination** [T17 pp.567–570]:

- **Emax ceiling (Eq.17-19~17-21)**: 두 full agonist가 같은 receptor에 작용하면 $E_{max}$가 단일 agonist 단독 사용 시와 같은 ceiling에 도달. 즉 두 약물의 "더하기"가 effect를 두 배로 만들지 않는다.

$$E=\frac{E_{max}\cdot(C_A/EC_{50,A}+C_B/EC_{50,B})}{1+C_A/EC_{50,A}+C_B/EC_{50,B}}$$  
[T17 Eq.17-21; source tag: T17 p.568]

- **Isobologram (Fig.17-21)**: 일정 effect level (예: 50% Emax)을 만드는 두 약물 농도 조합을 (x, y) plane에 그렸을 때:
...
- **Correction:** reversible inhibition은 inhibitor concentration이 줄면 빠르게 회복될 수 있지만, MBI는 enzyme pool recovery가 필요하다. Clarithromycin-like MBI는 회복 시간이 enzyme resynthesis에 묶인다 (CYP3A4의 경우 ~2주). [T17 pp.557–558]
- **Test:** single-dose vs multiple-dose perpetrator exposure에서 affected drug PK 변화가 질적으로 달라지는지 본다. $k_{inact}/k_E$ ratio가 클수록 reversible 식과 결과가 크게 다르다.
...
- **Correction:** chronic rifampin induction은 exposure를 낮출 수 있지만, acute rifampin은 transporter inhibition으로 exposure를 올릴 수 있다 (Atorvastatin-Rifampin acute IV에서 Cmax 10×, AUC 7×). Timing과 route를 분리해야 한다. [T17 pp.560–565]
...
- **Correction:** PD interaction은 concentration-time profile이 크게 변하지 않아도 response가 additive, synergistic, antagonistic하게 변할 수 있다. 경쟁적 길항(Eq.3:49)은 EC50을 $(1+A/EA_{50})$로 shift하고, 비경쟁적 길항(Eq.3:50)은 Emax를 $(1+A/EA_{50})^{-1}$로 감소시키며, 같은 receptor의 두 full agonist는 Emax ceiling을 공유한다. [G pp.224–227], [T17 pp.567–570]
- **Test:** unbound concentration-response relationship을 같이 확인하고, 같은 효과 level을 isobologram에 표시해 additivity 가정을 점검한다.
...
**Answer:** $C_{u,ss}=K_mR_0/(V_m-R_0)$에서 dosing rate $R_0$가 $V_m\approx 500$ mg/day에 가까워질수록 denominator가 작아져 concentration이 비선형적으로 증가한다. typical $K_m\approx 0.4$ mg/L (unbound 기준), $f_u\approx 0.1$이므로 total $K_m'\approx 4$ mg/L; 치료역 10–20 mg/L는 이미 capacity 근처다. [T16 pp.491, 503]

## Q5. [적용]
<!-- SELF-TEST -->
Ascorbic acid는 133-fold dose increase에도 concentration이 약 2-fold만 증가했다. phenytoin과 반대 방향의 비선형성이 생긴 이유는?  
**Answer:** 농도 상승과 함께 renal clearance가 saturable reabsorption escape valve로 인해 증가하기 때문이다 (CLR < 0.5 mL/min @ 9 mg/L → 21 mL/min @ 19 mg/L, >42-fold 증가). saturable reabsorption은 high-concentration에서 약물 제거를 가속하는 역방향 비선형성을 만든다. [T16 p.492], [T16 pp.507–510]

## Q6. [회상]
<!-- SELF-TEST -->
MM model에서 $V_{max}$와 $K_m$를 따로 추정하려면 어떤 concentration range가 필요한가? "Vmax–Km Ridge Lock"이란 무엇인가?  
**Answer:** curvature를 포함하고, 특히 $K_m$ 주변 또는 그 이하의 concentration이 필요하다. high concentration plateau만으로는 $V_{max}$/$K_m$ ridge가 생긴다 — single-dose data에서 흔히 $\rho(V_{max}, K_m) > 0.9$ (Gabrielsson p.116에서 $-0.96$ 사례). 이를 "Vmax–Km Ridge Lock"이라 부른다. rescue: dose level 추가 또는 $K_m$ prior fix. [G p.116, p.117]

## Q7. [적용]
<!-- SELF-TEST -->
반복투여 중 trough가 계속 낮아지고 single-dose model은 잘 맞는다. 어떤 mechanism을 먼저 의심해야 하는가? 이를 명명한 진단 시그너처로 부른다면?  
**Answer:** autoinduction 또는 time-dependent increase in clearance — "Trough Envelope Drift" + "Single-dose Fit Trap"의 결합. enzyme turnover compartment와 $k_{out}$ 식별 가능성을 확인한다. PK22 design에서 $k_{out}\approx 0.023$ h⁻¹, 즉 enzyme $t_{1/2}\approx 30$ h가 식별된다. [G pp.126–129], [G pp.580–585]

## Q8. [적용]
<!-- SELF-TEST -->
Clarithromycin 병용 후 midazolam exposure가 multiple dosing에서 더 커지고 회복도 느리다. reversible inhibition과 어떻게 정량적으로 다른가?  
**Answer:** mechanism-based inhibition은 enzyme activity를 제거하므로, 회복은 inhibitor 농도 소실뿐 아니라 enzyme resynthesis/degradation clock에 묶인다. clarithromycin 정량값으로: $k_{inact}\approx 0.07$ min⁻¹, $k_E\approx 0.23$ day⁻¹, $k_{inact}/k_E\approx 438$, $C_{u,I}/K_I\approx 0.025$ → 같은 inhibitor exposure로 reversible 식은 1.025-fold CL 감소를 예측하지만 MBI 식은 **약 11-fold** CL 감소를 예측한다. CYP3A4 회복은 ~2주 필요. [T17 pp.557–558]

## Q9. [적용]
<!-- SELF-TEST -->
Valproate 병용 후 total phenytoin은 낮아졌지만 unbound phenytoin은 유지된다. dose를 줄여야 하는가?  
**Answer:** displacement-only에서는 total concentration이 misleading할 수 있다. unbound concentration과 clinical response를 기준으로 판단해야 하며, total 감소만으로 dose 감량하면 안 된다. open in vivo system mass balance ($C_u = K^0/CL_u$)에서는 unbound clearance가 변하지 않는 한 unbound steady state가 유지되기 때문이다. [T17 p.543], [G §2.7.5]

## Q10. [계산 직관]
<!-- SELF-TEST -->
Affected drug의 $f_m$가 0.8이고 inhibitor가 해당 pathway를 강하게 억제한다. 왜 $f_m$가 0.3인 drug보다 AUC ratio가 훨씬 커지는가?  
**Answer:** Eq.17-11에서 inhibited pathway term $f_m/(1+C_{u,I}/K_I)$가 작아져도 residual $(1-f_m)$가 exposure ratio의 바닥을 정한다. $f_m=0.8$이면 남은 pathway가 0.2뿐이라 ratio가 크게 뛴다. 반대로 $f_m=0.3$이면 0.7의 우회 경로가 있어 ratio 상승이 제한된다. [T17 pp.550–552]

## Q11. [적용]
<!-- SELF-TEST -->
Oral midazolam interaction이 IV midazolam interaction보다 크다. 어떤 mechanism을 시사하는가? Fluconazole–midazolam 정량 anchor를 인용하라.  
**Answer:** gut wall/first-pass metabolism component가 크다는 뜻이다. fluconazole 400 mg 병용 시 oral midazolam AUC는 5.6× 증가, IV는 2× 증가; F 24%→63%; $F_H$ 0.42→0.72; $F_G$ 0.57→0.88로 $F_G$가 더 큰 비율로 변한다 → midazolam의 oral first-pass에 gut wall 기여가 상당함을 의미. [T17 pp.553–554, Fig.17-10]

## Q12. [회상]
<!-- SELF-TEST -->
Eq.17-18의 PM × inhibitor risk가 큰 이유는?  
**Answer:** polymorphic pathway가 이미 결손된 PM에서 남은 nonpolymorphic pathway가 inhibitor에 의해 막히면 residual clearance가 매우 작아질 수 있기 때문이다. Maximum exposure ratio = $1/(1-f_m^{POLY}-f_m^{NP})$이므로 두 항의 합이 1에 가까울수록 ratio가 발산. [T17 pp.558–559]

## Q13. [적용]
<!-- SELF-TEST -->
Rifampin 병용에서 acute IV study는 AUC 증가, chronic oral 병용은 AUC 감소를 보일 수 있다. 왜 모순이 아닌가?  
**Answer:** acute rifampin은 OATP1B1 같은 transporter inhibition으로 hepatic uptake를 차단해 혈중 누적; chronic rifampin은 enzyme induction으로 CL을 올려 노출을 감소. timing과 route가 mechanism을 바꾼다. Atorvastatin-rifampin acute IV: Cmax 10×, AUC 7×, $t_{1/2}$ 8→3h가 직접 anchor (transporter signature: AUC↑ + $t_{1/2}$↓). [T17 pp.560–565, Fig.17-19]

## Q14. [적용]
<!-- SELF-TEST -->
Response는 크게 변했지만 unbound concentration-time profile은 거의 변하지 않았다. 무엇을 의심해야 하는가? 경쟁적 길항과 비경쟁적 길항을 수식 한 줄로 구분하라.  
**Answer:** PK DDI보다 PD interaction을 의심한다. 경쟁적 길항(Eq.3:49)은 $E=E_{max}C/[C+EC_{50}(1+A/EA_{50})]$ — EC50을 shift하지만 Emax 보존. 비경쟁적 길항(Eq.3:50)은 $E=E_{max}/(1+A/EA_{50}) \cdot C/(C+EC_{50})$ — Emax를 감소. 같은 receptor 두 full agonist는 Eq.17-21로 Emax ceiling 공유. [G pp.225–226], [T17 pp.567–570]
...
**Answer:** Gabrielsson p.137 명시: "metabolite 단독 투여 데이터 없이는 metabolite의 disposition kinetics에 대한 추가 extrapolation은 안 된다." parent와 metabolite가 같은 MM enzyme을 공유하면 metabolite의 prolonged tail이 자체 distribution kinetics 때문인지 parent의 saturable elimination이 metabolite 형성 속도를 제한하기 때문인지 식별되지 않는다. → 임상적으로 active metabolite의 toxicity/efficacy 기여가 큰 약물(carbamazepine-10,11-epoxide, codeine-morphine 영역)에서는 metabolite 단독 투여 disposition study가 NDA 패키지의 식별성 보강 단계로 요구된다. [G pp.135–139]

## Q16. [회상] (ver2 신설)
<!-- SELF-TEST -->
R&T Table 17-5의 inhibitor strength 분류 기준을 답하라. Strong/Moderate/Weak는 substrate AUC 변화로 어떻게 구분되는가? MBI는 별도 표시 이유는?  
**Answer:** Strong = ≥5-fold AUC increase 또는 >80% CL 감소; Moderate = 2- to 5-fold AUC; Weak = 1.25- to 2-fold AUC. MBI inhibitor는 Reversible 식만으로 분류하면 강도를 과소평가하므로 별도 표시(MBI marker) — 같은 $C_{u,I}/K_I$에서도 reversible 1.025× vs MBI 11× CL 감소처럼 결과가 자릿수 단위로 다르기 때문. 이 분류는 textbook reference이며 FDA Guidance와 직접 동일시하지 않는다. [T17 p.549]
...
3. **Time-dependence는 enzyme/transporter/physiology의 clock이 drug concentration clock과 분리될 때 생긴다; master clock = $\max(t_{1/2}^{enzyme}, t_{1/2}^{inducer}, t_{1/2}^{drug})$.** [G pp.119–129], [T16 pp.516–519], [T17 pp.560–561]
4. **Binding은 total concentration을 속이고, open system에서는 $C_u=K^0/CL_u$ mass balance가 unbound exposure를 강제한다.** [G pp.129–134], [T17 pp.538–544]
5. **DDI prediction은 $f_m$, inhibitor strength ($C_{u,I}/K_I$, $k_{inact}/k_E$), route, PM/residual pathway, induction/MBI recovery를 함께 본다.** [T17 pp.546–561]
6. **Multifaceted DDI와 PD interaction은 "AUC 하나"로 mechanism을 단정하지 못하게 만든다 — Cmax, $t_{1/2}$, V/F, oral/IV ratio, isobologram을 함께 본다.** [T17 pp.563–570]
7. **Mass action equivalence triangle: Eq.2:266 ↔ Eq.16-7 ↔ Eq.17-11 — 비선형 PK·TDM·DDI 직관은 같은 분모 구조의 외화다.** [G p.115], [T16 p.503], [T17 p.550]
...
- **TDM:** phenytoin-like capacity limitation에서는 small dose change가 큰 concentration change를 만들고 (Vm 20%↓ → Css 2× 변화), total-only monitoring은 displacement/binding 상황에서 misleading하다. plateau 도달 시간이 capacity 근접에서 발산한다는 점도 모니터링 간격 결정에 직결. [T16 pp.491–506], [T17 p.543]
- **DDI study interpretation:** reversible inhibition, MBI ($k_{inact}/k_E$), induction (master clock 위계), transporter effect (acute vs chronic), route effect를 분리하지 못하면 initiation/withdrawal recommendation이 잘못된다. [T17 pp.546–565]
- **Drug–metabolite modeling:** parent-metabolite 동시 관측만으로 metabolite disposition이 충분히 식별된다고 가정하지 않는다. metabolite 단독 disposition study가 식별성 보강의 표준 단계. 이 항목은 source-backed identifiability caution으로만 유지하며, regulatory deficiency language는 쓰지 않는다. [G pp.135–139]
- **PD interaction modeling:** competitive (Eq.3:49 EC50 shift), noncompetitive (Eq.3:50 Emax 감소), 같은 receptor의 full agonist combination (Eq.17-21 ceiling), isobologram의 additivity 가정 점검 — PK 변화 없이 response가 변하는 경우 PD interaction이 1차 후보. [G pp.224–227], [T17 pp.567–570]
...
### B10. Micro-Patch Log (ver2)

ver1 Phase 4D 결과는 PART A 본문을 변경하지 않은 상태였으므로 micro-patch 0건이었다. ver2에서는 다음 surgical 추가만 수행하였고, 본문의 기존 문장은 변경하지 않았다. 모든 추가는 (a) 새 Annotation/Subsection/Equation 블록 형태로 기존 문장 사이에 삽입되었거나, (b) 이미 존재하는 anchor list/answer를 추가 정량값으로 확장한 형태이며, ver1의 모든 기존 문장과 figure marker 위치는 보존되었다.
...
### B11. Mastery Augmentation Log (ver1 + ver2 통합)

ver1에서 인정된 6개 augmentation에 ver2의 정량 anchor 회수 + 명명 진단 시그너처 + mass action equivalence triangle 추가 augmentation 8개를 통합 기록.
...
## ver2 Final Phase 4D Re-certification Statement

위 ver2 surgical augmentation 14개 항목은 모두:

1. **Source-boundary 위반 없음** — TEXTBOOK_DERIVED는 Audit T1 MATCH 분류 source page 인용으로 검증됨; CRUCIBLE_DERIVED는 (a) Audit MATCH 사실의 압축 통찰 또는 (b) 세 textbook 수식의 분모 구조 비교라는 interpretive synthesis로 명시 label됨.
2. **Splice Verification Table B8 변경 없음** — figure marker 위치는 ver1과 동일하므로 anchor·match count·inserted 모두 ver1과 동일. ver2 보강은 figure marker 사이의 본문에만 surgical하게 삽입됨.
3. **Regression guardrails (B5/B6/B7) 강화** — ver2가 회수한 정량값에 대한 추가 ver2-specific guardrail이 B5에 명시됨. Mass action equivalence triangle은 B6에서 CRUCIBLE_DERIVED label 보존 의무가 명시됨.
4. **Zero-Omission Coverage Matrix B9 업그레이드** — ver1의 PRESENT_COMPRESSED 항목 대부분이 PRESENT (ver2)로 전환됨. 학습자가 PART A만 읽고도 Audit T1 MATCH list의 verbatim anchor에 즉시 도달 가능한 상태.
5. **Mastery-Uplift 강화 — 거장 시각의 통합 회로 형성:** §1.2 거장의 30초 진단 시그널 → §2 카드 내부의 정식 명명 진단 시그너처 (Vmax-Km Ridge Lock, Trough Envelope Drift, Single-dose Fit Trap, Washout Trap) → §8.1 #7 mass action equivalence triangle로 통찰이 3중 회수되어, "수식 외움"이 아닌 "mechanism mapping 어휘"로 학습 목표가 끌어올려졌음.
