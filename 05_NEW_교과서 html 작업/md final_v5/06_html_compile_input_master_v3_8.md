...
Turbojoint® PK41 사례에서는 한 지원자에게 **310 / 520 / 780 µg·kg⁻¹**을 각각 5시간 IV 주입했더니, 투여량이 늘수록 **NCA-CL이 1.6 → 0.9 L·h⁻¹·kg⁻¹로 줄었습니다.** 같은 표 안에서 V는 2.3 → 1.8 L·kg⁻¹, MRT는 1.4 → 1.9 h로 움직였습니다. 이 장면이 알려주는 건 하나입니다 — NCA 표는 "최종 답"이 아니라, $V_{max}$·$K_m$·V를 갖는 Michaelis-Menten 회귀로 넘어가기 위한 **출발점**입니다. [G pp.661–664]
...
NCA는 흔히 "model-free"라고 부르지만, 더 정확하게는 **"compartment-free"**입니다. 특정 구획 모델을 안 세워도 되지만, **채혈 구획에서 농도에 비례해 빠진다는 가정(선형 소실)은 여전히 필요합니다.** 그래서 PK41처럼 용량을 올릴수록 NCA-CL이 떨어지는 신호가 보이면, NCA 결과는 최종 해석값이 아니라 **비선형 회귀의 초기 추정값(initial parameter anchor)**으로 내려와야 합니다. [G p.141; G pp.661–664]
...
- **AUC(영차 모멘트 면적, zero moment area)**: 농도-시간 곡선 아래 면적. [G p.142]
...
R&T Table A-1의 일반 경구(generic oral) 50 mg 예시는 이 계산으로 **총 AUC = 26.60 mg·hr/L**을 얻습니다. ⚠️ 이 예시는 **zileuton이 아닙니다**. Zileuton은 R&T Table A-2의 600 mg 경구 자료셋입니다 — 두 표를 헷갈리지 않도록 주의합니다. [T pp.759–762]

> 💡 **사다리꼴은 면적계가 아니라 감사 체인** — 선형 사다리꼴 값은 농도와 간격의 곱일 뿐이라, 숫자가 깔끔해 보여도 말단 꼬리가 틀리면 전체 AUC 해석이 틀립니다. 그리고 선형 방식은 **상승 구간에서 과소추정, 하강 구간에서 과대추정**합니다. 이 편향은 표본추출 간격이 반감기에 비해 클수록 커집니다. [G p.142]
...
적용 조건은 **하강 구간(descending data)**입니다. $C_i=0$ 또는 $C_{i+1}=C_i$이면 로그-선형은 정의되지 않으므로 그 구간만 선형으로 대체(linear fallback)합니다. [G p.144]

**혼합 방식(Mixed Approach)**: 증가/동일 구간은 선형, 감소 구간은 로그-선형으로 처리할 수 있습니다. 단 "항상 표준"이라고 단언하지는 않습니다 — 합리적으로 설계된 sampling에서는 두 방법의 차이가 임상적으로 작다는 저자의 균형도 같이 기억하면 됩니다. [G p.146]
...
관측 $C_{last}$가 말단 회귀선에서 벗어나면, 관측값 대신 회귀 기반 예측값 $\hat C_{last}$를 쓰는 게 일반적으로 권고됩니다. [G p.147]
...
저자 권고는 외삽 면적이 일반적으로 **총 AUC의 20–25%를 넘지 않는 것**입니다. 이를 넘으면 예비 추정값(preliminary estimate) 성격으로 보고합니다. [G p.148]

### C. $\lambda_z$ 선정과 맥락 보충

말단 기울기는 반로그 도표에서 개별 프로파일을 보고 정합니다. 최소 3–4개 관측값이 필요하고, 이상적으로는 말단기에서 **3–4 반감기**가 경과해야 합니다. [G p.146]
...
> **NCA-AUC 원자 노트(atom)**: AUC는 사다리꼴 합이 아니라 **"사다리꼴 합 + $\lambda_z$ 기반 꼬리"**입니다. 그래서 AUC 검토의 핵심은 값 자체가 아니라 **말단 기울기 선택, $\hat C_{last}$ 사용 여부, 외삽 면적 비율**입니다. [G pp.143–148]
...
- **AUMC**: $t\cdot C(t)$ 대 시간 곡선 아래 면적 — 즉 **일차 모멘트 면적**. [G p.142]
- **MRT**: 몸 또는 표본추출계 안에서 분자가 머무는 **평균 시간**. [T p.789]

R&T Appendix H는 분자량 300 g/mol인 약물 1 mg 안에 약 $2\times10^{18}$개의 분자가 있다는 직관에서 시작해, **각 분자의 체류시간을 평균낸 값**을 MRT로 정의합니다. [T p.789]
...
R&T Table H-1의 수치 예시는 $AUC=44.2$ mg·hr/L, $AUMC=177$ mg·hr²/L, 혈장 MRT $\approx 4.0$ hr, 소변 MRT $\approx 4.25$ hr, simulated MRT $\approx 4.3$ hr로 **세 경로의 값이 서로 일관**됩니다. [T p.792]

### C. 투여시간 보정(Input-Time Correction)

관측 MRT에는 약물이 전신순환에 들어오기까지 걸리는 **입력 시간**이 섞여 있습니다. 이대로 두면 투여 과정이 만든 지연을 진짜 체류시간으로 착각하게 됩니다. 그래서 평균 입력 시간(MIT, mean input time; 투여가 만든 평균 지연시간)을 빼야 합니다. [G pp.149–151; T p.793]
...
경구 흡수에서 $K_a$는 단순 흡수만이 아니라 분해(degradation) 같은 병렬 과정을 포함할 수 있습니다. 그래서 $1/K_a$를 "순수 흡수 시간"으로 과해석하지 않습니다. [G p.150]

### D. 맥락 확장과 경계 조건

대사체나 하류 구획에서는 $MBRT$에서 상류 입력 체류시간을 빼는 방식으로 $MDRT$를 정의합니다. 핵심은 체류시간이 **"공간별 고유 시간 + 입력 시간"으로 누적된다**는 점입니다. [G pp.155–156]

R&T는 소변 배설 자료에서도 MRT를 계산할 수 있음을 보입니다. 다만 이 세션의 주축은 혈장 MRT입니다 — "MRT는 혈장에서만 정의된다"는 오해를 막기 위해 이 한 줄을 둡니다. [T pp.790–792]
...
> **NCA-MRT 원자 노트(atom)**: MRT는 반감기가 아닙니다. MRT는 **"분자가 계 안에서 보낸 시간의 평균"**이고, 계산상으로는 $AUMC/AUC$이며, 투여 경로에 따라 MIT를 빼야 해석 가능한 체류시간이 됩니다. [G pp.149–151; T pp.789–793]
...
반감기가 입력 시간에 비해 짧으면 표준 MRT/$V_{ss}$ 계산이 불안정해질 수 있습니다. 여기서 핵심은 표준식이 **"틀렸다"**는 뜻이 아닙니다. 입력 기간이 체류시간 계산을 지배할 만큼 커지면서 **표준식이 유효하게 적용될 수 있는 영역을 벗어났다**는 뜻입니다. Gabrielsson은 이 경우 정상상태 이후 washout AUC를 이용한 대안식이 더 견고하고, **0이나 음수 $V_{ss}$를 피한다**고 설명합니다. [G pp.157–158]
...
> 💼 **실무 인사이트:** 표준 $V_{ss}$ 식(Eq.2:337)과 §2.8.8 대안식(Eq.2:366)을 함께 계산해 **두 값이 잘 맞으면** 정상 적용 영역에 가깝습니다. 크게 벌어지면 입력 시간과 반감기의 비율 자체가 문제라는 신호입니다. [G pp.151, 157]
...
| 비선형 소실 | CL과 $t_{1/2}$가 용량 의존적으로 변함 | → | 1차 파라미터 해석 실패 | capacity-limited model로 전환 [G p.664] |
| NCA-IIV | 잔차/오차 기여까지 IIV에 섞임 | → | PopPK $\omega^2$ 초기값 과대 | NCA 분산을 상한으로 보고 보수적 초기값 설정 |

> **NCA-volume 원자 노트(atom)**: $V_{ss}$는 "분포 부피 하나"가 아니라 **$AUC$, $AUMC$, CL, 투여 경로 보정이 모두 통과한 최종 요약값**입니다. 그래서 $V_{ss}$를 볼 때는 항상 "**어떤 MRT를 썼는가? 입력 시간을 뺐는가?**"를 같이 봐야 합니다. [G pp.149–157]
...
§2.9의 핵심은 "투여량(dose)"보다 **전신 노출(systemic exposure)**, 특히 **총 또는 비결합 전신 혈장 농도**를 보라는 것입니다. C1–C3가 노출을 계산해 파라미터로 변환하는 카드라면, C4는 그 노출 중 무엇을 PD/safety와 연결할지 고르는 카드입니다. 같은 투여량이라도 투여 경로, 초회통과, 비선형 소실, 활성 대사체, 단백 결합, 투여 방식에 따라 **실제 표적 노출이 달라집니다.** [G pp.158–164]

| 지표(Metric) | 의미 | 사용 직관 | 출처 |
|---|---|---|---|
| $C_{max}$ | 최고 노출(peak exposure) | 급성 독성, peak-driven effect | [G p.163] |
| AUC | 총 노출(total exposure) | 누적 노출, extent 비교 | [G p.163] |
| $t_d$ | 역치 이상 지속시간 | time-above-threshold 유형 판단 | [G p.163] |
| $C_{ss}$ / 평균 농도 | 정상상태 강도 | 만성 투여 노출 | [G p.163] |
| 비결합 농도 $C_u$ | 약리학적으로 사용 가능한 분율 | 단백 결합 차이 비교 | [G p.163] |
...
> 💡 **노출 지표는 endpoint의 언어입니다** — 독성이 peak-driven이면 $C_{max}$, 누적 부담이면 AUC, threshold라면 $t_d$가 먼저 질문에 답합니다. 그리고 **같은 총 농도라도 단백 결합이 다르면 비결합 농도 기준 효능 순위가 뒤집힐 수 있습니다.** [G p.163]
...
| 사례 | 핵심 교훈 | 출처 |
|---|---|---|
| 투여 경로/생체이용률 | 용량-반응이 투여 경로 차이로 흔들려도 농도-반응은 정렬될 수 있습니다. | [G p.159] |
| 비선형 소실 | 투여량 증가는 노출과 안전역을 **비선형적으로** 바꿉니다. | [G p.159] |
| 활성 대사체 | 모약물 농도만 보면 투여 경로 의존 반응을 오해할 수 있습니다. | [G p.160] |
| 투여 방식 | 같은 일일 투여량이라도 주입/섭식 패턴이 독성을 바꿀 수 있습니다. | [G p.160] |
| 점유율/바이오마커 | 리간드 농도, 수용체 점유율, 바이오마커 반응은 **같은 축이 아닙니다.** | [G p.161] |
| 반복 섭식 | 단회 투여 + 섭식 시뮬레이션이 반복급여 노출을 과대예측할 수 있습니다. | [G p.162] |
| U자형 반응 | 노출-반응이 단조가 아니면 투여량만으로 효과를 예측할 수 없습니다. | [G p.162] |
| 단백 결합 | 총 농도와 비결합 농도는 효능 순위를 **뒤집을 수** 있습니다. | [G p.163] |

### C. PK41 동적 출처 앵커(Dynamic Source Anchor) — NCA에서 비선형 회귀로

Turbojoint® PK41은 **한 지원자에게 310, 520, 780 µg·kg⁻¹을 각각 5시간 IV 주입**한 사례입니다. NCA에서 투여량이 늘수록 CL은 1.6 → 0.9 L·h⁻¹·kg⁻¹로 줄었고, V는 2.3 → 1.8 L·kg⁻¹, MRT는 1.4 → 1.9 h로 변했습니다. [G pp.661–662]
...
NCA 결과는 초기 추정값으로 $V\approx 2$ L/kg, $K_m\approx 150$ µg/L, $V_{max}\approx 310$ µg·h⁻¹·kg⁻¹ 등을 제공합니다. **최종 추정값은 $V_{max}=184$ µg·h⁻¹·kg⁻¹, $K_m=83$ µg/L, $V=1.8$ L/kg**이었습니다. 저자의 결론은 분명합니다 — 용량 제한 1구획 시스템의 파라미터는 CL과 V가 아니라 $V_{max}$, $K_m$, V이며, **CL과 $t_{1/2}$는 혼합 0차/1차 동력학과 양립할 수 없습니다(incompatible).** [G pp.663–664]

### D. 원자 노트와 그림 참조

> **노출 원자 노트(Exposure atom)**: 투여량은 **투입량**이고, 노출은 **몸이나 표적이 실제로 본 농도-시간 부담**입니다. §2.9의 모든 사례는 한 문장으로 수렴합니다 — **"투여량이 같아도 노출이 다르면 반응이 달라진다."** [G pp.158–164]
...
**⚡ 기억 고리 (Memory Hook)**: AUC 꼬리는 $\lambda_z$의 1차 항 하나뿐이지만, AUMC 꼬리는 **$1/\lambda_z$와 $1/\lambda_z^2$ 두 항이 동시에** 들어갑니다. 시간이 한 번 더 가중되는 순간 후방 꼬리가 지배적이 됩니다 — 그래서 같은 말단 기울기 오류라도 **MRT와 $V_{ss}$로 흘러갈 때 더 강하게 증폭**됩니다. [G pp.144–145]

**핵심 구분**: AUMC는 늦은 농도를 시간으로 다시 가중하므로, 같은 말단 기울기 오류라도 MRT와 $V_{ss}$에 더 민감하게 전달됩니다. [G pp.142–148]
...
**⚡ 기억 고리 (Memory Hook)**: MRT는 **분자가 머문 시간의 평균**, $t_{1/2}$는 **농도가 절반이 되는 시간**입니다. 1구획 단일지수(monoexponential)에서는 $MRT=1/\lambda_z$, $t_{1/2}=\ln(2)/\lambda_z$로 단순한 비율 관계($1/\ln 2\approx 1.443$)지만, 다구획에서는 **MRT가 분포기 정보까지 흡수하는 반면 $t_{1/2}$는 말단 기울기만 기술**합니다. 그래서 유효 반감기는 $\ln(2)\cdot MRT$로 다시 정의됩니다. [G p.151; T pp.789–793]

**핵심 구분**: MRT는 분포와 제거를 통합한 평균 시간이고, $t_{1/2}$는 말단 기울기의 시간 척도입니다. 1구획 단일지수 상황에서는 가까워질 수 있지만 같은 개념은 아닙니다. [G p.151; T pp.789–793]
...
**⚡ 기억 고리 (Memory Hook)**: $V_{ss}$의 분모에는 $AUMC$와 투여 경로 보정이 둘 다 들어오고, $V_z$의 분모에는 $AUC$와 $\lambda_z$만 들어갑니다. 그래서 **두 부피가 어긋나는 방향 자체가 어디서 가정이 흔들렸는지 알려주는 단서**가 됩니다 — 표준 $V_{ss}$만 비정상적으로 부풀거나 음수에 가까워지면 **투여 경로 보정 영역**의 문제, $V_z$만 부풀면 **말단 기울기 영역**의 문제입니다. [G pp.151, 157]
...
**정답**: 선형 규칙은 $\sum [(C_i+C_{i+1})/2]\Delta t$이며, 상승·평탄·하강 구간 모두 계산할 수 있지만 하강 구간에서 과대추정합니다. 로그-선형 규칙은 $(C_i-C_{i+1})/\ln(C_i/C_{i+1})\cdot\Delta t$이며, 하강 자료에서만 유효합니다. $C_i=0$ 또는 $C_{i+1}=C_i$에서는 선형 대체가 필요합니다. [G pp.143–145]

## Q2. $\lambda_z$ 추정의 최소 조건과 이상 조건은 무엇인가요?

**정답**: 최소 3–4개 말단 관측값이 필요하고, 이상적으로는 말단기에서 3–4 반감기가 경과해야 합니다. 관측 마지막 농도가 말단 회귀선에서 벗어나면 $\hat C_{last}$를 검토합니다. [G pp.146–147]

## Q3. 외삽 AUC가 총 AUC의 30%라면 어떻게 보고해야 하나요?

**정답**: Gabrielsson의 일반 권고 20–25%를 넘으므로, 최종 신뢰 파라미터처럼 쓰기보다 **예비 추정값** 성격을 명시합니다. 추가 표본추출 또는 말단기 재검토가 필요합니다. [G p.148]

## Q4. PK41에서 NCA 결과가 비선형 회귀의 초기 추정값으로 전환되는 흐름을 설명해 보세요.

**정답**: Turbojoint®에서 310/520/780 µg·kg⁻¹ 5h IV 주입 후 투여량 증가에 따라 NCA-CL이 1.6 → 0.9 L·h⁻¹·kg⁻¹로 감소했습니다. 이는 용량 제한 소실 신호이며, NCA는 최종 CL 보고가 아니라 $V_{max}$, $K_m$, V를 갖는 Michaelis-Menten 회귀의 **초기 추정값 앵커**가 됩니다. 최종 추정값은 $V_{max}=184$ µg·h⁻¹·kg⁻¹, $K_m=83$ µg/L, $V=1.8$ L/kg이었습니다. [G pp.661–664]

## Q5. R&T Table H-1에서 $AUC=44.2$, $AUMC=177$이면 혈장 MRT는 얼마인가요?

**정답**: $MRT=AUMC/AUC=177/44.2\approx 4.0$ hr입니다. 같은 예시에서 소변 MRT $\approx 4.25$ hr, simulated MRT $\approx 4.3$ hr로 일관됩니다. [T p.792]

## Q6. IV bolus에서 CL은 어떻게 계산하며, 경구 투여에서는 무엇이 달라지나요?

**정답**: IV bolus/IV 투여에서는 $CL=D_{iv}/AUC_0^\infty$입니다. 경구에서는 생체이용률(F)이 섞이므로 $CL/F=D_{po}/AUC_0^\infty$로 **겉보기 청소율(apparent clearance)**을 계산합니다. [G p.149]

## Q7. 급성 독성, 누적 노출, 역치 기반 효과에는 각각 어떤 노출 지표를 우선 볼까요?

**정답**: 급성 독성은 $C_{max}$, 누적 노출은 AUC, 역치 기반 효과는 $t_d$ 또는 역치 초과 AUC를 우선 봅니다. 실제 선택은 약물 기전과 반응 시간경과에 맞춰 정당화해야 합니다. [G p.163]

## Q8. 총 농도 기준 효능 순위와 비결합 농도 기준 순위가 달라질 수 있는 이유는 무엇인가요?

**정답**: 단백 결합이 다르면 총 농도가 같아도 **약리학적으로 사용 가능한 비결합 농도**가 달라집니다. §2.9의 메시지는 총 농도보다 **비결합 전신 노출**이 더 직접적인 비교 기준이 될 수 있다는 것입니다. [G p.163]

## Q9. 보스 딜레마(Boss Dilemma): 회사 SOP는 표준 Eq.2:337만 허용하지만, 반감기가 입력 시간에 비해 매우 짧아 §2.8.8 대안식이 더 타당해 보입니다. 어떻게 처리해야 하나요?

**정답**: 표준식과 §2.8.8 대안식을 **둘 다 계산해 차이를 제시**하고, 차이의 원인이 $T_{inf}/t_{1/2}$ 비율에 있음을 설명합니다. SOP deviation은 "더 예쁜 결과"가 아니라 **0 또는 음수 $V_{ss}$를 피하는 출처 기반 수학적 정당화**로 문서화합니다. [G pp.151, 157]
...
NCA는 PopPK를 **대체**하는 분석이 아니라 **모델링 전후에 활용하는 도구**입니다. 모델 선택 전에는 EDA와 초기 추정값을 만들고, 모델 추정 후에는 population prediction이 관측 노출의 규모와 일관되는지 확인하는 기준점이 됩니다. 다만 NCA는 선형 소실을 가정합니다 — 이 점을 잊으면 **"단순해서 강한 방법"이 "단순해서 틀린 방법"**으로 바뀝니다. [G p.141]

## B. 추적해야 할 실패 모드

| 변화 상황 | 1차 파라미터 | → | 임상 결과 | 조치 |
|---|---|---|---|---|
| 말단 기울기 오염 | $\lambda_z$ | → | AUC 꼬리, CL, $V_z$ 동시 왜곡 | selection plot, %extrapolated, $C_{last}$ 처리 기준 확인 [G pp.146–148] |
| 투여 경로 보정 누락 | MIT, MRT | → | MRT와 $V_{ss}$가 실제 체류/분포보다 길거나 짧게 해석 | infusion/oral input time 보정 [G pp.149–151; T p.793] |
