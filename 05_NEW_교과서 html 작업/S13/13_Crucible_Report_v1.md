...
---
먼저 첨부된 Step 1 Draft v0와 Audit Report v1을 정확히 대조해 작업했습니다. Audit의 MUST_FIX는 고정 제약으로 처리하고, Crucible Report는 그것 위에 P1 + P3 페르소나가 추가하는 가치만 응축했습니다. PDF 직접 인용 범위(R&T Ch.12-13, G&W Ch.4 + PK50)도 다시 교차 확인했습니다.작업 결과의 요지만 짧게 보고 드립니다.

**Crucible Report v1의 핵심 판정**: Step 1 Draft v0는 4-card 골격·PK50 anchor·Boss Dilemma 논리에서 PhD-level deliverable로 우수하나, Audit MUST_FIX 적용 후에도 *내면화 장벽 3개*와 *시스템 통합 지점 3개*가 명시되지 않은 채 남아 있어 Phase 4 진입 전 보강이 필요했습니다.

**가장 결정적인 발견 두 가지**:

첫째, Step 1의 Critical Blow가 **잘못된 자리**에 있습니다. 현재 Pair 1 (ω² vs σ²)에 적용된 Critical Blow는 NDA cost·CFR 주장에 의존하는데, Audit가 이를 NOT_IN_SOURCE로 분류해 무력화했습니다. P3 관점에서 *audit 제약 안에서도 살아남는* 가장 높은 파급력은 **Pair 2 (Total Cl vs Unbound Cl_u)**입니다 — G&W p.705의 "total plasma C < 50 µg/L" safety threshold와 unbound 좌표계 dose individualization이 충돌하면 라벨 reissue 사유가 *교과서 본문 안에서 직접* 도출됩니다. 이것이 A4 권고의 핵심.

둘째, ω²과 σ²은 **고정 데이터셋 안에서 zero-sum 관계**라는 *보존 법칙*의 직관이 Step 1에서 빠져 있습니다. C1과 C2가 별개의 카드로 분리되어 있어 학습자가 "잔차 모델 오설정 → ω² 새기"의 메커니즘을 *외울 수는 있어도 보지 못하는* 구조적 누락입니다. 이것이 A1 권고의 핵심 — 한 줄 삽입으로 cascade 직관을 정초합니다.

**T3 Trench Tips는 4개로 한정**했습니다(요청 범위 3-5 내). 모두 NONMEM 코드 블록 또는 Limitations에 1-2 문장으로 직접 삽입 가능한 형태이며, audit가 [교과서 외 구현 번역]으로 라벨링한 코드 블록 *안에* 자연스럽게 자리 잡습니다.

**T4 Deletion list 8건**은 비협상 — 적용하지 않으면 Step 1 v1의 무게중심이 audit 적용 후에도 외부 지식(Mixed RUV default·shrinkage threshold) 쪽으로 기울어 있는 채로 HTML 컴파일에 들어가게 됩니다.

다음 단계는 Step 1 v0 → v1 머지(Audit MUST_FIX + Crucible Grade A 6개 + T4 deletion 8건 동시 적용)이며, 그 후 Phase 4C(Figure 결정) → Step 2 HTML 순서가 됩니다.
