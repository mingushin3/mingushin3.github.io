...
# 09_HTML Compile Input Master — v3.2

## Version Note

**v3.2 변경 요지 (Korean-Dominant Readability Patch)**: v3.1의 모든 사실 주장·수치·page tag·figure marker·certification 판정·source-boundary는 그대로 유지된다. v3.2는 PART A의 학습자-facing 산문에서 남아있던 영어 산문·헤딩·테이블 헤더·callout 라벨을 한국어 중심으로 전환한 surgical patch이다. 커리어상 중요한 전문용어는 `한글(English)` 형식으로 병기하고, 수식·코드·기호·page tag·figure marker·source label·compiler marker는 일절 변경하지 않았다. PART B는 본 Version Note 추가 외에는 그대로 보존된다.

**v3.1 변경 요지 (Korean Readability Patch)**: v3의 모든 사실 주장·수치·page tag·figure marker·certification 판정·source-boundary는 그대로 유지된다. v3.1은 PART A의 학습자-facing 한국어 산문 가독성만 개선한 surgical patch이다. 적용된 27개 패치(KR-01~KR-27)는 모두 Low risk이며, 영어 산문을 한국어 산문으로 번역하거나(주로 §2 카드의 Five Cognitive Layers·Assumption·Limitation 항목, §5 Pair 3·4 영어 단락, Learner Navigation Aid 영어 안내문), 복합 장문을 가독성을 위해 분리한 것이다. 과학적 의미·수식·NONMEM/R 코드·고유 약어는 변경하지 않았다. PART B는 본 Version Note 추가 외에는 그대로 보존된다.
...
1. MUST 카드 5개를 순서대로 읽는다: M1 turnover → M2 protein/antibody PK → M3 TMDD curve phases → M4 Full TMDD → M5 MM boundary.
2. `FIGURE_POINTER`가 나오면 해당 교과서 그림을 직접 확인한다. 저작권상 이미지를 본 자료에 삽입하지 않는다.
3. §5로 자주 혼동되는 개념 쌍을 정리하고, §7로 능동 회상을 점검하며, §8로 전문가 수준의 통합 판단을 완성한다.
4. §7 Self-Test에서 ★★는 핵심 질문(자동으로 답이 나올 때까지 반복), ★는 중요(오답 시 재점검), ○는 표준 수준이다. 각 답변 뒤에 "다음 깊이 질문"이 이어져 후속 질문 층위를 열어준다.
5. §5에서 **Critical Blow**로 표시된 쌍이 이 세션에서 혼동 시 임상 의사결정에 가장 큰 왜곡을 일으키는 쌍이다.
...
- 1구획·2구획 선형 PK, Michaelis-Menten 표기법, ODE 상태변수(state variable) 개념을 이미 알고 있어야 한다.
- 학습 내내 이 질문을 가지고 다니라: "이 파라미터를 실제로 가르쳐주는 데이터는 무엇인가?"

**학습 완료 후 점검**

- `kin`, `kout`, `kon`, `koff`, `ke(RL)`, `Kd`, `Km`이 왜 서로 대체 불가한지 설명할 수 있어야 한다.
- Full TMDD가 필요한 상황과 MM 근사가 interpolation 도구에 머무는 상황을 데이터 기반으로 구분하고 방어할 수 있어야 한다.
- 고용량 fit 품질이 왜 저용량·저농도 외삽을 자동으로 정당화하지 않는지 설명할 수 있어야 한다.
...
**Mode**: A-Critical  
**Image rights**: None; 그림 작업은 Phase 4C로 유보된다.
...
> Baseline을 데이터 정리 관행이 아닌 모델 가정으로 취급하라. TMDD 작업에서 첫 번째 전문적 질문은 관찰된 baseline이 교란 후 회복할 수 있는 turnover pool처럼 거동하는지 여부다.
...
> 다음에 mAb·protein·hormone 데이터셋을 받으면, 첫 30초 안에 두 가지를 결정하라. 이 endpoint는 Fig.2.77 baseline scenario 중 어느 것에 가장 가까운가? predose DV는 turnover pool의 안정 상태인가, 아니면 oscillating·feedback·Zeitgeber 구조인가? 이 한 질문에 답하지 못한 채로 `R0`/`kout` 추정에 들어가면, 이후 모든 occupancy·recovery 해석이 baseline 가정의 노예가 된다.
...
Protein/antibody PK의 distinctive features는 다음 네 가지 게이트로 압축된다.
...
**G&W protein/antibody data anchors**: G&W는 cynomolgus monkey에 다섯 차례 연속 오름 용량(0.77–771 µmol·kg⁻¹)으로 실험용 항체를 투여한 사례를 제시하며, 단순한 central compartment 가정만으로 항체 배치(disposition)를 설명하는 것의 위험을 경고한다. 또한 정상·신장절제 rat에서의 r-hSOD 실험을 통해 대형 protein의 낮은 `Vd`와 renal clearance 손실을 보여준다. [G pp.99–100]
...
> 보고된 mAb 반감기(half-life) 하나를 그대로 받아들이는 것은, slow input, FcRn salvage, target-mediated clearance, 또는 ADA가 작용할 때 오해를 낳는 mental shortcut이 될 수 있다. 농도 범위와 기전이 명확하지 않은 한, 반감기는 농도 영역에 의존하는 요약값으로 읽어야 한다.
...
**PK27 dose anchor**: PK27의 full-TMDD 시범은 단클론 항체를 1.5, 5, 15, 45 mg·kg⁻¹로 급속 정맥 주사한 실험이다. 이것이 용량별 층화 곡선 판독이 장식이 아닌 핵심인 이유다. [G pp.602–603]
...
| D | `koff`와 `ke(RL)` 주도 말기상(terminal phase) | 매우 낮은 ligand; target-specific 제거 가시화 |

<!-- ANCHOR -->

### B. 시간 순서가 아닌 농도 위계(Concentration Hierarchy)

초심자는 Phase A→B→C→D를 시간 순서로 읽는다. 숙련된 모델러는 같은 곡선을 ligand concentration이 `R0`, `Km`, `Kd`를 차례로 지나는 concentration hierarchy로 읽는다. PK27에서는 `R0≈12 mg·L⁻¹`, `Km≈0.03 mg·L⁻¹`이며, `Kd`는 `koff/kon`으로 정의된다. 이 역치들이 저용량 곡선이 고용량 곡선으로는 보이지 않는 기울기를 드러내는 이유다. [G pp.603–610]

### C. 곡선이 가르치는 것

- 고용량 데이터만 관찰했다면 target-mediated route는 포화·선형으로 보일 수 있다.
- assay sensitivity가 낮아 저용량 데이터가 누락되면 Phase A/D가 보이지 않을 수 있다.
- sc absorption이 느리면 초기 rapid binding이 input kinetics에 가려질 수 있다.
- target·complex 데이터가 없으면 ligand-only fitting이 적합해 보여도 `kon`, `koff`, `ke(RL)`의 정밀도는 낮은 상태로 남는다. [G pp.603–609]

### D. 실무 판독 규칙(Practical Reading Rule)

TMDD plot은 용량별로 층화해서 점검해야 한다. 고용량 profile은 잘 맞지만 저용량 residual이 체계적으로 편향된다면, 모델이 phantom linearity를 가지고 있을 수 있다 — 즉, 중심 profile에서는 겉보기 성공이지만 저농도 외삽에서는 편향이 숨어 있는 상태다. 이것은 PK27 MM failure에서 도출된 진단적 해석이며, 교과서가 직접 제공하는 NONMEM rule이 아니다. [G p.609]

### E. 한계(Limitations)

4개 phase는 풍부하고 고품질의 다용량 IV형 데이터셋에서 가장 명확하게 드러난다. 임상의 희박 샘플링, sc absorption, total vs free assay의 모호성, 또는 membrane-bound target은 phase 경계를 흐릴 수 있다. [G pp.604–605]

### F. 다섯 인지 레이어(Five Cognitive Layers)

| 레이어 | 기억할 것 |
|---|---|
| L1 | TMDD는 매우 낮은 농도와 매우 높은 농도에서는 선형이며, 그 사이 구간에서 비선형이다. |
| L2 | Saturation이 apparent clearance를 바꾼다. |
| L3 | 4개 phase는 지배적인 ODE term에 각각 대응된다. |
...
> **전문가 통찰(Mastery Note) — CRUCIBLE_DERIVED**  
> Phase A–D는 시간 순서보다 농도 영역 지도로 기억해야 한다. 이것이 곡선 모양을 모델 선택 논거로 바꾸는 전환점이다.
...
Full TMDD의 상태변수(state variables)는 다음 4개다. [G pp.604–607; R&T p.711]
...
결합 속도항은 `kon·L·R`이며, `kin·L·R`이 아니다. `kin`은 target 합성에 속하고, `kon`은 ligand-target 결합의 2차 on-rate다. 이 구분은 단순 표기 문제가 아니라, turnover process와 binding process를 분리하는 핵심이다. [G p.604; G pp.606–607]

### C. 식별가능성(Identifiability): 어떤 데이터가 어떤 항을 가르치는가

PK27은 세 데이터셋을 사용한다: I = ligand 단독, II = ligand + target, III = ligand + target + complex. Target 및 complex 시간경과 데이터가 추가될수록 `kon`, `koff`, `ke(RL)`의 정밀도가 개선된다. Table 27.2에서 `kon` CV%는 17→2→1, `koff`는 27→13→3, `ke(RL)`는 27→23→2로 datasets I→II→III에 걸쳐 개선된다. [G pp.603, 608–609]

숙련된 모델러의 해석은 간명하다. ligand 데이터는 nonspecific disposition과 가시적 phase 구조를 가르치고, target 데이터는 target recovery와 saturation 경계를, complex 데이터는 sink 거동을 가르친다. 즉, 데이터의 풍부함은 장식이 아니다 — 어떤 ODE term이 추정 가능해지는지를 결정한다.

### D. 그럴듯한 오해(Plausible Fallacy): 기전 없는 적합 품질

기존 압축 진술은 다음과 같이 유지된다 — reduced model은 가장 높은 세 ligand profile은 상당히 잘 맞출 수 있지만 가장 낮은 profile에서는 실패한다. PK27에서 MM 모델은 `Km=3.7`을 산출했지만 TMDD 모델은 `Km=0.03`을 제시했으며, 이는 123배 과대추정에 해당한다. 이 편향은 trough 예측, target occupancy 추정, subgroup 외삽으로 전파될 수 있다. 정량적 영향은 일반적 주장이 아닌, 약물·환자 집단별 sensitivity analysis로 평가해야 한다. [G p.609]
...
- `Vc=0.05 L·kg⁻¹` 고정은 PK27의 고해상도 rapid-IV/고분자량 근사값이며, 범용 임상 상수가 아니다. [G p.608]
- free target, total target, complex assay는 모델의 state variable과 정렬되어야 한다. assay가 partial total target을 보고하는데 모델이 free `R`을 가정하면 `R0`와 `kout`의 해석이 달라진다.
- Full TMDD는 희박한 ligand 데이터만 존재하면 과모수화(over-parameterization)될 수 있다. PK26에서는 target/complex 및 `kon/koff/kd` 정보가 없어 full TMDD 적합에 실패했다. [G p.601]
...
| L2 | Target turnover와 binding은 별개의 과정이다. |
| L3 | `Kd`와 `Km`은 대체 불가하다. |
...
> Full TMDD에 복잡성을 더하기 전에, 관찰된 species 각각이 어떤 숨겨진 ODE term을 가르치는지 명명하라. ligand만 관찰된 데이터셋에서는 많은 mechanistic parameter가 numerically fitted 상태일 수 있어도, biologically learned 상태라고 볼 수 없다.

> **실패 모드(Failure Mode) — AUDIT_DERIVED**  
> `kin`/`kon` 구분은 구조적 온전성 점검이다. control stream이나 도출 과정에서 binding에 target 합성 표기(`kin`)를 사용하면, 무시할 수 있는 오타가 아니다 — 모델링하는 process 자체가 달라진다.
...
PK26 Efalizumab은 유용한 reduced-model 앵커다. 2구획 모델에 parallel linear/MM 제거가 사용되었는데, target/complex 및 `kon/koff/kd` 정보 없이 full TMDD가 실패했기 때문이다. 보고된 추정값은 `Vt=0.061`, `Vmax=0.039`, `Km=0.161`, `CLd=0.031`, `CLL=0.007`이었다. [G pp.599–601]

**PK26 dose anchor**: Efalizumab의 reduced model은 single-dose i.v. bolus 데이터의 5개 정맥주사 시간경과에 적합되었으므로, 제한적 데이터의 reduced-model 앵커로 읽어야지 MM이 보편적으로 mechanistic이라는 증거로 읽으면 안 된다. [G pp.599–601]

### C. MM이 무너지는 지점

PK27에서 MM은 가장 높은 세 농도 profile을 비교적 잘 적합했지만, 가장 낮은 농도 profile에서는 심각한 체계적 편차를 보였다. 추정된 `Km=3.7`은 TMDD 모델의 `0.03`에 비해 극적으로 과대추정되었다. [G p.609]

임상적 교훈은 "MM을 쓰지 말라"가 아니다. 교훈은 더 좁고 더 실용적이다 — **target saturation이 실증된 concentration/occupancy 범위 밖에서는 MM을 사용하지 말라**.

### D. Occupancy 검증

약 90–95% 이상의 occupancy가 필요한 경우 단순화가 허용될 수 있다. occupancy가 `Kd` 또는 biomarker 역가 이하로 떨어지면 MM이 충분하지 않을 수 있다. [G p.609]

실무 준칙: TMDD 가능성이 있는 mAb에 MM을 사용하는 보고서라면, 관찰된 농도 범위, target 농도 범위, 투여 간격 동안의 최소 예측 target occupancy를 명시해야 한다. 이것은 source boundary의 구현 해석이며, 규제 기관이 직접 인용한 요건이 아니다.

### E. 한계(Limitations)

- MM은 `R0`, `kout`, `kon`, `koff`, `ke(RL)`의 명시적 해석을 상실한다.
- 고용량 또는 중심 profile만 가중되면 저농도 편향이 숨겨질 수 있다.
- 추가 가정 없이는 target recovery나 complex sink 질문에 답할 수 없다.
- QSS/QE 근사는 관련 개념이지만 이 세션의 범위에서는 다루지 않는다; 후속 modeling 세션의 주제다.

### F. 다섯 인지 레이어(Five Cognitive Layers)

| 레이어 | 기억할 것 |
|---|---|
| L1 | MM은 TMDD의 reduction이지, 경쟁하는 별개 biology가 아니다. |
| L2 | `Km`은 binding `Kd`와 같지 않다. |
| L3 | MM은 내삽할 수 있지만 외삽에서는 실패할 수 있다. |
| L4 | Occupancy 범위가 유효성 경계를 결정한다. |
| L5 | target/complex dynamics가 임상적으로 중요하다면 Full TMDD 또는 더 풍부한 데이터가 필요하다. |
...
> MM은 질문이 관찰된 포화 도메인 안에 머물 때 방어 가능하다. 회복, 저농도 외삽, 또는 적합 범위 이하의 target occupancy에 의사결정이 달려 있을 때 취약해진다.
...
`Kd`와 `Km`이 다를 수밖에 없는 이유는 `Km`에 `ke(RL)`이 포함되기 때문이다. 보고서 작성 시, in vitro thermodynamic `Kd`와 in vivo model에서 도출된 apparent `Km`은 반드시 분리해야 한다. 두 값을 모두 "affinity"로 부르면 내부 혼동과 reviewer 입장에서의 불일치가 생기기 쉽다. [G pp.603–609; R&T pp.711–712]
...
|---|---|---|
| Very low ligand | target route가 포화되지 않아 비특이적·target-specific first-order route가 공존하기 때문이다. | assay sensitivity가 낮으면 이 구간을 놓친다. |
| Very high ligand | target route가 포화되어 비특이적 clearance가 지배하기 때문이다. | high-dose fit만 보고 MM/linear model을 과신한다. |
| Middle range | target saturation이 농도에 따라 변하기 때문이다. | nonlinear clearance가 가장 뚜렷하다. |
...
> 이 챕터에서 fit, affinity, linearity, half-life 같은 친숙한 용어는 해당 concentration regime이 명시될 때만 안전하다. 견고한 보고서는 파라미터를 옹호하기 전에 regime을 먼저 정의한다.
...
> 모든 self-test 답변에 한 문장을 추가하라: “어떤 관측 가능한 데이터가 이것을 가르쳤는가?” 이 습관은 측정되지 않은 target biology를 마치 측정된 것처럼 취급하는 오류를 방지한다.
