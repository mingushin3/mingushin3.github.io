# 02_HTML Compile Input Master — ver2

> **ver2 변경 사항 요약**: ver1의 모든 PART A learner body는 verbatim 유지. §8에 세 개의 라벨링된 mastery augmentation block 추가 (D 강화, E 신설, F 신설). PART B B9 Coverage Matrix 및 B11 Mastery Augmentation Log를 업데이트해 추가 augmentation을 추적. 출처-경계·페이지 태그·그림 결정은 ver1과 동일하게 유지.

## Phase 4D Certification

| Certificate | Status | Basis |
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A contains navigation aid plus §1-§8 learner body only; audit/compiler/process material is isolated in PART B. ver2의 §8 E·F augmentation도 learner-facing이며 verbatim 가독성 유지. |
| Zero-Omission Certificate | PASS | Scope MUST concepts, Audit MUST_FIX/SHOULD_FIX, retained figure pointers, page tags, and adopted Crucible Grade A logic are present or justifiably compressed. ver2에서 Crucible T2(b)의 미명명 GOF signature 2종(PK47 Single-Site Misfit, Vss vs Vβ Confusion)이 §8 E에서 명시적으로 명명되어 Crucible Grade A 통합도가 강화됨. |
| Mastery-Uplift Certificate | PASS | PART A contains 11 bounded, adjacent, source-labeled augmentation blocks for this A-Critical chapter (ver1: 8 → ver2: 11). 추가 3개 블록은 §8 D 시스템 통합 시나리오 강화, §8 E Practical Diagnostic Signatures, §8 F Mastery Sequence. |
| Source-Boundary Certificate | PASS | Augmentations are labeled TEXTBOOK_DERIVED, AUDIT_DERIVED, CRUCIBLE_DERIVED, or EXPERT_INFERENCE and add no new page tags or unsupported numerical claims. ver2 추가 블록은 Crucible T1(b)·T2(b)·T3(c) 및 EXPERT_INFERENCE로 명시 라벨링되며 새로운 외부 인용·새 수치·새 페이지 태그·새 그림 결정 없음. |
| HTML-Readiness Certificate | PASS | PART B contains Phase 5 rendering contract, marker mapping, page-tag rules, figure rules, audit/crucible guardrails, and navigation integrity rules. ver2의 §8 E·F 추가는 기존 marker 매핑(`<!-- MASTER LENS -->`, `<!-- RECAP -->` 등)만 사용하며 새 marker 도입 없음. |
...
5. Close with §8. §8 D walks through the two integration scenarios where all six cards work as one system; §8 E names the three diagnostic signatures (Albumin Covariate Asymmetry / PK47 Single-Site Misfit / Vss vs Vβ Confusion); §8 F gives a 4-step Mastery Sequence to apply this material to new PK problems.
...
- I can name the three diagnostic signatures from §8 E and describe their GOF patterns.
- I can apply the §8 F 4-step Mastery Sequence to a previously unseen PK case.
