...
**출력 상태:** Content Lock v2 — Phase 4C figure triage 전 텍스트 최종본.  
...
### 0.5 Pass 1 — Readability & Flow Polish Log

| Location (§ + card) | Edit type (R1–R5) | Before (excerpt) | After (excerpt) |
|---|---|---|---|
| §1 Big Idea | R1/R2 | “종간 체중 차이를 parameter scale로 번역…” | parameter scale의 의미를 바로 붙여 읽히도록 분리. |
| §1 지식 그래프 | R5 | “plasma protein binding…” | 첫 사용 지점에 자유 약물 관점의 짧은 gloss 삽입. |
| §2 C1 Big Idea | R1 | “알로메트리는 … 정량적 답” | allometry의 의미를 10단어 내 gloss로 보강. |
| §2 C2 Plausible Fallacy | R2/R3 | “b=1 linear per-kg scaling” | kg당 일정 가정이라는 오류 구조를 바로 노출. |
| §2 C3 Consequence | R2 | half-life와 rate constant의 반대 지수 | “절대 CL”과 “fraction 제거”의 차이를 더 가깝게 연결. |
| §2 C4 Big Idea | R5 | “Dedrick plot은 … 변환” | 종별 PK 곡선 중첩이라는 mental image를 첫 문장에 삽입. |
| §2 C5 Big Idea | R1/R2 | “equal unbound AUC” | free-drug exposure를 맞추는 backbone으로 명시. |
| §8 Failure Modes | R3/R4 | 긴 실무 진단 문장 | diagnostic trigger와 action을 더 짧게 분리. |

### 0.6 Pass 2 — Annotation Candidate & Recommendation Summary

| Type | Location (§ + card) | Current text (excerpt) | Proposed annotation | Risk |
|---|---|---|---|---|
| A | §1 Big Idea | parameter scale | parameter scale(← 체중 차이를 파라미터 단위로 옮기는 척도) | Low |
| A | §1 지식 그래프 | plasma protein binding | plasma protein binding(← 단백 결합으로 자유 약물 농도가 달라지는 현상) | Low |
| A | §2 C1 | 알로메트리 | 알로메트리(← 크기와 생리 변수의 거듭제곱 관계) | Low |
| A | §2 C2 | linear per-kg scaling | linear per-kg scaling(← kg당 값이 일정하다는 가정) | Low |
| A | §2 C4 | Dedrick plot | Dedrick plot(← 종별 PK 곡선을 같은 척도에 겹치는 변환) | Low |
| A | §2 C5 | equal unbound AUC | equal unbound AUC(← 자유 약물 AUC를 같게 맞추는 기준) | Low |
| B | §2 C3 | half-life와 rate constant 연결 | “같은 분수 구조의 앞뒤” bridge sentence | Medium |
| C | §2 C4 | 차원 분석 비유 | 구조 비유는 flow를 끊을 위험 | High |

**Final recommendation:** Low-risk Type A는 반영했다. Medium-risk C3 bridge는 1문장만 반영했다. High-risk analogy는 반영하지 않았다.
