# 14_HTML Compile Input Master — Version 3
...
**Each C-card closes with a ▶ Practice Brief that compresses four mastery axes — 왜 알아야 하는가 / 흐름상 역할 / 체화 꿀팁 / 실무 활용.** §2 closes with an Allometric Decision Walkthrough that links C1→C5 as a single workflow.
...
6. §2 폐막 Walkthrough: walk one animal-PK dataset through C1→C5 to lock the flow.
7. §5–§7: test whether you can block source discrepancies, unsupported regulatory language, and exponent-swapping errors.
...
> <!-- TRENCH -->
> **▶ Practice Brief — C1 체화 4축 — [EXPERT_INFERENCE, ver2 V2]**  
...
> - **실무에서 어떻게 드러나는가:** FIH 용량을 $b=0.75$로만 계산했다가 실제 human CL이 다른 exponent를 따를 경우, Phase 1 starting dose가 과대 또는 과소추정될 수 있다. Safety review에서 “exponent justification”을 요구하는 IND meeting 피드백이 나올 수 있다. 이 fallacy는 위의 `b=1` 오류와 **반대 방향의 거울상**이다 — 한쪽은 mg/kg에 묶여 $b$를 1로 보수적으로, 다른 쪽은 $b$를 0.75로 무비판적으로 신뢰한다. 두 fallacy를 동시에 차단해야 C2가 닫힌다.
...
> - **실무 활용:** FIH exposure translation 표에서 CL column을 채울 때, $b=0.75$를 primary로 두되 $b=0.5, 0.62, 0.75, 1.0$ sensitivity를 같은 표에 병기하는 것이 방어적이다 (Q8 보스 딜레마 응답 패턴). $f_u$ 차이가 크면 total CL이 아니라 $CL_u$를 스케일링한다는 Trench 규칙을 동시에 적용한다.
...
> $b\approx 0.75$ prior를 그대로 적용해도 되는지 묻는다. 종간 fu 차이, CYP isoform 차이, nonlinear MM kinetics, enterohepatic recirculation, route/formulation 차이, central/peripheral V ratio drift — 이 6개 trigger 중 어느 하나라도 강하게 의심되면 prior를 그대로 쓰지 말고 sensitivity analysis로 격하한다. fu 차이가 크면 total CL이 아니라 $CL_u$로 스케일링한다 (Eq.2:421 라인).
>
...
*왜 이 질문 — [EXPERT_INFERENCE, ver2 V4]:* 보스 딜레마. 점추정값과 physiological prior가 충돌할 때 무엇을 primary로 두고 무엇을 sensitivity로 보일지를 입으로 정당화할 수 있어야 한다. 동시에 ICH/FDA 단정문을 자발적으로 차단하는 source language discipline이 동작하는지 확인한다.
...
| C6 Page tags | Existing source page tags preserved; no new page tags fabricated by augmentation | PART A page tags inherited from Content Lock v2 and marker sources; ver2 adds no new tags | PRESENT | None |
...
No scientific micro-patches were needed. PART A equals exact-spliced learner-facing Content Lock v2.1 except approved Mastery Augmentation Layer (eight ver1 augmentations + five ver2 augmentations), learner navigation wrapper, and four non-scientific process-label neutralizations that remove `Draft v0`/`Content Lock` wording from learner-facing sentences without changing scientific meaning.

### B11. Mastery Augmentation Log — v3 (consolidated)
...
#### v3 augmentations (new — minimum surgical set)
...
## End of 14_html_compile_input_master_v3.md

**Final Verdict: GO for Phase 5 HTML compilation.**

v3는 ver2의 모든 PASS 인증과 mastery 강화 요소(Practice Brief 4축, Allometric
Decision Walkthrough, Target Mental Model, "왜 이 질문" rationale)를 그대로 유지하면서,
잔존하던 두 가지 마이너 갭(C2 Apex 표준 마커 미적용, §5 Pair 1·Pair 3 기억 고리
누락)을 **표면적 수술(surgical patch)**로 닫았다. ver2의 모든 사실 문장, 수식,
page tag, marker, figure pointer, RECAP, Mastery Note, Practice Lens, Trench tip,
Practice Brief, Walkthrough, Target Mental Model, Q rationale가 **그대로 보존**되었다.
...
인증은 모두 유지되며, v3에서 Apex 마커 표준화와 §5 Memory Hook 보강이 추가되었다.
