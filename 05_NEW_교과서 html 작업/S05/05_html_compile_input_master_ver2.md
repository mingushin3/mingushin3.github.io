# 05_HTML Compile Input Master — ver2

## Phase 4D Certification (ver2)

| Certificate | Status | Basis |
|---|---|---|
| Learner-Standalone Certificate | PASS | PART A starts at learner-facing §1, excludes audit/compiler/process tables, and contains the complete spliced study handout. ver2: Q9 보스 딜레마 추가로 standalone session closure 강화. |
| Zero-Omission Certificate | PASS | Scope, Audit, Crucible, Content Lock v2, PDF anchors, and Phase 4C marker coverage were checked; no unresolved high-impact omission remains. ver2: PK7 weighting scheme 경고와 Pseudo-Stable V signature 추가로 Crucible Grade A/B 잔여 항목 흡수. |
| Mastery-Uplift Certificate | PASS (강화) | ver1: Five bounded expert-synthesis augmentations adjacent to M1–M5. ver2: M3 [⚡ Apex Concept] 명시 배지, §7 Q9 Socratic Dilemma, §8C 7번째 Professional moat 항목, §8B Pseudo-Stable V 시그니처 — 4개 추가 보강이 모두 EXPERT_INFERENCE 또는 [실무 추론] 태그로 분리됨. |
| Source-Boundary Certificate | PASS | All augmentations are explicitly labeled EXPERT_INFERENCE/source-bound synthesis and add no new page tags, numbers, drugs, or external claims. ver2 신규 추가도 동일 — Q9는 PK8 condition numbers (29.69, 3,186)만 사용, Pseudo-Stable V는 본문 기존 gentamicin 정보만 인용, glosses는 일반 통계학 정의에 한정. |
| HTML-Readiness Certificate | PASS | PART B includes compilation contract, marker mapping, page-tag rules, figure rules, navigation-anchor integrity rules, and guardrails for Phase 5. ver2: Marker count 변동 없음 (7개 그대로), Annotation marker 4개 추가 (WRSS, RSE, ETA(V2), VPC, CWRES — 모두 인라인 ANNOTATION 형식). |

## ver2 Changelog

| # | Location | Type | Change | Source-Boundary | Rationale |
|---|---|---|---|---|---|
| V2-01 | §2 Card M3 heading | Apex Concept badge | `### Card M3 — V₁ / Vss / Vβ 3중 분포용적 체계` → `[⚡ Apex Concept]` 배지 추가 + 카드 최상단에 Apex 지정 사유 1문장. | 기존 Curation Map / Step1 Draft v0의 Apex 지정 시각화. 새 사실 없음. | M3가 본 세션의 최대 임상·규제 파급력 카드임을 명시. |
| V2-02 | §2 Card M4 Big Idea | First-use gloss | `WRSS` 첫 등장 위치에 `<!-- ANNOTATION -->(← weighted residual sum of squares; fit 잔차 크기 지표)` 추가. | 일반 통계학 정의. PDF 새 인용 없음. | PhD 학생 첫 등장 시 친절한 정의 — Pass 2 annotation 원칙 일관성. |
| V2-03 | §2 Card M4 RSE caveat | First-use gloss | `RSE` 첫 등장 위치에 `(← relative standard error; 상대표준오차, %)` 추가. | 일반 통계학 정의. | 동일 사유. |
| V2-04 | §2 Card M4 PK7 bridge | Sentence-level reinforcement | "weighting scheme이 다른 model을 단순 F-test로 비교하면 안 된다" → 메커니즘 + 결과 1문장 보강 (NDA submission 모델 정당화 논리 연결). | G&W p.512 직접 메시지 강화. 새 수치 없음. | Audit T4 #11 SHOULD_FIX의 명시적 강화. |
| V2-05 | §7 Q9 신규 | Socratic Dilemma question | PK8 데이터에서 Takada vs ODE 모델 채택 양자택일 질문 추가. Trade-off 논리 5단계 답변 포함. | PK8 condition number는 본문에서 이미 인용된 값 (29.69, 3,186). 새 numerical claim 없음. 모든 새 추론은 [실무 추론] 태그. | Step 1 Protocol Spec에 명시된 보스 딜레마 요구사항 충족. NDA defense 논리 체화. |
| V2-06 | §8 B Failure modes | New row | "Pseudo-Stable V across renal function" 진단 시그니처 행 추가 — gentamicin-like 약물에서 $V_\beta=CL/\beta$로 인한 신기능 영향 착시 + VPC 시그니처. | gentamicin 신기능 정보는 R&T Fig.19-17 영역에 이미 있음. 새 약물·새 수치 없음. [실무 추론] 태그. | Crucible T2(b) Grade A "Phantom V2 / Macro Drift / Initial Estimate Trap" 시리즈에서 누락된 1개 보강. M3 Apex의 임상 진단력으로 직접 전이. |
| V2-07 | §8 C Professional moat | New item #7 | Covariate model의 forward inclusion 단계에서 M1–M5 5개 카드 동시 활성화를 7번째 거장 능력으로 추가. | §1 Workflow anchor의 §8 재호출. 새 사실 없음. | Crucible T1(b) Grade A "system integration moment"의 §8 강화. |
| V2-08 | §8 B / §8 C 인라인 | First-use gloss (4종) | ETA(V2), VPC, CWRES 첫 등장 위치에 `<!-- ANNOTATION -->` gloss 추가. | 일반 PopPK terminology 정의. 새 인용 없음. | PhD 학생 친절함 일관성. |

### ver2 영향 분석

**텍스트 길이:** ver1 대비 약 +2.5% 증가 (Q9 보스 딜레마 + Pseudo-Stable V 행 + Apex 배지 + 4개 inline gloss + 1개 Professional moat 항목). 8% 한계 내. 학습자 부하 증가 미미.

**Marker 수:**
- FIGURE markers: 7개 (변동 없음)
- ANNOTATION markers: 기존 7개 → 11개 (WRSS, RSE, ETA(V2), VPC, CWRES 추가)
- SELF-TEST 블록: 기존 8개 → 9개 (Q9 추가)
- 기타 markers: 변동 없음

**Source-Boundary 검증:**
- 새 page tag 부착: 0개
- 새 약물/수치/임상 데이터 인용: 0개
- 새 EXPERT_INFERENCE 블록: 1개 (Q9 trade-off 논리)
- 새 [실무 추론] 태그: 2개 (Q9 + Pseudo-Stable V 행)
- ABSOLUTE constraint 위반: 0건

**Splice Verification (ver2):** PATCH MODE marker insertion은 ver1과 동일 — 7개 figure marker 모두 같은 anchor 위치 유지. ver2 추가 항목은 모두 PART A의 인라인 보강이며 splice anchor와 무관.
...
### Final Checklist Result (ver2)

| Checklist group | Status | Notes |
|---|---|---|
| Learner completeness | PASS | PART A is independently readable; audit/compiler/process material is not inside learner handout. ver2: Q9 Socratic Dilemma로 §7 closure 강화. |
| Content integrity | PASS | Approved markers present exactly once in PART A; no unapproved figure markers added; no page tags fabricated. ver2 신규 추가 5개 ANNOTATION + 1개 SELF-TEST는 모두 inline 보강. |
| Mastery augmentation | PASS | Five labeled, bounded blocks (AUG-01–05) + ver2 4개 추가 (AUG-06: Apex badge, AUG-07: Q9 Socratic Dilemma, AUG-08: Pseudo-Stable V, AUG-09: Professional moat #7). 모든 추가는 EXPERT_INFERENCE 또는 [실무 추론] 태그로 visibly separated. No new examples/numbers/page tags. |
| HTML readiness | PASS | PART B contains marker, figure, source-tag, navigation, print, and forbidden-restoration rules. Marker → component mapping에 `MASTER AUGMENTATION` blockquote 항목 이미 존재 (B4) — ver2 추가 EXPERT_INFERENCE 블록도 동일 매핑으로 렌더됨. |
| Source-Boundary | PASS | ver2 신규 추가는 모두 source-boundary 통과: Q9 PK8 numbers는 본문 기존 인용, Pseudo-Stable V는 본문 기존 gentamicin/Vβ relation만 사용, glosses는 일반 통계학 정의. ABSOLUTE constraint 위반 0건. |

### ver2 Final Go/No-Go Decision

**판정: GO** — 본 파일은 *교과서 범위에서 학습자가 반드시 체화해야 할 핵심 개념·예시·수식·그림 포인터·거장 통찰을 검증된 방식으로 묶은 HTML-ready 학습 원고*로서 Phase 5 HTML compilation을 진행할 수 있다.

확정 근거:
1. **Zero-Omission**: 5개 MUST 카드, 7개 figure marker, 9개 self-test, §5 5쌍 (Critical Blow 2개), §8 A/B/C/D 모두 완비.
2. **Mastery-Uplift**: 거장 시각 layer가 Big Idea + Master Augmentation 5개 + Apex 배지 + Socratic Dilemma + Professional moat 7항목으로 다층 구성.
3. **Source-Boundary**: 모든 [실무 추론]/[수학적 해석]/[실무 확장]/EXPERT_INFERENCE 태그가 보존됨. PDF 직접 인용과 추론 사이 경계 불변.
4. **HTML-Readiness**: PART B 가드레일 + Splice Verification + Marker mapping + Page tag rules + Navigation rules 완비.
5. **Learner-Friendly**: First-use gloss 11개 + Korean prose + 친절한 §1 Workflow anchor + 명시적 Apex 표지로 PhD 1학년 학습 가능.

**다음 단계:** `Phase 5 — HTML Compile` 세션에서 본 파일 (`05_html_compile_input_master_ver2.md`)을 입력으로 사용. PATCH MODE 자체는 이미 본 파일에서 해소되었으므로, Phase 5는 PART A 본문을 그대로 렌더링하고 PART B 가드레일을 준수한다.
