# 05_HTML Compile Input Master — v3
...
- **왜 틀렸는가:** terminal half-life는 $\beta$ slope의 역수 ($0.693/\beta$)로 결정되는데, $\beta$는 $CL,Q,V_1,V_2$의 복합함수다. $V_\beta=CL/\beta$로 계산되어 $V_{ss}$보다 항상 크거나 같으며, 두 구획 간 재분포 흐름이 반영된 겉보기 부피다. terminal half-life가 길더라도 $V_1$이 작고 $Q$가 느린 경우엔 초기 분포가 아니라 **재분포 속도**가 rate-limiting일 수 있다 — 즉 "넓은 분포"가 아니라 "느린 재분포"가 긴 terminal half-life의 진짜 원인일 수 있다. (이전 버전의 "$V_\beta$가 terminal curve geometry의 산물"이라는 메시지는 이 메커니즘 안에 흡수되어 있다.)
...
**PK7 model discrimination bridge:** G&W PK7은 mono/bi/tri-exponential discrimination에서 AIC/F-test, precision, correlations, residuals, simplest adequate model rule을 함께 보게 한다. 특히 **weighting scheme이 다른 두 모델은 단순 F-test로 비교하면 안 된다** — 두 모델의 likelihood가 서로 다른 measurement-error assumption 위에 있기 때문이며, 이를 무시하면 model discrimination 결론 자체가 무효가 된다. NDA submission의 모델 정당화 논리는 이 단순 사실 위에 서 있다. [G&W pp.505–512]
...
$\alpha$ phase는 약이 혈장에서 조직으로 **이사 가는** 분포 단계, $\beta$ phase는 주로 **청소**(제거)가 지배하는 단계다. 그러나 두 단계가 완전히 분리된 것이 아니라 항상 **동시에** 일어난다 — $\alpha$ 단계에도 제거가 일어나고, $\beta$ 단계에도 분포 평형 조정이 일어난다. "$\alpha$ = 분포, $\beta$ = 제거"는 **근사 레이블이지 기계적 분리가 아니다.** 이 레이블을 두 개의 elimination pathway로 곡해하는 순간 central elimination 가정이 무너진다.
...
| **치명적 타격** | NDA/IND submission 또는 critical care loading dose 결정에서 $V_\beta$를 $V_{ss}$ 대신 사용하면, gentamicin-like 약물의 첫 dose가 약 6배 과대 산정될 수 있어 (1) 심사관이 dosing rationale의 volume 정의 일관성 결여를 Deficiency Letter 사유로 지적하거나 (2) 환자에게 nephrotoxicity/ototoxicity 임계 농도를 첫 60분 내 초과하는 plasma peak이 발생할 수 있다. | |
...
7. **Covariate model의 forward inclusion 단계에서 5개 카드(M1–M5)가 동시에 켜지는 것을 본다** — $f_2$ 직관(M1/M5) → 좌표계 선택(M2) → $V_{ss}$ vs $V_\beta$ attach 결정(M3) → condition number/RSE 안정성 점검(M4)이 한 번의 covariate 추가에서 모두 작동한다. 이 동시 활성화를 못 보면 covariate가 *어디에 붙어야 하는지*조차 결정 못 한다. [실무 통합 — §1 Workflow anchor의 §8 재호출]
