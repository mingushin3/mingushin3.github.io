...
> **핵심 메시지:** 이 파일은 임상 통합·TDM·질환·치료 의사결정을 하나의 source-locked 학습 체인으로 읽어야 합니다.


## 학습자 항법 안내 (Learner Navigation Aid)
<!-- SLIDE_START: S02 | title: 학습자 항법 안내 | source_anchor: learner-navigation-aid -->
> **핵심 메시지:** 먼저 전체 학습 경로를 고정해야 이후의 모델·질환·TDM·용량 판단이 한 흐름으로 연결됩니다.
...
> **핵심 메시지:** 이 자료의 범위는 교과서 source boundary 안에서 임상 계량약리학의 의사결정 사슬을 묶는 것입니다.
...
> **핵심 메시지:** Session 016의 핵심은 shape에서 시작해 exposure/safety-margin report까지 이어지는 단일 판단 사슬입니다.
...
> **핵심 메시지:** 각 챕터는 독립 지식이 아니라 다음 단계로 넘길 의사결정 객체를 생산합니다.
...
> **핵심 메시지:** §2는 plot-reading grammar를 환자별 파라미터 추정과 용량 결정으로 단계적으로 연결합니다.
...
> **핵심 메시지:** Plot shape는 정답 모델이 아니라 후보 모델을 줄이는 첫 번째 triage 언어입니다.
...
> **핵심 메시지:** Fig.6.1과 Fig.6.12는 shape checklist를 실제 모델 후보 축소 workflow로 바꾸는 원그림입니다.
...
> **핵심 메시지:** 이 카드는 모델명을 고르는 법보다 관찰 순서를 고정해 후보를 줄이는 법을 훈련합니다.
...
> **핵심 메시지:** Peak-shift 방향은 모델 클래스 판결이 아니라 competing model candidates를 좁히는 진단 신호입니다.
...
> **핵심 메시지:** Fig.6.2와 Fig.6.3은 하나의 관찰 패턴을 하나의 모델명으로 환원하지 말라는 안전장치입니다.
...
> **핵심 메시지:** Peak-shift를 모델명으로 번역하는 순간 diagnostic signal이 deterministic overclaim으로 바뀝니다.
...
> **핵심 메시지:** Effect compartment, turnover, receptor on/off는 delay가 어느 state에 있는지를 구분하는 세 문법입니다.
...
> **핵심 메시지:** Effect compartment의 핵심은 혈장 농도가 아니라 효과부위 농도 $C_e$가 response를 지배한다는 점입니다.
...
>
> $$
> \underbrace{\frac{dC_e}{dt}}_{\text{effect-site 농도의 변화율}}
> =
> \underbrace{k_{e0}}_{\text{평형화 속도}}
> \left(\underbrace{C}_{\text{혈장 농도}}-\underbrace{C_e}_{\text{효과부위 농도}}\right)
> $$
>
> - **항별 의미:** 좌변은 효과부위 농도 $C_e$가 시간에 따라 얼마나 빨리 변하는지를 정의합니다. 우변은 혈장 농도 $C$와 효과부위 농도 $C_e$ 사이의 차이를 $k_{e0}$ 속도로 메우는 equilibration flux입니다.
> - **실무 직관:** $C-C_e$ 차이가 클수록 $C_e$가 더 빨리 움직이고, $k_{e0}$가 클수록 혈장-효과부위 지연이 짧아집니다. 이 식은 counter-clockwise hysteresis가 단순 assay 오류가 아니라 effect-site delay일 수 있음을 판단하게 합니다.
> - **오해 방지:** $C_e$는 새로 측정한 농도가 아니라 모델이 도입한 effect-site state입니다. 이 식만으로 turnover나 receptor binding을 배제하는 것은 과잉해석입니다.
...
> **핵심 메시지:** Turnover model의 지연은 약물 이동 지연이 아니라 response system의 생성·소실률 변화에서 생깁니다.
...
>
> $$
> \underbrace{\frac{dR}{dt}}_{\text{response의 순변화율}}
> =
> \underbrace{k_{in}\cdot I(C)}_{\text{농도에 의해 조절된 생성/input}}
> -
> \underbrace{k_{out}R}_{\text{현재 response의 소실/output}}
> $$
>
> - **항별 의미:** 좌변은 response variable $R$의 시간 변화량을 예측합니다. 우변 첫 항은 drug concentration $C$가 input function $I(C)$를 통해 생성률을 바꾼 부분이고, 둘째 항은 현재 $R$에 비례하는 소실입니다.
> - **실무 직관:** $I(C)$가 억제 함수라면 농도 증가가 생성항을 줄여 $R$을 낮춥니다. $k_{out}$이 클수록 새로운 평형으로 더 빨리 이동하므로, trough timing과 rebound 해석에 직접 연결됩니다.
> - **오해 방지:** 이 식의 delay는 약물이 effect site에 늦게 도달해서가 아니라 response system의 생성·소실 turnover가 바뀌어서 생깁니다.
...
> \underbrace{\frac{dR}{dt}}_{\text{response의 순변화율}}
> =
> \underbrace{k_{in}}_{\text{기본 생성률}}
> -
> \underbrace{k_{out}R\cdot S(C)}_{\text{농도에 의해 조절된 소실/loss}}
> $$
>
> - **항별 의미:** 좌변은 $R$의 시간 변화율입니다. 우변은 고정된 생성항 $k_{in}$에서, 현재 response $R$과 기본 소실속도 $k_{out}$ 및 농도 의존 stimulation function $S(C)$가 만든 loss를 뺍니다.
> - **실무 직관:** $S(C)$가 커지면 같은 $R$에서도 소실항이 커져 response가 더 빠르게 낮아집니다. 이 구조는 loss stimulation에 의한 nadir 이동과 rebound 판단에 쓰입니다.
> - **오해 방지:** $S(C)$가 있다고 해서 항상 receptor on/off 모델이라는 뜻은 아닙니다. finite receptor pool 없이도 turnover loss modulation이 유사한 peak-shift를 만들 수 있습니다.
...
> **핵심 메시지:** Finite receptor pool이 있으면 결합 형성과 해리의 경쟁이 response timing과 saturation을 만듭니다.
...
>
> $$
> \underbrace{\frac{dRC}{dt}}_{\text{drug--receptor complex 변화율}}
> =
> \underbrace{k_{on}C(R_T-RC)}_{\text{결합 형성: free receptor에 의존}}
> -
> \underbrace{k_{off}RC}_{\text{해리: complex에 비례}}
> $$
>
> - **항별 의미:** 좌변은 receptor complex $RC$가 시간에 따라 얼마나 늘거나 줄어드는지를 정의합니다. 우변 첫 항은 농도 $C$와 남아 있는 receptor pool $(R_T-RC)$가 만드는 결합 형성이고, 둘째 항은 이미 형성된 $RC$의 해리입니다.
> - **실무 직관:** $C$ 또는 $k_{on}$이 커지면 형성항이 커지지만, $R_T-RC$가 줄면 finite pool 때문에 증가가 둔화됩니다. 이 식은 saturation과 response extremum timing을 receptor binding 관점에서 해석하게 합니다.
> - **오해 방지:** $k_{on}$이 크다는 단일 사실만으로 left-shift를 확정하지 않습니다. turnover model도 유사한 관찰 패턴을 만들 수 있습니다.
...
> **핵심 메시지:** 세 prototype은 delay의 위치를 명명함으로써 `$DES` 구조와 임상 plot 해석을 동시에 정리합니다.
...
> **핵심 메시지:** Bayesian TDM은 농도 likelihood와 population prior의 상대적 정보량을 저울질해 개인 parameter를 추정합니다.
...
>
> $$
> \underbrace{OBJ_{Bayes}}_{\text{개인 추정의 총 penalty}}
> \approx
> \underbrace{\sum_i\frac{\left(C_{obs,i}-\hat C_i\right)^2}{\operatorname{var}(\hat C_i)}}_{\text{농도 자료가 요구하는 적합 penalty}}
> +
> \underbrace{\sum_j\frac{\left(P_{pop,j}-\hat P_j\right)^2}{\operatorname{var}(\hat P_j)}}_{\text{population prior에서 벗어난 penalty}}
> $$
>
> - **항별 의미:** 좌변은 Bayesian individual fitting에서 최소화하려는 목적함수입니다. 우변 첫 합은 관측 농도와 예측 농도의 불일치이고, 둘째 합은 개인 추정 파라미터가 population prior에서 벗어난 정도입니다.
> - **실무 직관:** 농도 오차 variance가 작거나 관측 농도가 정보성 높으면 likelihood 항의 영향이 커집니다. prior variance가 작으면 population prior가 강하게 작동해 개인 파라미터가 평균 쪽으로 당겨집니다.
> - **오해 방지:** posterior 숫자는 “개인화 완료”가 아닙니다. 그 숫자가 농도 정보로 식별된 것인지, prior에 의해 고정된 것인지를 별도로 판단해야 합니다.
...
> **핵심 메시지:** Fig.35.1과 Fig.18-13은 posterior 숫자를 믿기 전에 어떤 parameter가 식별되었는지 묻게 합니다.
...
> **핵심 메시지:** Posterior estimate는 채혈 시점이 지지하는 parameter 정보와 함께 읽을 때만 개인화 판단이 됩니다.
...
> **핵심 메시지:** Renal dosing은 환자 신기능 $RF$와 약물의 renal dependence $f_e$를 분리해야 안전하게 판단됩니다.
...
>
> $$
> \underbrace{R_d}_{\text{유지요법 필요량 비율}}
> =
> \underbrace{RF\cdot f_e}_{\text{신기능에 남아 있는 renal 제거 기여}}
> +
> \underbrace{(1-f_e)}_{\text{비신 제거 기여}}
> $$
>
> - **항별 의미:** 좌변은 정상 renal function 대비 유지 용량 또는 dosing rate가 어느 정도 필요한지를 정의합니다. 우변은 환자 신기능 $RF$에 의해 줄어드는 renal component와 보존되는 nonrenal component를 더합니다.
> - **실무 직관:** $f_e$가 클수록 $RF$ 감소가 $R_d$를 크게 낮춥니다. 반대로 $f_e$가 작으면 renal impairment가 maintenance exposure에 미치는 영향이 작아집니다.
> - **오해 방지:** 이 식은 maintenance dose 논리입니다. $V$가 지배하는 loading dose에 그대로 적용하면 초기 치료 농도를 놓칠 수 있습니다.
...
> =
> \underbrace{CL_{non\text{-}renal}}_{\text{비신 제거}}
> +
> \underbrace{f_e\cdot CL_{total}\cdot RF}_{\text{신기능 저하 후 남는 renal 제거 성분}}
> $$
>
> - **항별 의미:** 좌변은 전체 clearance입니다. 우변은 renal function의 직접 영향을 덜 받는 비신 제거와, 약물의 renal dependence $f_e$ 및 환자 renal function $RF$가 반영된 renal 제거 성분의 합입니다.
> - **실무 직관:** $RF$가 낮아져도 $f_e$가 작으면 총 $CL$ 변화는 작습니다. $f_e$와 $RF$가 모두 크거나 의미 있게 변할 때 maintenance dosing 판단이 크게 움직입니다.
> - **오해 방지:** 표 안의 `$\approx$` 표기는 문맥상 “거의 무시 가능”이라는 판단 언어입니다. 원문 보존 원칙에 따라 원 table 표기는 수정하지 않았습니다.
>
...
> **핵심 메시지:** $R_d$ schematic은 신기능 저하 라벨이 아니라 $f_e$와 $RF$의 조합이 maintenance requirement를 움직인다는 점을 보여 줍니다.
...
> **핵심 메시지:** $R_d$는 비신 clearance가 신기능에 의해 변하지 않는다는 가정까지 함께 점검해야 실무에 적용됩니다.
...
> **핵심 메시지:** Cockcroft-Gault는 renal function의 입력값을 만들지만, SCr lag 때문에 급성 변화에서는 과신하면 안 됩니다.
...
>
> $$
> \underbrace{CL_{cr}}_{\text{creatinine clearance 추정값}}\;(\mathrm{mL/min})
> =
> \frac{\underbrace{(140-age)}_{\text{연령 보정}}\cdot\underbrace{WT}_{\text{체중}}}{\underbrace{72\cdot SCr}_{\text{혈청 creatinine 기반 scaling}}}
> $$
>
> - **항별 의미:** 좌변은 creatinine clearance 추정값입니다. 우변은 연령, 체중, 혈청 creatinine을 이용해 adult stable renal function 상황의 renal function proxy를 계산합니다.
> - **실무 직관:** 나이와 SCr가 증가하면 추정 $CL_{cr}$은 낮아지고, 체중이 커지면 증가합니다. 이 값은 $RF$ 입력값으로 이어져 maintenance dosing 판단을 가능하게 합니다.
> - **오해 방지:** SCr는 실시간 renal function이 아니라 creatinine turnover가 반영된 지연 지표입니다. AKI에서는 계산식이 수학적으로 맞아도 오늘의 renal function을 과대평가할 수 있습니다.
...
> **핵심 메시지:** Dialysis supplement 판단은 half-life 단축이 아니라 세션 중 body amount가 실제로 얼마나 빠졌는지에 달려 있습니다.
...
>
> $$
> \underbrace{k_D}_{\text{dialysis 중 총 제거속도상수}}
> =
> \frac{\underbrace{CL_u+CL_{uD}}_{\text{환자 자체 unbound CL + dialysis CL}}}{\underbrace{V_u}_{\text{unbound 분포용적}}},
> \qquad
> \underbrace{1-\exp(-k_D\cdot T)}_{\text{session 동안 빠지는 분율}},
> \qquad
> \underbrace{\frac{CL_{uD}}{CL_u+CL_{uD}}}_{\text{총 제거 중 dialysis 기여}}
> $$
>
> - **항별 의미:** 첫 식의 좌변 $k_D$는 dialysis 중 제거 속도를 정의합니다. 우변은 환자 자체 clearance와 dialysis clearance를 더한 뒤 unbound volume으로 나눕니다. 둘째 표현은 세션 시간 $T$ 동안 실제로 빠지는 fraction이고, 셋째 표현은 제거 중 dialysis가 차지하는 몫입니다.
> - **실무 직관:** $CL_{uD}$가 커지면 dialysis 기여가 커지지만, $V_u$가 크면 같은 clearance라도 body amount 감소는 둔화됩니다. 따라서 보충 용량은 half-life 변화가 아니라 세션 중 제거량으로 판단합니다.
> - **오해 방지:** `half-life가 짧아졌다`와 `보충 용량이 필요할 만큼 많이 빠졌다`는 같은 말이 아닙니다. $V_u$, $CL_u$, $CL_{uD}$를 함께 봐야 합니다.
...
> **핵심 메시지:** Fig.15-18과 Fig.15-19는 dialysis effectiveness를 $V_u$, $CL_u$, $CL_{uD}$ 좌표에서 읽게 합니다.
...
> **핵심 메시지:** Hemodialysis 실무에서는 `반감기 단축`과 `보충 용량 필요`를 분리해야 합니다.
...
> **핵심 메시지:** TCS는 약물명 자동 적용이 아니라 측정값이 다음 처방 결정을 바꿀 수 있을 때만 유용한 보조 전략입니다.
...
> **핵심 메시지:** Loading dose는 $V$와 target concentration 문제이고, maintenance dose는 $CL$과 average exposure 문제입니다.
...
> =
> \frac{\underbrace{V}_{\text{분포용적}}\cdot\underbrace{C_{target}}_{\text{목표 농도}}}{\underbrace{F}_{\text{생체이용률}}}
> $$
>
> - **항별 의미:** 좌변은 목표 농도에 빠르게 도달하기 위한 loading dose입니다. 우변은 몸 안에서 채워야 할 apparent distribution space와 target concentration의 곱을, 실제 흡수되는 fraction $F$로 보정합니다.
> - **실무 직관:** $V$ 또는 $C_{target}$이 커지면 필요한 loading dose가 커지고, $F$가 낮으면 같은 target을 위해 투여량이 커집니다. 이 식은 초기 target attainment 판단을 가능하게 합니다.
> - **오해 방지:** loading dose는 $CL$이 아니라 $V$ 문제입니다. renal impairment가 있어도 $V$가 변하지 않았다면 LD 자동 감량은 부적절할 수 있습니다.
...
> **핵심 메시지:** Fig.18-5는 dosing accuracy 약속이 아니라 $V$, $CL$, $F$ variability 중 무엇이 설명되는지를 보여 줍니다.
...
> **핵심 메시지:** 신부전 환자에서도 LD와 MD를 같은 감량 논리로 묶는 순간 초기 치료 실패 위험이 생깁니다.
...
> **핵심 메시지:** Missed, unequal, erratic dosing은 자료 오염이 아니라 superposition으로 정량화해야 할 일반 임상 상황입니다.
...
>
> $$
> \underbrace{\frac{t_{1/2}}{\tau}}_{\text{반감기 대비 투여간격 비율}}
> \times
> \underbrace{\text{therapeutic window}}_{\text{농도 하락을 허용하는 폭}}
> \Rightarrow
> \underbrace{\text{forgiving/unforgiving regimen}}_{\text{missed dose 후 trough 유지 가능성}}
> $$
>
> - **항별 의미:** 좌측의 비율은 약물이 투여간격 동안 얼마나 천천히 사라지는지를 나타냅니다. therapeutic window는 농도가 어느 정도 내려가도 임상적으로 허용되는지를 결정합니다.
> - **실무 직관:** $t_{1/2}/\tau$가 클수록 한 번 또는 여러 번의 missed dose 후에도 trough가 덜 떨어져 forgiving regimen에 가까워집니다. window가 좁으면 같은 비율에서도 임상 위험이 커집니다.
> - **오해 방지:** 하루 총용량이 같다는 사실만으로 missed-dose risk가 같아지지 않습니다. 투여간격 구조와 therapeutic window를 함께 봐야 합니다.
>
...
> **핵심 메시지:** §5의 목적은 단일 관찰값을 단일 원인으로 고정하는 오류를 분해하는 것입니다.
...
> **핵심 메시지:** Effect compartment는 분포 지연이고 turnover는 response system 지연이라는 구분이 모델 선택을 바꿉니다.
...
> **핵심 메시지:** Peak movement는 모델명 판결이 아니라 어떤 후보가 남는지 묻는 diagnostic discriminator입니다.
...
> **핵심 메시지:** Bayesian TDM의 핵심은 posterior number 자체가 아니라 prior와 likelihood 중 무엇이 지배했는지 읽는 것입니다.
...
> **핵심 메시지:** 간 질환과 신질환은 모두 $CL$ 언어를 쓰지만 exposure를 바꾸는 해부학은 서로 다릅니다.
...
> **핵심 메시지:** LD를 신기능 비례로 줄이는 오류는 $R_d$ framework를 $V$ 문제에 잘못 적용할 때 발생합니다.
...
> **핵심 메시지:** Fit이 통과해도 sampling geometry가 지지하지 않은 parameter는 individual dosing 판단에 쓰기 어렵습니다.
...
> **핵심 메시지:** Self-test의 핵심은 계산 정답보다 어떤 정보가 없으면 해석이 불가능한지 즉시 말하는 것입니다.
...
>
> $$
> \underbrace{R_d}_{\text{유지요법 필요량}}
> =
> \underbrace{RF\cdot f_e}_{\text{환자 신기능이 조절하는 renal fraction}}
> +
> \underbrace{(1-f_e)}_{\text{보존되는 nonrenal fraction}}
> $$
>
> - **항별 의미:** 좌변은 유지요법 조정 비율이고, 우변은 환자 요인 $RF$와 약물 요인 $f_e$를 분리합니다.
> - **실무 직관:** $f_e$가 큰 약물일수록 같은 $RF$ 감소가 더 큰 maintenance 감량으로 이어집니다. 이 구조는 renal dosing에서 약물 속성과 환자 속성을 분리해 판단하게 합니다.
> - **오해 방지:** 이 식을 loading dose 판단에 재사용하면 $V$ 기반 초기 농도 달성 문제를 놓칩니다.
...
> **핵심 메시지:** 후반 self-test는 TCS, LD/MD, irregular dosing, source boundary, NONMEM identifiability를 방어 가능한 판단으로 묶습니다.
...
>
> $$
> \underbrace{CL\times\frac{1}{3}}_{\text{낮은 clearance 가정}}
> \;\approx_{\text{early sample likelihood에서 구분 어려움}}\;
> \underbrace{V\times3}_{\text{큰 분포용적 가정}},
> \qquad
> \underbrace{t\geq4\times t_{1/2}}_{\text{CL 식별을 위한 늦은 sampling anchor}}
> $$
>
> - **항별 의미:** 좌측 비교는 early sample에서 서로 다른 $CL$/$V$ 조합이 비슷한 농도 예측을 만들 수 있음을 보여줍니다. 오른쪽 조건은 $CL$ 정보를 얻기 위해 더 늦은 sampling이 필요하다는 판단 기준입니다.
> - **실무 직관:** early dense sampling은 $V$에는 민감하지만 $CL$에는 약할 수 있습니다. $CL$ shrinkage와 RSE가 커지면 posterior 숫자를 individual dosing에 그대로 쓰기보다 sampling design을 의심해야 합니다.
> - **오해 방지:** fit이 성공했다는 것은 모든 parameter가 식별되었다는 뜻이 아닙니다. 식별가능성은 objective function 성공 여부가 아니라 sampling geometry와 연결됩니다.
...
> **핵심 메시지:** §8은 Session 016의 개별 계산들을 임상 계량약리학의 반복 판단 패턴으로 압축합니다.


## A. 이 세션의 위치
<!-- SLIDE_START: S43 | title: 이 세션의 위치 | source_anchor: section-8-a-position -->
> **핵심 메시지:** 이 세션은 model building과 clinical dosing decision 사이의 연결부를 담당합니다.
...
> **핵심 메시지:** Renal dosing, TCS, missed-dose risk, CL 해석은 모두 두 축 이상의 결합 판단으로 읽어야 합니다.
...
>
> $$
> \underbrace{R_d}_{\text{renal maintenance 조정}}
> \sim
> \underbrace{f_e\times RF}_{\text{약물 속성 × 환자 속성}},
> \qquad
> \underbrace{\text{forgiveness}}_{\text{missed-dose 내성}}
> \sim
> \underbrace{\frac{t_{1/2}}{\tau}\times\text{therapeutic window}}_{\text{시간 여유 × 농도 여유}},
> \qquad
> \underbrace{TCS}_{\text{monitoring 전략}}
> \sim
> \underbrace{risk\times measurability}_{\text{위험 × 측정 가능성}}
> $$
>
> - **항별 의미:** 좌변들은 임상 판단 대상이고, 우변들은 각각 두 축의 곱 또는 결합으로 그 판단을 움직이는 조건입니다.
> - **실무 직관:** 한 축만 커져도 다른 축이 작으면 최종 판단은 제한될 수 있습니다. 이 메타패턴은 renal dosing, missed-dose counseling, TDM 도입 판단을 같은 사고 구조로 묶어 줍니다.
> - **오해 방지:** `~`는 엄밀한 동일식이 아니라 판단 구조의 유사성을 나타냅니다. 실제 용량 계산식으로 그대로 쓰는 표현이 아닙니다.
...
> **핵심 메시지:** 이 자료의 전문성은 source boundary 안에서 모델 후보, 질환 보정, posterior, dosing을 연결하는 데 있습니다.
...
> **핵심 메시지:** PK15는 처방 자체가 아니라 선택된 용량의 exposure와 safety margin을 보고하는 마지막 layer입니다.
...
> **핵심 메시지:** 최종 주의 목록은 source boundary를 넘어 deterministic overclaim이나 규제·소프트웨어 외삽으로 미끄러지는 것을 막습니다.
