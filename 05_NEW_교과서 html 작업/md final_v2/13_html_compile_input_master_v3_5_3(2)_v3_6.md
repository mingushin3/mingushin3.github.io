...
<!-- SLIDE_START: S01 | title: 학습자 네비게이션 안내(Learner Navigation Aid) | source_anchor: auto-s01 -->
> **핵심 메시지:** 먼저 변이 축의 전체 지도를 잡고, 카드별로 η·ε·covariate·subgroup 판단을 순서대로 고정해야 합니다.

## 학습자 네비게이션 안내(Learner Navigation Aid)

**이 교재 활용법**

1. §1을 한 번 통독하여 전체 변이 축 지도를 잡습니다. C1은 변이 분류 체계, C2는 잔차 모델, C3는 공변량 모델링과 재모수화의 구분, C4는 매끄러운 IIV 안에 숨은 하위군 구조를 다룹니다.
2. §2 카드를 순서대로 공부합니다. `FIGURE_POINTER`를 만날 때마다 해당 교과서 그림을 직접 확인합니다(이미지 저작권 문제로 원본 이미지는 삽입되지 않습니다).
3. §5를 활용해 주요 혼동쌍이 정리되었는지 점검합니다.
4. §8을 다시 읽기 전에 §7에 먼저 답합니다.
...
- CL, V, $t_{1/2}$, protein binding, NCA 요약, 기본 NONMEM control-stream 표기법의 의미를 확인합니다.
- 다음 질문을 염두에 두고 시작합니다: 관측된 변이는 PK인가, PD인가, $\eta$인가, $\varepsilon$인가, 공변량으로 설명 가능한가, 아니면 하위군/분포 구조에서 비롯된 것인가?

**완료 후 점검**

- $\theta$만으로는 왜 부족한지 설명할 수 있어야 합니다.
- 잔차 오차 모델의 잘못된 지정이 왜 가짜 IIV 또는 가짜 공변량 신호를 만드는지 설명할 수 있어야 합니다.
- PK50이 단순히 "fu가 $\omega^2$를 줄였다"는 이야기가 아니라, 좌표계 변환과 재모수화에 관한 수업임을 설명할 수 있어야 합니다.
- 이봉형(bimodal) 농도 또는 $\eta$ 분포를 매끄러운 로그정규 IIV 하나로 가려서는 안 되는 이유를 설명할 수 있어야 합니다.

---

<!-- SLIDE_START: S02 | title: 세션 13(Session 13) — 개체 간 변이(IIV/BSV)와 공변량 모델링 | source_anchor: auto-s02 -->
> **핵심 메시지:** Session 13은 평균 파라미터가 아니라 평균 주변의 개인차와 잔차를 읽는 훈련입니다.

# 세션 13(Session 13) — 개체 간 변이(IIV/BSV)와 공변량 모델링

**출처 고정(Source lock)**: R&T 5e Ch.12–13 (p.361–410); G&W 5e Ch.4 (p.333–352) + PK50 (p.704–710)  
**학습자 본문 범위(Learner-facing scope)**: §1 → §2 → §5 → §7 → §8  
**그림 안내(Figure note)**: 교과서 원그림은 삽입하지 않고, 승인된 figure pointer 또는 새 schematic brief만 사용합니다.

---

<!-- SLIDE_START: S03 | title: §1 — 세션 헤더 및 로드맵(Session Header & Roadmap) | source_anchor: auto-s03 -->
> **핵심 메시지:** C1→C2→C3→C4의 순서는 taxonomy에서 residual, covariate, subgroup으로 가는 변이 정리 순서입니다.
...
<!-- MASTER LENS -->
<!-- SLIDE_START: S04 | title: 핵심 아이디어(Big Idea) | source_anchor: auto-s04 -->
> **핵심 메시지:** NONMEM 혼합효과 모델의 핵심은 관측값을 인구 평균, 개인 편차, 잔차 noise로 분해하는 것입니다.

### 핵심 아이디어(Big Idea)

<!-- ANNOTATION -->NONMEM 혼합효과 모델(mixed-effects model)(← 고정효과와 무작위효과를 함께 추정하는 구조)의 본질은 관측값을 **$\theta$(인구 평균), $\eta$(개체 편차), $\varepsilon$(잔차 noise)** 로 나누어 보는 데 있습니다. 그다음 각 흔들림의 크기인 **$\omega^2$와 $\sigma^2$**를 따로 추정합니다. 공변량(covariate)은 $\omega^2$ 안에 섞여 있던 생리학적 원인을 꺼내는 도구이고, 유전 다형성(genetic polymorphism)은 매끄러워 보이는 $\omega^2$ 안에 숨어 있는 불연속 봉우리입니다. `[R&T p.369–373; p.393–410]`

G&W는 이론보다 먼저 데이터의 모양을 보라고 합니다. 따라서 공변량 모델은 control stream에서 바로 시작하지 않습니다. 먼저 spaghetti plot · dose-normalized plot · transformed plot에서 "어떤 개체들이 왜 갈라지는가"를 확인해야 합니다. `[G&W p.334–336]`

<!-- SLIDE_START: S05 | title: 체화 노트(Mastery Note) — [CRUCIBLE_DERIVED] · Variance Conservation 작업 가설 | source_anchor: auto-s05 -->
> **핵심 메시지:** ω²와 σ²는 같은 미설명 변이를 두고 경쟁하므로, 변이 그릇 배정 순서가 모델 해석을 결정합니다.

> **체화 노트(Mastery Note) — [CRUCIBLE_DERIVED] · Variance Conservation 작업 가설**  
> 이 세션의 모든 카드를 관통하는 단 하나의 작업 가설은 **보존 법칙(conservation law)**입니다. 같은 데이터셋 안에서 $\omega^2$와 $\sigma^2$는 같은 총 미설명 변이(total unexplained variability)를 두고 zero-sum으로 경쟁합니다. $\varepsilon$가 흡수하지 못한 분산은 반드시 $\eta$로 새고, 그 반대도 같습니다. 따라서 C1(taxonomy) → C2(잔차 그릇 정하기) → C3($\eta$에서 설명 가능한 부분 옮기기) → C4($\eta$ 분포의 모양 점검) 순서가 결정됩니다. 이 순서를 어기면 NONMEM은 돌아가지만 dose individualization은 무너집니다.

<!-- SLIDE_START: S06 | title: 로드맵(Roadmap) | source_anchor: auto-s06 -->
> **핵심 메시지:** C1→C2→C3→C4의 순서는 taxonomy에서 residual, covariate, subgroup으로 가는 변이 정리 순서입니다.
...
<!-- SLIDE_START: S07 | title: 지식 그래프상 위치(Knowledge Graph Position) | source_anchor: auto-s07 -->
> **핵심 메시지:** Session 13은 후속 FOCE, GOF, covariate selection, Bayesian TDM을 해석하게 해 주는 variance-axis 기반입니다.

### 지식 그래프상 위치(Knowledge Graph Position)

| 위치 | 내용 |
|---|---|
| 선행 전제 | 1-구획·2-구획 모델, Cl/V/$t_{1/2}$, 단백 결합(protein binding), NONMEM 기초 syntax |
| 본 세션 | IIV / 시점 간 변이(IOV) / 잔차 미설명 변이(RUV), $\theta$/$\eta$/$\varepsilon$, $\omega^2$/$\sigma^2$, 공변량(covariate), 유전형/표현형 변이(genotype/phenotype variability) |
| 직후 후속 | FOCE / FOCE-I, 적합도 진단(GOF) / CWRES / $\eta$-EBE, 수축(shrinkage), 단계적 공변량 선택(stepwise covariate selection) |
| 임상 번역 | Bayesian TDM, 용량 개체화(dose individualization), 치료적 창(therapeutic window), 집단 약동학 보고서 일관성(PopPK report consistency) |

<!-- SLIDE_START: S08 | title: v3.5.3 최소 브리지 — IIV, IOV, RUV는 서로 다른 변이 그릇입니다 | source_anchor: auto-s08 -->
> **핵심 메시지:** IIV는 사람 간 차이, IOV는 같은 사람 안의 occasion 간 차이, RUV는 관측값 수준의 잔차입니다.

### v3.5.3 최소 브리지 — IIV, IOV, RUV는 서로 다른 변이 그릇입니다

`[실무 통합, v3.5.3]`

IIV(inter-individual variability)는 **사람 사이의 지속적 차이**입니다. 어떤 개인은 전반적으로 CL이 높고, 다른 개인은 전반적으로 CL이 낮은 것처럼, 개인의 평균적인 위치가 모집단 중심에서 벗어나는 변이를 담습니다. IOV(inter-occasion variability)는 **같은 사람 안에서 occasion, period, cycle, visit, treatment occasion 사이에 반복적으로 달라지는 차이**입니다. 같은 환자라도 한 주기와 다음 주기, 첫 방문과 재방문, induction이 진행 중인 시점과 회복된 시점의 PK가 달라질 수 있습니다. RUV(residual unexplained variability)는 **관측값 수준의 잔차 오차**입니다. 같은 개인, 같은 occasion, 같은 구조 모델로 설명한 뒤에도 남는 측정·관측 수준의 흔들림입니다.

| 변이 그릇 | 질문 | 실무 해석 |
|---|---|---|
| IIV | 사람마다 지속적으로 다른가? | 개인의 평균적 위치 차이를 $\omega^2$가 담습니다. |
| IOV | 같은 사람 안에서 occasion마다 달라지는가? | visit/cycle/period/treatment occasion별 shift를 별도 random effect로 분리해 담습니다. |
| RUV | 관측값 하나하나에서 남는 오차인가? | prediction 주변의 residual spread를 $\sigma^2$가 담습니다. |

IOV가 중요한 이유는 변이의 배정 오류 때문입니다. IOV를 무시하면 같은 사람 안의 occasion별 변화가 IIV 또는 RUV로 잘못 흡수될 수 있습니다. 그 결과 $\omega^2$, $\sigma^2$, EBE, shrinkage, covariate signal 해석이 모두 왜곡될 수 있습니다. 특히 repeated period, crossover, cycle-based oncology dosing, TDM 재방문, enzyme induction/inhibition recovery, adherence fluctuation이 있는 자료에서는 “사람이 다른 것”과 “같은 사람의 occasion이 다른 것”을 분리해야 합니다.

NONMEM 실무 직관에서 IOV는 보통 occasion-level ETA 또는 occasion-specific random effect로 구현됩니다. 그러나 핵심은 단순히 `$OMEGA` block을 늘리는 일이 아닙니다. 핵심 질문은 **어느 변이 그릇에 어떤 반복 구조를 담을 것인가**입니다. 같은 개인 안의 visit/cycle/period별 systematic shift가 보이는가, residual plot만으로 설명되지 않는 occasion별 bias가 남는가, 공변량 효과로 보이는 신호가 사실 occasion imbalance 때문일 가능성이 있는가, IOV를 넣으면 IIV/RUV 또는 covariate effect가 해석 가능하게 정리되는가를 먼저 확인해야 합니다.

**핵심 한 줄**: IIV는 사람 간 차이, IOV는 같은 사람 안의 occasion 간 차이, RUV는 관측값 수준의 남은 오차입니다.

<!-- RECAP -->
**§1 요약(Recap)**: 이 세션의 질문은 "사람마다 다르다"가 아닙니다. 질문은 "그 다름 중 무엇이 $\theta$, 무엇이 $\eta$, 무엇이 $\varepsilon$, 무엇이 공변량(covariate)인가"입니다.

---

<!-- SLIDE_START: S09 | title: §2 — 개념 해부 카드(Concept Anatomy Cards) | source_anchor: auto-s09 -->
> **핵심 메시지:** 이 slide에서는 하나의 개념을 하나의 실무 판단으로 압축해 다음 모델링 단계와 연결해야 합니다.

## §2 — 개념 해부 카드(Concept Anatomy Cards)

---

<!-- SLIDE_START: S10 | title: C1. 변이의 구조적 분해 — $\theta$, $\eta$, $\varepsilon$ 분류 체계(taxonomy) | source_anchor: auto-s10 -->
> **핵심 메시지:** θ는 중심, η는 개인 위치, ε는 관측 잔차이며 이 세 자리가 분리되어야 OMEGA와 SIGMA가 의미를 가집니다.
...
$\theta$는 인구의 무게중심입니다. $\eta$는 각 개인이 그 중심에서 벗어난 정도이고, $\varepsilon$은 같은 개인을 같은 조건에서 다시 측정해도 남는 잔차입니다. 이 세 자리가 분리되지 않으면 `$OMEGA`와 `$SIGMA`는 숫자는 있어도 의미가 없습니다. `[R&T p.369–373]`

<!-- SLIDE_START: S11 | title: A. 형식적 정의(Formal Definition) | source_anchor: auto-s11 -->
> **핵심 메시지:** 이 slide에서는 현재 개념이 무엇을 예측·정의하고 어떤 항으로 분해되는지 먼저 고정해야 합니다.
...
>
> $$
> \underbrace{Y_{ij}}_{i번째\ 개인의\ j번째\ 관측값}=\underbrace{f(\boldsymbol{\theta},\boldsymbol{\eta_i},x_{ij})}_{구조모델이\ 예측한\ 중심값}+\underbrace{g(\boldsymbol{\theta},\boldsymbol{\eta_i},x_{ij})}_{오차\ 크기를\ 정하는\ 스케일}\underbrace{\varepsilon_{ij}}_{관측\ 수준\ 잔차}
> $$
>
> - **항별 의미:** 좌변은 개인 i의 j번째 관측값을 예측·정의합니다. 우변의 `f`는 θ와 η가 반영된 구조적 예측, `g`는 잔차 오차가 예측값에 어떻게 붙는지 정하는 스케일, ε는 관측 수준 noise입니다.
> - **실무 직관:** η가 커지면 개인별 예측 위치가 이동하고, g 또는 ε의 분산이 커지면 같은 개인 안의 관측값 산포가 커집니다.
> - **오해 방지:** 이 식은 모든 오차를 하나로 합치는 식이 아니라, 개인 간 변이와 관측 잔차를 서로 다른 층으로 나누는 식입니다.


- **$\theta$**: 고정효과(fixed effect), 모집단의 대표값(population typical value).
- **$\eta_i$**: 개체 간 편차(interindividual deviation); 분산-공분산 행렬이 **$\Omega$**, 대각 원소(diagonal element)가 **$\omega^2$**.
- **$\varepsilon_{ij}$**: 잔차 편차(residual deviation); 분산이 **$\Sigma$/$\sigma^2$**.

<!-- ANNOTATION -->여기서 $\Omega$/$\omega^2$는 개인 간 변이의 크기이고, $\Sigma$/$\sigma^2$는 관측 잔차의 크기입니다. 두 값은 서로 다른 "오차"가 아니라 서로 다른 수준(level)의 변이를 가리킵니다.
...
>
> $$
> \underbrace{CL_i}_{개인\ i의\ 청소율}=\underbrace{CL}_{모집단\ typical\ clearance}\cdot\underbrace{\exp(\eta_i)}_{개인별\ 상대\ 배율,\ 항상\ 양수}
> $$
>
> - **항별 의미:** 좌변은 개인별 CL입니다. 우변의 CL은 모집단 중심값이고, exp(ηi)는 그 개인이 중심값에서 몇 배 벗어나는지를 나타냅니다.
> - **실무 직관:** ηi가 0이면 개인 CL은 typical CL과 같고, ηi가 양수면 CLi는 증가하며, 음수면 감소하지만 음수가 되지는 않습니다.
> - **오해 방지:** η는 CL에 더하는 절대량이 아니라 log-scale의 개인 편차입니다. 따라서 일정한 CV에 가까운 해석이 가능합니다.


R&T는 지수형 오차 모델(exponential error model)이 CL을 음수로 만들지 않고 근사적 일정 변동계수(approximate constant CV)를 제공하기 때문에 집단 분석(population analysis)에서 자주 쓰인다고 설명합니다. `[R&T p.371]` 작은 $\omega$에서는 $CV(\%) \approx \omega\cdot 100$을 교육용 근사로 쓸 수 있습니다.

<!-- SLIDE_START: S12 | title: B. 데이터 앵커(Data Anchor): 평균보다 분포가 먼저입니다 | source_anchor: auto-s12 -->
> **핵심 메시지:** 평균 농도나 평균 용량은 분포의 폭과 모양을 가릴 수 있으므로 개별 프로파일을 먼저 보아야 합니다.

#### B. 데이터 앵커(Data Anchor): 평균보다 분포가 먼저입니다

R&T는 warfarin 200명에서 유사한 항응고 효과(anticoagulation)를 얻기 위한 1일 용량(daily dose)이 넓게 분포한다고 제시합니다. 또한 nortriptyline 25 mg tid를 복용한 263명에서 정상상태 농도(plateau concentration)는 선형 스케일에서는 비대칭(skewed)이고, 로그 스케일에서는 거의 대칭(symmetric)입니다. 즉, 평균만 보면 dose individualization의 문제는 해결되는 것이 아니라 가려집니다. `[R&T p.362–363]`

G&W는 다중 피험자 자료에서 먼저 개별 프로파일(individual profile)과 통합 자료(pooled data)를 그려 보라고 합니다. Figure 4.2의 spaghetti plot은 같은 dose를 받은 일본인(Japanese)과 백인(Caucasian) 그룹이 서로 다른 노출 프로파일(exposure profile)을 보일 수 있음을 보여 주며, 저자들은 오차 막대를 가진 평균 곡선(mean curve with error bars)보다 spaghetti plot이 변이를 더 잘 드러낸다고 설명합니다. `[G&W p.335–336]`

<!-- ANCHOR -->
**NAD/NPD/population bridge**: Naive Averaged Data(NAD)는 각 시간점(time point)의 평균 농도에 모델을 맞춥니다. Naive Pooled Data(NPD)는 모든 관측값을 한 개인의 자료처럼 적합합니다. G&W는 평균 자료에 대한 적합(mean data fitting)이 편향된 파라미터와 변이를 만들 수 있어 피해야 하며, 대안으로 집단 접근법(population approach)이 필요하다고 설명합니다. `[G&W p.335–336]`

<!-- SLIDE_START: S13 | title: C. 구조적 필요성(Structural Necessity) | source_anchor: auto-s13 -->
> **핵심 메시지:** 이 개념은 모델 숫자를 임상 판단으로 바꾸기 위해 반드시 필요한 구조적 조건입니다.

#### C. 구조적 필요성(Structural Necessity)

1. **$\theta$ 하나만으로는 부족하다**: 평균 청소율(mean clearance)만 있으면 "일반 환자"의 농도는 예측하지만, 좁은 치료적 창(narrow therapeutic window)에서 독성 또는 무효 노출을 겪을 개인을 설명하지 못합니다. `[R&T p.370]`
2. **$\omega^2$와 $\sigma^2$는 같은 미설명 변이를 두고 경쟁한다**: 잔차 모델이 설명하지 못한 분산은 $\omega^2$로 샙니다. 반대로 개인 간 변이를 너무 크게 허용하면 $\sigma^2$가 작아 보일 수 있습니다. 이 보존 법칙을 보지 못하면 C2의 잔차 모델(residual model)과 C3의 공변량 모델(covariate model)은 모두 흔들립니다.
3. **Population analysis는 동시 추정이다**: R&T는 표준 2단계 분석(standard two-stage)이 풍부한 표본(rich sampling)을 요구하는 반면, 집단 분석(population analysis)은 희소 자료(sparse data)를 합쳐 구조 파라미터(structural parameter)와 분산 파라미터(variance parameter)를 동시에 최적화한다고 설명합니다. `[R&T p.369]`
4. **OFV는 적합의 언어다**: 최대 가능도(maximum likelihood)는 $\theta$, ω, σ를 동시에 조정하여 OFV를 최소화합니다. 내포된 모델(nested model)에서는 $\Delta OFV$ 3.84(df=1, α=0.05)가 가능도비 검정(LRT) 기준이고, 비내포 모델(non-nested model)에서는 AIC가 쓰입니다. `[R&T p.373]`

<!-- SLIDE_START: S14 | title: D. 경계 조건(Boundary Conditions) | source_anchor: auto-s14 -->
> **핵심 메시지:** 모델 해석은 성립 조건과 source-boundary를 벗어나는 순간 쉽게 과잉해석으로 바뀝니다.

#### D. 경계 조건(Boundary Conditions)

| 경계 조건 | 위반 시 신호 |
|---|---|
| $\eta$ 분포가 단봉형(unimodal)/로그정규(log-normal)이어야 함 | 이봉형(bimodal) 히스토그램이면 유전형/표현형 하위군(genotype/phenotype subgroup) 가능성. `[R&T p.366, p.393–397]` |
| $\varepsilon$가 평균 0, 분산 $\sigma^2$의 무작위 잔차여야 함 | 잔차 추세(residual trend)가 남으면 구조 모델 또는 오차 모델이 변이의 원인을 설명하지 못한 것. `[R&T p.372]` |
| 희소 자료(sparse data)도 같은 모집단에서 왔다고 볼 수 있어야 함 | 이질적 모집단(outlier population)이 섞이면 모집단 평균과 $\omega^2$가 모두 왜곡. |
| 공변량이 식별 가능해야 함 | G&W의 식별가능성(identifiability)/추정가능성(estimability) 문제처럼, 데이터가 정보를 주지 않으면 파라미터는 숫자로만 존재합니다. `[G&W p.348–351]` |

<!-- SLIDE_START: S15 | title: E. 제텔카스텐 원자(Zettelkasten Atom) | source_anchor: auto-s15 -->
> **핵심 메시지:** 이 slide의 키워드는 이후 모델 진단과 보고서 언어를 연결하는 검색 가능한 지식 원자입니다.
...
<!-- RECAP -->
**C1 요약(Recap)**: $\theta$는 중심, $\eta$는 개인의 위치, $\varepsilon$은 남은 흔들림입니다. C2는 $\varepsilon$의 형태를 정하고, C3는 $\eta$ 안의 설명 가능한 부분을 꺼내며, C4는 $\eta$ 분포가 매끄럽지 않을 때의 생물학적 이유를 찾습니다.

---


<!-- SLIDE_START: S16 | title: Figure pointer | source_anchor: auto-s16 -->
> **핵심 메시지:** 교과서 figure는 본문 주장을 눈으로 검증하는 독립 slide로 확인해야 합니다.

<!-- FIGURE_POINTER -->
Source: Rowland & Tozer 5e, p.366, Figure 12-6 — 네 가지 가상 약물 청소율(clearance)의 빈도분포.
Why this matters: 같은 평균(mean)을 가져도 CV와 분포 모양(distribution shape)이 다르면 개별 용량 결정(individual dose decision)의 의미가 완전히 달라집니다. 특히 이봉형 분포(bimodal distribution)에서는 평균이 모집단(population)의 대표값이 아니라 오히려 가장 덜 그럴듯한 값이 될 수 있습니다.
When to look: 이 카드를 읽은 뒤 확인하시면 됩니다.
Learner instruction: A/B/C에서 평균(mean)은 같지만 산포(spread)가 달라지는 점을 먼저 보세요. 그다음 D에서 두 봉우리 사이에 평균이 놓이는 장면을 보고, 왜 $\theta$만으로는 하위군 위험(subgroup risk)을 설명할 수 없는지 연결하시면 됩니다.
<!-- /FIGURE_POINTER -->

> **체화 노트(Mastery Note) — [CRUCIBLE_DERIVED]**  
> C1의 핵심은 $\theta$, $\eta$, $\varepsilon$를 외우는 것이 아니라, 한 데이터셋 안의 변이가 어느 그릇에 배정되는지 보는 것입니다. $\varepsilon$가 설명하지 못한 패턴은 $\eta$처럼 보이고, $\eta$로 설명해야 할 구조가 빠지면 잔차처럼 남기 때문에 C1은 이후 모든 모델 진단의 좌표축입니다.

<!-- SLIDE_START: S17 | title: 체화 노트(Mastery Note) — [CRUCIBLE_DERIVED] · `$OMEGA` 구조를 `$THETA`보다 먼저  | source_anchor: auto-s17 -->
> **핵심 메시지:** OMEGA block 구조는 모델이 개인 간 생리학적 상관을 어떻게 가정했는지 가장 먼저 드러냅니다.

> **체화 노트(Mastery Note) — [CRUCIBLE_DERIVED] · `$OMEGA` 구조를 `$THETA`보다 먼저 보라**  
> 30년 베테랑이 NONMEM control stream을 받으면 가장 먼저 보는 줄은 `$THETA`가 아니라 `$OMEGA` block 구조입니다. Block diagonal과 full block의 결정은 Cl–V correlation 같은 mechanism-level 가정을 코드 한 줄로 인코딩합니다. 같은 organ size에 의해 함께 변할 것으로 예상되는 파라미터(예: Cl과 V)는 `$OMEGA BLOCK(2)`로 시작하는 것이 정도이며, diagonal은 데이터가 상관 없음을 강하게 시사할 때 후퇴 옵션으로 둡니다. **체화 꿀팁**: $\eta$_Cl과 $\eta_V$ scatter에 명백한 직선 추세가 있는데 diagonal로 적합하면, 모델은 그 상관을 $\varepsilon$로 흡수하거나 가짜 covariate로 포장한다 — 즉 Wall 1(보존 법칙)이 즉시 작동합니다. **실무 활용**: code review의 첫 30초에 `$OMEGA` 구조 → `$SIGMA` 구조 → 그 다음 `$THETA` 순서로 읽으면 모델이 어떤 mechanism-level prior를 코드화했는지 한 눈에 보입니다. (※ 본 직관은 Crucible 추출이며, NONMEM 문법 자체는 R&T/G&W 본문에 직접 인쇄되지 않은 `[교과서 외 구현 번역]` 영역입니다.)

<!-- SLIDE_START: S18 | title: 📋 실무 체화 요약(Practice Brief) — C1 · [CRUCIBLE_DERIVED, v3] | source_anchor: auto-s18 -->
> **핵심 메시지:** Practice Brief의 목적은 개념을 control stream, GOF, 보고서 문장으로 즉시 옮기는 것입니다.

> **📋 실무 체화 요약(Practice Brief) — C1 · [CRUCIBLE_DERIVED, v3]**  
> **목표**: $\theta$/$\eta$/$\varepsilon$ 좌표축을 control stream 한 장에서 즉시 읽어 내는 능력 확립.  
> **연습 1 (5분, 코드 독해)**: 임의의 PopPK control stream을 받았을 때 다음 순서로 읽습니다 — (a) `$OMEGA` block 구조, (b) `$SIGMA` 구조, (c) `$THETA` 초기값. 각 단계에서 한 줄로 답합니다: *"이 모델은 Cl–V 상관을 [있다 / 없다]고 가정합니다", "잔차는 [additive / proportional / combined] 구조다", "$\theta$에는 [N]개의 fixed effect가 있습니다."*  
> **연습 2 (10분, 분산 분류)**: 임의의 IIV 추정 결과(예: $\omega^2$(CL) = 0.09, $\omega^2$(V) = 0.12, $\sigma^2$(prop) = 0.04)에 대해 다음 자문 — *"이 변이의 어느 부분이 생리학적으로 설명 가능한가? 어느 부분이 모델 결함의 흡수통로일 수 있는가? 이 $\omega^2$ 크기를 보고 어떤 covariate를 a priori 후보로 잡겠는가?"*  
> **자기 점검 신호**: `$OMEGA`를 보지 않고 `$THETA`부터 읽기 시작했다면 1번부터 다시 합니다. C1의 좌표축이 잡히지 않은 채 C2–C4로 진입하면 모든 후속 진단이 흔들립니다.

<!-- SLIDE_START: S19 | title: 🛡 v3.4 실무 경고: shrinkage와 EBE는 $\eta$를 '보았다'는 착각을 만들 수 있다 · [EXPERT_INF | source_anchor: auto-s19 -->
> **핵심 메시지:** EBE는 개인 참값이 아니라 모델과 자료가 허용한 조건부 η 추정치이므로 shrinkage를 항상 함께 보아야 합니다.

> **🛡 v3.4 실무 경고: shrinkage와 EBE는 $\eta$를 '보았다'는 착각을 만들 수 있다 · [EXPERT_INFERENCE — PopPK implementation guardrail; textbook scope를 넘는 실무 진단 개념이므로 source-boundary를 명시함]**  
>
> **A. 정의(Definition)**  
> 경험적 베이즈 추정치(EBE; empirical Bayes estimate)는 모집단 수준 파라미터($\theta$, $\omega^2$, $\sigma^2$)와 그 개인의 관측치를 함께 사용하여 각 개인의 $\eta$를 사후추정(posterior estimate)한 값입니다. 즉 EBE는 개인의 "참값(true value)"이 아니라, 모델 가정과 그 사람이 제공한 자료의 양·질이 허용하는 범위 안에서 도출된 조건부 추정치입니다. 수축(shrinkage)은 그 개인이 제공한 정보가 충분하지 않을 때 EBE가 개인의 실제 $\eta$ 차이를 충분히 드러내지 못하고 모집단 평균(0) 쪽으로 끌려가는 현상을 가리킵니다.  
>
> **B. 실무 경고 — $\eta$-shrinkage 임계값(Operational Warning)**  
> $\eta$-shrinkage가 높으면 개체간 변이(IIV; interindividual variability)가 실제보다 작아 보이고, 개인 간 차이가 평탄화된 채 보고됩니다. 관행적으로 **$\eta$-shrinkage가 약 30%를 넘으면** EBE 기반 산점도(scatter plot), $\eta$-vs-공변량(covariate) plot, 그리고 EBE에 의존한 공변량 분석(covariate analysis)의 해석을 매우 조심해야 합니다. 단, **"30%"는 절대적인 법칙이 아니라 경고 임계값(rule-of-thumb threshold)이며**, sampling density·자료 설계·model parsimony에 따라 더 보수적으로 잡아야 할 때도 있습니다.  
>
> **C. 공변량 분석에서 발생하는 두 방향의 위험(Covariate Analysis Risk)**  
> *(1) 거짓 음성(false negative)*: $\eta$-shrinkage가 높을 때는 공변량(covariate)이 실제로 $\eta$와 관련이 있어도 $\eta$-vs-covariate plot에서 그 신호가 약하게 보이거나 사라질 수 있습니다. *(2) 거짓 양성(false positive)*: 잔차 오차 모델(residual error model)이 잘못 지정되어 $\varepsilon$가 $\eta$로 새면($\eta$–$\varepsilon$ leakage), 모델은 본래 잔차/잔차변이(RUV; residual unexplained variability)에 속해야 할 패턴을 $\eta$에 흡수하고 그 결과로 가짜 공변량 신호가 생깁니다. 이 두 위험은 모두 File 13의 보존 법칙(conservation law)에 직접 연결된다 — **$\omega^2$와 $\sigma^2$는 같은 unexplained variability를 두고 경쟁하며, 한쪽의 오지정(misspecification)은 반대쪽으로 새어 들어가 EBE의 개인별 모양 자체를 왜곡합니다.**  
>
> **D. 한 줄 기억 고리(Memory Hook)**  
> EBE는 개인의 참값이 아니라 모델과 자료가 허용한 조건부 추정치이며, shrinkage가 높을수록 그 개인별 모양은 더 조심해서 보아야 합니다.  
>
> **E. 범위 제한(Do Not Overexpand)**  
> 상세 알고리즘 유도는 Volume II / NONMEM module 범위이지만, EBE와 shrinkage 해석에 필요한 최소 직관은 여기서 잠급니다.

<!-- SLIDE_START: S20 | title: 최소 추정법 브리지: FOCE/FOCEI/Laplacian/SAEM이 EBE와 shrinkage 해석에 미치는 영향 [EXPERT_INFERENCE, v3.5.2] | source_anchor: auto-s20 -->
> **핵심 메시지:** EBE는 개인 참값이 아니라 모델과 자료가 허용한 조건부 η 추정치이므로 shrinkage를 항상 함께 보아야 합니다.

### 최소 추정법 브리지: FOCE/FOCEI/Laplacian/SAEM이 EBE와 shrinkage 해석에 미치는 영향 [EXPERT_INFERENCE, v3.5.2]

EBE(empirical Bayes estimate)는 개체별 “진짜 값”이 아니라, population model, observation richness, residual error model, estimation method가 함께 만든 조건부 추정치입니다. 따라서 ETA-EBE plot은 원자료를 직접 보는 창이 아니라, 모델과 추정법을 통과한 그림입니다. FOCE는 각 개인의 ETA 주변에서 likelihood를 근사해 개체별 정보를 population 구조에 연결합니다. FOCEI는 residual error가 prediction 또는 ETA와 상호작용할 때 그 interaction을 더 명시적으로 반영한다는 점에서, proportional/combined error처럼 오차 크기가 예측값에 의존하는 상황의 해석에 중요합니다. Laplacian은 비선형성이 강하거나 discrete/categorical likelihood, 복잡한 residual likelihood처럼 곡률(curvature)이 해석을 좌우할 때 근사의 질이 중요해지는 도구입니다. SAEM은 강한 비선형성, 복잡한 random effect, sparse data에서 simulation-based estimation으로 탐색 공간을 다루기 쉬울 수 있지만, 항상 우월한 방법이 아니라 문제 구조에 따라 선택하는 다른 도구입니다.
...
실무 기억 고리: **EBE plot은 현미경이 아니라, 모델·자료·추정법이 함께 만든 그림자입니다.** `[EXPERT_INFERENCE, v3.5.2]`

<!-- SLIDE_START: S21 | title: C2. 잔차 오차 모델 — 가법형(additive) / 비례형(proportional) / 지수형(exponential) | source_anchor: auto-s21 -->
> **핵심 메시지:** ε의 구조를 잘못 선언하면 residual error가 IIV 또는 covariate signal처럼 보입니다.
...
잔차 오차 모델(residual error model)은 "분석 측정(assay)과 관측 과정이 어떤 방식으로 틀리는가"에 대한 선언입니다. $\varepsilon$를 잘못 지정하면, 실제로는 측정 오차나 잔차 모델 구조에서 비롯된 문제가 마치 개체 간 변이($\omega^2$)나 공변량 효과처럼 보이게 됩니다. `[R&T p.371–373]`

<!-- SLIDE_START: S22 | title: A. 형식적 정의(Formal Definition) | source_anchor: auto-s22 -->
> **핵심 메시지:** 이 slide에서는 현재 개념이 무엇을 예측·정의하고 어떤 항으로 분해되는지 먼저 고정해야 합니다.

#### A. 형식적 정의(Formal Definition)

R&T가 제시하는 잔차 변이(residual variability) 모델은 다음 세 가집니다. `[R&T p.372]`
...
>
> $$
> \underbrace{Y}_{관측값}=\underbrace{F}_{모델\ 예측값}+\underbrace{\varepsilon}_{일정\ 절대오차}
> $$
>
> - **항별 의미:** 좌변은 관측값이고, F는 구조 모델 예측값, ε는 농도 크기와 무관하게 더해지는 잔차입니다.
> - **실무 직관:** ε의 분산이 커지면 모든 농도 범위에서 비슷한 절대폭의 산포가 커집니다.
> - **오해 방지:** 가법 오차는 낮은 농도에서 음수 예측 또는 과도한 상대오차 문제를 만들 수 있어 자료 범위와 함께 판단해야 합니다.


> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{Y}_{관측값}=\underbrace{F}_{모델\ 예측값}\left(1+\underbrace{\varepsilon}_{상대오차}\right)
> $$
>
> - **항별 의미:** 좌변은 관측값이고, F는 예측값이며, ε는 예측값에 비례해 붙는 상대 잔차입니다.
> - **실무 직관:** F가 커질수록 잔차의 절대폭도 커지므로 일정 CV에 가까운 assay/관측 구조를 표현합니다.
> - **오해 방지:** 비례오차만으로 LLOQ 근처의 절대오차를 설명하려 하면 저농도 잔차가 η 또는 covariate signal처럼 샐 수 있습니다.


> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{Y}_{관측값}=\underbrace{F}_{모델\ 예측값}\exp\left(\underbrace{\varepsilon}_{로그\ 영역\ 잔차}\right)
> $$
>
> - **항별 의미:** 좌변은 관측값이고, F는 예측값이며, exp(ε)는 양수 배율 형태의 잔차입니다.
> - **실무 직관:** ε가 증가하면 Y는 곱셈적으로 증가하고, 로그 변환 후에는 잔차가 더하기 형태로 단순해집니다.
> - **오해 방지:** 지수형은 “더 정교한 비례오차”가 아니라 양수 보장과 log-scale additive error 해석을 동시에 주는 좌표계입니다.


로그 변환(log transformation) 후 지수형 오차는 다음과 같이 가법형(additive form)이 됩니다. `[R&T p.373]`
...
>
> $$
> \underbrace{\ln Y}_{로그\ 관측값}=\underbrace{\ln F}_{로그\ 예측값}+\underbrace{\varepsilon}_{로그\ 스케일\ 가법\ 잔차}
> $$
>
> - **항별 의미:** 좌변은 관측값을 로그로 바꾼 값이고, 우변은 로그 예측값에 잔차를 더한 구조입니다.
> - **실무 직관:** ε가 증가하면 원래 스케일의 Y는 곱셈적으로 증가하지만, 로그 스케일에서는 단순한 가법 잔차로 보입니다.
> - **오해 방지:** 로그 변환은 단순한 그래프 조작이 아니라 잔차 구조의 좌표계를 바꾸는 모델링 판단입니다.


`[교과서 외 구현 번역]` 결합형/혼합형 잔차 모델(combined/mixed residual model)은 실무 NONMEM에서 자주 쓰이지만, R&T p.372가 직접 열거한 출처 모델은 가법형/비례형/지수형입니다. 따라서 결합형 모델은 source-derived가 아니라 구현 번역으로만 둡니다.

<!-- SLIDE_START: S23 | title: B. 코드 구조(Code Structure) | source_anchor: auto-s23 -->
> **핵심 메시지:** 코드 한 줄은 phenotype effect와 남은 ETA를 어느 그릇에 배정하는지 결정합니다.
...
>
> $$
> \underbrace{Y}_{관측값}=\underbrace{IPRED}_{개인별\ 예측값}+\underbrace{\sqrt{(\theta_{prop}\cdot IPRED)^2+\theta_{add}^2}}_{비례오차와\ 가법오차를\ 합친\ 잔차\ 스케일}\underbrace{\varepsilon}_{표준화\ 잔차}
> $$
>
> - **항별 의미:** 좌변은 관측값이고, IPRED는 개인별 예측값이며, 제곱근 항은 비례 성분과 가법 성분을 하나의 잔차 스케일 W로 결합합니다.
> - **실무 직관:** IPRED가 커지면 비례오차 항이 커지고, 낮은 농도에서는 가법오차 항이 잔차폭을 지탱합니다.
> - **오해 방지:** combined error는 공변량 효과가 아니라 assay/관측 오차가 농도 스케일에 따라 어떻게 달라지는지에 대한 선언입니다.


이 코드는 비례 성분(proportional component)과 가법 성분(additive component)을 하나의 `W`로 합친 구현입니다. 그러나 source-locked 본문에서 중요한 점은 코드가 아니라 **잔차가 무작위(random), 평균 0, 분산 $\sigma^2$이어야 한다**는 R&T의 조건입니다. `[R&T p.372]`

<!-- SLIDE_START: S24 | title: C. 구조적 필요성(Structural Necessity) | source_anchor: auto-s24 -->
> **핵심 메시지:** 이 개념은 모델 숫자를 임상 판단으로 바꾸기 위해 반드시 필요한 구조적 조건입니다.

#### C. 구조적 필요성(Structural Necessity)

<!-- ANCHOR -->
잔차 모델 결정은 공변량 선택(covariate selection)보다 반드시 앞서야 합니다. 예를 들어 고농도에서 잔차 산포(residual spread)가 커지는 자료에 가법 오차만 쓰면, 모델은 그 산포를 설명하려다 $\omega^2$를 과대 추정하거나 가짜 공변량 신호를 만듭니다. 반대로 LLOQ(정량하한치) 근처 절대오차가 큰 자료에 비례 오차만 쓰면, 저농도 잔차 패턴이 체계적으로(systematically) 남습니다.

R&T는 잔차 변이가 완전히 무작위여야 하며, 그렇지 않으면 구조 모델(structural model) 또는 오차 모델이 중요한 변이의 원인을 설명하지 못한 것이라고 명시합니다. `[R&T p.372]`

<!-- SLIDE_START: S25 | title: D. 경계 조건(Boundary Conditions) | source_anchor: auto-s25 -->
> **핵심 메시지:** 모델 해석은 성립 조건과 source-boundary를 벗어나는 순간 쉽게 과잉해석으로 바뀝니다.
...
<!-- SLIDE_START: S26 | title: E. 한계(Limitations) | source_anchor: auto-s26 -->
> **핵심 메시지:** 모델 해석은 성립 조건과 source-boundary를 벗어나는 순간 쉽게 과잉해석으로 바뀝니다.

#### E. 한계(Limitations)

- Shrinkage 공식, 20–30% 임계, Savic & Karlsson 2009는 이 PDF 범위에 없습니다. 본 세션에서는 `[교과서 외 후속 구현 지식]`으로만 남깁니다.
- 희소 표본(sparse sampling)에서 $\eta$-EBE와 잔차 플롯(residual plot)을 해석할 때는, 낮은 shrinkage와 정규분포처럼 보이는 $\eta$ 히스토그램을 좋은 모델의 증거로 단정하지 않습니다. 데이터가 EBE를 실제로 식별할 수 있었는지 먼저 묻습니다.

<!-- TRENCH -->
**실무 팁(Trench-Level Tip)**: C2에서 한 가지를 외운다면 "$\varepsilon$의 실패는 $\eta$의 성공처럼 보입니다"입니다. 잔차 모델을 틀리면 공변량 모델은 정교해지는 것이 아니라 정교하게 틀립니다.

<!-- SLIDE_START: S27 | title: F. 제텔카스텐 원자(Zettelkasten Atom) | source_anchor: auto-s27 -->
> **핵심 메시지:** 이 slide의 키워드는 이후 모델 진단과 보고서 언어를 연결하는 검색 가능한 지식 원자입니다.
...
<!-- RECAP -->
**C2 요약(Recap)**: $\varepsilon$은 남는 오차가 아니라 observation process의 모델입니다. $\varepsilon$가 잘못되면 $\omega^2$와 covariate가 모두 오염됩니다.

---


<!-- SLIDE_START: S28 | title: Figure pointer | source_anchor: auto-s28 -->
> **핵심 메시지:** 교과서 figure는 본문 주장을 눈으로 검증하는 독립 slide로 확인해야 합니다.

<!-- FIGURE_POINTER -->
Source: Rowland & Tozer 5e, p.371, Figure 12-12 — 등분산성(homoscedastic) vs 이분산성(heteroscedastic) 파라미터 변이.
Why this matters: 가법형(additive), 비례형(proportional), 지수형(exponential) 오차 모델은 식만 보면 서로 다른 문법처럼 보입니다. Figure 12-12는 잔차 산포(residual spread)가 일정한지, 예측값(prediction)의 크기에 따라 커지는지를 눈으로 구분하게 해 줍니다.
When to look: 이 카드를 읽은 뒤 확인하시면 됩니다.
Learner instruction: 오차 산포(error spread)가 농도(concentration) 또는 예측 스케일(prediction scale)과 함께 변하는지 확인하세요. 그 패턴을 C2의 "$\varepsilon$를 잘못 선언하면 $\omega^2$ 또는 공변량 효과처럼 보입니다"라는 문장에 다시 연결하시면 됩니다.
<!-- /FIGURE_POINTER -->

<!-- SLIDE_START: S29 | title: 실무 렌즈(Practice Lens) — [CRUCIBLE_DERIVED] | source_anchor: auto-s29 -->
> **핵심 메시지:** 이 slide에서는 하나의 개념을 하나의 실무 판단으로 압축해 다음 모델링 단계와 연결해야 합니다.

> **실무 렌즈(Practice Lens) — [CRUCIBLE_DERIVED]**  
> 공변량 탐색 전에 residual pattern을 먼저 안정화해야 합니다. 잔차 구조가 잘못 지정된 상태에서 공변량을 추가하면, 생리학적 원인을 발견한 것이 아니라 오차 모델의 실패를 공변량이 대신 흡수하는 결과가 됩니다.

<!-- SLIDE_START: S30 | title: 진단 시그니처(Diagnostic Signature) 1 — `ω²–σ² Leakage Funnel` · [CRUCIBLE_D | source_anchor: auto-s30 -->
> **핵심 메시지:** LLOQ 근처 funnel과 ω² inflate가 함께 보이면 covariate보다 residual model 좌표계를 먼저 의심해야 합니다.

> **진단 시그니처(Diagnostic Signature) 1 — `ω²–σ² Leakage Funnel` · [CRUCIBLE_DERIVED]**  
> **시그니처**: proportional-only RUV를 LLOQ-rich data(저농도 관측이 많은 자료)에 적용했을 때, $\omega^2$(CL) 추정치가 비현실적으로 inflate되고 CWRES vs PRED plot에서 LLOQ 근처 funnel 모양과 $\eta$-shrinkage spike가 동시에 나타납니다. **메커니즘**: low concentration에서 절대오차가 큰 자료에 proportional error만 쓰면 잔차가 설명하지 못한 spread가 $\omega^2$로 새어 가짜 IIV를 만듭니다. C1의 보존 법칙이 실시간으로 작동하는 장면입니다. **체화 꿀팁**: "$\omega^2$이 너무 크다"고 느낄 때 가장 먼저 의심할 것은 추가 covariate가 아니라 **잔차 모델 좌표계**다. CWRES plot의 funnel은 covariate 검색이 아니라 RUV 재구성을 가리킵니다. **실무 활용**: assay LLOQ가 관측 범위의 하단에 가깝다면 mixed residual로 a priori 시작하고, additive component는 assay validation 보고서의 SD에 a priori-tied하게 둡니다. post-hoc model selection 인상을 피하려면 mixed 채택 근거(assay characteristic, sampling design)를 모델 선택 시점에 동시 기록합니다.

<!-- SLIDE_START: S31 | title: 📋 실무 체화 요약(Practice Brief) — C2 · [CRUCIBLE_DERIVED, v3] | source_anchor: auto-s31 -->
> **핵심 메시지:** Practice Brief의 목적은 개념을 control stream, GOF, 보고서 문장으로 즉시 옮기는 것입니다.

> **📋 실무 체화 요약(Practice Brief) — C2 · [CRUCIBLE_DERIVED, v3]**  
> **목표**: residual model 선택을 a priori 결정 기준에 묶어 post-hoc fishing을 차단.  
> **연습 1 (10분, 사전 결정)**: 새 데이터셋을 받자마자 모델 적합 *전*에 다음 셋 중 하나를 사전에 결정한다 — (a) assay LLOQ(정량하한치)가 관측 농도 분포의 하위 25%에 들어오면 → combined residual로 시작, (b) 농도 dynamic range가 2 log 이상이면 → log-transformed + additive on log scale, (c) 둘 다 아니면 → proportional. 결정 근거를 한 줄로 lab notebook에 기록합니다.  
> **연습 2 (15분, GOF 추적)**: 첫 적합 후 CWRES vs PRED, CWRES vs TIME, CWRES histogram 세 plot을 그려 N3의 `Leakage Funnel` 시그니처(LLOQ 근처 funnel, $\eta$-shrinkage spike)를 점검합니다. funnel이 보이면 covariate를 추가하지 말고 residual model을 먼저 재고합니다. 점검 결과를 한 문장으로 기록: *"funnel [있음/없음] → residual model [수정 필요/유지]."*  
> **자기 점검 신호**: covariate 탐색 단계에 진입하기 전, C2의 결정이 a priori 기준에 의해 lock되었는지 확인. $\varepsilon$이 흔들리는 상태에서 covariate를 더하면 N3의 leakage가 시간 축으로 펼쳐져 Pair 4 Memory Hook의 시나리오가 발생합니다.

<!-- SLIDE_START: S32 | title: C3. 공변량 모델링 — CrCl, $f_u$, 재모수화(reparameterization)  `[⚡ Apex Concept]` | source_anchor: auto-s32 -->
> **핵심 메시지:** 공변량 모델링은 ω²를 줄이는 기술이 아니라 설명 가능한 생리학을 θ 구조로 재배정하는 과정입니다.
...
<!-- ANNOTATION -->공변량 모델링(covariate modeling)(← 변이의 설명변수를 모델에 넣는 작업)은 $\omega^2$을 "줄이는 기술"이 아닙니다. 더 정확히는 $\omega^2$ 안에 섞인 생리학을 분리해 이름을 붙이는 작업입니다. R&T의 크레아티닌 청소율(creatinine clearance) 예시는 신청소율(renal clearance)의 생리학을 CL_i에 연결하고, G&W PK50은 단백 결합 비율 $f_u$(fraction unbound)가 total parameter variability의 일부를 설명함을 보여 줍니다. `[R&T p.369–371; G&W p.706–709]`

<!-- SLIDE_START: S33 | title: ⚡ Apex Concept 선언 — [EXPERT_INFERENCE, v3] | source_anchor: auto-s33 -->
> **핵심 메시지:** C3의 핵심은 OFV 개선보다 분산을 생리학적 원인으로 재할당하는 판단입니다.

> **⚡ Apex Concept 선언 — [EXPERT_INFERENCE, v3]**  
> Session 13의 네 카드 중 가장 난해하고 임상·규제 파급력이 가장 큰 카드는 C3입니다. **C3의 Apex 명제**: *공변량 모델링은 OFV 향상을 위한 탐색이 아니라, 분산($\omega^2$)을 설명 가능한 생리학적 원인으로 재할당하는 과정입니다.* 이 명제를 놓치면 — 즉 공변량을 "$\omega^2$을 줄여 주는 통계적 도구"로 취급하면 — C1의 보존 법칙(N1)·C2의 잔차 모델 우선성·C4의 분포 모양 점검이 모두 무너집니다. C3는 다른 세 카드의 이론적 출구이자 규제 보고서의 용량 근거(dose rationale)를 결정하는 line item입니다.

<!-- SLIDE_START: S34 | title: 그럴듯한 오해(Plausible Fallacy) — `OFV-Driven Covariate Acceptance` · [EXPE | source_anchor: auto-s34 -->
> **핵심 메시지:** ΔOFV는 공변량 채택의 충분조건이 아니라 생리학, VPC, residual stability와 함께 보는 통계 필터입니다.

> **그럴듯한 오해(Plausible Fallacy) — `OFV-Driven Covariate Acceptance` · [EXPERT_INFERENCE, v3]**  
<!-- SLIDE_START: S35 | title: 그럴듯한 오해**: "$\Delta OFV$가 3.84 (df=1, α=0.05) 이상 감소하면 공변량이 의미 있습니다." | source_anchor: auto-s35 -->
> **핵심 메시지:** ΔOFV는 공변량 채택의 충분조건이 아니라 생리학, VPC, residual stability와 함께 보는 통계 필터입니다.

> **그럴듯한 오해**: "$\Delta OFV$가 3.84 (df=1, α=0.05) 이상 감소하면 공변량이 의미 있습니다."  

> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{\Delta OFV}_{모델\ 간\ 목적함수\ 변화}\ >\ \underbrace{3.84}_{df=1,\ \alpha=0.05\ 기준값}
> $$
>
> - **항별 의미:** 좌변은 covariate 추가 전후의 OFV 변화량이고, 우변은 df=1 LRT 상황에서 쓰는 통계적 기준값입니다.
> - **실무 직관:** ΔOFV가 커지면 모델 적합 지표는 개선된 것처럼 보일 수 있으나, 생리학적 타당성과 VPC 일관성까지 함께 확인해야 합니다.
> - **오해 방지:** ΔOFV 기준은 acceptance criterion 전체가 아니라 하나의 필터입니다. 잔차 모델 오류나 빈 phenotype cell도 ΔOFV를 움직일 수 있습니다.

> **왜 틀렸는가**: $\Delta OFV > 3.84$ 기준은 **하나의 통계적 필터**이지 임상적·생물학적 의미의 증거가 아닙니다. 세 가지 경로로 위양성(false positive)이 생깁니다. (1) 잔차 오차 구조가 잘못 지정되면(misspecified) 공변량 추가가 $\varepsilon$의 변동을 흡수하여 잘못된 $\Delta OFV$를 만듭니다 — N3의 `ω²–σ² Leakage Funnel`이 시간 축으로 펼쳐진 형태입니다. (2) 표현형 셀(phenotype cell) 크기가 작으면(N5의 `Empty Phenotype Cell`) 범주형 공변량의 $\Delta OFV$가 작은 셀의 ETA 흡수로부터 인공적으로 발생합니다. (3) 진짜 mechanistic covariate가 빠지면 상관 공변량(weight, BSA가 CrCl을 대신함)이 가짜로 OFV 개선을 만듭니다 — N4의 `Vanishing Covariate Cascade`의 거울상입니다.  
> **임상·NONMEM 시그니처**: 체중을 CL에 추가했더니 OFV가 12 감소했지만, **VPC를 그려보니 오히려 예측 구간이 넓어진다** — 공변량을 추가했는데 모델 적합도가 *분포 수준에서* 악화됩니다. 이름: `OFV-VPC Discordance`. 발생 시 첫 자문: 잔차 모델이 안정화되었는가? phenotype cell n이 충분한가? a priori 후보 공변량이 base에 포함되어 있는가? 세 질문에 모두 YES이기 전에는 $\Delta OFV$는 그저 한 줄 숫자일 뿐입니다.  
> **방어 문구 (규제 보고서)**: "Covariate selection was guided by $\Delta OFV$ combined with VPC consistency at covariate extremes and a priori physiological plausibility; $\Delta OFV$ alone was not used as the acceptance criterion."

<!-- SLIDE_START: S36 | title: A. 형식적 정의(Formal Definition) | source_anchor: auto-s36 -->
> **핵심 메시지:** 이 slide에서는 현재 개념이 무엇을 예측·정의하고 어떤 항으로 분해되는지 먼저 고정해야 합니다.

#### A. 형식적 정의(Formal Definition)

R&T는 크레아티닌 청소율이 신청소율의 개체 간 차이를 설명하는 공변량이 될 수 있다고 설명합니다. 예시 구조는 다음과 같습니다. `[R&T p.369–371]`
...
>
> $$
> \underbrace{CL_i}_{개인\ i의\ 청소율}=\underbrace{\theta_1}_{기저\ 청소율}+\underbrace{\theta_2\cdot GFR_i}_{신기능으로\ 설명되는\ 청소율\ 성분}+\underbrace{\eta_i}_{남은\ 개인\ 편차}
> $$
>
> - **항별 의미:** 좌변은 개인별 CL이고, θ1은 기준 청소율, θ2·GFRi는 신기능이 설명하는 CL 변화, ηi는 설명되지 않고 남은 개인 편차입니다.
> - **실무 직관:** GFRi 또는 θ2가 커질수록 CLi는 증가하며, 공변량이 설명한 부분은 η가 아니라 θ 구조로 이동합니다.
> - **오해 방지:** 공변량 모델은 η를 없애는 장치가 아니라, η 안에 숨어 있던 생리학적 설명분을 분리하는 장치입니다.


또는 NONMEM 구현에서는 흔히 다음과 같은 중심화된 공변량 형태(centered covariate form)를 씁니다.
...
>
> $$
> \underbrace{CL_i}_{개인\ 청소율}=\underbrace{\theta_1}_{중심화\ 기준\ CL}\cdot\underbrace{\left(\frac{CRCL_i}{90}\right)^{\theta_2}}_{CRCL\ 공변량\ 배율}\cdot\underbrace{\exp(\eta_i)}_{남은\ IIV\ 배율}
> $$
>
> - **항별 의미:** 좌변은 개인별 CL이며, θ1은 CRCL=90일 때의 기준 CL, power term은 신기능 공변량 효과, exp(ηi)는 남은 개체 간 변이입니다.
> - **실무 직관:** CRCL이 기준값보다 높고 θ2가 양수이면 CL은 증가합니다. 공변량이 설명하지 못한 차이는 exp(ηi)에 남습니다.
> - **오해 방지:** 중심화는 단순 미관이 아니라 θ1의 해석 기준점을 고정해 추정 안정성과 보고서 해석을 돕습니다.


여기서 공변량이 설명한 부분은 $\theta$ 구조로 이동합니다. 설명되지 않은 개인 간 차이만 $\eta$에 남습니다.

<!-- SLIDE_START: S37 | title: B. PK50 앵커(Anchor): $f_u$는 "$\omega^2$ reduction"이 아니라 좌표계 변환입니다 | source_anchor: auto-s37 -->
> **핵심 메시지:** 이 slide에서는 하나의 개념을 하나의 실무 판단으로 압축해 다음 모델링 단계와 연결해야 합니다.

#### B. PK50 앵커(Anchor): $f_u$는 "$\omega^2$ reduction"이 아니라 좌표계 변환입니다

<!-- ANNOTATION -->여기서 독자가 놓치기 쉬운 전환은 "공변량 모델"과 "재모수화(reparameterization)"의 구분입니다. 둘 다 변이 해석을 바꾸지만, 하나는 예측변수(predictor)(← 변수의 결과를 설명하기 위해 모델에 들어가는 입력 변수)를 모델에 추가하는 것이고, 다른 하나는 같은 자료를 다른 좌표계로 다시 표현하는 것입니다.

G&W PK50에서 12명 환자는 CpD 566 µg을 5시간 IV infusion으로 받았고, 총 농도(total concentration)와 비결합 농도(unbound concentration)가 함께 해석되었습니다. 혈장 단백 결합 데이터인 $f_u$는 total disposition parameter로부터 unbound parameter를 도출(derive)하는 데 사용되었습니다. `[G&W p.704–707]`
...
원전 그대로의 해석(Source-exact interpretation): **PK50은 total Cl CV 28%와 unbound Clu CV 9%를 보고합니다.** 이는 NONMEM 공변량 모델이 $\omega^2$(CL)을 28%에서 9%로 "깎았다"는 뜻이 아닙니다. 같은 12명 자료를 total coordinate와 unbound coordinate로 다시 표현했을 때 변이가 다르게 보인다는 뜻입니다. $f_u=0.016\pm0.0049$였으므로 단백 결합 변이가 total concentration/parameter variability의 일부를 설명합니다. `[G&W p.708–709]`

<!-- TRENCH -->
**Trench-Level Tip — ETA 위치**: $f_u$가 결정론적 재모수화(deterministic reparameterization)이면 $CL=f_u\cdot CL_u$의 좌표계 변환으로 처리합니다. 반대로 집단 공변량 모델(population covariate model)로 쓸 때는 `CLu`에 남길 $\eta$와 $f_u$의 측정 변이(measurement variability)를 분리해야 합니다. 두 경우를 섞으면 ETA가 생리학(physiology)이 아니라 회계 오차(accounting error)를 흡수합니다.

> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{CL}_{total\ coordinate\ 청소율}=\underbrace{f_u}_{비결합\ 분율}\cdot\underbrace{CL_u}_{unbound\ coordinate\ 청소율}
> $$
>
> - **항별 의미:** 좌변은 total 좌표계의 CL이고, fu는 비결합 분율, CLu는 unbound 좌표계의 청소율입니다.
> - **실무 직관:** fu가 커지면 같은 CLu에서도 total CL이 커집니다. 반대로 fu 변이가 크면 total parameter variability가 커 보일 수 있습니다.
> - **오해 방지:** 이 식은 covariate가 ω²를 줄인다는 식이 아니라 total과 unbound 좌표계를 연결하는 재모수화식입니다.


<!-- SLIDE_START: S38 | title: C. PK vs PD 변이 분리(PK vs PD variability separation) | source_anchor: auto-s38 -->
> **핵심 메시지:** 이 slide에서는 하나의 개념을 하나의 실무 판단으로 압축해 다음 모델링 단계와 연결해야 합니다.

#### C. PK vs PD 변이 분리(PK vs PD variability separation)

PK50의 결정적 메시지는 단백 결합이 모든 변이를 설명하지 않는다는 점입니다. CpD는 비슷한 노출(exposure)에서도 반응(response)이 8배(0.5–4.0 response units)까지 달랐습니다. 또한 비결합 농도(unbound concentration)를 사용해도 EC50 변이는 오히려 커졌습니다. 저자들은 $E_{max}<1$인 무반응자(non-responder)가 유전적 구성(genetic makeup)/수용체 밀도(receptor density)와 관련된다고 해석했습니다. `[G&W p.707–709]`

R&T도 혈장 농도(plasma concentration)와 반응(response)을 함께 측정해야 PK 변이와 PD 변이를 구분할 수 있다고 합니다. `[R&T p.363–364]` 따라서 공변량이 PK 파라미터 변이를 설명했다고 해서 반응 변이까지 설명했다고 보면 안 됩니다.

<!-- SLIDE_START: S39 | title: D. Structural Necessity | source_anchor: auto-s39 -->
> **핵심 메시지:** 이 개념은 모델 숫자를 임상 판단으로 바꾸기 위해 반드시 필요한 구조적 조건입니다.

#### D. Structural Necessity

- 공변량은 **$\eta$를 없애는 변수**가 아니라 **$\eta$ 안의 설명 가능한 구성요소를 $\theta$ 구조로 옮기는 변수**다.
- 공변량 도입 전에는 G&W식 EDA가 선행되어야 합니다: 개체 프로파일(individual profile), 통합 자료(pooled data), 용량 정규화(dose normalization), 변환(transformation)으로 하위군(subgroup) 또는 추세(trend)를 먼저 봅니다. `[G&W p.334–336]`
- 식별가능성(identifiability)이 없는 공변량은 이름만 생리학적일 뿐 모델 안에서는 또 다른 noise source가 됩니다. `[G&W p.348–351]`

<!-- SLIDE_START: S40 | title: E. 한계(Limitations) | source_anchor: auto-s40 -->
> **핵심 메시지:** 모델 해석은 성립 조건과 source-boundary를 벗어나는 순간 쉽게 과잉해석으로 바뀝니다.

#### E. 한계(Limitations)

- 분산 전파식(variance propagation) $CV(CL)^2 \approx CV(f_u)^2 + CV(CL_u)^2$는 본 문서에서 삭제합니다. PDF에 없고, 독립성 가정 및 산술 검증도 성립하지 않습니다.
- 총 농도 안전 임계(total concentration safety threshold)와 비결합 농도 개체화(unbound concentration individualization)는 서로 다른 좌표계입니다. G&W는 total C $>50$ µg·L⁻¹을 피하라고 하면서, 변이가 있는 단백 결합을 고려해 안전 여유(safety margin)는 total보다 unbound concentration 기반이 낫다고 결론냅니다. `[G&W p.705, p.709]`

<!-- SLIDE_START: S41 | title: F. 제텔카스텐 원자(Zettelkasten Atom) | source_anchor: auto-s41 -->
> **핵심 메시지:** 이 slide의 키워드는 이후 모델 진단과 보고서 언어를 연결하는 검색 가능한 지식 원자입니다.
...
<!-- RECAP -->
**C3 요약(Recap)**: covariate는 $\omega^2$을 마술처럼 줄이지 않습니다. 좌표계를 바꾸거나 physiology를 $\theta$ 구조로 옮겨서, 남은 $\eta$가 진짜 unexplained variability에 가까워지게 합니다.

---


<!-- SLIDE_START: S42 | title: Figure schematic | source_anchor: auto-s42 -->
> **핵심 메시지:** PK50 schematic은 total coordinate, fu 재모수화, unbound coordinate, PD variability를 분리해 보여 줍니다.

<!-- FIGURE_SCHEMATIC -->
Title: PK50 좌표계 분리 — total variability, $f_u$ 재모수화, 남는 PD variability
Mode: N
Visual objective: 5초 안에 "단백 결합(protein binding)은 total PK variability의 일부를 설명하지만, PD response variability는 그대로 남습니다"를 보이게 합니다.
Core message: PK50의 $f_u$는 NONMEM 공변량 모델(covariate model)의 $\omega^2$ reduction이 아니라, total coordinate를 unbound coordinate로 다시 표현하는 재모수화(reparameterization)입니다. 이 변환 후에도 response variability는 별도 PD source로 남습니다.
Elements to include: 왼쪽 lane: Total coordinate — $CL=11.4$ L·h⁻¹ (CV 28%), total concentration, total C $>50$ µg·L⁻¹ 회피; 중앙 node: $f_u=0.016\pm0.0049$ — deterministic reparameterization / coordinate change; 오른쪽 lane: Unbound coordinate — $CL_u=720$ L·h⁻¹ (CV 9%), unbound-based safety margin; 아래 branch: PD variability remains — response 8-fold, EC50 total 1.8 (CV 40%) vs EC50 unbound 0.029 (CV 60%), responder/non-responder; 경고 label: 다음처럼 읽지 마세요: $f_u$ covariate가 $\omega^2(CL)$을 28%→9%로 줄였습니다.
Elements to exclude: 개별 피험자 수준(individual subject-level)의 Table 50.3 값; infusion regimen 계산 세부사항; Fig.50.1, Fig.50.2, Table 50.1 또는 Table 50.2 layout의 정확한 재현; 새 분산 전파식(variance propagation equations).
Suggested rendering: Mermaid
Caption: PK50은 $f_u$가 PK 해석의 좌표계를 바꾸지만, PD response variability는 개체 간 변이(between-subject variability)의 별도 원천으로 남는다는 점을 보여 줍니다.
Alt text: total PK 파라미터가 $f_u$에 의해 unbound 파라미터로 변환되고, 별도의 아래쪽 가지가 남아 있는 약력학 반응 변이(pharmacodynamic response variability)를 나타내는 흐름도입니다.
...
>
> $$
> \underbrace{CL=11.4\ \mathrm{L\cdot h^{-1}}}_{total\ coordinate\ typical\ CL}\quad (\underbrace{CV=28\%}_{total\ variability}),\qquad \underbrace{CL_u=720\ \mathrm{L\cdot h^{-1}}}_{unbound\ coordinate\ typical\ CL}\quad (\underbrace{CV=9\%}_{unbound\ variability})
> $$
>
> - **항별 의미:** 두 값은 서로 다른 좌표계의 CL 요약값입니다. total CL은 단백결합 변이가 섞인 좌표, CLu는 fu로 재표현한 unbound 좌표입니다.
> - **실무 직관:** total에서 CV가 크고 unbound에서 CV가 작다면 단백결합 차이가 total parameter variability의 일부를 설명한다는 판단으로 이어집니다.
> - **오해 방지:** 이 비교는 같은 자료를 다른 좌표계로 읽은 것이며, covariate step이 ω²를 28%에서 9%로 직접 절단했다는 뜻이 아닙니다.


> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{C_{total}}_{총\ 농도}\ >\ \underbrace{50\ \mu\mathrm{g\cdot L^{-1}}}_{회피해야\ 할\ total\ safety\ threshold}
> $$
>
> - **항별 의미:** 좌변은 total concentration이고, 우변은 원문에서 제시된 total 농도 안전 경계입니다.
> - **실무 직관:** total C가 이 경계를 넘는 방향으로 증가하면 안전성 우려가 커지는 좌표계 판단으로 연결됩니다.
> - **오해 방지:** total threshold와 unbound-based safety margin은 같은 문장 안에 있어도 같은 좌표계가 아닙니다.


> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{E_{max}}_{최대\ 반응능}\ <\ \underbrace{1}_{완전\ 반응\ 기준}
> $$
>
> - **항별 의미:** 좌변은 최대반응 파라미터이고, 우변은 완전 반응을 뜻하는 기준값입니다.
> - **실무 직관:** Emax가 1보다 작으면 농도를 올려도 완전 반응에 도달하지 못하는 PD variability 또는 non-responder 구조를 의심하게 합니다.
> - **오해 방지:** unbound concentration으로 PK 좌표계를 정리해도 PD 반응 한계가 자동으로 사라지는 것은 아닙니다.


<!-- SLIDE_START: S43 | title: 체화 노트(Mastery Note) — [AUDIT_DERIVED] | source_anchor: auto-s43 -->
> **핵심 메시지:** 이 slide에서는 하나의 개념을 하나의 실무 판단으로 압축해 다음 모델링 단계와 연결해야 합니다.

> **체화 노트(Mastery Note) — [AUDIT_DERIVED]**  
> PK50의 $f_u$ 사례는 '공변량을 넣어 $\omega^2$을 줄였다'는 이야기가 아니라, total coordinate와 unbound coordinate를 구분해야 한다는 이야기입니다. 같은 자료를 다른 생물학적 좌표계로 표현했을 때 줄어드는 변이와, 모델 안에서 covariate가 설명해 줄이는 변이를 섞지 않는 것이 C3의 핵심 판단입니다.

<!-- SLIDE_START: S44 | title: 진단 시그니처(Diagnostic Signature) 3 — `Vanishing Covariate Cascade` · [CRU | source_anchor: auto-s44 -->
> **핵심 메시지:** 진짜 mechanistic covariate가 base model에 빠지면 η가 신호를 흡수해 후속 stepwise가 가짜 proxy를 고를 수 있습니다.

> **진단 시그니처(Diagnostic Signature) 3 — `Vanishing Covariate Cascade` · [CRUCIBLE_DERIVED]**  
> **시그니처**: 진짜 mechanistic covariate(예: renal clearance에서 CrCl)가 모델에 빠져 있으면 $\eta_{CL}$이 그것을 흡수하고, 후속 stepwise selection에서 상관 covariate(weight, BSA)가 가짜로 detect되는 cascade. 시각적으로 $\eta_{EBE}$ vs CrCl scatter는 평탄하게 보여 "CrCl 효과 없음"으로 잘못 판독되지만, **raw observation 좌표계의 Cl_obs vs CrCl는 명확한 기울기**를 갖습니다. **메커니즘**: stepwise는 $\eta$에 남은 잔여 신호로 covariate를 평가하므로, $\eta$가 이미 covariate를 흡수해 버리면 그 covariate는 통계적으로 사라집니다. C3의 "covariate는 $\eta$ 안의 설명 가능한 부분을 $\theta$ 구조로 옮기는 변수"라는 정의가 시간 순서를 잘못 지키면 정반대로 작동합니다. **체화 꿀팁**: $\eta$에 의존한 covariate scan 한 번, raw observation에 직접 적용한 covariate scan 한 번 — 두 결과가 일치하지 않으면 "효과 없음"이 아니라 "효과 흡수됨"을 의심합니다. **실무 활용**: domain knowledge 기반 candidate covariate(생리학적으로 CL을 결정하는 변수: CrCl, body size, hepatic function)를 a priori 정의해 모델 base에 포함시킨 뒤 stepwise를 시작합니다. stepwise에 모든 판단을 위임하지 않습니다.

<!-- SLIDE_START: S45 | title: 📋 실무 체화 요약(Practice Brief) — C3 · [CRUCIBLE_DERIVED, v3] | source_anchor: auto-s45 -->
> **핵심 메시지:** Practice Brief의 목적은 개념을 control stream, GOF, 보고서 문장으로 즉시 옮기는 것입니다.

> **📋 실무 체화 요약(Practice Brief) — C3 · [CRUCIBLE_DERIVED, v3]**  
> **목표**: covariate 탐색을 OFV-driven fishing이 아닌 mechanism-driven hypothesis testing으로 전환.  
> **연습 1 (15분, a priori 정의)**: stepwise 시작 전에 다음 표를 작성한다 — *"파라미터 [CL/V/ka], 기대 covariate [CrCl/체중/연령/유전형], 생리학적 근거 [신청소율/생리학적 분포용적/대사효소 기저활성/유전자 기능], 예상 방향 [+/-]"*. 이 표가 채워지지 않은 covariate는 stepwise 후보에서 제외합니다.  
> **연습 2 (20분, 이중 scan)**: $\eta$에 의존한 covariate scan($\eta$-EBE vs covariate plot)과 raw observation 좌표 scan(individual NCA estimate vs covariate plot) 두 가지를 모두 수행합니다. 두 결과가 불일치하면 N4의 `Vanishing Covariate Cascade`를 의심하고, base model에 a priori candidate를 강제 포함시킨 후 재실행.  
> **연습 3 (PK50 좌표계 점검)**: 단백질 결합이 강한 약물($f_u<0.1$)을 다룰 때, total parameter 모델과 unbound parameter 모델의 $\omega^2$을 나란히 보고합니다. fu가 covariate 모델로 작동하는지 reparameterization으로 작동하는지 구분하여 명시.  
> **자기 점검 신호**: stepwise selection 결과가 a priori 표와 크게 다르면, "데이터가 새로운 가설을 보여 줬다"가 아니라 **"a priori 기준이 부족했거나 N4 cascade가 발생했다"**를 먼저 의심합니다.

<!-- SLIDE_START: S46 | title: C4. 유전적 다형성 — IIV의 불연속 하위구조(substructure) | source_anchor: auto-s46 -->
> **핵심 메시지:** η가 매끄러운 로그정규 하나처럼 보여도 실제로는 유전 phenotype 하위군의 혼합일 수 있습니다.
...
C1–C3은 $\eta$를 대개 매끄러운(smooth) 로그정규 분포(log-normal distribution)로 다룹니다. 그러나 약리유전학(pharmacogenetics)은 $\eta$가 실제로는 하나의 매끄러운 분포가 아닐 수 있음을 보여 줍니다. 실제로는 PM/IM/EM/UM 또는 반응자/무반응자(responder/non-responder)처럼 불연속적으로 구분되는 여러 하위집단(subpopulation)의 혼합일 수 있습니다. `[R&T p.393–410]`

<!-- SLIDE_START: S47 | title: A. 형식적 정의(Formal Definition) | source_anchor: auto-s47 -->
> **핵심 메시지:** 이 slide에서는 현재 개념이 무엇을 예측·정의하고 어떤 항으로 분해되는지 먼저 고정해야 합니다.

#### A. 형식적 정의(Formal Definition)

R&T는 유전 다형성(genetic polymorphism)을 모집단 안에서 유전 표현형(inherited phenotype)이 <!-- ANNOTATION -->다봉형 분포(polymodal distribution)(← 봉우리가 여러 개인 분포)를 만들 수 있는 현상으로 다룹니다. 핵심은 유전 형질(inherited trait)이 다른 변이의 원인과 달리 평생 지속되는 개인 특성이라는 점입니다. `[R&T p.393]`
...
>
> $$
> \underbrace{p^2}_{한\ 대립유전자\ 동형접합}+\underbrace{2pq}_{이형접합}+\underbrace{q^2}_{다른\ 대립유전자\ 동형접합}=\underbrace{1}_{전체\ genotype\ frequency}
> $$
>
> - **항별 의미:** 좌변은 세 genotype 빈도의 합이고, 우변은 모집단 전체 빈도 1을 뜻합니다.
> - **실무 직관:** q가 커지면 q²와 2pq 구성이 바뀌어 slow/fast acetylator 같은 phenotype cell 크기 해석이 달라집니다.
> - **오해 방지:** 이 식은 개인의 유전형을 예측하는 식이 아니라 모집단 수준 genotype frequency를 보존하는 식입니다.


R&T는 느린 acetylator(slow acetylator)가 열성 동형접합체(recessive homozygote)일 때, 대립유전자 빈도(allele frequency)로 이형접합/동형접합 빠른 acetylator(heterozygous/homozygous fast acetylator) 빈도를 계산하는 방식을 설명합니다. `[R&T p.402]`

<!-- SLIDE_START: S48 | title: B. 출처 고정 유전 예시(Source-Locked Genetic Examples) | source_anchor: auto-s48 -->
> **핵심 메시지:** 유전 예시들은 η 안에 실제 생물학적 subgroup 구조가 숨어 있을 수 있음을 보여 줍니다.

#### B. 출처 고정 유전 예시(Source-Locked Genetic Examples)

| 예시 | 출처 고정 사실 | 모델링 함의 |
|---|---|---|
| Nortriptyline 쌍둥이 | 일란성 쌍둥이의 쌍 내 변이(intrapair variability)가 이란성 쌍둥이보다 작아 유전이 PK 변이에 크게 기여. `[R&T p.394]` | "설명되지 않은 $\eta$" 안에 유전 성분이 있을 수 있음. |
| CYP2D6 / nortriptyline | 기능적 CYP2D6 유전자 사본 수가 많을수록 청소율(clearance)이 커지고 노출이 낮아짐. `[R&T p.397]` | $\eta$(CL)이 매끄럽지 않고 유전자 사본 범주(gene-copy category)로 갈라질 수 있음. |
| CYP2C9 / warfarin | CYP2C9 변이가 S-warfarin 대사와 용량 변이에 기여. `[R&T p.398, p.406]` | PK 유전형과 PD 유전형(VKORC1)을 동시에 고려해야 함. |
| Codeine | CYP2D6-매개 모르핀(morphine) 형성은 minor pathway라도 강력한 대사체(potent metabolite) 때문에 임상적으로 중요함. `[R&T p.399, p.404]` | 모약물 청소율(parent drug clearance)만 보면 독성/활성 대사체 위험을 놓칩니다. |
| TPMT / thiopurines | TPMT 다형성은 thiopurine 용량과 독성에 큰 영향. `[R&T p.400]` | Poor metabolizer 하위군은 평균 용량으로 다룰 수 없음. |
| NAT2 / isoniazid | 483명에서 6시간째 isoniazid 농도의 이봉형 분포(bimodal distribution). `[R&T p.402]` | 이봉형 $\eta$ 분포의 대표 예시. |
| VKORC1 / warfarin | CYP2C9, VKORC1, 연령/체중 등이 warfarin 용량 분산을 나누어 설명. `[R&T p.406]` | 단일 공변량 결정론(single covariate determinism) 금지. |

Table 13-1의 poor metabolizer 빈도는 출처 값만 유지합니다: CYP2D6 PM은 백인(Caucasians) 5–10%, 아프리카계 미국인(African Americans) 3.8%, 아시아인(Asians) 0.9%; CYP2C9 PM은 백인 1%; CYP2C19 PM은 백인 3–5%, 아시아인 16%; TPMT 결핍은 백인 0.3%, 아시아인 0.04%; NAT2 느린 acetylator는 백인/아프리카계 미국인 60%, 아시아인 10–20%; UGT1A1 poor metabolizer 상태는 백인 11%, 아시아인 1–3%. `[R&T p.395]`

<!-- SLIDE_START: S49 | title: C. Code Structure | source_anchor: auto-s49 -->
> **핵심 메시지:** 코드 한 줄은 phenotype effect와 남은 ETA를 어느 그릇에 배정하는지 결정합니다.
...
>
> $$
> \underbrace{CL_i}_{개인\ 청소율}=\underbrace{\theta_1}_{기준\ CL}\cdot\underbrace{\theta_2^{I(CYP2D6PM_i=1)}}_{PM\ 범주형\ 효과}\cdot\underbrace{\exp(\eta_i)}_{남은\ IIV}
> $$
>
> - **항별 의미:** 좌변은 개인별 CL이고, θ1은 기준 CL, θ2는 PM일 때 적용되는 범주형 효과, exp(ηi)는 phenotype으로도 설명되지 않은 남은 IIV입니다.
> - **실무 직관:** PM indicator가 1이면 θ2가 CL에 곱해져 phenotype 효과를 반영하고, indicator가 0이면 기준 CL과 η만 남습니다.
> - **오해 방지:** 범주형 covariate를 넣었다고 유전 구조가 항상 식별되는 것은 아니며, 작은 cell에서는 θ2가 ETA에 흡수될 수 있습니다.


<!-- TRENCH -->
**Trench-Level Tip — phenotype level 통합**: 범주형 공변량은 수준(level)별 표본 크기가 충분하지 않으면 THETA factor가 ETA에 흡수됩니다. PM $n=3$ 같은 빈 셀에서는 "효과 없음"이 아니라 "효과를 식별할 정보가 없음"일 수 있습니다.

<!-- SLIDE_START: S50 | title: D. Structural Necessity | source_anchor: auto-s50 -->
> **핵심 메시지:** 이 개념은 모델 숫자를 임상 판단으로 바꾸기 위해 반드시 필요한 구조적 조건입니다.

#### D. Structural Necessity

- 평균(mean)과 분산(variance)뿐 아니라 **분포 모양(distribution shape)**도 중요합니다. R&T의 가상 청소율 분포에서 이봉형(bimodal) 분포는 평균이 치료적으로 거의 무의미할 수 있음을 보여 줍니다. `[R&T p.366]`
- 유전 다형성은 $\eta$ 히스토그램의 "두 봉우리"를 생리학(physiology)으로 번역합니다.
- R&T는 인종(ethnicity)이 유전 형질(genetic characteristic)의 대체지표(proxy)일 뿐이며, 그룹 내 변이도 클 수 있다고 경고합니다. 따라서 인종 공변량을 유전형(genotype)처럼 해석하면 안 됩니다. `[R&T p.395, p.409]`
- CYP3A4처럼 큰 변이가 있어도 명확한 유전 인자가 드러나지 않는 경우가 있습니다. 모든 큰 $\omega^2$이 유전형 봉우리(genotype peak)를 뜻하지는 않습니다. `[R&T p.399, p.410]`

<!-- SLIDE_START: S51 | title: E. 한계(Limitations) | source_anchor: auto-s51 -->
> **핵심 메시지:** 모델 해석은 성립 조건과 source-boundary를 벗어나는 순간 쉽게 과잉해석으로 바뀝니다.

#### E. 한계(Limitations)

- HLA-B*5701 빈도, 최신 FDA 약리유전체 라벨 수, 자세한 PM/IM/EM/UM 빈도표는 이 PDF 범위 밖이므로 삭제합니다.
- 약리유전학은 강력하지만 완결된 설명이 아닙니다. R&T는 유전형 외에도 인구학적 특성(demographics), 복약 순응(adherence) 등 다른 요인이 최적 용량 결정에 필요하다고 마무리합니다. `[R&T p.409]`

<!-- SLIDE_START: S52 | title: F. 제텔카스텐 원자(Zettelkasten Atom) | source_anchor: auto-s52 -->
> **핵심 메시지:** 이 slide의 키워드는 이후 모델 진단과 보고서 언어를 연결하는 검색 가능한 지식 원자입니다.
...
<!-- RECAP -->
**C4 요약(Recap)**: 유전 다형성은 covariate 목록이 아니라 $\eta$ 분포의 모양을 바꾸는 생물학입니다. Smooth log-normal assumption이 깨지는 순간, 평균 dose는 subgroup toxicity 또는 inefficacy를 숨깁니다.

---


<!-- SLIDE_START: S53 | title: Figure pointer | source_anchor: auto-s53 -->
> **핵심 메시지:** 교과서 figure는 본문 주장을 눈으로 검증하는 독립 slide로 확인해야 합니다.

<!-- FIGURE_POINTER -->
Source: Rowland & Tozer 5e, p.402, Figure 13-6 — 483명에서 9.8 mg/kg 경구 투여 후 6시간째 isoniazid 농도 분포.
Why this matters: C4의 핵심은 유전 다형성이 단순 공변량 label이 아니라 분포 모양(distribution shape) 자체를 바꾼다는 점입니다. Figure 13-6은 NAT2 acetylator status가 실제 농도 분포(concentration distribution)를 이봉형(bimodal)으로 만드는 장면을 보여 줍니다.
When to look: 이 카드를 읽은 뒤 확인하시면 됩니다.
Learner instruction: 두 peak가 어디에 생기는지 먼저 확인하세요. 그런 다음 "$\eta$ histogram이 두 봉우리라면 smooth log-normal IIV 하나로 덮으면 안 됩니다"라는 C4 요약(Recap)과 연결하시면 됩니다.
<!-- /FIGURE_POINTER -->

<!-- SLIDE_START: S54 | title: Failure Mode — [CRUCIBLE_DERIVED] | source_anchor: auto-s54 -->
> **핵심 메시지:** 작은 phenotype subgroup은 효과가 없는 것이 아니라 효과를 식별할 정보가 부족한 상태일 수 있습니다.

> **Failure Mode — [CRUCIBLE_DERIVED]**  
> 범주형 유전 구조가 실제로 존재하는데 표본의 한 subgroup이 작거나 비어 있으면, 모델은 그 효과를 독립된 phenotype 효과로 보지 못하고 $\eta$ 안에 흡수할 수 있습니다. 그래서 C4에서는 genotype label 자체보다, $\eta$ distribution과 concentration distribution의 모양이 단봉인지 다봉인지를 먼저 확인해야 합니다.

<!-- SLIDE_START: S55 | title: 진단 시그니처(Diagnostic Signature) 2 — `Empty Phenotype Cell` · [CRUCIBLE_D | source_anchor: auto-s55 -->
> **핵심 메시지:** Phenotype cell이 비어 있으면 categorical effect가 ETA로 흡수되어 false negative처럼 보일 수 있습니다.

> **진단 시그니처(Diagnostic Signature) 2 — `Empty Phenotype Cell` · [CRUCIBLE_DERIVED]**  
> **시그니처**: phenotype 4-level full stratification(PM/IM/EM/UM)을 처음부터 펼쳤을 때, 작은 n cell(특히 $PM=3$ 같은 희귀 셀)의 THETA factor RSE가 80%를 넘고 `$COV step`이 "covariance matrix is non-positive definite"로 실패합니다. PM 환자 5명의 ETA(1)이 모두 약 0으로 보이는 것은 "효과 없음"이 아니라 **"효과 흡수됨"**의 가장 흔한 false negative 시그니처입니다. **메커니즘**: cell이 비면 모델은 그 phenotype 효과를 추정할 정보가 없어 ETA로 흡수하고, $\omega^2$이 인공적으로 inflate됩니다. 다시 한 번 보존 법칙이 작동합니다. **체화 꿀팁**: PM($n=3$) 결과를 받았을 때 처음 자문해야 할 질문은 "효과가 있는가?"가 아니라 "**효과를 식별할 정보가 데이터에 있는가?**"입니다. 이 둘은 다른 질문입니다. **실무 활용**: phenotype level은 데이터 size에 비례해 통합합니다. 인접 phenotype을 합쳐 "reduced function" 같은 단일 카테고리로 시작하거나, informative prior로 작은 cell을 안정화합니다. 4-level full stratification은 데이터가 명확히 허용할 때만 펼칩니다. 작은 cell의 categorical effect를 보고서에 "null result"로 기재하지 않습니다.

<!-- SLIDE_START: S56 | title: 📋 실무 체화 요약(Practice Brief) — C4 · [CRUCIBLE_DERIVED, v3] | source_anchor: auto-s56 -->
> **핵심 메시지:** Practice Brief의 목적은 개념을 control stream, GOF, 보고서 문장으로 즉시 옮기는 것입니다.

> **📋 실무 체화 요약(Practice Brief) — C4 · [CRUCIBLE_DERIVED, v3]**  
> **목표**: $\eta$ 분포 모양과 phenotype cell 크기를 covariate 신뢰성 평가에 통합.  
> **연습 1 (10분, 분포 모양 점검)**: 모델 적합 후 $\eta$-EBE histogram을 그리고 다음 점검표를 작성 — *"$\eta_{CL}$: [unimodal / bimodal / skewed / 너무 narrow], $\eta$-shrinkage: [%], 데이터가 EBE를 식별하기에 sufficient한가? [Y/N]"*. shrinkage $>30%$이면 histogram 모양으로 결론 내리지 않습니다.  
> **연습 2 (15분, cell 크기 점검)**: categorical covariate를 추가하기 전, 각 level의 n을 표로 만듭니다. 어떤 level의 $n<10$이면 N5 `Empty Phenotype Cell` 위험을 명시하고 다음 셋 중 하나를 사전 결정 — (a) 인접 level 통합 후 시작, (b) informative prior 적용, (c) full stratification 후 RSE $$>80%$$ 발생 시 자동 통합으로 회귀.  

> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{RSE}_{상대\ 표준오차}\ >\ \underbrace{80\%}_{불안정\ 추정\ 경고선}
> $$
>
> - **항별 의미:** 좌변은 categorical effect 등 추정값의 상대 표준오차이고, 우변은 매우 불안정한 추정으로 보는 경고 기준입니다.
> - **실무 직관:** RSE가 이 경계를 넘으면 추정값의 크기보다 식별가능성 자체를 먼저 의심해야 합니다.
> - **오해 방지:** RSE가 큰 결과는 효과가 없다는 뜻이 아니라 이 자료가 그 효과를 안정적으로 추정하지 못했다는 뜻일 수 있습니다.
> **검토 메모:** 이 부등식은 원문에서 practice decision rule로 제시된 경고 기준으로 읽는 것이 자연스럽습니다. 원문 보존 원칙에 따라 원 표현은 수정하지 않았습니다.

> **연습 3 (보고서 언어 훈련)**: PM($n=3$)에서 categorical effect THETA factor가 "통계적으로 유의하지 않음"으로 나왔을 때, 보고서에 *"null effect"*로 쓰지 않습니다. 대신 *"effect not identifiable from this dataset ($n=3$, RSE=[X]%)"*로 명시합니다. 이 한 줄이 N5 시그니처에 대한 규제 방어 문구입니다.  
> **자기 점검 신호**: $\eta$ histogram이 single log-normal로 보인다고 해서 유전 구조가 없는 것이 아닙니다. shrinkage가 높거나 PM cell이 비어 있으면 "보이지 않는 것"과 "없는 것"을 구분할 수 없습니다.

<!-- SLIDE_START: S57 | title: §5 — 혼동쌍 해부(Confusion Pair Dissection) | source_anchor: auto-s57 -->
> **핵심 메시지:** Session 13의 주요 혼동은 모두 변이가 어느 좌표계와 어느 층에 속하는지의 문제입니다.

## §5 — 혼동쌍 해부(Confusion Pair Dissection)

<!-- CONFUSION -->
<!-- SLIDE_START: S58 | title: Pair 1. $\omega^2$(IIV) vs $\sigma^2$(RUV) | source_anchor: auto-s58 -->
> **핵심 메시지:** ω²는 사람 간 차이이고 σ²는 관측 잔차이므로 원인과 처치법이 다릅니다.

### Pair 1. $\omega^2$(IIV) vs $\sigma^2$(RUV)

| 구분 | $\omega^2$ / $\eta$ | $\sigma^2$ / $\varepsilon$ |
|---|---|---|
...
>
> $$
> \underbrace{\omega^2}_{\eta의\ 분산,\ 개인\ 간\ 변이}\quad \text{vs}\quad \underbrace{\sigma^2}_{\varepsilon의\ 분산,\ 잔차\ 변이}
> $$
>
> - **항별 의미:** ω²는 개인별 random effect η의 분산이고, σ²는 관측 잔차 ε의 분산입니다.
> - **실무 직관:** ω²가 커지면 사람 간 typical parameter 차이가 커지고, σ²가 커지면 예측 주변의 관측 산포가 커집니다.
> - **오해 방지:** 둘 다 variability이지만 같은 층의 오차가 아니므로 처방은 covariate 탐색과 residual model 재검토로 갈립니다.


**결정적 차이(Critical distinction)**: $\omega^2$와 $\sigma^2$는 같은 총 미설명 변이를 두고 경쟁합니다. 잔차 모델을 틀리면 공변량이 단순히 보이거나 사라지는 것이 아닙니다. 분산이 잘못된 그릇으로 이동합니다.

> **⚡ 기억 고리(Memory Hook) — Pair 1 · [EXPERT_INFERENCE, v3]**  
> *개인 고유의 차이 vs 측정·모델의 잡음.* $\omega^2$ (`$OMEGA`)는 같은 약물, 같은 용량에서도 개인마다 CL·V가 다른 **생물학적 개체간 차이** — 이것은 실제로 존재하는 차입니다. $\sigma^2$ (`$SIGMA`)는 하나의 개인 내에서 예측값과 관측값이 일치하지 않는 **측정·모델·시간 내 노이즈** — 이것은 설명 실패입니다. 따라서 두 그릇의 처치법이 **구조적으로 다르다**: $\omega^2$를 줄이는 것은 IIV의 원인을 찾는 것(covariate 추가), $\sigma^2$를 줄이는 것은 모델 구조 또는 측정 오차 모델을 개선하는 것(residual model 재선택, assay 재검증). 이 둘을 혼동하면 covariate를 늘려도 $\sigma^2$ 문제는 해결되지 않고, error model을 바꿔도 IIV는 그대로 남습니다.

<!-- CONFUSION -->
<!-- SLIDE_START: S59 | title: Pair 2. Total Cl vs Unbound Clu — PK50 앵커 기반 | source_anchor: auto-s59 -->
> **핵심 메시지:** Total과 unbound 좌표계를 혼동하면 단백결합 변이와 노출-안전성 판단이 엇갈립니다.

### Pair 2. Total Cl vs Unbound Clu — PK50 앵커 기반
...
**수정된 치명적 타격(Corrected Critical Blow)**: PK50은 "fu covariate가 $\omega^2$을 줄였다"가 아니라, 같은 12명을 total과 unbound 좌표계로 비교했을 때 변이 해석이 달라짐을 보여 줍니다. 용량/안전성 판단에서 total C $>50$ µg·L⁻¹ 회피 기준과 unbound 기반 안전 여유(unbound-based safety margin)가 혼동되면, 노출-반응(exposure-response) 해석의 좌표계가 불일치합니다. `[G&W p.705, p.709]`

> **⚡ 기억 고리(Memory Hook) — Pair 2 · [EXPERT_INFERENCE, v3]**  
> *전체 통행량 vs 무료 차선 통행량.* Total CL은 bound + unbound 약물 모두를 기준으로 한 **전체 제거 능력**입니다. Unbound CLu = CL/fu는 유리 약물만을 기준으로 한 **실제 작용 약물의 제거 능력**입니다. 저알부민혈증 환자에서 fu가 증가하면 total CL이 변하지 않아도 unbound CL (= CL/fu) 좌표는 다르게 보입니다. PK50에서 보여준 것처럼, total coordinate에서 "IIV가 크다"(CV 28%)는 신호가 unbound coordinate로 좌표 변환하면 **다른 숫자(CV 9%)** 가 된다 — 이것은 $\omega^2$이 줄어든 것이 아니라 **좌표축이 회전한 것**입니다. 좌표계를 잘못 선택하면 covariate 탐색의 방향이 반대가 됩니다.

> **🔴 치명적 타격(Critical Blow) — Pair 2 · [EXPERT_INFERENCE, v3]**  
> 이 혼동을 품고 NDA의 dose individualization 섹션을 강행하면, total coordinate에서 산출된 $\omega^2$을 마치 covariate 모델이 reduction을 만든 것처럼 보고하게 됩니다. 규제 reviewer는 (a) total/unbound coordinate split이 명시되지 않았거나, (b) safety threshold(total)와 dose recommendation(unbound 기반)이 좌표계 측면에서 일치하지 않음을 deficiency 사유로 지적합니다. 임상적으로는 저알부민혈증 환자에서 total concentration이 정상이어도 unbound이 높아 독성이 발생할 수 있는 시나리오를 보호하지 못합니다. `[G&W p.705, p.709]`

<!-- CONFUSION -->
<!-- SLIDE_START: S60 | title: Pair 3. PK 변이 vs PD 변이 | source_anchor: auto-s60 -->
> **핵심 메시지:** PK 노출을 설명해도 PD 반응 차이는 receptor density나 genetic makeup 같은 별도 축으로 남을 수 있습니다.

### Pair 3. PK 변이 vs PD 변이

PK 변이는 농도-시간 프로파일(concentration-time profile)이 왜 다른가를 묻습니다. PD 변이는 같은 노출(exposure)에서 반응(response)이 왜 다른가를 묻습니다. R&T는 혈장 농도 측정(plasma concentration measurement)이 PK와 PD 변이를 분리하는 전제라고 합니다. `[R&T p.363–364]`

PK50에서 단백 결합은 총 농도 변이의 일부를 설명했지만, 반응은 비슷한 노출에서도 8배 차이를 보였습니다. 이것은 단일 공변량이 모든 변이를 설명한다는 사고가 왜 위험한지 보여 주는 핵심 예입니다. `[G&W p.707–709]`

> **⚡ 기억 고리(Memory Hook) — Pair 3 · [EXPERT_INFERENCE, v3]**  
> *약이 어떻게 움직이는가 vs 어떻게 반응하는가.* PK IIV는 개인별 CL·V·F의 차이 — 같은 용량에서 **농도가 얼마나 다른가**. PD variability는 개인별 EC50·Emax·γ의 차이 — 같은 농도에서 **반응이 얼마나 다른가**. 두 가지가 **수학적으로 독립적**이므로 PK IIV가 작아도 PD variability가 크면 농도만 맞춰도 반응 개체화가 어렵습니다. 개인화 의료에서 TDM이 혈중농도를 맞추는 것(PK 영역)과 반응을 맞추는 것(PD 영역)이 왜 다른 문제인지가 여기서 나옵니다. PK50의 "response 8배 차이"는 PK 좌표계의 covariate 모델로는 도달할 수 없는 잔여 variability입니다.

<!-- CONFUSION -->
<!-- SLIDE_START: S61 | title: Pair 4. 평균 분포(mean distribution) vs 분포 모양(shape distribution) | source_anchor: auto-s61 -->
> **핵심 메시지:** 평균이 같아도 분포 모양이 다르면 용량 개체화와 subgroup risk 판단은 달라집니다.

### Pair 4. 평균 분포(mean distribution) vs 분포 모양(shape distribution)

R&T의 nortriptyline 예시는 로그정규 분포(log-normal distribution)를 보여 주고, 가상 청소율 분포는 이봉형 분포(bimodal distribution)에서 평균이 치료적 대표값이 되기 어렵다는 점을 보여 줍니다. `[R&T p.363, p.366]` 평균과 CV만 보고 $\eta$ 분포의 모양을 확인하지 않으면, C4에서 다룬 유전 하위군(genetic subgroup)을 놓칩니다.

> **⚡ 기억 고리(Memory Hook) — Pair 4 · [EXPERT_INFERENCE, v3]**  
> *진짜 신호 vs 모델 결함의 메아리.* Residual error model이 잘못 지정되면(예: 실제로는 proportional+additive인데 proportional만 쓴 경우) $\sigma^2$가 저농도에서 과소평가되어 이 오차가 $\eta$에 흡수됩니다. 그 결과 $\omega^2$가 부풀어 오르고, 이 부풀어 오른 $\omega^2$를 "설명하기" 위해 covariate가 $\Delta OFV$를 달성한다 — 그러나 그 covariate는 **residual model artifact의 흡수통로**에 불과합니다. 진짜 covariate signal과 model artifact를 구분하려면 **순서를 지킨다**: residual model을 먼저 안정화한 후 covariate를 탐색합니다. C2의 N3(`Leakage Funnel`)이 정적 시그니처라면, 이 hook은 그 신호가 시간 축으로 펼쳐져 covariate selection 단계까지 왜곡되는 동적 시그니처입니다.

<!-- RECAP -->
**§5 요약(Recap)**: 이 세션의 혼동은 거의 모두 "어느 좌표계의 변이인가"로 귀결됩니다. 즉, 개체 vs 잔차(individual vs residual), total vs unbound, PK vs PD, 평균 vs 모양(mean vs shape)을 구분해야 합니다.

---

<!-- SLIDE_START: S62 | title: §7 — 자기 테스트(Self-Test): 능동 회상 모듈(Active Recall Module) | source_anchor: auto-s62 -->
> **핵심 메시지:** Self-test의 목적은 변이를 잘못된 그릇에 넣는 오류를 능동 회상으로 차단하는 것입니다.

## §7 — 자기 테스트(Self-Test): 능동 회상 모듈(Active Recall Module)

<!-- SELF-TEST -->
<!-- SLIDE_START: S63 | title: Q1. $CL_i=CL\cdot\exp(\eta_i)$를 쓰는 가장 직접적인 이유는 무엇인가? | source_anchor: auto-s63 -->
> **핵심 메시지:** 지수형 IIV는 CL의 양수성을 보장하면서 η를 정규분포로 둘 수 있게 합니다.

### Q1. $CL_i=CL\cdot\exp(\eta_i)$를 쓰는 가장 직접적인 이유는 무엇인가?

**정답(Answer)**: CL이 음수가 되지 않도록 보장하면서, $\eta$를 정규 분포로 둘 수 있기 때문입니다. R&T는 지수형 오차 모델(exponential error model)이 근사적 일정 CV를 제공하고 계산상의 어려움(computational difficulty)을 줄인다고 설명합니다. `[R&T p.371]`

> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{CL_i}_{개인\ 청소율}=\underbrace{CL}_{모집단\ 중심}\cdot\underbrace{\exp(\eta_i)}_{양수\ 배율}
> $$
>
> - **항별 의미:** 좌변은 개인 CL이고, 우변은 모집단 중심 CL에 개인별 exp(ηi) 배율을 곱한 값입니다.
> - **실무 직관:** ηi가 증가하면 CLi가 곱셈적으로 증가하고, ηi가 감소하면 CLi가 감소하지만 0 아래로 내려가지 않습니다.
> - **오해 방지:** Q1의 핵심은 exp 형태 자체가 아니라 양수 보장과 일정 CV 해석을 동시에 주는 이유입니다.


<!-- SELF-TEST -->
<!-- SLIDE_START: S64 | title: Q2. 잔차 변이가 평균 0의 무작위가 아니면 무엇을 뜻하는가? | source_anchor: auto-s64 -->
> **핵심 메시지:** 잔차가 무작위가 아니면 구조 모델이나 오차 모델이 아직 중요한 변이를 설명하지 못한 것입니다.

### Q2. 잔차 변이가 평균 0의 무작위가 아니면 무엇을 뜻하는가?

**정답(Answer)**: 구조 모델 또는 오차 모델이 아직 중요한 변이의 원인을 설명하지 못했다는 뜻입니다. 이 상태에서 공변량을 찾으면, 공변량이 생물학(biology)이 아니라 잔차 모델의 잘못된 지정(residual misspecification)을 설명할 수 있습니다. `[R&T p.372]`

<!-- SELF-TEST -->
<!-- SLIDE_START: S65 | title: Q3. PK50에서 "total Cl CV 28%, unbound Clu CV 9%"를 어떻게 해석해야 하는가? | source_anchor: auto-s65 -->
> **핵심 메시지:** PK50의 CV 차이는 covariate가 ω²를 깎은 결과가 아니라 좌표계 재표현의 결과입니다.

### Q3. PK50에서 "total Cl CV 28%, unbound Clu CV 9%"를 어떻게 해석해야 하는가?

**정답(Answer)**: $f_u$를 공변량으로 넣어 $\omega^2$을 28%에서 9%로 줄였다는 뜻이 아닙니다. 같은 12명 CpD 자료를 total 좌표와 unbound 좌표로 표현했을 때, 단백 결합 변이가 total parameter variability의 일부를 설명한다는 뜻입니다. `[G&W p.706–709]`

<!-- SELF-TEST -->
<!-- SLIDE_START: S66 | title: Q4. $CV(CL)^2 \approx CV(f_u)^2 + CV(CL_u)^2$를 이 문서에서 쓰지 않는 이유는 무엇인가? | source_anchor: auto-s66 -->
> **핵심 메시지:** 출처와 가정이 방어되지 않는 분산 전파식은 source-locked handout에서 사용하지 않아야 합니다.

### Q4. $CV(CL)^2 \approx CV(f_u)^2 + CV(CL_u)^2$를 이 문서에서 쓰지 않는 이유는 무엇인가?

**정답(Answer)**: 이 분산 전파식(variance propagation)은 첨부 PDF에 없고, 독립성 가정도 제시되지 않았으며, Audit에서 산술적으로도 검증되지 않는다고 판정되었습니다. 따라서 source-locked Content Lock에서는 삭제합니다.

> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{CV(CL)^2}_{total\ CL\ 변이}\approx\underbrace{CV(f_u)^2}_{단백결합\ 변이}+\underbrace{CV(CL_u)^2}_{unbound\ CL\ 변이}
> $$
>
> - **항별 의미:** 좌변은 total CL의 변이, 우변은 fu 변이와 unbound CL 변이를 더해 설명하려는 근사 구조입니다.
> - **실무 직관:** 이 식을 쓰려면 좌표계, 독립성, 분산 전파 가정이 모두 방어되어야 합니다.
> - **오해 방지:** 이 문서에서는 이 식을 채택하지 않습니다. 따라서 교육적 annotation도 ‘왜 쓰지 않는가’를 설명하는 경계 표시로 읽어야 합니다.
> **검토 메모:** 원문은 이 식을 사용하지 않는 이유를 설명합니다. 원문 보존 원칙에 따라 식 자체와 삭제 판단은 변경하지 않았습니다.


<!-- SELF-TEST -->
<!-- SLIDE_START: S67 | title: Q5. PK50에서 비결합 농도(unbound concentration)를 써도 PD 변이가 남은 이유는 무엇인가? | source_anchor: auto-s67 -->
> **핵심 메시지:** Unbound PK 좌표계를 정리해도 반응 변이는 수용체와 유전 구성 같은 PD 축으로 남을 수 있습니다.

### Q5. PK50에서 비결합 농도(unbound concentration)를 써도 PD 변이가 남은 이유는 무엇인가?

**정답(Answer)**: 단백 결합은 PK 좌표계 변이의 일부를 설명하지만, 반응 변이(response variability)는 수용체 밀도(receptor density)/유전적 구성(genetic makeup) 같은 PD 원인을 포함합니다. G&W는 $E_{max}<1$인 무반응자(non-responder)가 표적의 완전한 발현(full target expression)을 결여한 것으로 해석합니다. `[G&W p.707–709]`

> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{E_{max}}_{도달\ 가능한\ 최대효과}\ <\ \underbrace{1}_{완전\ 표적\ 반응\ 기준}
> $$
>
> - **항별 의미:** 좌변은 개체가 도달 가능한 최대효과이고, 우변은 완전 반응으로 해석되는 기준입니다.
> - **실무 직관:** Emax가 낮으면 농도 또는 unbound concentration을 설명해도 반응 한계가 남을 수 있습니다.
> - **오해 방지:** PK 변이를 설명했다고 PD 변이까지 설명된 것으로 간주하면 안 됩니다.


<!-- SELF-TEST -->
<!-- SLIDE_START: S68 | title: Q6. $\eta$-EBE 히스토그램이 두 봉우리로 보이면 가장 먼저 무엇을 의심해야 하는가? | source_anchor: auto-s68 -->
> **핵심 메시지:** EBE는 개인 참값이 아니라 모델과 자료가 허용한 조건부 η 추정치이므로 shrinkage를 항상 함께 보아야 합니다.

### Q6. $\eta$-EBE 히스토그램이 두 봉우리로 보이면 가장 먼저 무엇을 의심해야 하는가?

**정답(Answer)**: 매끄러운 로그정규 IIV 하나가 아니라, 유전형/표현형 하위군(genotype/phenotype subgroup) 또는 혼합형 모집단 구조(mixture-like population structure)가 섞였는지 의심해야 합니다. 단, 희소 표본(sparse sampling)이면 히스토그램 자체의 정보량을 먼저 점검해야 합니다. `[R&T p.366, p.393–397]`

<!-- SELF-TEST -->
<!-- SLIDE_START: S69 | title: Q7. 인종 공변량(ethnicity covariate)을 유전형(genotype)처럼 해석하면 왜 위험한가? | source_anchor: auto-s69 -->
> **핵심 메시지:** Ethnicity는 individual genotype의 불완전한 proxy이므로 mechanistic genetic covariate처럼 해석하면 안 됩니다.

### Q7. 인종 공변량(ethnicity covariate)을 유전형(genotype)처럼 해석하면 왜 위험한가?

**정답(Answer)**: R&T는 인종이 개인 유전 형질(individual genetic characteristics)의 불완전한 대체지표(proxy)이며, 그룹 내 변이도 크다고 경고합니다. 인종 항목(ethnicity term)은 탐색적 공변량(exploratory covariate)일 수 있지만 mechanistic 유전형의 대체물은 아닙니다. `[R&T p.395, p.409]`

<!-- SELF-TEST -->
<!-- SLIDE_START: S70 | title: Q8. 다음 중 먼저 고칠 것은 무엇인가: 잔차 모델의 잘못된 지정 vs 공변량 모델 추가? | source_anchor: auto-s70 -->
> **핵심 메시지:** Residual misspecification이 의심되면 공변량을 추가하기보다 ε 구조 안정화를 먼저 방어해야 합니다.

### Q8. 다음 중 먼저 고칠 것은 무엇인가: 잔차 모델의 잘못된 지정 vs 공변량 모델 추가?

<!-- SLIDE_START: S71 | title: ⚡ 보스 딜레마(Boss Dilemma) ★★ — [EXPERT_INFERENCE, v3] | source_anchor: auto-s71 -->
> **핵심 메시지:** 이 slide에서는 하나의 개념을 하나의 실무 판단으로 압축해 다음 모델링 단계와 연결해야 합니다.

> **⚡ 보스 딜레마(Boss Dilemma) ★★ — [EXPERT_INFERENCE, v3]**  
> 잔차 모델의 잘못된 지정(residual model misspecification)이 의심되는 상황에서 공변량 탐색을 시작하려 합니다. 두 가지 전략이 충돌합니다:  
>
> **선택지 A — 잔차 안정화 우선**: $\varepsilon$ 구조를 먼저 안정화합니다. $\varepsilon$이 자리 잡으면 $\omega^2$의 진짜 공변량 신호(covariate signal)가 드러납니다. 단점: 잔차 모델 선택이 지연되면 전체 모델 구축 일정이 늦어지고, sponsor가 timeline pressure를 가하는 후기 단계에서는 부담이 됩니다.  
>
> **선택지 B — 공변량 탐색 우선**: $\omega^2$ 감소 패턴이 어떤 공변량이 IIV를 설명하는지 보여 주고, 이후 잔차 모델을 조정합니다. 단점: 잔차 모델 artifact가 공변량으로 잘못 흡수될 위험이 있습니다(N3 `Leakage Funnel`이 시간축으로 펼쳐지는 시나리오; Pair 4 기억 고리 참조).  
>
> 각 선택을 규제 보고서에서 어떻게 방어하는가?

**정답(Answer, 표층)**: 잔차 모델의 잘못된 지정을 먼저 고칩니다. $\varepsilon$의 구조가 틀린 상태에서 공변량을 추가하면 $\omega^2$와 $\sigma^2$의 경쟁이 왜곡되어, 생물학이 아니라 오차 모델의 실패를 공변량이 설명하게 됩니다. `[R&T p.371–373]`

**거장의 상충 논리(Trade-off) — [EXPERT_INFERENCE, v3]**:  
A는 순서상 올바르지만 초기 데이터 탐색 단계에서는 잔차 모델이 아직 불확실하므로 무한 재탐색 루프(reseach loop)에 빠질 수 있습니다. 실무 베테랑은 **A를 default로 하되 single-iteration 한도를 둔다** — 즉 잔차 모델 후보 2–3개를 a priori 정의하고 단 한 번의 비교로 결정한 후 공변량 탐색으로 진행합니다.

B는 탐색적이지만 최종 모델에서 잔차 모델을 명시적으로 재검토했다는 증거를 제시해야 합니다. 이 경로를 택한다면 보고서에 **반드시 민감도 분석(sensitivity analysis)**(다른 잔차 모델에서 공변량 선택 결과가 어떻게 달라지는지)을 명시합니다.
...
<!-- RECAP -->
**§7 요약(Recap)**: 시험 문제의 정답은 대부분 "변이의 원인을 잘못된 그릇에 넣지 말라"입니다.

---

<!-- SLIDE_START: S72 | title: §8 — 메타 프레임 및 큰 그림(Meta-Frame & Big Picture) | source_anchor: auto-s72 -->
> **핵심 메시지:** Session 13은 PopPK curriculum에서 평균이 아니라 variance axis를 책임지는 핵심 세션입니다.

## §8 — 메타 프레임 및 큰 그림(Meta-Frame & Big Picture)

<!-- SLIDE_START: S73 | title: A. 위치 설정(Positioning) — 28-session architecture | source_anchor: auto-s73 -->
> **핵심 메시지:** 이 세션의 variance decomposition은 이후 estimation, diagnostics, covariate selection, Bayesian TDM의 전제입니다.

### A. 위치 설정(Positioning) — 28-session architecture

이 세션은 PopPK 커리큘럼의 **분산 축(variance axis)**다. 이전 세션들이 Cl, V, ka, $t_{1/2}$ 같은 구조 파라미터(structural parameter)의 평균을 세웠다면, 이 세션은 그 평균 주변의 개인차와 잔차를 분해합니다. 따라서 이후 FOCE/FOCE-I, GOF, 공변량 선택, Bayesian TDM은 모두 이 분해 위에서만 의미가 있습니다.

<!-- SLIDE_START: S74 | title: B. 의존성(Dependencies) — 대충 넘겼을 때 발생하는 실패 모드 | source_anchor: auto-s74 -->
> **핵심 메시지:** θ·η·ε·fu·genotype 좌표계를 대충 넘기면 보고서 표는 있어도 dose rationale은 무너집니다.

### B. 의존성(Dependencies) — 대충 넘겼을 때 발생하는 실패 모드

| 의존 항목 | 대충 넘기면 생기는 실패 |
|---|---|
| C1 $\theta$/$\eta$/$\varepsilon$ | `$OMEGA`와 `$SIGMA`를 숫자로만 읽고, 생리학과 잔차 noise를 구분하지 못합니다. |
| C2 잔차 오차 | 잘못된 $\varepsilon$가 $\omega^2$로 새어 가짜 IIV 또는 가짜 공변량을 만듭니다. |
| C3 공변량/재모수화 | $f_u$, CrCl, 체중을 모두 같은 방식의 공변량으로 취급해 좌표계 변환과 인과적 예측변수(causal predictor)를 혼동합니다. |
| C4 유전 다형성 | 이봉형 분포를 단일 로그정규 $\eta$로 덮어 하위군 용량 위험을 숨깁니다. |
| EDA 우선 | spaghetti plot과 변환 플롯 없이 control stream부터 쓰다가 하위군 신호를 놓칩니다. `[G&W p.334–336]` |
| 규제 PopPK 일관성 | $\theta$, $\omega^2$, $\sigma^2$, 잔차 모델, 공변량 효과가 서로 일관되지 않으면 보고서 표는 완성되어 있어도 용량 근거(dose rationale)가 무너집니다. `[R&T p.373]` |

<!-- SLIDE_START: S75 | title: 체화 노트(Mastery Note) — [CRUCIBLE_DERIVED] · NDA Section 5.2 line-item 일 | source_anchor: auto-s75 -->
> **핵심 메시지:** Regulatory reviewer는 각 line item보다 ω², σ², residual model, covariate effect 사이의 일관성을 봅니다.

> **체화 노트(Mastery Note) — [CRUCIBLE_DERIVED] · NDA Section 5.2 line-item 일관성**  
> 규제 reviewer는 PopPK 보고서의 $\omega^2$, $\sigma^2$, 잔차 모델, $\eta_{EBE}$ 분포, 공변량 효과를 **순차적인 line item으로** 읽습니다. 그러나 reviewer가 실제로 감사하는 것은 line item 각각이 아니라 **그들 사이의 상호 일관성**입니다. 예를 들어 $\sigma^2$(proportional)으로 보고된 CV가 분석법 검증 보고서(bioanalytical assay validation)의 CV와 동떨어지면, 모델 전체의 prior justification이 흔들리고 첫 번째 의문 한 줄이 후속 모든 표에 cascade됩니다. **체화 꿀팁**: Session 13의 모든 분산 결정은 **자체로 정합적일 뿐 아니라 인접 보고서들의 좌표계와 일치해야 한다** — 즉 $\sigma^2$ ↔ assay CV, total threshold ↔ unbound dose recommendation, $\eta$ 분포 모양 ↔ 모집단 약리유전 컨텍스트(population pharmacogenetic context)가 한 묶음으로 검토됩니다. **실무 활용**: 모델 lock 전에 (1) bioanalytical CV 표, (2) 안전 임계 좌표계(safety threshold 좌표계), (3) 모집단 유전 reference를 한 번 더 펼쳐 line item별 cross-check sheet를 만듭니다. 이는 deficiency letter 자체에 대한 외부 주장이 아니라 R&T p.373이 말하는 "regulatory requirement"의 실무 번역입니다.

<!-- SLIDE_START: S76 | title: C. 전문가의 해자(Professional Moat) — 이 세션이 전문가를 가르는 지점 | source_anchor: auto-s76 -->
> **핵심 메시지:** 전문가는 IIV가 크다는 말에서 멈추지 않고 그 변이가 PK, PD, residual, coordinate, subgroup 중 어디에 속하는지 묻습니다.

### C. 전문가의 해자(Professional Moat) — 이 세션이 전문가를 가르는 지점

초급자는 "IIV가 큽니다"라고 말합니다. 중급자는 "CL에 covariate를 넣겠습니다"라고 말합니다. 전문가의 질문은 다릅니다.

1. 이 변이는 **PK인가 PD인가**? `[R&T p.363–364; G&W p.707–709]`
2. 이 변이는 **$\eta$인가 $\varepsilon$인가**? `[R&T p.369–373]`
...
<!-- SLIDE_START: S77 | title: 🔬 NONMEM 출력 시그니처 매핑 — [EXPERT_INFERENCE, v3] | source_anchor: auto-s77 -->
> **핵심 메시지:** 거장의 질문은 추상적 논평이 아니라 OMEGA, SIGMA, shrinkage, VPC, covariance output에서 검증되어야 합니다.

> **🔬 NONMEM 출력 시그니처 매핑 — [EXPERT_INFERENCE, v3]**  
> 위 5개 질문은 추상적 사고가 아닙니다. 각 질문은 **실제 NONMEM 출력 또는 진단 파일의 특정 시그니처**로 답해야 합니다. 전문가 해석 지점(Professional Moat)은 질문을 던지는 능력이 아니라, 그 질문을 출력 파일에서 읽어 내는 능력입니다.
>
> | 거장의 질문 | NONMEM/진단 출력 시그니처 |
> |---|---|
> | Q1. PK인가 PD인가? | PK 자료(농도)와 PD 자료(반응)을 **별도 모델로 적합**한 후 $\omega^2$(PK 파라미터)와 $\omega^2$(PD 파라미터)를 비교; 같은 환자에서 $\eta_{CL}$과 $\eta_{EC50}$의 상관이 낮으면 두 영역이 독립적인 IIV 원인임을 시사. |
> | Q2. $\eta$인가 $\varepsilon$인가? | **OMEGA 행렬의 대각 원소 변화** vs **SIGMA 추정값 변화**를 잔차 모델 변경 전후로 비교. $\varepsilon$ 모델을 바꿨을 때 $\omega^2$(CL)이 크게 움직이면 N3의 leakage funnel이 작동 중. $\eta$-shrinkage $<30%$일 때만 $\eta$-EBE 기반 진단을 신뢰. |
> | Q3. Total인가 unbound 좌표계인가? | Total 파라미터 적합 출력의 $\omega^2$(CL)과 unbound 파라미터 적합 출력의 $\omega^2$(CLu)을 **나란히 비교**; PK50 패턴(28% vs 9%)이 재현되면 fu가 결정론적 재모수화로 작동 중. fu 자체에 $\eta$를 부여한 모델과 부여하지 않은 모델의 $\Delta OFV$로 fu 측정 변이 분리 여부 판정. |
> | Q4. Smooth인가 하위군 구조인가? | **$\eta$-EBE 히스토그램**(R: `xpose4::eta.dist()` 또는 PsN sse output)에서 이봉형 여부 확인; 혼합 모델(mixture model) `$MIX`의 $\Delta OFV$ vs 단일 로그정규 IIV — 단, 작은 n cell에서는 N5 `Empty Phenotype Cell` 시그니처(RSE $>80%$, `$COV step` 실패) 우선 점검. |
> | Q5. Mechanistic인가 proxy인가 식별 불가능한가? | **Bootstrap 또는 SIR**으로 공변량 효과의 95% 신뢰구간 폭을 확인; CI가 0을 포함하지 않더라도 폭이 비대칭하거나 매우 넓으면 식별가능성(identifiability) 결손 의심. `$COV step`의 condition number($>1000$이면 ill-conditioned)와 공변량-공변량 상관행렬($|r|>0.7$이면 다중공선성(collinearity))을 함께 봅니다. |

> **Annotated equation — 수식 해부**
>
> $$
> \underbrace{\eta\text{-shrinkage}}_{EBE\ 분해능\ 저하\ 지표}<\underbrace{30\%}_{EBE\ 진단\ 신뢰\ 경고선},\quad \underbrace{RSE}_{추정\ 불확실성}>\underbrace{80\%}_{불안정\ 경고},\quad \underbrace{condition\ number}_{수치\ 조건성}>\underbrace{1000}_{ill\text{-}conditioned\ 경고},\quad \underbrace{|r|}_{공변량\ 상관}>\underbrace{0.7}_{다중공선성\ 경고}
> $$
>
> - **항별 의미:** 각 부등식은 EBE 진단, categorical effect 안정성, covariance step 조건성, covariate collinearity를 판단하는 실무 경고선입니다.
> - **실무 직관:** shrinkage가 낮을수록 EBE plot 해석 여지가 커지고, RSE·condition number·|r|가 커질수록 추정 안정성은 약해집니다.
> - **오해 방지:** 이 경고선들은 모델을 자동 판정하는 법칙이 아니라, 보고서에서 추가 확인이 필요한 위험 신호입니다.

>
> **체화 꿀팁**: 각 거장 질문에 NONMEM 출력 한 곳을 1:1로 매핑하지 못하면 그 질문은 보고서에서 방어할 수 없습니다. 보고서 line item마다 어느 출력에서 그 결론이 나왔는지 cross-reference 가능해야 한다 — 이것이 N6(NDA 5.2 line-item consistency)이 실무에서 작동하는 방식입니다.

<!-- RECAP -->
<!-- SLIDE_START: S78 | title: 최종 요약(Final Recap) | source_anchor: auto-s78 -->
> **핵심 메시지:** ω²는 단순한 남은 변이가 아니라 아직 이름 붙이지 못한 생리학·유전학·행동·측정 구조의 혼합물입니다.

### 최종 요약(Final Recap)

Session 13의 최종 메시지는 하나입니다. **$\omega^2$은 "남은 변이"가 아니라 아직 이름 붙이지 못한 생리학·유전학·행동·측정 구조의 혼합물입니다.** C2는 잔차 noise를 정리하고, C3는 설명 가능한 생리학을 꺼냅니다. C4는 매끄러운 IIV 안에 숨어 있는 유전적 불연속성(genetic discontinuity)을 드러냅니다. 이 순서를 지키지 않으면 NONMEM은 돌아가지만, 용량 개체화(dose individualization)는 성립하지 않습니다.
