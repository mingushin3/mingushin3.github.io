...
> **핵심 메시지:** 이 자료는 0.75를 외우는 자료가 아니라, allometry를 노출·시간·용량 판단으로 연결하는 항법도입니다.
...
> **핵심 메시지:** $Y=a\cdot BW^b$는 단순 회귀식이 아니라, 종간 체중 차이를 PK 파라미터 판단으로 번역하는 중심 언어입니다.
...
>
> $$
> \overbrace{b=1}^{\text{kg당 선형 가정}}\quad vs\quad \overbrace{b=0.75}^{\text{흐름 변수 prior}}
> $$
>
> - **항별 의미:** 좌변/우변이라기보다 두 지수 선택의 대비입니다. $b=1$은 체중에 선형 비례하는 kg당 사고이고, $b=0.75$는 CL 같은 흐름 변수의 생리학적 prior입니다.
> - **실무 직관:** 동일한 체중비에서도 $b=1$은 $b=0.75$보다 human CL과 target-AUC dose를 훨씬 크게 만듭니다. 따라서 지수 선택은 초기 노출 판단을 바꿉니다.
> - **오해 방지:** $b=0.75$도 자동 정답은 아닙니다. 이 block은 $b=1$ 오류를 막기 위한 비교이지, 모든 약물에 0.75를 고정하라는 뜻이 아닙니다.
...
> **핵심 메시지:** CL, V, 시간, Dedrick, 용량은 별개 주제가 아니라 하나의 allometric spine에서 순서대로 파생됩니다.
...
> **핵심 메시지:** 이 schematic의 목적은 C1–C5를 공식 모음이 아니라 의사결정 흐름으로 보이게 하는 것입니다.
...
>
> $$
> \underbrace{Y=a\cdot BW^b}_{\text{마스터 식}}\rightarrow\underbrace{CL\sim BW^{0.75}}_{\text{흐름}}\; /\;\underbrace{V\sim BW^1}_{\text{공간}}\rightarrow\underbrace{t_{1/2}\sim BW^{d-b}}_{\text{시간}}\; /\;\underbrace{k_{el}\sim BW^{b-d}}_{\text{속도상수}}\rightarrow\underbrace{Dose\sim BW^b}_{\text{equal-AUCu 백본}}
> $$
>
> - **항별 의미:** 이 schematic 안 수식군은 하나의 좌변을 가진 단일 방정식이 아니라, 변수 유형별 지수 선택과 그 결과를 연결한 흐름입니다.
> - **실무 직관:** CL과 V의 지수 차이가 시간축을 만들고, CL 지수는 equal-AUCu dose backbone으로 이어집니다. 각 화살표는 다음 실무 판단으로 넘어가는 gate입니다.
> - **오해 방지:** 이 흐름도를 개별 공식 암기 목록으로 읽으면 안 됩니다. 각 식은 앞 식의 지수 판단을 상속합니다.
...
> **핵심 메시지:** $Y=a\cdot BW^b$의 핵심은 체중 변화가 변수 값을 몇 배로 바꾸는지 하나의 지수로 압축한다는 점입니다.
...
>
> $$
> \underbrace{Y}_{\text{예측·정의할 변수}}=\underbrace{a}_{\text{기준 절편}}\cdot \underbrace{BW}_{\text{체중}}^{\overbrace{b}^{\text{스케일링 지수}}}
> $$
>
> - **항별 의미:** 좌변 $Y$는 CL, V, 용량 같은 예측 대상 PK/생리 변수를 뜻합니다. 우변의 $a$는 기준 체중에서의 절편이고, $BW^b$는 체중 차이가 변수 크기로 번역되는 비선형 배율입니다.
> - **실무 직관:** $BW$가 커지면 $b>0$인 한 $Y$는 증가하지만, $b<1$이면 체중 증가보다 느리게 증가합니다. 이 식은 종간 외삽에서 어떤 지수를 쓸지 판단하게 합니다.
> - **오해 방지:** $b$를 모든 변수에 같은 상수로 고정하면 안 됩니다. CL의 $b$와 V의 $d$는 생리학적 의미가 다릅니다.
...
>
> $$
> \underbrace{\ln(Y)}_{\text{로그 변수}}=\underbrace{\ln(a)}_{\text{로그 절편}}+\overbrace{\underbrace{b}_{\text{기울기}}\cdot\underbrace{\ln(BW)}_{\text{로그 체중}}}^{\text{log-log 직선의 체중 효과}}
> $$
>
> - **항별 의미:** 좌변은 원 변수 $Y$를 로그 스케일에서 정의합니다. 우변은 로그 절편 $\ln(a)$와 로그 체중에 대한 기울기 $b$로 나뉩니다.
> - **실무 직관:** $b$가 커질수록 log-log plot에서 기울기가 가팔라지고, 체중 차이가 $Y$ 차이로 더 크게 반영됩니다. 이 변환은 회귀 추정과 지수 해석을 가능하게 합니다.
> - **오해 방지:** 로그-로그 직선성이 보인다고 해서 기전이 자동으로 동일하다는 뜻은 아닙니다. scale-invariance 가정의 타당성을 별도로 확인해야 합니다.
...
> **핵심 메시지:** 거듭제곱 law는 편리한 곡선맞춤이 아니라, 크기 변화에도 유지되는 생리학적 스케일 구조를 가정합니다.
...
> **핵심 메시지:** CL은 공간 크기가 아니라 시간당 처리 흐름이므로, 선형 kg당 사고가 아니라 $b\approx0.75$ prior에서 출발합니다.
...
>
> $$
> \underbrace{CL_i}_{\text{개체 }i\text{의 청소율}}=\underbrace{a}_{\text{CL 기준 절편}}\cdot\underbrace{BW_i}_{\text{개체 }i\text{ 체중}}^{\overbrace{b}^{\text{CL 흐름 지수}}}
> $$
>
> - **항별 의미:** 좌변은 개체 $i$의 clearance입니다. 우변은 기준 CL 절편 $a$와 체중 기반 흐름 지수 $b$로 구성됩니다.
> - **실무 직관:** $BW_i$ 또는 $b$가 커지면 CL 예측값은 증가합니다. 다만 $b\approx0.75$이면 체중에 선형 비례하지 않고 느리게 증가합니다.
> - **오해 방지:** CL은 공간 크기가 아니라 시간당 처리 흐름입니다. 따라서 $b=1$ kg당 선형 스케일링을 기본값처럼 넣는 것이 가장 흔한 오류입니다.
...
> **핵심 메시지:** $b=1$과 $b=0.75$의 차이는 소수점 차이가 아니라 첫 human exposure estimate의 자릿수를 바꿀 수 있는 차이입니다.
...
>
> $$
> \underbrace{\left(\frac{BW_{human}}{BW_{animal}}\right)^{1-0.75}}_{\text{선형 kg당 가정과 0.75 prior의 fold 차이}}=\underbrace{\left(\frac{BW_{human}}{BW_{animal}}\right)^{0.25}}_{\text{species pair별 격차}}
> $$
>
> - **항별 의미:** 좌변은 $b=1$로 계산한 값이 $b=0.75$ 계산보다 몇 배 큰지를 정의합니다. 우변의 0.25는 두 지수의 차이입니다.
> - **실무 직관:** 체중비가 클수록 이 fold 차이는 커집니다. 그래서 mouse→human과 rat→human은 같은 '0.25 차이'라도 다른 fold를 냅니다.
> - **오해 방지:** 4.1, 7.4, 7.7을 서로 바꾸어 쓰면 안 됩니다. species pair와 기준 체중이 달라서 생기는 값입니다.
...
> **핵심 메시지:** $b\approx0.75$는 출발 prior일 뿐이며, binding·metabolism·nonlinearity 신호가 있으면 total CL 대신 기전과 $CL_u$를 먼저 점검해야 합니다.
...
> **핵심 메시지:** 이 figure pointer는 linear per-kg scaling이 human CL과 dose를 얼마나 크게 밀어 올릴 수 있는지 시각적으로 확인시키는 장치입니다.
...
> **핵심 메시지:** 넓은 예측구간은 단순 잡음이 아니라 mechanism 또는 data-quality signal로 읽어야 합니다.
...
> **핵심 메시지:** V는 흐름이 아니라 분포 공간이므로, CL의 $b$와 분리해 $d\approx1$ prior로 읽어야 합니다.
...
>
> $$
> \underbrace{V_i}_{\text{개체 }i\text{의 분포용적}}=\underbrace{c}_{\text{V 기준 절편}}\cdot\underbrace{BW_i}_{\text{체중}}^{\overbrace{d}^{\text{분포 범위 지수}}}
> $$
>
> - **항별 의미:** 좌변은 개체 $i$의 volume of distribution입니다. 우변의 $c$는 V 절편이고, $d$는 체중이 분포 공간으로 번역되는 지수입니다.
> - **실무 직관:** $d\approx1$이면 체중이 2배일 때 V도 대략 2배로 증가합니다. 이 판단은 half-life와 Dedrick 시간축의 출발점입니다.
> - **오해 방지:** V의 지수 $d$를 CL의 $b\approx0.75$와 바꾸어 쓰면 안 됩니다. V는 흐름이 아니라 공간/extent 변수입니다.
...
> **핵심 메시지:** $V/CL$과 $CL/V$를 동시에 보면, 큰 동물에서 반감기는 길어지고 제거 fraction rate는 작아지는 이유가 한 번에 보입니다.
...
>
> $$
> \underbrace{t_{1/2}}_{\text{반감기}}=\underbrace{\ln2}_{\text{50\% 감소 상수}}\cdot\frac{\overbrace{V}^{\text{분포 공간}}}{\overbrace{CL}^{\text{제거 흐름}}}=\ln2\cdot\frac{\underbrace{c\cdot BW^d}_{\text{V 스케일}}}{\underbrace{a\cdot BW^b}_{\text{CL 스케일}}}\propto \underbrace{BW^{d-b}}_{\text{시간 지수}}
> $$
>
> - **항별 의미:** 좌변은 concentration이 절반으로 줄어드는 시간입니다. 우변은 분포 공간 $V$를 제거 흐름 $CL$로 나눈 시간 비율이며, allometry를 대입하면 $BW^{d-b}$가 남습니다.
> - **실무 직관:** $d>b$이면 체중이 큰 종에서 반감기가 길어집니다. 이 식은 큰 동물이 절대 CL은 크지만 fraction 제거는 느린 이유를 설명합니다.
> - **오해 방지:** $t_{1/2}$만 외우고 $V/CL$ 구조를 잊으면, V 지수와 CL 지수를 바꾸거나 $k_{el}$ 방향을 반대로 해석하기 쉽습니다.
...
>
> $$
> \underbrace{t_{1/2}}_{\text{시간 결과}}\propto \underbrace{BW^{0.25}}_{\text{큰 종에서 증가}},\qquad \underbrace{k_{el}}_{\text{제거 속도상수}}=\frac{\overbrace{CL}^{\text{흐름}}}{\overbrace{V}^{\text{공간}}}\propto\underbrace{BW^{b-d}}_{\text{큰 종에서 감소}}\approx BW^{-0.25}
> $$
>
> - **항별 의미:** 좌변들은 반감기와 제거 속도상수를 각각 정의합니다. 우변은 같은 $V$와 $CL$의 비율을 서로 반대 방향으로 읽은 결과입니다.
> - **실무 직관:** $BW$가 커지면 $t_{1/2}$는 증가하고 $k_{el}$은 감소합니다. 임상적으로는 큰 종에서 concentration curve가 더 천천히 내려가는 방향입니다.
> - **오해 방지:** 큰 동물의 절대 CL이 크다는 사실과 $k_{el}$이 작다는 사실은 모순이 아닙니다. 분포 공간이 더 크게 증가하기 때문입니다.
...
> **핵심 메시지:** V 지수는 대체로 1이지만, 조직 친화성·체조성·binding이 바뀌면 시간축 전체가 흔들릴 수 있습니다.
...
> **핵심 메시지:** 이 schematic은 CL 지수와 V 지수를 서로 바꾸는 순간 half-life와 $k_{el}$ 해석이 동시에 무너진다는 점을 고정합니다.
...
>
> $$
> \underbrace{CL=a\cdot BW^b}_{\text{흐름 지수}}\quad vs\quad\underbrace{V=c\cdot BW^d}_{\text{공간 지수}}\quad\Rightarrow\quad\underbrace{t_{1/2}\propto BW^{d-b}}_{\text{시간 증가}}\; ,\;\underbrace{k_{el}\propto BW^{b-d}}_{\text{fraction 속도 감소}}
> $$
>
> - **항별 의미:** 이 figure-marker 수식군은 CL과 V의 지수 대비가 시간·속도 결과를 어떻게 만드는지 정의합니다.
> - **실무 직관:** $b\approx0.75$, $d\approx1$이면 큰 종에서 반감기는 증가하고 $k_{el}$은 감소합니다. 이 판단은 Dedrick 시간축 선택으로 연결됩니다.
> - **오해 방지:** CL과 V를 모두 체중의 같은 지수로 처리하면, 반감기와 rate constant의 방향성을 설명할 수 없습니다.
...
> **핵심 메시지:** Dedrick plot은 예쁜 overlay가 아니라 CL 지수와 V 지수의 조합이 데이터에서 버티는지 보는 진단 도구입니다.
...
>
> $$
> \underbrace{y}_{\text{정규화 농도}}=\frac{\overbrace{C}^{\text{농도}}}{\underbrace{Dose/BW}_{\text{kg당 투여량}}},\qquad \underbrace{x}_{\text{정규화 시간}}=\frac{\overbrace{t}^{\text{관찰 시간}}}{\underbrace{BW^{1-b}}_{\text{elementary 시간 보정}}}
> $$
>
> - **항별 의미:** 좌변의 $y$와 $x$는 elementary Dedrick plot의 농도축과 시간축입니다. $d=1$을 가정하므로 농도는 $Dose/BW$, 시간은 $BW^{1-b}$로 정규화됩니다.
> - **실무 직관:** $b$가 작을수록 시간축 보정 $1-b$가 커져 종간 시간 차이가 더 크게 반영됩니다. 이 변환은 curve collapse가 가능한지 진단하게 합니다.
> - **오해 방지:** Elementary Dedrick은 $d=1$일 때의 특수형입니다. V 지수가 1에서 벗어나면 이 축 변환을 그대로 고집하면 안 됩니다.
...
>
> $$
> \underbrace{y}_{\text{정규화 농도}}=\frac{\overbrace{C}^{\text{농도}}}{\underbrace{Dose/BW^d}_{\text{분포 지수 반영 투여량}}}=\frac{\overbrace{C}^{\text{농도}}\cdot\overbrace{BW^d}^{\text{분포 공간 보정}}}{\underbrace{Dose}_{\text{투여량}}},\qquad \underbrace{x}_{\text{정규화 시간}}=\frac{\overbrace{t}^{\text{시간}}}{\underbrace{BW^{d-b}}_{\text{complex 시간 보정}}}
> $$
>
> - **항별 의미:** 좌변은 complex Dedrick의 농도축과 시간축입니다. V 지수 $d$를 농도 정규화와 시간 정규화에 모두 반영합니다.
> - **실무 직관:** $d$가 1보다 크면 큰 체중에서 농도축과 시간축 보정이 elementary보다 강해집니다. PK29처럼 $d
eq1$인 경우 curve superposition 판단에 필요합니다.
> - **오해 방지:** Complex Dedrick은 장식적 대안이 아니라 $d=1$ 가정이 깨질 때 필요한 일반형입니다.
...
> **핵심 메시지:** 농도축은 $Dose/BW^d$로, 시간축은 $BW^{d-b}$로 정규화해야 지수 구조가 curve 위에서 검증됩니다.
...
>
> $$
> \underbrace{C}_{\text{농도}}=\frac{\underbrace{D_{iv}}_{\text{IV 투여량}}}{\underbrace{V}_{\text{분포용적}}}\cdot \overbrace{e^{-(CL/V)t}}^{\text{일차 제거에 따른 감소}}
> $$
>
> - **항별 의미:** 좌변은 시간 $t$에서의 농도입니다. 우변의 $D_{iv}/V$는 초기 농도 크기, 지수항은 $CL/V$ 속도상수에 따른 감소를 뜻합니다.
> - **실무 직관:** $V$가 커지면 초기 농도는 낮아지고, $CL/V$가 커지면 농도는 더 빠르게 감소합니다. 이 식은 Dedrick 도출의 원형입니다.
> - **오해 방지:** $CL$이 크다고 항상 농도 감소가 빠른 것은 아닙니다. 제거 속도는 $CL$ 단독이 아니라 $CL/V$로 결정됩니다.
...
>
> $$
> \underbrace{C}_{\text{농도}}=\frac{\underbrace{D_{iv}}_{\text{투여량}}}{\underbrace{c\cdot BW^d}_{\text{V allometry}}}\cdot \overbrace{e^{-\left(\underbrace{a/c}_{\text{CL/V 절편비}}\right)\cdot\underbrace{BW^{b-d}}_{\text{속도상수 체중 지수}}\cdot \underbrace{t}_{\text{시간}}}}^{\text{체중 보정된 일차 제거}}
> $$
>
> - **항별 의미:** 좌변은 allometric substitution 후의 농도입니다. 우변은 투여량을 V scaling으로 나눈 초기 농도와, $BW^{b-d}$가 들어간 제거 지수항으로 나뉩니다.
> - **실무 직관:** $d$가 커지면 초기 농도는 낮아지는 방향이고, $b-d$가 더 음수이면 큰 종에서 제거 속도상수가 더 작아집니다. 이 식은 Dedrick 축 변환의 수학적 근거입니다.
> - **오해 방지:** 농도축과 시간축을 별도로 정규화해야 합니다. 시간축만 조정하면 V scaling 차이를 놓칠 수 있습니다.
...
> **핵심 메시지:** PK28은 elementary Dedrick의 기준점이고, PK29는 $d\neq1$에서 complex Dedrick으로 넘어가야 하는 기준점입니다.
...
> **핵심 메시지:** 이 figure pointer는 $d=1$ 가정이 성립할 때 시간축과 농도축 정규화가 어떻게 curve를 겹치게 하는지 확인시키는 기준점입니다.
...
> **핵심 메시지:** PK29는 $d\neq1$과 multicompartment scaling이 들어올 때 elementary plot만으로는 부족하다는 최종 시각 증거입니다.
...
> **핵심 메시지:** C5 수식은 최종 FIH starting dose가 아니라 equal-AUCu를 맞추기 위한 allometric exposure backbone입니다.
...
>
> $$
> \underbrace{AUC_{u,rat}}_{\text{rat 자유 노출}}=\underbrace{AUC_{u,man}}_{\text{human 자유 노출}}=\frac{\overbrace{Dose_{rat}}^{\text{rat 투여량}}}{\underbrace{CL_{u,rat}}_{\text{rat 자유 청소율}}}=\frac{\overbrace{Dose_{man}}^{\text{human 투여량}}}{\underbrace{CL_{u,man}}_{\text{human 자유 청소율}}}
> $$
>
> - **항별 의미:** 좌변은 rat과 human의 unbound AUC를 같게 둔다는 정의입니다. 우변은 각 종에서 dose를 unbound clearance로 나눈 노출 관계입니다.
> - **실무 직관:** $CL_u$가 커지면 같은 AUC를 얻기 위해 dose도 커져야 합니다. 이 식은 FIH dose 자체가 아니라 exposure-matching backbone을 제공합니다.
> - **오해 방지:** equal AUCu는 동일한 전체 concentration-time shape나 동일 PD를 보장하지 않습니다. Cmax-driven toxicity나 time-above-threshold metric은 별도 판단이 필요합니다.
...
>
> $$
> \underbrace{Dose_{man}}_{\text{human 총 투여량}}=\underbrace{Dose_{rat}}_{\text{rat 총 투여량}}\cdot\left(\frac{\overbrace{BW_{man}}^{\text{human 체중}}}{\overbrace{BW_{rat}}^{\text{rat 체중}}}\right)^{\overbrace{b}^{\text{CL/AUC 지수}}}
> $$
>
> - **항별 의미:** 좌변은 equal-AUCu를 맞추기 위한 human total dose입니다. 우변은 rat dose에 체중비의 $b$승을 곱한 allometric dose backbone입니다.
> - **실무 직관:** $BW_{man}/BW_{rat}$ 또는 $b$가 커지면 human total dose는 증가합니다. 하지만 $b<1$이면 체중비보다 느리게 증가합니다.
> - **오해 방지:** 이 식을 standalone FIH starting-dose rule로 쓰면 안 됩니다. safety margin, toxicology, PD metric, route/formulation을 별도로 얹어야 합니다.
...
>
> $$
> \underbrace{CL_{u,man}}_{\text{human 자유 청소율}}=\underbrace{CL_{u,rat}}_{\text{rat 자유 청소율}}\cdot\left(\frac{\overbrace{BW_{man}}^{\text{human 체중}}}{\overbrace{BW_{rat}}^{\text{rat 체중}}}\right)^{\overbrace{b}^{\text{자유 CL 지수}}}
> $$
>
> - **항별 의미:** 좌변은 human unbound clearance 예측값입니다. 우변은 rat unbound clearance를 체중비의 $b$승으로 확대한 값입니다.
> - **실무 직관:** $CL_{u,rat}$, 체중비, 또는 $b$가 증가하면 $CL_{u,man}$도 증가합니다. 종간 fu 차이가 크면 total CL 대신 이 구조가 더 적절합니다.
> - **오해 방지:** 단백 결합 차이가 큰데 total CL만 스케일링하면 exposure translation이 왜곡될 수 있습니다.
...
>
> $$
> \underbrace{\frac{Dose_{man}/BW_{man}}{Dose_{rat}/BW_{rat}}}_{\text{mg/kg dose 비}}=\left(\frac{\overbrace{BW_{man}}^{\text{큰 종 체중}}}{\overbrace{BW_{rat}}^{\text{작은 종 체중}}}\right)^{\overbrace{b-1}^{\text{kg당 용량 지수}}}
> $$
>
> - **항별 의미:** 좌변은 human과 rat의 mg/kg dose 비입니다. 우변은 total dose scaling 지수 $b$에서 kg당 나눗셈의 1을 뺀 $b-1$입니다.
> - **실무 직관:** $b<1$이면 체중비가 커질수록 큰 종의 mg/kg dose는 작아집니다. 이 식은 total dose와 mg/kg dose의 방향을 분리해 줍니다.
> - **오해 방지:** total dose는 증가할 수 있지만 mg/kg dose는 감소할 수 있습니다. 두 방향을 섞는 것이 흔한 오해입니다.
...
> **핵심 메시지:** AUC equality는 full curve equality나 동일 PD를 보장하지 않으므로, metric·linearity·fu·safety layer를 반드시 분리해야 합니다.
...
> **핵심 메시지:** $BW^{0.75}$는 소아·노인 용량의 시작점일 뿐이며, functional age와 장기 기능이 그 위에 얹혀야 합니다.
...
>
> $$
> \underbrace{BSA}_{\text{체표면적 근사}}=\underbrace{1.73}_{\text{70 kg 기준 BSA}}\cdot\left(\frac{\overbrace{Weight}^{\text{체중}}}{\underbrace{70}_{\text{기준 체중}}}\right)^{\overbrace{0.75}^{\text{체중 기반 BSA 지수}}}
> $$
>
> - **항별 의미:** 좌변은 BSA 근사값입니다. 우변은 70 kg 기준 1.73에 체중비의 0.75승을 곱합니다.
> - **실무 직관:** 체중이 증가하면 BSA도 증가하지만 선형보다 느리게 증가합니다. 이 식은 pediatric/elderly context에서 체중 기반 근사의 한계를 판단하게 합니다.
> - **오해 방지:** BSA라는 이름이 붙었다고 maturation, renal function, obesity 문제가 해결되는 것은 아닙니다.
...
>
> $$
> \underbrace{Child\ maintenance\ dose}_{\text{소아 유지용량}}=\underbrace{1.5}_{\text{성인 대비 보정 계수}}\cdot\left(\frac{\overbrace{Weight_{child}}^{\text{소아 체중}}}{\underbrace{70}_{\text{성인 기준 체중}}}\right)^{\overbrace{0.75}^{\text{체중 allometry}}}\cdot\underbrace{Typical\ adult\ maintenance\ dose}_{\text{성인 유지용량}}
> $$
>
> - **항별 의미:** 좌변은 소아 유지용량입니다. 우변은 성인 유지용량에 체중 allometry와 1.5 보정 계수를 곱한 형태입니다.
> - **실무 직관:** $Weight_{child}$가 커지면 소아 유지용량은 증가하지만, 0.75승 때문에 선형 증가보다 완만합니다. 성인 유지용량 선택이 결과에 직접 연결됩니다.
> - **오해 방지:** 이 식도 기능적 연령, 성숙도, 신·간 기능을 대체하지 않습니다. pediatric dosing의 전체 공식처럼 쓰면 안 됩니다.
...
> **핵심 메시지:** 이 figure pointer는 체중 allometry만으로 pediatric/elderly dosing을 닫을 수 없다는 C5 caveat를 시각적으로 고정합니다.
...
> **핵심 메시지:** C1–C5는 하나의 동물 PK 데이터셋을 human exposure translation으로 통과시키는 5단계 판단 절차로 닫힙니다.
...
> **핵심 메시지:** $b$와 $d$를 혼동하면 CL, V, half-life, dose narrative가 한꺼번에 어긋납니다.
...
>
> $$
> \underbrace{b}_{\text{CL 흐름 지수}}\ne\underbrace{d}_{\text{V 공간 지수}},\qquad \underbrace{t_{1/2}}_{\text{시간}}\propto\underbrace{BW^{d-b}}_{\text{두 지수의 차이}}
> $$
>
> - **항별 의미:** 이 table의 핵심 수식은 CL 지수 $b$와 V 지수 $d$가 서로 다르며, 그 차이가 시간 결과를 만든다는 점입니다.
> - **실무 직관:** $d$를 CL에 넣으면 dose가 과대 산출될 수 있고, $b$를 V에 넣으면 V와 half-life 해석이 흔들립니다.
> - **오해 방지:** $b/d$ 차이는 표의 용어 구분이 아니라 모델링·용량·시간 해석을 동시에 바꾸는 구조적 차이입니다.
...
> **핵심 메시지:** Kallynochron은 $d=1$ 시간축이고, Apolysichron은 $d$를 반영한 시간축이라는 차이를 놓치면 Dedrick 해석이 무너집니다.
...
> $$
>
> - **항별 의미:** 이 table의 좌우 축은 elementary와 complex Dedrick의 시간 정규화를 정의합니다. 첫 항은 $d=1$ 가정, 둘째 항은 추정된 $d$를 반영합니다.
> - **실무 직관:** $d
eq1$이면 $t/BW^{1-b}$가 curve fan-out을 남길 수 있고, $t/BW^{d-b}$가 이를 줄일 수 있습니다.
> - **오해 방지:** Kallynochron과 Apolysichron을 이름만 다르게 외우면 안 됩니다. 차이는 $d$를 시간축에 반영하는지 여부입니다.
...
> **핵심 메시지:** BSA는 allometry의 대안이 아니라 임상적으로 굳어진 $BW^{0.75}$ 근사이며, maturation과 organ function을 대신하지 못합니다.
...
> $$
>
> - **항별 의미:** 이 table은 변수별 allometry와 BSA scaling을 대비합니다. 전자는 변수에 따라 $b$를 선택하거나 추정하고, 후자는 체중 0.75승에 기반한 임상 근사입니다.
> - **실무 직관:** BSA는 체중 증가를 선형보다 완만하게 반영합니다. 하지만 변수별 PK mechanism을 모두 설명하지는 못합니다.
> - **오해 방지:** BSA scaling을 쓰면 pediatric/elderly의 maturation, renal function, body composition 문제가 자동 해결된다고 보면 안 됩니다.
...
> **핵심 메시지:** Self-test의 목적은 공식 암기가 아니라 primary assumption, sensitivity, source discipline을 말로 방어하는 능력입니다.
...
> **핵심 메시지:** $a$와 $b$를 한 문장으로 설명하지 못하면 이후 모든 allometric 판단은 숫자 암기로만 남습니다.
...
> **핵심 메시지:** $b=1$ 오류는 species pair별 fold 차이를 명시해야만 보고서에서 방어 가능한 설명이 됩니다.
...
> **핵심 메시지:** $V/CL$과 $CL/V$의 부호 차이를 손으로 도출해야 half-life와 rate constant를 동시에 지킬 수 있습니다.
...
>
> $$
> \underbrace{t_{1/2}}_{\text{반감기}}=\underbrace{\ln2}_{\text{반감 상수}}\cdot\frac{\overbrace{V}^{\text{분포용적}}}{\overbrace{CL}^{\text{청소율}}}=\ln2\cdot\left(\frac{\underbrace{c}_{\text{V 절편}}}{\underbrace{a}_{\text{CL 절편}}}\right)\cdot\underbrace{BW^{d-b}}_{\text{종간 시간 지수}}
> $$
>
> - **항별 의미:** 좌변은 반감기입니다. 우변은 V/CL 구조를 절편비 $c/a$와 체중 지수 $d-b$로 분해합니다.
> - **실무 직관:** $d-b$가 양수이면 체중이 큰 종에서 반감기가 증가합니다. self-test에서는 이 구조를 직접 도출할 수 있어야 합니다.
> - **오해 방지:** $\ln2$나 $c/a$보다 중요한 판단은 지수 차이 $d-b$입니다. 부호를 놓치면 시간축 해석이 반대로 갑니다.
...
>
> $$
> \underbrace{k_{el}}_{\text{제거 속도상수}}=\frac{\overbrace{CL}^{\text{청소율}}}{\overbrace{V}^{\text{분포용적}}}=\left(\frac{\underbrace{a}_{\text{CL 절편}}}{\underbrace{c}_{\text{V 절편}}}\right)\cdot\underbrace{BW^{b-d}}_{\text{속도상수 지수}}
> $$
>
> - **항별 의미:** 좌변은 제거 속도상수입니다. 우변은 CL/V 구조를 절편비 $a/c$와 체중 지수 $b-d$로 분해합니다.
> - **실무 직관:** $b-d$가 음수이면 체중이 큰 종에서 $k_{el}$은 감소합니다. 이는 half-life 증가와 같은 사실을 반대 방향에서 읽은 것입니다.
> - **오해 방지:** $k_{el}$을 CL의 절대 크기와 동일시하면 안 됩니다. 속도상수는 분포용적까지 나누어진 fraction 제거 속도입니다.
...
> **핵심 메시지:** Elementary와 Complex의 차이는 plot 이름이 아니라 시간축과 농도축에 $d$를 반영하는지의 차이입니다.
...
> **핵심 메시지:** PK28/PK29의 source-internal discrepancy는 숨기는 것이 아니라 citation scope로 드러내야 합니다.
...
> **핵심 메시지:** 같은 mouse CL이라도 $b=0.75$와 $b=1$ 선택은 human CL과 target-AUC dose를 여러 배 차이로 벌립니다.
...
>
> $$
> \underbrace{CL_{human}}_{\text{human CL}}=\underbrace{CL_{mouse}}_{0.012}\cdot\left(\frac{\overbrace{70}^{\text{human kg}}}{\overbrace{0.025}^{\text{mouse kg}}}\right)^{\overbrace{b}^{\text{선택한 지수}}}
> $$
>
> - **항별 의미:** 좌변은 예측할 human CL입니다. 우변은 mouse CL에 human/mouse 체중비의 $b$승을 곱합니다.
> - **실무 직관:** $b=0.75$에서 약 4.6 L/hr, $b=1$에서 약 33.6 L/hr이 되어 지수 선택만으로 약 7.3배 차이가 납니다.
> - **오해 방지:** 계산값만 비교하지 말고 target AUC dose 방향까지 연결해야 합니다. CL을 크게 예측하면 목표 AUC dose도 크게 산출될 수 있습니다.
...
> **핵심 메시지:** 소아·노인에서는 $BW^{0.75}$ 위에 maturation, renal/hepatic function, body composition을 얹어야 합니다.
...
> **핵심 메시지:** 두 종 회귀 점추정값과 physiological prior가 충돌하면 primary와 sensitivity를 분리해 방어해야 합니다.
...
> **핵심 메시지:** 이 세션은 animal-to-human translation, PopPK covariate choice, PBPK parameter scaling에서 같은 지수 판단을 반복 적용하는 시스템입니다.
...
> **핵심 메시지:** Allometry가 약할 때의 실패는 숫자 오류보다 slope drift, V lock-in, functional age omission으로 나타납니다.
...
> **핵심 메시지:** 전문가의 차이는 0.75를 아는 데서 끝나지 않고, 언제 그 prior를 밀어내야 하는지 진단하는 데서 생깁니다.
...
> **핵심 메시지:** 최종 사고 모델은 변수 유형 → 지수 → 시간축 → dose/exposure → 예외 gate → source language 순서로 작동해야 합니다.
