...
<!-- SLIDE_START: S01 | title: 학습자 내비게이션 안내 | source_anchor: PART A — 학습자용 전체 핸드아웃 -->
> **핵심 메시지:** 이 장의 학습 목표는 $K_d$, $EC_{50}$, Hill $n$, hysteresis, AUEC를 하나의 PK/PD 판단 흐름으로 연결하는 것입니다.
...
<!-- SLIDE_START: S02 | title: 세션 개요와 로드맵 | source_anchor: §1 -->
> **핵심 메시지:** PD 모델링은 비선형 농도-효과 관계와 시간 지연을 동시에 해부해야 용량·간격·반응 지속성을 판단할 수 있습니다.
...
<!-- SLIDE_START: S03 | title: 세션 전체 시각 앵커 | source_anchor: §1 FIGURE_POINTER -->
> **핵심 메시지:** Fig.3.1은 C1–C5를 potency/efficacy와 onset/intensity/duration의 두 축으로 묶어 읽게 하는 출발점입니다.
...
<!-- SLIDE_START: S04 | title: 개념 해부 카드 진입 | source_anchor: §2 -->
> **핵심 메시지:** 각 카드는 하나의 PK/PD 판단 실패를 막기 위한 독립 개념 단위로 읽어야 합니다.
...
<!-- SLIDE_START: S05 | title: C1 Big Idea — $K_d$와 $EC_{50}$ 층위 분리 | source_anchor: Card C1 -->
> **핵심 메시지:** $K_d$는 결합 친화도이고 $EC_{50}$는 조직·신호전달까지 포함한 기능적 감수성이므로 같은 숫자로 취급하면 안 됩니다.
...
>
> $$
> \underbrace{E_{max}}_{\text{최대 가능 효과}}
> =
> \underbrace{\alpha}_{\text{고유활성}}
> \underbrace{[R_T]}_{\text{총 수용체량}}
> $$
>
> - **항별 의미:** 좌변은 모든 수용체가 기능적으로 동원될 때의 최대 반응을 정의합니다. 우변의 $\alpha$는 수용체 1단위가 만들어내는 효과 효율이고, $[R_T]$는 사용 가능한 총 수용체 풀입니다.
> - **실무 직관:** $\alpha$가 작으면 총 수용체가 많아도 최대효과가 낮아지고, $[R_T]$가 줄면 같은 효능 약물에서도 천장이 낮아질 수 있습니다.
> - **오해 방지:** $E_{max}$는 단순히 “고농도에서 관찰된 마지막 점”이 아니라 수용체 총량과 신호 효율이 함께 만든 구조적 천장입니다.

### B. 도출(Derivation)


<!-- SLIDE_START: S06 | title: C1 Derivation 1 — 평형 결합 속도식 | source_anchor: Card C1 §B -->
> **핵심 메시지:** 수용체 복합체가 더 이상 변하지 않는 평형 조건이 $E_{max}$ 골격의 출발점입니다.
...
>
> $$
> \underbrace{\frac{d[RC]}{dt}}_{\text{복합체 변화율}}
> =
...
> $$
>
> - **항별 의미:** 좌변은 결합된 수용체 복합체 $[RC]$가 시간에 따라 얼마나 변하는지를 나타냅니다. 우변의 첫 항은 유리 수용체 $[R]$와 농도 $[C]$가 만나 복합체를 만드는 속도이고, 둘째 항은 이미 형성된 복합체가 해리되는 속도입니다.
> - **실무 직관:** $k_1$, $[R]$, $[C]$가 커지면 복합체 생성이 증가하고, $k_{-1}$ 또는 $[RC]$가 커지면 해리 손실이 커집니다. 평형에서는 두 흐름이 같아져 순변화가 0이 됩니다.
> - **오해 방지:** $d[RC]/dt=0$은 반응이 없다는 뜻이 아니라 결합과 해리가 같은 속도로 균형을 이룬다는 뜻입니다.



<!-- SLIDE_START: S07 | title: C1 Derivation 2 — $K_d$ 정의 | source_anchor: Card C1 §B -->
> **핵심 메시지:** $K_d$는 해리와 결합의 상대 속도를 농도 단위의 결합 친화도 지표로 압축합니다.
...
> =
> \underbrace{\frac{k_{-1}}{k_1}}_{\text{해리/결합 속도비}}
> =
> \underbrace{\frac{[C][R]}{[RC]}}_{\text{유리 성분/결합 복합체}}
> $$
>
> - **항별 의미:** 좌변은 결합 친화도를 농도 단위로 요약한 해리상수입니다. 우변은 해리 속도상수와 결합 속도상수의 비이며, 평형에서는 유리 리간드·유리 수용체와 결합 복합체의 농도비로 표현됩니다.
> - **실무 직관:** $k_{-1}$이 커지거나 $k_1$이 작아지면 $K_d$가 커져 친화도가 낮아지고, 같은 농도에서 점유율이 낮아집니다.
> - **오해 방지:** $K_d$는 기능적 효과의 절반 농도가 아니라 결합층에서 정의되는 친화도 지표입니다.



<!-- SLIDE_START: S08 | title: C1 Derivation 3 — 수용체 총량 보존 | source_anchor: Card C1 §B -->
> **핵심 메시지:** 총 수용체 보존을 넣으면 점유율은 0과 1 사이에 갇힌 포화 함수가 됩니다.
...
>
> $$
> \underbrace{\frac{[RC]}{[R_T]}}_{\text{수용체 점유율}}
> =
> \underbrace{\frac{[C]}{[C]+K_d}}_{\text{농도와 친화도의 경쟁}}
> $$
>
> - **항별 의미:** 좌변은 전체 수용체 중 약물이 결합한 비율입니다. 우변의 $[C]$는 점유율을 올리는 입력이고, $K_d$는 같은 농도에서 점유율을 낮추는 친화도 기준입니다.
> - **실무 직관:** $[C]$가 $K_d$보다 커질수록 점유율은 1에 가까워지고, $K_d$가 클수록 같은 농도에서 점유율은 낮아집니다.
> - **오해 방지:** 이 식은 점유율을 말할 뿐, 조직 반응이 반드시 같은 비율로 증가한다는 뜻은 아닙니다.



<!-- SLIDE_START: S09 | title: C1 Derivation 4 — 점유율에서 반응으로 | source_anchor: Card C1 §B -->
> **핵심 메시지:** 점유율 식에 반응 비례성을 얹으면 결합 모델이 곧 $E_{max}$ 반응식으로 번역됩니다.
...
>
> $$
> \underbrace{\frac{E}{E_{max}}}_{\text{최대 대비 반응}}
> =
> \underbrace{\frac{[C]}{[C]+K_d}}_{\text{점유율 근사}}
> ,\qquad
> \underbrace{E}_{\text{예측 반응}}
> =
> \frac{\underbrace{E_{max}}_{\text{반응 천장}}\cdot \underbrace{[C]}_{\text{농도 입력}}}{\underbrace{[C]+K_d}_{\text{농도+친화도 기준}}}
> $$
>
> - **항별 의미:** 좌변 첫 식은 반응을 최대효과 대비 비율로 정의하고, 두 번째 식은 실제 반응 크기를 $E_{max}$와 농도 입력으로 예측합니다.
> - **실무 직관:** $[C]$가 증가하면 $E$는 증가하지만 분모에도 $[C]$가 함께 들어가므로 증가 폭은 점차 줄고 $E_{max}$에 접근합니다.
> - **오해 방지:** 이 식은 수용체 점유율과 반응이 비례한다는 가정 아래의 형태이며, 여분 수용체나 신호 증폭이 크면 기능적 $EC_{50}$와 달라질 수 있습니다.



<!-- SLIDE_START: S10 | title: C1 Derivation 5 — 기능적 $EC_{50}$ 치환 | source_anchor: Card C1 §B -->
> **핵심 메시지:** 기능적 반응식에서 $EC_{50}$는 결합 이후 증폭과 조직 반응까지 포함한 시스템 감수성입니다.
...
>
> $$
> \underbrace{E}_{\text{예측 효과}}
> =
> \frac{\underbrace{E_{max}}_{\text{효과 천장}}\cdot \underbrace{C}_{\text{농도 입력}}}{\underbrace{EC_{50}}_{\text{절반효과 농도}}+\underbrace{C}_{\text{농도 입력}}}
> $$
>
> - **항별 의미:** 좌변은 농도 $C$에서의 기능적 효과입니다. 우변의 $E_{max}$는 최대효과, $EC_{50}$는 효과가 절반에 도달하는 농도, $C$는 현재 작용 농도입니다.
> - **실무 직관:** $C$가 올라가면 효과는 증가하지만, $C\gg EC_{50}$가 되면 추가 증량의 효과 이득은 작아집니다.
> - **오해 방지:** $EC_{50}$는 결합 친화도만이 아니라 조직 반응과 신호전달까지 포함한 기능적 지표입니다.


`[출처: G&W 5e, Ch.3, p.202–204, Eq.3:1–3:10]`


<!-- SLIDE_START: S11 | title: C1 구조적 필연성 | source_anchor: Card C1 §C -->
> **핵심 메시지:** $[C]/([C]+K_d)$ 구조는 리간드 증가와 수용체 총량 보존을 동시에 만족시키기 때문에 효과가 천장을 넘지 않습니다.
...
>
> $$
> \underbrace{EC_{50}}_{\text{기능적 절반 효과 농도}}
> \neq
> \underbrace{K_d}_{\text{결합 친화도 지표}}
> $$
>
> - **항별 의미:** 좌변은 조직 반응이 절반에 도달하는 기능적 농도이고, 우변은 리간드-수용체 결합의 해리상수입니다.
> - **실무 직관:** 신호 증폭이나 여분 수용체가 크면 $EC_{50}$는 $K_d$보다 낮아질 수 있고, 조직 감수성이 낮으면 반대로 멀어질 수 있습니다.
> - **오해 방지:** $EC_{50}\neq K_d$는 오류가 아니라 결합층과 기능 반응층이 서로 다를 수 있음을 알리는 핵심 경계입니다.
...
<!-- SLIDE_START: S12 | title: C1 Schematic — $K_d$와 $EC_{50}$ 분리 | source_anchor: Card C1 FIGURE_SCHEMATIC -->
> **핵심 메시지:** 이 도식의 판단 포인트는 결합 곡선과 기능적 반응 곡선이 서로 다른 층위에 있을 수 있다는 점입니다.
...
<!-- SLIDE_START: S13 | title: C2 Big Idea — Sigmoid $E_{max}$ / Hill | source_anchor: Card C2 -->
> **핵심 메시지:** Hill $n$은 기전 증명이 아니라 좁은 농도 범위에서 효과가 얼마나 급격히 바뀌는지를 정하는 안전 마진 파라미터입니다.
...
<!-- SLIDE_START: S14 | title: C2 Equation — Sigmoid stimulation model | source_anchor: Card C2 §A -->
> **핵심 메시지:** Sigmoid $E_{max}$ 식은 천장, 중간점, 가파름을 분리해 용량 조절 민감도를 읽게 합니다.
...
> \underbrace{E}_{\text{예측 효과}}
> =
> \frac{\underbrace{E_{max}}_{\text{천장}}\cdot \underbrace{C^n}_{\text{가파름이 반영된 농도}}}{\underbrace{EC_{50}^n}_{\text{중간점 기준}}+\underbrace{C^n}_{\text{농도 입력}}}
> $$
>
> - **항별 의미:** 좌변은 농도에서 예측되는 효과입니다. $E_{max}$는 최대효과, $EC_{50}$는 중간점, $n$은 농도 변화가 효과 변화로 번역되는 가파름입니다.
> - **실무 직관:** $n$이 커질수록 $EC_{50}$ 주변의 작은 농도 차이가 큰 효과 차이로 바뀌며, 용량 조절 폭은 더 좁아집니다.
> - **오해 방지:** $n>1$은 자동으로 협동성의 증명이 아니라, 관찰된 곡률을 설명하는 현상학적 파라미터입니다.



<!-- SLIDE_START: S15 | title: C2 Equation — inhibitory $I_{max}$ model | source_anchor: Card C2 §A -->
> **핵심 메시지:** 억제형 $I_{max}$ 모델에서는 효과가 기저치에서 아래로 내려가므로 부호와 기저치 위치를 먼저 확인해야 합니다.
...
>
> $$
> \underbrace{E}_{\text{남은 반응}}
> =
> \underbrace{E_0}_{\text{기저치}}
> -
> \frac{\underbrace{I_{max}}_{\text{최대 억제폭}}\cdot \underbrace{C^n}_{\text{가파름 반영 농도}}}{\underbrace{IC_{50}^n}_{\text{절반 억제 기준}}+\underbrace{C^n}_{\text{농도 입력}}}
> $$
>
> - **항별 의미:** 좌변은 억제 후 관찰되는 반응입니다. $E_0$는 약물 전 기저치이고, 분수항은 농도에 따라 기저치에서 빠지는 억제량입니다.
> - **실무 직관:** $C$ 또는 $I_{max}$가 커지면 $E$는 낮아지고, $IC_{50}$가 커지면 같은 농도에서 억제는 약해집니다. $n$이 커지면 억제 전환이 더 급격해집니다.
> - **오해 방지:** 자극 모델처럼 위로 올라가는 식이 아니라 기저치에서 억제량을 빼는 구조이므로 부호를 바꾸면 임상 해석이 뒤집힙니다.
...
<!-- SLIDE_START: S16 | title: C2 Parameter recovery — slope and Hill $n$ | source_anchor: Card C2 §B -->
> **핵심 메시지:** 그래프 기울기는 Hill $n$과 $E_{max}$가 함께 만든 중간부 가파름을 역산하는 단서입니다.
...
>
> $$
> \underbrace{m}_{\text{중간부 기울기}}
> =
> \frac{\underbrace{n}_{\text{Hill 가파름}}\cdot \underbrace{E_{max}}_{\text{천장}}}{\underbrace{4}_{\text{중간점 스케일}}}
> ,\qquad
> \underbrace{n}_{\text{복원된 가파름}}
> =
> \frac{4\underbrace{m}_{\text{관찰 기울기}}}{\underbrace{E_{max}}_{\text{최대효과}}}
> $$
>
> - **항별 의미:** 첫 식은 sigmoid 곡선 중간부 기울기 $m$이 Hill $n$과 $E_{max}$에 의해 결정됨을 나타내고, 둘째 식은 관찰 기울기에서 $n$을 역산합니다.
> - **실무 직관:** 같은 $E_{max}$에서 $m$이 크면 $n$이 크고, 같은 $m$이라도 $E_{max}$가 크면 상대적 가파름 $n$은 작아집니다.
> - **오해 방지:** 이 관계는 초기 추정과 직관을 돕는 도구이지, 독립적 기전 증거 없이 $n$의 생물학적 원인을 증명하지는 않습니다.



<!-- SLIDE_START: S17 | title: C2 Parameter recovery — intercept | source_anchor: Card C2 §B -->
> **핵심 메시지:** $C_0$ 복원식은 곡선 중간부의 위치와 가파름을 초기 추정치로 연결하는 실무 도구입니다.
...
>
> $$
> \underbrace{C_0}_{\text{절편 농도}}
> =
> \underbrace{EC_{50}}_{\text{중간점 농도}}
> \cdot
> \underbrace{e^{-2/n}}_{\text{Hill 가파름 보정}}
> $$
>
> - **항별 의미:** 좌변은 그래프 절편에서 복원되는 농도 기준입니다. 우변은 $EC_{50}$에 Hill 가파름에 따른 지수 보정을 곱한 형태입니다.
> - **실무 직관:** $n$이 커지면 $e^{-2/n}$이 1에 가까워져 $C_0$가 $EC_{50}$에 가까워지고, $n$이 작으면 더 왼쪽으로 이동합니다.
> - **오해 방지:** $C_0$는 새 potency 파라미터가 아니라 그래프 기반 초기 추정과 곡선 위치를 연결하는 보조량입니다.


`[출처: G&W p.219, Eq.3:38–3:42]`


<!-- SLIDE_START: S18 | title: C2 Design anchor — PD3 식별 가능성 | source_anchor: Card C2 §B Insertion I2 -->
> **핵심 메시지:** Sigmoid 모델의 필요성은 식 자체가 아니라 $EC_{50}$ 주변과 고원부를 실제로 건드린 농도 범위에서 판별됩니다.
...
<!-- SLIDE_START: S19 | title: C2 구조적 필연성 | source_anchor: Card C2 §C -->
> **핵심 메시지:** $C^n$이 분자와 분모에 함께 들어가기 때문에 가파름을 바꾸면서도 0–$E_{max}$ 경계가 보존됩니다.
...
<!-- SLIDE_START: S20 | title: C2 Equation — unbound concentration transformation | source_anchor: Card C2 §C -->
> **핵심 메시지:** 단백결합 차이가 클 때는 총 농도 $EC_{50}$가 아니라 비결합 $EC_{u50}$로 potency를 비교해야 합니다.
...
> \underbrace{E}_{\text{예측 효과}}
> =
> \frac{\underbrace{E_{max}}_{\text{천장}}\underbrace{(C_u/f_u)^n}_{\text{총농도 환산 입력}}}{\underbrace{EC_{50}^n}_{\text{총농도 기준 중간점}}+\underbrace{(C_u/f_u)^n}_{\text{환산 농도}}}
> =
> \frac{\underbrace{E_{max}}_{\text{천장}}\cdot \underbrace{C_u^n}_{\text{비결합 농도 입력}}}{\underbrace{EC_{u50}^n}_{\text{비결합 중간점}}+\underbrace{C_u^n}_{\text{비결합 농도 입력}}}
> $$
>
> - **항별 의미:** 좌변은 효과이고, 첫 우변은 비결합 농도 $C_u$를 분율 $f_u$로 나누어 총농도 기준으로 표현한 형태이며, 둘째 우변은 같은 관계를 비결합 $EC_{u50}$ 기준으로 다시 쓴 형태입니다.
> - **실무 직관:** $f_u$가 변하면 총농도 기준 $EC_{50}$ 해석이 흔들릴 수 있지만, 비결합 농도 기준으로 보면 실제 표적에 도달한 농도 비교가 더 직접적입니다.
> - **오해 방지:** 총 $EC_{50}$ 차이를 곧바로 수용체 감수성 차이라고 읽으면 단백결합 차이를 기전 차이로 오인할 수 있습니다.
...
<!-- SLIDE_START: S21 | title: C2 Plausible Fallacy — 곡률 억제 | source_anchor: Card C2 §C-2 -->
> **핵심 메시지:** 일반 $E_{max}$도 대충 맞는다는 말은 곡률이 없다는 뜻이 아니라 농도 범위가 곡률을 볼 만큼 넓었는지를 먼저 묻는 신호입니다.
...
<!-- SLIDE_START: S22 | title: C2 Boundary conditions and limits | source_anchor: Card C2 §D -->
> **핵심 메시지:** Hill $n$ 해석은 농도 범위, 고원부 관찰, 기저치 안정성을 통과한 뒤에만 실무적으로 방어할 수 있습니다.
...
<!-- SLIDE_START: S23 | title: C2 Figure pointer — PD3 model discrimination | source_anchor: Card C2 FIGURE_POINTER -->
> **핵심 메시지:** PD3 원자료는 sigmoid 모델 우월성이 농도 범위와 파라미터 정밀도에 어떻게 의존하는지를 확인하는 핵심 증거입니다.
...
<!-- SLIDE_START: S24 | title: C3 Big Idea — hysteresis as diagnostic | source_anchor: Card C3 -->
> **핵심 메시지:** 히스테리시스는 잡음이 아니라 혈장이 효과 부위를 즉시 대표하지 못한다는 구조적 경고 신호입니다.
...
<!-- SLIDE_START: S25 | title: C3 Diagnostic logic | source_anchor: Card C3 §B -->
> **핵심 메시지:** 시간-농도 도표에서 애매한 지연도 농도-반응 고리로 그리면 구조 선택을 요구하는 신호가 됩니다.
...
<!-- SLIDE_START: S26 | title: C3 Delay-source interpretation | source_anchor: Card C3 §C -->
> **핵심 메시지:** 반시계 방향 고리는 지연을 말하지만, 그 원인이 분포인지 전환인지까지 자동으로 말해주지는 않습니다.
...
<!-- SLIDE_START: S27 | title: C3 Figure pointer — hysteresis loop | source_anchor: Card C3 FIGURE_POINTER -->
> **핵심 메시지:** Fig.8-1/8-2는 같은 농도에서도 시간 방향에 따라 반응이 달라지는 경로 의존성을 눈으로 확인하게 합니다.
...
<!-- SLIDE_START: S28 | title: C4 Big Idea — delay-source topology | source_anchor: Card C4 -->
> **핵심 메시지:** C4의 핵심 판단은 지연이 작용 부위 평형화인지 생물학적 시스템 전환인지 구분하는 것입니다.
...
> \rightarrow
> \underbrace{C_e(t)}_{\text{효과 부위 농도}}
> \rightarrow
> \underbrace{E(t)}_{\text{관찰 반응}}
> $$
>
> - **항별 의미:** 좌변의 시간함수 $C(t)$는 혈장 입력이고, 가운데 $C_e(t)$는 지연을 흡수한 효과 부위 농도이며, 마지막 $E(t)$는 실제 관찰 반응입니다.
> - **실무 직관:** $C_e(t)$가 천천히 따라오면 같은 혈장 농도에서도 상승기와 하강기의 효과가 달라져 hysteresis loop가 생깁니다.
> - **오해 방지:** 이 화살표는 새 생리 구획을 증명하는 식이 아니라, 혈장 직접 연결이 실패할 때 지연을 구조화하는 모델링 언어입니다.


<!-- SLIDE_START: S29 | title: C4a Effect compartment | source_anchor: Card C4 §A -->
> **핵심 메시지:** 효과 구획은 실제 장기 구획이 아니라 혈장과 효과 부위 사이의 총괄 평형화 지연 표현입니다.
...
<!-- SLIDE_START: S30 | title: C4b Indirect response and turnover | source_anchor: Card C4 §B -->
> **핵심 메시지:** 간접 반응 모델은 약물이 반응 자체가 아니라 생성·소멸 속도를 바꿀 때 필요한 시간 지연 구조입니다.
...
> =
> \underbrace{k_{in}}_{\text{생성/유입}}
> -
> \underbrace{k_{out}R}_{\text{소실/유출}}
> $$
>
> - **항별 의미:** 좌변은 반응 변수 $R$이 시간에 따라 변하는 속도입니다. $k_{in}$은 반응을 만들어내는 유입·생성 속도이고, $k_{out}R$은 현재 반응량에 비례해 사라지는 유출·소실 속도입니다.
> - **실무 직관:** $k_{in}$이 커지면 $R$은 올라가고, $k_{out}$ 또는 $R$이 커지면 소실이 커져 $R$은 내려갑니다. 약물이 어느 항을 자극하거나 억제하는지가 간접 반응 모델의 핵심입니다.
> - **오해 방지:** 이 식은 혈장 농도와 효과를 직접 연결하는 식이 아니라, 반응 시스템 자체의 turnover를 표현하는 골격입니다.
...
<!-- SLIDE_START: S31 | title: C4 Structural necessity — peak-time diagnostic | source_anchor: Card C4 §C -->
> **핵심 메시지:** 최대 효과 시점의 용량 의존성은 효과 구획과 간접 반응을 가르는 실무적 진단 단서입니다.
...
>
> $$
> \underbrace{\frac{dR}{dt}}_{\text{반응 변화율}}=\underbrace{0}_{\text{최대/평형 조건}}
> \quad\Longleftrightarrow\quad
> \underbrace{\text{input}}_{\text{유입}}=
> \underbrace{\text{output}}_{\text{유출}}
> $$
>
> - **항별 의미:** 좌변은 반응이 더 이상 증가하지 않는 시점의 조건을 정의하고, 우변은 생성과 소실이 균형을 이룬다는 동역학적 해석입니다.
> - **실무 직관:** 용량이 커져도 시스템 전환 속도 자체가 바뀌지 않으면 최대 효과 시점은 크게 움직이지 않을 수 있습니다.
> - **오해 방지:** 최대 효과 시점이 일정하다는 관찰만으로 간접 반응이 증명되지는 않지만, 효과 구획보다 시스템 전환 가설을 우선 검토하게 만듭니다.
...
<!-- SLIDE_START: S32 | title: C4 Schematic — delay-source topology | source_anchor: Card C4 FIGURE_SCHEMATIC -->
> **핵심 메시지:** 이 도식은 같은 지연 패턴이라도 외삽 거동이 전혀 다른 두 인과 구조를 비교하게 합니다.
...
<!-- SLIDE_START: S33 | title: C5 Big Idea — AUEC and duration | source_anchor: Card C5 -->
> **핵심 메시지:** 효과 지속 시간은 혈장 반감기가 아니라 농도 입력과 반응 시스템 회복 속도가 함께 정하는 반응-시간 지표입니다.
...
<!-- SLIDE_START: S34 | title: C5 Equation — AUEC definition | source_anchor: Card C5 §A -->
> **핵심 메시지:** AUEC는 약동학적 노출이 아니라 실제 반응-시간 곡선을 적분한 효과 부담량입니다.
...
>
> $$
> \underbrace{AUEC}_{\text{총 반응 부담량}}
> =
> \int_{\underbrace{0}_{\text{시작}}}^{\underbrace{\infty}_{\text{충분한 추적}}}
> \underbrace{E(t)}_{\text{시간별 반응}}\,dt
> $$
>
> - **항별 의미:** 좌변은 반응-시간 곡선 아래 면적입니다. 우변은 시간 0부터 반응이 사라질 만큼 충분히 긴 시간까지 순간 반응 $E(t)$를 누적합니다.
> - **실무 직관:** 반응 크기가 크거나 오래 지속되면 AUEC가 커지고, 같은 peak라도 빨리 사라지는 반응은 AUEC가 작을 수 있습니다.
> - **오해 방지:** AUEC는 PK AUC가 아니라 효과 곡선을 적분한 값이므로 농도 노출과 동일하게 해석하면 안 됩니다.
...
<!-- SLIDE_START: S35 | title: C5 Equation — AUEC closed form | source_anchor: Card C5 §A -->
> **핵심 메시지:** AUEC 폐쇄형 식은 일반 $E_{max}$와 1차 제거라는 조건이 맞을 때만 사용할 수 있는 제한된 요약식입니다.
...
>
> $$
> \underbrace{AUEC}_{\text{총 반응 면적}}
> =
> \underbrace{\frac{E_{max}}{K}}_{\text{천장/제거 시간척도}}
> \ln\left(
> 1+
> \underbrace{\frac{D}{EC_{50}\cdot V}}_{\text{용량 대비 감수성·분포 스케일}}
> \right)
> $$
>
> - **항별 의미:** 좌변은 반응 면적입니다. $E_{max}/K$는 최대효과와 제거 속도에 따른 시간 스케일이고, 로그 안의 $D/(EC_{50}V)$는 투여량이 감수성 기준 농도와 분포용적에 비해 얼마나 큰지를 나타냅니다.
> - **실무 직관:** $D$와 $E_{max}$가 커지면 AUEC는 증가하고, $K$, $EC_{50}$, $V$가 커지면 같은 조건에서 AUEC는 감소하거나 증가 폭이 둔화됩니다.
> - **오해 방지:** 이 폐쇄형 식은 일반 $E_{max}$와 1차 제거 조건의 결과이므로 sigmoid 모델이나 복잡한 지연 구조에 그대로 외삽하면 안 됩니다.
...
<!-- SLIDE_START: S36 | title: C5 Duration and log-dose relation | source_anchor: Card C5 §B -->
> **핵심 메시지:** 지속 시간 식은 용량 증가가 최대 효과보다 역치 통과 시간을 더 선명하게 바꿀 수 있음을 보여줍니다.
...
>
> $$
> \underbrace{t_D}_{\text{효과 지속 시간}}
> =
> \underbrace{\frac{1}{K}}_{\text{감소 시간척도}}
> \ln\left(
> \underbrace{\frac{D}{A_{min}}}_{\text{용량/최소 유효량}}
> \right)
> $$
>
> - **항별 의미:** 좌변은 효과가 역치 이상으로 유지되는 시간입니다. $1/K$는 감소 속도의 시간 척도이고, $D/A_{min}$은 투여량이 최소 유효량보다 얼마나 큰지를 나타냅니다.
> - **실무 직관:** $D$가 증가하면 지속 시간은 로그 형태로 늘고, $K$가 커지면 더 빨리 감소하므로 지속 시간은 짧아집니다.
> - **오해 방지:** 용량을 두 배로 늘려도 지속 시간이 두 배가 되는 것이 아니라, 조건이 맞을 때 로그-용량 관계에 따라 제한적으로 늘어납니다.
...
<!-- SLIDE_START: S37 | title: C5 PK-rate-limited vs PD-rate-limited | source_anchor: Card C5 §C -->
> **핵심 메시지:** PK/PD 율속은 약물 이름이 아니라 혈장 시간 척도와 시스템 회복 시간 척도의 비율로 판단해야 합니다.
...
>
> $$
> \underbrace{t_{1/2}}_{\text{혈장 반감기}}
> \quad\text{vs}\quad
> \underbrace{t_{recovery}}_{\text{반응 회복 시간}}
> $$
>
> - **항별 의미:** 좌변은 약물이 혈장에서 줄어드는 시간 척도이고, 우변은 생리 반응이 기저치로 돌아오는 시간 척도입니다.
> - **실무 직관:** 두 시간이 비슷하면 PK 율속 가능성이 커지고, 반응 회복이 훨씬 느리면 PD/시스템 회복이 투여 간격 판단을 지배합니다.
> - **오해 방지:** 같은 계열 약물이라도 $t_{1/2}$와 $t_{recovery}$의 비율이 다르면 지속 시간 판단은 달라질 수 있습니다.
...
>
> $$
> \underbrace{\frac{\tau_{drug}}{\tau_{system}}}_{\text{약물 시간척도 / 시스템 시간척도}}
> $$
>
> - **항별 의미:** 좌변은 혈장 약물 동역학 시간 척도를 생리 시스템 회복 시간 척도와 비교하는 무차원 판단 비율입니다.
> - **실무 직관:** 이 비율이 1에 가까우면 혈장 PK가 효과 지속성을 더 잘 설명하고, 크게 작거나 크면 시스템 회복 지연을 별도로 모델링해야 합니다.
> - **오해 방지:** $\tau_{drug}/\tau_{system}$는 새 문헌 수치가 아니라 투여 간격 판단을 정리하기 위한 시간척도 비교 프레임입니다.


<!-- SLIDE_START: S38 | title: C5 Figure pointer — PD-rate-limited persistence | source_anchor: Card C5 FIGURE_POINTER -->
> **핵심 메시지:** Aspirin과 omeprazole 그림은 혈장 반감기와 효과 지속 시간이 서로 다른 시간축에 놓일 수 있음을 확인하게 합니다.
...
<!-- SLIDE_START: S39 | title: C5 Figure pointer — duration threshold crossing | source_anchor: Card C5 FIGURE_POINTER -->
> **핵심 메시지:** 지속 시간 그림은 용량 증가가 역치 통과 시간을 어떻게 늘리는지 수식보다 직관적으로 고정합니다.
...
<!-- SLIDE_START: S40 | title: Confusion pairs overview | source_anchor: §5 -->
> **핵심 메시지:** 혼동 쌍은 같은 기호처럼 보이는 개념을 서로 다른 실무 판단 층위로 분리하기 위한 안전장치입니다.

# §5 — 혼동 쌍 해부


<!-- SLIDE_START: S41 | title: Pair 1 — $K_d$ vs $EC_{50}$ | source_anchor: §5 Pair 1 -->
> **핵심 메시지:** $K_d$와 $EC_{50}$의 차이를 놓치면 결합 친화도 변화와 시스템 감수성 변화를 잘못 연결하게 됩니다.
...
<!-- SLIDE_START: S42 | title: Pair 2 — ordinary vs sigmoid $E_{max}$ | source_anchor: §5 Pair 2 -->
> **핵심 메시지:** 일반 $E_{max}$와 sigmoid $E_{max}$의 차이는 모양 취향이 아니라 용량 조절 안전 마진의 차이입니다.
...
<!-- SLIDE_START: S43 | title: Pair 3 — PK-rate-limited vs PD-rate-limited | source_anchor: §5 Pair 3 -->
> **핵심 메시지:** 효과 감소가 혈장 농도 감소를 따르는지 시스템 회복을 따르는지에 따라 투여 간격 판단이 달라집니다.
...
<!-- SLIDE_START: S44 | title: Pair 4 — direct link, effect compartment, indirect response | source_anchor: §5 Pair 4 -->
> **핵심 메시지:** 혈장 직접 연결, 효과 구획, 간접 반응은 적합 순서가 아니라 서로 다른 인과 가정을 가진 모델 후보입니다.
...
<!-- SLIDE_START: S45 | title: Self-test cluster — active recall | source_anchor: §7 -->
> **핵심 메시지:** 자기 테스트의 목적은 계산 정답보다 어떤 가정을 먼저 버리고 어떤 구조를 선택할지 설명하는 능력입니다.
...
<!-- SLIDE_START: S46 | title: Self-test cluster — delay and duration decisions | source_anchor: §7 Q5–Q9 -->
> **핵심 메시지:** Q5–Q9는 히스테리시스, 간접 반응, AUEC, PD 율속을 실제 모델 선택 순서로 연결하는 점검입니다.
...
<!-- SLIDE_START: S47 | title: Meta frame and synthesis | source_anchor: §8 -->
> **핵심 메시지:** 이 장의 큰 그림은 농도-효과 곡선의 모양과 반응-시간 지연을 분리해 읽는 것입니다.
...
<!-- SLIDE_START: S48 | title: Boundary flags and source discipline | source_anchor: §8 C -->
> **핵심 메시지:** 경계 플래그는 출처 밖 추론과 실무 확장을 구분해 HTML 컴파일 후에도 source discipline을 유지하게 합니다.
