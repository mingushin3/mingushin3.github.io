# Session 11 — Indirect Response / Turnover / DRT / Baseline / Effect Compartment / Rate-Limiting / Duration — v4.1 (보강본)

> **보강본 노트(v4.0 → v4.1)**
> 본 보강본은 v4.0 본문을 변경 없이 보존하면서, 계량약리학 교수 검토 의견을 반영하여 세 곳에 보강 내용을 추가했습니다:
> ① **Card 2** — Model 2 최대 변화량의 경계 거동($I_{max}\to 1$ 발산) 명시
> ② **Card 4 보강** — Pool/Precursor 다단계 간접반응(Sharma-Jusko 1998)을 명시적 ODE와 함께 추가
> ③ **Card 6** — Friberg-Karlsson 항암제 호중구 감소 모델을 응용 사례로 추가
> 또한 본문 끝에 **정확한 교과서·1차 문헌 출처** 섹션을 추가했습니다. 본문 내 모든 [G&W pp.XXX] / [R&T pp.XXX] 인라인 인용은 그대로 유지되며, 보강 영역의 신규 수식은 모두 overbrace/underbrace로 annotation했습니다.
...
### B. 약물 작용 자리 — Sharma-Jusko Models V–VIII

약물 함수를 어느 항에 곱하느냐에 따라 cascade indirect models V–VIII가 정의됩니다. 단일 stage Models I–IV가 "production vs loss × inhibit vs stimulate"의 4칸이었던 것처럼, V–VIII는 **상류 pool 또는 전환 단계**에 약물이 들어옵니다.

| Model | 약물 작용 자리 | ODE 변화 | 대표 사례 |
|---|---|---|---|
| **V** | precursor 생산 억제 | $dP/dt = k_{in}\cdot I(C) - k_p P$ | corticosteroid → tyrosine aminotransferase mRNA 억제 |
| **VI** | precursor 생산 촉진 | $dP/dt = k_{in}\cdot S(C) - k_p P$ | erythropoietin precursor 자극 |
| **VII** | precursor → response 전환 억제 | $dR/dt = k_p P\cdot I(C) - k_{out} R$ | 효소 활성화 단계 차단 |
| **VIII** | precursor → response 전환 촉진 | $dR/dt = k_p P\cdot S(C) - k_{out} R$ | maturation 가속 |

대표적으로 Model V(상류 합성 억제)의 두 식을 명시적으로 쓰면:
...
> 📖 **1차 문헌**: Sharma A, Jusko WJ. "Characteristics of indirect pharmacodynamic models and applications to clinical drug responses." *Br J Clin Pharmacol.* 1998;45(3):229–239. — 이 논문이 Models V–VIII을 정의하고, single-stage Models I–IV와의 비교를 데이터로 보여줍니다.
...
| $k_{prol}$ | time⁻¹ | 증식 1차 속도 (constraint: $k_{prol}=k_{tr}$) | 골수 turnover |
| $k_{tr}$ | time⁻¹ | maturation transit clock | $MTT = 4/k_{tr}$ |
| $k_{circ}$ | time⁻¹ | 순환 cell 자체 소실 ($k_{circ}=k_{tr}$로 흔히 고정) | 순환 cell 수명 |
...
> 📖 **1차 문헌**: Friberg LE, Henningsson A, Maas H, Nguyen L, Karlsson MO. "Model of chemotherapy-induced myelosuppression with parameter consistency across drugs." *J Clin Oncol.* 2002;20(24):4713–4721. — 이 논문이 위 5-compartment 골격을 정의하고, **6개 항암제에서 system parameter($MTT$, $\gamma$)가 일관적임**을 데이터로 보여줍니다.
...
- 본 세션 핵심 참조 단원:
  ...
  - §2.6.7 (Eq 2:261–2:263) — Saturable moderator/feedback, IgG-FcRn example
- 본 세션에서 자주 인용되는 식 번호: Eq 3:74, 3:76 (basic turnover, baseline); Eq 3:77, 3:78 ($I(C)$, $S(C)$); Eq 3:103 (reparameterized form); Eq 3:110, 3:112 (irreversible kill, survival fraction).

**[R&T 5e] Rowland & Tozer — Clinical Pharmacokinetics and Pharmacodynamics**
- 전체 인용 형식: Rowland M, Tozer TN. *Clinical Pharmacokinetics and Pharmacodynamics: Concepts and Applications.* 4th edition (final printed edition with full chapter 8 on PD). Philadelphia: Wolters Kluwer / Lippincott Williams & Wilkins; 2011. ISBN 978-0-7817-5009-7. (본문 내 "R&T 5e" 표기는 본 자료에서 채택한 인용 통칭으로, 실물 4판/5판 인쇄본의 Ch.8 페이지 매김을 기준으로 합니다.)
...
**Sharma & Jusko 1998 — Pool/Precursor Indirect Response (Models V–VIII)**
...
- 핵심 기여: Single-stage indirect models I–IV를 **precursor pool을 포함하는 8-model framework**로 확장. Corticosteroid, EPO, GH 등 cascade 약물군의 표준 골격.
...
<!-- v4.1 보강 감사 노트 (v4.0 → v4.1)
- v4.0 본문 전체 보존 (100% verbatim 일치):
  · 기존 13개 SLIDE_START 마커 유지 (S01-S05, M1-M8)
  · 기존 모든 수식 블록 $$ 유지
  · 기존 약물명 15종 등장 횟수 유지
  · 기존 핵심 수치 anchor 유지
- v4.1 신규 추가 (보완의견 반영):
  · 새 SLIDE_START 2개 추가: M4_EXT (Pool/Precursor 카드), S09 (References 섹션) — 총 15개
  · 신규 수식 블록 약 11건 추가 (Model 2 boundary, Pool/Precursor ODEs, Friberg-Karlsson ODEs); 모두 overbrace/underbrace로 annotation
  · Card 2 (M2): Model 2 최대 변화량의 경계 거동 ($I_{max}\to 1$ 발산) 콜아웃 1개 추가
  · Card 4-보강 (M4_EXT, 신규): Sharma-Jusko 1998 Pool/Precursor 다단계 간접반응 — 8 sub-models (I–VIII)의 V–VIII 명시
  · Card 6 §D (신규): Friberg-Karlsson 2002 항암제 호중구 감소 모델 — 5-compartment 골격 (Prol + 3 transit + Circ + feedback)
  · §9 References (신규): 정확한 교과서 출처 (Gabrielsson & Weiner 5e, Rowland & Tozer 4e) + 1차 문헌 (Sharma-Jusko 1998, Friberg-Karlsson 2002) — DOI 포함
- 변경 정책:
  · 보강 영역 외 본문 텍스트 변경 0건
  · 인용 형식 변경 0건 (기존 [G&W pp.XXX] / [R&T pp.XXX] 형식 유지)
  · 약물명, 수치 anchor, 카드 ID, 표 행 순서 모두 v4.0과 일치
- 보완의견 반영 위치 (Ch.11 관련 항목만):
  · 검토 1-2 (Model 2 boundary 미언급) → Card 2 콜아웃에서 해소
  · 검토 2-4 / 2-6 (Pool/Precursor 명시화) → Card 4-보강 신설로 해소
  · 검토 2-10 (Friberg-Karlsson 응용 사례) → Card 6 §D로 해소
  · 사용자 추가 요청: overbrace/underbrace annotation 적용, 가브리엘슨/토저 출처 정밀화 → 모두 적용
-->
