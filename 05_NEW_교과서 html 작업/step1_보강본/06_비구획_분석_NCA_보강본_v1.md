# 비구획 분석(NCA) — AUC, AUMC, MRT, $V_{ss}$와 노출 평가의 실무 경계 — v4.0 **보강본**
...
**출처**: Gabrielsson & Weiner 5e Ch.2 §2.8–§2.9, PK41; Rowland & Tozer 5e Appendix A·H, Ch.11. [G&W pp.141–164, G&W pp.661–664; R&T pp.759–762, R&T pp.789–793, R&T pp.319–340]
...
### E. BLQ(Below Limit of Quantification) 처리 규칙 — 위치별 분기

G&W p.153 본문은 일반 원칙을 분명히 못 박아요 — **"BLQ는 결측(missing) 처리. 0이나 LOQ 대입은 금지, 특히 지수모델에서는 더 위험."** 그런데 실제 NCA 실무에서는 BLQ의 **위치**에 따라 처리 방식이 갈립니다. 규제 가이던스(FDA Bioanalytical Method Validation 2018; EMA Bioanalytical Method Validation Guideline 2011) 기준 위치별 규칙은 다음과 같아요.

| BLQ 위치 | 표준 처리 | 합리화 | AUC/$\lambda_z$ 영향 |
|---|---|---|---|
| **Pre-first quantifiable BLQ**<br>(첫 정량 시점 이전) | **0 처리** | 흡수 전 시점은 농도 0이 생리학적으로 합리적 가정 | 면적 누락 없음; $AUC_{0-t_{first}}$ 영향 최소 |
| **Embedded / Intervening BLQ**<br>(두 정량 사이) | **missing** 또는 **linear interpolation** | 0 처리 시 AUC 과소추정, LOQ 대입 시 과대추정 | 선형 보간 권장; missing 처리 시 사다리꼴 폭만 조정 |
| **Terminal BLQ**<br>(말단 정량 이후) | **missing** — $\lambda_z$ 계산에서 제외 | 0 처리 시 $\lambda_z$가 인위적으로 가팔라짐(편향↑); LOQ 대입 시 꼬리 과대 | $\lambda_z$·$AUC_{tail}$·$V_z$·$V_{ss}$ **동시** 편향 위험 |

$$
\overbrace{C^{\text{handled}}_{i}}^{\text{처리된 농도}}
=\begin{cases}
\underbrace{0}_{\text{흡수 전 가정}} & t_i<t_{\text{first quantifiable}}\\[4pt]
\underbrace{\text{interp}(C_{i-1},C_{i+1})}_{\text{선형 보간}}\ \text{or}\ \underbrace{\text{missing}}_{\text{결측}} & t_{\text{first}}<t_i<t_{\text{last quantifiable}}\\[4pt]
\underbrace{\text{missing}}_{\text{결측 — }\lambda_z\text{ 제외}} & t_i>t_{\text{last quantifiable}}
\end{cases}
$$

> 💼 **실무 인사이트:** Phase 1 SAD/MAD 분석에서 가장 흔한 함정은 **terminal BLQ에 0을 대입하는 자동 NCA 출력**입니다. 이렇게 두면 $\lambda_z$ 회귀선이 인위적으로 가팔라져서 $t_{1/2}$가 짧게 보이고, **$AUC_{tail}$이 과소추정되며, $V_z$와 $V_{ss}$가 동시에 작아져요.** Adjusted $R^2$가 높게 보여도, $\lambda_z$ window에 BLQ=0 점이 끼었는지를 직접 확인해야 합니다. [G&W p.153]

🔑 **BLQ 보고 규칙:** NCA 보고서에는 처리 방식(missing / interpolation / 0)을 위치별로 명시합니다. BA/BE 표준 절차에서 audit trail의 일부예요 — 처리 방식 한 줄이 빠지면 BE 통계의 재현성 자체가 흔들립니다.

### F. 부분 AUC($AUC_{0-t^*}$) — Modified Release와 노출 형상

총 $AUC_{0-\infty}$만 보면 같은 노출이라도 **방출 패턴(release profile) 차이**가 묻혀버릴 수 있어요. 그래서 EMA Guideline on the Investigation of Bioequivalence(2010)는 modified release(MR) 제형의 immediate release(IR) 대비 BE 평가에서 **부분 AUC** — 예를 들어 $AUC_{0-1.5h}$, $AUC_{1.5-12h}$ — 를 노출 형상의 metric으로 명시합니다.

$$
\underbrace{AUC_{0-t^*}}_{\text{부분 AUC}}
=\sum_{i:\,t_{i+1}\leq t^*}\underbrace{\frac{C_i+C_{i+1}}{2}\Delta t}_{\text{사다리꼴 항}}
$$

| 비교 상황 | $AUC_{0-\infty}$ 비교만 | 부분 AUC 비교 |
|---|---|---|
| IR vs MR 같은 일일 용량 | 같을 수 있음 | 다른 release pattern을 노출 형상으로 직접 검출 |
| ER 자극제(Methylphenidate 등) | 총 노출 BE 통과해도 | $AUC_{0-3h}$, $AUC_{3-7h}$로 onset/duration 차이 검출 |

💡 **부분 AUC는 "총 노출은 같지만 패턴이 다르다"를 정량으로 잡는 도구입니다.** PD가 onset speed에 민감한 경우(stimulant, opioid, hypnotic), 총 AUC BE 통과만으로는 임상 동등성 보장이 안 돼요.

---

이 카드 핵심내용을 한 줄로 정리하면 — **AUC는 사다리꼴 합이 아니라 "사다리꼴 합 + $\lambda_z$ 기반 꼬리 + BLQ 처리 규칙"입니다.** 그래서 AUC 검토는 값 자체가 아니라 **말단 기울기 선택, $\hat C_{last}$ 사용 여부, 외삽 면적 비율, BLQ 위치별 처리** 네 가지를 봅니다. [G&W pp.143–148, 153]
...
| $V_{ss}$ | 부피 | 정상상태 분포 부피 | AUC, AUMC, CL, MIT |
| $V_z$ | 부피 | 말단 기울기 기준 분포 부피 | AUC, CL, $\lambda_z$ |
| $t_{1/2,eff}$ | 시간 | 평균 체류시간 기반 유효 반감기 | MRT |
...
> 💡 **방향 주의 (Direction Clarification):** "반감기가 입력 시간에 비해 짧다"는 표현은 곧 **"입력 시간이 반감기에 비해 길다 ($T_{inf}/t_{1/2}\gg 1$)"**는 뜻이에요. 즉 표준식이 무너지는 방향은 **짧은 infusion이 아니라 긴 infusion** 쪽입니다 — 주입이 elimination을 지배할 만큼 길어지면서 표준 MRT 식이 0이나 음수 $V_{ss}$를 만들어요. 학생들이 가장 자주 헷갈리는 자리니까 방향을 한 번 더 명시합니다.
>
> $$\underbrace{\frac{T_{inf}}{t_{1/2}}\gg 1}_{\text{긴 infusion}}\ \Rightarrow\ \underbrace{\text{Eq.2:337 표준식 불안정}}_{\text{0 또는 음수 }V_{ss}}\ \Rightarrow\ \underbrace{\text{Eq.2:366 washout 대안식 사용}}_{\text{§2.8.8}}$$
...
### D. 정상상태 NCA 지표(Steady-State NCA Parameters) — 다중투여 노출 형상

C4의 노출 지표들은 단회 투여 기준이에요. 다중투여 정상상태(SS)에서는 투여 간격당 노출($AUC_{0-\tau,ss}$)과 그 안의 **변동(fluctuation)**·**축적(accumulation)**을 같이 봅니다. 핵심 지표 4가지를 정량 anchor와 함께 못 박고 갑니다.

**1) 투여 간격당 SS AUC** — 선형 PK의 등가성 [G&W p.152]

$$
\underbrace{AUC_{0-\tau,ss}}_{\text{투여간격당 SS 노출}}
=\int_0^\tau\!C_{ss}(t)\,dt
\;\overset{\text{선형 PK}}{=}\;
\underbrace{AUC_{0-\infty}^{\text{단회}}}_{\text{단회 총노출}}
$$

이 등식은 **선형 PK 한정**이에요. 잔류 면적과 다음 투여 외삽 면적의 상쇄(superposition)로 성립합니다 — PK41처럼 비선형이면 $AUC_{0-\tau,ss}\neq AUC_{0-\infty}^{\text{단회}}$. 그래서 **두 AUC 일치 여부가 선형성의 sanity check**입니다. [G&W p.152]

**2) 평균 SS 농도** — 질량평형 정의

$$
\underbrace{C_{av,ss}}_{\text{평균농도}}
=\overbrace{\frac{AUC_{0-\tau,ss}}{\tau}}^{\text{투여간격 평균}}
=\overbrace{\frac{F\cdot Dose}{CL\cdot\tau}}^{\text{질량평형}}
$$

**3) 변동지수(Peak-Trough Fluctuation, PTF%)** [R&T p.340]

$$
\underbrace{PTF\%}_{\text{변동지수}}
=\overbrace{\frac{C_{max,ss}-C_{min,ss}}{C_{av,ss}}}^{\text{진폭/평균}}\times 100
$$

**4) 축적비(Accumulation Index, $R_{ac}$)** [R&T p.325, Eq.11-10]

$$
\underbrace{R_{ac}}_{\text{축적비}}
=\overbrace{\frac{AUC_{0-\tau,ss}}{AUC_{0-\tau}^{\text{단회}}}}^{\text{관측 정의}}
=\underbrace{\frac{1}{1-e^{-\lambda_z\tau}}}_{\text{선형 PK·1구획 한정}}
$$

같은 일일 용량에서 분할 간격($\tau$)을 바꾸면 어떻게 달라지는지를 한 표에 모아두면:

| 같은 일일 용량, 다른 분할 | $C_{av,ss}$ | PTF% | $R_{ac}$ | 임상 함의 |
|---|---|---|---|---|
| q24h (1회/일) | 동일 | **크다** | 비교적 작음 | peak-driven 독성↑, trough-driven 효능 불안정 |
| q12h (2회/일) | 동일 | 중간 | 중간 | 일반적 균형점 |
| q8h (3회/일) | 동일 | **작다** | 거의 1 ($\tau\ll t_{1/2}$일 때) | 평탄 SS, $C_{max}$ 노출↓ |

💡 **PTF가 핵심이에요.** 같은 $C_{av,ss}$/AUC에서도 분할 간격이 다르면 PTF가 다르고, **peak-driven endpoint**(독성 또는 receptor saturation)에서는 반응이 완전히 달라집니다. 같은 일일 용량인데도 daptomycin 같은 약물에서 dose fractionation 차이로 근육독성이 달라지는 현상의 정량적 근거가 바로 PTF예요.

🔑 **SS NCA 보고 규칙:** $AUC_{0-\tau,ss}$만 적지 말고 **$C_{max,ss}$, $C_{min,ss}$, PTF%, $R_{ac}$**까지 같이 보고합니다. 단회/SS AUC 일치 여부가 곧 **선형성 sanity check**입니다. [G&W p.152; R&T p.325, Eq.11-10; R&T p.340]

> 💼 **실무 인사이트:** PopPK에서 SS NCA 지표가 잘못 보고되는 가장 흔한 자리는 **trough 단독 보고**예요. trough(=$C_{min,ss}$)만 적으면 같은 trough에서도 PTF가 다른 두 regimen이 같은 노출처럼 보입니다. Vancomycin의 trough-only TDM이 AUC/MIC 기반으로 옮겨간 배경이 정확히 이거예요 — **노출 형상 정보가 trough 하나에 압축되지 않는다**는 게 SS NCA 보고의 본질입니다.

### E. BA/BE 통계 표준(BA/BE Statistical Framework)

NCA 출력이 BE 판정의 **직접 입력**이라서, $C_{max}$와 $AUC$ 계산의 audit trail이 그대로 통계적 검정력에 들어갑니다. 정형 절차는 다음과 같아요. [FDA Bioequivalence Studies Guidance 2014; EMA Guideline on the Investigation of Bioequivalence 2010]

| 단계 | 내용 |
|---|---|
| 1. log 변환 | $C_{max}$, $AUC_{0-t}$, $AUC_{0-\infty}$를 자연로그 변환 |
| 2. GMR 계산 | $GMR=\exp(\bar{Y}_T-\bar{Y}_R)$ (test/reference **기하평균비**) |
| 3. 90% CI 산출 | crossover ANOVA 잔차 분산 기반 90% 신뢰구간 |
| 4. Acceptance | **0.80–1.25 범위 안에 90% CI가 들어가야 BE 인정** |

$$
\overbrace{H_0:\ GMR\notin[0.80,\ 1.25]}^{\text{불일치 가설(귀무)}}\ \ \text{vs}\ \ \overbrace{H_1:\ GMR\in[0.80,\ 1.25]}^{\text{동등 가설(대립)}}
$$

$$
\underbrace{\text{BE 인정}}_{\text{판정}}\iff
\underbrace{[\text{CI}_{90\%}^{\text{lower}},\ \text{CI}_{90\%}^{\text{upper}}]}_{\text{GMR의 90\% CI}}
\subseteq\underbrace{[0.80,\ 1.25]}_{\text{acceptance window}}
$$

**HVD (Highly Variable Drugs, intra-subject CV ≥ 30%)** — reference-scaled average BE (RSABE) framework로 acceptance 한계를 부분적으로 확장합니다. CV가 클수록 한계가 넓어지지만, **GMR 점추정 자체는 0.80–1.25 범위 안에 있어야 한다는 "point estimate constraint"는 유지**돼요. [FDA RSABE 2014]

> 💼 **실무 인사이트:** BE 통계가 흔들리는 가장 흔한 자리는 **NCA의 외삽 분율, BLQ 처리, $\lambda_z$ 선택**이에요. 같은 raw data에서도 $\lambda_z$ window를 다르게 잡으면 $AUC_{0-\infty}$가 5–10% 흔들리고, 그게 GMR을 0.05 흔들면 90% CI 한쪽 끝이 0.80 또는 1.25 경계를 넘기 직전까지 갑니다. **BE 보고서의 통계 결론은 NCA의 audit trail 위에 얹혀 있다**는 점을 잊으면 안 됩니다 — C1의 처리 기록이 BE 판정의 1차 결정 요인이에요.
...
📖 **R&T p.325, Fig 11-3 + R&T p.340, Fig 11-14:** 같은 평균 일일 용량을 점점 더 자주 분할하면 평균 농도는 같은 채로 PTF만 줄어드는 모습(Fig 11-3), 그리고 SS AUC와 단회 AUC를 직접 비교해 축적비를 검증하는 도식(Fig 11-14)이 SS 노출 형상의 정량 anchor를 잡아줍니다.

이 카드 핵심내용을 한 줄로 — **dose가 같아도 response가 같다는 보장은 없습니다.** route·first-pass·비선형 소실·단백결합·활성 대사체 — 이 다섯 가지가 dose와 effective exposure 사이에 끼어들어요. exposure-response를 정당화하려면 endpoint가 peak-driven인지, 누적인지, threshold인지, unbound인지, **multiple-dose에서는 PTF/$R_{ac}$까지** 정해야 합니다.
...
🔁 **실무 보완(Practical Caveat):** 한 가지 못 박고 갑니다 — 정작 임상 부하용량 산정은 보통 **중앙구획 부피 $V_c$ (또는 $V_1$) 기준**이지 $V_{ss}$가 아니에요. 다구획 약물(gentamicin, amikacin 등)에서 $V_{ss}$로 LD를 계산하면 약물이 아직 말초에 분포되기 전 시점의 **중앙구획 농도가 표적 peak를 크게 초과**할 수 있고, 이는 이독성·신독성 위험으로 직결됩니다. "$V_z>V_{ss}$이므로 $V_z$로 LD 계산은 과대"라는 방향성 자체는 맞지만, "그러면 $V_{ss}$로 LD를 계산하면 되는가?"라는 결론으로 비약하면 안 돼요 — **1구획 근사가 합리적일 때만 $V_{ss}$ 기반 LD가 통합니다.** 2구획 이상 약물에서는 표준 framework이 다릅니다:

$$
\underbrace{LD_{\text{다구획}}}_{\text{부하용량}}
=\underbrace{V_c\cdot C_{\text{target}}}_{\text{중앙구획 기준}}
\qquad\text{vs}\qquad
\underbrace{MD_{\text{유지}}}_{\text{유지용량}}
=\underbrace{CL\cdot C_{av,ss}\cdot\tau/F}_{V_{ss}\text{에 의존하지 않음}}
$$

즉 $V_c$는 LD를 결정하고, $V_{ss}$는 분포의 총 규모를 기술하며, $V_z$는 말단 부피의 진단 지표예요. 세 부피의 역할이 다 다릅니다. 파일 05 M3의 gentamicin 사례($V_1/V_{ss}/V_\beta$ 6배 차이)와 정합되는 framework예요. [협역 치료역 약물의 임상 표준; 파일 05 M3와 정합]
...
**§5 요약(Recap)**: 이 세션의 혼동쌍은 모두 **"같아 보이는 요약 숫자"** 문제예요. AUC/AUMC, MRT/반감기, $V_{ss}/V_z$는 출력 표에서 가까이 붙어 있지만 **서로 다른 질문에 답합니다.** 같은 표 안의 값이라도 "무엇을 묻는 숫자인가"를 바꾸면 해석 대상이 완전히 달라집니다. 그리고 **부하용량 산정처럼 임상 의사결정에 들어가는 자리**에서는 $V_c$/$V_{ss}$/$V_z$를 모두 분리해서 보는 게 표준이에요.
...
**정답:** 표준식과 §2.8.8 대안식을 **둘 다 계산해서 차이를 같이 제시**하고, 차이의 원인이 $T_{inf}/t_{1/2}$ 비율에 있다는 걸 설명합니다 — **방향은 "긴 infusion일수록 표준식이 무너진다"** 쪽이에요. SOP deviation은 "더 예쁜 결과"가 아니라 **0이나 음수 $V_{ss}$를 피하는 출처 기반 수학적 정당화**로 문서화해요 — 보고서 안에 두 식의 출력 차이, $T_{inf}/t_{1/2}$ 비율, Eq.2:337 적용 영역 이탈 근거, 대안식 채택 사유를 한 박스에 넣어두면 감독관도 그대로 따라갈 수 있습니다. [G&W pp.151, 157]

## Q10. BLQ(LOQ 미만) 값을 NCA에서 어떻게 처리해야 하나요? 위치별 규칙과 합리화를 설명해 보세요.

**정답:** G&W p.153은 일반 원칙으로 "BLQ는 결측 처리 — 0이나 LOQ 대입 금지, 특히 지수모델에서"를 권고합니다. 실무 규제 가이던스(FDA BMV 2018, EMA BMV 2011) 기준 **위치별 규칙**은:
① **Pre-first quantifiable BLQ**(첫 정량 시점 이전) → **0 처리** — 흡수 전 시점은 농도 0이 생리학적으로 합리적 가정.
② **Embedded BLQ**(두 정량 사이) → **missing** 또는 **linear interpolation** — 0 처리 시 AUC 과소, LOQ 대입 시 과대추정.
③ **Terminal BLQ**(말단 정량 이후) → **missing**, $\lambda_z$ 계산에서 제외 — 0 처리 시 $\lambda_z$가 인위적으로 가팔라지면서 $AUC_{tail}$·$V_z$·$V_{ss}$가 동시 편향.

Phase 1 NCA에서 가장 흔한 실수는 terminal BLQ=0 자동 처리이고, 이는 $t_{1/2}$ 과소·$AUC_{tail}$ 과소·$V_{ss}$ 과소의 연쇄 편향을 일으킵니다. [G&W p.153]

## Q11. 같은 일일 용량을 q24h와 q8h로 나눠 줄 때 $C_{av,ss}$와 $AUC_{0-\tau,ss}$는 같은데 PTF가 다른 이유는 무엇이고, 어떤 endpoint에서 이 차이가 중요해지나요?

**정답:** $C_{av,ss}=F\cdot Dose/(CL\cdot\tau)$로 일일 용량과 CL이 같으면 $C_{av,ss}$는 같습니다. 그러나 $PTF\%=(C_{max,ss}-C_{min,ss})/C_{av,ss}\times 100$은 투여 간격 $\tau$에 강하게 의존해서, q24h는 PTF가 크고(peak↑/trough↓), q8h는 PTF가 작아 평탄해집니다. **Peak-driven 독성**(예: daptomycin 근육독성에서 같은 일일 용량의 분할 차이), **peak-driven 효능**(aminoglycoside $C_{max}/MIC$), **trough-driven 효능**(β-lactam time-above-MIC) endpoint에서 이 차이가 임상 결정에 직결돼요. [R&T p.340]

## Q12. BE 판정의 acceptance criterion(GMR 90% CI 0.80–1.25)이 NCA 출력에 어떻게 의존하나요? NCA 단계에서 흔드는 자리 3곳을 짚어 보세요.

**정답:** BE는 log-transformed $C_{max}$/$AUC_{0-t}$/$AUC_{0-\infty}$의 **test/reference 기하평균비(GMR)의 90% 신뢰구간이 0.80–1.25 안에 들어가야** 인정됩니다. NCA 단계에서 흔드는 자리는:
① **외삽 분율** — $\lambda_z$ window가 흔들리면 $AUC_{0-\infty}$가 5–10% 흔들리고 GMR에 직접 전파.
② **BLQ 처리** — terminal BLQ에 0 vs missing 처리 차이가 $\lambda_z$를 바꿈.
③ **$\lambda_z$ 선택 자체** — 분포기 점이 섞이면 $t_{1/2}$·$V_z$·$AUC_{tail}$ 모두 동반 왜곡.

HVD(intra-subject CV ≥ 30%)는 reference-scaled framework로 한계가 부분 확장되지만 GMR 점추정의 0.80–1.25 constraint는 유지됩니다. [FDA BE Guidance 2014; EMA BE Guideline 2010]

## Q13. 부하용량(loading dose) 산정에서 $V_{ss}$를 그대로 쓰면 왜 위험한가요? 어떤 부피를 써야 하나요?

**정답:** $V_{ss}$는 분포 평형 도달 **후**의 평균 분포 부피라서, 다구획 약물에서 LD를 $V_{ss}\cdot C_{target}$으로 계산하면 분포 평형 도달 **이전** 시점의 **중앙구획 농도가 표적 peak를 크게 초과**합니다 — 약이 중앙구획에 갇혀 있는 짧은 시간 동안의 농도 spike가 문제예요. 다구획 약물에서는 **LD = $V_c\cdot C_{target}$**(중앙구획 부피 $V_c$ 또는 $V_1$ 기준)으로 계산하고, $V_{ss}$는 분포의 총 규모를 기술하는 지표로, maintenance dose의 일부 계산에 쓰입니다. Gentamicin·amikacin 같은 협역 치료역 약물에서 이 구분이 무너지면 초기 spike가 이독성·신독성을 일으킵니다. "$V_z$로 LD 계산하지 말라"는 원칙은 맞지만, "그러면 $V_{ss}$로 계산"이라는 단순화는 1구획 근사가 합리적일 때만 통합니다. [파일 05 M3와 정합; 협역 치료역 약물 임상 표준]

**§7 요약(Recap)**: Q1–Q3은 AUC 신뢰도, Q4는 비선형 전환, Q5–Q6은 NCA 계산, Q7–Q8은 노출 해석, Q9는 C3 Apex의 적용 경계, **Q10은 BLQ 처리 규칙, Q11은 SS 노출 형상(PTF), Q12는 BE 통계 의존성, Q13은 다구획 LD 산정**이에요.
...
| **BLQ 처리 규칙 위반** | 위치별 BLQ 처리, $\lambda_z$ | → | terminal BLQ=0 시 $\lambda_z$·꼬리·$V_z$·$V_{ss}$ 동반 편향, BE GMR까지 전파 | pre-first BLQ=0, intervening BLQ=missing/보간, terminal BLQ=missing [G&W p.153] |
| **SS NCA 단일 metric 보고** | $AUC_{0-\tau,ss}$ 또는 trough 단독 | → | 같은 일일 용량 다른 분할의 노출 형상 차이 미검출 | $C_{max,ss}$, $C_{min,ss}$, PTF%, $R_{ac}$ 동시 보고 [R&T p.325, 340] |
| **긴 infusion의 $V_{ss}$ 표준식 적용** | $T_{inf}/t_{1/2}\gg 1$에서 Eq.2:337 사용 | → | 0 또는 음수 $V_{ss}$, $V_{ss}$ 해석 실패 | Eq.2:366 washout 대안식과 함께 보고 [G&W pp.157–158] |
...
| **BA/BE 보고서 검토** | $AUC$/$C_{max}$ GMR, 90% CI, intra-subject CV | → | BE 판정 통계 결론의 강건성 결정 | NCA audit trail($\lambda_z$/BLQ/외삽률)이 BE 통계 검정력의 1차 결정 요인 [FDA BE 2014; EMA BE 2010] |
| **다중투여 노출 형상** | $C_{max,ss}$, $C_{min,ss}$, PTF%, $R_{ac}$ | → | 같은 일일 용량 다른 분할 regimen 비교 | $C_{av,ss}$/AUC만으로는 peak/trough-driven endpoint 정당화 불가 [R&T pp.325, 340] |
...
| BA/BE·clinical pharmacology 보고서 | $\lambda_z$, %extrapolated, BLQ 처리, exposure metric audit trail | 숫자표만 있고 가정 방어가 없는 보고서, GMR/CI 재현 불가 |
| Exposure-response 분석 | endpoint별 $C_{max}$/AUC/$t_d$/$C_u$ 선택 | dose-only 해석으로 PD/safety 논증 실패 |
| **다중투여 regimen 설계** | $AUC_{0-\tau,ss}$, $C_{max,ss}$, PTF%, $R_{ac}$ | 같은 일일 용량 다른 분할 비교 불가, peak/trough-driven endpoint 정당화 실패 |
| **Model validation chain (CWRES / NPDE / Bayesian MAP)** | NCA-derived 노출이 individual prediction의 sanity 기준 | NCA-PopPK 비교 없이 validation 시 systematic bias 감지 어려움 |

## E. 최종 압축 원칙(Final Compressed Doctrine)

NCA의 본질은 **"모델을 쓰지 않는다"가 아니라 "최소한의 선형 소실 가정으로 면적과 시간 모멘트를 요약한다"**예요. AUC는 노출의 출발점, AUMC는 체류시간의 출발점, MRT는 $V_{ss}$의 출발점입니다. **PK41은 이 모든 것이 비선형 동력학 앞에서 초기 추정값으로 격하되는 순간**을 보여줍니다. 그리고 **NCA 표는 단회 분석에서 끝나지 않습니다** — SS NCA의 PTF/$R_{ac}$, BE 통계의 GMR/CI, BLQ 위치별 처리는 모두 같은 NCA 골격 위에서 다른 질문에 답하는 도구예요.

**§8 요약(Recap)**: 30년 pharmacometrics 연구자가 10분 핸드아웃에 남길 메시지는 하나예요 — NCA 출력 표를 믿기 전에 **말단 기울기 → BLQ 위치별 처리 → 외삽 분율 → 투여 경로 보정 → $T_{inf}/t_{1/2}$ 비율 진단 → 용량 의존성 → SS 노출 형상(PTF/$R_{ac}$) → 노출 지표 선택 → BE 통계 연결**을 순서대로 점검합니다.

> 💼 **실무 인사이트:** 완성된 NCA 보고서는 **숫자 표가 아니라 가정 점검 기록**이어야 해요. 독자가 말단 기울기, BLQ 처리, 꼬리 분율, 투여 경로 보정, $T_{inf}/t_{1/2}$ 비율, 용량 의존성, SS 노출 형상, 노출 지표 선택, BE 통계 연결을 따라갈 수 있으면, 이후 PopPK 모델링과 BE 보고서의 출발점으로도 방어 가능해집니다. 그리고 NCA 보고서의 1차 파라미터 자리에는 $AUC$가 아니라 **$CL$ 또는 $CL/F$**를 두고 $AUC$는 노출 지표로 보고하는 편이, **생리학적 해석성과 공변량 회귀 가능성을 동시에 보존**합니다 — 보고 순서가 바뀌면 이후 PopPK에서 신·간 기능 공변량 신호가 약하게 나타날 수 있어요.

<!--
# 출력 후 자가검증 체크리스트 (v4.0 보강본)
...
| §5 혼동 쌍 표 행 순서 고정 (3행) | §5 전체 | ✓ |
| 기호 일관성 | 첫 등장 기준 통일 | ✓ |
| 메타-언어 헤더 제거 | 전체 | ✓ |
| 수치 anchor 보존 | Turbojoint PK41 등 원본 수치 | ✓ |
| Plausible Fallacy 산문 통합 | Apex 카드 전체 | ✓ |
| 약물 3-step 즉맥락 | digoxin, aminoglycoside | ✓ |
| BLQ 위치별 처리 규칙 (보강) | C1 §E, §7 Q10, §8 B | ✓ |
| 부분 AUC (보강) | C1 §F | ✓ |
| SS NCA 지표 (PTF, R_ac) (보강) | C4 §D, §7 Q11 | ✓ |
| BA/BE 통계 standard (보강) | C4 §E, §7 Q12 | ✓ |
| V_ss 표준식 방향 명확화 (보강) | C3 §C 방향 주의, §7 Q9 | ✓ |
| V_c 기준 LD 보강 | §5 쌍 3 실무 보완, §7 Q13 | ✓ |
| Model validation chain 후속 연결 | §8 D | ✓ |
| overbrace/underbrace 수식 표기 | 신규 수식 전체 | ✓ |
| G&W·R&T 출처 정확 인용 | 신규 anchor 전체 | ✓ |
-->
<!-- v4.0 → v4.0 보강본 변환 노트
- 원문 보존: 모든 기존 §, 카드, 표, 수식, 콜아웃, 출처 anchor는 **무수정으로 보존**
- 보강 위치:
  * C1 §E (BLQ 위치별 처리 규칙) — 신규 추가, [G&W p.153] anchor 확장, FDA BMV 2018 / EMA BMV 2011 보강
  * C1 §F (부분 AUC, Modified Release) — 신규 추가, [EMA BE Guideline 2010] anchor
  * C3 §C (방향 주의 박스) — "긴 infusion → 표준식 위험" 방향 명시, 기존 텍스트 무수정
  * C4 §D (SS NCA 지표: AUC_τ,ss, C_av,ss, PTF, R_ac) — 신규 추가, [G&W p.152; R&T p.325 Eq.11-10; R&T p.340] anchor
  * C4 §E (BA/BE 통계 standard: GMR, 90% CI, 0.80-1.25, RSABE) — 신규 추가, [FDA BE 2014; EMA BE 2010] anchor
  * §5 쌍 3 "실무 보완 (V_c 기준 LD)" — 신규 추가, 기존 Critical Blow와 정합, 파일 05 M3 cross-reference
  * §7 Q10-Q13 — 신규 추가 (BLQ / PTF / BE / LD with V_c)
  * §8 B (실패 모드 표): BLQ 위반, SS 단일 metric 보고, 긴 infusion V_ss 표준식 — 3 행 추가
  * §8 C (Professional Moat): BA/BE 보고서 검토, 다중투여 노출 형상 — 2 행 추가
  * §8 D (후속 세션): 다중투여 regimen 설계, Model validation chain (CWRES/NPDE/Bayesian MAP) — 2 행 추가
  * §8 E (최종 압축 원칙): SS NCA / BE / BLQ 한 줄 통합 추가
- 수식 표기: 신규 수식 전체에 \overbrace / \underbrace annotation 적용
- 출처 표기: G&W (Gabrielsson & Weiner 5e Ch.2) + R&T (Rowland & Tozer 5e Appendix A·H, Ch.11) 인라인 page 인용 형식 통일
- 보존 감사 (보강본):
  * SLIDE_START 마커: 9개 (§1, §2, C1-C4, §5, §7, §8) — 무변화
  * 기존 수식 블록($$): 무수정으로 보존
  * 신규 수식 블록($$): 13개 추가 (BLQ 처리, 부분 AUC, 방향 주의, SS 4식, BE 2식, LD/MD, 통합)
  * 기존 출처 anchor: 무변경 보존
  * 신규 출처 anchor: [G&W p.153], [G&W p.152], [R&T p.325 Eq.11-10], [R&T p.340], [EMA BE Guideline 2010], [FDA BE Guidance 2014], [FDA RSABE 2014], [FDA/EMA BMV]
  * 수치·약물명: 모든 기존 anchor (PK41, Compound A, Table A-1/A-2/H-1, digoxin/aminoglycoside) 보존
-->

---

<!-- 고유번호 (Trace ID): C-260516-114512-K7Q -->
