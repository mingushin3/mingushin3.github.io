# 08_Content Lock v1 — 비선형 PK · MM · DDI 기전

**산출 단계:** Phase 4A — Adjudication + Compression + Content Lock v1  
...
**Figure note:** Audit T6 Figure Inventory는 본 4A adjudication 대상에서 제외하고 Phase 4C로 이관한다. 본 문서에는 FIGURE marker를 삽입하지 않는다.
...
<!-- MASTER LENS -->
이번 session의 한 문장 핵심은 다음이다. **비선형 PK는 “선형 superposition이 깨지는 현상”이 아니라, capacity·time·flow·binding·interaction 중 어느 물리적 병목이 농도, 시간, route, 환자 genotype과 결합했는지를 식별하는 문제다.** Gabrielsson은 이 문제를 mechanism과 ODE로 보여주고, Rowland & Tozer Ch.16은 phenytoin·alcohol·ascorbic acid 같은 임상 dose-response 비대칭으로 보여주며, Ch.17은 그 비대칭이 drug interaction의 AUC ratio, induction time course, PM risk, route effect로 확장되는 방식을 보여준다. [G pp.112–114], [T16 pp.491–492], [T17 pp.531–532]

<!-- ANCHOR -->
읽는 순서는 진단 → capacity → time → binding → DDI → PD interaction이다. 먼저 dose-normalized plot으로 “선형이 아닌지”를 확인하고, 그 다음 $V_{max}/K_m$, enzyme turnover, $f_u$, $f_m$, route/extraction을 순서대로 점검한다. 이 순서를 어기면 nonlinear model을 fitting할 수는 있어도, 왜 parameter가 흔들리는지는 설명하지 못한다. [G p.113], [T16 p.492], [T17 pp.532–533]
...
<!-- MASTER LENS -->
**Big Idea:** 비선형성의 첫 진단은 “모델을 무엇으로 짤까?”가 아니라, 같은 route·formulation·method에서 dose-normalized concentration-time profile, AUC vs Dose, AUC/Dose vs Dose가 superimpose 되는지 확인하는 것이다. 선형이면 dose-normalized 관측값이 겹치고, 비선형이면 clearance, bioavailability, distribution, binding 중 적어도 하나가 dose 또는 time의 함수가 된다. [G pp.112–114], [T16 p.492]
...
<!-- TRENCH -->
**Trench tip:** nonlinear PK 검토에서 첫 그림은 spaghetti concentration-time plot이 아니라 **dose-normalized concentration-time plot + AUC/Dose vs Dose**다. 이 두 그림 없이 바로 $V_{max}$/$K_m$를 fitting하면, capacity limitation인지 F 변화인지, time-dependence인지, binding artifact인지 구분하지 못한다. [G p.113]
...
<!-- MASTER LENS -->
**Big Idea:** Michaelis–Menten capacity limitation은 clearance가 상수가 아니라 concentration의 함수가 되는 상황이다. Gabrielsson의 MM clearance는 Eq.2:266–2:274 영역에 속하며, Eq.2:264는 linear superposition 식이므로 MM ODE의 citation으로 쓰지 않는다. [G pp.114–119]
...
이 식의 핵심은 $R_0$가 $V_m$에 가까워질수록 denominator가 작아지고, $C_{u,ss}$가 급격히 상승한다는 점이다. $K_m$은 unbound concentration 기준이고, total concentration으로 작업할 때의 $K_m'$와 구분해야 한다. [T16 pp.503–506]
...
<!-- TRENCH -->
**Trench tip:** $V_{max}$와 $K_m$를 따로 추정하려면 농도 범위가 curvature를 지나야 하며, 특히 $K_m$ 주변 또는 그 이하의 관측값이 필요하다. high-concentration plateau만 있으면 $V_{max}$와 $K_m$가 ridge처럼 같이 움직인다. [G p.117]
...
<!-- ANCHOR -->
C2는 Stage 3 DDI의 수학적 원형이다. $K_m+C$의 분모 구조를 이해하지 못하면, inhibitor가 등장해 $1+C_{u,I}/K_I$ factor를 만드는 Eq.17-9–17-11도 새 식처럼 외우게 된다. [G pp.115–118], [T17 pp.550–552]
...
<!-- MASTER LENS -->
**Big Idea:** Time-dependent kinetics는 concentration이 달라져서 clearance가 달라 보이는 현상이 아니라, enzyme/transporter/physiology 자체가 시간에 따라 변하는 현상이다. 따라서 약물 concentration time course와 enzyme turnover time course라는 두 개의 시계를 분리해서 읽어야 한다. [G p.112], [G pp.119–129], [T16 pp.516–519]
...
MBI는 reversible inhibition과 달리 enzyme을 기능적으로 제거하고, 회복은 inhibitor concentration이 사라지는 속도만이 아니라 enzyme resynthesis/degradation clock에 의해 제한된다. Clarithromycin example에서 $k_{inact}$, $k_E$, $C_{u,I}/K_I$가 결합해 reversible inhibition보다 훨씬 큰 effect를 만들 수 있으며, CYP3A4 회복에는 2주 또는 그 이상이 필요할 수 있다고 설명된다. [T16 p.519], [T17 pp.557–558]

<!-- ANCHOR -->
Eq.17-13/17-14의 MBI term은 C3 turnover ODE의 정상상태 사고방식을 DDI prediction에 붙인 것이다. Reversible inhibition이 “분모에 inhibitor competition을 더하는 문제”라면, MBI는 “enzyme pool 자체를 줄인 뒤 회복 clock까지 계산하는 문제”다. [T17 p.558]
...
<!-- MASTER LENS -->
**Big Idea:** Nonlinear binding은 total concentration과 unbound concentration의 관계를 비선형화한다. 특히 open in vivo system과 closed in vitro binding experiment를 혼동하면, protein binding displacement를 실제 unbound exposure 변화로 과대해석하게 된다. [G pp.129–134], [T17 pp.538–544]
...
<!-- TRENCH -->
**Trench tip:** Protein binding nonlinearity가 의심되는 Phase 1/PopPK package에서는 total concentration만 모으지 말고 dose level별 $f_u$를 측정하라. total-only dataset은 binding nonlinearity와 clearance nonlinearity를 aliasing한다. [G pp.129–134], [T16 pp.511–516]
...
<!-- MASTER LENS -->
**Big Idea:** Ch.17의 DDI 정량 framework는 “상호작용이 있다/없다”가 아니라, affected drug의 clearance 중 얼마가 inhibited pathway에 의존하는지($f_m$), inhibitor exposure가 $K_I$에 비해 얼마나 큰지, route/first-pass가 얼마나 관여하는지를 묻는다. [T17 pp.531–532], [T17 pp.546–552]

### Reversible competitive inhibition spine

Eq.17-9–17-12는 inhibited pathway의 clearance 감소와 whole-body AUC ratio를 연결한다. 핵심 직관은 다음이다. [T17 pp.550–552]
...
Interaction은 all-or-none이 아니라 concentration, timing, sequence, initiation/withdrawal에 따라 graded하게 나타난다. Inhibitor를 추가할 때와 제거할 때의 위험은 대칭이 아니며, induction/MBI에서는 회복 지연이 critical하다. [T17 pp.532–534], [T17 pp.557–561]
...
<!-- MASTER LENS -->
**Big Idea:** 실제 DDI는 metabolic inhibition 하나로 끝나지 않는다. Transporter inhibition, altered distribution, gut/hepatic first-pass, induction, PD additivity/synergy/antagonism이 겹칠 수 있으며, 같은 drug라도 acute/chronic, oral/IV 조건에서 반대 방향 effect를 보일 수 있다. [T17 pp.563–567]
...
Rifampin 같은 perpetrator는 acute transporter inhibition과 chronic enzyme induction이 서로 반대 방향 effect를 만들 수 있다. 따라서 실무적으로는 acute IV, acute oral, chronic IV, chronic oral을 구분해 mechanism을 생각해야 한다. 이 문장은 label language가 아니라 교과서 사례에서 도출한 diagnostic framework다. [T17 pp.560–565]
...
<!-- TRENCH -->
**Trench tip:** DDI case를 검토할 때 “AUC가 증가했다”로 끝내지 말고, 함께 움직인 Cmax, t½, V/F, oral/IV ratio를 보라. AUC↑와 t½↓가 같이 나오면 단순 metabolic inhibition이 아니라 distribution/transporter mechanism을 의심해야 한다. [T17 pp.563–565]
...
**Answer:** 선형이면 dose-normalized 관측값이 superimpose된다. 겹치지 않으면 CL, F, V/distribution, binding, time-dependent process 중 하나가 dose/time의 함수라는 신호다. [G pp.112–114], [T16 p.492]
...
<!-- MASTER LENS -->
Session 08의 기억 모델은 다음 여섯 문장으로 고정한다.
...
<!-- RECAP -->
**Final recap:** 이 Content Lock v1의 최종 spine은 “diagnose nonlinearity → identify capacity/time/binding mechanism → translate to DDI prediction → avoid route/timing/unbound/PM traps”이다. 이 spine만 PhD 1년차가 확실히 체화하면, 비선형 PK와 DDI chapter의 대부분은 암기가 아니라 mechanism triage로 회수된다.
